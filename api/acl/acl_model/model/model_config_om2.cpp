...
aclError CheckMdlLoadPriority(const void *const attrValue, const size_t valueSize) {
  ...
  if (!mdlLoadPriorityFlag) {
    ...
    if (aclErr != ACL_ERROR_NONE) {
      ACL_LOG_CALL_ERROR(
          "[Get][PriorityRange]get range of stream priority failed, "
          "runtime errorCode = %d",
          static_cast<int32_t>(aclErr));
      return aclErr;
    }
    mdlLoadPriorityFlag = true;
    ACL_LOG_INFO(
        "execute aclrtDeviceGetStreamPriorityRange success, greatest load priority = %d, "
        "least load priority = %d",
...
aclError CheckMdlLoadType(const void *const attrValue, const size_t valueSize) {
  ...
  if ((type < static_cast<size_t>(ACL_MDL_LOAD_FROM_FILE)) ||
    ...
    ACL_LOG_ERROR("[Check][Type]type[%zu] is invalid, it should be in [%d, %d]", type, ACL_MDL_LOAD_FROM_FILE,
                  ACL_MDL_LOAD_FROM_MEM_WITH_Q);
    const std::string errMsg = acl::AclErrorLogManager::FormatStr("it should be in [%d, %d]", ACL_MDL_LOAD_FROM_FILE,
                                                                  ACL_MDL_LOAD_FROM_MEM_WITH_Q);
    acl::AclErrorLogManager::ReportInputError(
        acl::INVALID_PARAM_MSG, std::vector<const char *>({"param", "value", "reason"}),
...
aclError CheckMdlLoadReuseZeroCopy(const void *const attrValue, const size_t valueSize) {
  ...
  if ((type > static_cast<size_t>(ACL_WORKSPACE_MEM_OPTIMIZE_INPUTOUTPUT))) {
    ACL_LOG_ERROR("[Check][Type]type[%zu] is invalid, it should be in [%d, %d]", type,
    ...
    const std::string errMsg = acl::AclErrorLogManager::FormatStr(
        "it should be in [%d, %d]", ACL_WORKSPACE_MEM_OPTIMIZE_DEFAULT, ACL_WORKSPACE_MEM_OPTIMIZE_INPUTOUTPUT);
    acl::AclErrorLogManager::ReportInputError(
        acl::INVALID_PARAM_MSG, std::vector<const char *>({"param", "value", "reason"}),
        std::vector<const char *>({"*attrValue", std::to_string(type).c_str(), errMsg.c_str()}));
    return ACL_ERROR_INVALID_PARAM;
...
aclError aclmdlSetConfigOptImplOm2(aclmdlConfigHandle *handle, aclmdlConfigAttr attr, const void *attrValue,
                                   size_t valueSize) {
  ...
  if (g_setMdlConfigMap.find(attr) != g_setMdlConfigMap.end()) {
    ...
    const std::string errMsg =
        acl::AclErrorLogManager::FormatStr("it should be [%d, %d]", static_cast<int32_t>(ACL_MDL_PRIORITY_INT32),
                                           static_cast<int32_t>(ACL_MDL_WEIGHT_PATH_PTR));
    acl::AclErrorLogManager::ReportInputError(
        acl::INVALID_PARAM_MSG, std::vector<const char *>({"param", "value", "reason"}),
  ...
  if (ret != ACL_SUCCESS) {
    ACL_LOG_ERROR(
        "[Check][Params]check params by aclmdlSetConfigOpt error, result[%d], attr[%d], "
        "valueSize[%zu]",
        ret, static_cast<int32_t>(attr), valueSize);
    return ret;
  }
...
aclError aclmdlSetExternalWeightAddressImplOm2(aclmdlConfigHandle *handle, const char *weightFileName, void *devPtr,
                                               size_t size) {
  ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(handle);
...
aclError aclmdlSetExecConfigOptImplOm2(aclmdlExecConfigHandle *handle, aclmdlExecConfigAttr attr, const void *attrValue,
                                       size_t valueSize) {
  ...
  if (g_setMdlExecConfigMap.find(attr) != g_setMdlExecConfigMap.end()) {
    ...
    const std::string errMsg =
        acl::AclErrorLogManager::FormatStr("it should be [%d, %d]", static_cast<int32_t>(ACL_MDL_STREAM_SYNC_TIMEOUT),
  ...
  if (ret != ACL_SUCCESS) {
    ACL_LOG_ERROR(
        "[Check][Params]check params by aclmdlSetExecConfigOpt error, result[%d], attr[%d], "
        "valueSize[%zu]",
        ret, static_cast<int32_t>(attr), valueSize);
    return ret;
  }
