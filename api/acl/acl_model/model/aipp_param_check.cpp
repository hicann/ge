  ...
  if (npuArch == NPUARCH_TO_STR(NpuArch::DAV_1001) || npuArch == NPUARCH_TO_STR(NpuArch::DAV_3002) ||
    ...
    if (flag) {
      ACL_LOG_INNER_ERROR(
          ...
          "RGB888_U8, YUV400_U8, cceInputFormat = %d",
          npuArch.c_str(), static_cast<int32_t>(inputFormat));
      return ACL_ERROR_INVALID_PARAM;
    }
  } else {
...
aclError AippScfSizeCheck(const aclmdlAIPP *const aippParmsSet, const size_t batchIndex) {
  ...
  if (cropSwitch == 1) {
    ...
    if (flag) {
      ACL_LOG_INNER_ERROR(
          "[Check][Params]when enable crop and scf, scfInputSizeW[%d] must be "
    ...
    if (flag) {
      ACL_LOG_INNER_ERROR(
          "[Check][Params]when disable crop and enable scf, scfInputSizeW[%d] "
  ...
  if (flag) {
    ACL_LOG_INNER_ERROR("[Check][Params]resize_input_h[%d] should be within [16, 4096]!", scfInputSizeH);
    return ACL_ERROR_INVALID_PARAM;
  }
  ...
  ge::float32_t scfRatio =
      (static_cast<ge::float32_t>(scfOutputSizeW) * 1.0F) / static_cast<ge::float32_t>(scfInputSizeW);
...
static aclError AippCropSizeCheck(const aclmdlAIPP *const aippParmsSet, const size_t batchIndex) {
  ...
  ACL_CHECK_WITH_INNER_MESSAGE_AND_RETURN((cropStartPosW + cropSizeW) <= srcImageSizeW, ACL_ERROR_INVALID_PARAM,
                                          ...
                                          "larger than srcImageSizeW[%d]",
                                          cropStartPosW, cropSizeW, srcImageSizeW);
  ACL_CHECK_WITH_INNER_MESSAGE_AND_RETURN((cropStartPosH + cropSizeH) <= srcImageSizeH, ACL_ERROR_INVALID_PARAM,
                                          ...
                                          "larger than srcImageSizeH[%d]",
                                          cropStartPosH, cropSizeH, srcImageSizeH);

  const enum CceAippInputFormat inputFormat = static_cast<enum CceAippInputFormat>(aippParmsSet->aippParms.inputFormat);
  if (inputFormat == CCE_YUV420SP_U8) {
    // determine whether it is even
...
ACL_FUNC_VISIBILITY aclError GetAippOutputHW(const aclmdlAIPP *const aippParmsSet, const size_t batchIndex,
  ...
  if (aippParmsSet->aippBatchPara[batchIndex].paddingSwitch == 1) {
    ...
    const bool flag = (npuArch == NPUARCH_TO_STR(NpuArch::DAV_1001)) ||
                      (npuArch == NPUARCH_TO_STR(NpuArch::DAV_2002)) ||
                      (npuArch == NPUARCH_TO_STR(NpuArch::DAV_2201)) || (npuArch == NPUARCH_TO_STR(NpuArch::DAV_3510));
    if (flag) {
      ACL_CHECK_WITH_INNER_MESSAGE_AND_RETURN(
          aippOutputW <= 1080, ACL_ERROR_INVALID_PARAM,
...
static aclError AippDynamicBatchParaCheck(const aclmdlAIPP *const aippParmsSet, const std::string &npuArch) {
  ...
  for (uint64_t i = 0UL; i < batchSize; i++) {
    ...
    if (flag) {
      ACL_LOG_INNER_ERROR(
          "[Check][Params]the %lu batch output size must be equal to the first "
          "batch aipp output size! aippBatchOutputW = %d, aippBatchOutputH = %d, aippFirstOutputW = %d, "
          "aippFirstOutputH = %d.",
          (i + 1UL), aippBatchOutputW, aippBatchOutputH, aippFirstOutputW, aippFirstOutputH);
      return ACL_ERROR_INVALID_PARAM;
    }
  }
...
ACL_FUNC_VISIBILITY aclError AippParamsCheck(const aclmdlAIPP *const aippParmsSet, const std::string &npuArch) {
  ...
  bool flag = false;
  flag = ((inputFormat == CCE_YUV400_U8) || (inputFormat == CCE_RAW10) || (inputFormat == CCE_RAW12) ||
