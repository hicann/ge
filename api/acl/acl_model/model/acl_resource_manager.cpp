...
void AclResourceManager::GetRuntimeV2Env() {
  ...
  ACL_LOG_EVENT("runtime v2 flag : model flag = %d, singleOp flag = %d", static_cast<int32_t>(enableRuntimeV2ForModel_),
...
std::shared_ptr<gert::Allocators> AclResourceManager::GetAllocators(const aclrtStream stream,
                                                                    bool createDefaultAllocator) {
...
std::shared_ptr<gert::Allocators> AclResourceManager::CreateAllocators(
...
std::shared_ptr<gert::Allocators> AclResourceManager::CreateDefaultAllocators(const void *const cacheKey) {
  std::shared_ptr<ge::Allocator> deviceAllocator(gert::AllocatorFactory::Create(gert::kOnDeviceHbm).release());
...
std::shared_ptr<gert::Allocators> AclResourceManager::CreateExternalAllocators(const void *const cacheKey,
...
std::shared_ptr<gert::Allocators> AclResourceManager::UpdateExternalAllocators(aclrtStream stream) {
  ...
  if (!old_desc_exist && new_desc_exist) {
    ExternalAllocatorDesc allocatorDesc =
        ExternalAllocatorDesc(allocator, allocFunc, freeFunc, allocAdviseFunc, getAddrFromBlockFunc);
