...
class ExternalAllocatorDesc {
 public:
  ExternalAllocatorDesc()
      : obj(nullptr), allocFunc(nullptr), freeFunc(nullptr), allocAdviseFunc(nullptr), getAddrFromBlockFunc(nullptr) {}
  ExternalAllocatorDesc(aclrtAllocator allocator, aclrtAllocatorAllocFunc allocFunction,
                        aclrtAllocatorFreeFunc freeFunction, aclrtAllocatorAllocAdviseFunc allocAdviseFunction,
...
class ACL_FUNC_VISIBILITY AclResourceManager {
 ...
 private:
  ...
  std::map<const void *, std::shared_ptr<gert::Allocators>>
      streamDefaultAllocator_;  // cacheKey + default_allocator_handle
