...
const std::unordered_set<aclmdlConfigAttr> kOm2SupportedLoadConfigOpts = {
    ACL_MDL_PRIORITY_INT32,  ACL_MDL_LOAD_TYPE_SIZET,     ACL_MDL_PATH_PTR,
    ACL_MDL_MEM_ADDR_PTR,    ACL_MDL_MEM_SIZET,           ACL_MDL_WEIGHT_ADDR_PTR,
    ACL_MDL_WEIGHT_SIZET,    ACL_MDL_WORKSPACE_ADDR_PTR,  ACL_MDL_WORKSPACE_SIZET,
...
aclError Om2GetModelTensorDesc(const std::vector<ge::Om2TensorDesc> *&inputDesc,
  ...
  if (output->blobs.size() != outputNum) {
    ACL_LOG_ERROR("Output blobs size mismatch. actual_size[%zu], expected_size[%zu]", output->blobs.size(), outputNum);
    return ACL_ERROR_INVALID_PARAM;
  }
  return ACL_SUCCESS;
}
...
aclError ConstructOm2ModelLoadArg(
...
aclError Om2ModelLoadFromFileWithMem(const char_t *const modelPath, uint32_t *const modelId,
  ...
  *modelId = acl::AclResourceManagerOm2::GetInstance().GenerateModelId();
  ...
  std::unique_ptr<gert::Om2ModelExecutor> executor = gert::LoadOm2ExecutorFromData(modelData, loadArgsWithModelId, ret);
  if (ret != ge::SUCCESS) {
...
aclError Om2ModelLoadFromMemWithMem(const void *const model, const size_t modelSize, uint32_t *const modelId,
...
aclError Om2ModelExecuteCommon(uint32_t modelId, const aclmdlDataset *input, aclmdlDataset *output, bool isAsync,
                               aclrtStream stream) {
  ...
  if (needCalcByMaxShapeRange) {
    ...
    for (size_t i = 0U; i < shapeRanges.size(); ++i) {
      if (shapeRanges[i].second <= 0) {
        ACL_LOG_INFO("max shape of shapeRanges[%zu] is [%ld], index[%zu]", i, shapeRanges[i].second, idx);
        return 0U;
      }
...
static aclError GetDimsRange(const aclmdlDesc *const modelDesc, const TensorType tensorType, const size_t idx,
  ...
  const bool isDynamicGear =
      (!modelDesc->dynamicBatch.empty() || !modelDesc->dynamicDims.empty() || !modelDesc->dynamicHW.empty());
  if (isDynamicGear) {
  ...
  if (idx >= descSize) {
    ACL_LOG_INNER_ERROR(
        "[Check][Params]GetDimsRange failed, index[%zu] can not greater than or equal to tensor "
        "size[%zu]",
        idx, descSize);
    return ACL_ERROR_INVALID_PARAM;
  }
  ...
  if (dynamicDimSize == 0U) {
    ...
    if (dynamicDimSize != tensorDesc.dims.size()) {
      ACL_LOG_INNER_ERROR(
          "[Check][dynamicDimSize]GetDimsRange failed, size of shapeRanges[%zu] does not equal "
          "to size of dims[%zu]",
          dynamicDimSize, tensorDesc.dims.size());
      return ACL_ERROR_INTERNAL_ERROR;
    }
...
static const char *aclmdlGetNameByIndex(const std::vector<aclmdlTensorDesc> &desc, const size_t idx) {
  if (idx >= desc.size()) {
    ACL_LOG_ERROR("[Check][index]get name by index failed, index[%zu] is larger than or equal to desc size[%zu]", idx,
                  desc.size());
    const std::string errMsg =
        acl::AclErrorLogManager::FormatStr("cannot larger than or equal to desc size[%zu]", desc.size());
...
static aclFormat aclmdlGetFormat(const std::vector<aclmdlTensorDesc> &desc, const size_t idx) {
  if (idx >= desc.size()) {
    ACL_LOG_INNER_ERROR(
        "[Check][index]get data format by index failed, index[%zu] is larger "
        "than or equal to desc size[%zu]",
        idx, desc.size());
...
static aclDataType aclmdlGetDataType(const std::vector<aclmdlTensorDesc> &desc, const size_t idx) {
  if (idx >= desc.size()) {
    ACL_LOG_INNER_ERROR(
        "[Check][Index]get data type by index failed, index[%zu] is larger than or "
        "equal to desc size[%zu]",
        idx, desc.size());
...
static aclError aclmdlGetIndexByName(const std::vector<aclmdlTensorDesc> &desc, const char_t *const name,
                                     size_t *const idx) {
...
static const char_t *TransTensorNameToReal(const aclmdlDesc *const modelDesc, const std::string &tensorName) {
  ...
  if (valArr[0U] != TENSOR_NAME_PREFIX) {
    ACL_LOG_INNER_ERROR("[Check][Param]cannot find Attr[%s] in tensorName[%s]", TENSOR_NAME_PREFIX, tensorName.c_str());
    return nullptr;
  }
  ...
  if ((valArr[1U] == MODEL_ID_STR) && (acl::StringUtils::IsDigit(valArr[2U]))) {
    ...
    ACL_LOG_INNER_ERROR("[Check][modelId]cannot find attr[%s] or modelId in tensorName[%s]", MODEL_ID_STR,
                        tensorName.c_str());
    return nullptr;
  }
  if (acl::StringUtils::IsDigit(valArr[4U])) {
    ...
    if (valArr[3U] == TENSOR_INPUT_STR) {
      if (idex >= modelDesc->inputDesc.size()) {
        ACL_LOG_INNER_ERROR("[Check][index]inputDesc index[%lu] should be in [0, %zu), tensorName[%s]", idex,
                            modelDesc->inputDesc.size(), tensorName.c_str());
        return nullptr;
      }
...
static aclError LoadFromFile(const aclmdlConfigHandle *handle, const std::vector<ge::FileConstantMem> &fileConstantMems,
  ...
  ACL_REQUIRES_OK(ConstructOm2ModelLoadArg(nullptr, 0U, nullptr, 0U, handle->priority, loadArgs, nullptr,
                                           fileConstantMems, handle->withoutGraph, (handle->reuseZeroCopy != 0U)));
...
static aclError LoadFromFileWithMem(const aclmdlConfigHandle *handle,
  ...
  ACL_REQUIRES_OK(ConstructOm2ModelLoadArg(handle->workPtr, handle->workSize, handle->weightPtr, handle->weightSize,
                                           handle->priority, loadArgs, nullptr, fileConstantMems, handle->withoutGraph,
...
aclTensorDesc *aclmdlGetDatasetTensorDescImplOm2(const aclmdlDataset *dataset, size_t index) {
  ...
  if (index >= dataset->blobs.size()) {
    ACL_LOG_ERROR("[Check][Index]input param index[%zu] must be smaller than output databuf size[%zu]", index,
                  dataset->blobs.size());
    acl::AclErrorLogManager::ReportInputError(acl::INVALID_PARAM_MSG,
                                              std::vector<const char *>({"param", "value", "reason"}),
                                              std::vector<const char *>({"index", std::to_string(index).c_str(),
...
aclError aclmdlSetDatasetTensorDescImplOm2(aclmdlDataset *dataset, aclTensorDesc *tensorDesc, size_t index) {
  ...
  if (index >= dataset->blobs.size()) {
    ACL_LOG_ERROR("[Check][Index]input param index[%zu] must be smaller than input databuf size[%zu]", index,
                  dataset->blobs.size());
    acl::AclErrorLogManager::ReportInputError(
        acl::INVALID_PARAM_MSG, std::vector<const char *>({"param", "value", "reason"}),
...
aclError aclmdlLoadFromFileImplOm2(const char *modelPath, uint32_t *modelId) {
  ...
  gert::Om2ModelLoadArg loadArgs;
  ACL_REQUIRES_OK(ConstructOm2ModelLoadArg(nullptr, 0U, nullptr, 0U, 0, loadArgs));
...
aclError aclmdlLoadFromFileWithMemImplOm2(const char *modelPath, uint32_t *modelId, void *workPtr, size_t workSize,
                                          void *weightPtr, size_t weightSize) {
  ACL_PROFILING_REG(acl::AclProfType::AclmdlLoadFromFileWithMem);
  ACL_LOG_INFO("[OM2] start to execute aclmdlLoadFromFileWithMem, workSize[%zu], weightSize[%zu]", workSize,
               weightSize);
  ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(modelPath);
  ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(modelId);

  gert::Om2ModelLoadArg loadArgs;
  ACL_REQUIRES_OK(ConstructOm2ModelLoadArg(workPtr, workSize, weightPtr, weightSize, 0, loadArgs));
...
aclError aclmdlLoadFromMemWithQImplOm2(const void *model, size_t modelSize, uint32_t *modelId, const uint32_t *inputQ,
                                       size_t inputQNum, const uint32_t *outputQ, size_t outputQNum) {
  (void)model;
  (void)modelSize;
  (void)modelId;
...
aclError aclmdlExecuteAsyncImplOm2(uint32_t modelId, const aclmdlDataset *input, aclmdlDataset *output,
                                   aclrtStream stream) {
...
aclError aclmdlQuerySizeImplOm2(const char *fileName, size_t *workSize, size_t *weightSize) {
  ...
  *workSize = work;
  *weightSize = weight;
  ACL_LOG_INFO("[OM2] success to get size from file[%s], work size[%zu] bytes, weight size[%zu] bytes", fileName,
               *workSize, *weightSize);
...
aclError aclmdlSetDynamicBatchSizeImplOm2(uint32_t modelId, aclmdlDataset *dataset, size_t index, uint64_t batchSize) {
  ACL_LOG_INFO("[OM2] start to execute aclmdlSetDynamicBatchSize, modelId[%u], index[%zu], batchSize[%lu]", modelId,
               index, batchSize);
  ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(dataset);
  if (batchSize == 0U) {
  ...
  const ge::Status ret = executor->SetDynamicSize({batchSize}, static_cast<int32_t>(ge::DYNAMIC_BATCH));
  if (ret != ge::SUCCESS) {
    ACL_LOG_CALL_ERROR("[Set][DynamicSize] set dynamic batch size failed, ge result[%u], modelId[%u]", ret, modelId);
    return ACL_GET_ERRCODE_GE(static_cast<int32_t>(ret));
  }
  ...
  if (memSize < batchSizeU32) {
    ACL_LOG_INNER_ERROR(
        "[Check][MemSize] device buffer size[%lu] is less than required[%lu] for "
        "batch size, modelId[%u]",
        static_cast<unsigned long>(memSize), static_cast<unsigned long>(batchSizeU32), modelId);
  ...
  const aclError aclRet = aclrtMemcpy(devPtr, memSize, &batchSize, elementSize, ACL_MEMCPY_HOST_TO_DEVICE);
  if (aclRet != ACL_SUCCESS) {
    ACL_LOG_CALL_ERROR("[Memcpy] write batchSize to device failed, acl result[%d], modelId[%u]", aclRet, modelId);
    return aclRet;
  }
  ...
  ACL_LOG_INFO("[OM2] successfully execute aclmdlSetDynamicBatchSize, modelId[%u], batchSize[%lu]", modelId, batchSize);
  return ACL_SUCCESS;
}

aclError aclmdlSetDynamicHWSizeImplOm2(uint32_t modelId, aclmdlDataset *dataset, size_t index, uint64_t height,
                                       uint64_t width) {
  ACL_LOG_INFO("[OM2] start to execute aclmdlSetDynamicHWSize, modelId[%u], index[%zu], h[%lu], w[%lu]", modelId, index,
               height, width);
  ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(dataset);
  if ((height == 0U) || (width == 0U)) {
    ACL_LOG_INNER_ERROR("[Check][Params] height[%lu] or width[%lu] is zero, modelId[%u]", height, width, modelId);
    return ACL_ERROR_INVALID_PARAM;
  }
  ...
  const ge::Status ret = executor->SetDynamicSize({height, width}, static_cast<int32_t>(ge::DYNAMIC_IMAGE));
  if (ret != ge::SUCCESS) {
    ACL_LOG_CALL_ERROR("[Set][DynamicSize] set dynamic HW size failed, ge result[%u], modelId[%u]", ret, modelId);
  ...
  if (memSize < hwInputSizeU32) {
    ACL_LOG_INNER_ERROR(
        "[Check][MemSize] device buffer size[%lu] is less than required[%lu] for "
        "HW size, modelId[%u]",
        static_cast<unsigned long>(memSize), static_cast<unsigned long>(hwInputSizeU32), modelId);
    return ACL_ERROR_GE_DYNAMIC_INPUT_LENGTH_INVALID;
  }
  uint64_t elementSize = sizeof(uint32_t);
  ...
  for (size_t i = 0U; i < hwDimNum; ++i) {
    const aclError aclRet = aclrtMemcpy(static_cast<uint8_t *>(devPtr) + (elementSize * i), memSize - (elementSize * i),
                                        &hwBuf[i], elementSize, ACL_MEMCPY_HOST_TO_DEVICE);
    if (aclRet != ACL_SUCCESS) {
      ACL_LOG_CALL_ERROR("[Memcpy] write HW size[%zu] to device failed, acl result[%d], modelId[%u]", i, aclRet,
                         modelId);
      return aclRet;
    }
  }
  ...
  ACL_LOG_INFO("[OM2] successfully execute aclmdlSetDynamicHWSize, modelId[%u], h[%lu], w[%lu]", modelId, height,
               width);
  return ACL_SUCCESS;
}
...
aclError aclmdlSetInputDynamicDimsImplOm2(uint32_t modelId, aclmdlDataset *dataset, size_t index,
  ...
  ACL_LOG_INFO("[OM2] start to execute aclmdlSetInputDynamicDims, modelId[%u], index[%zu]", modelId, index);
  ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(dataset);
  ...
  if (descRet != ge::SUCCESS) {
    ACL_LOG_CALL_ERROR("[Get][UserDesignateShapeOrder] failed, ge result[%u], modelId[%u]", descRet, modelId);
    return ACL_ERROR_INTERNAL_ERROR;
  }
  ...
  if (curDynamicDims.empty()) {
    ACL_LOG_INNER_ERROR(
        "[Check][DynamicDims] no dynamic axis found in model, modelId[%u]. "
        "Static model does not support aclmdlSetInputDynamicDims.",
        modelId);
    return ACL_ERROR_INVALID_PARAM;
  }

  // Step 2: Validate gear against model's dynamic_batch_info (contains only dynamic axis values)
  const ge::Status ret = executor->SetDynamicSize(curDynamicDims, static_cast<int32_t>(ge::DYNAMIC_DIMS));
  if (ret != ge::SUCCESS) {
    ACL_LOG_CALL_ERROR("[Set][DynamicSize] set dynamic dims failed, ge result[%u], modelId[%u]", ret, modelId);
    return ACL_ERROR_GE_DYNAMIC_BATCH_SIZE_INVALID;
  }
  ...
  if (memSize < dynamicInputSizeU32) {
    ACL_LOG_INNER_ERROR(
        ...
        static_cast<unsigned long>(memSize), static_cast<unsigned long>(dynamicInputSizeU32), dynamicDimNum, modelId);
    return ACL_ERROR_GE_DYNAMIC_INPUT_LENGTH_INVALID;
  }
  uint64_t elementSize = sizeof(uint32_t);
  ...
  for (size_t i = 0U; i < dynamicDimNum; ++i) {
    const aclError aclRet = aclrtMemcpy(static_cast<uint8_t *>(devPtr) + (elementSize * i), memSize - (elementSize * i),
                                        &curDynamicDims[i], elementSize, ACL_MEMCPY_HOST_TO_DEVICE);
    if (aclRet != ACL_SUCCESS) {
      ACL_LOG_CALL_ERROR("[Memcpy] write dynamic dim[%zu] to device failed, acl result[%d], modelId[%u]", i, aclRet,
                         modelId);
      return aclRet;
    }
  }
...
aclError aclmdlGetOutputDimsImplOm2(const aclmdlDesc *modelDesc, size_t index, aclmdlIODims *dims) {
  ...
  if (ret != ACL_SUCCESS) {
    ACL_LOG_INNER_ERROR("[Get][Dims]get output dims failed, result[%d], index[%zu], modelId[%u]", ret, index,
                        modelDesc->modelId);
  }
...
aclError aclmdlGetInputDimsV2ImplOm2(const aclmdlDesc *modelDesc, size_t index, aclmdlIODims *dims) {
  ...
  if (ret != ACL_SUCCESS) {
    ACL_LOG_INNER_ERROR("[Get][Dims]get input dims(v2) failed, result[%d], index[%zu], modelId[%u]", ret, index,
                        modelDesc->modelId);
  }
...
aclError aclmdlGetInputDimsRangeImplOm2(const aclmdlDesc *modelDesc, size_t index, aclmdlIODimsRange *dimsRange) {
  ...
  if (ret != ACL_SUCCESS) {
    ACL_LOG_INNER_ERROR("[Get][DimsRange]get input dims range failed, result[%d], index[%zu], modelId[%u]", ret, index,
                        modelDesc->modelId);
  }
...
aclError aclmdlGetCurOutputDimsImplOm2(const aclmdlDesc *modelDesc, size_t index, aclmdlIODims *dims) {
  ...
  if (index >= descSize) {
    ACL_LOG_INNER_ERROR("[Check][Index] index[%zu] >= output tensor count[%zu], modelId[%u]", index, descSize, modelId);
    return ACL_ERROR_INVALID_PARAM;
  }
  ...
  if (geShapeInfo.empty()) {
    ...
    const aclError aclRet =
        acl::GetDims(modelDesc, TensorType::OUTPUT_TENSOR_TYPE, DimsType::DIMS_TYPE_V1, index, dims);
  ...
  ACL_REQUIRES_OK_WITH_INNER_MESSAGE(aclRet, "[Get][CurGearIndex] failed, result[%d], modelId[%u]", aclRet, modelId);

  // Look up output shape for this gear + output index
  aclRet = acl::GetCurOuputShapeInfo(modelDesc, index, curGearIndex, dims);
  ACL_REQUIRES_OK_WITH_INNER_MESSAGE(aclRet, "[Get][CurOutputShapeInfo] failed, result[%d], index[%zu], modelId[%u]",
                                     aclRet, index, modelId);
...
aclFormat aclmdlGetOutputFormatImplOm2(const aclmdlDesc *modelDesc, size_t index) {
  ...
  if (modelDesc == nullptr) {
    ACL_LOG_ERROR("[Check][ModelDesc]modelDesc is null");
    acl::AclErrorLogManager::ReportInputError(acl::INVALID_NULL_POINTER_MSG, std::vector<const char *>({"params"}),
                                              std::vector<const char *>({"modelDesc"}));
    return ACL_FORMAT_UNDEFINED;
  }
...
aclError aclmdlGetDynamicBatchImplOm2(const aclmdlDesc *modelDesc, aclmdlBatch *batch) {
  ...
  if (batchCnt > static_cast<size_t>(ACL_MAX_BATCH_NUM)) {
    ACL_LOG_ERROR("[Check][batchCnt]aclmdlGetBatch failed, batch count[%zu] is larger than max batch num[%d]", batchCnt,
                  ACL_MAX_BATCH_NUM);
    const std::string errMsg = acl::AclErrorLogManager::FormatStr("batch count[%zu] is larger than max batch num[%d]",
...
aclError aclmdlGetDynamicHWImplOm2(const aclmdlDesc *modelDesc, size_t index, aclmdlHW *hw) {
  ...
  if (hwCnt > static_cast<size_t>(ACL_MAX_HW_NUM)) {
    ACL_LOG_INNER_ERROR("[Check][hwCnt]aclmdlGetHW failed, hw count[%zu] is larger than max[%d]", hwCnt,
                        ACL_MAX_HW_NUM);
    return ACL_ERROR_STORAGE_OVER_LIMIT;
  }
...
aclError aclmdlGetInputDynamicGearCountImplOm2(const aclmdlDesc *modelDesc, size_t index, size_t *gearCount) {
  ...
  if (dimCnt > static_cast<size_t>(ACL_MAX_DIM_CNT)) {
    ACL_LOG_INNER_ERROR(
        "[Check][dimCnt]aclmdlGetInputDynamicGearCount failed, dimCnt[%zu] is "
        "larger than max[%d]",
        dimCnt, ACL_MAX_DIM_CNT);
    return ACL_ERROR_STORAGE_OVER_LIMIT;
  }
...
aclError aclmdlCreateAndGetOpDescImplOm2(uint32_t deviceId, uint32_t streamId, uint32_t taskId, char *opName,
  ...
  ACL_LOG_INFO("[OM2] start to execute aclmdlCreateAndGetOpDesc, deviceId[%u], streamId[%u], taskId[%u]", deviceId,
               streamId, taskId);
  ...
  if (copyRet != EOK) {
    ACL_LOG_INNER_ERROR(
        ...
        "real opNameLen = %zu",
        copyRet, opNameLen, opDescInfo.op_name.length());
    return ACL_ERROR_FAILURE;
  }

  ACL_REQUIRES_OK(CreateAclTensorDescFromOpDescInfo(opDescInfo, inputDesc, numInputs, outputDesc, numOutputs));
  ACL_LOG_INFO(
      "[OM2] successfully execute aclmdlCreateAndGetOpDesc, deviceId[%u], streamId[%u], "
