...
aclError aclmdlCheckQueueParam(const uint32_t *const inputQ, const size_t inputQNum, const uint32_t *const outputQ,
  ...
  if ((inputQNum == 0U) || (outputQNum == 0U)) {
    ACL_LOG_INNER_ERROR("[Check][QNum]inputQNum[%zu] or outputQNum[%zu] is invalid, can't be zero", inputQNum,
                        outputQNum);
...
static aclError ModelLoadFromFileWithMem(const char_t *const modelPath, uint32_t *const modelId,
  ...
  ACL_LOG_INFO(
      "start to execute ModelLoadFromFileWithMem, modelPath[%s], "
      "workSize[%zu], weightSize[%zu], priority[%d]",
      modelPath, loadArgs.mem_size, loadArgs.weight_size, priority);
  ...
  data.om_path = path;
  ACL_LOG_INFO("call ge interface executor.LoadDataFromFile, workSize[%zu], weightSize[%zu]", loadArgs.mem_size,
               loadArgs.weight_size);
...
static aclError RuntimeV2ModelLoadCommon(ge::ModelData &modelData, uint32_t *const modelId, const void *const weightPtr,
                                         const size_t weightSize, std::vector<ge::FileConstantMem> file_constant_mems,
  ...
  auto rtSession =
      (rtSessionExternal != nullptr) ? rtSessionExternal : acl::AclResourceManager::GetInstance().CreateRtSession();
  ACL_REQUIRES_NOT_NULL(rtSession);
  gert::LoadExecutorArgs args = {.rt_session = rtSession.get(), .file_constant_mems = std::move(file_constant_mems)};
...
static aclError RuntimeV2ModelLoadFromFileWithMem(const char_t *const modelPath, uint32_t *const modelId,
  ...
  ACL_LOG_INFO("start to execute RuntimeV2ModelLoadFromFileWithMem, priority[%d], weightSize[%zu]", priority,
               weightSize);
...
static aclError ModelLoadFromMemWithMem(const void *const model, const size_t modelSize, const std::string &modelPath,
  ...
  *modelId = id;
  ACL_LOG_INFO(
      "successfully execute ModelLoadFromMemWithMem, modelSize[%zu], workSize[%zu], "
      "weightSize[%zu], modelId[%u]",
      modelSize, loadArgs.mem_size, loadArgs.weight_size, *modelId);
  return ACL_SUCCESS;
}
...
static aclError GetBundleNumAndOffset(const void *const model, const size_t modelSize, size_t &varSize,
  ...
  for (size_t i = 0; i < partitionTable->num; ++i) {
    ...
    ACL_REQUIRES_OK(acl::CheckSizeTAddOverflow(currentOffset, partitionTable->partition[i].mem_size, currentOffset));
    ACL_REQUIRES_LE(currentOffset, modelSize);
  }
  return ACL_SUCCESS;
}
...
aclError DetectModelTypeInLoadWithConfig(const aclmdlConfigHandle *handle, const char *filePath, const void *model,
                                         const size_t modelSize, bool &isSupportRT2) {
  if (filePath != nullptr) {
...
static aclError RuntimeV2ModelLoadFromMemWithMem(const void *const model, const size_t modelSize,
                                                 const std::string &modelPath, uint32_t *const modelId,
                                                 void *const weightPtr, const size_t weightSize,
                                                 const char_t *const weightPath, const int32_t priority,
                                                 std::vector<ge::FileConstantMem> file_constant_mems = {},
                                                 const std::shared_ptr<gert::RtSession> &rtSessionExternal = nullptr) {
...
static aclError ModelLoadFromFileWithQ(const char_t *const modelPath, uint32_t *const modelId,
  ...
  ACL_LOG_INFO(
      "start to execute ModelLoadFromFileWithQ, modelPath[%s], inputQNum[%zu], "
      "outputQNum[%zu], priority[%d]",
      modelPath, args.input_queue_ids.size(), args.output_queue_ids.size(), priority);
  ...
  *modelId = id;
  ACL_DELETE_ARRAY(modelData.model_data);
  ACL_LOG_INFO(
      "successfully execute ModelLoadFromFileWithQ, modelPath[%s], inputQNum[%zu], outputQNum[%zu], "
      "modelId[%u]",
      modelPath, args.input_queue_ids.size(), args.output_queue_ids.size(), *modelId);
  return ACL_SUCCESS;
}
...
static aclError ModelLoadFromMemWithQ(const void *const model, const size_t modelSize, uint32_t *const modelId,
  ...
  ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(modelId);
  ACL_LOG_INFO(
      "start to execute ModelLoadFromMemWithQ, modelSize[%zu], inputQNum[%zu], outputQNum[%zu], "
      "priority[%d]",
      modelSize, args.input_queue_ids.size(), args.output_queue_ids.size(), priority);

  if (modelSize == 0U) {
    ACL_LOG_INNER_ERROR(
        "[Check][Params]modelSize[%zu] is invalid, "
        "can't be zero",
        modelSize);
    return ACL_ERROR_INVALID_PARAM;
  }
  ...
  *modelId = id;
  ACL_LOG_INFO(
      "successfully execute ModelLoadFromMemWithQ, modelSize[%zu], inputQNum[%zu], outputQNum[%zu], "
      "modelId[%u]",
      modelSize, args.input_queue_ids.size(), args.output_queue_ids.size(), *modelId);

  return ACL_SUCCESS;
}

static void SetInputData(const std::vector<AclModelTensor> &blobs, std::vector<ge::GeTensorDesc> &inputGeDesc,
                         bool &isDynamic) {
...
static void SetInputData(const std::vector<AclModelTensor> &blobs, std::vector<gert::Tensor> &inputTensor) {
...
static aclError RuntimeV2ModelExecute(const uint32_t modelId, const aclmdlDataset *const input,
  ...
  if (input->blobs.size() < inputNum) {
    ACL_LOG_ERROR("input blobs %zu cannot be smaller than input desc size %zu", input->blobs.size(), inputNum);
  ...
  for (size_t i = 0UL; i < inputNum; ++i) {
    ...
    if (dataBuffer == nullptr) {
      ACL_LOG_ERROR(
          "[Check][dataBuffer]input dataset blobs is null, "
          "modelId[%d], index[%zu]",
          modelId, i);
      acl::AclErrorLogManager::ReportInputError(acl::INVALID_NULL_POINTER_MSG, std::vector<const char *>({"param"}),
    ...
    if ((dataBuffer->data == nullptr) && (!isAsync)) {
      ...
      if (memSize > 0UL) {
        ACL_REQUIRES_CALL_RTS_OK(
            aclrtMalloc(reinterpret_cast<void **>(&dataBuffer->data), memSize, ACL_MEM_TYPE_HIGH_BAND_WIDTH),
            aclrtMalloc);
        dataBuffer->length = memSize;
        ACL_LOG_DEBUG(
            "ModelExecute, assign acl-malloced output addr to user-defined buffer, addr:[%p], "
            "len:[%lu]",
  ...
  for (size_t i = 0UL; i < outputTensor.size(); ++i) {
    ...
    if ((dataBuffer->data == nullptr) && (!isAsync)) {
      ...
      ACL_REQUIRES_CALL_RTS_OK(
          ...
          aclrtMalloc);
      ACL_REQUIRES_CALL_RTS_OK(aclrtMemcpy(dataBuffer->data, outputSize, outputTensor[i].MutableTensorData().GetAddr(),
                                           outputSize, ACL_MEMCPY_DEVICE_TO_DEVICE),
                               aclrtMemcpy);
...
static aclError ModelExecute(const uint32_t modelId, const aclmdlDataset *const input, aclmdlDataset *const output,
                             const bool basync, const aclrtStream stream) {
  ACL_LOG_INFO("start to execute ModelExecute, modelId[%u]", modelId);
  ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(input);
  ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(output);
  ...
  if ((!needMallocIndexes.empty()) && (!basync)) {
    ...
    if (outputDesc.size() < output->blobs.size()) {
      ACL_LOG_ERROR("[Check][OutputDesc] outputDesc size [%zu] mismatch with blobs size[%zu].", outputDesc.size(),
                    output->blobs.size());
      return ACL_ERROR_INVALID_PARAM;
    }
  ...
  ACL_LOG_INFO("call ge interface executor.ExecModel, modelId[%u], asyncMode[%d]", modelId,
               static_cast<int32_t>(basync));
  std::vector<ge::GeTensorDesc> outputGeDesc;
  const ge::Status ret = executor.ExecModel(modelId, stream, inputData, inputGeDesc, outputData, outputGeDesc, basync);
  if (ret != ge::SUCCESS) {
    ACL_LOG_CALL_ERROR("[Exec][Model]Execute model failed, ge result[%u], modelId[%u]", ret, modelId);
    return ACL_GET_ERRCODE_GE(static_cast<int32_t>(ret));
  }
  ...
  for (size_t i = 0UL; i < outputGeDesc.size(); ++i) {
    ...
    if ((dataBuffer->data == nullptr) && (!basync)) {
      ...
      ACL_REQUIRES_CALL_RTS_OK(
          ...
          aclrtMalloc);
      ACL_REQUIRES_CALL_RTS_OK(
          aclrtMemcpy(dataBuffer->data, outputSize, outputData.blobs[i].data, outputSize, ACL_MEMCPY_DEVICE_TO_DEVICE),
          aclrtMemcpy);
      dataBuffer->length = outputSize;
      ACL_LOG_DEBUG(
          "ModelExecute, assign acl-malloced output addr to user-defined buffer, addr:[%p], "
...
static void GetTensorInfo(std::vector<aclmdlTensorDesc> &inputDesc, const gert::ModelIoDesc *const inputs,
  ...
  for (size_t i = 0U; i < inputNum; ++i) {
    ...
    (void)tensorDesc.shapeRanges.insert(tensorDesc.shapeRanges.cend(), shapeRangeVector.cbegin(),
                                        shapeRangeVector.cend());
...
static aclError RuntimeV2GetDesc(aclmdlDesc *const modelDesc, const uint32_t modelId) {
  ...
  if (retVal != ACL_SUCCESS) {
    ACL_LOG_INNER_ERROR("[Get][DynamicTensorInfo]get model dynamic info failed, result[%d], model id[%u]", retVal,
                        modelId);
    return retVal;
  }
...
aclError aclmdlGetDescImpl(aclmdlDesc *modelDesc, uint32_t modelId) {
  ...
  std::vector<ge::TensorDesc> outputDescV2;
  {
    ...
    if (ret != ge::SUCCESS) {
      ACL_LOG_CALL_ERROR("[Get][ModelDescInfo]get model description failed, ge result[%u], model id[%u]", ret, modelId);
      return ACL_GET_ERRCODE_GE(static_cast<int32_t>(ret));
    }
  ...
  if ((inputDescV2.size() < inputDesc.size()) || (outputDescV2.size() < outputDesc.size())) {
    ACL_LOG_CALL_ERROR("[Get][ModelDescInfo]description v2 size less than description v1 size, model id[%u]", modelId);
...
static void GetModelTensorDesc(const ge::ModelInOutInfo &info, aclmdlDesc *modelDesc, bool isInput) {
  ...
  for (const auto &desc : descs) {
    ...
    tmpDesc.format = (desc.format != ge::FORMAT_RESERVED) ? static_cast<aclFormat>(desc.format) : ACL_FORMAT_UNDEFINED;
    tmpDesc.dataType = (desc.dataType != ge::DT_UNDEFINED) ? static_cast<aclDataType>(desc.dataType) : ACL_DT_UNDEFINED;
...
static aclError QueryBundleSubModelInfo(const void *data, size_t modelSize, aclmdlBundleQueryInfo *queryInfo) {
  ...
  for (const auto &ele : subModelOffsetAndSize) {
    ...
    if (!isSupportRT2) {
      ge::GeExecutor executor;
      ACL_REQUIRES_CALL_GE_OK(
          executor.GetMemAndWeightSize(currentModePtr, ele.second, tmpInfo.workSize, tmpInfo.weightSize),
          "Query size failed");
    }
...
static aclError LoadBundleSubModelFromMem(const void *const currentModePtr, const size_t modelSize,
  ...
  if (isSupportRT2) {
    ret = acl::RuntimeV2ModelLoadFromMemWithMem(currentModePtr, modelSize, modelPath, &currentModelId,
                                                loadArgs.weight_ptr, loadArgs.weight_size, weightPath, priority,
                                                loadArgs.file_constant_mems, rtSession);
  } else {
    ret = acl::ModelLoadFromMemWithMem(currentModePtr, modelSize, modelPath, &currentModelId, loadArgs, weightPath,
                                       priority);
  }
  return ret;
}
...
static aclError BundleInitFromMem(std::shared_ptr<const uint8_t> model, size_t modelSize, const std::string &modelPath,
  ...
  if ((varWeightPtr != nullptr) && (varWeightSize < varSize)) {
    ACL_LOG_ERROR("varWeightSize %zu is invalid, it cannot be smaller than model required size %zu", varWeightSize,
                  varSize);
...
static aclError BundleLoadFromMem(std::shared_ptr<const uint8_t> model, size_t modelSize, const std::string &modelPath,
  ...
  for (const auto &subInfo : info.subModelInfos) {
    ...
    aclError ret = LoadBundleSubModelFromMem(currentModePtr, subInfo.modelSize, modelPath, info.rtSession, loadArgs,
                                             currentModelId);
    if (ret != ACL_SUCCESS) {
...
aclError aclmdlLoadFromFileWithMemImpl(const char *modelPath, uint32_t *modelId, void *workPtr, size_t workSize,
...
aclError aclmdlBundleGetModelIdImpl(uint32_t bundleId, size_t index, uint32_t *modelId) {
  ...
  if (bundleInfos.isInit) {
    ACL_LOG_ERROR(
        "aclmdlBundleGetModelId is supported only aclmdlBundleLoadFromFile or aclmdlBundleLoadFromMem is executed");
    return ACL_ERROR_API_NOT_SUPPORT;
  }
  if (index >= bundleInfos.loadedSubModelId.size()) {
    ACL_LOG_ERROR("bundleId %u input index %zu should be smaller than bundle size %zu", bundleId, index,
                  bundleInfos.loadedSubModelId.size());
    return ACL_ERROR_INVALID_PARAM;
  }
...
aclError aclmdlBundleInitFromFileImpl(const char *modelPath, void *varWeightPtr, size_t varWeightSize,
  ...
  ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(modelPath);
  ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(bundleId);
  ACL_LOG_INFO("start to execute aclmdlBundleInitFromFile, model path %s, varWeightSize %zu", modelPath, varWeightSize);
  ge::ModelData modelData;
  modelData.om_path = modelPath;
  ACL_LOG_INFO("call ge interface gert::LoadDataFromFile");
  ...
  ACL_LOG_INFO("end to execute aclmdlBundleInitFromFile, model path %s, varWeightSize %zu, bundleId %u", modelPath,
               varWeightSize, *bundleId);
  return ACL_SUCCESS;
}

aclError aclmdlBundleInitFromMemImpl(const void *model, size_t modelSize, void *varWeightPtr, size_t varWeightSize,
                                     uint32_t *bundleId) {
  ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(model);
  ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(bundleId);
...
aclError aclmdlBundleLoadModelWithMemImpl(uint32_t bundleId, size_t index, void *workPtr, size_t workSize,
  ...
  const auto loadArgs =
      ConstructGeModelLoadArg(workPtr, workSize, weightPtr, weightSize, bundleInfos.rtSession.get(), {});
  ACL_REQUIRES_OK(LoadBundleSubModelFromMem(currentModePtr, modelSize, bundleInfos.fromFilePath, bundleInfos.rtSession,
                                            loadArgs, *modelId));
...
aclError aclmdlBundleLoadModelWithConfigImpl(uint32_t bundleId, size_t index, aclmdlConfigHandle *handle,
  ...
  void *currentModePtr = ge::ValueToPtr(ge::PtrToValue(bundleInfos.bundleModelData.get()) + offset);
  const auto loadArgs =
      ConstructGeModelLoadArg(handle->workPtr, handle->workSize, handle->weightPtr, handle->weightSize,
                              bundleInfos.rtSession.get(), handle->fileConstantMem, handle->withoutGraph);

  ACL_REQUIRES_OK(LoadBundleSubModelFromMem(currentModePtr, modelSize, bundleInfos.fromFilePath, bundleInfos.rtSession,
                                            loadArgs, *modelId, handle->weightPath.c_str(), handle->priority));
  acl::AclResourceManager::GetInstance().AddBundleSubmodelId(bundleId, *modelId);
  ACL_LOG_INFO("end to execute aclmdlBundleLoadModelWithConfig, bundleId %u, index %zu, model id is %u", bundleId,
               index, *modelId);
  return ACL_SUCCESS;
}
...
aclError aclmdlLoadFromMemWithMemImpl(const void *model, size_t modelSize, uint32_t *modelId, void *workPtr,
                                      size_t workSize, void *weightPtr, size_t weightSize) {
  ACL_PROFILING_REG(acl::AclProfType::AclmdlLoadFromMemWithMem);
  ACL_LOG_INFO("start to execute aclmdlLoadFromMemWithMem, modelSize[%zu], workSize[%zu], weightSize[%zu]", modelSize,
               workSize, weightSize);
  ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(model);
  ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(modelId);
  ...
  aclError ret = ACL_SUCCESS;
  if (isSupportRT2) {
    ret = acl::RuntimeV2ModelLoadFromMemWithMem(model, modelSize, "", modelId, weightPtr, weightSize, nullptr, 0);
  } else {
    const auto loadArgs = ConstructGeModelLoadArg(workPtr, workSize, weightPtr, weightSize);
    ret = acl::ModelLoadFromMemWithMem(model, modelSize, "", modelId, loadArgs, nullptr, 0);
  }
...
aclError aclmdlLoadFromFileWithQImpl(const char *modelPath, uint32_t *modelId, const uint32_t *inputQ, size_t inputQNum,
  ...
  ACL_LOG_INFO("successfully execute aclmdlLoadFromFileWithQ, inputQNum[%zu], outputQNum[%zu]", inputQNum, outputQNum);
  return ACL_SUCCESS;
}

aclError aclmdlLoadFromMemWithQImpl(const void *model, size_t modelSize, uint32_t *modelId, const uint32_t *inputQ,
                                    size_t inputQNum, const uint32_t *outputQ, size_t outputQNum) {
  ACL_PROFILING_REG(acl::AclProfType::AclmdlLoadFromMemWithQ);
  if (aclmdlCheckQueueParam(inputQ, inputQNum, outputQ, outputQNum) != ACL_SUCCESS) {
    return ACL_ERROR_INVALID_PARAM;
  }
  ACL_LOG_INFO("start to execute aclmdlLoadFromMemWithQ, modelSize[%zu], inputQNum[%zu], outputQNum[%zu]", modelSize,
...
aclError aclmdlExecuteAsyncImpl(uint32_t modelId, const aclmdlDataset *input, aclmdlDataset *output,
                                aclrtStream stream) {
...
aclError aclmdlQuerySizeImpl(const char *fileName, size_t *workSize, size_t *weightSize) {
  ...
  *workSize = work;
  *weightSize = weight;
  ACL_LOG_INFO("success to get size from file[%s], work size[%zu] bytes, weight size[%zu] bytes", fileName, *workSize,
               *weightSize);
...
aclError aclmdlSetDynamicBatchSizeImpl(uint32_t modelId, aclmdlDataset *dataset, size_t index, uint64_t batchSize) {
  ...
  ACL_LOG_INFO("start to execute aclmdlSetDynamicBatchSize, modelId[%u], index[%zu], batchSize[%lu]", modelId, index,
               batchSize);
  ...
  ACL_LOG_INFO("successfully execute aclmdlSetDynamicBatchSize, modelId[%u], index[%zu], batchSize[%lu]", modelId,
               index, batchSize);
  return ACL_SUCCESS;
}

aclError aclmdlSetDynamicHWSizeImpl(uint32_t modelId, aclmdlDataset *dataset, size_t index, uint64_t height,
                                    uint64_t width) {
  ACL_PROFILING_REG(acl::AclProfType::AclmdlSetDynamicHWSize);
  ACL_LOG_INFO("start to execute aclmdlSetDynamicHWSize, modelId[%u], index[%zu], height[%lu], width[%lu]", modelId,
               index, height, width);
  ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(dataset);
  ...
  if (buffer == nullptr) {
    ACL_LOG_INNER_ERROR("[Check][buffer]get data buffer by index[%zu] failed, dataset buffer cannot be null", index);
    return ACL_ERROR_INVALID_PARAM;
  }
  ...
  ACL_LOG_INFO("successfully execute aclmdlSetDynamicHWSize, modelId[%u], index[%zu], height[%lu], width[%lu]", modelId,
               index, height, width);
  return ACL_SUCCESS;
}

aclError aclmdlSetInputDynamicDimsImpl(uint32_t modelId, aclmdlDataset *dataset, size_t index,
  ...
  ACL_LOG_INFO("start to execute aclmdlSetInputDynamicDims, modelId[%u], index[%zu], dimCount[%zu]", modelId, index,
               dims->dimCount);
  aclDataBuffer *const buffer = aclmdlGetDatasetBufferImplOm2(dataset, index);
  ACL_CHECK_WITH_INNER_MESSAGE_AND_RETURN(
      buffer != nullptr, ACL_ERROR_INVALID_PARAM,
  ...
  ACL_CHECK_WITH_INNER_MESSAGE_AND_RETURN(devPtr != nullptr, ACL_ERROR_INVALID_PARAM,
                                          "[Get][devPtr]get addr by index[%zu] failed, data buffer addr cannot be null",
                                          index);
  const uint64_t memSize = aclGetDataBufferSizeV2(buffer);
  ...
  ACL_LOG_INFO("successfully execute aclmdlSetInputDynamicDims, modelId[%u], index[%zu], dimCount[%zu]", modelId, index,
               dims->dimCount);
  return ACL_SUCCESS;
}
...
aclError aclmdlGetCurOutputDimsImpl(const aclmdlDesc *modelDesc, size_t index, aclmdlIODims *dims) {
  ...
  ACL_REQUIRES_OK_WITH_INNER_MESSAGE(aclRet,
                                     ...
                                     "modelId[%u], dynamicBatchSize[%zu], dynamicHWSize[%zu]",
                                     aclRet, index, modelId, modelDesc->dynamicBatch.size(),
                                     modelDesc->dynamicHW.size());
...
const char *aclmdlGetOpAttrImpl(aclmdlDesc *modelDesc, const char *opName, const char *attr) {
  ...
  if (attrStr != ACL_ATTR_NAME_DATA_DUMP_ORIGIN_OP_NAMES) {
    ACL_LOG_INNER_ERROR(
        "failed to execute aclmdlGetOpAttr, attr[%s] is invalid, only support "
        "ACL_ATTR_NAME_DATA_DUMP_ORIGIN_OP_NAMES",
        attrStr.c_str());
    return nullptr;
  }
  ...
  ge::GeExecutor executor;
  const uint32_t modelId = modelDesc->modelId;
  ACL_LOG_INFO("Call ge interface executor.GetOpAttr, modelId is [%u], opName is [%s], attr is [%s]", modelId, opName,
               attr);
...
aclError aclmdlCreateAndGetOpDescImpl(uint32_t deviceId, uint32_t streamId, uint32_t taskId, char *opName,
  ...
  ACL_LOG_INFO("start to execute aclmdlCreateAndGetOpDesc, deviceId[%u], streamId[%u], taskId[%u]", deviceId, streamId,
               taskId);
  ...
  if (opNameLen <= opDescInfo.op_name.length()) {
    ACL_LOG_INNER_ERROR("[Check][opNameLen]input length = %zu must be larger than op name real length = %zu", opNameLen,
                        opDescInfo.op_name.length());
    return ACL_ERROR_INVALID_PARAM;
  }
  const auto ret = strncpy_s(opName, opNameLen, opDescInfo.op_name.c_str(), opDescInfo.op_name.length());
  if (ret != EOK) {
    ACL_LOG_INNER_ERROR(
        "[Copy][OpName]copy op name failed, copy errorCode = %d, input opNameLen = %zu, "
        "real opNameLen = %zu",
        ret, opNameLen, opDescInfo.op_name.length());
    return ACL_ERROR_FAILURE;
  }
  ...
  *numOutputs = outputNum;
  ACL_LOG_INFO(
      "successfully execute aclmdlCreateAndGetOpDesc, deviceId[%u], streamId[%u], "
      "taskId[%u], numInputs[%zu], numOutputs[%zu]",
      deviceId, streamId, taskId, *numInputs, *numOutputs);
  return ACL_SUCCESS;
}
...
static aclError LoadFromFile(const aclmdlConfigHandle *handle, const std::vector<ge::FileConstantMem> &fileConstantMems,
  ...
  if (isSupportRT2) {
    return acl::RuntimeV2ModelLoadFromFileWithMem(handle->loadPath.c_str(), modelId, nullptr, 0U, handle->priority,
                                                  fileConstantMems);
  }
  ge::ModelLoadArg loadArgs{};
  loadArgs.file_constant_mems = fileConstantMems;
...
static aclError LoadFromFileWithMem(const aclmdlConfigHandle *handle,
                                    const std::vector<ge::FileConstantMem> &fileConstantMems, uint32_t *const modelId) {
  bool isSupportRT2 = false;
  ACL_REQUIRES_OK(acl::DetectModelTypeInLoadWithConfig(handle, handle->loadPath.c_str(), nullptr, 0, isSupportRT2));
  if (isSupportRT2) {
    return acl::RuntimeV2ModelLoadFromFileWithMem(handle->loadPath.c_str(), modelId, handle->weightPtr,
                                                  handle->weightSize, handle->priority, fileConstantMems);
  }
...
static aclError LoadFromMem(const aclmdlConfigHandle *handle, const std::vector<ge::FileConstantMem> &fileConstantMems,
  ...
  bool isSupportRT2 = false;
  ACL_REQUIRES_OK(
      acl::DetectModelTypeInLoadWithConfig(handle, nullptr, handle->mdlAddr, handle->mdlSize, isSupportRT2));
  if (isSupportRT2) {
    return acl::RuntimeV2ModelLoadFromMemWithMem(handle->mdlAddr, handle->mdlSize, handle->loadPath, modelId, nullptr,
                                                 0U, handle->weightPath.c_str(), handle->priority, fileConstantMems);
  }
  ge::ModelLoadArg loadArgs{};
  loadArgs.file_constant_mems = fileConstantMems;
  loadArgs.need_clear_dfx_cache = handle->withoutGraph;
...
static aclError LoadFromMemWithMem(const aclmdlConfigHandle *handle,
  ...
  if (isSupportRT2) {
    return acl::RuntimeV2ModelLoadFromMemWithMem(handle->mdlAddr, handle->mdlSize, handle->loadPath, modelId,
                                                 handle->weightPtr, handle->weightSize, handle->weightPath.c_str(),
...
static aclError LoadFromFileWithQ(const aclmdlConfigHandle *handle,
                                  const std::vector<ge::FileConstantMem> &fileConstantMems, uint32_t *const modelId) {
  if (aclmdlCheckQueueParam(handle->inputQ, handle->inputQNum, handle->outputQ, handle->outputQNum) != ACL_SUCCESS) {
    return ACL_ERROR_INVALID_PARAM;
  }
  ...
  ge::ModelQueueArg args{std::move(inputQVec), std::move(outputQVec), fileConstantMems, handle->withoutGraph};
...
aclError aclmdlLoadWithConfigImpl(const aclmdlConfigHandle *handle, uint32_t *modelId) {
  ...
  switch (static_cast<int32_t>(handle->mdlLoadType)) {
    ...
    default:
      ACL_LOG_INNER_ERROR("[Load][Model]model load type[%zu] is invalid, it should be in [%d, %d]", handle->mdlLoadType,
