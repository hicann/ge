...
aclTensorDesc::aclTensorDesc(const aclDataType aclTensorDataType, const std::initializer_list<int64_t> shape,
    ...
    : dataType(aclTensorDataType), format(aclTensorFormat), dims(shape) {
  if (acl::AclResourceManagerOm2::GetInstance().IsRuntimeV2Enable(false)) {  // how to deal with it for model?
    this->storageDims = dims;
  }
}

aclTensorDesc::aclTensorDesc(const aclDataType aclTensorDataType, const size_t numDims,
                             const int64_t *const aclTensorDims, const aclFormat aclTensorFormat)
...
aclTensorDesc *aclCreateTensorDescImplOm2(aclDataType dataType, int numDims, const int64_t *dims, aclFormat format) {
  ...
  if (numDims < 0) {
    ...
    acl::AclErrorLogManager::ReportInputError(
        ...
        std::vector<const char *>({"numDims", std::to_string(numDims).c_str(), "can't smaller than 0"}));
    return nullptr;
  }
...
aclError aclSetTensorShapeRangeImplOm2(aclTensorDesc *desc, size_t dimsCount,
...
aclError aclGetTensorDescDimRangeImplOm2(const aclTensorDesc *desc, size_t index, size_t dimRangeNum,
                                         int64_t *dimRange) {
  ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(desc);
  ACL_REQUIRES_NOT_NULL_WITH_INPUT_REPORT(dimRange);
  if (index >= desc->shapeRange.size()) {
    ACL_LOG_ERROR("[Check][Index]index out of range. index = %zu, numDims = %zu", index, desc->shapeRange.size());
    const std::string errMsg =
        acl::AclErrorLogManager::FormatStr("index out of range. numDims = %zu", desc->shapeRange.size());
    acl::AclErrorLogManager::ReportInputError(
        acl::INVALID_PARAM_MSG, std::vector<const char *>({"param", "value", "reason"}),
  ...
  if (dimRangeNum < static_cast<size_t>(ACL_TENSOR_SHAPE_RANGE_NUM)) {
    ACL_LOG_ERROR("[Check][DimRangeNum]dimRangeNum cannot be less than 2. dimRangeNum = %zu", dimRangeNum);
    acl::AclErrorLogManager::ReportInputError(
        acl::INVALID_PARAM_MSG, std::vector<const char *>({"param", "value", "reason"}),
        std::vector<const char *>({"dimRangeNum", std::to_string(dimRangeNum).c_str(), "cannot be less than 2"}));
    return ACL_ERROR_INVALID_PARAM;
  }
...
int64_t aclGetTensorDescDimImplOm2(const aclTensorDesc *desc, size_t index) {
  ...
  if (index >= desc->dims.size()) {
    ACL_LOG_ERROR("[Check][Index]index out of range. index = %zu, numDims = %zu", index, desc->dims.size());
...
aclError aclGetTensorDescDimV2ImplOm2(const aclTensorDesc *desc, size_t index, int64_t *dimSize) {
  ...
  if (index >= desc->dims.size()) {
    ACL_LOG_ERROR("[Check][Index]index out of range. index = %zu, numDims = %zu", index, desc->dims.size());
    const std::string errMsg =
        acl::AclErrorLogManager::FormatStr("index out of range. numDims = %zu", desc->dims.size());
    acl::AclErrorLogManager::ReportInputError(
        acl::INVALID_PARAM_MSG, std::vector<const char *>({"param", "value", "reason"}),
        std::vector<const char *>({"index", std::to_string(index).c_str(), errMsg.c_str()}));
    return ACL_ERROR_INVALID_PARAM;
...
aclError aclSetTensorConstImplOm2(aclTensorDesc *desc, void *dataBuffer, size_t length) {
  ...
  desc->constDataBuf.reset(constData, [](const char_t *const ptr) {
    delete[] ptr;
    ACL_LOG_DEBUG("delete const data in aclSetTensorConst");
  });
