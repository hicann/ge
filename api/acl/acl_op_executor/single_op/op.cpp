...
aclError aclopSetModelDirImpl(const char *modelDir) {
  ...
  if (checkPathRet != EN_OK) {
    ...
    acl::AclErrorLogManager::ReportInputError(
        ...
        std::vector<const char *>({modelDir, (std::string("cannot convert to realpath: ") + formatErrMsg).c_str()}));
...
aclError aclopCreateHandleImpl(const char *opType, int numInputs, const aclTensorDesc *const inputDesc[],
...
aclError aclopExecWithHandleImpl(aclopHandle *handle, int numInputs, const aclDataBuffer *const inputs[],
  ...
  if (numInputs != opHandle.numInputs) {
    ...
    const std::string errMsg = acl::AclErrorLogManager::FormatStr("input num mismatch: expect %d", opHandle.numInputs);
    acl::AclErrorLogManager::ReportInputError(
        acl::INVALID_PARAM_MSG, std::vector<const char *>({"param", "value", "reason"}),
...
aclError aclopExecuteImpl(const char *opType, int numInputs, const aclTensorDesc *const inputDesc[],
...
aclError aclopExecuteV2Impl(const char *opType, int numInputs, aclTensorDesc *inputDesc[], aclDataBuffer *inputs[],
...
aclError aclopCreateKernelImpl(const char *opType, const char *kernelId, const char *kernelName, void *binData,
                               int binSize, aclopEngineType enginetype, aclDataDeallocator deallocator) {
  ...
  ACL_REQUIRES_OK(
      acl::OpKernelRegistry::GetInstance().Register(std::unique_ptr<acl::OpKernelRegistration>(registration)));
  ACL_LOG_DEBUG("Successfully created kernel. opType = %s, kernelId = %s, kernelName = %s", opType, kernelId,
                kernelName);
  return ACL_SUCCESS;
}

aclError aclopUpdateParamsImpl(const char *opType, int numInputs, const aclTensorDesc *const inputDesc[],
                               int numOutputs, const aclTensorDesc *const outputDesc[], const aclopAttr *attr) {
...
aclError aclopSetKernelArgsImpl(aclopKernelDesc *kernelDesc, const char *kernelId, uint32_t blockDim, const void *args,
                                uint32_t argSize) {
...
static aclError LoadOpsProto() {
  ...
  if (!GetOpsProtoPath(opsprotoPath)) {
    ACL_LOG_INNER_ERROR(
        "[Check][ProtoPath]The environment variable(ASCEND_OPP_PATH) is not set or path is "
        "invalid.");
...
static aclError UpdateOutPutDesc(const ge::Operator &inferOp, const int32_t numOutputs,
                                 aclTensorDesc *const outputDesc[]) {
...
static aclError AddDataInput(const aclTensorDesc *const inputDesc, const aclDataBuffer *const inputs,
                             std::unique_ptr<uint8_t[]> &constData, ge::Operator &constOp) {
  // create const operator
  std::vector<int64_t> dims;
  ConvertSvecToVec(inputDesc->dims, dims);
  const ge::TensorDesc constDesc(ge::Shape(dims), static_cast<::ge::Format>(inputDesc->format),
...
aclError aclopRegisterCompileFuncImpl(const char *opType, aclopCompileFunc func) {
  ...
  if (ret) {
    ...
    ACL_LOG_INNER_ERROR(
        "[Register][Function]Failed to register compile function due to repeat registration. "
        "op type = %s",
        opType);
...
aclError aclopInferShapeImpl(const char *opType, int numInputs, aclTensorDesc *inputDesc[], aclDataBuffer *inputs[],
  ...
  for (int32_t i = 0; i < numInputs; ++i) {
    ...
    const ge::Format format =
        (aclFormat == ACL_FORMAT_UNDEFINED) ? ge::FORMAT_RESERVED : static_cast<ge::Format>(aclFormat);
  ...
  if (retInfer != ge::GRAPH_SUCCESS) {
    ACL_LOG_CALL_ERROR("[Infer][ShapeAndType]the op:%s inferShape failed. ge result = %u", opType, retInfer);
    ACL_DELETE_ARRAY_AND_SET_NULL(constData);
