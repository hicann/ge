...
aclError OpModelParser::DeserializeModel(const OpModel &opModel, ge::Model &model)
{
    ...
    if (geRet != ge::SUCCESS) {
        ACL_LOG_CALL_ERROR("[Get][Model]Get MODEL_DEF Partition failed. modelSize = %lu, ge result = %u",
            modelSize, geRet);
        return ACL_ERROR_DESERIALIZE_MODEL;
    }
...
aclError OpModelParser::ParseModelContent(const OpModel &opModel, uint64_t &modelSize, uint8_t *&modelData)
{
    if (opModel.size <= sizeof(ge::ModelFileHeader)) {
        ACL_LOG_INNER_ERROR("[Check][Size]invalid model. length[%u] is smaller than or equal to "
            "ModelFileHeader size[%zu]", opModel.size, sizeof(ge::ModelFileHeader));
    ...
    if ((modelSize + sizeof(ge::ModelFileHeader)) != opModel.size) {
        ACL_LOG_INNER_ERROR("[Check][Length]invalid model. header size = %zu, model size = %lu,"
            "file size = %u", sizeof(ge::ModelFileHeader), modelSize, opModel.size);
        return ACL_ERROR_PARSE_MODEL;
    }
...
aclError OpModelParser::ToModelConfig(ge::Model &model, OpModelDef &modelDef)
{
    ...
    if ((!supportDynamic) && (buildMode == 0)) {
        ACL_LOG_INNER_ERROR("[Check][SupportDynamic]model[%s] does not support dynamic shape",
