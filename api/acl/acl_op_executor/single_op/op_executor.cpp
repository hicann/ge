...
static void ConstructRT2Tensor(const aclTensorDesc *desc, const aclDataBuffer *buffer, gert::Tensor &rt2Tensor) {
  ...
  const gert::TensorPlacement placement =
      desc->memtype == ACL_MEMTYPE_DEVICE ? gert::TensorPlacement::kOnDeviceHbm : gert::TensorPlacement::kOnHost;
...
static void ConstructInputAndOutput(
    const aclTensorDesc *const *desc, const aclDataBuffer *const *buffer, const int32_t num, const size_t filterdNum,
...
static void RefreshOutput(
    const AclOp &aclOp, const size_t filterOutNum,
...
aclError OpExecutor::DoExecuteAsync(ge::SingleOp *const singleOp, const AclOp &aclOp,
  ...
  for (int32_t i = 0; i < aclOp.numInputs; ++i) {
    ...
    buffer.placement =
        ((aclOp.inputDesc[i]->memtype == ACL_MEMTYPE_DEVICE) ? static_cast<uint32_t>(aclOp.inputDesc[i]->memtype)
                                                             : static_cast<uint32_t>(ACL_MEMTYPE_HOST));
  ...
  for (int32_t i = 0; i < aclOp.numOutputs; ++i) {
    ...
    buffer.placement =
        ((aclOp.outputDesc[i]->memtype == ACL_MEMTYPE_DEVICE) ? static_cast<uint32_t>(aclOp.outputDesc[i]->memtype)
                                                              : static_cast<uint32_t>(ACL_MEMTYPE_HOST));
...
aclError OpExecutor::DoExecuteAsync(ge::DynamicSingleOp *const singleOp, const AclOp &aclOp,
                                    const aclDataBuffer *const inputs[], const aclDataBuffer *const outputs[],
                                    const bool executeWithExactModel) {
  size_t inputNum = 0U;
  size_t outputNum = 0U;
...
aclError OpExecutor::DoExecuteAsync(std::shared_ptr<gert::StreamExecutor> &streamExecutor, const AclOp &aclOp,
                                    const aclrtStream stream, const bool executeWithExactModel) {
  ...
  ConstructInputAndOutput(aclOp.inputDesc, aclOp.inputs, aclOp.numInputs, filterInNum, executeWithExactModel,
                          inputTensor, inputTensorPtr);
  ConstructInputAndOutput(aclOp.outputDesc, aclOp.outputs, aclOp.numOutputs, filterOutNum, executeWithExactModel,
                          outputTensor, outputTensorPtr);
  ...
  const ge::graphStatus ret =
      executor->Execute(arg, inputTensorPtr.data(), filterInNum, outputTensorPtr.data(), filterOutNum);
  if (ret != ge::SUCCESS) {
    ACL_LOG_CALL_ERROR("[Exec][Op]Execute op failed. op type = %s, ge result = %u", aclOp.opType.c_str(), ret);
    return ACL_GET_ERRCODE_GE(static_cast<int32_t>(ret));
  }
...
aclError OpExecutor::LoadSingleOp(const OpModel &modelInfo, const aclrtStream stream, ge::SingleOp **const singleOp) {
  ...
  const auto allocator = allocators->GetAllocator(gert::TensorPlacement::kOnDeviceHbm,
                                                  static_cast<size_t>(gert::AllocatorUsage::kAllocNodeWorkspace));
  ...
  const auto status = ge::GeExecutor::LoadSingleOpV2(modelInfo.name, modelData, stream, singleOp, modelInfo.opModelId);
...
aclError OpExecutor::LoadDynamicSingleOp(const OpModel &modelInfo, const aclrtStream stream,
  ...
  const auto status =
      ge::GeExecutor::LoadDynamicSingleOpV2(modelInfo.name, modelData, stream, dynamicSingleOp, modelInfo.opModelId);
  if (status != ge::SUCCESS) {
    ACL_LOG_CALL_ERROR("[Load][Op]Load dynamic operator failed. model = %s, ge result = %u", modelInfo.name.c_str(),
                       status);
    return ACL_GET_ERRCODE_GE(status);
  }
...
aclError OpExecutor::ExecuteAsync(const AclOp &aclOp, const aclDataBuffer *const inputs[],
...
aclError OpExecutor::DoExecuteRT1(const AclOp &aclOp, const aclDataBuffer *const inputs[],
                                  aclDataBuffer *const outputs[], const aclrtStream stream, const OpModel *opModelPtr,
                                  const bool isDynamic, const bool isExactModel) {
  aclError ret;
  if (isDynamic) {
...
aclError OpExecutor::ExecuteAsync(OpHandle &opHandle, const aclDataBuffer *const inputs[],
  ...
  if (opHandle.kernelDesc != nullptr) {
    ...
    auto *const streamExecutor = Executors::GetOrCreate(context, stream);
    ACL_CHECK_MALLOC_RESULT(streamExecutor);
    return streamExecutor->ExecuteAsync(*opHandle.kernelDesc, opHandle.numInputs, inputs, opHandle.numOutputs, outputs);
  }
  aclError ret;
    ...
    ge::DynamicSingleOp *dynamicSingleOp = nullptr;
    {
      ...
      if (it != cachedExecutors.cend()) {
        ...
        if (cachedExecutors.size() > static_cast<size_t>(MAX_CACHED_NUM)) {
          ...
          ACL_LOG_WARN("cache[%zu] reaches max size[%d], evict one object. stream = %p", cachedExecutors.size(),
                       MAX_CACHED_NUM, toErase->first);
    ...
    ge::SingleOp *singleOp = nullptr;
    {
      ...
      if (it != cachedExecutors.end()) {
        ...
        if (cachedExecutors.size() > static_cast<size_t>(MAX_CACHED_NUM)) {
          ...
          ACL_LOG_INFO("cache[%zu] reaches max size[%d], evict one object. stream = %p", cachedExecutors.size(),
                       MAX_CACHED_NUM, toErase->first);
          (void)cachedExecutors.erase(toErase);
        }
