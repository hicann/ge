...
aclError ConstructAclOp(acl::AclOp &aclOp, const char *opType, const int32_t numInputs,
  ...
  if (compileFlag == ACL_COMPILE_UNREGISTERED) {
    if (opPath == nullptr) {
      ACL_LOG_ERROR("[Check][OpPath]opPath cannot be null while compileFlag is %d", static_cast<int32_t>(compileFlag));
...
aclError aclSetCompileopt(aclCompileOpt opt, const char *value) {
  ...
  if (optStr.empty()) {
    ACL_LOG_INNER_ERROR(
        "[Check][Opt]Cannot find any options[%d] valid in enum aclCompileOpt, "
