...
bool AttrValueEquals(const ge::GeAttrValue &lhs, const ge::GeAttrValue &rhs) {
  ...
  if (rhs.GetValueType() != type) {
    ACL_LOG_INFO("type mismatch, lhs type = %d, rhs type = %d", static_cast<int32_t>(type),
                 static_cast<int32_t>(rhs.GetValueType()));
    return false;
  }
...
static bool GetInputData(const aclDataBuffer *const dataBuffer, const aclDataType dataType,
  ...
  switch (dataType) {
    ...
    case ACL_FLOAT16:
      for (size_t i = 0U; i < (dataBuffer->length / sizeof(aclFloat16)); ++i) {
        inputFloatData.push_back(Fp16ToFloat(*(static_cast<const aclFloat16 *>(dataBuffer->data) + i)));
      }
      break;
    case ACL_INT8:
  ...
  ACL_LOG_INFO("Get input data success, dt is %d, int type size is %zu, float type size is %zu", dataType,
               inputIntData.size(), inputFloatData.size());
  return true;
}
...
static bool ConstToAttr(const vector<aclTensorDesc> &tensorDesc, std::vector<std::string> &constStr) {
...
static bool ConstToAttr(const int32_t tensorNum, const aclTensorDesc *const tensorDesc[],
                        std::vector<std::string> &constStr) {
