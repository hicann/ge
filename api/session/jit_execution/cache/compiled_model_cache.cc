...
namespace ge {
CompiledModelCache::CompiledModelCache(uint32_t user_graph_id, CompileContext &context, GraphManager &graph_manager)
    : user_graph_id_(user_graph_id), compile_context_(context), graph_manager_(graph_manager) {
...
Status CompiledModelCache::CreateKeyOptionForGuardedExecutionPoint(const GuardedExecutionPoint *gep,
...
Status CompiledModelCache::SaveCache(ExecutionOrder &order) {
  if (user_graph_key_.empty() || root_dir_.empty()) {
    GELOGI("user_graph_key_ = %s and root_dir_ = %s, cmc will not save caches.", user_graph_key_.c_str(),
           root_dir_.c_str());
    return SUCCESS;
  }
