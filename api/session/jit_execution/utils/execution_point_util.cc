...
static Status ReadGEPListFromFile(const std::string &gep_list_file, GuardedExecutionPointInfoList &gep_info_list) {
  ...
  GE_CHK_STATUS_RET(ModelCache::ReadJsonFile(gep_list_file, json_obj), "Failed to read gep_list file[%s]",
                    gep_list_file.c_str());
  try {
...
Status ExecutionPointUtil::GetGuardedExecutionPointGraphKey(const GuardedExecutionPoint *gep,
                                                            std::string &gep_graph_key) {
  return gep_util_.GetGuardedExecutionPointGraphKey(gep, gep_graph_key);
}

Status ExecutionPointUtil::CreateKeyOptionForGuardedExecutionPoint(const std::string root_dir,
                                                                   const std::string user_graph_key,
...
Status ExecutionPointUtil::RestoreExecutionPoint(const std::string root_dir, const std::string user_graph_key,
  ...
  GE_CHK_BOOL_RET_STATUS(ModelCache::CheckFileExist(gep_list_file_path), FAILED,
                         "Cannot find the gep list file, the path is: %s", gep_list_file_path.c_str());
  ...
  // restore the slice graph and remaining graph from slice_graph_sub_dir
  ...
  GE_CHK_BOOL_RET_STATUS(ModelCache::CheckFileExist(slice_graph_file), FAILED,
                         "The slice graph file[%s] does not exist.", slice_graph_file.c_str());
  ...
  if (ModelCache::CheckFileExist(rem_graph_file)) {
    GE_CHK_GRAPH_STATUS_RET(
        remaining_graph_tmp.LoadFromFile(rem_graph_file.c_str()));  // load .pb file and generate Graph
  ...
  exec_point_ptr =
      MakeUnique<ExecutionPoint>(slice_graph_info.slice_graph_id, slice_graph, remaining_graph, ep_options);
...
Status ExecutionPointUtil::SaveExecutionPoint(const std::string root_dir, const std::string user_graph_key,
  ...
  if (!ModelCache::CheckFileExist(slice_graph_file)) {
    GE_CHK_STATUS_RET(slice_graph.SaveToFile(slice_graph_file.c_str()), "Save slice graphs to pb failed, file_path:%s",
                      slice_graph_file.c_str());
  ...
  GE_CHK_BOOL_RET_STATUS(!geps.empty(), FAILED,
                         "EP with slice_id %ld has newly generated GEPs, but the guard cache is empty.",
                         exec_point_ptr->GetId());
