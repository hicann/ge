...
struct UserGraphExecution {
  ...
  // for RunGraphAsync
  UserGraphExecution(uint32_t graph_id, std::vector<gert::Tensor> &&inputs, const RunAsyncCallbackV2 &callback_func,
                     uint64_t session_id_param)
      : user_graph_id(graph_id),
        callback(callback_func),
        session_id(session_id_param),
        input_tensors_holder(std::move(inputs)),
        external_rt_inputs(&input_tensors_holder) {
...
std::vector<std::pair<K, V>> SortMapByValue(const std::map<K, V> &input_map, bool is_ascend = true) {
  ...
  std::sort(target_vec.begin(), target_vec.end(), [is_ascend](const std::pair<K, V> &a, const std::pair<K, V> &b) {
