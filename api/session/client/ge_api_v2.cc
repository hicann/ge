...
Status GeSession::SetGraphConstMemoryBase(uint32_t graph_id, const void *const memory, size_t size) {
  ...
  if (EnableSliceSchedule()) {
    GELOGE(UNSUPPORTED,
           "[Construct][GeSession]SetGraphConstMemoryBase does not support slice schedule currently, session_id:%lu, "
           "graph_id:%u, memory:%p, size:%zu",
           GetSessionId(), graph_id, memory, size);
    REPORT_INNER_ERR_MSG("E19999",
                         "SetGraphConstMemoryBase does not support slice schedule currently, session_id:%lu, "
                         "graph_id:%u, memory:%p, size:%zu",
                         GetSessionId(), graph_id, memory, size);
    return UNSUPPORTED;
  }
...
Status GeSession::UpdateGraphFeatureMemoryBase(uint32_t graph_id, const void *const memory, size_t size) {
  ...
  if (EnableSliceSchedule()) {
    GELOGE(UNSUPPORTED,
           "[Construct][GeSession]UpdateGraphFeatureMemoryBase does not support slice schedule currently, "
           "session_id:%lu, "
           "graph_id:%u, memory:%p, size:%zu",
           GetSessionId(), graph_id, memory, size);
    REPORT_INNER_ERR_MSG("E19999",
                         "UpdateGraphFeatureMemoryBase does not support slice schedule currently, session_id:%lu, "
                         "graph_id:%u, memory:%p, size:%zu",
                         GetSessionId(), graph_id, memory, size);
    return UNSUPPORTED;
  }
...
Status GeSession::SetGraphFixedFeatureMemoryBaseWithType(uint32_t graph_id, MemoryType type, const void *const memory,
  ...
  if (EnableSliceSchedule()) {
    GELOGE(UNSUPPORTED,
           "[Construct][GeSession]SetGraphFixedFeatureMemoryBaseWithType does not support slice schedule currently, "
           "session_id:%lu, graph_id:%u, type:%d, memory:%p, size:%zu",
           GetSessionId(), graph_id, type, memory, size);
    REPORT_INNER_ERR_MSG(
        "E19999",
        "SetGraphFixedFeatureMemoryBaseWithType does not support slice schedule currently, session_id:%lu, "
        "graph_id:%u, memory:%p, size:%zu",
        GetSessionId(), graph_id, memory, size);
    return UNSUPPORTED;
  }
...
Status GeSession::UpdateGraphRefreshableFeatureMemoryBase(uint32_t graph_id, const void *const memory, size_t size) {
  ...
  if (EnableSliceSchedule()) {
    GELOGE(UNSUPPORTED,
           "[Construct][GeSession]UpdateGraphRefreshableFeatureMemoryBase does not support slice schedule currently, "
           "session_id:%lu, graph_id:%u, memory:%p, size:%zu",
           GetSessionId(), graph_id, memory, size);
    REPORT_INNER_ERR_MSG(
        "E19999",
        "UpdateGraphRefreshableFeatureMemoryBase does not support slice schedule currently, session_id:%lu, "
        "graph_id:%u, memory:%p, size:%zu",
        GetSessionId(), graph_id, memory, size);
    return UNSUPPORTED;
  }
