...
class AddCustomOnline : public CompilableOp, public EagerExecuteOp, public ShapeInferOp {
 public:
  ~AddCustomOnline() {
  ...
  graphStatus Compile(gert::OpCompileContext *ctx) override {
    ...
    char so_tmpl[] = "/tmp/tilelang_online_XXXXXX.so";
    ...
    dlerror();
    auto call_func = reinterpret_cast<CallFunc>(dlsym(handle, kCallFuncName));
    ...
    kernel_entries_[key] = {handle, call_func};
    std::cout << "TileLang kernel compiled and loaded, key=" << key << std::endl;
    return GRAPH_SUCCESS;
  }
  ...
  graphStatus Execute(gert::EagerOpExecutionContext *ctx) override {
    ...
    if (it == kernel_entries_.end()) {
      std::cerr << "Execute: kernel not compiled for key=" << key << ", CompileGraph must be called before execution"
                << std::endl;
      return GRAPH_FAILED;
    }
...
REG_AUTO_MAPPING_OP(AddCustomOnline);
