...
bool GenGraph(Graph &graph) {
  const std::vector<int64_t> shape_data = {8192};
  ...
  data2.update_input_desc_x(desc_data);
  data2.update_output_desc_y(desc_data);

  auto add = op::AnnotatedAddCustom("annotated_add").set_input_x1(data).set_input_x2(data2);

  std::vector<Operator> inputs{data, data2};
