...
CUSTOM_OP_SOURCE_PATH="${CUSTOM_OP_DIR}/add_custom_kernel.cpp"
CUSTOM_OP_PROTO_HEADER_PATH="${OUTPUT_DIR}/op_graph/include/add_custom_ir.h"
...
cmake -S "${OFFLINE_DIR}" -B "${BUILD_DIR}" -DCMAKE_BUILD_TYPE=Release
...
(
  cd "${OUTPUT_DIR}"
  "${BUILD_DIR}/annotated_args_refresh_add_graph_build"
)
...
(
  cd "${BUILD_DIR}"
  ./annotated_args_refresh_add_model_exec "${OM_PATH}"
)
