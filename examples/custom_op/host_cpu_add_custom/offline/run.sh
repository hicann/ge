...
OM_FILE_PATH="${OM_PATH}_$(detect_opp_os_dir)_$(detect_opp_arch_dir).om"
...
run_atc() {
  ...
  if [[ ${atc_rc} -ne 0 ]]; then
    if [[ -s "${OM_FILE_PATH}" ]]; then
      echo "[WARN] atc exited with code ${atc_rc}, but OM was generated: ${OM_FILE_PATH}. Continue pipeline."
      return 0
    fi
    error "ATC failed with exit code ${atc_rc}, and OM file was not generated: ${OM_FILE_PATH}"
    exit "${atc_rc}"
  fi

  if [[ ! -s "${OM_FILE_PATH}" ]]; then
    error "ATC succeeded, but OM file was not generated: ${OM_FILE_PATH}"
    exit 1
  fi

  info "ATC done: ${OM_FILE_PATH}"
}
...
info "Step 2/4: run graph_build to generate AIR"
rm -f "${AIR_PATH}"
(
  cd "${OUTPUT_DIR}"
  "${BUILD_DIR}/single_add_graph_build"
)
...
if [[ ! -f "${OM_FILE_PATH}" ]]; then
  error "OM file was not generated: ${OM_FILE_PATH}"
  exit 1
fi
...
(
  cd "${BUILD_DIR}"
  ./single_add_model_exec "${OM_FILE_PATH}"
)
