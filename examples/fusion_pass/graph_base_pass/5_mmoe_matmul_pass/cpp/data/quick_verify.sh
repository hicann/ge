...
cleanup() {
    installed_so=${CANN_DIR}/opp/vendors/pass_so_dir/custom_fusion_passes/libmmoe_matmul_pass.so
...
echo "========================================"
echo "MmoeMatmulPass 快速验证"
echo "========================================"
echo ""
echo "【测试配置】"
echo "  experts=$EXPERTS (m=64, k=128, n=256)"
echo ""
...
if [ ! -f ${PASS_DIR}/build/libmmoe_matmul_pass.so ]; then
    echo "  Pass未编译，开始自动编译..."
    echo ""
...
atc --model=./model.onnx --framework=5 --output=./model_fused --soc_version=Ascend910B3 2>&1 | grep -E "MmoeMatmulPass|success|fail" | tail -10
...
if [ -n "$pre_run_file" ]; then
    ...
    echo "  MatMul节点数: $matmul_before"
    if [ "$matmul_before" -lt "$EXPERTS" ]; then
        echo "  [FAIL] 融合前图MatMul节点数($matmul_before)少于预期($EXPERTS)，当前模型不符合本样例验证目标"
        exit 1
    fi
else
...
if [ -n "$after_fusion_file" ]; then
    ...
    splitv_count=$(grep -c 'type: "SplitV"' "$after_fusion_file" 2>/dev/null || true)
    splitv_count=${splitv_count:-0}
    concat_count=$(grep -c 'type: "ConcatV2D"' "$after_fusion_file" 2>/dev/null || true)
    concat_count=${concat_count:-0}
    echo "  MatMul节点数: $matmul_after"
    echo "  SplitV节点数: $splitv_count"
    echo "  ConcatV2D节点数: $concat_count"

    if [ "$matmul_after" -eq 1 ] && [ "$splitv_count" -ge 1 ]; then
        echo ""
        echo "  [PASS] 融合验证通过：$EXPERTS 个MatMul已被合并为1个大MatMul + SplitV"
    else
        echo ""
        echo "  [FAIL] 融合验证失败：未发现预期的1个MatMul + SplitV替换"
        exit 1
    fi
else
