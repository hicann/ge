...
# BatchMatMul Flatten Pass 一键式验证脚本
...
echo "========================================"
echo "BatchMatMul Flatten Pass 快速验证"
echo "========================================"
echo ""
echo "【测试配置】"
echo "  Shape: batch=$BATCH, m=$M, k=$K, n=$N"
echo "  batch×m=$((BATCH * M))"
echo "  测试轮数: $TEST_ROUNDS轮"
echo "  每轮执行: 预热50次 + 测试1000次"
echo ""
...
if [ ! -f ${PASS_DIR}/build/libbatch_matmul_flatten_pass.so ]; then
    echo "  Pass未编译，开始自动编译..."
    echo "  预计时间：约10分钟"
    echo ""
...
# ========================================
# 步骤0b：检查并编译benchmark_model
# ========================================
echo "========================================"
echo "步骤0b：检查并编译benchmark_model"
echo "========================================"
...
if [ -n "$after_fusion_file" ]; then
    ...
    reshape_count=$(grep -c "type: \"Reshape\"" "$after_fusion_file" 2>/dev/null || true)
    reshape_count=${reshape_count:-0}
    matmulv2_count=$(grep -c "type: \"MatMulV2\"" "$after_fusion_file" 2>/dev/null || true)
    matmulv2_count=${matmulv2_count:-0}
    batch_matmul_after=$(grep -c "type: \"BatchMatMul" "$after_fusion_file" 2>/dev/null || true)
    batch_matmul_after=${batch_matmul_after:-0}
    echo "  Reshape节点数: $reshape_count"
    echo "  MatMulV2节点数: $matmulv2_count"
    echo "  BatchMatMulV2节点数: $batch_matmul_after"

    if [ "$reshape_count" -ge 2 ] && [ "$matmulv2_count" -ge 1 ] && [ "$batch_matmul_after" -eq 0 ]; then
        echo ""
        echo "  ✓✓✓ 融合验证通过：BatchMatMulV2已被Reshape+MatMulV2+Reshape替换"
    else
        echo ""
        echo "  ✗✗✗ 融合验证失败：未发现预期的Reshape+MatMulV2+Reshape替换"
        exit 1
    fi
else
...
results_file="/tmp/bmm_flatten_verify_results.txt"
echo "" > $results_file
...
if [ -f "$results_file" ]; then
    python3 << PYTHON
results = []
with open('/tmp/bmm_flatten_verify_results.txt', 'r') as f:
    for line in f:
