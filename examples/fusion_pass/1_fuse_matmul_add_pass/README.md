...
本样例为MatMul+Add融合为GEMM自定义pass样例，
...
5. 执行如下命令编译自定义pass so，并将编译后的动态库文件libfuse_matmul_add_pass.so拷贝到自定义融合pass目录下，其中“xxx”为用户自定义目录。
   ```
   make
   cp ./libfuse_matmul_add_pass.so ${ASCEND_PATH}/opp/vendors/xxx/custom_fusion_passes/libfuse_matmul_add_pass.so
   ```
