  ...
  for (size_t i = 0UL; i < ref_out_tensors.size(); i++) {
    ...
    for (auto &tensor : ref_data_tensors[i]) {
      ...
      if (data_shape.GetDims() != out_shape.GetDims()) {
        GELOGI("After infer, While %s %zu output shape [%s] does not match with input shape [%s].Need infer again.",
...
graphStatus UpdateSubGraphDataNodes(const ConstNodePtr &node) {
  ...
  for (const auto &name : sub_graph_names) {
    ...
    if (sub_graph == nullptr) {
      ...
      GE_LOGE("[Get][Graph] cannot find the subgraph %s for node %s", name.c_str(), node->GetName().c_str());
      return GRAPH_FAILED;
    }
