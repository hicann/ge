...
// 因为存量的用例会校验原来的dump图的字段，所以我们设置一个触发tensordesc结构化的下限值
...
void OnnxUtils::AddShapeFormatAndDtypeToProto(const ge::ConstGeTensorDescPtr &desc, const std::string &prefix,
...
void OnnxUtils::AddListAttrProto(const std::string &attr_name, const ::ge::proto::AttrDef &attr_def,
...
bool OnnxUtils::ConvertGeModelToModelProto(const ge::Model &model, onnx::ModelProto &model_proto) {
  ...
  auto dump_level = (dump_ge_graph != nullptr)
                        ? static_cast<ge::DumpLevel>(std::strtol(dump_ge_graph, nullptr, kDecimalBase))
                        : DumpLevel::NO_DUMP;
