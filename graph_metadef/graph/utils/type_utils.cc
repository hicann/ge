...
std::string TypeUtilsInner::ImplyTypeToSerialString(const domi::ImplyType imply_type) {
  ...
  if (it != kImplyTypeToString.end()) {
    ...
    REPORT_INNER_ERR_MSG("E18888", "ImplyTypeToSerialString: imply_type %u is not supported",
                         static_cast<uint32_t>(imply_type));
    GELOGE(GRAPH_FAILED, "[Check][Param] ImplyTypeToSerialString: imply_type %u is not supported",
           static_cast<uint32_t>(imply_type));
    return "UNDEFINED";
  }
}
