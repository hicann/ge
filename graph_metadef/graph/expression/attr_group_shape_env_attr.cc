...
graphStatus ShapeEnvAttr::FindRootExpr(const ge::Expression &expr, ge::Expression &root_expr) const {
  ...
  GE_ASSERT_TRUE(iter != replacements_.end(), "Cannot find replacement of expr: %s",
...
graphStatus ShapeEnvAttr::MergeReplacement(const ge::Expression &expr1, const ge::Expression &expr2) {
...
graphStatus ShapeEnvAttr::AppendSymbolAssertInfo(const ge::Expression &expr, const std::string &file,
                                                 const int64_t line) {
...
graphStatus ShapeEnvAttr::AppendSymbolCheckInfo(const ge::Expression &expr, const std::string &file,
                                                const int64_t line) {
