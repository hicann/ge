...
ExpressionImplPtr ExpressionImpl::Deserialize(const std::string &expr_str) {
  ...
  if (ret == nullptr) {
    GELOGE(ge::FAILED, "Parse expression str %s failed or incomplete.", expr_str.c_str());
    return nullptr;
  }
  ...
  GELOGE(ge::FAILED, "Parse expression str %s abnormal, reserialized to %s, they are not semantically equal.",
         expr_str.c_str(), reserialized.c_str());
  return nullptr;
}
