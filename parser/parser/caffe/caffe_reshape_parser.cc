...
Status CaffeReshapeParser::ParseParams(const Message *op_src, ge::OpDescPtr &op) {
  ...
  if (layer == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Reshape Dynamic cast op_src to LayerParameter failed");
...
Status CaffeReshapeParser::AddConstInput(ge::NodePtr &node) {
  ...
  if (owner_graph == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "node's graph is empty, name: %s", node->GetName().c_str());
  ...
  if (state != ge::GRAPH_SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "op:%s UpdateInputDesc failed.", op->GetName().c_str());
