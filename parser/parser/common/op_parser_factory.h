...
class OpParserRegisterar {
 public:
  ...
  OpParserRegisterar(const domi::FrameworkType framework, const std::string &op_type, OpParserFactory::CREATOR_FUN fun,
    ...
    const auto &fwk_parser_register = OpParserFactory::Instance(framework);
    if (fwk_parser_register != nullptr) {
      fwk_parser_register->RegisterCreator(op_type, fun, is_fusion_op);
    }
  }
  ~OpParserRegisterar() {}
};
