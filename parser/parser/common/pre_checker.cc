...
FMK_FUNC_HOST_VISIBILITY PreChecker &PreChecker::Instance() {
  static thread_local PreChecker instance;
  return instance;
}
...
Status PreChecker::CheckName(OpId id) {
  ...
  for (auto &v : op_map_) {
    ...
    if (id != v.first && info.name == v.second.name) {
      ...
      REPORT_PREDEFINED_ERR_MSG("E13009", std::vector<const char *>({"opname"}),
                                std::vector<const char *>({info.name.c_str()}));
  ...
  if (fmk_op_types_ == nullptr) {
    ...
    if (!domi::OpRegistry::Instance()->GetOmTypeByOriOpType(type, op_type)) {
      ...
      REPORT_PREDEFINED_ERR_MSG("E13010", std::vector<const char *>({"opname", "optype"}),
                                std::vector<const char *>({name.c_str(), type.c_str()}));
      GE_RETURN_WITH_LOG_IF_ERROR(AddCause(id, cause), "[Add][Cause] failed.");
    }
    return SUCCESS;
  }
