...
ge::Status CopyFile(const std::string &src, const std::string &dst) {
  ...
  if (input.is_open() && output.is_open()) {
    ...
    REPORT_INNER_ERR_MSG("E19999", "open [%s] or [%s] file failed.", src.c_str(), dst.c_str());
...
bool SaveIdentifierOpMapInfo(const string &line,  std::map<int, std::pair<string, string>> &identifier_op_map,
  ...
  if (op_param_info[0] != kOptional && op_param_info[0] != kRepeated && op_param_info[0] != kRequired) {
    REPORT_INNER_ERR_MSG("E19999", "Split line[%s] failed.", line.c_str());
...
Status ProtoFileParser::FindConflictLine(const char *proto_file, int identifier,
  ...
  read_file.open(proto_file, std::ios::in);
  if (read_file.fail()) {
    REPORT_INNER_ERR_MSG("E19999", "open file[%s] failed.", proto_file);
  ...
  read_file.close();
  REPORT_INNER_ERR_MSG("E19999", "find line according to identifier[%d] failed.", identifier);
...
Status ProtoFileParser::CombineProtoFileMultiCustomProto(const char *caffe_proto_file, const char *custom_proto_paths,
  ...
  if (!CheckRealPath(caffe_proto_file)) {
    REPORT_INNER_ERR_MSG("E19999", "caffe proto[%s] does not exist.", caffe_proto_file);
    GELOGE(FAILED, "[Check][Param] caffe proto[%s] does not exist.", caffe_proto_file);
    return FAILED;
  }
