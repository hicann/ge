...
Status TensorFlowAutoMappingParserAdapter::ParseParams(const Message *op_src, ge::OpDescPtr &op_dest) {
  if (op_src == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Param op_src is nullptr, check invalid");
  ...
  if (op_dest == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Param op_dest is nullptr, check invalid");
  ...
  if (ret != SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "call auto mapping failed for node:%s", ParserUtils::GetOperatorName(op).c_str());
  ...
  if (op_dest->GetType() == IDENTITYN) {
    ...
    if (!(TensorFlowUtil::FindAttrValue(node, kTfAttrT, attr_num))) {
      GELOGW("In NodeDef %s dynamic attr [%s] does not exist.", op_dest->GetName().c_str(), kTfAttrT);
    }
    ...
    GE_CHK_STATUS_RET(op_dest->AddDynamicOutputDesc("y", dynamic_tensor_num), "AddDynamicOutputDesc failed");
