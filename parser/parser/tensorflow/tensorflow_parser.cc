...
graphStatus aclgrphParseTensorFlow(const char *model_file, ge::Graph &graph) {
  ...
  domi::Status init_ret = domi::SUCCESS;
  ...
  ge::ComputeGraphPtr compute_graph =
      ge::parser::MakeShared<ge::ComputeGraph>("tmpGraph" + std::to_string(ge::parser::GetCurrentTimestamp()));
  if (compute_graph == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "New ComputeGraph failed");
  ...
  if (model_parser == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "No Model Parser for tensorflow, check invalid");
  ...
  std::string trace_log = "call aclgrphParseTensorFlow to parse graph " + ParserUtils::GetGraphName(graph);
  PARSER_TIMESTAMP_EVENT_END(aclgrphParseTensorFlow, trace_log.c_str());
  return ge::SUCCESS;
}
...
graphStatus aclgrphParseTensorFlow(const char *model_file, const std::map<AscendString, AscendString> &parser_params,
  ...
  // Create an empty computegraph
  string graph_name =
      output_name.empty() ? ("tmpGraph" + std::to_string(ge::parser::GetCurrentTimestamp())) : output_name;
...
Status GenSubgraphParseTasks(const ge::ComputeGraphPtr &parent_graph, std::deque<ParseArg> &args) {
  ...
  for (auto &node : parent_graph->GetDirectNode()) {
    ...
    for (const auto &subgraph_name_to_index : op_desc->GetSubgraphNameIndexes()) {
      ...
      if (subgraph == nullptr) {
        REPORT_INNER_ERR_MSG("E19999", "New ComputeGraph failed when create subgraph:%s", subgraph_iname.c_str());
      ...
      if (ret != SUCCESS) {
        REPORT_INNER_ERR_MSG("E19999", "Set subgraph:%s to node:%s(%s) failed, index:%u", subgraph_iname.c_str(),
...
Status PostOpProcessForSubgraph(const ParseArg &arg) {
  ...
  auto post_func = domi::OpRegistry::Instance()->GetParseSubgraphPostFunc(op_type, domi::TENSORFLOW);
  if (post_func == nullptr) {
    GELOGW("The subgraph post func for node %s type %s is null", op_name.c_str(), op_type.c_str());
    if (domi::OpRegistry::Instance()->GetParseSubgraphPostFunc(op_type, domi::TENSORFLOW, parse_func_v2) != SUCCESS ||
  ...
  if (ret != SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "Call ParseSubgraphPostFunc:%s failed, subgraph:%s, node:%s(%s), ret:0x%X",
...
Status MappingAndAddSubGraph(const NodePtr &node, const Graph &graph, const ComputeGraphPtr &root_graph) {
  // Inner function, data format need be set by parent node
  GE_CHK_STATUS_RET(AutoMappingSubgraphDataFormat(node, graph),
                    ...
                    node->GetName().c_str(), root_graph->GetName().c_str(), ParserUtils::GetGraphName(graph).c_str());
  ...
  if (status != SUCCESS) {
    ...
    REPORT_INNER_ERR_MSG("E19999", "Failed to map sub graph input and output, node:%s, sub graph name:%s.",
  ...
  if (ret != GRAPH_SUCCESS) {
    ...
    REPORT_INNER_ERR_MSG("E19999", "Failed to set sub graph, node: %s, sub graph name: %s.", node->GetName().c_str(),
                         compute_graph->GetName().c_str());
    return INTERNAL_ERROR;
  }
  for (const auto &sub_graph : compute_graph->GetAllSubgraphs()) {
    ...
    if (ret != GRAPH_SUCCESS) {
      ...
      REPORT_INNER_ERR_MSG("E19999", "Failed to add sub graph to root graph, node:%s, sub graph name:%s.",
...
Status TensorFlowModelParser::DefunToPartitionedCall(const domi::tensorflow::NodeDef *node_def,
  ...
  for (size_t i = 0; i < input_tensor_num; ++i) {
    ...
    if (op->AddInputDesc(input_tensor) != ge::GRAPH_SUCCESS) {
      REPORT_INNER_ERR_MSG("E19999", "Add input desc to op:%s(%s) failed", op->GetName().c_str(),
                           op->GetType().c_str());
  ...
  for (size_t i = 0; i < output_tensor_num; ++i) {
    ...
    if (op->AddOutputDesc(output_tensor) != ge::GRAPH_SUCCESS) {
      REPORT_INNER_ERR_MSG("E19999", "Add output desc to op:%s(%s) failed", op->GetName().c_str(),
                           op->GetType().c_str());
  ...
  if (ParserUtils::GetOperatorName(op_factory) != node_name || op_type == ge::parser::DATA) {
    if (std::find(kMakeOperatorNotByIr.begin(), kMakeOperatorNotByIr.end(), op_type) != kMakeOperatorNotByIr.end()) {
      ...
      REPORT_PREDEFINED_ERR_MSG("E10501", std::vector<const char *>({"opname", "optype"}),
                                std::vector<const char *>({node_name.c_str(), op_type.c_str()}));
...
Status TensorFlowModelParser::ParseOpParams(const domi::tensorflow::NodeDef *node_def, ge::OpDescPtr &op,
  ...
  if (parse_param_by_op_fn == nullptr) {
    ...
    if (status != SUCCESS) {
      REPORT_INNER_ERR_MSG("E19999", "Auto mapping node_def:%s(%s) to operator failed", node_def->name().c_str(),
                           node_def->op().c_str());
  ...
  ParseOpAttributes(node_def, op);
  return SUCCESS;
}

void TensorFlowModelParser::ParseOpAttributes(const domi::tensorflow::NodeDef *node_def, ge::OpDescPtr &op) {
  const static std::unordered_map<std::string, std::string> kTfToGeRequiredAttrNames = {
      {ATTR_NAME_QOS_SERVICE_LABEL, ATTR_NAME_QOS_SERVICE_LABEL},
      {"_grappler_do_not_remove", ATTR_NAME_DO_NOT_CONSTANT_FOLDING}};
  for (const auto &attr_name : kTfToGeRequiredAttrNames) {
    domi::tensorflow::AttrValue attr;
    if (ge::TensorFlowUtil::FindAttrValue(node_def, attr_name.first, attr)) {
      AutoMappingUtil::ConvertValue(attr_name.second, attr, op);
      GELOGI("Parse attr from [%s] to [%s] for op[%s].", attr_name.first.c_str(), attr_name.second.c_str(),
             op->GetNamePtr());
    }
  }
}
...
Status TensorFlowModelParser::AddNode(const domi::tensorflow::NodeDef *node_def, ge::ComputeGraphPtr &graph,
  ...
  if (needFusion) {
    ...
    if (iter == fusion_op_nodedef_map_.end()) {
      REPORT_INNER_ERR_MSG("E19999", "FusionOp node %s has no children node, check invalid", node_name.c_str());
...
Status TensorFlowModelParser::AddEdges(ge::ComputeGraphPtr &graph) {
  ...
  for (auto &src_iter : op_node_context_map_) {
    ...
    for (auto &src_output_iter : src_output_map) {
      ...
      std::map<std::string, std::vector<std::pair<int32_t, int32_t>>>::const_iterator input_iter =
          dest_input_map.find(src_op_name);
      ...
      if (src_output_iter.second.size() != input_iter->second.size()) {
        REPORT_INNER_ERR_MSG("E19999", "Input size of op[%s]:%zu is not equal to Output size of op[%s]:%zu.",
      ...
      for (auto &outputpair : src_output_iter.second) {
        ...
        if (!control) {
          ...
          if (ge::GraphUtils::AddEdge(out_archor_ptr, in_archor_ptr) != ge::GRAPH_SUCCESS) {
            REPORT_INNER_ERR_MSG("E19999", "Add link from op:%s to op:%s failed", src->GetName().c_str(),
                                 dest->GetName().c_str());
...
Status TensorFlowModelParser::CheckOpShapeDim(const domi::tensorflow::NodeDef *node_def, const std::set<int> &dims,
  ...
  GE_CHK_BOOL_EXEC(input_attr_value.has_list(),
                   REPORT_INNER_ERR_MSG("E19999", "Attr:%s of node_def:%s(%s) is empty, check invalid",
...
Status TensorFlowModelParser::ParseNodeDef(TensorFlowModelParser *parser, ge::ComputeGraphPtr &graph,
  ...
  if (iterator == parser->adaptedOpTypeMap_.cend()) {
    REPORT_INNER_ERR_MSG("E19999", "get adapted op type failed, node name = %s", node_name.c_str());
  ...
  GE_IF_BOOL_EXEC((implyType == domi::ImplyType::TVM) && (op_type != ge::parser::FRAMEWORKOP),
                  GELOGD("TBE %s parsing", node_op.c_str()););
  GE_IF_BOOL_EXEC((implyType == domi::ImplyType::CCE) && (op_type != ge::parser::FRAMEWORKOP),
                  GELOGD("CCE %s parsing", node_op.c_str()););
  GE_IF_BOOL_EXEC((implyType == domi::ImplyType::HCCL) && (op_type != ge::parser::FRAMEWORKOP),
                  GELOGD("HCCL %s parsing", node_op.c_str()););
  GE_IF_BOOL_EXEC(op_type == ge::parser::FRAMEWORKOP, GELOGD("FRAMEWORKOP %s parsing", node_op.c_str()););
  ...
  if (ParserUtils::GetOperatorName(op_factory) != node_name) {
    if (std::find(kMakeOperatorNotByIr.begin(), kMakeOperatorNotByIr.end(), op_type) != kMakeOperatorNotByIr.end()) {
      ...
      REPORT_PREDEFINED_ERR_MSG("E10501", std::vector<const char_t *>({"opname", "optype"}),
                                std::vector<const char_t *>({node_name.c_str(), op_type.c_str()}));
  ...
  if (node == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Add node:%s(%s) to graph:%s failed", op->GetName().c_str(), op->GetType().c_str(),
                         graph->GetName().c_str());
    GELOGE(FAILED, "add node failed.");
    return INTERNAL_ERROR;
  }
...
Status TensorFlowModelParser::AddFmkNode(ge::ComputeGraphPtr &graph, shared_ptr<ge::ScopeGraph> &scope_graph,
  ...
  for (size_t j = 0; j < op_node_list_size; j++) {
    ...
    const domi::tensorflow::NodeDef *node_def = nodedef_map_[op_node_name];
    GE_CHECK_NOTNULL(node_def);
    std::future<Status> f = executor.commit(TensorFlowModelParser::ParseNodeDef, this, graph_tmp, &graphMutex,
                                            scope_graph, node_def, error_message::GetErrMgrContext());
    if (!f.valid()) {
...
Status TensorFlowModelParser::AddNodeToGraphAndMarkFormat(ge::ComputeGraphPtr &graph,
  ...
  for (size_t j = 0; j < op_node_list_size; j++) {
    ...
    if (iterator == node_map_.end()) {
      REPORT_INNER_ERR_MSG("E19999", "node:%s can't find in node_map_, check invalid", op_node_name.c_str());
      GELOGE(FAILED, "add node failed.");
      return INTERNAL_ERROR;
    }
...
Status TensorFlowModelParser::ExcuteScopeFusionPasses(domi::tensorflow::GraphDef *const graph_def,
  ...
  const auto &context_instance = ge::parser::GetParserContextInstance();
  if (!context_instance.enable_scope_fusion_passes.empty()) {
    ge::GetParserContext().enable_scope_fusion_passes = context_instance.enable_scope_fusion_passes;
  }
  ...
  if (impl == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "ScopeFusionPassRegistry is not properly initialized.");
...
Status TensorFlowModelParser::ParseFromMemory(const char *data, uint32_t size, ge::ComputeGraphPtr &graph) {
  ...
  if (!read) {
    REPORT_INNER_ERR_MSG("E19999", "read graph proto from binary failed");
  ...
  for (int i = 0; i < graph_def.node_size(); i++) {
    ...
    GE_CHK_STATUS_EXEC(AddFmkNodeDefToMap(node_def, op_node_name_list), has_error = true, "add node failed.");
  }
  ...
  for (size_t i = 0; i < op_node_name_list.size(); i++) {
    ...
    if (node_def == nullptr) {
      REPORT_INNER_ERR_MSG("E19999", "Node:%s can't find in nodedef_map_, check invalid", op_node_name.c_str());
...
Status TensorFlowModelParser::GetFunctionProto(const string &file,
  ...
  GELOGI("Function def library path is %s.", graph_def_path.c_str());
  ...
  if (!read) {
    ...
    REPORT_PREDEFINED_ERR_MSG("E12029", std::vector<const char *>({}), std::vector<const char *>({}));
    return FAILED;
  }
...
Status TensorFlowModelParser::Parse(const char *model_path, ge::ComputeGraphPtr &root_graph) {
  ...
  while (!tasks.empty()) {
    ...
    if (arg.proto == nullptr) {
      ...
      const std::map<std::string, domi::tensorflow::GraphDef>::const_iterator iter =
          function_name_to_graphdef.find(arg.function_name);
      if (iter == function_name_to_graphdef.end()) {
        REPORT_PREDEFINED_ERR_MSG("E12013", std::vector<const char *>({"functionname"}),
                                  std::vector<const char *>({arg.function_name.c_str()}));
    ...
    GE_ASSERT_NOTNULL(model_parser);
...
// Get the input relation of opnode including input_op and input_const
...
Status TensorFlowModelParser::RunScopeFusionPass(const vector<string> &scope_passes_list,
  ...
  for (auto &pass_name : scope_passes_list) {
    ...
    if (pass == nullptr) {
      REPORT_INNER_ERR_MSG("E19999", "Scope fusion pass[%s] is not registered.", pass_name.c_str());
      GELOGE(INTERNAL_ERROR, "Scope fusion pass[%s] is not registered.", pass_name.c_str());
      return INTERNAL_ERROR;
    }
    Status ret = pass_manager.AddPass(pass);
    if (ret != SUCCESS) {
      REPORT_INNER_ERR_MSG("E19999", "Add scope fusion pass[%s] failed.", pass_name.c_str());
...
bool TensorFlowModelParser::IsFusionOpChild(const string &node_name, ge::ScopeFusionOpInfo *info) {
  GE_CHK_BOOL_EXEC(info != nullptr, REPORT_INNER_ERR_MSG("E19999", "Param info is nullptr, check invalid");
  ...
  for (auto &op_node_name : op_node_name_list) {
    ...
    if (iter != op_node_context_map_.end()) {
      ...
      if (IsFusionOpChild(op_node_name, &info) && nodedef_map_[op_node_name]->op() != TENSORFLOWF_NODE_OP_CONST) {
        ...
        const std::map<std::string, OpNodeContext>::const_iterator fusion_iter =
            tmp_fusion_op_node_context_map.find(info.fusion_node_name);
  ...
  for (auto &iter : normal_op_node_context.input_map) {
    ...
    if (IsFusionOpChild(input_node_name, &from_info)) {
      ...
      for (auto &pair : pairs) {
        ...
        UpdateEdgesControlInfo(info);
        GELOGD("[Update op context] update fusion input map for normal input, %s:%d TO %s:%d", input_node_name.c_str(),
               from_index, info.fusion_node_name.c_str(), to_index);
      }
    }
  }
  return SUCCESS;
...
Status TensorFlowModelParser::ParseProtoWithSubgraph(const google::protobuf::Message *root_proto,
  ...
  while (!tasks.empty()) {
    ...
    if (arg.proto == nullptr) {
      ...
      if (proto == nullptr) {
        REPORT_INNER_ERR_MSG("E19999", "callback execute failed, func_name:%s", arg.function_name.c_str());
  ...
  if (!const_value_map.empty()) {
    for (auto &node : *graph_def.mutable_node()) {
      if (node.op() == "Const") {
        ...
        GE_ASSERT_TRUE(tensor.ParseFromString(iter->second));
        domi::tensorflow::AttrValue attr;
        *attr.mutable_tensor() = tensor;
        node.mutable_attr()->insert({"value", attr});
      }
    }
  }
...
Status TensorFlowModelParser::ParseProtoWithSubgraph(
    ...
    const std::map<ge::AscendString, ge::AscendString> &const_value_map, domi::GetGraphCallbackV3 callback,
    ge::ComputeGraphPtr &root_graph) {
  ...
  for (const auto &const_value_pair : const_value_map) {
    const std::string &key = std::string(const_value_pair.first.GetString(), const_value_pair.first.GetLength());
    const std::string &val = std::string(const_value_pair.second.GetString(), const_value_pair.second.GetLength());
...
Status TensorFlowModelParser::OptimizeSnapShot(domi::tensorflow::NodeDef *curr_mode_def,
  ...
  if (context_iter == op_node_context_map_.end()) {
    REPORT_INNER_ERR_MSG("E19999", "Node:%s can't find in op_node_context_map_, check invalid", curr_node_name.c_str());
...
Status TensorFlowModelParser::GraphDefOptimizeSnapShot(domi::tensorflow::GraphDef *graph_def,
  ...
  for (auto &curr_node_def : nodedef_to_optimize) {
    ...
    if (data_input_cnt != 1) {
      REPORT_INNER_ERR_MSG("E19999", "Node:%s's input data size:%u not equal to 1, check invalid",
...
Status TensorFlowModelParser::GraphDefOptimizeDestroyTemporaryVariable(
  ...
  if (graph_def == nullptr || nodeCurrent == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Param graph_def or nodeCurrent is nullptr, check invalid");
  ...
  if (!clearInputFlag) {
    REPORT_INNER_ERR_MSG("E19999", "Optimize DestroyTemporaryVariable failed, node name is :%s.",
    ...
    for (const string &current_input : current_inputs) {
      ...
      GE_CHK_BOOL_EXEC(node_lookup.count(current_input) > 0U,
                       REPORT_PREDEFINED_ERR_MSG("E10016", std::vector<const char *>({"parameter", "opname"}),
                                                 std::vector<const char *>({"input_shape", current_input.c_str()}));
...
Status TensorFlowModelParser::TrimGraphByOutput(const domi::tensorflow::GraphDef &input_graph_def,
  ...
  while (!current_inputs.empty()) {
    ...
    for (const string &current_input : current_inputs) {
      ...
      GE_CHK_BOOL_EXEC(node_lookup.count(current_input) > 0U,
                       REPORT_PREDEFINED_ERR_MSG("E10016", std::vector<const char *>({"parameter", "opname"}),
                                                 std::vector<const char *>({"out_nodes", current_input.c_str()}));
...
Status TensorFlowModelParser::FusionNodeParseParams(shared_ptr<OpParser> &op_parser,
  ...
  if (iter == fusion_op_nodedef_map_.end()) {
    REPORT_INNER_ERR_MSG("E19999", "Node:%s can't find in fusion_op_nodedef_map_, check invalid",
                         node_def->name().c_str());
  ...
  if (parse_param_func == nullptr) {
    ...
    for (const auto &node_def_src : node_def_v) {
      ...
      if (status != SUCCESS) {
        REPORT_INNER_ERR_MSG("E19999", "Auto mapping node_def:%s(%s) to operator failed", node_def_src->name().c_str(),
...
Status TensorFlowModelParser::OptimizeConstNodes4CustomOp(domi::tensorflow::GraphDef *graph_def) const {
  ...
  for (auto &it_node_map : all_nodedef_map) {
    ...
    const bool is_unknown_custom_op =
        move_input_vec.empty() ||
    ...
    for (const auto &it : move_input_vec) {
      ...
      if (it.inputIdx >= 0) {
        ...
        GE_IF_BOOL_EXEC(
            ...
            REPORT_PREDEFINED_ERR_MSG(
                "E12004", std::vector<const char *>({"opname", "inputIdx", "inputsize"}),
                std::vector<const char *>({current_op_name.c_str(), std::to_string(-it.inputIdx).c_str(),
                                           std::to_string(current_node->input_size()).c_str()}));
            GELOGE(INTERNAL_ERROR,
      ...
      if (it.moveType == domi::RemoveInputType::OMG_REMOVE_TYPE_WITH_COND) {
        ...
        GE_IF_BOOL_EXEC(!(ge::TensorFlowUtil::FindAttrValue(current_node, it.attrName, attr_value)),
                        REPORT_INNER_ERR_MSG("E19999", "Op:%s register AttrName[%s] has no value, check invalid",
  ...
  for (const auto &remove_input : removed_inputs_vec) {
    ...
    if ((input_node_def->op() == parser::SWITCH) || (input_node_def->op() == parser::REFSWITCH)) {
      ...
      remove_input_name = remove_input_name.find(":") == std::string::npos
                              ? input_node_name
                              : (remove_input_name.replace(remove_input_name.find(":"), 1, "_"));
...
Status TensorFlowModelParser::RemoveIsolateNode(domi::tensorflow::GraphDef *graph_def) {
  ...
  for (int i = 0; i < graph_def->node_size(); i++) {
    ...
    if (node_inputs_outputs_map_.find(node_name) == node_inputs_outputs_map_.end()) {
      REPORT_INNER_ERR_MSG("E19999", "Node:%s can't find in node_inputs_outputs_map_, check invalid",
                           node_name.c_str());
...
Status TensorFlowModelParser::UpdateInnerNodeContext(const string &fusion_op_name,
  ...
  if (fusion_iter == op_node_context_map_.end()) {
    REPORT_INNER_ERR_MSG("E19999", "Node:%s can't find in op_node_context_map_, check invalid", fusion_op_name.c_str());
...
Status TensorFlowModelParser::AddScopeInnerNode(TensorFlowModelParser *parser, ge::ComputeGraphPtr &graph,
  ...
  if (iter == parser->scope_inner_node_map_.end()) {
    REPORT_INNER_ERR_MSG("E19999", "Node:%s can't find in scope_inner_node_map_, check invalid", node_name.c_str());
  ...
  for (auto &node : compute_graph->GetDirectNode()) {
    ...
    for (auto &in_anchor : node->GetAllInDataAnchors()) {
      ...
      if ((peer_out_anchor != nullptr) && (in_desc == nullptr)) {
        ...
        if (ret != ge::GRAPH_SUCCESS) {
          REPORT_INNER_ERR_MSG("E19999", "Update index:%d of input desc in op:%s(%s) failed", in_anchor->GetIdx(),
  ...
  for (const NodePtr &node : root_graph->GetAllNodes()) {
    ...
    if (AttrUtils::GetStr(node->GetOpDesc(), kExternalModel, model_data) && !model_data.empty()) {
      ...
      if (load_ret != GRAPH_SUCCESS) {
        ...
        REPORT_INNER_ERR_MSG("E19999", "Failed to parse external model, node:%s.", node->GetName().c_str());
        return INTERNAL_ERROR;
      }
      ...
      if (ret != SUCCESS) {
        ...
        REPORT_INNER_ERR_MSG("E19999", "Failed to map and add sub graph, node:%s.", node->GetName().c_str());
        return INTERNAL_ERROR;
      }
