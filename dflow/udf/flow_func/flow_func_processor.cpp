...
FlowFuncProcessor::FlowFuncProcessor(const std::shared_ptr<FlowFuncParams> &params, const std::string &flow_func_name,
                                     ...
                                     const std::shared_ptr<AsyncExecutor> &async_executor)
    : flow_func_name_(flow_func_name),
      pp_name_(params->GetName()),
      cache_output_data_(all_out_queue_infos.size()),
      writers_(all_out_queue_infos.size()),
      params_(params),
      input_queue_infos_(input_queue_infos),
      all_output_queue_infos(all_out_queue_infos),
      usable_output_indexes_(usable_output_indexes),
...
int32_t FlowFuncProcessor::SetOutput(uint32_t out_idx, const std::shared_ptr<FlowMsg> &out_msg) {
  if (out_idx >= writers_.size()) {
    UDF_LOG_ERROR("out_idx is over output num, flow_func_info=%s, out_idx=%u, output num=%zu.", flow_func_info_.c_str(),
                  out_idx, writers_.size());
  ...
  if (out_msg == nullptr) {
    UDF_LOG_WARN("output msg is null, no need publish, flow_func_info=%s, out_idx=%u.", flow_func_info_.c_str(),
                 out_idx);
    return FLOW_FUNC_SUCCESS;
  }
...
void FlowFuncProcessor::CreateQueueWrapper(const QueueDevInfo &queue_dev_info,
  ...
  if (queue_dev_info.is_proxy_queue) {
    queue_wrapper.reset(
        new (std::nothrow) ProxyQueueWrapper(static_cast<uint32_t>(queue_dev_info.device_id), queue_dev_info.queue_id));
  } else {
    queue_wrapper.reset(new (std::nothrow)
                            QueueWrapper(static_cast<uint32_t>(queue_dev_info.device_id), queue_dev_info.queue_id));
  }
}
...
int32_t FlowFuncProcessor::InitReader() {
  ...
  for (size_t input_idx = 0; input_idx < input_queue_infos_.size(); ++input_idx) {
    ...
    if (input_queue_wrappers[input_idx] == nullptr) {
      UDF_LOG_ERROR("alloc queue_wrapper failed, flow_func_info=%s, is_proxy_queue=%d, queue_id=%u.",
                    flow_func_info_.c_str(), static_cast<int32_t>(input_queue_info.is_proxy_queue),
  ...
  if (align_max_cache_num_ > 0) {
    data_aligner.reset(new (std::nothrow) DataAligner(input_queue_infos_.size(), align_max_cache_num_, align_timeout_,
                                                      drop_when_not_align_));
  ...
  reader_.reset(new (std::nothrow)
                    MbufReader(flow_func_info_, input_queue_wrappers, data_callback, std::move(data_aligner)));
  if (reader_ == nullptr) {
  ...
  for (size_t idx = 0UL; idx < all_output_queue_infos.size(); ++idx) {
    ...
    if (output_queue_wrapper == nullptr) {
      UDF_LOG_ERROR("alloc output queue_wrapper failed, flow_func_info=%s, is_proxy_queue=%d, queue_id=%u.",
                    flow_func_info_.c_str(), static_cast<int32_t>(output_queue_info.is_proxy_queue),
                    output_queue_info.queue_id);
      return FLOW_FUNC_FAILED;
    }
    writers_[idx].reset(new (std::nothrow) MbufWriter(output_queue_wrapper));
    if (writers_[idx].get() == nullptr) {
      UDF_LOG_ERROR("alloc MbufWriter failed, flow_func_info=%s, qid=%u.", flow_func_info_.c_str(),
                    output_queue_info.queue_id);
      return FLOW_FUNC_FAILED;
    }
  ...
  if (params_->GetNeedReportStatusFlag() || params_->GetEnableRaiseException()) {
    ...
    if (status_output_queue_wrapper == nullptr) {
      UDF_LOG_ERROR("alloc status output queue_wrapper failed, flow_func_info=%s, is_proxy_queue=%d, queue_id=%u.",
                    flow_func_info_.c_str(), static_cast<int32_t>(status_output_queue.is_proxy_queue),
...
void FlowFuncProcessor::DumpInputData(const std::string &op_name, const std::vector<MbufTensor *> &tensors,
  ...
  auto fut = async_executor_->Commit(
      [&op_name, &tensors, &step_id]() { return FlowFuncDumpManager::DumpInput(op_name, tensors, step_id); });
  if (fut.get() != FLOW_FUNC_SUCCESS) {
    UDF_LOG_WARN("DumpInput failed, flow_func_info=%s.", flow_func_info_.c_str());
  }
}

void FlowFuncProcessor::DumpOutputData(const std::string &op_name, uint32_t out_idx, const MbufTensor *tensor,
                                       uint32_t step_id) const {
  if (tensor == nullptr) {
...
void FlowFuncProcessor::SetInputData(std::vector<Mbuf *> &data) {
  ...
  if (!empty_inputs.empty()) {
    ...
    if (empty_data_msg == nullptr) {
      UDF_LOG_ERROR("input %s is empty, but all empty msg failed, flow_func_info=%s.", ToString(empty_inputs).c_str(),
                    flow_func_info_.c_str());
      return;
    }
    ...
    UDF_LOG_WARN("input %s is empty, set to empty msg, retCode=%d, flow_func_info=%s.", ToString(empty_inputs).c_str(),
                 FLOW_FUNC_ERR_DATA_ALIGN_FAILED, flow_func_info_.c_str());
  }
...
void FlowFuncProcessor::ProcCallFlowFuncFailed() {
  ...
  if (try_clear_and_suspend_) {
    ...
    UDF_LOG_ERROR("call flow func failed, flow_func_info=%s, trans_id=%lu, ret=%d.", flow_func_info_.c_str(),
...
void FlowFuncProcessor::DumpFlowFuncInfo(bool with_queue_info) const {
  UDF_RUN_LOG_INFO(
      "flow_func_info=%s, status=%d, last trans_id=%lu, statistic info: input ready times=%lu,"
...
void FlowFuncProcessor::DumpFlowFuncQueueInfo() const {
  ...
  for (size_t output_index = 0; output_index < all_output_queue_infos.size(); ++output_index) {
    ...
    UDF_RUN_LOG_INFO(
        "flow_func_info=%s output[%zu] queue info=[queue:%u, device_id:%u, device_type:%u], "
        "set output times=%lu, cached nums=%lu",
        flow_func_info_.c_str(), output_index, output_queue.queue_id, output_queue.deploy_device_id,
        output_queue.device_type, set_output_times_[output_index].load(), cached_nums_[output_index].load());
  }
}
...
void FlowFuncProcessor::SendEventToExecutor(uint32_t event_id, T msg) {
  ...
  if (ret != static_cast<int32_t>(DRV_ERROR_NONE)) {
    UDF_LOG_WARN("Send report status event failed, processor_idx=%u, ret=%d.", processor_idx_,
...
int32_t FlowFuncProcessor::WriteStatusOutputQueue(const ReportStatusMbufGenFunc &report_status_mbuf_gen_func) {
  if (params_->GetNeedReportStatusFlag()) {
    ...
    input_consume_num_++;
    Mbuf *mbuf = nullptr;
    const auto gen_mbuf_ret =
        report_status_mbuf_gen_func(input_queue_infos_, params_->GetModelUuid(), input_consume_num_, mbuf);
