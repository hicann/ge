...
constexpr uint64_t kWaitEventMask =
    (1ULL << UdfEvent::kEventIdProcessorInit) | (1ULL << UdfEvent::kEventIdFlowFuncInit) |
    (1ULL << static_cast<uint32_t>(EVENT_QUEUE_EMPTY_TO_NOT_EMPTY)) |
    (1ULL << static_cast<uint32_t>(EVENT_QUEUE_FULL_TO_NOT_FULL)) | (1ULL << UdfEvent::kEventIdTimer) |
    (1ULL << UdfEvent::kEventIdFlowFuncReportStatus) | (1ULL << UdfEvent::kEventIdNotifyThreadExit) |
    (1ULL << UdfEvent::kEventIdFlowFuncSuspendFinished) | (1ULL << UdfEvent::kEventIdFlowFuncRecoverFinished) |
    (1ULL << UdfEvent::kEventIdSwitchToSoftSchedMode) | (1ULL << UdfEvent::kEventIdRaiseException) |
    (1ULL << UdfEvent::kEventIdSingleFlowFuncInit);

constexpr uint64_t kWorkerWaitEventMask =
    (1ULL << UdfEvent::kEventIdFlowFuncExecute) | (1ULL << UdfEvent::kEventIdNotifyThreadExit);

// invoke model group subscribe event
constexpr uint64_t kInvokeModelWaitEventMask = (1ULL << static_cast<uint32_t>(EVENT_QUEUE_EMPTY_TO_NOT_EMPTY)) |
...
constexpr uint64_t kFlowMsgQueueWaitEventMask = (1ULL << static_cast<uint32_t>(EVENT_QUEUE_EMPTY_TO_NOT_EMPTY));
...
FlowFuncExecutor::FlowFuncExecutor()
    : udf_thread_pool_("udf_worker_"),
      event_proc_func_map_({{EVENT_QUEUE_EMPTY_TO_NOT_EMPTY, &FlowFuncExecutor::ProcessEmptyToNotEmptyEvent},
...
int32_t FlowFuncExecutor::CreateFlowFuncProcessor(const FlowFuncModel &model,
  ...
  for (const auto &input_map : input_maps) {
    ...
    std::shared_ptr<FlowFuncProcessor> flow_func_processor(new (std::nothrow) FlowFuncProcessor(
        params, input_map.first, input_map.second, all_outputs_queue_infos, output_indexes, async_executor_));
    ...
    for (const auto &input_queue_info : input_map.second) {
      ...
      dev_input_queue_map_[input_queue_info.device_id].emplace(input_queue_info.queue_id,
                                                               input_queue_info.is_proxy_queue);
      ...
      std::map<uint32_t, size_t>::const_iterator context_iter =
          input_to_flow_func_processor_idx_.find(input_queue_info.queue_id);
      if (context_iter != input_to_flow_func_processor_idx_.cend()) {
        UDF_LOG_ERROR("flow_func_name[%s]'s input_queue_id=%u is used by flow func processor %zu",
                      input_map.first.c_str(), input_queue_info.queue_id, context_iter->second);
        return FLOW_FUNC_FAILED;
      }
    ...
    for (const auto output_index : output_indexes) {
      ...
      dev_output_queue_map_[output_queue_info.device_id].emplace(output_queue_info.queue_id,
                                                                 output_queue_info.is_proxy_queue);
    ...
    UDF_RUN_LOG_INFO("Flow func[instance_name:%s, funcName:%s] IO info: inputQs=%s, outputQs=%s.",
                     instance_name.c_str(), input_map.first.c_str(), ToString(input_map.second).c_str(),
...
int32_t FlowFuncExecutor::CheckInputOutputMapsValid(const std::map<std::string, std::vector<uint32_t>> &input_maps,
  ...
  for (auto iter = input_maps.begin(); iter != input_maps.end(); iter++) {
    for (auto index : iter->second) {
      if (index >= inputs_num) {
        UDF_LOG_ERROR("func name[%s]'s input map index[%u] is invalid, valid range is [0, %u).", iter->first.c_str(),
                      index, inputs_num);
        return FLOW_FUNC_FAILED;
      }
    }
  ...
  for (auto iter = output_maps.begin(); iter != output_maps.end(); iter++) {
    for (auto index : iter->second) {
      if (index >= outputs_num) {
        UDF_LOG_ERROR("func name[%s]'s output map index[%u] is invalid, valid range is [0, %u).", iter->first.c_str(),
                      index, outputs_num);
        return FLOW_FUNC_FAILED;
      }
    }
  ...
  if (multi_func_input_maps.empty()) {
    ...
    for (auto &input_maps : multi_func_input_maps) {
      ...
      std::transform(input_maps.second.cbegin(), input_maps.second.cend(), std::back_inserter(queue_infos),
                     [&input_queue_infos](uint32_t index) { return input_queue_infos[index]; });
...
int32_t FlowFuncExecutor::GetUsrInvokedModelKey(const std::string &name_with_scope, const std::string &scope,
  ...
  UDF_LOG_ERROR("Get the usr invoked model key failed, dataFlowScope=%s, invoked model key=%s.", scope.c_str(),
                name_with_scope.c_str());
  return FLOW_FUNC_FAILED;
}
...
int32_t FlowFuncExecutor::CreateFlowModel(const std::shared_ptr<FlowFuncParams> &params, const FlowFuncModel &model) {
  ...
  for (const auto &invoked_model_queue_info : invokedModelQueueInfos) {
    std::string invoke_name;
    if (GetUsrInvokedModelKey(invoked_model_queue_info.first, model.GetDataFlowScope(), invoke_name) !=
        FLOW_FUNC_SUCCESS) {
      UDF_LOG_ERROR("GetUsrInvokedModelKey failed, name=%s, flow_func_name=%s, invoked model key=%s.", name.c_str(),
                    flow_func_name.c_str(), invoked_model_queue_info.first.c_str());
      return FLOW_FUNC_FAILED;
    }
    std::unique_ptr<DataAligner> dataAligner;
    if (input_align_attrs.align_max_cache_num > 0) {
      dataAligner.reset(new (std::nothrow) DataAligner(
          invoked_model_queue_info.second.fetch_queue_infos.size(), input_align_attrs.align_max_cache_num,
          input_align_attrs.align_timeout, input_align_attrs.drop_when_not_align));
      if (dataAligner == nullptr) {
        UDF_LOG_ERROR("alloc DataAligner failed, name=%s, flow_func_name=%s.", name.c_str(), flow_func_name.c_str());
        return FLOW_FUNC_FAILED;
      }
    }
    std::unique_ptr<FlowModel> impl(new (std::nothrow) FlowModelImpl(invoked_model_queue_info.second.feed_queue_infos,
                                                                     invoked_model_queue_info.second.fetch_queue_infos,
                                                                     std::move(dataAligner)));
    if (impl == nullptr) {
      UDF_LOG_ERROR("alloc FlowModelImpl failed, name=%s, flow_func_name=%s, invoked model key=%s.", name.c_str(),
                    flow_func_name.c_str(), invoke_name.c_str());
      return FLOW_FUNC_FAILED;
    }
    params->AddFlowModel(invoke_name, std::move(impl));
    UDF_RUN_LOG_INFO("Flow func(ppName:%s) invoke model IO info: invoked key=%s, feedQs=%s, fetchQs=%s.", name.c_str(),
                     invoke_name.c_str(), ToString(invoked_model_queue_info.second.feed_queue_infos).c_str(),
...
int32_t FlowFuncExecutor::Init(const std::vector<std::unique_ptr<FlowFuncModel>> &models) {
  ...
  for (size_t model_idx = 0UL; model_idx < models.size(); ++model_idx) {
    ...
    std::shared_ptr<FlowFuncParams> params =
        MakeShared<FlowFuncParams>(name, input_queue_infos.size(), output_queue_infos.size(), running_device_id,
                                   GlobalConfig::Instance().GetDeviceId());
    if (params == nullptr) {
      UDF_LOG_ERROR("Failed to create params, name[%s], output queue size[%zu].", name.c_str(),
                    output_queue_infos.size());
      return FLOW_FUNC_FAILED;
    }
    ...
    if (CreateFlowFuncProcessor(*model, real_input_queue_maps, multi_func_output_maps, output_queue_infos, params) !=
        FLOW_FUNC_SUCCESS) {
      UDF_LOG_ERROR("create multi func processor failed, name=%s, flow_func_name=%s", name.c_str(),
    ...
    if (!GlobalConfig::Instance().IsOnDevice()) {
      ...
      if (model->GetCpuNumFromAttr(cpu_num, is_attr_get) != FLOW_FUNC_SUCCESS) {
        UDF_LOG_ERROR("get cpu num from attr fail, name=%s, flow_func_name=%s.", name.c_str(), flow_func_name.c_str());
        return FLOW_FUNC_FAILED;
      }
      if (is_attr_get) {
    ...
    UDF_RUN_LOG_INFO(
        "[UdfModelEschedPriority]flow func init end, name=%s, flow_func_name=%s, idx=%zu, "
        "process priority=%d, event priority=%d.",
        name.c_str(), flow_func_name.c_str(), model_idx, esched_process_priority_, esched_event_priority_);
  }
...
int32_t FlowFuncExecutor::SetExecutorEschedPriority() const {
  ...
  if (esched_process_priority_ != kUserUnsetESchedPriority) {
    const drvError_t drv_ret =
        halEschedSetPidPriority(device_id, static_cast<SCHEDULE_PRIORITY>(esched_process_priority_));
    if (drv_ret != DRV_ERROR_NONE) {
      UDF_LOG_ERROR("Failed to set pid priority, device_id=%u, priority=%d, drv_ret=%d.", device_id,
...
int32_t FlowFuncExecutor::Start() {
  ...
  auto ret = udf_thread_pool_.Init([this](uint32_t thread_idx) { ThreadLoop(thread_idx); }, cpu_num_);
  if (ret != FLOW_FUNC_SUCCESS) {
  ...
  statistic_timer_handle_ = FlowFuncTimer::Instance().CreateTimer([this]() { DumpMetrics(); });
...
void FlowFuncExecutor::CheckReplenishSchedule() {
  for (size_t idx = 0UL; idx < func_processors_.size(); ++idx) {
    if (func_processors_[idx]->NeedReplenishSchedule()) {
      UDF_LOG_INFO("processor[%s] need replenish schedule event", func_processors_[idx]->GetFlowFuncInfo().c_str());
...
int32_t FlowFuncExecutor::SubscribeInvokeModelEvent(uint32_t thread_idx) const {
  ...
  drvError_t ret =
      halEschedSubscribeEvent(device_id, invoke_model_sched_group_id, thread_idx, kInvokeModelWaitEventMask);
  if (ret != DRV_ERROR_NONE) {
  ...
  if (ret != DRV_ERROR_NO_EVENT) {
    UDF_LOG_ERROR(
        ...
        thread_idx, kInvokeModelWaitEventMask);
    return FLOW_FUNC_ERR_DRV_ERROR;
  }
  return FLOW_FUNC_SUCCESS;
}
...
int32_t FlowFuncExecutor::SubscribeFlowMsgQueueEvent(uint32_t thread_idx) const {
  ...
  drvError_t ret =
      halEschedSubscribeEvent(device_id, flow_msg_queue_sched_group_id, thread_idx, kFlowMsgQueueWaitEventMask);
  if (ret != DRV_ERROR_NONE) {
    UDF_LOG_ERROR("halEschedSubscribeEvent failed, device_id[%u], group_id[%u], thread_idx[%u] eventBitmap[%lu].",
  ...
  if (ret != DRV_ERROR_NO_EVENT) {
    UDF_LOG_ERROR(
        ...
        thread_idx, kFlowMsgQueueWaitEventMask);
    return FLOW_FUNC_ERR_DRV_ERROR;
  }
  return FLOW_FUNC_SUCCESS;
}
...
void FlowFuncExecutor::ThreadLoop(uint32_t thread_idx) {
  ...
  UDF_RUN_LOG_INFO("thread[%u] subscribe event end, sched_group_id=%u, eventBitmap=%lu.", thread_idx, sched_group_id,
                   wait_event_mask);
  ...
  while (running_) {
    ...
    if (sched_ret == DRV_ERROR_NONE) {
      ...
      if ((timeout_times % 10U) == 0U) {
        UDF_LOG_DEBUG("wait event timeout,thread index=%u, continuous timeout times=%u.", thread_idx, timeout_times);
      }
      ++timeout_times;
      ...
      // LOG ERROR
      UDF_LOG_ERROR("wait event failed, device_id=%u, threadIndex=%u, group_id=%u, sched_ret=%d.", device_id,
                    thread_idx, sched_group_id, static_cast<int32_t>(sched_ret));
    }
  }
  ...
  for (auto &flow_funcs_processor : func_processors_) {
    ...
    if (ret != FLOW_FUNC_SUCCESS) {
      UDF_LOG_ERROR("flow_func_processor init failed, flowFuncInfo=%s, ret=%d.",
                    flow_funcs_processor->GetFlowFuncInfo().c_str(), ret);
      Stop();
      return;
    }
  }
...
void FlowFuncExecutor::ProcessSingleFlowFuncInitEvent(const struct event_info &event, uint32_t thread_idx) {
  ...
  if (ret == FLOW_FUNC_ERR_INIT_AGAIN) {
    ret = SubmitEvent(GlobalConfig::Instance().GetMainSchedGroupId(), UdfEvent::kEventIdSingleFlowFuncInit,
  ...
  if (ret != FLOW_FUNC_SUCCESS) {
    UDF_LOG_ERROR("flow_func_processor init flow func failed, flowFuncInfo=%s, ret=%d",
                  flow_func_processor->GetFlowFuncInfo().c_str(), ret);
    Stop();
    return;
  }
  ...
  if (ret != FLOW_FUNC_SUCCESS) {
    UDF_LOG_ERROR("ScheduleFlowFunc failed, ret=%d, idx=%zu, flowFuncInfo=%s.", ret, flow_func_processor_idx,
...
void FlowFuncExecutor::ProcessFlowFuncInitEvent(const struct event_info &event, uint32_t thread_idx) {
  ...
  for (size_t idx = 0UL; idx < func_processors_.size(); ++idx) {
    ...
    if (ret != FLOW_FUNC_SUCCESS) {
      UDF_LOG_ERROR("ScheduleFlowFunc failed, ret=%d, idx=%zu, flowFuncInfo=%s.", ret, idx,
    ...
    for (const auto &queue : dev_input.second) {
      ...
      if (!is_proxy_queue) {
        ...
        drv_ret = halQueueSubscribe(queue_device_id, queue_id, GlobalConfig::Instance().GetMainSchedGroupId(),
                                    static_cast<int32_t>(QUEUE_TYPE_SINGLE));
        if (drv_ret != DRV_ERROR_NONE) {
          UDF_LOG_ERROR("Failed to subscribe event for queue[%u], ret[%d].", queue_id, static_cast<int32_t>(drv_ret));
          return FLOW_FUNC_ERR_QUEUE_ERROR;
        }
...
int32_t FlowFuncExecutor::SubscribeStatusOutputQueue() const {
  for (const auto &status_output_queue : status_output_queue_map_) {
    ...
    for (const auto queue_id : status_output_queue.second) {
      ...
      if (drv_ret != DRV_ERROR_NONE) {
        UDF_LOG_ERROR("attached status output queue[%u] failed, ret[%d], queue_device_id[%d]", queue_id,
                      static_cast<int32_t>(drv_ret), queue_device_id);
        return FLOW_FUNC_ERR_QUEUE_ERROR;
      }
...
void FlowFuncExecutor::ProcessFlowFuncExecuteEvent(const struct event_info &event, uint32_t thread_idx) {
  ...
  UDF_LOG_DEBUG("Flow func execute event begin, flow_func_processor_idx=%zu, thread_idx=%u.", flow_func_processor_idx,
                thread_idx);
  if (flow_func_processor_idx >= func_processors_.size()) {
    UDF_LOG_ERROR("FlowFuncExecute event invalid, flow_func_processor_idx=%zu.", flow_func_processor_idx);
    return;
...
void FlowFuncExecutor::ProcessEmptyToNotEmptyEvent(const struct event_info &event, uint32_t thread_idx) {
  ...
  if (flow_func_idx >= func_processors_.size()) {
    UDF_LOG_ERROR("flow_func_idx is invalid, queue_id=%u, flow_func_idx=%zu, funcProcessors size=%zu", queue_id,
                  flow_func_idx, func_processors_.size());
    return;
  }
...
int32_t FlowFuncExecutor::ProcessExceptionMsg(const ff::deployer::ExecutorRequest_DataflowExceptionNotify &exp_msg) {
  ...
  if (exp_msg.type() == static_cast<uint32_t>(ExceptionType::kAddException)) {
    ...
    auto ret = memcpy_s(exp_info.exp_context, kMaxMbufHeadLen, exp_msg.exception_context().data(),
                        exp_msg.exception_context().size());
    if (ret != EOK) {
      UDF_LOG_ERROR(
          "Copy exception info from message failed. ret=%d, trans_id[%lu], exp_code[%d],"
          "user_context_id[%lu]",
          static_cast<int32_t>(ret), exp_info.trans_id, exp_info.exp_code, exp_info.user_context_id);
      return FLOW_FUNC_FAILED;
    }
    ...
    for (size_t i = 0UL; i < func_processors_.size(); ++i) {
      ...
      if (ret != FLOW_FUNC_SUCCESS) {
        UDF_LOG_ERROR("Schedule func failed for trans_id[%lu], exp_code[%d], user_context_id[%lu]", exp_info.trans_id,
                      exp_info.exp_code, exp_info.user_context_id);
        return ret;
      }
    }
...
void FlowFuncExecutor::ProcessFullToNoFullEvent(const struct event_info &event, uint32_t thread_idx) {
  ...
  std::map<uint32_t, std::vector<size_t>>::const_iterator context_iter =
      output_to_flow_func_processor_idx_.find(queue_id);
  ...
  for (auto flow_func_idx : context_iter->second) {
    if (flow_func_idx >= func_processors_.size()) {
      UDF_LOG_ERROR("flow_func_idx is invalid, queue_id=%u, flow_func_idx=%zu, flow_func_processor size=%zu", queue_id,
                    flow_func_idx, func_processors_.size());
      return;
    }
    auto flow_func_processor = func_processors_[flow_func_idx];
...
void FlowFuncExecutor::ConstructException(const std::string &current_scope, const UdfExceptionInfo &exception_info,
  ...
  UDF_LOG_INFO("Construct exception message: trans_id[%lu] exp_code[%d] scope[%s] success.", exception_info.trans_id,
               exception_info.exp_code, current_scope.c_str());
}

void FlowFuncExecutor::ProcessRaiseExceptionEvent(const struct event_info &event, uint32_t thread_idx) {
  auto flow_func_processor_idx = static_cast<size_t>(event.comm.subevent_id);
  UDF_LOG_DEBUG("Raise exception event begin, flow_func_processor_idx=%zu, thread_idx=%u.", flow_func_processor_idx,
                thread_idx);
  if (flow_func_processor_idx >= func_processors_.size()) {
...
void FlowFuncExecutor::ProcessReportStatusEvent(const struct event_info &event, uint32_t thread_idx) {
  auto flow_func_processor_idx = static_cast<size_t>(event.comm.subevent_id);
  UDF_LOG_DEBUG("Report status event begin, flow_func_processor_idx=%zu, thread_idx=%u.", flow_func_processor_idx,
                thread_idx);
  if (flow_func_processor_idx >= func_processors_.size()) {
  ...
  if (ret != FLOW_FUNC_SUCCESS) {
    UDF_LOG_ERROR("Report status failed, flow_func_processor_idx=%zu, thread_idx=%u.", flow_func_processor_idx,
                  thread_idx);
    Stop();
    return;
  }
  UDF_LOG_DEBUG("Report status event end, flow_func_processor_idx=%zu, thread_idx=%u.", flow_func_processor_idx,
                thread_idx);
}
...
int32_t FlowFuncExecutor::ReportStatus(const size_t flow_func_processor_idx) {
  ReportStatusMbufGenFunc mbuf_gen_func = [this](const std::vector<QueueDevInfo> &input_queue_infos,
                                                 const uint32_t model_uuid, const uint32_t input_consume_sum,
                                                 Mbuf *&mbuf_to_generate) -> int32_t {
    ...
    for (const auto input_queue_info : input_queue_infos) {
      ...
      if (drv_ret != DRV_ERROR_NONE) {
        UDF_LOG_WARN("query queue info failed, queue id[%u], device id[%u], ret[%d].", input_queue_id,
                     input_queue_info.device_id, static_cast<int32_t>(drv_ret));
      } else {
    ...
    UDF_LOG_INFO("Generate report status mbuf success, status[%s].", sub_model_status.DebugString().c_str());
    return FLOW_FUNC_SUCCESS;
  };
...
void FlowFuncExecutor::ProcessReportSuspendEvent(const struct event_info &event, uint32_t thread_idx) {
  ...
  UDF_LOG_INFO("Process suspend finish event, flow_func_processor_idx=%zu, thread_idx=%u.", flow_func_processor_idx,
               thread_idx);
}
...
void FlowFuncExecutor::ProcessReportRecoverEvent(const struct event_info &event, uint32_t thread_idx) {
  ...
  UDF_LOG_INFO("Process recover finish event end, flow_func_processor_idx=%zu, thread_idx=%u.", flow_func_processor_idx,
               thread_idx);
}
