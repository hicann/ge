...
int32_t FlowFuncModel::CheckBufCfgItem(const BufferConfigItem &item) {
  if ((item.total_size == 0U) || (item.blk_size == 0U) || (item.max_buf_size == 0U)) {
    UDF_LOG_ERROR(
        "Any of the following configuration items equal to 0 is not allowed. "
        "total size[%u] blk size[%u] or max buf size[%u].",
        item.total_size, item.blk_size, item.max_buf_size);
    return FLOW_FUNC_FAILED;
  }
  if ((item.total_size <= item.max_buf_size) || (item.max_buf_size < item.blk_size)) {
    UDF_LOG_ERROR(
        "The following three configuration parameters must meet the requirements: "
        "Total size[%u] > max_buf_size[%u] >= blk_size[%u].",
  ...
  if (item.page_type == kPageTypeNormal) {
    ...
    if (item.total_size % kHugePageAtomicValue != 0) {
      UDF_LOG_ERROR(
          "Total size[%u] is expected as an integer multiple of 2 * 1024 * 1024(2M) "
          "for huge page type.",
          item.total_size);
      return FLOW_FUNC_FAILED;
    }
  ...
  if (item.total_size % item.blk_size != 0U) {
    UDF_LOG_ERROR("Total size[%u] is expected as an integer multiple of blk size[%u]", item.total_size, item.blk_size);
    return FLOW_FUNC_FAILED;
  }
  return FLOW_FUNC_SUCCESS;
}
...
int32_t FlowFuncModel::CheckAndSetBufCfg(const ff::udf::UdfDef &udf_def) {
  ...
  for (int32_t i = 0; i < udf_def.user_buf_cfg_size(); ++i) {
    ...
    BufferConfigItem item = {.total_size = cfg.total_size(),
    ...
    if (item.page_type == kPageTypeNormal) {
      if (item.max_buf_size <= last_normal_max_buf_size) {
        UDF_LOG_ERROR(
            "Normal page type buf cfg should be sorted. But current size[%u] is "
            "less or equal to previous gear size[%u].",
    ...
    if (item.page_type == kPageTypeHuge) {
      if (item.max_buf_size <= las_huge_max_buf_size) {
        UDF_LOG_ERROR(
            "Huge page type buf cfg should be sorted. But current size[%u] is "
            "less or equal to previous gear size[%u].",
            item.max_buf_size, las_huge_max_buf_size);
        return FLOW_FUNC_FAILED;
      }
...
int32_t FlowFuncModel::ParseUdfBasicInfos(const ff::udf::UdfDef &udf_def, const std::string &real_model_path) {
  ...
  if (bin_name == kUdfBuiltInBinName) {
    ...
    work_path_ = "";
    lib_path_ = bin_name;
    UDF_LOG_INFO("%s is built in lib, funcName=%s, name=%s.", bin_name.c_str(), flow_func_name_.c_str(), name_.c_str());
  } else {
...
int32_t FlowFuncModel::Init() {
  ...
  for (auto &proto_attr : proto_attrs) {
    try {
      ...
      UDF_LOG_ERROR("new AttrValueImpl failed, funcName=%s, model_path=%s, error=%s.", flow_func_name_.c_str(),
                    model_param_.model_path.c_str(), e.what());
      return FLOW_FUNC_FAILED;
    }
  }
...
int32_t FlowFuncModel::ParseModelAttrs(const ff::deployer::ExecutorRequest_LoadModelRequest &model_request) {
  ...
  if (attrs.contains(kModelEschedProcessPriorityAttrName)) {
    ...
    UDF_LOG_DEBUG("Parse attr:%s, value:%d", kModelEschedProcessPriorityAttrName, model_esched_process_priority_);
  }
  if (attrs.contains(kModelEschedEventPriorityAttrName)) {
    ...
    UDF_LOG_DEBUG("Parse attr:%s, value:%d", kModelEschedEventPriorityAttrName, model_esched_event_priority_);
  }
  if (model_request.has_input_align_attrs()) {
    ...
    if ((input_align_attrs_.align_timeout != align_never_timeout) &&
      ...
      UDF_LOG_ERROR("align_timeout=%d is invalid, must be -1 or in range(0, %d]ms.", input_align_attrs_.align_timeout,
                    align_timeout_max_value);
      return FLOW_FUNC_FAILED;
    }
...
int32_t FlowFuncModel::ConstructModelParam(const ff::deployer::ExecutorRequest_LoadModelRequest &model,
  ...
  if (flow_func_model_param.need_report_status || (model.enable_exception_catch())) {
    ...
    const auto check_ret =
        CheckAndConvert(model.status_queues().output_queues_attrs(0), flow_func_model_param.status_output_queue);
...
std::unique_ptr<FlowFuncModel> FlowFuncModel::ParseSingleModel(
    const ff::deployer::ExecutorRequest_LoadModelRequest &model) {
  FlowFuncModelParam flow_func_model_param = {};
  ...
  for (const auto &invoked_model_queue_attr : model.invoked_model_queues_attrs()) {
    ...
    UDF_RUN_LOG_INFO("Invoke queue size[%zu] after deduplicate is [%zu].", feed_queue_infos.size(),
                     real_feed_queue_infos.size());
...
int32_t FlowFuncModel::CheckAndConvert(const ff::deployer::ExecutorRequest_ModelQueuesAttrs &model_queue_attrs,
                                       std::vector<QueueDevInfo> &input_queue_infos,
                                       std::vector<QueueDevInfo> &output_queue_infos) {
...
int32_t FlowFuncModel::GetCpuNumFromAttr(uint32_t &cpu_num, bool &is_attr_get) const {
  ...
  if (attr_iter == attr_map_.cend() || attr_iter->second == nullptr) {
    UDF_LOG_INFO("no attr found, attrName=%s, funcName=%s, name=%s.", kModelCpuNumAttrName, flow_func_name_.c_str(),
                 name_.c_str());
    is_attr_get = false;
    return FLOW_FUNC_SUCCESS;
  }
  ...
  if (ret != FLOW_FUNC_SUCCESS) {
    UDF_LOG_ERROR("get attr val fail, attrName=%s, funcName=%s, name=%s.", kModelCpuNumAttrName,
                  flow_func_name_.c_str(), name_.c_str());
    return ret;
  }
  if (attr_cpu_num <= 0L || attr_cpu_num >= static_cast<int64_t>(UINT32_MAX)) {
    UDF_LOG_ERROR("attr %s val %ld is invalid, funcName=%s, name=%s.", kModelCpuNumAttrName, attr_cpu_num,
                  flow_func_name_.c_str(), name_.c_str());
    return FLOW_FUNC_ERR_PARAM_INVALID;
  }
  is_attr_get = true;
...
int32_t FlowFuncModel::GetVisibleDeviceEnableFromAttr(bool &visible_device_enable) const {
  ...
  if (attr_iter != attr_map_.cend() && attr_iter->second != nullptr) {
    ...
    if (ret != FLOW_FUNC_SUCCESS) {
      UDF_LOG_ERROR("get attr val fail, attrName=%s, funcName=%s, name=%s.", kAttrNameDataFlowVisibleDeviceEnable,
                    flow_func_name_.c_str(), name_.c_str());
      return ret;
    }
    ...
    UDF_LOG_INFO("attr found, attrName=%s, val=%d, funcName=%s, name=%s.", kAttrNameDataFlowVisibleDeviceEnable,
                 static_cast<int32_t>(visible_device_enable), flow_func_name_.c_str(), name_.c_str());
  }
  return FLOW_FUNC_SUCCESS;
}
...
int32_t FlowFuncModel::EnableVisibleDeviceId() const {
  ...
  if (ret != FLOW_FUNC_SUCCESS) {
    UDF_LOG_ERROR("get visible device enable attr fail, funcName=%s, name=%s.", flow_func_name_.c_str(), name_.c_str());
    return ret;
  }
...
int32_t FlowFuncModel::SetDataFlowScope() {
  ...
  if (attr_iter != attr_map_.cend() && attr_iter->second != nullptr) {
    ...
    if (ret != FLOW_FUNC_SUCCESS) {
      UDF_LOG_ERROR("get attr val fail, attrName=%s, funcName=%s, name=%s.", kAttrNameDataFlowDataFlowScope,
                    flow_func_name_.c_str(), name_.c_str());
      return ret;
    }
    data_flow_scope_ = data_flow_scope.GetString();
    UDF_LOG_INFO("attr found, attrName=%s, val=%s, funcName=%s, name=%s.", kAttrNameDataFlowDataFlowScope,
                 data_flow_scope_.c_str(), flow_func_name_.c_str(), name_.c_str());
  }
  return FLOW_FUNC_SUCCESS;
}
...
int32_t FlowFuncModel::SetDataFlowInvokeScopes() {
  ...
  if (attr_iter != attr_map_.cend() && attr_iter->second != nullptr) {
    ...
    if (ret != FLOW_FUNC_SUCCESS) {
      UDF_LOG_ERROR("get attr val fail, attrName=%s, funcName=%s, name=%s.", kAttrNameDataFlowDataFlowInvokeScopes,
                    flow_func_name_.c_str(), name_.c_str());
      return ret;
    }
    ...
    UDF_LOG_INFO("attr found, attrName=%s, size=%zu, funcName=%s, name=%s.", kAttrNameDataFlowDataFlowInvokeScopes,
                 invoked_scopes_.size(), flow_func_name_.c_str(), name_.c_str());
  }
  return FLOW_FUNC_SUCCESS;
}
}  // namespace FlowFunc
