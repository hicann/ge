...
void RewriteQueueName(const std::string &model_name, const std::map<std::string, std::string> &name_map,
...
void RewriteQueueNames(const std::string &model_name, const std::map<std::string, std::string> &name_map,
...
Status RewriteQueueNames(const std::string &model_name, const ModelRelation::ModelEndpointInfo &model_queue_info,
                         ModelRelation &model_relation, std::vector<Endpoint> &internal_queue_defs) {
  ...
  std::for_each(model_relation.endpoints.begin(), model_relation.endpoints.end(),
...
Status HeterogeneousDeployPlanner::SelectTargetDevice(
  ...
  for (const auto &it : name_to_models) {
    ...
    GE_CHK_BOOL_RET_STATUS(!available_devices.empty(), FAILED, "No available device for engine: %s",
                           it.second->GetModelType().c_str());
    GE_CHK_STATUS_RET(AssignDevices(model_name, it.second, engine_type, available_devices, target_devices),
...
Status HeterogeneousDeployPlanner::ReindexDevices() {
  for (const auto &device_info : device_list_) {
    auto device = ResourceManager::GetInstance().GetDeviceInfo(device_info.GetNodeId(), device_info.GetDeviceId(),
                                                               device_info.GetType());
...
Status HeterogeneousDeployPlanner::GetLogicalDeviceId(const PneModel &pne_model, const std::string &engine_type,
...
void HeterogeneousDeployPlanner::NormalizeLogicalDeviceId(std::vector<std::string> &logical_device_ids) {
  for (auto &device_id : logical_device_ids) {
    ...
    if (StringUtils::Split(device_id, ':').size() == kIncompletedSize) {
      device_id = "0:0:" + device_id;  // append cluster_id:server_id
    }
  }
}
...
Status HeterogeneousDeployPlanner::ParseLogicalDeviceIds(const std::string &logical_device_id_str,
  ...
  GE_CHK_BOOL_RET_STATUS(std::regex_match(normalized, match_result, remove_brackets), PARAM_INVALID,
                         "Invalid logical_device_id: %s", logical_device_id_str.c_str());
  logical_device_ids = StringUtils::Split(match_result.str(1), ',');
  return SUCCESS;
}

Status HeterogeneousDeployPlanner::CheckAssignedDevice(const std::string &model_name, const std::string &engine_type,
...
Status HeterogeneousDeployPlanner::AssignDevices(
    const std::string &model_name, const PneModelPtr &pne_model, const std::string &engine_type,
  ...
  GE_CHK_BOOL_RET_STATUS(redundant_logical_device_ids.size() < logical_device_ids.size(), PARAM_INVALID,
                         "redundant logical device ids size[%u] is less than "
...
Status HeterogeneousDeployPlanner::PrepareForSingleFlowModel(std::map<std::string, PneModelPtr> &name_to_models,
  ...
  GE_CHK_STATUS_RET(ValidateModelAndRelation(name_to_models, model_relation), "Failed to validate model and relation");
  return SUCCESS;
}
...
Status HeterogeneousDeployPlanner::UnfoldSubModel(const ModelRelation::ModelEndpointInfo &model_endpoint_info,
  ...
  GE_CHK_STATUS_RET(
      RewriteQueueNames(model.GetModelName(), model_endpoint_info, model_relation, merged_model_relation_.endpoints),
      "Failed to rewrite queue names, model name = %s", model.GetModelName().c_str());
...
void HeterogeneousDeployPlanner::AddDynamicSchedModel(
    std::map<std::string, PneModelPtr> &model_instance_to_models, ModelRelation &resolved_model_relation,
...
void HeterogeneousDeployPlanner::AddDynamicSchedRelation(std::map<std::string, PneModelPtr> &model_instance_to_models,
  ...
  for (const auto &device_info : device_list_) {
    ...
    if (HasDeployedModelOnDevice(device_info)) {
      ...
      GELOGI("DynamicSched model datagw request queue_name=%s, response queue_name=%s.", request_queue_name.c_str(),
             queue_name.c_str());

      Endpoint queue_def(queue_name, EndpointType::kQueue);
      QueueNodeUtils(queue_def).SetDepth(kDepDefQueDepth).SetEnqueuePolicy("FIFO").SetNodeAction(kQueueActionSched);
      ...
      QueueNodeUtils(request_queue_def)
          .SetDepth(kDepDefQueDepth)
          .SetEnqueuePolicy("FIFO")
...
Status HeterogeneousDeployPlanner::AssignSubmodelsQueueDeviceInfo(const std::string &model_name,
  ...
  if (queue_device_info.GetType() == static_cast<int32_t>(CPU)) {
    auto device = ResourceManager::GetInstance().GetDeviceInfo(
        queue_device_info.GetNodeId(), queue_device_info.GetDeviceId(), queue_device_info.GetType());
    GE_CHECK_NOTNULL(device);
  ...
  GELOGI("Model[%s] queues will be deployed on device[%s]", model_name.c_str(),
...
Status HeterogeneousDeployPlanner::PrepareTargetDevices(std::map<std::string, PneModelPtr> &name_to_models,
  ...
  for (const auto &it : target_devices) {
    ...
    for (const auto &target_device : it.second) {
      ...
      const auto model_instance_name = model_name + "@" + std::to_string(process_id++) + "@" +
                                       target_device.first.GetKey() + "@" + std::to_string(is_redundant);
      // add submodel
...
Status HeterogeneousDeployPlanner::CreateTransferInfo(const DeployPlan::DeviceInfo &src_device_info,
  ...
  const std::string route_name =
      src_device_info.GetKey() + "_TO_" + dst_device_info.GetKey() + "_T" + std::to_string(plan_id_gen_);
...
void HeterogeneousDeployPlanner::SelectHeadAndTailDevice(DeployPlan::DeviceInfo &device_info) {
  ...
  auto device = ResourceManager::GetInstance().GetDeviceInfo(
      default_device_info.GetNodeId(), default_device_info.GetDeviceId(), default_device_info.GetType());
  if (device != nullptr) {
