...
#include "dflow/runner/compiler/pne/process_node_engine.h"
...
class FlowModelBuilder {
 ...
 private:
  ...
  Status BuildHeterogeneousModel(ComputeGraphPtr &root_graph, const std::vector<GeTensor> &input_tensors,
                                 const std::map<std::string, std::string> &options,
                                 const FlowModelPtr &flow_model) const;

  Status DoBuildGraph(ComputeGraphPtr &compute_graph, const std::map<std::string, std::string> &options,
