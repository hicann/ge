...
build_ascgen-dev() {
  ...
  echo "cp metadef libs from ${BUILD_PATH} to ${METADEF_LIB_PATH}"
}
...
build_test() {
  ...
  RUN_TEST_CASE=${BUILD_PATH}/tests/autofuse/ut/test_main_regbase && ${RUN_TEST_CASE}
  if [ $? -ne 0 ]
  then
    echo "execute command: test_main_regbase  failed."
    return 1
  fi
  cd -
  ...
  cd -
  echo "ascgen-dev test success!"
}

build_test_ascir_st() {
  echo "create build directory and build test_ascir_st";
  cd "${BUILD_PATH}"
  ...
  echo "run test_ascir_st success!"
}

build_ut_optimize() {
  echo "create build directory and build optimize st";
  cd "${BUILD_PATH}"
  ...
  echo "optimize_st test successfully!"
}
...
build_ut_common () {
  ...
  echo "test_common test successfully!"
}
...
build_ut_autofusion() {
  echo "create build directory and build autofusion ut";
  cd "${BUILD_PATH}"
  ...
  echo "autofusion_ut test successfully!"
}

build_st_autofuse() {
  echo "create build directory and build autofuse st";
  cd "${BUILD_PATH}"
  ...
  echo "autofuse_st test successfully!"
}
...
build_st_common() {
  echo "create build directory and build common st";
  cd "${BUILD_PATH}"
  ...
  echo "common_st test successfully!"
}
...
run_codegen_one_e2e_st() {
  ...
  if [[ $test_name == *"$v2"* ]]; then
    ...
    cp ${BUILD_PATH}/tests/autofuse/st/codegen/e2e_v2/${dir_name}/${test_name}  ${OUTPUT_PATH}
  else
...
run_backend_one_e2e_st() {
  ...
  if [[ $test_name == *"$v2"* ]]; then
    dir_name="${test_name%_e2e_v2}"
    cp ${BUILD_PATH}/tests/autofuse/st/backend_e2e_v2/${dir_name}/${test_name}  ${OUTPUT_PATH}
  else
    dir_name="${test_name%_e2e}"
...
codegen_e2e_st() {
  ...
  MAKE_TARGET_LIST="load_where_store_expect_code_e2e \
                    ...
                    load_rprod_store_e2e \
                    load_compare_ub_scalar_tensor_eq_store_e2e \
                    load_compare_half_tensor_eq_store_e2e \
                    load_compare_int64_tensor_eq_store_e2e \
                    load_compare_int64_tensor_ne_store_e2e \
                    load_compare_int64_tensor_gt_store_e2e \
                    load_broadcast_axis_s1_store_expect_code_e2e \
                    clip_by_value_float_e2e \
                    pow_int32_e2e \
                    discrete_store_e2e \
                    discrete_load_e2e \
                    discrete_store_merge_axis_e2e \
  ...
  if [ $? -ne 0 ]
  then
    echo "execute command: make codegen_e2e_st failed."
    return 1
  fi
  ...
  echo "codegen e2e_st execute success!"
}
...
build_backend() {
  ...
  if [ $? -ne 0 ]
  then
    echo "execute command: make backend_e2e_st failed."
    return 1
  fi
  ...
  echo "backend e2e_st execute success!"
}
...
build_st_att() {
  echo "create build directory and build att st";
  cd "${BUILD_PATH}"
...
build_ut() {
  case ${MODEL_NAME} in
    ...
    "all")
      ...
      ;;
    *)
      echo "输入无效，输入范围att/autofuse/optimizecodegen/all."
      ;;
  esac
}
