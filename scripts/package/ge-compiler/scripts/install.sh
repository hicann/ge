...
log() {
    ...
    local log_type="$1"
    local log_msg="$2"
    local log_format="[ge-compiler] [$cur_date] [$log_type]: $log_msg"
...
update_install_param() {
    ...
    local install_info_key_array="GE-COMPILER_Install_Type GE-COMPILER_UserName GE-COMPILER_UserGroup GE-COMPILER_Install_Path_Param"
...
update_install_info() {
    ...
    update_install_param "GE-COMPILER_Install_Type" "$installmode" "$install_info"
    update_install_param "GE-COMPILER_UserName" "$username" "$install_info"
    update_install_param "GE-COMPILER_UserGroup" "$usergroup" "$install_info"
    update_install_param "GE-COMPILER_Install_Path_Param" "$install_path_param" "$install_info"
}
...
install_run() {
    ...
    local ge_compiler_install_path_param=""
    update_install_path
    update_install_info
    local ge_compiler_input_install_path=$(get_install_param "GE-COMPILER_Install_Path_Param" "${install_info}")
    local ge_compiler_install_type=$(get_install_param "GE-COMPILER_Install_Type" "${install_info}")
    if [ "$is_docker_install" = "y" ]; then
        ge_compiler_install_path_param=$(concat_docker_install_path "${docker_root}" "${ge_compiler_input_install_path}")
    else
        ge_compiler_install_path_param="${ge_compiler_input_install_path}"
    fi
    if [ "$1" = "install" ]; then
        unchattr_files
        chmod_start
        new_echo "INFO" "install ${ge_compiler_install_path_param} ${ge_compiler_install_type}"
        log "INFO" "install ${ge_compiler_install_path_param} ${ge_compiler_install_type}"
        bash "${curpath}/run_ge-compiler_install.sh" "install" "${ge_compiler_input_install_path}" "${ge_compiler_install_type}" \
            "${is_quiet}" "${pylocal}" "${input_setenv}" "${docker_root}" "${in_install_for_all}" "$pkg_version_dir"
        if [ $? -eq 0 ]; then
            update_version_info_version
            log "INFO" "ge-compiler package installed successfully! The new version takes effect immediately."
            log_operation "${operation}" "succeeded"
            chmod_end
            prompt_set_env "${install_path_param}"
        else
            chmod_end
            log "ERROR" "ge-compiler package install failed, please retry after uninstall!"
...
upgrade_run() {
    ...
    if [ -f "$install_info" ]; then
        local ge_compiler_input_install_path=$(get_install_param "GE-COMPILER_Install_Path_Param" "${install_info}")
        local ge_compiler_install_type=$(get_install_param "GE-COMPILER_Install_Type" "${install_info}")
    elif [ -f "$install_info_old" ] && [ $(grep -c -i ge-compiler_install_path_param "$install_info_old") -ne 0 ]; then
        local ge_compiler_input_install_path="$(grep -iw ge-compiler_install_path_param "$install_info_old" | cut -d"=" -f2-)"
        local ge_compiler_install_type="$(grep -iw ge-compiler_install_type "$install_info_old" | cut -d"=" -f2-)"
    else
    ...
    if [ "$1" = "upgrade" ]; then
        chmod_start
        new_echo "INFO" "upgrade ${ge_compiler_install_path_param} ${ge_compiler_install_type}"
        log "INFO" "upgrade ${ge_compiler_install_path_param} ${ge_compiler_install_type}"
        bash "${curpath}/run_ge-compiler_upgrade.sh" "upgrade" "${ge_compiler_input_install_path}" "${ge_compiler_install_type}" \
            "${is_quiet}" "${pylocal}" "${input_setenv}" "${docker_root}" "${in_install_for_all}" "$pkg_version_dir"
        if [ $? -eq 0 ]; then
            update_version_info_version
            log "INFO" "ge-compiler package upgraded successfully! The new version takes effect immediately."
            log_operation "${operation}" "succeeded"
            chmod_end
            prompt_set_env "${install_path_param}"
        else
            chmod_end
            log "ERROR" "ge-compiler package upgrade failed, please retry after uninstall!"
            log_operation "${operation}" "failed!"
            exit_install_log 1
        fi
    fi
...
uninstall_run() {
    ...
    if [ -f "$upgrade_install_info" ]; then
        local ge_compiler_input_install_path=$(get_install_param "GE-COMPILER_Install_Path_Param" "${upgrade_install_info}")
        local ge_compiler_install_type=$(get_install_param "GE-COMPILER_Install_Type" "${upgrade_install_info}")
    elif [ -f "$install_info_old" ] && [ $(grep -c -i ge-compiler_install_path_param "$install_info_old") -ne 0 ]; then
        local ge_compiler_input_install_path="$(grep -iw ge-compiler_install_path_param "$install_info_old" | cut -d"=" -f2-)"
        local ge_compiler_install_type="$(grep -iw ge-compiler_install_type "$install_info_old" | cut -d"=" -f2-)"
    else
    ...
    if [ ! -f "$upgrade_default_dir/script/run_ge-compiler_uninstall.sh" ]; then
        log "WARNING" "run_ge-compiler_uninstall.sh not found."
        return $?
    fi
    if [ "$1" = "uninstall" ]; then
        chmod_start "$upgrade_default_dir"
        new_echo "INFO" "uninstall ${ge_compiler_install_path_param} ${ge_compiler_install_type}"
        log "INFO" "uninstall ${ge_compiler_install_path_param} ${ge_compiler_install_type}"
        bash "$upgrade_default_dir/script/run_ge-compiler_uninstall.sh" "uninstall" "${ge_compiler_input_install_path}" "${ge_compiler_install_type}" "${is_quiet}" \
        ...
        if [ $? -eq 0 ]; then
            ...
            if [ $(id -u) -eq 0 ]; then
                if [ "$uninstall" = "y" ] && [ -f "$install_info_old" ] && [ $(grep -c -i ge-compiler_install_path_param "$install_info_old") -ne 0 ]; then
                    sed -i '/ge-compiler_install_path_param=/Id' "$install_info_old" 2> /dev/null
                    sed -i '/ge-compiler_install_type=/Id' "$install_info_old" 2> /dev/null
                fi
            fi
            new_echo "INFO" "ge-compiler package uninstalled successfully! Uninstallation takes effect immediately."
            log "INFO" "ge-compiler package uninstalled successfully! Uninstallation takes effect immediately."
            log_operation "${operation}" "succeeded"
        else
            log "ERROR" "ge-compiler package uninstall failed!"
            log_operation "${operation}" "failed!"
            exit_uninstall_log 1
        fi
    fi
...
judgmentpath() {
    ...
    if [ $? -ne 0 ]; then
        log "ERROR" "The ge-compiler install_path ${1} is invalid, only characters in [a-z,A-Z,0-9,-,_] are supported!"
        exit 1
    fi
}
...
migrate_user_assets_v2() {
    if [ "$pkg_is_multi_version" = "true" ]; then
        get_package_last_installed_version_dir "last_installed_dir" "$pkg_install_path" "ge-compiler"
        if [ -n "$last_installed_dir" ]; then
            ...
            local data_dir="ge-compiler/data/fusion_strategy"
...
if [ "$docker_install" = "y" ]; then
    log "ERROR" "ERR_NO:0x0004;ERR_DES:Unsupported parameters, operation failed."
    log "INFO" "--docker not uesd in ge-compiler"
    exit 1
fi
...
if [ "$pkg_is_multi_version" = "true" ]; then
    default_dir="${pkg_install_path}/$pkg_version_dir/share/info/ge-compiler"
else
    default_dir="${pkg_install_path}/ge-compiler"
fi
...
if [ "$input_pre_check" = "y" ]; then
    log "INFO" "ge-compiler do pre check started."
    pre_check
    if [ $? -ne 0 ]; then
        log "WARNING" "ge-compiler do pre check failed."
    else
        log "INFO" "ge-compiler do pre check finished."
    fi
...
if [ "$check" = "y" ]; then
    ver_check
    preinstall_check --install-path="$pkg_install_path/$pkg_version_dir" --script-dir="$curpath" --package="ge-compiler" --logfile="$logfile" --docker-root="$docker_root"
    if [ $? -ne 0 ]; then
        exit_install_log 1
    else
    ...
    ver_check
    preinstall_process --install-path="$pkg_install_path/$pkg_version_dir" --script-dir="$curpath" --package="ge-compiler" --logfile="$logfile" --docker-root="$docker_root"
    if [ $? -ne 0 ]; then
        exit_install_log 1
    else
        log "INFO" "version compatibility check successfully!"
...
if [ "x$version_installed" != "x" -a "$version_installed" != "none" ] || [ -f "${install_info}" ]; then
    ...
    # 安装场景
        ...
        if [ "$is_quiet" = "n" ]; then
            log "INFO" "ge-compiler package has been installed on the path ${pkg_install_path}, the version is ${version_installed}, and the version of this package is ${version_of_package}, do you want to continue? [y/n]"
            while true
            do
                read yn
                if test "$yn" = n; then
