...
dflow_executor_func_path="${curpath}/dflow-executor_func.sh"
...
update_install_param() {
    ...
    local install_info_key_array="Dflow-Executor_Install_Type Dflow-Executor_UserName Dflow-Executor_UserGroup Dflow-Executor_Install_Path_Param"
...
update_install_info() {
    ...
    update_install_param "Dflow-Executor_Install_Type" "$installmode" "$install_info"
    update_install_param "Dflow-Executor_UserName" "$username" "$install_info"
    update_install_param "Dflow-Executor_UserGroup" "$usergroup" "$install_info"
    update_install_param "Dflow-Executor_Install_Path_Param" "$install_path_param" "$install_info"
}
...
install_run() {
    ...
    local dflow_executor_install_path_param=""
    update_install_path
    update_install_info
    local dflow_executor_input_install_path=$(get_install_param "Dflow-Executor_Install_Path_Param" "${install_info}")
    local dflow_executor_install_type=$(get_install_param "Dflow-Executor_Install_Type" "${install_info}")
    if [ "$is_docker_install" = "y" ]; then
        dflow_executor_install_path_param=$(concat_docker_install_path "${docker_root}" "${dflow_executor_input_install_path}")
    else
        dflow_executor_install_path_param="${dflow_executor_input_install_path}"
    fi
    if [ "$1" = "install" ]; then
        unchattr_files
        chmod_start
        new_echo "INFO" "install ${dflow_executor_install_path_param} ${dflow_executor_install_type}"
        log "INFO" "install ${dflow_executor_install_path_paramm} ${dflow_executor_install_type}"
        bash "${curpath}/run_dflow-executor_install.sh" "install" "${dflow_executor_input_install_path}" "${dflow_executor_install_type}" \
            "${is_quiet}" "${pylocal}" "${input_setenv}" "${docker_root}" "${in_install_for_all}" "$pkg_version_dir"
        if [ $? -eq 0 ]; then
            update_version_info_version
            log "INFO" "dflow-executor package installed successfully! The new version takes effect immediately."
            log_operation "${operation}" "succeeded"
            chmod_end
            prompt_set_env "${install_path_param}"
        else
            chmod_end
            log "ERROR" "dflow-executor package install failed, please retry after uninstall!"
...
upgrade_run() {
    ...
    if [ -f "$install_info" ]; then
        local dflow_executor_input_install_path=$(get_install_param "Dflow-Executor_Install_Path_Param" "${install_info}")
        local dflow_executor_install_type=$(get_install_param "Dflow-Executor_Install_Type" "${install_info}")
    elif [ -f "$install_info_old" ] && [ $(grep -c -i dflow_executor_install_path_param "$install_info_old") -ne 0 ]; then
        local dflow_executor_input_install_path="$(grep -iw dflow_executor_install_path_param "$install_info_old" | cut -d"=" -f2-)"
        local dflow_executor_install_type="$(grep -iw dflow_executor_install_type "$install_info_old" | cut -d"=" -f2-)"
    else
    ...
    if [ "$1" = "upgrade" ]; then
        chmod_start
        new_echo "INFO" "upgrade ${dflow_executor_install_path_param} ${dflow_executor_install_type}"
        log "INFO" "upgrade ${dflow_executor_install_path_param} ${dflow_executor_install_type}"
        bash "${curpath}/run_dflow-executor_upgrade.sh" "upgrade" "${dflow_executor_input_install_path}" "${dflow_executor_install_type}" \
            "${is_quiet}" "${pylocal}" "${input_setenv}" "${docker_root}" "${in_install_for_all}" "$pkg_version_dir"
        if [ $? -eq 0 ]; then
            update_version_info_version
            log "INFO" "dflow-executor package upgraded successfully! The new version takes effect immediately."
            log_operation "${operation}" "succeeded"
            chmod_end
            prompt_set_env "${install_path_param}"
        else
            chmod_end
            log "ERROR" "dflow-executor package upgrade failed, please retry after uninstall!"
            log_operation "${operation}" "failed!"
            exit_install_log 1
        fi
    fi
...
uninstall_run() {
    ...
    if [ -f "$upgrade_install_info" ]; then
        local dflow_executor_input_install_path=$(get_install_param "Dflow-Executor_Install_Path_Param" "${upgrade_install_info}")
        local dflow_executor_install_type=$(get_install_param "Dflow-Executor_Install_Type" "${upgrade_install_info}")
    elif [ -f "$install_info_old" ] && [ $(grep -c -i dflow_executor_install_path_param "$install_info_old") -ne 0 ]; then
        local dflow_executor_input_install_path="$(grep -iw dflow_executor_install_path_param "$install_info_old" | cut -d"=" -f2-)"
        local dflow_executor_install_type="$(grep -iw dflow_executor_install_type "$install_info_old" | cut -d"=" -f2-)"
    else
    ...
    if [ ! -f "$upgrade_default_dir/script/run_dflow-executor_uninstall.sh" ]; then
        log "WARNING" "run_dflow-executor_uninstall.sh not found."
        return $?
    fi
    if [ "$1" = "uninstall" ]; then
        chmod_start "$upgrade_default_dir"
        new_echo "INFO" "uninstall ${dflow_executor_install_path_param} ${dflow_executor_install_type}"
        log "INFO" "uninstall ${dflow_executor_install_path_param} ${dflow_executor_install_type}"
        bash "$upgrade_default_dir/script/run_dflow-executor_uninstall.sh" "uninstall" "${dflow_executor_input_install_path}" "${dflow_executor_install_type}" "${is_quiet}" \
        ...
        if [ $? -eq 0 ]; then
            ...
            if [ $(id -u) -eq 0 ]; then
                if [ "$uninstall" = "y" ] && [ -f "$install_info_old" ] && [ $(grep -c -i dflow_executor_install_path_param "$install_info_old") -ne 0 ]; then
                    sed -i '/dflow_executor_install_path_param=/Id' "$install_info_old" 2> /dev/null
                    sed -i '/dflow_executor_install_type=/Id' "$install_info_old" 2> /dev/null
                fi
            fi
            new_echo "INFO" "dflow-executor package uninstalled successfully! Uninstallation takes effect immediately."
            log "INFO" "dflow-executor package uninstalled successfully! Uninstallation takes effect immediately."
            log_operation "${operation}" "succeeded"
        else
            log "ERROR" "dflow-executor package uninstall failed!"
            log_operation "${operation}" "failed!"
            exit_uninstall_log 1
        fi
    fi
...
judgmentpath() {
    ...
    if [ $? -ne 0 ]; then
        log "ERROR" "The dflow-executor install_path ${1} is invalid, only characters in [a-z,A-Z,0-9,-,_] are supported!"
        exit 1
    fi
}
...
migrate_user_assets_v2() {
    if [ "$pkg_is_multi_version" = "true" ]; then
        get_package_last_installed_version_dir "last_installed_dir" "$pkg_install_path" "dflow-executor"
        if [ -n "$last_installed_dir" ]; then
            ...
            local data_dir="dflow-executor/data/fusion_strategy"
...
if [ "${is_quiet}" = "y" ]; then
    if [ "${upgrade}" = "y" ] || [ "${full_install}" = "y" ] || [ "${run_install}" = "y" ] || [ "${devel_install}" = "y" ] || [ "${uninstall}" = "y" ]; then
        ...
        log "ERROR" "'--quiet' is not supported to used by this way, please use with '--full', '--devel', '--run', '--upgrade', '--uninstall' or '--check'"
        exit 1
    fi
fi
...
if [ "$docker_install" = "y" ]; then
    log "ERROR" "ERR_NO:0x0004;ERR_DES:Unsupported parameters, operation failed."
    log "INFO" "--docker not uesd in dflow-executor"
    exit 1
fi
...
if [ "$pkg_is_multi_version" = "true" ]; then
    default_dir="${pkg_install_path}/$pkg_version_dir/share/info/dflow-executor"
else
    default_dir="${pkg_install_path}/share/info/dflow-executor"
fi
...
if [ "$check" = "y" ]; then
    ver_check
    preinstall_check --install-path="$pkg_install_path/$pkg_version_dir" --script-dir="$curpath" --package="dflow-executor" --logfile="$logfile" --docker-root="$docker_root"
    if [ $? -ne 0 ]; then
        exit_install_log 1
    else
    ...
    ver_check
    preinstall_process --install-path="$pkg_install_path/$pkg_version_dir" --script-dir="$curpath" --package="dflow-executor" --logfile="$logfile" --docker-root="$docker_root"
    if [ $? -ne 0 ]; then
        exit_install_log 1
    else
        log "INFO" "version compatibility check successfully!"
...
if [ "x$version_installed" != "x" -a "$version_installed" != "none" ] || [ -f "${install_info}" ]; then
    ...
    # 安装场景
        ...
        if [ "$is_quiet" = "n" ]; then
            log "INFO" "dflow-executor package has been installed on the path ${pkg_install_path}, the version is ${version_installed}, and the version of this package is ${version_of_package}, do you want to continue? [y/n]"
            while true
            do
                read yn
                if test "$yn" = n; then
