...
log() {
    ...
    if [ "$log_type" = "INFO" -o "$log_type" = "WARNING" -o "$log_type" = "ERROR" ]; then
        echo -e "[dflow-executor] [$cur_date] [$log_type]: $*"
    else
        echo "[dflow-executor] [$cur_date] [$log_type]: $*" 1> /dev/null
    fi
    echo "[dflow-executor] [$cur_date] [$log_type]: $*" >> "$logfile"
}
...
dflow_executor_install_package() {
    local _package="$1"
...
migrate_user_assets() {
    ...
    if [ "x$path_list" != "x" ]; then
        ...
        do
            dst=${src##*$pkg_name/}
            dst="$common_parse_dir/share/info/dflow-executor/$dst"
...
custom_install() {
    ...
    if [ "$hetero_arch" != "y" ]; then
        if [ "x$stage" = "xinstall" ]; then
            log "INFO" "dflow-executor do migrate user assets."
    ...
    if [ ! -e $common_parse_dir/lib64/stub/linux/aarch64/libflow_func.so ]; then
      log "DEBUG" "dflow-executor link $common_parse_dir/lib64/stub/linux/aarch64/libflow_func.so->$common_parse_dir/devlib/device/libflow_func.so."
      ln -srfn $common_parse_dir/devlib/device/libflow_func.so $common_parse_dir/lib64/stub/linux/aarch64/libflow_func.so
    fi
    return 0
}
