...
SOURCE_INSTALL_COMMON_PARSER_FILE="${common_parse_dir}/share/info/ge-executor/script/install_common_parser.sh"
SOURCE_FILELIST_FILE="${common_parse_dir}/share/info/ge-executor/script/filelist.csv"
...
##########################################################################
log "INFO" "step into run_ge-executor_uninstall.sh ......"
log "INFO" "uninstall target dir $common_parse_dir, type $common_parse_type."

if [ ! -d "$common_parse_dir/share/info/ge-executor" ]; then
    log "ERROR" "ERR_NO:0x0001;ERR_DES:path $common_parse_dir/share/info/ge-executor is not exist."
    exit 1
fi

new_uninstall() {
    if [ -f "${common_parse_dir}/share/info/ge-executor/data/version.info" ]; then
        log "INFO" "need to uninstall costmodel files."
        bash "${common_parse_dir}/share/info/ge-executor/data/script/install.sh" -- -- --uninstall --install-path="${common_parse_dir}"
    fi

    if [ ! -d "${common_parse_dir}/share/info/ge-executor" ]; then
        log "INFO" "no need to uninstall ge-executor files."
        return 0
    fi
    ...
    sh "$SOURCE_INSTALL_COMMON_PARSER_FILE" --package="ge-executor" --uninstall --username="$username" --usergroup="$usergroup" ${recreate_softlink_option} \
    ...
    create_acl_empty_headers "$common_parse_dir" "$pkg_version_dir"
