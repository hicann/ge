...
log() {
    ...
    if [ "$log_type" = "INFO" -o "$log_type" = "WARNING" -o "$log_type" = "ERROR" ]; then
        echo -e "[ge-executor] [$cur_date] [$log_type]: $*"
    else
        echo "[ge-executor] [$cur_date] [$log_type]: $*" 1> /dev/null
    fi
    echo "[ge-executor] [$cur_date] [$log_type]: $*" >> "$logfile"
}
...
migrate_user_assets() {
    ...
    if [ "x$path_list" != "x" ]; then
        ...
        do
            dst=${src##*$pkg_name/}
            dst="$common_parse_dir/share/info/ge-executor/$dst"
...
custom_install() {
    if [ -z "$common_parse_dir/share/info/ge-executor" ]; then
        log "ERROR" "ERR_NO:0x0001;ERR_DES:ge-executor directory is empty"
        exit 1
    fi

    if [ "$hetero_arch" != "y" ]; then
        local arch_name="$(get_arch_name $common_parse_dir/share/info/ge-executor)"
    else
        local arch_name="$(get_arch_name $common_parse_dir/share/info/ge-executor)"
    fi
    ...
    if [ "$hetero_arch" != "y" ]; then
        if [ "x$stage" = "xinstall" ]; then
            log "INFO" "ge-executor do migrate user assets."
