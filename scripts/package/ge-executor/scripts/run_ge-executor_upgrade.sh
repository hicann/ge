...
install_info="${common_parse_dir}/share/info/ge-executor/ascend_install.info"
...
##########################################################################
log "INFO" "step into run_ge-executor_upgrade.sh ......"
...
new_upgrade() {
    if [ ! -d "${sourcedir}" ]; then
        log "INFO" "no need to upgrade ge-executor files."
        return 0
    fi
    output_progress 10
    ...
    sh "$curpath/install_common_parser.sh" --package="ge-executor" --install --username="$username" --usergroup="$usergroup" --set-cann-uninstall --upgrade \
