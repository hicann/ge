...
// user set FileConstant device memory
struct FileConstantMem {
  std::string file_name;
  const void *device_mem;
  size_t mem_size;
};

struct ModelQueueArg {
...
struct OpDescInfoId {
  ...
  OpDescInfoId(const uint32_t task, const uint32_t stream, const uint32_t context, const uint32_t thread,
      ...
      : task_id(task), stream_id(stream), context_id(context), thread_id(thread), device_id(dev_id) {}
};
