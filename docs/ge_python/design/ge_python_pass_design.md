...
- `cp39-cp312` 提供预编译 `pybind11` native 子 wheel
...
换句话说，要求的是“整体发布流水线覆盖 `cp39-cp312`”，而不是“每台开发机或构建机必须同时具备全部版本”。
...
当前开发环境中 Python 为 `3.13`，而正式矩阵为 `cp39-cp312`。因此本地验证必须依赖 fallback 或开发态本地编译能力，不能假定预编译包直接可用。
...
也就是说，后续不建议按 `cp39/cp310/cp311/cp312` 长期维护多份手写 bridge 源码分支。
...
- 本地开发环境是 Python 3.13，而正式 wheel 规划是 `cp39-cp312`，本地验证必须依赖 fallback 或开发态编译。
