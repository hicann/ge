# ResetFlowFuncState（MetaFlowFunc类）<a name="ZH-CN_TOPIC_0000002096526421"></a>
...
此接口为虚函数，当用户的FlowFunc未实现ResetFlowFuncState函数时，框架默认返回FLOW\_FUNC\_ERR\_NOT\_SUPPORT（认为当前FlowFunc不支持此操作），把原来创建的FlowFunc删除掉，重新创建一个新的FlowFunc，然后再调用[Init](Init（MetaFlowFunc类）.md)接口进行初始化。
