...
AscendC IR（如下简称AscIR）是一套基于Ascend C语言定义的IR，专门负责自动融合软件栈的后端工作，AscIR与[Ascend C API](https://gitcode.com/cann/asc-devkit/blob/9.2.0/docs/zh/api/README.md)一一对应：AscIR的类型（type）用于区分对应的Ascend C API；AscIR定义中的输入、输出、属性等内容，分别映射到Ascend C API的各类参数。
