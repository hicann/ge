...
1. 算子开发阶段，用户在算子原型注册过程中调用Ascend C的EnableFallBack接口，自动生成fallback函数：
    ...
    当前fallback函数支持support\_aclnn和aclnn\_only两种调用模式，可通过ExtendCfgInfo接口的aclnnSupport.value参数进行配置，详细可参见[《Ascend C API》](https://gitcode.com/cann/asc-devkit/blob/9.2.0-beta.1/docs/zh/api/README.md)。
