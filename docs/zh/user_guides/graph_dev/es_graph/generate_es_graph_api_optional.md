...
3. 使用生成的产物
    ...
    > [!NOTE]说明
    >
    >因为ES构图API是基于算子IR原型生成，部分算子IR无法支撑所有信息表达（比如Conv类算子）或者IR表达有冗余信息（比如IdentityN算子），所以推荐用户自行封装ES API，该场景下用户可以自定义ES API，详细介绍请参见[自定义ES API](https://gitcode.com/cann/ge/blob/9.2.0-beta.2/examples/custom_es_api)。
