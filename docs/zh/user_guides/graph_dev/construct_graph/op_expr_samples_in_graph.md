...
从Cast算子原型定义可以看到，有一个必选属性dst\_type，表示转换后的数据类型，设置为0表示转换后的数据类型为float32，值和数据类型对应关系请参见《[基础数据结构和接口](https://gitcode.com/cann/metadef/blob/9.2.0/docs/zh/api/README.md)》中的"ge命名空间\>DataType"。
