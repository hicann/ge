...
在AI推理中，传统方式需将数据从主机内存拷贝至设备内存，推理完成后再回传，这种Host-to-Device与Device-to-Host的拷贝是推理延迟的主要来源。零拷贝旨在消除中间拷贝，实现用户缓冲区与设备内存的直接映射读写。

默认状态下，每次模型执行都会重新分配零拷贝I/O内存区域，开启此选项后，GE会将模型输入输出的零拷贝内存块独立规划，不与其他内存块合并，并在多次执行间复用该区域，避免重复的内存分配，从而显著降低推理延迟

**参数取值：**

- 0：（默认值）关闭，不复用零拷贝内存，每次执行重新分配。
- 1：开启，复用零拷贝内存，输入输出I/O内存跨执行复用。
...
- 若环境中配置环境变量ASCEND\_WORK\_PATH，则权重文件会落盘至`${ASCEND_WORK_PATH}/tmp_weight_<pid>_<sessionid>`目录下，关于ASCEND\_WORK\_PATH的详细说明，可参见《[环境变量参考](https://gitcode.com/cann/docs/blob/9.2.0/docs/zh/env-vars/README.md)》。
