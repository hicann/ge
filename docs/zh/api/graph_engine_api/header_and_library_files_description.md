...
| 接口分类 | 定义接口的头文件 | 用途 | 对应的库文件 |
| --- | --- | --- | --- |
| ge-compiler编译类接口 | ge/hcom_gradient_split_tune.h | HCOM梯度切分调优指标、输入和结果接口。 | libhcom_gradient_split_tune.so |
...
| ge-executor执行类接口 | ge/ge_api_types.h | GE公共状态、张量、图和Session相关类型的聚合入口。 | Header-only聚合头；按实际接口链接 libgraph.so/libgraph_base.so等 |
