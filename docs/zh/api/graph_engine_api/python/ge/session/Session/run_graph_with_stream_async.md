...
- 调用该接口前，需要通过acl提供的**acl.rt.create\_stream\(\)**接口创建Stream，且只支持Stream为默认Context的场景；得到输出运行结果前，需要通过**acl.rt.synchronize\_stream**接口保证Stream上的任务已经执行完。接口详细说明请参见“[Stream管理](https://gitcode.com/cann/runtime/blob/9.2.0/docs/zh/api_ref/README.md)”。
