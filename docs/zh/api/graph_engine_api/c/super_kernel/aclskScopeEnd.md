...
- Atlas 训练系列产品：不支持
<!-- end id6 -->
<!-- @ref: ge/res/docs/zh/api/graph_engine_api/c/super_kernel/aclskScopeEnd_res.md#id1 -->
...
标记Super Kernel作用域的结束位置。调用该接口后，会在指定Stream上下发scope\_begin kernel及placeholder kernel，用于标识一组需要融合执行的算子操作的结束边界。
...
- 与[aclskScopeBegin](aclskScopeBegin.md)成对使用，传入的scopeName与aclskScopeBegin一致。
