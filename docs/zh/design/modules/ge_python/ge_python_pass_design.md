...
5. bridge loader 解析目标 Python runtime key
6. bridge loader 按“预制 artifact、runtime fallback codegen”的顺序选择并装载 `libge_python_pass_bridge.so`
7. bridge loader 通过 `GeGetPythonFusionBasePassBridgeApi()` 获取 bridge 导出的稳定入口，并把 registrar 回调传给 bridge
8. `libge_python_pass_bridge.so` 初始化或复用 Python 运行时并导入 `ge.passes._bridge`
9. bridge 将当前进程环境中的 `ASCEND_GE_PY_PASS_PATH` 同步到 Python `os.environ`，再调用 `load_and_get_pass_descriptors()`
10. bootstrap 发现并导入用户 pass 模块
11. 用户 pass 模块通过装饰器把 pass 注册到 `ge.passes.registry`
12. bridge 读取注册表并通过 registrar 回调把 descriptor 动态注册回 `PassRegistry`
...
- native 子 wheel 通过标准 `bdist_wheel` 生成，承载 `ge/passes/python_pass_artifacts/<python_tag>-<platform>` 下的一套 bridge/native artifact set
- ge-compiler run 包中继续安装原有 `ge_py-0.0.1-py3-none-any.whl`，并额外把 matrix 输出目录中的 `ge_py_pass_bridge-*.whl` 安装到 `ge-compiler/lib64`
- `ge_python_pass_fallback_codegen` 目标通过 `gen_fallback_resources.py` 生成 `ge/passes/fallback_codegen/` 资源，包括 `build_config.json` 和可临时展开 `src/{bridge,native}`、`include/{bridge,native}` 的 Python 资源模块
- 运行时选择优先级为“预制 artifact set > runtime fallback codegen”
...
对于 run 包中的 packaged default 路径，还需要满足以下固定约束：

- `libge_python_pass_bridge.so` 随 native artifact wheel 放置到 `ge/passes/python_pass_artifacts/<python_tag>-<platform>/`
- GE 内部 loader 通过 manifest 解析 artifact set 中的 bridge/native 路径
- `_ge_pass_native.so` 与 `libge_python_pass_bridge.so` 遵循同一套 artifact set 装配约束
