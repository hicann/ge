...
自定义算子与 UDF 都用于扩展框架能力，但定位不同：自定义算子扩展**模型内的计算能力**——它是 GE 计算图的节点，为单模型提供新的计算原语并参与图优化与融合；UDF 扩展**模型间的编排能力**——它是 DataFlow 数据流图的节点，面向多模型串接场景插入自定义处理逻辑。

| 维度 | 自定义算子（含 AICore / AICPU 型） | UDF |
|------|----------------------------------|-----|
| 开发形态与依赖 | 在框架算子体系内开发：AICore 型用 Ascend C/TBE（框架编译托管），AICPU 型用 C/C++ 编译为 SO（由 AICPU 算子运行时托管加载）；受原型注册与算子框架接口约束 | 处理函数自由开发，可依赖三方库：链接库与 UDF SO 一并打进自包含 release 包，部署时整体搬运到执行环境（多版本同名 SO 经 md5 重命名隔离符号冲突），复杂逻辑处理更灵活 |
| 交付件 | 原型 + 实现 + 信息库 + 适配，四件套 | 处理函数 + 构图：C++ 一个类与注册宏，Python 一个装饰器（零 C++） |
| 数据模型 | Tensor/Desc 体系，shape/dtype/format 编译期强约束 | FlowMsg（mbuf）：Tensor 消息或任意序列化数据，运行期解释 |
| 编译链路 | 原型注册 → shape/格式推导 → tiling → 内存规划 → task 生成，产物编入模型 OM | UDF 引擎编译为 SO（C++）或自动生成工程（Python），部署期独立加载 |
| 执行模型 | 编入模型 OM：AICore 型 task sink 执行，AICPU 型在 AICPU 算子运行时执行 | 独立 udf_executor 进程（每 UDF 一进程，进程级隔离），host/device 位置可配 |
| 编排语义 | 同步数据流：一次输入一次输出，随图同步执行 | 异步流水：队列驱动，支持一对多/多对一、多实例负载均衡、流式输入输出 |

选型建议：单模型内需要新的计算原语（参与图优化与融合）用自定义算子（按计算特点选择 AICore 或 AICPU 型）；模型间需要编排逻辑或带三方依赖的复杂处理用 UDF。两者互补而非替代——DataFlow 图中 GraphPp 子图内部仍可包含自定义算子。
