...
- Boundary adjustments (such as module ownership changes) need to consider all usage scenarios
- Backward compatibility is the primary constraint of interface evolution

**Concrete Manifestation in GE:**

- CC file ownership adjustments between different SOs need to evaluate multiple deployment scenarios (stub package, non-stub package, tiny scenario)
...
- Nested calls, repeated calls and other scenarios need foolproof mechanisms designed in advance
...
**Design Philosophy:**
The concurrency model is an important constraint of module design and must be clearly defined at the design stage. An unclear concurrency model will lead to hard-to-reproduce concurrency issues.

**Architecture Principles:**

- Modules must clearly define their concurrency model (single-threaded, multi-threaded read, thread-safe, etc.)
...
- If concurrency support is needed in the future, interface semantics and usage constraints must be clearly defined at the design stage
...
- Avoid using containers with non-deterministic iteration order (such as `unordered_map`)
...
- Graph serialization is the main debugging and testing method, and must guarantee completeness
