...
In the Dynamic Shape multi-stream path, stream_id continuity has two steps. First, holes are eliminated according to engine priority, StreamLabel and other rules. If `ge.autoMultistreamParallelMode` is an explicit `LoadBalance:N` or `MainStream:N` DAG mode, where `N` is an integer in `[1, 64]`, valid stream_ids actually used by ordinary nodes in the root graph and unknown-shape subgraphs are collected together, remapped to continuous IDs in ascending order, and `stream_nodes_` is rebuilt. Nodes with no valid stream_id, forced-main-stream nodes, and nodes owning subgraphs do not retain old stream_ids written by the custom Pass; they join stream 0 during refresh so that no empty stream remains. At least one main stream is kept when there are no ordinary nodes. The `cv` mode does not enter the DAG node-based continuity branch and keeps the engine-priority continuity result. Compatibility for bare `LoadBalance` defaulting to eight streams has been retired. Subsequent cross-stream Event insertion is based on the final stream_id, avoiding holes or incorrect in-stream node ordering after auto multi-stream rewrites node streams.
