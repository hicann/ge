...
Additionally, `InnerPp` (the built-in PP extension base, providing a unified attribute extension mechanism for built-in PP types) and its derived `ModelPp` (loads pre-compiled OM models, loads directly without compilation) exist in the code. Both are internal implementations and do not provide external interfaces.
...
In addition, `pydflow/wrapper/` provides pybind11 C++ extension modules: `dflow_wrapper` (graph construction classes such as FlowGraph/FlowNode plus FlowBufferFactory) and `data_wrapper` (DataType and FuncDataType enums) support the user-side API, while `flowfunc_wrapper` (FlowMsg/MetaRunContext/RuntimeTensorDesc layout, etc., source directory `wrapper/flow_func_wrapper/`) supports UDF execution within the udf_executor process.
...
| `EmptyDataFlowMsg` | `[RuntimeTensorDesc]` (no data body) | Empty data message, used for notifications such as end-of-sequence |

FlowMsg is based on Ascend runtime **rtMbuf** for zero-copy implementation: producers fill mbuf data and pass it through queues to consumers; `rtMbufCopyBufRef` only increments the reference count, allowing multiple consumers to share the same data block without copying. EOS (End Of Sequence) determination is independent of the message subclass: the EOS flag is recorded in the mbuf head (with magic-number verification); when the user feeds an empty input carrying the EOS flag, the framework sends description-only empty data messages to each input queue, and downstream senses the end of stream accordingly.
...
`MbufHeadMsg` carries all control information needed for transaction tracking and data routing; `RuntimeTensorDesc` (1024-byte fixed layout: dataAddr/dtype/shape[33] (shape[0] stores the dim count, followed by DIM0~DIM31)/original_shape[33]/format/data_size, etc.) describes the tensor metadata in the data area, where original_shape preserves the original shape for restoration in dynamic shape scenarios.
...
Key-value pairs in `build_options` are **passed through to the GE compiler** as GraphPp subgraph compilation parameters. All GE-supported graph compilation parameters can be set here, allowing each GraphPp subgraph to independently configure compilation behavior (such as dynamic shape, output memory pre-allocation, and so on). When `ge.inputShape` is set, the framework automatically derives and appends dynamic execution mode and shape range related options. For specific option names and value formats, refer to the GE compilation parameter documentation.
...
| CPUProcessNodeEngine | Inherits NPU; FlowModelBuilder injects `EXEC_PLACEMENT=HOST` when compiling CPU subgraphs | GraphModel (OM) |
...
| Sub-slave scheduling node | `sub_deployer` | Model loading, fork executor | Daemon child process, attaches to daemon's MemoryGroup, initializes `GeExecutor` with model loading capability. Communicates with daemon through message queue; abnormalities are detected by the head node through heartbeat detection |
...
**Why is sub_deployer independent from daemon?** The daemon needs to run stably long-term and serve multiple user connections; it should not bear heavy logic such as model loading. sub_deployer is isolated by client; abnormalities only affect that user; the head node senses node abnormalities through heartbeat detection and triggers exception handling, and the daemon only forks a new sub_deployer for a new connection when it is established.
...
- `feed_data`/`fetch_data`: Tensor-specific high-performance path, directly using `ge::Tensor` with no serialization overhead (the fetch side returns zero-copy through memory views)
...
- Output: `_convert_object_to_flow_msg` serializes return values (df.Tensor -> FlowMsg conversion: Tensors allocated by `df.alloc_tensor` are converted zero-copy through shared mbuf references, other Tensors are copied before writing / None -> empty message / the rest -> serialized data written into mbuf)

Generator functions output per yield (streaming output). Users can register custom types and serialization functions through `df.utils.msg_type_register` (custom msg_types conventionally start from 1024; registration does not enforce a lower-bound check); unregistered types default to cloudpickle (msg_type=65535).
...
**Why does C++ UDF default to device?** C++ UDF `heavy_load` defaults to false (`compile_config_json.h`), and when compilation output includes both Ascend and host types, `SelectResourceType` selects Ascend. Users can set `heavy_load` to true in the FunctionPp compilation configuration JSON to force host.
...
- **Instance naming**: `model_name@process_id@device_key@is_redundant` (`PrepareTargetDevices` in `heterogeneous_deploy_planner.cc`); under the same model, process_id increments from 0, and the last segment indicates whether the instance is redundant
