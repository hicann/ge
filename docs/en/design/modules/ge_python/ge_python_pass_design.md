...
- `build_python_native_matrix.py` has built-in support for the version set `cp39/cp310/cp311/cp312/cp313/cp314`; CMake generates a component configuration for pass/runtime/custom-op and passes target, sources, artifacts, wheel metadata, ABI, and Python link policy through `--component-config`; `--tag` is only used for manually specifying a build version
...
- The repository provides the generic `build_python_native_matrix.py` and `build_python_native_wheel.py` tools and the `ge_python_pass_native_wheel_matrix` target in the `python_pass_native_build` directory. The tools only consume the CMake-generated `--component-config` and automatically sniff available `python3.9` through `python3.14` in CI/local environments; the matrix does not reconfigure the entire repository CMake, but reuses compile/link metadata generated by the parent build, only replacing Python include/lib before recompiling `libge_python_pass_bridge.so` and `_ge_pass_native.so`
...
5. If no match is found or candidate loading fails, runtime fallback codegen is triggered, calling `ge.passes.fallback_runtime.run_fallback_codegen()`; if fallback artifacts are still unavailable, the loader directly returns failure.
...
`ge.passes.fallback_runtime` in the main wheel is responsible for:
...
- It is more appropriate for `ge.passes.fallback_runtime` to uniformly decide "prebuilt / fallback" artifact selection
