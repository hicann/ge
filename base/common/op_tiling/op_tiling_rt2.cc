...
gert::KernelContextHolder BuildTilingParseContextHolder(
    const ge::OpDescPtr &op_desc, const char_t *const compile_info, const fe::PlatFormInfos &platform_infos,
    const gert::OpImplKernelRegistry::OpImplFunctions *const funcs) {
...
ge::Status AddDataNodeForAtomic(ge::ComputeGraphPtr &graph, ge::NodePtr &clean_node, size_t output_size) {
  ...
  ge::graphStatus ret =
      ge::GraphUtils::AddEdge(workspace_data_node->GetOutDataAnchor(0), clean_node->GetInDataAnchor(0));
  if (ret != ge::SUCCESS) {
...
ge::graphStatus UpdateNodeShapeBySliceInfo(const ffts::ThreadSliceMapDyPtr &slice_info_ptr,
                                           const ge::OpDescPtr &op_desc, const uint32_t thread_id,
                                           vector<int64_t> &ori_shape, bool &same_shape) {
...
ge::graphStatus GetNodeDeterministic(const ge::OpDescPtr &op_desc, int32_t &deterministic, bool &has_deterministic) {
  ...
  if (node_deterministic_str == nullptr) {
    GELOGE(ge::FAILED, "Attr %s of node %s should be string.", kDeterministicAttr, op_desc->GetNamePtr());
    return ge::GRAPH_FAILED;
  }
  int32_t node_deterministic = 0;
  if ((ge::ConvertToInt32(*node_deterministic_str, node_deterministic) != ge::SUCCESS) || (node_deterministic < 0) ||
      (node_deterministic > kMaxDeterministic)) {
    GELOGE(ge::FAILED, "Attr %s of node %s should be in {0,1}, but got %s.", kDeterministicAttr, op_desc->GetNamePtr(),
           node_deterministic_str->c_str());
    return ge::GRAPH_FAILED;
  }
...
ge::graphStatus GetNodeDeterministicLevel(const ge::OpDescPtr &op_desc, int32_t &deterministic_level,
  ...
  GE_ASSERT_NOTNULL(op_desc);
  has_deterministic_level = false;
  if (!ge::AttrUtils::HasAttr(op_desc, kDeterministicLevelAttr)) {
    return ge::GRAPH_SUCCESS;
  }

  const std::string *const node_deterministic_level_str = ge::AttrUtils::GetStr(op_desc, kDeterministicLevelAttr);
  if (node_deterministic_level_str == nullptr) {
    GELOGE(ge::FAILED, "Attr %s of node %s should be string.", kDeterministicLevelAttr, op_desc->GetNamePtr());
    return ge::GRAPH_FAILED;
  }
  int32_t node_deterministic_level = 0;
  if ((ge::ConvertToInt32(*node_deterministic_level_str, node_deterministic_level) != ge::SUCCESS) ||
      (node_deterministic_level < 0) || (node_deterministic_level > kMaxDeterministicLevel)) {
    GELOGE(ge::FAILED, "Attr %s of node %s should be in {0,1,2,3}, but got %s.", kDeterministicLevelAttr,
           op_desc->GetNamePtr(), node_deterministic_level_str->c_str());
    return ge::GRAPH_FAILED;
  }
  deterministic_level = node_deterministic_level;
...
ge::graphStatus RtParseAndTiling(const ge::Operator &op, const char_t *const compile_info,
  ...
  GE_ASSERT_SUCCESS(
      ge::CoreNumUtils::UpdatePlatformInfosWithOpDesc(platform_info, op_desc, platform_infos_bak, is_op_core_num_set));
   ...
   * gert_TilingContextBuilder_SetDeterministicLevel实现，调用前可以通过该符号是否为空或者aclsysGetVersionNum > 80500000
   * (8.5.0) 判断是否是支持该能力的metadef版本。
   */
  auto context_builder = gert::TilingContextBuilder();
  const gert::KernelContextHolder tiling_context_holder =
      context_builder.CompileInfo(*parse_context_holder->context_->GetOutputPointer<void **>(0))
...
static gert::KernelContextHolder DoTilingParse(const fe::PlatFormInfos &platform_infos, const ge::OpDescPtr &op_desc,
                                               void *handle) {
  const std::string tiling_parse_func_name("TilingParse");
  TilingParse const tiling_parse_func = reinterpret_cast<TilingParse>(mmDlsym(handle, tiling_parse_func_name.c_str()));
  GE_ASSERT_NOTNULL(tiling_parse_func);
  auto autofuse_tiling_parse_context_holder =
...
static ge::graphStatus AutofuseNodeTiling(const ge::Operator &op, const fe::PlatFormInfos &platform_infos,
  ...
  GE_MAKE_GUARD(close_handle, [&handle]() { (void)mmDlclose(handle); });
  ...
  const auto autofuse_tiling_context_holder = gert::KernelRunContextBuilder()
...
ge::graphStatus AicoreRtParseAndTiling(const ge::Operator &op, const fe::PlatFormInfos &platform_infos,
  ...
  if (ge::OpTypeUtils::IsAutofuseNode(op_desc)) {
    ...
    GE_ASSERT_GRAPH_SUCCESS(fe::PlatformInfoManager::GeInstance().GetPlatformInfoWithOutSocVersion(
        ge_platform_infos, opti_compilation_info));
...
ge::graphStatus AtomicRtParseAndTiling(const ge::Operator &op, const fe::PlatFormInfos &platform_infos,
  ...
  GE_ASSERT_TRUE(ge::AttrUtils::GetStr(origin_node->GetOpDesc(), attr_name, op_compile_info_json),
...
ge::graphStatus GetDeterministicLevel(int32_t &deterministic_level, bool &has_deterministic_level) {
  ...
  has_deterministic_level =
      (ge::GetThreadLocalContext().GetOption("ge.deterministicLevel", deterministic_level_str) == ge::GRAPH_SUCCESS);
  ...
  if (ret != ge::SUCCESS || deterministic_level < 0 || deterministic_level > kMaxDeterministicLevel) {
    ...
    GELOGE(ge::FAILED, "Valid values for %s are {0,1,2,3}, given value is %s", readable_name.c_str(),
           deterministic_level_str.c_str());
    (void)REPORT_PREDEFINED_ERR_MSG(
        "E10001", std::vector<const char_t *>({"parameter", "value", "reason"}),
        std::vector<const char_t *>({readable_name.c_str(), deterministic_level_str.c_str(), error_msg.c_str()}));
    return ge::GRAPH_FAILED;
  }
  return ge::GRAPH_SUCCESS;
}
