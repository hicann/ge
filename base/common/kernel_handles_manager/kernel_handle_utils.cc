...
void LogLaunchKernelParam(const LaunchKernelParam &launch_param) {
  GELOGI("LaunchKernelParam info: block_dim = %u, stream = %p, args = %p, "
      "args_size = %u, is_host_args = %d",
      launch_param.block_dim,
      launch_param.stream,
      launch_param.args,
      launch_param.args_size,
      launch_param.is_host_args);
  GELOGI("LaunchKernelConfig info: schedule_mode = %u, local_memory_size = %u, "
      ...
      launch_param.launch_config.schedule_mode,
      launch_param.launch_config.local_memory_size,
      static_cast<int>(launch_param.launch_config.engine_type),
      launch_param.launch_config.block_dim_offset,
      launch_param.launch_config.is_block_task_prefetch,
      launch_param.launch_config.is_data_dump,
...
graphStatus KernelHandleUtils::LaunchKernel(const rtFuncHandle func_handle, const LaunchKernelParam &launch_param) {
  ...
  attrs[actual_cfg_num].value.schemMode = launch_param.launch_config.engine_type;
  actual_cfg_num++;
  ...
  if (launch_param.is_host_args) {
    GE_ASSERT_RT_OK(rtsLaunchKernelWithHostArgs(func_handle, launch_param.block_dim, launch_param.stream,
        &rt_launch_config, launch_param.args, launch_param.args_size, nullptr, 0));
  } else {
