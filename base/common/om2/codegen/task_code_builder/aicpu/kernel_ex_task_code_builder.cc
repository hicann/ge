...
Status KernelExTaskCodeBuilder::InitLaunchInfo(const TaskSemanticContributeContext &context) {
  ...
  GE_ASSERT_TRUE(func_handle_it != context.func_handle_indices->end(),
    "[OM2] func handle not found, key: %s", context.op_desc->GetType().c_str());
...
Status KernelExTaskCodeBuilder::InitArgsTableInfo(const TaskSemanticContributeContext &context) {
  ...
  if (!ext_info.empty()) {
    ...
    GE_CHK_STATUS_RET(ext_handle.Parse(ext_info), "[OM2]Parse kernel ext info failed, ext_info_size: %zu", ext_info.size());
...
Status KernelExTaskCodeBuilder::InitTaskExtInfo(const TaskSemanticContributeContext &context) {
  ...
  const uint32_t num_outputs = static_cast<uint32_t>(context.op_desc->GetOutputsSize());
  om2::Om2AicpuExtInfoHandler ext_handle(context.op_desc->GetName(), num_inputs, num_outputs, unknown_type);
  GE_CHK_STATUS_RET(ext_handle.Parse(ext_info), "[OM2][Parse][KernelExtInfo] failed, ext_info_size: %zu", ext_info.size());
  ...
  if (all_shape) {
    ...
    for (uint32_t i = 0U; i < num_inputs; i++) {
      ...
      GE_CHK_STATUS_RET(ext_handle.UpdateInputShapeAndType(i, *input_desc),
                        "[OM2][Call][UpdateInputShapeAndType] Input[%u] update input shape failed, op:%s.",
                        i, context.op_desc->GetName().c_str());
    }
    if (unknown_type != DEPEND_COMPUTE) {
      for (uint32_t i = 0U; i < num_outputs; ++i) {
        ...
        GE_CHK_STATUS_RET(ext_handle.UpdateOutputShapeAndType(i, *output_desc),
                          "[OM2][Call][UpdateOutputShapeAndType] Output[%u] update output shape failed, op:%s.",
                          i, context.op_desc->GetName().c_str());
      }
    }
  }
...
FunctionDef *KernelExTaskCodeBuilder::BuildTfAicpuKernelTaskDistribute() const {
  ...
  return ast_.DefineFunction("TfAicpuKernelTaskDistribute", {
    io_addrs, args_info, kernel_buf, kernel_buf_size, func_handle, block_dim, stream, config },
    "aclError", {
      ...
      ChkStatus(AclrtLaunchKernelV2(func_handle,block_dim,kernel_buf,kernel_buf_size,
        config,stream)),
...
FunctionDef *KernelExTaskCodeBuilder::BuildAssembleTfAicpuArgs() const {
  ...
  return ast_.DefineFunction("AssembleTfAicpuArgs",{
    kernel_ex_args, kernel_ex_args_len, target_args_ptr, target_args_len, tf_ai_cpu_ex_info },
    "aclError", {
      ast_.VarDecl(tmp_args),
      tmp_args.Resize(target_args_len),
      ChkStatus(MemcpyS(tmp_args.Data(), target_args_len, kernel_ex_args,
        kernel_ex_args_len)),
      ChkStatus(MemcpyS(tmp_args.Data() + ast_.Sizeof("uint32_t") * 2,
        ast_.Sizeof("uint64_t"),tf_ai_cpu_ex_info.Arrow("sessionID").Addr(),
        ast_.Sizeof("uint64_t"))),
      ChkStatus(MemcpyS(tmp_args.Data() + ast_.Sizeof("uint32_t") * 2
        + ast_.Sizeof("uint64_t") * 2, ast_.Sizeof("uint64_t"),
        tf_ai_cpu_ex_info.Arrow("kernelID").Addr(), ast_.Sizeof("uint64_t"))),
      ChkStatus(MemcpyS(tmp_args.Data() + ast_.Sizeof("uint32_t") * 2
        + ast_.Sizeof("uint64_t") * 9, ast_.Sizeof("uint64_t"),
        tf_ai_cpu_ex_info.Arrow("workspaceBaseAddr").Addr(), ast_.Sizeof("uint64_t"))),
      ChkStatus(MemcpyS(tmp_args.Data() + ast_.Sizeof("uint32_t") * 2
        + ast_.Sizeof("uint64_t") * 10, ast_.Sizeof("uint64_t"),
        tf_ai_cpu_ex_info.Arrow("inputOutputAddr").Addr(), ast_.Sizeof("uint64_t"))),
      ChkStatus(MemcpyS(tmp_args.Data() + ast_.Sizeof("uint32_t") * 2
        + ast_.Sizeof("uint64_t") * 11, ast_.Sizeof("uint64_t"),
        tf_ai_cpu_ex_info.Arrow("extInfoLen").Addr(), ast_.Sizeof("uint64_t"))),
      ChkStatus(MemcpyS(tmp_args.Data() + ast_.Sizeof("uint32_t") * 2
        + ast_.Sizeof("uint64_t") * 12, ast_.Sizeof("uint64_t"),
        tf_ai_cpu_ex_info.Arrow("extInfoAddr").Addr(), ast_.Sizeof("uint64_t"))),
      ChkStatus(MemcpyS(target_args_ptr, target_args_len, tmp_args.Data(),
        target_args_len)),
      ast_.Return("ACL_SUCCESS")
    });
}
...
FunctionDef *KernelExTaskCodeBuilder::BuildAssembleTfAicpuExSessionIdInfo() const {
  ...
  return ast_.DefineFunction("AssembleTfAicpuExSessionIdInfo",{session_id, op_kernel_size,
    func_handle,block_dim,stream, config, tf_ai_cpu_ex_info,mem_ptrs },
  "aclError", {
    ast_.Assign(tf_ai_cpu_ex_info.Arrow("sessionID"), ast_.Deref(session_id)),
    ast_.VarDecl(tmp_args),
    tmp_args.Resize(op_kernel_size),
    ast_.VarDecl(iow_addrs),
    ChkStatus(MemcpyS(tmp_args.Data() + ast_.Sizeof("uint32_t") * 1,
   ast_.Sizeof("uint64_t"),session_id, ast_.Sizeof("uint64_t"))),
    ast_.VarDecl(device_id, 0),
    ChkStatus(ast_.Call("aclrtGetDevice", {device_id.Addr() })),
    ast_.VarDecl(device_base, nullptr),
    ChkStatus(AclrtMallocAlign32(device_base.Addr(), op_kernel_size,
      "ACL_MEM_TYPE_HIGH_BAND_WIDTH")),
    mem_ptrs.PushBack(device_base),
    ChkStatus(AclrtMemcpy(device_base, op_kernel_size, tmp_args.Data(),
      op_kernel_size, "ACL_MEMCPY_HOST_TO_DEVICE")),
    ...
    ChkStatus(ast_.Call("aclrtSynchronizeStream", {stream })),
    ast_.Return("ACL_SUCCESS")
  });
}
...
FunctionDef *KernelExTaskCodeBuilder::BuildAssembleTfAicpuExKernelIdInfo() const {
  ...
  auto tf_ai_cpu_ex_info = ast_.Var("TfAiCpuExInfo *", "tf_ai_cpu_ex_info");
  return ast_.DefineFunction("AssembleTfAicpuExKernelIdInfo",{kernel_id, tf_ai_cpu_ex_info },
    "aclError", {
...
FunctionDef *KernelExTaskCodeBuilder::BuildAssembleTfAicpuExWorkSpaceAddrInfo() const {
  ...
  auto mem_ptrs = ast_.Var("std::vector<void *> &", "mem_ptrs");
  return ast_.DefineFunction("AssembleTfAicpuExWorkSpaceAddrInfo",{kernel_ex_task_info,
    kernel_ex_task_info_len,tf_ai_cpu_ex_info,mem_ptrs },
    "aclError", {
      ast_.VarDecl(workspace_dev_ptr, nullptr),
      ChkStatus(AclrtMallocAlign32(workspace_dev_ptr.Addr(), kernel_ex_task_info_len,
        "ACL_MEM_MALLOC_HUGE_FIRST")),
      mem_ptrs.PushBack(workspace_dev_ptr),
      ChkStatus(AclrtMemcpy(workspace_dev_ptr, kernel_ex_task_info_len,
        kernel_ex_task_info, kernel_ex_task_info_len, "ACL_MEMCPY_HOST_TO_DEVICE")),
      ast_.Assign(tf_ai_cpu_ex_info.Arrow("workspaceBaseAddr"), ast_.StaticCast("uint64_t",
        ast_.ReinterpretCast("uintptr_t", workspace_dev_ptr))),
      ast_.Return("ACL_SUCCESS")
 	     });
}
...
FunctionDef *KernelExTaskCodeBuilder::BuildAssembleTfAicpuExInputOutputAddrInfo() const {
  auto args_info = ast_.Var("ArgsInfo *", "args_info");
  auto tf_ai_cpu_ex_info = ast_.Var("TfAiCpuExInfo *", "tf_ai_cpu_ex_info");
  return ast_.DefineFunction("AssembleTfAicpuExInputOutputAddrInfo",{
    args_info,
    tf_ai_cpu_ex_info },
    "aclError", {
      ChkNotNull(args_info),
      ast_.Assign(tf_ai_cpu_ex_info.Arrow("inputOutputAddr"), ast_.StaticCast("uint64_t",
...
FunctionDef *KernelExTaskCodeBuilder::BuildAssembleTfAicpuExExtInfo() const {
  ...
  auto mem_ptrs = ast_.Var("std::vector<void *> &", "mem_ptrs");
  return ast_.DefineFunction("AssembleTfAicpuExExtInfo",{
    kernel_ex_ext_info,
    kernel_ex_ext_info_len,
    tf_ai_cpu_ex_info,
    mem_ptrs },
    "aclError", {
      ast_.VarDecl(ext_info_dev_ptr, nullptr),
      ChkStatus(AclrtMallocAlign32(ext_info_dev_ptr.Addr(), kernel_ex_ext_info_len,
        "ACL_MEM_MALLOC_HUGE_FIRST")),
      mem_ptrs.PushBack(ext_info_dev_ptr),
      ChkStatus(AclrtMemcpy(ext_info_dev_ptr, kernel_ex_ext_info_len,
        kernel_ex_ext_info, kernel_ex_ext_info_len, "ACL_MEMCPY_HOST_TO_DEVICE")),
      ast_.Assign(tf_ai_cpu_ex_info.Arrow("extInfoAddr"),ast_.StaticCast("uint64_t",
        ast_.ReinterpretCast("uintptr_t", ext_info_dev_ptr))),
      ast_.Assign(tf_ai_cpu_ex_info.Arrow("extInfoLen"), ast_.StaticCast("uint64_t",
        kernel_ex_ext_info_len)),
      ast_.Return("ACL_SUCCESS")
    });
}
