...
class CodeGeneratorBase {
 ...
 protected:
  ExprRef ChkStatus(Arg expr) const { return ast_.Call("OM2_CHK_STATUS", {expr}); }
  ExprRef ChkRt(Arg expr) const { return ast_.Call("OM2_CHK_RT", {expr}); }
  ExprRef ChkNotNull(Arg expr) const { return ast_.Call("OM2_CHK_NOTNULL", {expr}); }
  ExprRef ChkTrue(Arg expr) const { return ast_.Call("OM2_CHK_TRUE", {expr}); }
  ExprRef GetAddr(Arg mem_ptr, Arg offset) const { return ast_.Call("GET_ADDR", {mem_ptr, offset}); }
  ExprRef FlattenHostArgs(std::initializer_list<Arg> args) const { return ast_.Call("FlattenHostArgs", args); }
  ExprRef FlattenHostArgs(const std::vector<Arg> &args) const { return ast_.Call("FlattenHostArgs", args); }
  ...
  ExprRef AclmdlRIBuildBegin(Arg model_handle, Arg priority) const { return ast_.Call("aclmdlRIBuildBegin", {model_handle, priority}); }
  ExprRef AclmdlRIBuildEnd(Arg model_handle, Arg stream) const { return ast_.Call("aclmdlRIBuildEnd", {model_handle, stream}); }
  ExprRef AclmdlRIDestroy(Arg model_handle) const { return ast_.Call("aclmdlRIDestroy", {model_handle}); }
  ...
  ExprRef AclmdlRIExecute(Arg model_handle, Arg timeout) const { return ast_.Call("aclmdlRIExecute", {model_handle, timeout}); }
  ...
  ExprRef AclrtMalloc(Arg dev_ptr, Arg size, Arg policy) const { return ast_.Call("aclrtMalloc", {dev_ptr, size, policy}); }
  ...
  ExprRef AclrtCreateNotify(Arg notify, Arg notify_flag) const { return ast_.Call("aclrtCreateNotify", {notify, notify_flag}); }
  ...
  ExprRef AclrtCreateLabel(Arg label) const { return ast_.Call("aclrtCreateLabel", {label}); }
  ...
  ExprRef AclrtDestroyLabel(Arg label) const { return ast_.Call("aclrtDestroyLabel", {label}); }
  ExprRef AclrtDestroyLabelList(Arg rt_label_list) const { return ast_.Call("aclrtDestroyLabelList", {rt_label_list}); }
  ExprRef AclrtDestroyEvent(Arg event) const { return ast_.Call("aclrtDestroyEvent", {event}); }
  ExprRef AclrtDestroyNotify(Arg notify) const { return ast_.Call("aclrtDestroyNotify", {notify}); }
  ExprRef AclrtDestroyStream(Arg stream) const { return ast_.Call("aclrtDestroyStream", {stream}); }
  ExprRef AclrtBinaryUnLoad(Arg bin_handle) const { return ast_.Call("aclrtBinaryUnLoad", {bin_handle}); }
  ExprRef AclrtFree(Arg dev_ptr) const { return ast_.Call("aclrtFree", {dev_ptr}); }
  ExprRef AclrtRecordEvent(Arg event, Arg stream) const { return ast_.Call("aclrtRecordEvent", {event, stream}); }
  ExprRef AclrtResetEvent(Arg event, Arg stream) const { return ast_.Call("aclrtResetEvent", {event, stream}); }
  ExprRef AclrtSetLabel(Arg label, Arg stream) const { return ast_.Call("aclrtSetLabel", {label, stream}); }
  ExprRef AclrtActiveStream(Arg active_stream, Arg stream) const {
    return ast_.Call("aclrtActiveStream", {active_stream, stream});
  }
  ExprRef AclrtRecordNotify(Arg notify, Arg stream) const { return ast_.Call("aclrtRecordNotify", {notify, stream}); }
  ...
  ExprRef RtKernelFusionStart(Arg stream) const { return ast_.Call("rtKernelFusionStart", {stream}); }
  ExprRef RtKernelFusionEnd(Arg stream) const { return ast_.Call("rtKernelFusionEnd", {stream}); }
  ...
  ExprRef AclmdlRIEndTask(Arg model_handle, Arg stream) const { return ast_.Call("aclmdlRIEndTask", {model_handle, stream}); }
