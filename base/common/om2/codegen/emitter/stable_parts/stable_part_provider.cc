  ...
  switch (id) {
    ...
    case StablePartId::kInterfaceDumpApis:
      output =
          ...
          "using GetDataDumpEnabledInfoFunc = int32_t (*)(void *instance_handle, struct GertModelDumpEnabledInfo "
          "*info);\n"
    ...
    case StablePartId::kCustomTaskHelpers:
      output =
          "using GeTensorPtr = std::shared_ptr<gert::Tensor>;\n"
          "\n"
          "GeTensorPtr BuildGeTensor(const Om2Tensor &om2_tensor) {\n"
          "  gert::StorageShape storage_shape;\n"
          "  for (auto i = 0U; i < om2_tensor.shape_dims_num; ++i) {\n"
          "    auto dim = om2_tensor.shape_dims[i];\n"
          "    (void)storage_shape.MutableStorageShape().AppendDim(dim);\n"
          "    (void)storage_shape.MutableOriginShape().AppendDim(dim);\n"
          "  }\n"
          "  gert::StorageFormat storage_format(static_cast<ge::Format>(om2_tensor.format),\n"
          "                                     static_cast<ge::Format>(om2_tensor.format), {});\n"
          "  return std::make_shared<gert::Tensor>(storage_shape, storage_format, gert::kOnDeviceHbm,\n"
          "                                        static_cast<ge::DataType>(om2_tensor.data_type),\n"
          "                                        reinterpret_cast<gert::TensorAddress>(om2_tensor.device_address));\n"
          "}\n"
          "\n"
          ...
          "                                                 const std::vector<GeTensorPtr> &inputs,\n"
          "                                                 const std::vector<GeTensorPtr> &outputs, void *allocator,\n"
          "                                                 void *stream) {\n"
          ...
          "    return holder;\n"
          "  }\n"
          "  auto compute_node_info = PtrToPtr<uint8_t, "
          "gert::ComputeNodeInfo>(holder.compute_node_extend_holder_.get());\n"
          ...
          "    holder.value_holder_[holder_inputs.size() + i].Set(holder_outputs[i].first, holder_outputs[i].second);\n"
          "  }\n"
          "\n"
          "  return holder;\n"
          "}\n"
          ...
          "std::vector<Om2Tensor> &inputs,\n"
          "                                  const std::vector<Om2Tensor> &outputs, aclrtStream stream) {\n"
          "  std::vector<GeTensorPtr> op_inputs;\n"
          "  std::vector<GeTensorPtr> op_outputs;\n"
          "  for (auto &elem : inputs) {\n"
          "    op_inputs.emplace_back(BuildGeTensor(elem));\n"
          "  }\n"
          "  for (auto &elem : outputs) {\n"
          "    op_outputs.emplace_back(BuildGeTensor(elem));\n"
          "  }\n"
          "  auto allocator = std::make_shared<AllocatorFaker>();\n"
          "  auto eager_context_holder =\n"
          "      BuildKernelContextHolder(op_name, op_type, op_inputs, op_outputs, allocator.get(), stream);\n"
