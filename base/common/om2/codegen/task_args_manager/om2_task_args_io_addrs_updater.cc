...
#include "om2_task_args_io_addrs_updater.h"
...
Status ArgsIoAddrsUpdater::Init(std::vector<MemAllocation> &logical_mem_allocations,
  ...
  op_info_ = op_info;
  GELOGI("[OM2][Args][Init] op_name:%s, op_type:%s, logical_addrs size:%zu.", op_info_.op_name.c_str(),
  ...
  for (size_t io_index = 0U; io_index < logical_addrs.size(); io_index++) {
    ...
    if (id_addr_matched >= logical_mem_allocations.size()) {
      REPORT_INNER_ERR_MSG("E19999", "[OM2]Cannot match any logic mem item");
      GELOGE(INTERNAL_ERROR, "[OM2]addr:0x%" PRIx64 " cannot match any logic mem item", addr);
...
Status ArgsIoAddrsUpdater::SetArgIoAddrs(const std::vector<uint64_t> &active_mem_base_addr, void *const host_args,
  ...
  GELOGI("[OM2][Args][Updater] op_name:%s, op_type:%s, host_args:%p.", op_info_.op_name.c_str(),
  ...
  for (const auto &item : v_mem_allocation_id_and_offset_) {
    ...
    GELOGI("[OM2][Args][Updater] op_name:%s, op_type:%s, active_addr[%zu]:0x%" PRIx64
           ", id:%zu, "
