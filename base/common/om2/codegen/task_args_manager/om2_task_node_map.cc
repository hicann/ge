...
Status TaskNodeMap::Init(const ComputeGraphPtr &graph, size_t task_num) {
  ...
  for (const auto &node : graph->GetAllNodes()) {
    ...
    GELOGD("[OM2] node index %lld, name %s, type %s", node->GetOpDesc()->GetId(), node->GetName().c_str(),
           node->GetType().c_str());
  }
...
Status TaskNodeMap::AddRelation(size_t task_index, int64_t node_id) {
  ...
  if (iter == node_ids_to_node_.cend()) {
    GELOGE(PARAM_INVALID, "[OM2] Failed to add relation between task %zu and node %lld, invalid node id", task_index,
           node_id);
    return PARAM_INVALID;
  }
  if (task_index >= task_indexes_to_node_.size()) {
    GELOGE(PARAM_INVALID, "[OM2] Failed to add relation between task %zu and node %lld(%s), task index out of range",
