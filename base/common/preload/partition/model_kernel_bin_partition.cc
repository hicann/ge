...
namespace ge {
Status ModelKernelBinPartition::Init(const GeModelPtr &ge_model, const uint8_t type) {
  (void)ge_model;
  (void)type;
  GELOGD("begin to generate kernel bin.");
  const std::string str = "this is model kernel bin partition";
  ...
  GELOGD("end generate kernel bin info, buff_size_:%u", buff_size_);
  return SUCCESS;
}
}  // namespace ge
