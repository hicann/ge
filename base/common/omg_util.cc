...
Status SetNextIteration(const NodePtr &node, const NodePtr &next_node) {
  ...
  GE_CHK_STATUS_RET_NOLOG(SetIterationName(node->GetOpDesc(), next_node->GetName()));
  GE_CHK_STATUS_RET_NOLOG(SetIterationName(next_node->GetOpDesc(), node->GetName()));
  return SUCCESS;
}
...
void AlignMemSize(int64_t &mem_size, const int64_t align_size) {
  ...
  mem_size = (mem_size + align_size - 1L) / align_size * align_size;
}
...
Status GetMemorySize(const NodePtr &node, int64_t &output_size) {
  GE_CHECK_NOTNULL(node->GetOpDesc());
  const auto output_op_desc = node->GetOpDesc()->GetOutputDescPtr(kBufferPoolNodeOutIndex);
  GE_CHECK_NOTNULL(output_op_desc);
  int64_t size = 0;
  const auto ret = ge::TensorUtils::GetSize(*output_op_desc, size);
