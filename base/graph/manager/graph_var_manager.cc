...
ge::Status InitVarIfHasInitValue(const VarDevAddrMgr *const var_mgr, void *var_dev_addr) {
  ...
  if (init_value != nullptr) {
    ...
    GE_ASSERT_TRUE(init_value_size <= static_cast<size_t>(var_size),
                   "_init_value size too big."
                   " var_size: %" PRId64 ", init_value_size: %zu",
                   var_size, init_value_size);
    GE_CHK_ACL_RET(aclrtMemcpy(var_dev_addr, static_cast<uint64_t>(var_size), init_value->GetData().GetData(),
                               init_value_size, ACL_MEMCPY_HOST_TO_DEVICE));
    GELOGI("variable offset[%p] has _init_value attr, init value success, var_dev_addr: %p, tensor size: %" PRId64
           ","
           " value size: %zu, ",
           var_mgr->logic_addr, var_dev_addr, var_size, init_value_size);
  }
  return SUCCESS;
}
}  // namespace
...
ge::Status VarResource::GetVarAddr(const std::string &var_name, const ge::GeTensorDesc &tensor_desc,
  ...
  if (dev_ptr == nullptr) {
    REPORT_INNER_ERR_MSG("E19999",
                         "Param dev_ptr is nullptr, var_name:%s, session_id:%" PRIu64
                         ", "
                         "check invalid",
                         var_name.c_str(), session_id_);
    GELOGE(FAILED, "[Check][Param] Param dev_ptr is nullptr, var_name:%s, session_id:%" PRIu64 "", var_name.c_str(),
           session_id_);
    return FAILED;
  }
  ...
  if (iter == var_addr_mgr_map_.end()) {
    REPORT_INNER_ERR_MSG("E19999",
                         "var_key:%s can't find in var_addr_mgr_map_, var_name:%s, session_id:%" PRIu64
                         ", "
                         "check invalid",
...
Status VarResource::GetFileConstantReuseAddr(const OpDescPtr &op_desc, uint8_t **const dev_ptr,
  ...
  for (const auto &var_maps : var_addr_mgr_map_) {
    ...
    if (memcmp(values->Get(), tmp_weight->MutableData().GetAlignedPtr()->Get(), weight_size) == 0) {
      ...
      GELOGD("[IMAS]AssignVarMem Set session_%" PRIu64 " name[%s] output[%d] offset to [%" PRIu64 "] size[%" PRIu64
             "]"
             "realsize[%" PRIu64 "], reuse address of node: %s.",
             session_id_, op_desc->GetName().c_str(), 0, var_map.offset,
             real_size + (kSessionMemAlignSize * kSessionMemAlignUnit), real_size, var_map.op_desc->GetName().c_str());
...
ge::Status VarResource::SaveVarAddr(const std::string &var_name, const ge::GeTensorDesc &tensor_desc,
  ...
  if (var_addr_mgr_map_.count(var_key) == 0U) {
    ...
    var_dev_addr_mgr_map_[logic_address] = {tensor_desc, reinterpret_cast<uint8_t *>(logic_address), device_addr,
                                            is_extern_mem};
  ...
  REPORT_INNER_ERR_MSG("E19999",
                       "var_key:%s conflict in var_addr_mgr_map_, var_name:%s, session_id:%" PRIu64
                       ", "
                       "check invalid",
                       var_key.c_str(), var_name.c_str(), session_id_);
...
bool VarResource::IsVarReady(const std::string &var_name, const ge::GeTensorDesc &tensor_desc,
...
ge::Status VarResource::RecordStagedVarDesc(const uint32_t graph_id, const std::string &var_name,
...
ge::Status VarResource::RenewCurVarDesc(const std::string &var_name, const GeTensorDesc &tensor_desc) {
  const auto var_key_name = GetBatchVarKeyName(var_name);
  if (cur_var_tensor_desc_map_.count(var_key_name) == 0U) {
    GELOGI("There is no this node[%s] key[%s] in var tensor_desc map. so no need renew!", var_name.c_str(),
           var_key_name.c_str());
    return SUCCESS;
  }
  ...
  GELOGI("Renew variable data for %s from format %s to %s success", var_name.c_str(),
         TypeUtils::FormatToSerialString(curr_desc.GetFormat()).c_str(),
         TypeUtils::FormatToSerialString(tensor_desc.GetFormat()).c_str());
...
ge::Status VarResource::RenewCurVarDesc(const std::string &var_name, const ge::OpDescPtr &op_desc) {
  ...
  if (op_desc == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Param op_desc is nullptr, var_name:%s, session_id:%" PRIu64 ", check invalid",
                         var_name.c_str(), session_id_);
    GELOGE(FAILED, "[Check][Param] input opdesc is nullptr, var_name:%s, session_id:%" PRIu64 "", var_name.c_str(),
           session_id_);
    return FAILED;
  }
  ...
  GELOGI("Trans variable data for %s from format %s to %s success", var_name.c_str(),
         TypeUtils::FormatToSerialString(curr_desc.GetFormat()).c_str(),
  ...
  if (iter == var_addr_mgr_map_.end()) {
    REPORT_INNER_ERR_MSG("E19999",
                         "var_key:%s can't find in var_addr_mgr_map_, var_name:%s, "
                         "session_id:%" PRIu64 ", op:%s(%s), check invalid",
                         key.c_str(), var_name.c_str(), session_id_, op_desc->GetName().c_str(),
                         op_desc->GetType().c_str());
    GELOGE(FAILED,
           "[Check][Param] var_key:%s can't find in var_addr_mgr_map_, var_name:%s, "
           "session_id:%" PRIu64 ", op:%s(%s)",
...
ge::Status VarResource::CheckLogicAddrValid(const uint32_t device_id, const uint8_t *const logic_addr,
                                            uint64_t &inner_offset_tmp, uint64_t &logic_addr_tmp) {
  GELOGD("[VarResource] Begin to check logic addr is valid.");
  inner_offset_tmp = 0U;
...
Status VarResource::VarResourceToDeserial(const deployer::VarResourceInfo *const var_resource_info) {
  ...
  for (const auto &x : var_resource_info->var_addr_mgr_map()) {
    ...
    const struct VarAddrMgr addr_mgr = {tensor_desc, PtrToPtr<void, uint8_t>(ValueToPtr(x.second.address())),
                                        static_cast<uint64_t>(x.second.offset()),
                                        static_cast<rtMemType_t>(x.second.memory_type()), nullptr};
...
Status HbmMemResource::AssignVarMem(const std::string &var_name, const uint64_t size, const uint64_t session_id,
  ...
  GELOGI("[IMAS]AssignVarMem Set session_%" PRIu64
         " name[%s] output[%d] offset to [%zu] size["
         "%" PRIu64 "] realsize[%" PRIu64 "].",
...
Status RdmaMemResource::AssignVarMem(const std::string &var_name, const uint64_t size, const uint64_t session_id,
  ...
  if (buffer == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "malloc rdma memory fail, var_size:%" PRIu64 ", var_name:%s", size,
                         var_name.c_str());
  ...
  var_mem_size_ += size;
  GELOGI("[IMAS]AssignVarMem Set session_%" PRIu64 " name[%s] output[%d] addr to [%p] size[%" PRIu64 "].", session_id,
         var_name.c_str(), 0, buffer, size);
  return SUCCESS;
}
...
Status HostMemResource::AssignVarMem(const std::string &var_name, const uint64_t size, const uint64_t session_id,
  ...
  if (buffer == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "malloc host memory fail, var_size:%" PRIu64 ", var_name:%s", size,
                         var_name.c_str());
...
int64_t VarManager::GetVarMemSize(const rtMemType_t memory_type) const {
  ...
  if (mem_resource == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Find no mem_resource in map, memory_type:%u, session_id:%" PRIu64 "", memory_type,
                         session_id_);
...
bool VarManager::IsVarReady(const std::string &var_name, const ge::GeTensorDesc &tensor_desc,
                            const uint32_t device_id) const {
  const std::lock_guard<std::recursive_mutex> lock(mutex_);
...
ge::Status VarManager::VarManagerToSerial(const uint64_t session_id, deployer::VarManagerInfo &info) const {
  ...
  for (auto &mem_resource : mem_resource_map_) {
    ...
    if (mem_resource.second == nullptr) {
      REPORT_INNER_ERR_MSG("E19999",
                           "Find no mem_resource in map, memory_type:%u, session_id:"
                           "%" PRIu64 ".",
                           mem_resource.first, session_id_);
      GELOGE(ge::INTERNAL_ERROR,
             "[Check][Param] MemResource is invalid, memory_type:%u, "
             "session_id:%" PRIu64 ".",
             mem_resource.first, session_id_);
      return INTERNAL_ERROR;
    }
...
ge::Status VarManager::VarManagerToDeserial(const uint64_t session_id, const deployer::VarManagerInfo &info) {
  ...
  GELOGD(
      "[VarManager] Success to deserial var manager objection, the session id is "
      "%" PRIu64 ".",
      session_id);
  return SUCCESS;
}
...
ge::Status VarManager::RecordStagedVarDesc(const uint32_t graph_id, const std::string &var_name,
                                           const GeTensorDesc &tensor_desc) {
  const std::lock_guard<std::recursive_mutex> lock(mutex_);
...
uint8_t *VarManager::GetVarMemoryAddr(const std::string &graph_name, const uint8_t *const logic_addr,
  ...
  if ((external_var_addr_ != nullptr) && (memory_type == RT_MEMORY_HBM)) {
    ...
    GE_ASSERT_TRUE(PtrToValue(logic_addr) >= var_mem_logic_base_,
                   "logic offset %lu cannot be smaller than logic base %lu", PtrToValue(logic_addr),
                   var_mem_logic_base_);
    ...
    GELOGI("external var scene, real_offset is %lu, logic offset %lu, base %lu, ret_p %p", real_offset,
           PtrToValue(logic_addr), var_mem_logic_base_, ret_p);
    return ret_p;
  }
...
ge::Status VarManager::GetVarMallocSize(const ge::GeTensorDesc &var_desc, int64_t &malloc_size) {
  ...
  // align 1024 BYTE
  GE_CHK_STATUS_RET(
      CheckUint64AddOverflow(static_cast<uint64_t>(malloc_size), kSessionMemAlignSize * kSessionMemAlignUnit),
      "Add var size:%lu uint64 overflow.", static_cast<uint64_t>(malloc_size));
  malloc_size = malloc_size + static_cast<int64_t>(kSessionMemAlignSize) * static_cast<int64_t>(kSessionMemAlignUnit);
  return SUCCESS;
}

uint8_t *VarManager::GetAutoMallocVarAddr(const std::string &graph_name, const uint8_t *const logic_addr,
                                          const rtMemType_t memory_type, const uint32_t device_id) {
...
void VarManager::LogGraphVarMemInfo() const {
  for (const auto &it : graph_var_mem_statistic_) {
    ...
    GEEVENT("model_metrics:name=%s, alloc_var_mem=%lu B, shared_var_mem=%lu B", graph_name.c_str(), mem_info.alloc_size,
            mem_info.shared_size);
  }
}
...
Status VarManager::SetAllMemoryMaxValue(const std::map<std::string, std::string> &options) {
  ...
  if (use_max_mem_size_ > kMaxMemorySize) {
    REPORT_INNER_ERR_MSG("E19999",
                         "all mem_use size:%" PRIu64 " can not exceed limit:%" PRIu64
                         ", "
                         "session_id:%" PRIu64 ", check invalid",
                         use_max_mem_size_, kMaxMemorySize, session_id_);
    GELOGE(ge::GE_GRAPH_OPTIONS_INVALID,
           "[Check][Param] kUseMaxMemorySize:%zu cannot exceed "
           "max memory size:%zu, session_id:%" PRIu64 ".",
           use_max_mem_size_, kMaxMemorySize, session_id_);
...
Status VarManager::SetMemoryMallocSize(const std::map<std::string, std::string> &options, const size_t total_mem_size) {
  ...
  graph_mem_max_size_ =
      static_cast<size_t>(floor(static_cast<float64_t>(total_mem_size) * kGraphMemoryManagerMallocRatio));
