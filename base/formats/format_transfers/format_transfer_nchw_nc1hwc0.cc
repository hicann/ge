...
Status CheckArgsForNchwToNc1hwc0(const TransArgs &args) {
  ...
  if (expect_5d_shape != args.dst_shape) {
    ...
    REPORT_INNER_ERR_MSG("E19999", "Failed to trans formats, the src and dst shape are not "
...
Status GetDstDataAfterTransOfNchw2Nc1hwc0(const TransArgs &args, TransResult &result, const int32_t size,
  ...
  if (c0 <= 0) {
    ...
    REPORT_INNER_ERR_MSG("E19999", "Check shape failed, the c0 is invalid %" PRId64 ", data_type %s",
                      c0, TypeUtils::DataTypeToSerialString(args.src_data_type).c_str());
    return ACL_ERROR_GE_DATATYPE_INVALID;
  }
            ...
            if (cIdx < c) {
              ...
              if (ret != EOK) {
		       ...
		       c0_idx, dst_offset, ret);
                REPORT_INNER_ERR_MSG("E19999", "Failed to copy data from NCHW[%" PRId64 "] offset %" PRId64 " "
