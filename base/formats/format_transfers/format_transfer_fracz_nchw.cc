...
namespace formats {
namespace {
bool CheckDataTypeSupportedForFracZToNchw(const DataType data_type) { return GetSizeByDataType(data_type) > 0; }
...
Status CheckArgsForFracZToNchw(const TransArgs &args) {
  ...
  if (!CheckDataTypeSupportedForFracZToNchw(args.src_data_type)) {
    ...
    REPORT_INNER_ERR_MSG("E19999", "Failed to trans shape from FORMAT_FRACTAL_Z to NCHW, "
                       "invalid data type %s",
                       TypeUtils::DataTypeToSerialString(args.src_data_type).c_str());
  ...
  if ((src_shape.at(kFracZHWC1) != (dst_shape.at(kNchwH) * dst_shape.at(kNchwW) * c1)) ||
    ...
    REPORT_INNER_ERR_MSG("E19999", "Failed to check relationship between src and dst shape, "
...
Status GetDstDataAfterTransForFracZToNchw(const TransArgs &args, TransResult &result,
      ...
      for (int64_t h_idx = 0; h_idx < h; h_idx++) {
        ...
        for (int64_t w_idx = 0; w_idx < w; w_idx++) {
          ...
          if (ret != EOK) {
            ...
            REPORT_INNER_ERR_MSG("E19999", "Failed to copy data from FracZ offset %" PRId64 " to "
