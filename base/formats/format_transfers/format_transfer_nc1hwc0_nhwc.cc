...
namespace formats {
namespace {
bool CheckDataTypeForNc1hwc0ToNhwc(const DataType &data_type) { return GetSizeByDataType(data_type) > 0; }
...
Status CheckArgsForNc1hwc0ToNhwc(const TransArgs &args) {
  ...
  if ((src_shape.at(kNc1hwc0H) != dst_shape.at(kNhwcH)) || (src_shape.at(kNc1hwc0W) != dst_shape.at(kNhwcW)) ||
    ...
    REPORT_INNER_ERR_MSG("E19999", "Failed to check relationship between src shape %s "
                       "and dst shape %s",
                       ShapeToString(src_shape).c_str(), ShapeToString(dst_shape).c_str());
    return ACL_ERROR_GE_SHAPE_INVALID;
  }
...
Status GetDstDataForNc1hwc0ToNhwc(const TransArgs &args, TransResult &result,
  ...
  if (dst == nullptr) {
    GELOGE(ACL_ERROR_GE_MEMORY_ALLOCATION, "[Allocate][DSTMemory]Failed, memory for dst buf %" PRId64 ", "
      ...
      for (int64_t w_idx = 0; w_idx < w; w_idx++) {
        ...
        for (int64_t c_idx = 0; c_idx < c; c_idx++) {
          ...
          if (ret != EOK) {
		   ...
		   "offset %" PRId64 ", err-code %d", n_idx, c1_idx, h_idx, w_idx, c0_idx, src_offset, n_idx,
		   c_idx, h_idx, w_idx, dst_offset, ret);
            REPORT_INNER_ERR_MSG("E19999", "Failed to copy data from NC1HWC0[%" PRId64 ", %" PRId64 ", %" PRId64 ", "
			      "%" PRId64 ", %" PRId64 "] offset %" PRId64 " to NHWC[%" PRId64 ", %" PRId64 ", "
			      "%" PRId64 ", %" PRId64 "] offset %" PRId64 ", err-code %d", n_idx, c1_idx, h_idx,
			      w_idx, c0_idx, src_offset, n_idx, c_idx, h_idx, w_idx, dst_offset, ret);
...
Status FormatTransferNc1hwc0Nhwc::TransFormat(const TransArgs &args, TransResult &result) {
  ...
  if (ret != SUCCESS) {
    ...
    REPORT_INNER_ERR_MSG("E19999", "[Get][Data]Failed, after trans, src shape %s, "
