...
- **计划支持**：未来版本将支持只 `source setenv.bash` 后直接 `find_package`（无需手动添加路径）
...
- **Python 包**: setuptools, wheel（仅 `add_es_library_and_whl` 需要）,推荐使用setuptools==59.6.0、
wheel==0.37.1的配套版本或更高的配套版本
...
1. 确认已安装 CANN run 包
2. 执行 `source ${ASCEND_HOME_PATH}/bin/setenv.bash`或者设置了正确的CMAKE_MODULE_PATH
