...
AlignedPtr::AlignedPtr(const size_t buffer_size, const size_t alignment) {
  ...
  if (alignment == 0U) {
    ...
    aligned_addr_ = llm::PtrToPtr<void, uint8_t>(
        llm::ValueToPtr((llm::PtrToValue(llm::PtrToPtr<uint8_t, void>(base_.get())) + offset) & ~offset));
  }
}
...
std::unique_ptr<uint8_t[], AlignedPtr::Deleter> AlignedPtr::Reset(uint8_t *const data,
  ...
  if ((data == nullptr) || (delete_func == nullptr)) {
    ...
    LLMLOGE(ge::FAILED, "[Check][Param] data/delete_func is null");
    return nullptr;
  }
  auto ptr = Reset();
  base_.reset(data);
...
std::shared_ptr<AlignedPtr> AlignedPtr::BuildFromAllocFunc(const AlignedPtr::Allocator &alloc_func,
  ...
  if ((alloc_func == nullptr) || (delete_func == nullptr)) {
    ...
    LLMLOGE(ge::FAILED, "[Check][Param] alloc_func/delete_func is null");
    return nullptr;
  }
  const auto aligned_ptr = llm::MakeShared<AlignedPtr>();
  if (aligned_ptr == nullptr) {
    REPORT_INNER_ERR_MSG("E18888", "create AlignedPtr failed.");
    LLMLOGE(ge::FAILED, "[Create][AlignedPtr] make shared for AlignedPtr failed");
    return nullptr;
  }
  ...
  if (aligned_ptr->base_ == nullptr) {
    ...
    LLMLOGE(ge::FAILED, "[Call][AllocFunc] allocate for AlignedPtr failed");
    return nullptr;
  }
