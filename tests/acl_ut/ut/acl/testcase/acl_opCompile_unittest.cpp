...
TEST_F(UTEST_ACL_OpCompiler, InitLocalCompilerTest) {
  ...
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), Finalize()).WillRepeatedly(Return((ge::PARAM_INVALID)));

  EXPECT_CALL(MockFunctionTest::aclStubInstance(), GEFinalize()).WillRepeatedly(Return((ge::PARAM_INVALID)));
}
...
TEST_F(UTEST_ACL_OpCompiler, SetCompileStrategyTest) {
  ...
  CompileStrategy strategy = static_cast<CompileStrategy>(3);
...
TEST_F(UTEST_ACL_OpCompiler, aclGetCompileoptTestDisable) {
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), aclrtGetSocName()).WillRepeatedly(Return("Ascend910B1"));
...
TEST_F(UTEST_ACL_OpCompiler, TestSetOptionFail) {
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), aclrtGetSocName()).WillOnce(Return(nullptr));
...
TEST_F(UTEST_ACL_OpCompiler, ReadOpModelFromFileTest) {
  ...
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), ReadBytesFromBinaryFile(_, _, _)).WillOnce(Return((false)));
  EXPECT_NE(ReadOpModelFromFile(path, opModel), ACL_SUCCESS);

  EXPECT_CALL(MockFunctionTest::aclStubInstance(), ReadBytesFromBinaryFile(_, _, _)).WillOnce(Return((true)));
...
TEST_F(UTEST_ACL_OpCompiler, TestOnlineCompile02) {
  ...
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), Initialize(_, _)).WillOnce(Return(ge::PARAM_INVALID));
  EXPECT_NE(compiler.OnlineCompileAndDump(param, modelData, modelSize, nullptr, nullptr), ACL_SUCCESS);

  EXPECT_CALL(MockFunctionTest::aclStubInstance(), Initialize(_, _)).WillOnce(Return(ge::SUCCESS));
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), BuildSingleOpModel(_, _, _, _, _, _)).WillOnce(Return(ge::SUCCESS));
...
TEST_F(UTEST_ACL_OpCompiler, aclopCompile_Check_JitCompileDefaultValue) {
  ...
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), Initialize(_, _)).WillRepeatedly(Return(ge::SUCCESS));
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), BuildSingleOpModel(_, _, _, _, _, _))
      .WillRepeatedly(Return(ge::SUCCESS));
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), Ge_Generator_Finalize()).WillRepeatedly(Return(ge::SUCCESS));

  aclError ret =
      aclopCompile(opType, numInputs, inputDesc, numOutputs, outputDesc, &attr, engineType, compileFlag, opPath);
  ...
  opPath = nullptr;
  ret = aclopCompile(opType, numInputs, inputDesc, numOutputs, outputDesc, &attr, engineType, compileFlag, opPath);
  ASSERT_NE(options.find("ge.jit_compile"), options.cend());
  EXPECT_STREQ(options["ge.jit_compile"].c_str(), "0");
  EXPECT_EQ(GetGlobalCompileFlag(), 0);
  ASSERT_NE(options.find("ge.shape_generalized"), options.cend());
...
TEST_F(UTEST_ACL_OpCompiler, TestInitLocalCompiler02) {
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), GEInitialize(_)).WillOnce(Return(ge::SUCCESS));
...
TEST_F(UTEST_ACL_OpCompiler, aclopCompileAndExecuteTest) {
  ...
  aclError ret = aclopCompileAndExecute(opType, numInputs, inputDesc, inputs, numOutputs, outputDesc, outputs, &attr,
                                        engineType, compileFlag, opPath, stream);
  EXPECT_EQ(ret, ACL_ERROR_API_NOT_SUPPORT);

  compileFlag = ACL_COMPILE_UNREGISTERED;
  ret = aclopCompileAndExecute(opType, numInputs, inputDesc, inputs, numOutputs, outputDesc, outputs, &attr, engineType,
                               compileFlag, opPath, stream);
  EXPECT_EQ(ret, ACL_ERROR_INVALID_PARAM);
...
TEST_F(UTEST_ACL_OpCompiler, aclopCompileAndExecuteCheckInput) {
  ...
  aclError ret = aclopCompileAndExecute(opType, numInputs, nullptr, inputs, numOutputs, outputDesc, outputs, &attr,
                                        engineType, compileFlag, opPath, stream);
  EXPECT_EQ(ret, ACL_ERROR_INVALID_PARAM);

  ret = aclopCompileAndExecute(opType, numInputs, inputDesc, nullptr, numOutputs, outputDesc, outputs, &attr,
                               engineType, compileFlag, opPath, stream);
  EXPECT_EQ(ret, ACL_ERROR_INVALID_PARAM);
  aclDestroyTensorDesc(inputDesc[0]);
  aclDestroyTensorDesc(inputDesc[1]);
...
TEST_F(UTEST_ACL_OpCompiler, aclopCompileAndExecuteV2Test) {
  ...
  aclError ret = aclopCompileAndExecuteV2(opType, numInputs, inputDesc, inputs, numOutputs, outputDesc, outputs, &attr,
                                          engineType, compileFlag, opPath, stream);
  EXPECT_EQ(ret, ACL_ERROR_API_NOT_SUPPORT);

  compileFlag = ACL_COMPILE_UNREGISTERED;
  ret = aclopCompileAndExecuteV2(opType, numInputs, inputDesc, inputs, numOutputs, outputDesc, outputs, &attr,
                                 engineType, compileFlag, opPath, stream);
  EXPECT_EQ(ret, ACL_ERROR_INVALID_PARAM);
...
TEST_F(UTEST_ACL_OpCompiler, aclopCompileAndExecuteV2CheckInput) {
  ...
  aclError ret = aclopCompileAndExecuteV2(opType, numInputs, nullptr, inputs, numOutputs, outputDesc, outputs, &attr,
                                          engineType, compileFlag, opPath, stream);
  EXPECT_EQ(ret, ACL_ERROR_INVALID_PARAM);

  ret = aclopCompileAndExecuteV2(opType, numInputs, inputDesc, nullptr, numOutputs, outputDesc, outputs, &attr,
                                 engineType, compileFlag, opPath, stream);
  EXPECT_EQ(ret, ACL_ERROR_INVALID_PARAM);
  aclDestroyTensorDesc(inputDesc[0]);
  aclDestroyTensorDesc(inputDesc[1]);
...
TEST_F(UTEST_ACL_OpCompiler, aclGenGraphAndDumpForOpTest) {
  ...
  aclError ret = aclGenGraphAndDumpForOp("Add", numInputs, inputDesc, inputs, numOutputs, outputDesc, outputs, &attr,
                                         engineType, "./", opt);
  EXPECT_EQ(ret, ACL_ERROR_INVALID_PARAM);
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), aclrtGetCurrentContext(_)).WillRepeatedly(Return(ACL_SUCCESS));
  ret = aclGenGraphAndDumpForOp("Add", numInputs, inputDesc, inputs, numOutputs, outputDesc, outputs, &attr, engineType,
                                "./", nullptr);
