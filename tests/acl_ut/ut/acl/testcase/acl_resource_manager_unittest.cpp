...
aclError ListFilesMock(const std::string &dirName, FileNameFilterFn filter, std::vector<std::string> &names,
                       int maxDepth) {
  (void)dirName;
  (void)filter;
  (void)maxDepth;
...
TEST(UTEST_ACL_Resource_Manager, GetOpModelSucc) {
  ...
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), GetListTensor(_, _, _)).WillRepeatedly(Return(true));
...
TEST(UTEST_ACL_Resource_Manager, LoadModelFromSharedMemTest) {
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), Init(_, _)).WillRepeatedly(Return(0));

  EXPECT_CALL(MockFunctionTest::aclStubInstance(), GetModelPartition(_, _)).WillRepeatedly(Return(0));

  EXPECT_CALL(MockFunctionTest::aclStubInstance(), Load(_, _, _)).WillRepeatedly(Return(0));

  EXPECT_CALL(MockFunctionTest::aclStubInstance(), GetListTensor(_, _, _)).WillRepeatedly(Return(true));

  EXPECT_CALL(MockFunctionTest::aclStubInstance(), GetInt(_, _, _)).WillRepeatedly(Invoke(GetInt_invoke));

  EXPECT_CALL(MockFunctionTest::aclStubInstance(), HasAttr(_, _)).WillRepeatedly(Return(true));
...
TEST(UTEST_ACL_Resource_Manager, LoadModelFromSharedMemDynamicModelTest) {
  ...
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), GetInt(_, _, _)).WillRepeatedly(Return(false));

  EXPECT_CALL(MockFunctionTest::aclStubInstance(), HasAttr(_, _)).WillRepeatedly(Return(true));
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), GetBool(_, _, _)).WillRepeatedly(Invoke(GetBool_true_invoke));
  size_t modelSize = sizeof(struct ge::ModelFileHeader) + 128UL;
  auto &instance = AclOpResourceManager::GetInstance();
  ge::ModelFileHeader header;
  header.length = 128;
...
TEST(UTEST_ACL_Resource_Manager, TestStream) {
  ...
  EXPECT_CALL(MockFunctionTest::aclStubInstance(), Create(_)).WillRepeatedly(Invoke(CreateAllocators_fail));
