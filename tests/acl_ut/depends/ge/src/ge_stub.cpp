...
Status aclStub::BuildSingleOpModel(OpDescPtr &op_desc, const std::vector<GeTensor> &inputs,
                                   const std::vector<GeTensor> &outputs, OpEngineType engine_type, int32_t compile_flag,
                                   ModelBufferData &model_buff, GraphStage graph_stage,
...
Status aclStub::LoadSingleOpV2(const std::string &modelName, const ModelData &modelData, void *stream,
...
Status aclStub::LoadDynamicSingleOpV2(const std::string &model_name, const ge::ModelData &modelData, void *stream,
                                      DynamicSingleOp **single_op, const uint64_t model_id) {
  return SUCCESS;
}

Status aclStub::ExecuteAsync(DynamicSingleOp *executor, const std::vector<GeTensorDesc> &input_desc,
                             const std::vector<DataBuffer> &inputs, std::vector<GeTensorDesc> &output_desc,
                             std::vector<DataBuffer> &outputs) {
  return SUCCESS;
}

Status aclStub::ExecuteAsync(SingleOp *executor, const std::vector<DataBuffer> &inputs,
                             std::vector<DataBuffer> &outputs) {
  return SUCCESS;
}
...
bool aclStub::GetListNamedAttrs(ge::AttrUtils::ConstAttrHolderAdapter &&obj, std::string const &name,
                                vector<AnyValue::NAMED_ATTRS> &value) {
  return true;
}
...
Status aclStub::LoadModelFromData(uint32_t &model_id, const ModelData &modelData, void *dev_ptr, size_t memsize,
...
Status aclStub::LoadModelFromDataWithArgs(uint32_t &model_id, const ModelData &model_data,
...
std::unique_ptr<gert::ModelV2Executor> aclStub::LoadExecutorFromModelDataWithRtSession(
...
std::unique_ptr<gert::ModelV2Executor> aclStub::LoadExecutorFromModelDataWithMem(const ge::ModelData &model_data,
                                                                                 ge::graphStatus &error_code,
...
std::unique_ptr<gert::StreamExecutor> aclStub::LoadStreamExecutorFromModelData(const ge::ModelData &model_data,
                                                                               ...
                                                                               const size_t weight_size,
                                                                               ge::graphStatus &error_code) {
  std::unique_ptr<gert::StreamExecutor> executor =
      std::unique_ptr<gert::StreamExecutor>(new gert::StreamExecutor(nullptr));
  error_code = ge::GRAPH_SUCCESS;
  return executor;
}

std::unique_ptr<gert::StreamExecutor> aclStub::LoadStreamExecutorFromModelData(
...
Status aclStub::SetDynamicImageSize(uint32_t model_id, void *dynamic_input_addr, uint64_t length, uint64_t image_height,
...
Status aclStub::GetAllAippInputOutputDims(uint32_t model_id, uint32_t index, std::vector<InputOutputDims> &input_dims,
                                          std::vector<InputOutputDims> &output_dims) {
  return SUCCESS;
}

Status aclStub::GetAllAippInputOutputDims(uint32_t index, std::vector<InputOutputDims> &input_dims,
                                          std::vector<InputOutputDims> &output_dims) {
  return SUCCESS;
}
...
Status aclStub::TransShape(const TensorDesc &src_desc, Format dst_format, std::vector<int64_t> &dst_shape) {
  return SUCCESS;
}
...
bool aclStub::GetListTensor(AttrUtils::ConstAttrHolderAdapter &&obj, const string &name,
...
std::unique_ptr<gert::ModelV2Executor> aclStub::LoadExecutorFromFile(const char *file_path,
                                                                     ge::graphStatus &error_code) {
...
ge::graphStatus aclStub::Execute(const gert::ModelExecuteArg &arg, gert::Tensor **inputs, size_t input_num,
                                 gert::Tensor **outputs, size_t output_num) {
  return ge::GRAPH_SUCCESS;
}
ge::graphStatus aclStub::ExecuteSync(gert::Tensor **inputs, size_t input_num, gert::Tensor **outputs,
                                     size_t output_num) {
  return ge::GRAPH_SUCCESS;
}
...
uint32_t aclStub::GetPlatformInfos(const std::string SoCVersion, fe::PlatFormInfos &platformInfo,
...
void MockFunctionTest::ResetToDefaultMock() {
  ON_CALL(*this, SetModelStreamPriority).WillByDefault([this](uint32_t model_id, uint32_t priority) {
    return this->aclStub::SetModelStreamPriority(model_id, priority);
  });
  ON_CALL(*this, GetModelStreamPriority).WillByDefault([this](uint32_t model_id, uint32_t &priority) {
...
Status GeExecutor::SetDynamicAippData(uint32_t model_id, void *dynamic_input_addr, uint64_t length,
                                      const std::vector<kAippDynamicBatchPara> &aippBatchPara,
                                      const kAippDynamicPara &aippParms) {
  return MockFunctionTest::aclStubInstance().SetDynamicAippData(model_id, dynamic_input_addr, length, aippBatchPara,
                                                                aippParms);
}
...
Status GeExecutor::SetDynamicImageSize(uint32_t model_id, void *dynamic_input_addr, uint64_t length,
  ...
  return MockFunctionTest::aclStubInstance().SetDynamicImageSize(model_id, dynamic_input_addr, length, image_height,
                                                                 image_width);
}
...
Status GeExecutor::LoadModelFromData(uint32_t &model_id, const ModelData &modelData, void *dev_ptr, size_t memsize,
                                     void *weight_ptr, size_t weightsize) {
  return MockFunctionTest::aclStubInstance().LoadModelFromData(model_id, modelData, dev_ptr, memsize, weight_ptr,
                                                               weightsize);
}
...
Status GeExecutor::ExecModel(uint32_t model_id, void *stream, const ge::RunModelData &run_input_data,
                             const std::vector<ge::GeTensorDesc> &input_desc, ge::RunModelData &run_output_data,
                             std::vector<ge::GeTensorDesc> &output_desc, bool async_mode) {
  return MockFunctionTest::aclStubInstance().ExecModel(model_id, stream, run_input_data, input_desc, run_output_data,
                                                       output_desc, async_mode);
}
...
Status GeExecutor::ExecuteAsync(DynamicSingleOp *executor, const std::vector<GeTensorDesc> &input_desc,
                                const std::vector<DataBuffer> &inputs, std::vector<GeTensorDesc> &output_desc,
                                std::vector<DataBuffer> &outputs) {
  return MockFunctionTest::aclStubInstance().ExecuteAsync(executor, input_desc, inputs, output_desc, outputs);
}

Status GeExecutor::LoadSingleOpV2(const std::string &modelName, const ModelData &modelData, void *stream,
                                  SingleOp **single_op, const uint64_t model_id) {
...
bool AttrUtils::GetListTensor(ge::AttrUtils::ConstAttrHolderAdapter &&obj, const string &name,
                              vector<ConstGeTensorPtr> &value) {
  return MockFunctionTest::aclStubInstance().GetListTensor(std::move(AttrUtils::ConstAttrHolderAdapter(obj)), name,
                                                           value);
}
...
bool AttrUtils::GetBool(ConstAttrHolderAdapter &&obj, const string &name, bool &value) {
  return MockFunctionTest::aclStubInstance().GetBool(std::move(AttrUtils::ConstAttrHolderAdapter(obj)), name, value);
}
...
bool AttrUtils::GetListNamedAttrs(ge::AttrUtils::ConstAttrHolderAdapter &&obj, std::string const &name,
                                  vector<AnyValue::NAMED_ATTRS> &value) {
  return MockFunctionTest::aclStubInstance().GetListNamedAttrs(std::move(AttrUtils::ConstAttrHolderAdapter(obj)), name,
                                                               value);
}

graphStatus GraphUtils::DumpGEGraphByPath(const ge::ComputeGraphPtr &graph, const std::string &file_path,
...
Status GeGenerator::BuildSingleOpModel(ge::OpDescPtr &op_desc, const std::vector<GeTensor> &inputs,
  ...
  return MockFunctionTest::aclStubInstance().BuildSingleOpModel(op_desc, inputs, outputs, engine_type, compile_flag,
                                                                model_buff);
}
...
Status GeGenerator::BuildSingleOpModel(OpDescPtr &op_desc, const std::vector<GeTensor> &inputs,
                                       ...
                                       ComputeGraphPtr &compute_graph) {
  return MockFunctionTest::aclStubInstance().BuildSingleOpModel(op_desc, inputs, outputs, engine_type, compile_flag,
                                                                model_buff, graph_stage, compute_graph);
}
...
bool AttrUtils::GetInt(ge::AttrUtils::ConstAttrHolderAdapter &&obj, const std::string &name, int32_t &value) {
  return MockFunctionTest::aclStubInstance().GetInt(std::move(AttrUtils::ConstAttrHolderAdapter(obj)), name, value);
}
...
bool AttrUtils::GetListInt(ge::AttrUtils::ConstAttrHolderAdapter &&obj, std::string const &,
...
std::unique_ptr<gert::ModelV2Executor> LoadExecutorFromModelDataWithMem(const ge::ModelData &model_data,
                                                                        ge::graphStatus &error_code,
                                                                        const void *weight_ptr,
                                                                        const size_t weight_size) {
  return MockFunctionTest::aclStubInstance().LoadExecutorFromModelDataWithMem(model_data, error_code, weight_ptr,
                                                                              weight_size);
}
...
ge::Allocator *gert::Allocators::GetAllocator(const gert::TensorPlacement &placement, const size_t &usage) {
  return nullptr;
}
...
ge::graphStatus ModelV2Executor::Execute(const gert::ModelExecuteArg &arg, Tensor **inputs, size_t input_num,
...
std::unique_ptr<gert::StreamExecutor> LoadStreamExecutorFromModelData(const ge::ModelData &model_data,
                                                                      ...
                                                                      ge::graphStatus &error_code) {
  return MockFunctionTest::aclStubInstance().LoadStreamExecutorFromModelData(model_data, weight_ptr, weight_size,
                                                                             error_code);
}
std::unique_ptr<gert::StreamExecutor> LoadStreamExecutorFromModelData(const ge::ModelData &model_data,
...
ge::Status GlobalProfilingWrapper::ProfileStepTrace(const uint64_t index_id, const uint32_t model_id,
...
ge::graphStatus Om2ModelExecutor::Run(std::vector<gert::Tensor *> &inputs, std::vector<gert::Tensor *> &outputs) const {
  (void)inputs;
  (void)outputs;
  return ge::GRAPH_SUCCESS;
}
...
uint32_t PlatformInfoManager::GetRuntimePlatformInfosByDevice(const uint32_t &device_id, PlatFormInfos &platform_infos,
                                                              bool need_deep_copy) {
  (void)need_deep_copy;
...
uint32_t fe::PlatformInfoManager::GetPlatformInfos(const std::string SoCVersion, fe::PlatFormInfos &platformInfo,
                                                   fe::OptionalInfos &optionalInfo) {
