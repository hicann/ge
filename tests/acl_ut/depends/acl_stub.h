...
class aclStub {
 public:
  ...
  virtual ge::Status BuildSingleOpModel(OpDescPtr &op_desc, const std::vector<GeTensor> &inputs,
                                        const std::vector<GeTensor> &outputs, OpEngineType engine_type,
                                        int32_t compile_flag, ModelBufferData &model_buff, GraphStage graph_stage,
  ...
  virtual ge::Status LoadSingleOpV2(const std::string &modelName, const ModelData &modelData, void *stream,
                                    SingleOp **single_op, const uint64_t model_id);
  virtual ge::Status LoadDynamicSingleOpV2(const std::string &model_name, const ge::ModelData &modelData, void *stream,
                                           DynamicSingleOp **single_op, const uint64_t model_id);
  virtual ge::Status ExecuteAsync(DynamicSingleOp *executor, const std::vector<GeTensorDesc> &input_desc,
                                  const std::vector<DataBuffer> &inputs, std::vector<GeTensorDesc> &output_desc,
                                  std::vector<DataBuffer> &outputs);
  virtual ge::Status ExecuteAsync(SingleOp *executor, const std::vector<DataBuffer> &inputs,
                                  std::vector<DataBuffer> &outputs);
  ...
  virtual bool GetListNamedAttrs(AttrUtils::ConstAttrHolderAdapter &&obj, std::string const &name,
  ...
  virtual ge::Status LoadModelFromData(uint32_t &model_id, const ModelData &modelData, void *dev_ptr, size_t memsize,
                                       void *weight_ptr, size_t weightsize);
  virtual ge::Status LoadModelFromDataWithArgs(uint32_t &model_id, const ModelData &model_data,
  ...
  virtual ge::Status GetMemAndWeightSize(const void *model_data, size_t model_size, size_t &mem_size,
                                         size_t &weight_size);
  ...
  virtual ge::Status SetDynamicBatchSize(uint32_t model_id, void *dynamic_input_addr, uint64_t length,
                                         uint64_t batch_size);
  virtual ge::Status SetDynamicImageSize(uint32_t model_id, void *dynamic_input_addr, uint64_t length,
  ...
  virtual ge::Status GetOpAttr(uint32_t model_id, const std::string &op_name, const std::string &attr_name,
  ...
  virtual ge::Status GetAllAippInputOutputDims(uint32_t index, std::vector<InputOutputDims> &input_dims,
                                               std::vector<InputOutputDims> &output_dims);
  ...
  virtual ge::Status TransShape(const TensorDesc &src_desc, Format dst_format, std::vector<int64_t> &dst_shape);
  ...
  virtual bool GetListTensor(AttrUtils::ConstAttrHolderAdapter &&obj, const string &name,
  ...
  virtual std::unique_ptr<gert::ModelV2Executor> LoadExecutorFromFile(const char *file_path,
                                                                      ge::graphStatus &error_code);
  virtual std::unique_ptr<gert::ModelV2Executor> LoadExecutorFromModelData(const ge::ModelData &model_data,
                                                                           ge::graphStatus &error_code);
  virtual std::unique_ptr<gert::ModelV2Executor> LoadExecutorFromModelDataWithRtSession(
      const ge::ModelData &model_data, gert::RtSession *const rt_session, ge::graphStatus &error_code);
  virtual ge::graphStatus LoadDataFromFileV2(const char *path, ge::ModelData &model_data);
  virtual std::unique_ptr<gert::ModelV2Executor> LoadExecutorFromModelDataWithMem(const ge::ModelData &model_data,
                                                                                  ge::graphStatus &error_code,
  ...
  virtual std::unique_ptr<gert::StreamExecutor> LoadStreamExecutorFromModelData(const ge::ModelData &model_data,
                                                                                ...
                                                                                const size_t weight_size,
                                                                                ge::graphStatus &error_code);
  virtual std::unique_ptr<gert::StreamExecutor> LoadStreamExecutorFromModelData(
  ...
  virtual ge::graphStatus Execute(const gert::ModelExecuteArg &arg, gert::Tensor **inputs, size_t input_num,
                                  gert::Tensor **outputs, size_t output_num);
  virtual ge::graphStatus ExecuteSync(gert::Tensor **inputs, size_t input_num, gert::Tensor **outputs,
                                      size_t output_num);
  ...
  virtual uint32_t GetPlatformInfos(const std::string SoCVersion, fe::PlatFormInfos &platformInfo,
  ...
  virtual uint32_t UpdateRuntimePlatformInfosByDevice(const uint32_t &device_id, fe::PlatFormInfos &platform_infos);
  ...
  virtual rtError_t rtFunctionRegister(void *binHandle, const void *stubFunc, const char *stubName, const void *devFunc,
                                       uint32_t funcMode);
  ...
  virtual aclError aclrtMemcpyAsync(void *dst, size_t destMax, const void *src, size_t count, aclrtMemcpyKind kind,
                                    aclrtStream stream);
  ...
  virtual aclError aclrtLaunchCallback(aclrtCallback fn, void *userData, aclrtCallbackBlockType blockType,
                                       aclrtStream stream);
  ...
  virtual aclError aclrtAllocatorGetByStream(aclrtStream stream, aclrtAllocatorDesc *allocatorDesc,
  ...
  virtual aclError aclInitCallbackRegister(aclRegisterCallbackType type, aclInitCallbackFunc cbFunc, void *userData);
  virtual aclError aclInitCallbackUnRegister(aclRegisterCallbackType type, aclInitCallbackFunc cbFunc);
  virtual aclError aclFinalizeCallbackRegister(aclRegisterCallbackType type, aclFinalizeCallbackFunc cbFunc,
                                               void *userData);
  virtual aclError aclFinalizeCallbackUnRegister(aclRegisterCallbackType type, aclFinalizeCallbackFunc cbFunc);
...
class MockFunctionTest : public aclStub {
 public:
  ...
  MOCK_METHOD6(BuildSingleOpModel, ge::Status(ge::OpDescPtr &op_desc, const std::vector<GeTensor> &inputs,
                                              const std::vector<GeTensor> &outputs, OpEngineType engine_type,
                                              int32_t compile_flag, ModelBufferData &model_buff));
  MOCK_METHOD8(BuildSingleOpModel,
               ge::Status(OpDescPtr &op_desc, const std::vector<GeTensor> &inputs, const std::vector<GeTensor> &outputs,
  ...
  MOCK_METHOD5(LoadDynamicSingleOpV2, ge::Status(const std::string &model_name, const ge::ModelData &modelData,
                                                 void *stream, DynamicSingleOp **single_op, const uint64_t model_id));
  MOCK_METHOD5(ExecuteAsync, ge::Status(DynamicSingleOp *executor, const std::vector<GeTensorDesc> &input_desc,
                                        const std::vector<DataBuffer> &inputs, std::vector<GeTensorDesc> &output_desc,
                                        std::vector<DataBuffer> &outputs));
  MOCK_METHOD3(ExecuteAsync,
               ge::Status(SingleOp *executor, const std::vector<DataBuffer> &inputs, std::vector<DataBuffer> &outputs));
  ...
  MOCK_METHOD3(GetListNamedAttrs, bool(ge::AttrUtils::ConstAttrHolderAdapter &&obj, std::string const &name,
                                       vector<GeAttrValue::NAMED_ATTRS> &value));
  ...
  MOCK_METHOD3(GetDynamicBatchInfo,
               ge::Status(uint32_t model_id, std::vector<std::vector<int64_t>> &batch_info, int32_t &dynamic_type));
  ...
  MOCK_METHOD6(LoadModelFromData, ge::Status(uint32_t &model_id, const ModelData &modelData, void *dev_ptr,
                                             size_t memsize, void *weight_ptr, size_t weightsize));
  MOCK_METHOD3(LoadModelFromDataWithArgs,
               ge::Status(uint32_t &model_id, const ModelData &model_data, const ModelLoadArg &load_arg));
  MOCK_METHOD2(LoadDataFromFile, ge::graphStatus(std::string const &path, ModelData &modelData));
  MOCK_METHOD4(LoadModelWithQ,
               ge::Status(uint32_t &model_id, const ge::ModelData &ge_model_data,
                          const std::vector<uint32_t> &input_queue_ids, const std::vector<uint32_t> &output_queue_ids));
  MOCK_METHOD3(LoadModelWithQ,
               ge::Status(uint32_t &model_id, const ge::ModelData &ge_model_data, const ge::ModelQueueArg &queue_arg));
  ...
  MOCK_METHOD4(GetMemAndWeightSize,
  ...
  MOCK_METHOD7(ExecModel, ge::Status(uint32_t model_id, void *stream, const ge::RunModelData &run_input_data,
                                     const std::vector<ge::GeTensorDesc> &input_desc, ge::RunModelData &run_output_data,
                                     std::vector<ge::GeTensorDesc> &output_desc, bool async_mode));
  MOCK_METHOD4(SetDynamicBatchSize,
               ge::Status(uint32_t model_id, void *dynamic_input_addr, uint64_t length, uint64_t batch_size));
  MOCK_METHOD5(SetDynamicImageSize, ge::Status(uint32_t model_id, void *dynamic_input_addr, uint64_t length,
                                               uint64_t image_height, uint64_t image_width));
  MOCK_METHOD4(SetDynamicDims, ge::Status(uint32_t model_id, void *dynamic_input_addr, uint64_t length,
                                          const vector<uint64_t> &dynamic_dims));
  MOCK_METHOD3(GetCurDynamicDims,
               ge::Status(uint32_t model_id, const vector<uint64_t> &dynamic_dims, vector<uint64_t> &cur_dynamic_dims));
  ...
  MOCK_METHOD3(GetCurShape,
               ge::Status(const uint32_t model_id, std::vector<int64_t> &batch_info, int32_t &dynamic_type));
  MOCK_METHOD2(GetModelAttr, ge::Status(uint32_t model_id, std::vector<std::string> &dynamic_output_shape_info));
  MOCK_METHOD4(GetOpAttr, ge::Status(uint32_t model_id, const std::string &op_name, const std::string &attr_name,
                                     std::string &attr_value));
  ...
  MOCK_METHOD4(GetAllAippInputOutputDims,
               ge::Status(uint32_t model_id, uint32_t index, std::vector<InputOutputDims> &input_dims,
                          std::vector<InputOutputDims> &output_dims));
  MOCK_METHOD3(GetAllAippInputOutputDims, ge::Status(uint32_t index, std::vector<InputOutputDims> &input_dims,
                                                     std::vector<InputOutputDims> &output_dims));
  MOCK_METHOD5(SetDynamicAippData,
               ge::Status(uint32_t model_id, void *dynamic_input_addr, uint64_t length,
                          const std::vector<kAippDynamicBatchPara> &aippBatchPara, const kAippDynamicPara &aippParms));
  MOCK_METHOD0(Init, int());
  MOCK_METHOD1(OpsProtoManager_Initialize, bool(const std::map<std::string, std::string> &options));
  MOCK_METHOD3(TransShape, ge::Status(const TensorDesc &src_desc, Format dst_format, std::vector<int64_t> &dst_shape));
  ...
  MOCK_METHOD2(LoadExecutorFromFile,
               std::unique_ptr<gert::ModelV2Executor>(const char *file_path, ge::graphStatus &error_code));
  MOCK_METHOD2(LoadExecutorFromModelData,
               std::unique_ptr<gert::ModelV2Executor>(const ge::ModelData &model_data, ge::graphStatus &error_code));
  MOCK_METHOD3(LoadExecutorFromModelData,
               std::unique_ptr<gert::ModelV2Executor>(const ge::ModelData &model_data,
                                                      const gert::LoadExecutorArgs &args, ge::graphStatus &error_code));
  MOCK_METHOD3(LoadExecutorFromModelDataWithRtSession,
               std::unique_ptr<gert::ModelV2Executor>(const ge::ModelData &model_data,
                                                      gert::RtSession *const rt_session, ge::graphStatus &error_code));
  MOCK_METHOD2(LoadDataFromFileV2, ge::graphStatus(const char *path, ge::ModelData &modelData));
  MOCK_METHOD4(LoadExecutorFromModelDataWithMem,
               std::unique_ptr<gert::ModelV2Executor>(const ge::ModelData &model_data, ge::graphStatus &error_code,
                                                      const void *weight_ptr, const size_t weight_size));
  MOCK_METHOD4(LoadStreamExecutorFromModelData,
               std::unique_ptr<gert::StreamExecutor>(const ge::ModelData &model_data, const void *weight_ptr,
                                                     const size_t weight_size, ge::graphStatus &error_code));
  MOCK_METHOD3(LoadStreamExecutorFromModelData,
               std::unique_ptr<gert::StreamExecutor>(const ge::ModelData &model_data,
  ...
  MOCK_METHOD5(Execute, ge::graphStatus(const gert::ModelExecuteArg &arg, gert::Tensor **inputs, size_t input_num,
                                        gert::Tensor **outputs, size_t output_num));
  MOCK_METHOD4(ExecuteSync,
               ge::graphStatus(gert::Tensor **inputs, size_t input_num, gert::Tensor **outputs, size_t output_num));
  ...
  MOCK_METHOD3(GetPlatformInfos, uint32_t(const std::string SoCVersion, fe::PlatFormInfos &platformInfo,
                                          fe::OptionalInfos &optionalInfo));
  ...
  MOCK_METHOD2(UpdateRuntimePlatformInfosByDevice,
               uint32_t(const uint32_t &device_id, fe::PlatFormInfos &platform_infos));
  ...
  MOCK_METHOD6(aclrtMemcpyAsync, aclError(void *dst, size_t destMax, const void *src, size_t count,
  ...
  MOCK_METHOD3(aclrtRegStreamStateCallback,
               aclError(const char *regName, aclrtStreamStateCallback callback, void *args));
  MOCK_METHOD3(aclrtRegDeviceStateCallback,
               aclError(const char *regName, aclrtDeviceStateCallback callback, void *args));
  MOCK_METHOD4(aclrtLaunchCallback,
               aclError(aclrtCallback fn, void *userData, aclrtCallbackBlockType blockType, aclrtStream stream));
  ...
  MOCK_METHOD7(aclrtAllocatorGetByStream,
               aclError(aclrtStream stream, aclrtAllocatorDesc *allocatorDesc, aclrtAllocator *allocator,
                        aclrtAllocatorAllocFunc *allocFunc, aclrtAllocatorFreeFunc *freeFunc,
                        aclrtAllocatorAllocAdviseFunc *allocAdviseFunc,
                        aclrtAllocatorGetAddrFromBlockFunc *getAddrFromBlockFunc));
  MOCK_METHOD3(aclInitCallbackRegister,
               aclError(aclRegisterCallbackType type, aclInitCallbackFunc cbFunc, void *userData));
  MOCK_METHOD2(aclInitCallbackUnRegister, aclError(aclRegisterCallbackType type, aclInitCallbackFunc cbFunc));
  MOCK_METHOD3(aclFinalizeCallbackRegister,
               aclError(aclRegisterCallbackType type, aclFinalizeCallbackFunc cbFunc, void *userData));
  MOCK_METHOD2(aclFinalizeCallbackUnRegister, aclError(aclRegisterCallbackType type, aclFinalizeCallbackFunc cbFunc));
