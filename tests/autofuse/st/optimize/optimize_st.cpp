...
TEST_F(OptimizerSt, TestSoftmaxGraph_OptimizeSuccess) {
  ...
  auto max_phase2_workspace_post = impl_graph_max_phase2.FindNode("b0_max_to_b1_broadcast_Workspace_0");
  ...
  auto sum_phase1_workspace1 = impl_graph_sum_phase1.FindNode("b0_max_to_b1_broadcast_Workspace_0");
  ...
  auto sum_phase2_workspace2 = impl_graph_sum_phase2.FindNode("b2_sum_to_b3_broadcast_Workspace_0");
  ...
  auto div_workspace2 = impl_graph_div.FindNode("b2_sum_to_b3_broadcast_Workspace_0");
...
TEST_F(OptimizerSt, RemovePad_not_align_broadcast) {
  ge::AscGraph graph("RemovePad_not_align_broadcast");
  auto s0 = graph.CreateSizeVar("s0");
  auto s1 = graph.CreateSizeVar("s1");
  auto s2 = graph.CreateSizeVar("s2");
...
TEST_F(OptimizerSt, BufQueAllocator_RemovePad_MemUnique) {
  ...
  *store.y.strides = {s1, One};
  ...
  EXPECT_EQ(impl_graph2_brc1->outputs[0].attr.buf.id, 0);
  EXPECT_EQ(impl_graph2_rpd->outputs[0].attr.buf.id, 1);
...
TEST_F(OptimizerSt, StoreLoadCancellation) {
  ge::AscGraph graph("store_load");
  ...
  *store_op1.y.repeats = {ge::ops::One};
  ...
  auto cg = ge::AscGraphUtils::GetComputeGraph(optimize_graph);
  auto store_node = cg->FindNode("store");
  EXPECT_EQ(store_node, nullptr);
  auto load1_node = cg->FindNode("load1");
...
TEST_F(OptimizerSt, StaticGraphRecomputeSplit) {
  ge::AscGraph graph("StaticGraphRecomputeSplit");
  auto s0 = ge::Symbol(2048);
  auto s1 = ge::Symbol(126);
...
TEST_F(OptimizerSt, NodeCacheMarkerReduce) {
  ge::AscGraph graph("NodeCacheMarkerReduce");
  auto s0 = ge::Symbol(128);
  auto s1 = ge::Symbol(2889);
  auto s2 = ge::Symbol(4);
  ...
  *add0.y.strides = {s1 * s2, s2, One};
  ...
  ge::ascir_op::Sum reduce0("reduce0");
  reduce0.x = add0.y;
  reduce0.attr.sched.axis = {z0.id, z1.id, z2.id};
  reduce0.attr.api.compute_type = ComputeType::kComputeReduce;
  reduce0.y.dtype = ge::DT_FLOAT16;
  *reduce0.y.axis = {z0.id, z1.id, z2.id};
  *reduce0.y.repeats = {s0, One, s2};
  *reduce0.y.strides = {s2, Zero, One};
  ...
  store0.y.dtype = ge::DT_FLOAT16;
  *store0.y.axis = {z0.id, z1.id, z2.id};
  *store0.y.repeats = {s0, One, s2};
  *store0.y.strides = {s2, Zero, One};
  ...
  y0.y.dtype = ge::DT_FLOAT16;
  *y0.y.axis = {z0.id, z1.id, z2.id};
  *y0.y.repeats = {s0, One, s2};
  *y0.y.strides = {s2, Zero, One};
...
TEST_F(OptimizerSt, MutmulAndAdd) {
  ge::AscGraph graph("mutmul");
...
TEST_F(OptimizerSt, MutmulAndBroadcastAdd) {
  ge::AscGraph graph("mutmul");
...
TEST_F(OptimizerSt, JustMutmulBias) {
  ge::AscGraph graph("mutmul");
...
TEST_F(OptimizerSt, JustMutmulOffset) {
  ge::AscGraph graph("mutmul");
...
TEST_F(OptimizerSt, JustMutmulBaisOffset) {
  ge::AscGraph graph("mutmul");
...
TEST_F(OptimizerSt, JustBatchMutmul) {
  ge::AscGraph graph("mutmul");
...
TEST_F(OptimizerSt, BatchMutmulAndAdd) {
  ge::AscGraph graph("mutmul");
...
TEST_F(OptimizerSt, BatchMutmulAndBroadcastAdd) {
  ge::AscGraph graph("mutmul");
...
TEST_F(OptimizerSt, JustBatchMutmulBias) {
  ge::AscGraph graph("mutmul");
...
TEST_F(OptimizerSt, JustBatchMutmulOffset) {
  ge::AscGraph graph("mutmul");
...
TEST_F(OptimizerSt, JustBatchMutmulBaisOffset) {
  ge::AscGraph graph("mutmul");
...
TEST_F(OptimizerSt, MutmulAndAbs) {
  ge::AscGraph graph("mutmul");
  auto s0 = graph.CreateSizeVar(64);
  auto s1 = graph.CreateSizeVar(64);
  auto z0 = graph.CreateAxis("z0", s0);
...
TEST_F(OptimizerSt, MatmulAddExpAddAdd) {
  ...
  *store_op.y.repeats = {s0, s1, s2};
  ...
  EXPECT_EQ(fused_scheduled_result.node_idx_to_scheduled_results[0][0].schedule_groups.size(), 2UL);
  EXPECT_EQ(fused_scheduled_result.node_idx_to_scheduled_results[0][0].schedule_groups[0].impl_graphs.size(), 1UL);
  EXPECT_EQ(fused_scheduled_result.node_idx_to_scheduled_results[0][0].schedule_groups[1].impl_graphs.size(), 2UL);
  ...
  EXPECT_EQ(fused_scheduled_result.node_idx_to_scheduled_results[0][0].schedule_groups[1].impl_graphs[1].GetName(),
            "matmul_1_ub_general_nil_nil_1_nil");
}

TEST_F(OptimizerSt, MatmulAndCastAdd) {
  ge::AscGraph graph("matmul");
...
TEST_F(OptimizerSt, MatmulAndCastMultiRefsAdd) {
  ge::AscGraph graph("matmul");
...
TEST_F(OptimizerSt, MatmulAndBrcLoadMultiRefsAdd) {
  ge::AscGraph graph("matmul");
...
TEST_F(OptimizerSt, MatmulAndCastBrcMultiRefsAdd) {
  ge::AscGraph graph("matmul");
  ...
  *store_op.y.repeats = {s0, s1, s2};
  ...
  EXPECT_EQ(optimizer.Optimize(graph, fused_scheduled_result), 0);
  EXPECT_EQ(fused_scheduled_result.node_idx_to_scheduled_results.size(), 1UL);
  EXPECT_EQ(fused_scheduled_result.node_idx_to_scheduled_results[0][0].schedule_groups.size(), 0UL);
}
