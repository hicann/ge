...
extern "C" __global__ __aicore__ void sub_abs_test(GM_ADDR x1, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling);
...
class E2E_BackendSubAbs_Code : public testing::Test, public testing::WithParamInterface<std::vector<int>> {
};

TEST_P(E2E_BackendSubAbs_Code, CalculateCorrect) {
  ...
  ICPU_RUN_KF(sub_abs_test, tiling_data.block_dim, (uint8_t *)input1, (uint8_t *)y, nullptr, (uint8_t*)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_BackendSubAbs_Code,
    ::testing::Values(std::vector<int>{32, 32, 32},  // 用例输入的维度需要与构图接口的dims_size匹配
