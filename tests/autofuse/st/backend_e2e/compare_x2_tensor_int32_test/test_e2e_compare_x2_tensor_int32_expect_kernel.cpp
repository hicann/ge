...
class E2E_BackendCompareX2TensorInt32_Code : public testing::Test, public testing::WithParamInterface<std::vector<int>> {
};

TEST_P(E2E_BackendCompareX2TensorInt32_Code, CalculateCorrect) {
  ...
  int32_t *x1 = (int32_t *)AscendC::GmAlloc(test_size * sizeof(int32_t));
  uint8_t *x2 = (uint8_t *)AscendC::GmAlloc(test_size * sizeof(uint8_t));
  int32_t *x3 = (int32_t *)AscendC::GmAlloc(test_size * sizeof(int32_t));
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_BackendCompareX2TensorInt32_Code,
    ::testing::Values(std::vector<int>{32, 7}, // 用例输入的维度需要与构图接口的dims_size匹配
                      std::vector<int>{32, 144},
                      std::vector<int>{2, 272}
                      ));
