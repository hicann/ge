...
class TestBackendAddNegE2e : public testing::Test {
};

TEST_F(TestBackendAddNegE2e, AddNegE2eCodegen) {
...
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
  ...
  // shape_info 和 AddNegFusedGraph入参dims_size匹配（个数相同，命名规则为s开头、编号从0开始）
  std::map<std::string, std::string> shape_info(
        { {"s0", "stub_s0"}, {"s1", "stub_s1"}, {"s2", "stub_s2"} }
    );
  auto graph = ascir::ShareGraph::AddNegFusedGraph(3);
