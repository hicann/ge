...
namespace ge{
namespace ascir{
extern std::vector<std::unique_ptr<ge::TmpBufDesc>> CalcPowTmpSize(const ge::AscNode &node);
const Expression MAX_TMP_BUFFER_SIZE = ge::Symbol(255 * 256 + 32);

using namespace testing;

class CalcPowTmpSizeTest:public::testing::Test{
protected:
    void SetUp() override{}
 ...
 */
TEST_F(CalcPowTmpSizeTest, CalcPowTmpSize_ShouldReturnCorrectSize_WhenNodelsValid)
{
    ...
    *y.y.strides = {s1, Symbol(1)};
    ...
    std::vector<std::unique_ptr<ge::TmpBufDesc>> result = CalcPowTmpSize(*node);
    ASSERT_EQ(result.size(), 1);
    ASSERT_EQ(result[0]->size, sym::Min(ge::Symbol(4) * s1 * s0, MAX_TMP_BUFFER_SIZE));
    ASSERT_EQ(result[0]->life_time_axis_id, -1);
}

TEST_F(CalcPowTmpSizeTest, CalcPowTmpSize_WhenInputsAreAllScalarOrUbScalar)
{
    ge::AscGraph graph("test");
    auto s0 = graph.CreateSizeVar("s0");
    auto s1 = graph.CreateSizeVar("s1");
