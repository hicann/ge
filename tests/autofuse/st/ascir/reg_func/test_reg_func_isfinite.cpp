...
namespace ge{
namespace ascir{
extern std::vector<std::unique_ptr<ge::TmpBufDesc>> CalcIsFiniteTmpSize(const ge::AscNode &node);
...
TEST_F(CalcIsFiniteTmpSizeTest, CalcIsFiniteTmpSize_ShouldReturnCorrectSize_WhenNodelsValid)
{
    ...
    *y.y.strides = {s1, Symbol(1)};
    ...
    std::shared_ptr<ge::AscNode> node = graph.FindNode("isFinite");
    node->inputs[0].attr.vectorized_strides = {s1, Symbol(1)};
    std::vector<std::unique_ptr<ge::TmpBufDesc>> result = CalcIsFiniteTmpSize(*node);
