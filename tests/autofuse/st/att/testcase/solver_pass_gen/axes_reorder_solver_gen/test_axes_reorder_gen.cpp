...
TEST_F(TestAxesReorderSolverGen, GenPgoSolverwithClassImpl) {
    AxesReorderSolverGen solver("case_test", "TilingData");
    std::string className = "AxesReorderSolverGen";
    std::string genRes = solver.GenPgoSolverClassImpl(className);
    EXPECT_TRUE(genRes.find(className) != std::string::npos);
}