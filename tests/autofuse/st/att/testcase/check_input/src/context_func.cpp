...
#include <iostream>
#include "stub/tiling/platform_ascendc.h"
...
class TilingData {
  public:
    ...
    void set_workspaceSize(uint32_t workspace) {workspaceSize = workspace;}
...
bool CheckPlatformInfo(gert::TilingContext *context) {
  ...
  if (platformInfoPtr == nullptr) {
    OP_LOG("platformInfoPtr is nullptr!");
    return false;
  }
  ...
  if ((aivNum == 0) || (aicNum == 0) || (hbm_size == 0) || (ub_size == 0)) {
    OP_LOG("platform info get failed!");
    return false;
  }
  return true;
}

bool PreTiling(TilingData &tiling_data, gert::TilingContext *context) {
  if (!CheckPlatformInfo(context)) {
    OP_LOG("Check platforminfo failed!");
    return false;
  }
  auto platformInfoPtr = context->GetPlatformInfo();
  auto ascendcPlatform = platform_ascendc::PlatformAscendC(platformInfoPtr);
...
bool TilingVarsNumCheck0(gert::TilingContext *context) {
  if (context->GetComputeNodeInfo()->GetInputsNum() != 2) {
    OP_LOG("context->GetComputeNodeInfo()->GetInputsNum() != 2, invalid input num!");
    return false;
  }
  return true;
}
...
bool TilingVarsDtypeCheck0(gert::TilingContext *context) {
  if (static_cast<uint32_t>(context->GetInputTensor(0)->GetDataType()) != 1) {
    OP_LOG("static_cast<uint32_t>(context->GetInputTensor(0)->GetDataType()) != 1, invalid input dtype!");
    return false;
  }
  if (static_cast<uint32_t>(context->GetInputTensor(1)->GetDataType()) != 1) {
    OP_LOG("static_cast<uint32_t>(context->GetInputTensor(1)->GetDataType()) != 1, invalid input dtype!");
    return false;
  }
  return true;
}
...
bool TilingVarsFormatCheck0(gert::TilingContext *context) {
  if (static_cast<uint32_t>(context->GetInputTensor(0)->GetStorageFormat()) != 2) {
    OP_LOG("static_cast<uint32_t>(context->GetInputTensor(1)->GetStorageFormat()) != 2, invalid input format!");
    return false;
  }
  if (static_cast<uint32_t>(context->GetInputTensor(1)->GetStorageFormat()) != 2) {
    OP_LOG("static_cast<uint32_t>(context->GetInputTensor(2)->GetStorageFormat()) != 2, invalid input format!");
    return false;
  }
  return true;
}
...
bool TilingInputVarsCheck0(gert::TilingContext *context) {
  if (!TilingVarsNumCheck0(context)) {
    return false;
  }
  if (!TilingVarsDtypeCheck0(context)) {
    return false;
  }
  if (!TilingVarsFormatCheck0(context)) {
    return false;
  }
  if (!TilingVarsShapeDimCheck0(context)) {
    return false;
  }
  if (!TilingVarsShapeCheck0(context)) {
    return false;
  }
  return true;
}
...
bool TilingVarsValidCheck0(TilingData &tiling_data) {
  if ((tiling_data.get_B() < 1) || (tiling_data.get_B() > 100000)) {
    OP_LOG("(tiling_data.get_B() < 1) || (tiling_data.get_B() > 100000), invalid input var!");
    return false;
  }
  if ((tiling_data.get_D() < 1) || (tiling_data.get_D() > 100000)) {
    OP_LOG("(tiling_data.get_D() < 1) || (tiling_data.get_D() > 100000), invalid input var!");
    return false;
  }
  if ((tiling_data.get_G() < 1) || (tiling_data.get_G() > 100000)) {
    OP_LOG("(tiling_data.get_G() < 1) || (tiling_data.get_G() > 100000), invalid input var!");
    return false;
  }
  if ((tiling_data.get_N() < 1) || (tiling_data.get_N() > 100000)) {
    OP_LOG("(tiling_data.get_N() < 1) || (tiling_data.get_N() > 100000), invalid input var!");
    return false;
  }
  if ((tiling_data.get_S1() < 1) || (tiling_data.get_S1() > 100000)) {
    OP_LOG("(tiling_data.get_S1() < 1) || (tiling_data.get_S1() > 100000), invalid input var!");
    return false;
  }
  if ((tiling_data.get_S2() < 1) || (tiling_data.get_S2() > 100000)) {
    OP_LOG("(tiling_data.get_S2() < 1) || (tiling_data.get_S2() > 100000), invalid input var!");
    return false;
  }
  return true;
}

bool CheckandSetInput(TilingData &tiling_data, gert::TilingContext *context, int32_t tilingCaseId) {
  switch (tilingCaseId) {
    case 0u:
      if (TilingInputVarsCheck0(context) != true) {
        return false;
      }
      if (TilingAttrCheck0(context) != true) {
        return false;
      }
      SetAxisSize0(tiling_data, context);
...
bool GetWorkSpaceSize(TilingData& tiling_data)
{
  double B = static_cast<double>(tiling_data.get_B());
  double N = static_cast<double>(tiling_data.get_N());
  double G = static_cast<double>(tiling_data.get_G());
  tiling_data.set_workspaceSize(static_cast<uint32_t>(B * N * G * 16));
  return true;
}

bool PostTiling(TilingData &tiling_data, gert::TilingContext *context) {
