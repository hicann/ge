...
void LoadDivStore_BeforeAutofuse(ge::AscGraph &graph) {
...
void LoadDivStore_AfterInferOutput(ge::AscGraph &graph) {
    ...
    div->outputs[0].attr.dtype =(ge::DataType)load1->outputs[0].attr.dtype;
    ...
    div->attr.tmp_buffers = {{{ge::Symbol(8192), -1}, MemAttr()}};

    auto store = graph.FindNode("store");
    store->outputs[0].attr.dtype = (ge::DataType)div->outputs[0].attr.dtype;
...
void LoadDivStore_AfterGetApiInfo(ge::AscGraph &graph) {
    auto x1 = graph.FindNode("x1");
...
void LoadDivStore_AfterScheduler(ge::AscGraph &graph) {
    ...
    div->attr.sched.loop_axis = z1T->id;
...
void LoadDivStore_AfterQueBufAlloc(ge::AscGraph &graph) {
    auto x1 = graph.FindNode("x1");
    ...
    div->outputs[0].attr.buf.id = ge::kIdNone;
    div->outputs[0].attr.que.id = 2;
    div->outputs[0].attr.mem.reuse_id = 2;
