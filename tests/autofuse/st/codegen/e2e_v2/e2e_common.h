...
#define Rational(a, b) (static_cast<double>(a) / static_cast<double>(b))
  ...
  ss << "}" << std::endl;
	...
	ss << PGOSearchFuncInputOutputCallBackDefStub(fused_schedule_result);
	ss << "void* stream, uint32_t workspaceSize, double& out_best_perf"
	   << ") {return true;} " << std::endl;
	ss << "bool PGOByCoreNumSearchTilingKey(std::vector<AutofuseTilingData>& tiling_data_list, AutofuseTilingData* tiling_data, uint32_t max_block_dim=48) {return true;}" << std::endl;
	ss << "}" << std::endl;
