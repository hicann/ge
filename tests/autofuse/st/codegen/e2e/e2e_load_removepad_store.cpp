...
void LoadRemovePadStore_BeforeAutofuse(ge::AscGraph &graph, ge::DataType in_data_type) {
  ...
  *load.y.strides = {s1*s2, s2, One};
  ...
  ge::ascir_op::RemovePad removepad("removepad");
  graph.AddNode(removepad);
  removepad.x = load.y;
  removepad.y.dtype = in_data_type;
  removepad.attr.sched.axis = {z0.id, z1.id, z2.id};
  *removepad.y.repeats = {s0, s1, s2};
  *removepad.y.strides = {s1*s2, s2, One};
  removepad.attr.tmp_buffers = {{{ge::Symbol(8192), -1}, MemAttr()}};

  Store store("store");
  graph.AddNode(store);
  store.x = removepad.y;
  store.attr.sched.axis = {z0.id, z1.id, z2.id};
  store.y.dtype = in_data_type;
  ...
  *store.y.strides = {s1*s2, s2, One};
  ...
  y.y.dtype = in_data_type;
  *y.y.axis = {z0.id, z1.id, z2.id};
}

void LoadRemovePadStore_AfterInferOutput(ge::AscGraph &graph, ge::DataType in_data_type) {
    ...
    auto removepad = graph.FindNode("removepad");
    removepad->outputs[0].attr.dtype = in_data_type;
    removepad->outputs[0].attr.axis = load->outputs[0].attr.axis;
    removepad->outputs[0].attr.repeats = load->outputs[0].attr.repeats;
    removepad->outputs[0].attr.strides = load->outputs[0].attr.strides;
    removepad->attr.api.compute_type = ComputeType::kComputeElewise;

    auto store = graph.FindNode("store");
    store->outputs[0].attr.dtype =  in_data_type;
...
void LoadRemovePadStore_AfterGetApiInfo(ge::AscGraph &graph) {
    auto x = graph.FindNode("x");
    ...
    auto removepad = graph.FindNode("removepad");
    removepad->attr.api.type = ApiType::kAPITypeCompute;
    removepad->attr.api.unit = ComputeUnit::kUnitVector;
...
void LoadRemovePadStore_AfterScheduler(ge::AscGraph &graph) {
    ...
    // ApplySplit on load, removepad, store
    auto load = graph.FindNode("load");
    ...
    auto removepad = graph.FindNode("removepad");
    graph.ApplySplit(removepad, z1T->id, z1t->id);
    graph.ApplySchedAxisMerge(removepad, block_axis->id, m_axis->from);
    graph.ApplySplit(removepad, z0B->id, z0b->id);
    removepad->attr.sched.loop_axis = z0b->id;
    removepad->outputs[0].attr.vectorized_axis = vectorized_axis;
    removepad->outputs[0].attr.vectorized_strides = vectorized_strides;
...
void LoadRemovePadStore_AfterQueBufAlloc(ge::AscGraph &graph) {
    auto x = graph.FindNode("x");
    ...
    auto removepad = graph.FindNode("removepad");
    removepad->outputs[0].attr.mem.tensor_id = 2;
    removepad->outputs[0].attr.mem.alloc_type = AllocType::kAllocTypeQueue;
    removepad->outputs[0].attr.mem.hardware = MemHardware::kMemHardwareUB;
    removepad->outputs[0].attr.mem.position = Position::kPositionVecOut;
    removepad->outputs[0].attr.buf.id = ge::kIdNone;
    removepad->outputs[0].attr.que.id = 1;
    removepad->outputs[0].attr.mem.reuse_id = 1;
    removepad->outputs[0].attr.que.depth = 2;
    removepad->outputs[0].attr.que.buf_num = 2;
    removepad->outputs[0].attr.opt.ref_tensor = ge::kIdNone;
    removepad->outputs[0].attr.opt.merge_scope = ge::kIdNone;
