...
void LoadRminStore_BeforeAutofuse(ge::AscGraph &graph, ge::DataType data_type) {
  ...
  *rmin.y.strides = {One, Zero};
  rmin.attr.tmp_buffers = {{{ge::Symbol(8192), -1}, MemAttr()}};
...
void LoadRminStore_AfterAutofuse(ge::AscGraph& graph, ge::DataType data_type) {
  ...
  rmin->outputs[0].attr.buf.id = ge::kIdNone;
  ...
  rmin->outputs[0].attr.opt.ref_tensor = ge::kIdNone;
  rmin->outputs[0].attr.opt.merge_scope = ge::kIdNone;

  auto impl = ascgen_utils::GetAscIrCodegenImpl(rmin->GetType());
  std::vector<std::unique_ptr<ge::TmpBufDesc>> buffers = impl->CalcTmpBufSize(*rmin);
