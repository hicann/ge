...
void LoadErfStore_BeforeAutofuse(ge::AscGraph &graph) {
  ...
  *erf.y.strides = {s1*s2, s2, One};
  erf.attr.tmp_buffers = {{{ge::Symbol(8192), -1}, MemAttr()}};
...
void LoadErfStore_AfterInferOutput(ge::AscGraph &graph) {
...
void LoadErfStore_AfterGetApiInfo(ge::AscGraph &graph) {
  auto x = graph.FindNode("x");
...
void LoadErfStore_AfterScheduler(ge::AscGraph &graph) {
...
void LoadErfStore_AfterQueBufAlloc(ge::AscGraph &graph) {
  auto x = graph.FindNode("x");
  ...
  erf->outputs[0].attr.buf.id = ge::kIdNone;
  ...
  erf->outputs[0].attr.opt.ref_tensor = ge::kIdNone;
  erf->outputs[0].attr.opt.merge_scope = ge::kIdNone;
