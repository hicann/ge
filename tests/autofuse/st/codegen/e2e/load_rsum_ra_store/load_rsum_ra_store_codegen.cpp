...
class LoadRsumRAStoreSt : public testing::Test {
};

TEST_F(LoadRsumRAStoreSt, reduceSumTest) {
  bool gen_success = true;

  ge::AscGraph test_graph("load_rsum_ra_store");
  std::string tilig_stub = R"(
#define REGISTER_TILING_DEFAULT(tiling)
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
)";
  LoadRsumRAStore_BeforeAutofuse(test_graph, ge::DT_FLOAT);
  LoadRsumRAStore_AfterAutofuse(test_graph, ge::DT_FLOAT);

  std::vector<ge::AscGraph> test_impl_graphs = {ge::AscGraph("load_rsum_ra_store_general_0_nil_0_nil")};
  ...
  std::string tiling_data_src_file_name = parts[2]; // load_abs_store_tiling_data.h
  ...
  try {
    ...
    fused_schedule_result.fused_graph_name = ge::AscendString("load_rsum_ra_store");
