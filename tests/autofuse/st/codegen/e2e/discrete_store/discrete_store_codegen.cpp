...
TEST_F(LoadStoreUT, DiscreteStoreCodegen) {

  bool gen_success = true;
  ge::AscGraph test_graph("discrete_store");
  std::string tilig_stub = R"(
#define REGISTER_TILING_DEFAULT(tiling)
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
)";
  LoadAbsStore_BeforeAutofuse_DiscreteStore(test_graph);
  LoadAbsStore_AfterInferOutput(test_graph);

  std::vector<ge::AscGraph> test_impl_graphs = {ge::AscGraph("discrete_store_general_0_nil_0_nil")};
  test_impl_graphs[0].CopyFrom(test_graph);
  LoadAbsStore_AfterGetApiInfo(test_impl_graphs[0]);
  LoadAbsStore_AfterScheduler_z0_SplitTo_z0Bz0b(test_impl_graphs[0]);
  ...
  try {
    ...
    fused_schedule_result.fused_graph_name = ge::AscendString("discrete_store");
