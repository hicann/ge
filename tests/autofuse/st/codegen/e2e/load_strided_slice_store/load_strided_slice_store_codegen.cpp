...
class LoadSridedSliceStoreUT : public testing::Test {
};

TEST_F(LoadSridedSliceStoreUT, LoadSridedSliceStoreCodegen) {
  ge::AscGraph test_graph("load_strided_slice_store");
  std::string tilig_stub = R"(
#define REGISTER_TILING_DEFAULT(tiling)
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
)";
  LoadStridedSliceStore_BeforeAutofuse(test_graph);
  LoadStridedSliceStore_AfterInferOutput(test_graph);

  std::vector<ge::AscGraph> test_impl_graphs = {ge::AscGraph("load_strided_slice_store_general_0_nil_0_nil")};
  test_impl_graphs[0].CopyFrom(test_graph);
  LoadStridedSliceStore_AfterGetApiInfo(test_impl_graphs[0]);
  LoadStridedSliceStore_AfterScheduler(test_impl_graphs[0]);
  LoadStridedSliceStore_AfterQueBufAlloc(test_impl_graphs[0]);
  ...
  fused_schedule_result.fused_graph_name = ge::AscendString("load_strided_slice_store");
