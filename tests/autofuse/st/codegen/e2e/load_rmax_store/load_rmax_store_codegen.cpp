...
class LoadRmaxStoreSt : public testing::Test {
};

TEST_F(LoadRmaxStoreSt, reduceMaxTest) {
  bool gen_success = true;

  ge::AscGraph test_graph("load_rmax_store");
  std::string tilig_stub = R"(
#define REGISTER_TILING_DEFAULT(tiling)
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
)";
  LoadRmaxStore_BeforeAutofuse(test_graph, ge::DT_FLOAT);
  LoadRmaxStore_AfterAutofuse(test_graph, ge::DT_FLOAT);

  std::vector<ge::AscGraph> test_impl_graphs = {ge::AscGraph("load_rmax_store_general_0_nil_0_nil")};
  ...
  try {
    ...
    fused_schedule_result.fused_graph_name = ge::AscendString("load_rmax_store");
