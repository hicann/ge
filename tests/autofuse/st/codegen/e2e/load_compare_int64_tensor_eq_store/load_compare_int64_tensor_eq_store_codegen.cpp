...
class LoadCompareTensorEqStoreUT : public testing::Test {
};

TEST_F(LoadCompareTensorEqStoreUT, CompareTwoInt64TensorCodegen) {

  bool gen_success = true;
  ge::AscGraph test_graph("load_compare_int64_tensor_eq_store");
  std::string tilig_stub = R"(
#define REGISTER_TILING_DEFAULT(tiling)
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
)";
  std::vector<ge::AscGraph> test_impl_graphs;
  LoadCompareTensorEqStore_AfterAutofuse(test_graph, test_impl_graphs, ge::DT_INT64);

  std::cout<<"ATT_SO_NAME="<<ATT_SO_NAME<<std::endl;
  std::cout<<"KERNEL_SRC_LIST="<<KERNEL_SRC_LIST<<std::endl;
  std::vector<std::string> parts = splitString(KERNEL_SRC_LIST, ':');
  std::string kernel_src_file_name = parts[0];      // load_compare_int64_tensor_eq_store_kernel.cpp
  std::string tiling_src_file_name = parts[1];      // load_compare_int64_tensor_eq_store_tiling.cpp
  ...
  try {
    ...
    fused_schedule_result.fused_graph_name = ge::AscendString("load_compare_int64_tensor_eq_store");
