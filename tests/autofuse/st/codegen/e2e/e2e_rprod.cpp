...
void LoadRprodStore_BeforeAutofuse(ge::AscGraph &graph, ge::DataType data_type) {
  ...
  *load.y.strides = {s1, One};
  ...
  ge::ascir_op::Prod rprod("rprod");
  graph.AddNode(rprod);
  rprod.x = load.y;
  rprod.attr.sched.axis = {z0.id, z1.id};
  *rprod.y.axis = {z0.id, z1.id};
  *rprod.y.repeats = {s0, One};
  *rprod.y.strides = {One, Zero};
  rprod.attr.tmp_buffers = {{{ge::Symbol(8192), -1}, MemAttr()}};
...
void LoadRprodStore_AfterAutofuse(ge::AscGraph& graph, ge::DataType data_type) {
  ...
  auto rprod = graph.FindNode("rprod");
  rprod->attr.api.compute_type = ComputeType::kComputeReduce;
  rprod->outputs[0].attr.dtype = data_type;

  rprod->attr.api.type = ApiType::kAPITypeCompute;
  rprod->attr.api.unit = ComputeUnit::kUnitVector;
  ...
  rprod->attr.sched.loop_axis = z1T->id;
  rprod->outputs[0].attr.vectorized_axis = {z0t->id, z1t->id};
  rprod->outputs[0].attr.vectorized_strides = {One, Zero};
  ...
  rprod->outputs[0].attr.mem.tensor_id = 2;
  rprod->outputs[0].attr.mem.alloc_type = AllocType::kAllocTypeQueue;
  rprod->outputs[0].attr.mem.hardware = MemHardware::kMemHardwareUB;
  rprod->outputs[0].attr.mem.position = Position::kPositionVecOut;
  rprod->outputs[0].attr.buf.id = ge::kIdNone;
  rprod->outputs[0].attr.que.id = 1;
  rprod->outputs[0].attr.mem.reuse_id = 1;
  rprod->outputs[0].attr.que.depth = 2;
  rprod->outputs[0].attr.que.buf_num = 2;
  rprod->outputs[0].attr.opt.ref_tensor = ge::kIdNone;
  rprod->outputs[0].attr.opt.merge_scope = ge::kIdNone;

  auto impl = ascgen_utils::GetAscIrCodegenImpl(rprod->GetType());
  std::vector<std::unique_ptr<ge::TmpBufDesc>> buffers = impl->CalcTmpBufSize(*rprod);
  for (auto &buf_desc : buffers) {
    if (buf_desc != nullptr) {
      ge::TmpBuffer temp_buffer;
      temp_buffer.buf_desc = std::move(*buf_desc);
      rprod->attr.tmp_buffers.emplace_back(temp_buffer);
    }
  }
