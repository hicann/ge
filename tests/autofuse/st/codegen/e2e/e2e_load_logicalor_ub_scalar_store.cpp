...
void LoadLogicalOrUbScalarStore_BeforeAutofuse(ge::AscGraph &graph, ge::DataType in_data_type, ge::DataType out_data_type) {
  ...
  *load2.y.strides = {One, One, One};
  ...
  logical_or_op.x1 = load1.y;
  logical_or_op.x2 = load2.y;
  logical_or_op.y.dtype = out_data_type;
  logical_or_op.attr.sched.axis = {z0.id, z1.id, z2.id};
  *logical_or_op.y.repeats = {s0, s1, s2};
  *logical_or_op.y.strides = {s1*s2, s2, One};
  logical_or_op.attr.tmp_buffers = {{{ge::Symbol(8192), -1}, MemAttr()}};

  Store store("store");
  graph.AddNode(store);
  store.x = logical_or_op.y;
...
void LoadLogicalOrUbScalarStore_AfterInferOutput(ge::AscGraph &graph, ge::DataType in_data_type, ge::DataType out_data_type) {
    ...
    logical_or->outputs[0].attr.dtype = out_data_type;
    logical_or->outputs[0].attr.axis = load1->outputs[0].attr.axis;
    logical_or->outputs[0].attr.repeats = load1->outputs[0].attr.repeats;
    logical_or->outputs[0].attr.strides = load1->outputs[0].attr.strides;
    logical_or->attr.api.compute_type = ComputeType::kComputeElewise;
...
void LoadLogicalOrUbScalarStore_AfterGetApiInfo(ge::AscGraph &graph) {
    auto x1 = graph.FindNode("x1");
    ...
    auto logical_or = graph.FindNode("logical_or");
    logical_or->attr.api.type = ApiType::kAPITypeCompute;
    logical_or->attr.api.unit = ComputeUnit::kUnitVector;
...
void LoadLogicalOrUbScalarDivStore_AfterScheduler(ge::AscGraph &graph) {
    ...
    auto logical_or = graph.FindNode("logical_or");
    graph.ApplySplit(logical_or, z1T->id, z1t->id);
    graph.ApplySchedAxisMerge(logical_or, block_axis->id, m_axis->from);
    graph.ApplySplit(logical_or, z0B->id, z0b->id);
    logical_or->attr.sched.loop_axis = z0b->id;
    logical_or->outputs[0].attr.vectorized_axis = vectorized_axis;
    logical_or->outputs[0].attr.vectorized_strides = vectorized_strides;
...
void LoadLogicalOrUbScalarStore_AfterQueBufAlloc(ge::AscGraph &graph) {
    auto x1 = graph.FindNode("x1");
    ...
    auto logical_or = graph.FindNode("logical_or");
    logical_or->outputs[0].attr.mem.tensor_id = 4;
    logical_or->outputs[0].attr.mem.alloc_type = AllocType::kAllocTypeQueue;
    logical_or->outputs[0].attr.mem.hardware = MemHardware::kMemHardwareUB;
    logical_or->outputs[0].attr.mem.position = Position::kPositionVecOut;
    logical_or->outputs[0].attr.buf.id = ge::kIdNone;
    logical_or->outputs[0].attr.que.id = 2;
    logical_or->outputs[0].attr.mem.reuse_id = 1;
    logical_or->outputs[0].attr.que.depth = 2;
    logical_or->outputs[0].attr.que.buf_num = 2;
    logical_or->outputs[0].attr.opt.ref_tensor = ge::kIdNone;
