...
class ConcatSmallTailDimUT : public testing::Test {
};

TEST_F(ConcatSmallTailDimUT, ConcatSmallTailDim) {
  ge::AscGraph test_graph("concat_small_tail_dim");
  std::string tilig_stub = R"(
#define REGISTER_TILING_DEFAULT(tiling)
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
)";
  LoadConcatStore_SmallTailBeforeAutofuse(test_graph);
  LoadConcatStore_SmallTailAfterInferOutput(test_graph);

  std::vector<ge::AscGraph> test_impl_graphs = {ge::AscGraph("concat_small_tail_dim_general_0_nil_0_nil")};
  ...
  fused_schedule_result.fused_graph_name = ge::AscendString("concat_small_tail_dim");
