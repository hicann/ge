...
void LoadLogicalNotStore_BeforeAutofuse(ge::AscGraph &graph, ge::DataType in_data_type, ge::DataType out_data_type) {
  ...
  *logical_not_op.y.strides = {s1*s2, s2, One};
  logical_not_op.attr.tmp_buffers = {{{ge::Symbol(8192), -1}, MemAttr()}};

  Store store("store");
  graph.AddNode(store);
  store.x = logical_not_op.y;
...
void LoadLogicalNotStore_AfterInferOutput(ge::AscGraph &graph, ge::DataType in_data_type, ge::DataType out_data_type) {
  ...
  logical_not->outputs[0].attr.dtype = out_data_type;
  logical_not->outputs[0].attr.axis = load->outputs[0].attr.axis;
  logical_not->outputs[0].attr.repeats = load->outputs[0].attr.repeats;
  logical_not->outputs[0].attr.strides = load->outputs[0].attr.strides;
  logical_not->attr.api.compute_type = ComputeType::kComputeElewise;
...
void LoadLogicalNotStore_AfterGetApiInfo(ge::AscGraph &graph) {
  ...
  auto logical_not = graph.FindNode("logical_not");
  logical_not->attr.api.type = ApiType::kAPITypeCompute;
  logical_not->attr.api.unit = ComputeUnit::kUnitVector;
...
void LoadLogicalNotStore_AfterScheduler(ge::AscGraph &graph) {
  ...
  auto load1 = graph.FindNode("load");
  ...
  auto logical_not = graph.FindNode("logical_not");
  graph.ApplySplit(logical_not, z1T->id, z1t->id);
  graph.ApplySchedAxisMerge(logical_not, block_axis->id, m_axis->from);
  graph.ApplySplit(logical_not, z0B->id, z0b->id);
  logical_not->attr.sched.loop_axis = z0b->id;
  logical_not->outputs[0].attr.vectorized_axis = vectorized_axis;
  logical_not->outputs[0].attr.vectorized_strides = vectorized_strides;
...
void LoadLogicalNotStore_AfterQueBufAlloc(ge::AscGraph &graph) {
  ...
  auto logical_not = graph.FindNode("logical_not");
  logical_not->outputs[0].attr.mem.tensor_id = 4;
  logical_not->outputs[0].attr.mem.alloc_type = AllocType::kAllocTypeQueue;
  logical_not->outputs[0].attr.mem.hardware = MemHardware::kMemHardwareUB;
  logical_not->outputs[0].attr.mem.position = Position::kPositionVecOut;
  logical_not->outputs[0].attr.buf.id = ge::kIdNone;
  logical_not->outputs[0].attr.que.id = 2;
  logical_not->outputs[0].attr.mem.reuse_id = 1;
  logical_not->outputs[0].attr.que.depth = 2;
  logical_not->outputs[0].attr.que.buf_num = 2;
  logical_not->outputs[0].attr.opt.ref_tensor = ge::kIdNone;
