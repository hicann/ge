 ...
 */
#include "e2e_load_compare_tensor_ne_store.h"
...
void LoadCompareTensorNeStore_BeforeAutofuse(ge::AscGraph &graph, ge::DataType data_type) {
  ...
  *load2.y.strides = {One};
  ...
  ne.x1 = load1.y;
  ne.x2 = load2.y;
  ne.attr.sched.axis = {z0.id};
  ne.attr.tmp_buffers = {{{ge::Symbol(8192), -1}, MemAttr()}};
...
void LoadCompareTensorNeStore_AfterInferOutput(ge::AscGraph &graph, ge::DataType data_type) {
  ...
  ne->outputs[0].attr.axis = load1->outputs[0].attr.axis;
  ne->outputs[0].attr.repeats = load1->outputs[0].attr.repeats;
  ne->outputs[0].attr.strides = load1->outputs[0].attr.strides;
  ne->attr.api.compute_type = ComputeType::kComputeElewise;
...
void LoadCompareTensorNeStore_AfterGetApiInfo(ge::AscGraph &graph) {
  auto x1 = graph.FindNode("x1");
  ...
  auto ne = graph.FindNode("ne");
  ne->attr.api.type = ApiType::kAPITypeCompute;
  ne->attr.api.unit = ComputeUnit::kUnitVector;
...
void LoadCompareTensorNeStore_AfterScheduler(ge::AscGraph &graph) {
...
void LoadCompareTensorNeStore_AfterQueBufAlloc(ge::AscGraph &graph) {
  int tensor_id = 0;
  ...
  auto ne = graph.FindNode("ne");
  ne->outputs[0].attr.mem.tensor_id = tensor_id++;
  ne->outputs[0].attr.mem.alloc_type = AllocType::kAllocTypeQueue;
  ne->outputs[0].attr.mem.hardware = MemHardware::kMemHardwareUB;
  ne->outputs[0].attr.mem.position = Position::kPositionVecOut;
  ne->outputs[0].attr.buf.id = ge::kIdNone;
  ne->outputs[0].attr.que.id = 2;
  ne->outputs[0].attr.mem.reuse_id = 2;
  ne->outputs[0].attr.que.depth = 1;
  ne->outputs[0].attr.que.buf_num = 1;
  ne->outputs[0].attr.opt.ref_tensor = ge::kIdNone;
...
void LoadCompareTensorNeStore_AfterAutofuse(ge::AscGraph &graph, std::vector<ge::AscGraph> &impl_graphs,
                                            ge::DataType data_type) {
   LoadCompareTensorNeStore_BeforeAutofuse(graph, data_type);
   LoadCompareTensorNeStore_AfterInferOutput(graph, data_type);

   impl_graphs.push_back(ge::AscGraph("load_compare_half_tensor_ne_store_general_0_nil_0_nil"));
