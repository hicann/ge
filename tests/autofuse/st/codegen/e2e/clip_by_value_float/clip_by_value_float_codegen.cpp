...
class ClipByValueFloatUT : public testing::Test {
};

TEST_F(ClipByValueFloatUT, CompareTwoHalfTensorCodegen) {

  bool gen_success = true;
  ge::AscGraph test_graph("clip_by_value_float");
  std::string tilig_stub = R"(
#define REGISTER_TILING_DEFAULT(tiling)
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
)";
  std::vector<ge::AscGraph> test_impl_graphs;
  ClipByValueFloat_AfterAutofuse(test_graph, test_impl_graphs, ge::DT_FLOAT);

  std::cout<<"ATT_SO_NAME="<<ATT_SO_NAME<<std::endl;
  std::cout<<"KERNEL_SRC_LIST="<<KERNEL_SRC_LIST<<std::endl;
  std::vector<std::string> parts = splitString(KERNEL_SRC_LIST, ':');
  std::string kernel_src_file_name = parts[0];      // clip_by_value_float_kernel.cpp
  std::string tiling_src_file_name = parts[1];      // clip_by_value_float_tiling.cpp
  ...
  try {
    ...
    fused_schedule_result.fused_graph_name = ge::AscendString("clip_by_value_float");
