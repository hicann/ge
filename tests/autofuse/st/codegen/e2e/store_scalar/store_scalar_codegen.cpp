...
TEST_F(LoadStoreUT, DiscreteStoreCodegen) {
  ...
  ge::AscGraph test_graph("store_scalar");
  std::string tilig_stub = R"(
#define REGISTER_TILING_DEFAULT(tiling)
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
)";
  LoadAbsStore_BeforeAutofuse_StoreScalar(test_graph);
  LoadAbsStore_AfterInferOutput(test_graph);

  std::vector<ge::AscGraph> test_impl_graphs = {ge::AscGraph("store_scalar_general_0_nil_0_nil")};
  ...
  try {
    ...
    fused_schedule_result.fused_graph_name = ge::AscendString("store_scalar");
