...
class Concat3DLastDimUT : public testing::Test {
};

TEST_F(Concat3DLastDimUT, Concat3DLastDimCodegen) {

  bool gen_success = true;
  ge::AscGraph test_graph("concat_3d_last_dim");
  std::string tilig_stub = R"(
#define REGISTER_TILING_DEFAULT(tiling)
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
)";
  LoadConcatStore_BeforeAutofuse3dLastAxis(test_graph);
  LoadConcatStore_AfterInferOutput(test_graph);

  std::vector<ge::AscGraph> test_impl_graphs = {ge::AscGraph("concat_3d_last_dim_general_0_nil_0_nil")};
  test_impl_graphs[0].CopyFrom(test_graph);
  LoadConcatStore_AfterGetApiInfo(test_impl_graphs[0]);
  LoadConcatStore_AfterScheduler3dLastAxis(test_impl_graphs[0]);
  ...
  fused_schedule_result.fused_graph_name = ge::AscendString("concat_3d_last_dim");
