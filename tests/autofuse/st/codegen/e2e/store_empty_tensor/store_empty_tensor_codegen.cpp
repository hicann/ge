...
class StoreEmptyTensor : public testing::Test {
};

TEST_F(StoreEmptyTensor, EmptyTensorCodeGen) {

  bool gen_success = true;
  ge::AscGraph test_graph("store_emptytensor");
  std::string tilig_stub = R"(
#define REGISTER_TILING_DEFAULT(tiling)
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
)";
  LoadAbsStore_BeforeAutofuse_StoreEmptyTensor(test_graph);
  LoadAbsStore_AfterInferOutput(test_graph);

  std::vector<ge::AscGraph> test_impl_graphs = {ge::AscGraph("store_emptytensor_general_0_nil_0_nil")};
  test_impl_graphs[0].CopyFrom(test_graph);
  LoadAbsStore_AfterGetApiInfo(test_impl_graphs[0]);
  LoadAbsStore_AfterScheduler_store_scalar(test_impl_graphs[0]);
  ...
  try {
    ...
    fused_schedule_result.fused_graph_name = ge::AscendString("store_emptytensor");
