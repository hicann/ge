...
void LoadReciprocalStore_BeforeAutofuse(ge::AscGraph &graph) {
  ...
  *reciprocal.y.strides = {s1*s2, s2, One};
  reciprocal.attr.tmp_buffers = {{{ge::Symbol(8192), -1}, MemAttr()}};
...
void LoadReciprocalStore_AfterInferOutput(ge::AscGraph &graph) {
  ...
  reciprocal->attr.api.compute_type = ComputeType::kComputeElewise;

  auto store = graph.FindNode("store");
  store->outputs[0].attr.dtype = (ge::DataType)reciprocal->outputs[0].attr.dtype;
...
void LoadReciprocalStore_AfterGetApiInfo(ge::AscGraph &graph) {
  auto x = graph.FindNode("x");
  ...
  auto reciprocal = graph.FindNode("reciprocal");
  reciprocal->attr.api.type = ApiType::kAPITypeCompute;
  reciprocal->attr.api.unit = ComputeUnit::kUnitVector;
...
void LoadReciprocalStore_AfterScheduler_z0z1z2_splitTo_z0z1TBz0z1Tbz1tz2(ge::AscGraph &graph) {
  ...
  // ApplySplit on load, reciprocal, store
  auto load = graph.FindNode("load");
  ...
  auto reciprocal = graph.FindNode("reciprocal");
  graph.ApplySplit(reciprocal, z1T->id, z1t->id);
  graph.ApplySchedAxisMerge(reciprocal, block_axis->id, m_axis->from);
  graph.ApplySplit(reciprocal, z0B->id, z0b->id);
  reciprocal->attr.sched.loop_axis = z0b->id;
  reciprocal->outputs[0].attr.vectorized_axis = vectorized_axis;
  reciprocal->outputs[0].attr.vectorized_strides = vectorized_strides;
...
void LoadReciprocalStore_AfterQueBufAlloc(ge::AscGraph &graph) {
  auto x = graph.FindNode("x");
  ...
  auto reciprocal = graph.FindNode("reciprocal");
  reciprocal->outputs[0].attr.mem.tensor_id = 2;
  reciprocal->outputs[0].attr.mem.alloc_type = AllocType::kAllocTypeQueue;
  reciprocal->outputs[0].attr.mem.hardware = MemHardware::kMemHardwareUB;
  reciprocal->outputs[0].attr.mem.position = Position::kPositionVecOut;
  reciprocal->outputs[0].attr.buf.id = ge::kIdNone;
  reciprocal->outputs[0].attr.que.id = 1;
  reciprocal->outputs[0].attr.mem.reuse_id = 1;
  reciprocal->outputs[0].attr.que.depth = 2;
  reciprocal->outputs[0].attr.que.buf_num = 2;
  reciprocal->outputs[0].attr.opt.ref_tensor = ge::kIdNone;
  reciprocal->outputs[0].attr.opt.merge_scope = ge::kIdNone;
