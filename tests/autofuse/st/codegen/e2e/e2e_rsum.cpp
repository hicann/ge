...
void LoadRsumStore_BeforeAutofuse(ge::AscGraph &graph, ge::DataType data_type) {
  ...
  *rsum.y.strides = {One, Zero};
  rsum.attr.tmp_buffers = {{{ge::Symbol(8192), -1}, MemAttr()}};
...
void LoadRsumStore_AfterAutofuse(ge::AscGraph& graph, ge::DataType data_type) {
  ...
  rsum->outputs[0].attr.buf.id = ge::kIdNone;
  ...
  rsum->outputs[0].attr.opt.ref_tensor = ge::kIdNone;
  rsum->outputs[0].attr.opt.merge_scope = ge::kIdNone;

  auto impl = ascgen_utils::GetAscIrCodegenImpl(rsum->GetType());
  std::vector<std::unique_ptr<ge::TmpBufDesc>> buffers = impl->CalcTmpBufSize(*rsum);
...
void LoadRsumRAStore_BeforeAutofuse(ge::AscGraph &graph, ge::DataType data_type) {
  auto s0 = graph.CreateSizeVar("s0");
  auto s1 = graph.CreateSizeVar("s1");
...
void LoadRsumRAStore_AfterAutofuse(ge::AscGraph& graph, ge::DataType data_type) {
  auto x = graph.FindNode("x");
  x->attr.api.compute_type = ComputeType::kComputeInvalid;
  x->attr.api.type = ApiType::kAPITypeBuffer;
  x->attr.api.unit = ComputeUnit::kUnitNone;
  ...
  auto size = ge::GetSizeByDataType(data_type);
  vectorized_strides[1] = ge::sym::Align(graph.FindAxis(z3)->size, 32 / size);
...
void LoadRsumUbStore_AfterAutofuse(ge::AscGraph& graph, ge::DataType data_type) {
  auto x = graph.FindNode("x");
  x->attr.api.compute_type = ComputeType::kComputeInvalid;
  x->attr.api.type = ApiType::kAPITypeBuffer;
  x->attr.api.unit = ComputeUnit::kUnitNone;
...
void LoadRsumInvalidAxisStore_BeforeAutofuse(ge::AscGraph &graph, ge::DataType data_type) {
  auto s0 = graph.CreateSizeVar("s0");
  auto s1 = graph.CreateSizeVar("s1");
  auto s2 = graph.CreateSizeVar("s2");
  auto s3 = graph.CreateSizeVar("s3");
...
void LoadRsumInvalidAxisStore_AfterAutofuse(ge::AscGraph& graph, ge::DataType data_type) {
  auto x = graph.FindNode("x");
  x->attr.api.compute_type = ComputeType::kComputeInvalid;
  x->attr.api.type = ApiType::kAPITypeBuffer;
  x->attr.api.unit = ComputeUnit::kUnitNone;
  ...
  auto size = ge::GetSizeByDataType(data_type);
  vectorized_strides[2] = ge::sym::Align(graph.FindAxis(z3)->size, 32 / size);
