...
extern "C" int64_t PgoAutofuseTiling(const char* config_file, uint32_t s2, uint32_t s3, AutofuseTilingData* tiling, uint32_t* workspaceSize, uint32_t *blockDim, ResLimit *res_limit = nullptr)
{
...
extern "C" ge::graphStatus TilingFunc(gert::TilingSymbolEvalContext *context)
{
  ...
  auto ret = PgoAutofuseTiling(config_file, s2, s3, tiling_data, &workspace_size, &block_dim, &limit);
