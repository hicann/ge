...
extern "C" __global__ __aicore__ void scalar_brc_test(GM_ADDR input, GM_ADDR output, GM_ADDR workspace, GM_ADDR gm_tiling_data);
...
namespace {
class E2E_ScalarBrc_Code : public testing::Test, public testing::WithParamInterface<std::vector<int>> {
};

TEST_P(E2E_ScalarBrc_Code, CalculateCorrect){
  ...
  float* x = (float *)AscendC::GmAlloc(1 * sizeof(float) + 32);
  ...
  ICPU_RUN_KF(scalar_brc_test, tiling_data.block_dim, (uint8_t *)x, (uint8_t *)y, nullptr, (uint8_t*)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_ScalarBrc_Code,
