...
class TestBackendLoadBrcE2e : public testing::Test {
 protected:
  void SetUp() override {
  ...
  TestBackendLoadBrcE2e() : optimizer(optimize::OptimizerOptions{}) {}
};

TEST_F(TestBackendLoadBrcE2e, LoadBrcE2eCodegen) {
...
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
  ...
  auto graph = ascir::ShareGraph::LoadBrcFusedGraph(7);
  std::cout<<"KERNEL_SRC_LIST="<<KERNEL_SRC_LIST<<std::endl;
  std::vector<std::string> parts = splitString(KERNEL_SRC_LIST, ':');
  std::string kernel_src_file_name = parts[0];      // load_brc_test_tiling.cpp
  std::string tiling_src_file_name = parts[1];      // load_brc_test_kernel.cpp
