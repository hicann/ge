...
extern "C" __global__ __aicore__ void cast_abs_test(GM_ADDR x, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling);
...
class E2E_BackendCastAbsFloat16Float_Code : public testing::Test, public testing::WithParamInterface<std::vector<int>> {
};

TEST_P(E2E_BackendCastAbsFloat16Float_Code, CalculateCorrect) {
  ...
  half* input = (half *)AscendC::GmAlloc(test_size * sizeof(half) + 32);
  ...
  for (int i = 0; i < test_size; i++) {
    input[i] = i; //rand() / (double)RAND_MAX;

    expect[i] = std::fabs(static_cast<float>(input[i]));
  }
  ...
  ICPU_RUN_KF(cast_abs_test, tiling_data.block_dim, (uint8_t *)input, (uint8_t *)y, nullptr, (uint8_t*)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_BackendCastAbsFloat16Float_Code,
