 ...
 *        ouput0
 */
static void CreateLoadLogicalNotStoreAscGraph(ge::AscGraph &graph, size_t dims_size, DataType dt = ge::DT_FLOAT) {
  ge::ascir_op::Data x1("data0", graph);
  x1.ir_attr.SetIndex(0);
  x1.y.dtype = dt;
...
ge::ComputeGraphPtr ShareGraph::LoadLogicalNotStoreFusedGraph(size_t dims_size, DataType dt) {
  auto builder = GraphBuilder("load_logical_not_store_test");
  auto data0 = builder.AddNode("data0", "Data", 0, 1);
  ge::AttrUtils::SetInt(data0->GetOpDescBarePtr(), "_parent_node_index", 0);
  ...
  auto ascbc_node = compute_graph->FindNode("ascbc");
  ge::AscGraph sub_graph("load_logical_not_store");
  CreateLoadLogicalNotStoreAscGraph(sub_graph, dims_size, dt);
...
static void LoadGatherOneAxisAbsStore_BeforeAutofuse(ge::AscGraph &graph, int64_t gather_axis, ge::DataType data_type) {
   ...
   *gather.y.axis = {z1.id};
   *gather.y.repeats = {s1};
   *gather.y.strides = {ge::ops::One};

   ge::ascir_op::Abs abs("abs");
   graph.AddNode(abs);
   abs.x = gather.y;
   abs.attr.sched.axis = {z1.id};
   *abs.y.axis = {z1.id};
   *abs.y.repeats = {s1};
   *abs.y.strides = {ge::ops::One};
