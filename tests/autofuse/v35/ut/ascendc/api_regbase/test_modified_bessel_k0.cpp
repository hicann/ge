...
class TestRegbaseApiModifiedBesselK0UT : public testing::Test {
 protected:
  // Tensor - Tensor 场景
  template <typename T>
  static void InvokeTensorTensorKernel(UnaryInputParam<T> &param) {
    ...
    ModifiedBesselK0Extend(l_y, l_x, l_tmp, param.size);
  ...
  static void CreateTensorInput(UnaryInputParam<T> &param) {
    ...
    for (uint32_t i = 0; i < param.size; i++) {
      // K0 only defined for x > 0, generate range [0.0, 20.0]
      ...
      if (param.x1[i] == 0.0f) {
        ...
        param.exp[i] = static_cast<T>(std::cyl_bessel_k(0, param.x1[i]));
      }
    }
  }
  ...
  // Tensor - Tensor 测试
  template <typename T>
  static void ModifiedBesselK0Test(uint32_t size) {
...
TEST_F(TestRegbaseApiModifiedBesselK0UT, ModifiedBesselK0_TensorTensor_Test) {
  ModifiedBesselK0Test<float>(ONE_BLK_SIZE / sizeof(float));
  ModifiedBesselK0Test<float>(ONE_REPEAT_BYTE_SIZE / sizeof(float));
  ModifiedBesselK0Test<float>(MAX_REPEAT_NUM * ONE_REPEAT_BYTE_SIZE / 2 / sizeof(float));
  ModifiedBesselK0Test<float>((ONE_BLK_SIZE - sizeof(float)) / sizeof(float));
  ModifiedBesselK0Test<float>((ONE_REPEAT_BYTE_SIZE - ONE_BLK_SIZE) / sizeof(float));
  ModifiedBesselK0Test<float>((MAX_REPEAT_NUM - 1) * ONE_REPEAT_BYTE_SIZE / 2 / sizeof(float));
  ModifiedBesselK0Test<float>(((MAX_REPEAT_NUM - 1) * ONE_REPEAT_BYTE_SIZE + (ONE_REPEAT_BYTE_SIZE - ONE_BLK_SIZE) +
