...
class TestRegbaseApiModifiedBesselI0UT : public testing::Test {
 protected:
  // Tensor - Tensor 场景
  template <typename T>
  static void InvokeTensorTensorKernel(UnaryInputParam<T> &param) {
    ...
    ModifiedBesselI0Extend(l_y, l_x, l_tmp, param.size);
  ...
  // Tensor - Tensor 测试
  template <typename T>
  static void ModifiedBesselI0Test(uint32_t size) {
...
TEST_F(TestRegbaseApiModifiedBesselI0UT, ModifiedBesselI0_TensorTensor_Test) {
  ModifiedBesselI0Test<float>(ONE_BLK_SIZE / sizeof(float));
  ModifiedBesselI0Test<float>(ONE_REPEAT_BYTE_SIZE / sizeof(float));
  ModifiedBesselI0Test<float>(MAX_REPEAT_NUM * ONE_REPEAT_BYTE_SIZE / 2 / sizeof(float));
  ModifiedBesselI0Test<float>((ONE_BLK_SIZE - sizeof(float)) / sizeof(float));
  ModifiedBesselI0Test<float>((ONE_REPEAT_BYTE_SIZE - ONE_BLK_SIZE) / sizeof(float));
  ModifiedBesselI0Test<float>((MAX_REPEAT_NUM - 1) * ONE_REPEAT_BYTE_SIZE / 2 / sizeof(float));
  ModifiedBesselI0Test<float>(((MAX_REPEAT_NUM - 1) * ONE_REPEAT_BYTE_SIZE + (ONE_REPEAT_BYTE_SIZE - ONE_BLK_SIZE) +
