...
class TestRegbaseApiSinhUT : public testing::Test {
 protected:
  // Tensor - Tensor 场景
  template <typename T>
  static void InvokeTensorTensorKernel(SinhInputParam<T> &param) {
    TPipe tpipe;
    ...
    AscendC::Sinh(l_y, l_x, l_tmp, param.size);
    UbToGm(param.y, l_y, param.size);
  }

  template <typename T>
  static void CreateTensorInput(SinhInputParam<T> &param) {
    ...
    for (uint32_t i = 0; i < param.size; i++) {
      ...
      param.x[i] = distr(eng);
      param.exp[i] = static_cast<T>(std::sinh(static_cast<double>(param.x[i])));
    }
  }

  template <typename T>
  static uint32_t Valid(SinhInputParam<T> &param) {
  ...
  // Tensor - Tensor 测试
  template <typename T>
  static void SinhTest(uint32_t size) {
    SinhInputParam<T> param{};
    param.size = size;
...
TEST_F(TestRegbaseApiSinhUT, Sinh_TensorTensor_Test) {
  SinhTest<half>(ONE_BLK_SIZE / sizeof(half));
  SinhTest<half>(ONE_REPEAT_BYTE_SIZE / sizeof(half));
  SinhTest<half>(MAX_REPEAT_NUM * ONE_REPEAT_BYTE_SIZE / 2 / sizeof(half));
  SinhTest<half>((ONE_BLK_SIZE - sizeof(half)) / sizeof(half));
  SinhTest<half>((ONE_REPEAT_BYTE_SIZE - ONE_BLK_SIZE) / sizeof(half));
  SinhTest<half>((MAX_REPEAT_NUM - 1) * ONE_REPEAT_BYTE_SIZE / 2 / sizeof(half));
  SinhTest<float>(ONE_BLK_SIZE / sizeof(float));
  SinhTest<float>(ONE_REPEAT_BYTE_SIZE / sizeof(float));
  SinhTest<float>(MAX_REPEAT_NUM * ONE_REPEAT_BYTE_SIZE / 2 / sizeof(float));
