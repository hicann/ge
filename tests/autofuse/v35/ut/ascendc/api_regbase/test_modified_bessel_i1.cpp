...
class TestRegbaseApiModifiedBesselI1UT : public testing::Test {
 protected:
  // Tensor - Tensor 场景
  template <typename T>
  static void InvokeTensorTensorKernel(UnaryInputParam<T> &param) {
    ...
    ModifiedBesselI1Extend(l_y, l_x, l_tmp, param.size);
  ...
  static void CreateTensorInput(UnaryInputParam<T> &param) {
    ...
    for (uint32_t i = 0; i < param.size; i++) {
      ...
      // I1 preserves sign: I1(-x) = -I1(x)
      int sign = (param.x1[i] < 0) ? -1 : 1;
      param.exp[i] = static_cast<T>(sign * std::cyl_bessel_i(1, std::fabs(param.x1[i])));
    }
  }
  ...
  // Tensor - Tensor 测试
  template <typename T>
  static void ModifiedBesselI1Test(uint32_t size) {
...
TEST_F(TestRegbaseApiModifiedBesselI1UT, ModifiedBesselI1_TensorTensor_Test) {
  ModifiedBesselI1Test<float>(ONE_BLK_SIZE / sizeof(float));
  ModifiedBesselI1Test<float>(ONE_REPEAT_BYTE_SIZE / sizeof(float));
  ModifiedBesselI1Test<float>(MAX_REPEAT_NUM * ONE_REPEAT_BYTE_SIZE / 2 / sizeof(float));
  ModifiedBesselI1Test<float>((ONE_BLK_SIZE - sizeof(float)) / sizeof(float));
