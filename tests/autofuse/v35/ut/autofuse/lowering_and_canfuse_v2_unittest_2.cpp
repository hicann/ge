...
static std::vector<int64_t> RepeatsToInt64Vec(const std::vector<ge::Expression> &repeats) {
...
static void BuildConv2DGraphWithBiasAndAttr(es::Graph& graph, const std::string& data_format,
  ...
  if (!pad_mode.empty()) {
    (void)ge::AttrUtils::SetStr(conv2d.GetEsbTensor()->GetProducer()->GetOpDesc(), "padding", pad_mode);
    (void)ge::AttrUtils::SetBool(conv2d.GetEsbTensor()->GetProducer()->GetOpDesc(), "enable_hf32", enable_hf32);
  }
...
static void CheckConv2DOffsetX(const NodePtr& asc_node, const std::string& node_type, int64_t expected_offset_x) {
  ...
  if (node_type == "Conv2DBias") {
    auto conv2d_attr = node_attr->ir_attr->DownCastTo<ge::ascir_op::Conv2DBias::AscConv2DBiasIrAttrDef>();
    ...
    auto conv2d_attr = node_attr->ir_attr->DownCastTo<ge::ascir_op::Conv2D::AscConv2DIrAttrDef>();
    if (conv2d_attr != nullptr) {
      (void)conv2d_attr->GetOffset_x(offset_x_attr);
      EXPECT_EQ(offset_x_attr, expected_offset_x);
    }
