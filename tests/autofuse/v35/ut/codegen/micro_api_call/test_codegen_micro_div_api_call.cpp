...
TEST(CodegenKernel, MicroDivApiCall_Load_Div_Float_Store) {
  ge::AscGraph graph("test_div_graph");
  ...
  *store_op.y.strides = {s1 * s2 * s3 * Four, s2 * s3 * Three, s3 * Two, One};
  ...
  div_node->attr.api.compute_type = ge::ComputeType::kComputeLoad;
  div_node->attr.api.type = ge::ApiType::kAPITypeCompute;
  div_node->attr.api.unit = ge::ComputeUnit::kUnitMTE2;
  div_node->attr.sched.loop_axis = z0.id;
  div_node->outputs[0].attr.vectorized_axis = {z1.id, z2.id, z3.id};
  div_node->outputs[0].attr.vectorized_strides = {ge::Symbol(8), ge::Symbol(2), One};
  div_node->outputs[0].attr.dtype = ge::DT_FLOAT;
  div_node->outputs[0].attr.mem.position = ge::Position::kPositionVecIn;
  div_node->outputs[0].attr.mem.tensor_id = 1;
  div_node->outputs[0].attr.mem.position = ge::Position::kPositionVecIn;
  div_node->outputs[0].attr.mem.alloc_type = ge::AllocType::kAllocTypeQueue;
  div_node->outputs[0].attr.que.id = 2;
  div_node->outputs[0].attr.opt.merge_scope = ge::kIdNone;
...
TEST(CodegenKernel, MicroDivApiCall_Load_Div_Half_Store) {
  ...
  *store_op.y.strides = {s1 * s2 * s3 * Four, s2 * s3 * Three, s3 * Two, One};
  ...
  div_node->outputs[0].attr.vectorized_axis = {z1.id, z2.id, z3.id};
  div_node->outputs[0].attr.vectorized_strides = {ge::Symbol(8), ge::Symbol(2), One};
  div_node->outputs[0].attr.dtype = ge::DT_FLOAT16;
  div_node->outputs[0].attr.mem.position = ge::Position::kPositionVecIn;
  div_node->outputs[0].attr.mem.tensor_id = 1;
  div_node->outputs[0].attr.mem.position = ge::Position::kPositionVecIn;
  div_node->outputs[0].attr.mem.alloc_type = ge::AllocType::kAllocTypeQueue;
