...
// 测试：Split 尾轴连接 Concat 的场景
// 场景：Data -> Load -> Split(尾轴) -> Concat(尾轴) -> Store -> Output
// 期望：触发 concat_dim == split_dim 优化，只生成 1 张 Split 转 Load 的图
TEST_F(TestOptimizerV2, SplitTailAxisConcatSameDim) {
  ge::AscGraph graph("split_tail_concat_test");

  auto s0 = graph.CreateSizeVar(2);
  auto s1 = graph.CreateSizeVar(4);
  auto s_split = graph.CreateSizeVar(24);

  auto z0 = graph.CreateAxis("z0", s0);
  auto z1 = graph.CreateAxis("z1", s1);
  auto z_split = graph.CreateAxis("z_split", s_split);

  // Data -> Load
  ge::ascir_op::Data data("data", graph);
  data.ir_attr.SetIndex(0);
  data.attr.sched.axis = {z0.id, z1.id, z_split.id};
  ...
  *data.y.axis = {z0.id, z1.id, z_split.id};
  *data.y.repeats = {s0, s1, s_split};
  *data.y.strides = {s1 * s_split, s_split, One};

  ge::ascir_op::Load load("load");
  load.x = data.y;
  load.attr.sched.axis = {z0.id, z1.id, z_split.id};
  load.attr.api.compute_type = ComputeType::kComputeLoad;
  load.y.dtype = ge::DT_FLOAT16;
  *load.y.axis = {z0.id, z1.id, z_split.id};
  *load.y.repeats = {s0, s1, s_split};
  *load.y.strides = {s1 * s_split, s_split, One};

  // Split (在尾轴 split)
  ge::ascir_op::Split split("split");
  split.InstanceOutputy(2U);
  split.x = load.y;
  split.attr.sched.axis = {z0.id, z1.id, z_split.id};

  split.y[0].dtype = ge::DT_FLOAT16;
  *split.y[0].axis = {z0.id, z1.id, z_split.id};
  *split.y[0].repeats = {s0, s1, ge::Symbol(8)};
  *split.y[0].strides = {s1 * s_split, s_split, One};

  split.y[1].dtype = ge::DT_FLOAT16;
  *split.y[1].axis = {z0.id, z1.id, z_split.id};
  *split.y[1].repeats = {s0, s1, ge::Symbol(16)};
  *split.y[1].strides = {s1 * s_split + s1 * ge::Symbol(8), s_split, One};

  // Concat (在尾轴 concat)
  ge::ascir_op::Concat concat("concat");
  concat.x = {split.y[0], split.y[1]};
  concat.attr.sched.axis = {z0.id, z1.id, z_split.id};
  concat.attr.api.compute_type = ComputeType::kComputeInvalid;
  concat.y.dtype = ge::DT_FLOAT16;
  *concat.y.axis = {z0.id, z1.id, z_split.id};
  *concat.y.repeats = {s0, s1, s_split};
  *concat.y.strides = {s1 * s_split, s_split, One};

  ge::ascir_op::Store store("store");
  store.x = concat.y;
  store.attr.sched.axis = {z0.id, z1.id, z_split.id};
  store.attr.api.compute_type = ComputeType::kComputeStore;
  store.y.dtype = ge::DT_FLOAT16;
  *store.y.axis = {z0.id, z1.id, z_split.id};
  *store.y.repeats = {s0, s1, s_split};
  *store.y.strides = {s1 * s_split, s_split, One};
  ...
  EXPECT_EQ(split_node->GetType(), ge::ascir_op::Split::Type);

  auto concat_node = graph.FindNode("concat");
  ASSERT_NE(concat_node, nullptr);
  EXPECT_EQ(concat_node->GetType(), ge::ascir_op::Concat::Type);
