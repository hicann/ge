...
class TestBackendBf16SigmoidE2e : public testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(TestBackendBf16SigmoidE2e, Bf16SigmoidE2eCodegen) {
...
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
  ...
  // shape_info 和 BF16SigmoidFusedGraph入参dims_size匹配（个数相同，命名规则为s开头、编号从0开始）
  std::map<std::string, std::string> shape_info({{"s0", "stub_s0"}, {"s1", "stub_s1"}});
  auto graph = ascir::ShareGraph::BF16SigmoidFusedGraph(2);
