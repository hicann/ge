...
class TestBackendLoadLoopModeE2e : public testing::Test {
  void SetUp() override {
...
TEST_F(TestBackendLoadLoopModeE2e, LoadLoopModeE2eCodegen) {
...
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
  ...
  auto graph = ascir::ShareGraph::LoadNeedLoopModeFusedGraph(3);
  std::cout << "KERNEL_SRC_LIST=" << KERNEL_SRC_LIST << std::endl;
  std::vector<std::string> parts = splitString(KERNEL_SRC_LIST, ':');
  std::string kernel_src_file_name = parts[0];       // load_loop_mode_test_tiling.cpp
  std::string tiling_src_file_name = parts[1];       // load_loop_mode_test_kernel.cpp
