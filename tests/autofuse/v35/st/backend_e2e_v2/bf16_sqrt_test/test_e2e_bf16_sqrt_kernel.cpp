...
extern "C" __global__ __aicore__ void sqrt_bf16_test(GM_ADDR x1, GM_ADDR y1, GM_ADDR workspace, GM_ADDR tiling);
...
class E2E_BackendBf16Sqrt_Code : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

TEST_P(E2E_BackendBf16Sqrt_Code, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    ...
    expect[i] = static_cast<bfloat16_t>(std::sqrt(val));
  }
  ...
  ICPU_RUN_KF(sqrt_bf16_test, tiling_data.block_dim, (uint8_t *)x, (uint8_t *)y, nullptr, (uint8_t *)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_BackendBf16Sqrt_Code, ::testing::Values(std::vector<int>{32, 16}));