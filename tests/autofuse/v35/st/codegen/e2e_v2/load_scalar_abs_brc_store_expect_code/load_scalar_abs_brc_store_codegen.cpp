...
class LoadScalarAbsBrcStoreTest : public testing::Test {
};

TEST_F(LoadScalarAbsBrcStoreTest, LoadScalarAbsBrcStoreCodegen) {

  bool gen_success= true;
  ge::AscGraph test_graph("load_scalar_abs_brc_store");
  std::string tilig_stub = R"(
#define REGISTER_TILING_DEFAULT(tiling)
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
)";
  LoadScalarAbsBrcStore_BeforeAutofuse(test_graph);
  LoadScalarAbsBrcStore_AfterAutofuse(test_graph);

  std::vector<ge::AscGraph> test_impl_graphs = {ge::AscGraph("load_scalar_abs_brc_store_general_0_nil_0_nil")};
  test_impl_graphs[0].CopyFrom(test_graph);

  std::vector<std::string> parts = splitString(KERNEL_SRC_LIST, ':');
  std::string kernel_src_file_name = parts[0];      // load_scalar_abs_brc_store_kernel.cpp
  std::string tiling_src_file_name = parts[1];      // load_scalar_abs_brc_store_tiling.cpp
  ...
  try {
    ...
    fused_schedule_result.fused_graph_name = ge::AscendString("load_scalar_abs_brc_store");
