...
#include "e2e_load_nan_out_for_store.h"
...
class LoadNanOutForStoreTest : public testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(LoadNanOutForStoreTest, LoadNanOutForStoreCodegen) {
  bool gen_success = true;
  ge::AscGraph test_graph("load_nan_out_for_store");
...
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
  ...
  std::vector<ge::AscGraph> test_impl_graphs = {ge::AscGraph("load_nan_out_for_store_general_0_nil_0_nil")};
  test_impl_graphs[0].CopyFrom(test_graph);
  LoadNanOutForStore_AfterGetApiInfo(test_impl_graphs[0]);
  ...
  try {
    ...
    fused_schedule_result.fused_graph_name = ge::AscendString("load_nan_out_for_store");
