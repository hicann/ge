...
TEST_F(TestGenModelInfo, ModelInfoParserForFlashSoftmaxApiTiling) {
  std::string json_info;
  std::vector<ge::AscGraph> graphs;
  TilingModelInfo model_info_list;
  ge::AscGraph graph("graph");
  ASSERT_EQ(ge::ascir::cg::BuildFlashSoftmaxAscendGraph(graph), ge::SUCCESS);
  graphs.emplace_back(graph);
  const auto &tiling_data_name = graph.GetName() + "TilingData";
  EXPECT_EQ(GenerateModelInfo(graphs, model_info_list, {{kTilingDataTypeName, tiling_data_name}}), ge::SUCCESS);
  ASSERT_EQ(model_info_list.size(), 1);
  auto softmax_api_tiling_map = ApiTilingMgr::Instance().GetApiTilingFunc(0U);
  ASSERT_EQ(softmax_api_tiling_map.size(), 1UL);
  auto code_impl = softmax_api_tiling_map["GetsoftmaxTiling(tiling_data)"];
  std::cout << "code_impl:" << code_impl << std::endl;
  EXPECT_TRUE(!code_impl.empty());
  EXPECT_NE(code_impl.find("void GetsoftmaxTiling(graphTilingData &tiling_data)"), std::string::npos);
}
