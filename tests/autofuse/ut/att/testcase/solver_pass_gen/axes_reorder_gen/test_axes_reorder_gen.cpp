...
#include "gen_model_info/api_perf_register/v2/perf_param_v2.h"
using namespace att;
...
TEST_F(TestAxesReorderSolverGen, TEST_ARRANGE)
{
  ...
  std::map<Expr, uint32_t, ExprCmp> arg_align_map;
...
TEST_F(TestAxesReorderSolverGen, InitiateArgs_MixedValidInvalidArgs) {
    ...
    solver_.input_align_ = {{valid, 4}, {invalid, 1}}; // One needs alignment
    solver_.mc_args_ = {};
    ...
    std::string expected = 
        "    Variable valid;\n"
        "    valid.value = (tiling_data.get_valid() + 4 - 1) / 4 * 4;\n"
        "    Variable invalid;\n"
...
TEST_F(TestAxesReorderSolverGen, GenInputInfoTest) {
    ...
    arg_align_map_[var1] = 64;
    arg_align_map_[var2] = 32;
    ...
    EXPECT_TRUE(result.find("local_var1.align = 64") != std::string::npos);
    EXPECT_TRUE(result.find("local_var2.align = 32") != std::string::npos);
...
TEST_F(TestAxesReorderSolverGen, test_contain_heavy_op_A5) {
  // Case 3: Normal case with UB and variables
  AxesReorderSolverGen solver("case_test", "TilingData");
  std::vector<Expr> mc_args_;
  std::vector<Expr> local_buffer_tiling_vars_;
...
TEST_F(TestAxesReorderSolverGen, GenPgo_SolverwithClassImpl) {
  ...
  std::string genPgoRes = solver.GenPgoSolverClassImpl(className);
  std::string genRunRes = solver.GenRunImpl(className);
  solver.GenPGOSolverFuncInvoke();
  solver.enable_autofuse_pgo_ = true;
  solver.GenSolverClassImpl();
  solver.GenSolverFuncImpl();
  EXPECT_TRUE(genPgoRes.find(className) != std::string::npos);
  EXPECT_TRUE(genRunRes.find(className) != std::string::npos);
}
