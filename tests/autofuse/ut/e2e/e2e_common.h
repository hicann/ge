...
std::string OptilingStub() {
  std::stringstream ss;
    ...
    ss << "bool PGOSearchTilingKey(std::vector<AutofuseTilingDataPerf>& tiling_data_list, "
       ...
       << "void *input, void *output, void* stream, uint32_t workspaceSize, double& out_best_perf, "
       << "std::vector<uint32_t*> block_dim_vec={}) {return true;}" << std::endl;
	ss << "bool PGOByCoreNumSearchTilingKey(std::vector<AutofuseTilingData>& tiling_data_list, AutofuseTilingData* tiling_data, "
	   << "uint32_t max_block_dim=48) {return true;}" << std::endl;
	ss << "}" << std::endl;
