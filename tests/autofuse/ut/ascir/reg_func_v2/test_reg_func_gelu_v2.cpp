...
namespace ge {
namespace ascir {
extern std::vector<std::unique_ptr<ge::TmpBufDesc>> CalcGeluTmpSizeV2(const ge::AscNode &node);
...
TEST_F(CalcGeluTmpSizeV2Test, CalcGeluTmpSizeV2_ShouldReturnCorrectSize_WhenNodelsValid)
{
    ...
    std::vector<std::unique_ptr<ge::TmpBufDesc>> result = CalcGeluTmpSizeV2(*node);
    ASSERT_EQ(result.size(), 1);
    ASSERT_EQ(result[0]->size, ge::Symbol(768));
