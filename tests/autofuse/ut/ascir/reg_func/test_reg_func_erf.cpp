...
namespace ge{
namespace ascir{
extern std::vector<std::unique_ptr<ge::TmpBufDesc>> CalcErfTmpSize(const ge::AscNode &node);
...
class CalcErfTmpSizeTest:public::testing::Test{
protected:
    void SetUp() override{}
...
/**
 * @tc.name:CalcErfTmpSize_ShouldReturnCorrectSize_WhenNodelsValid
 * @tc.number: CalcErfTmpSize_Test_001
 * @tc.desc: Test when node is valid then CalcErfTmpSize returns correct size
 */
TEST_F(CalcErfTmpSizeTest, CalcErfTmpSize_ShouldReturnCorrectSize_WhenNodelsValid)
{
    ...
    *y.y.strides = {s1, Symbol(1)};
    ...
    std::vector<std::unique_ptr<ge::TmpBufDesc>> result = CalcErfTmpSize(*node);
    ASSERT_EQ(result.size(), 1);
    ASSERT_EQ(result[0]->size, Symbol(12)*s0*s1);
