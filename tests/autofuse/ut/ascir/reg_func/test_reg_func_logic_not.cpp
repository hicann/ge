...
namespace ge{
namespace ascir{
extern std::vector<std::unique_ptr<ge::TmpBufDesc>> CalcLogicalNotTmpSize(const ge::AscNode &node);

using namespace testing;

class CalcLogicalNotTmpSizeTest:public::testing::Test{
protected:
    void SetUp() override{}
...
/**
 * @tc.name:CalcLogicalNotTmpSize_ShouldReturnCorrectSize_WhenNodelsValid
 * @tc.number: CalcDivTmpSize_Test_001
 * @tc.desc: Test when node is valid then CalcLogicalNotTmpSize returns correct size
 */
TEST_F(CalcLogicalNotTmpSizeTest, CalcLogicalNotTmpSize_ShouldReturnCorrectSize_WhenNodelsValid)
{
    ...
    *load1.y.vectorized_axis = {z0.id, zo_s_0.id};
    ...
    logical_not.x = load1.y;
    logical_not.attr.sched.axis = {z0.id, zo_s_0.id};
    logical_not.y.dtype = ge::DT_FLOAT;
    *logical_not.y.axis = {z0.id, zo_s_0.id};
    *logical_not.y.repeats = {s0, s1};
    *logical_not.y.strides = {s1, Symbol(1)};

    store.x = logical_not.y;
    ...
    *y.y.strides = {s1, Symbol(1)};
    ...
    std::shared_ptr<ge::AscNode> node = graph.FindNode("logical_not");
    node->inputs[0].attr.vectorized_strides = {s1, Symbol(1)};
    std::vector<std::unique_ptr<ge::TmpBufDesc>> result = CalcLogicalNotTmpSize(*node);
    ASSERT_EQ(result.size(), 1);
    ASSERT_EQ(result[0]->size, sym::Min(ge::Symbol(2) * s1 * s0 + ge::Symbol(32), ge::Symbol(65312)));
