...
namespace ge{
namespace ascir{
extern std::vector<std::unique_ptr<ge::TmpBufDesc>> CalcLogicalAndTmpSize(const ge::AscNode &node);

using namespace testing;

class CalcLogicalAndTmpSizeTest:public::testing::Test{
protected:
    void SetUp() override{}
...
/**
 * @tc.name:CalcLogicalAndTmpSize_ShouldReturnCorrectSize_WhenNodelsValid
 * @tc.number: CalcDivTmpSize_Test_001
 * @tc.desc: Test when node is valid then CalcLogicalAndTmpSize returns correct size
 */
TEST_F(CalcLogicalAndTmpSizeTest, CalcLogicalAndTmpSize_ShouldReturnCorrectSize_WhenNodelsValid)
{
    ...
    std::vector<std::unique_ptr<ge::TmpBufDesc>> result = CalcLogicalAndTmpSize(*node);
