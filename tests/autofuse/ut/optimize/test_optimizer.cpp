...
TEST_F(TestOptimizer, RemoveSpecificAxes_ConcatScenario) {
  // 测试 RemoveSpecificAxes 在 concat 场景下正确删除轴并重新编号
  // 场景：图中有多个轴，删除其中一个轴后，剩余轴的ID应从0开始重新编号
  ge::AscGraph graph("concat_remove_axes_graph");
  ...
  // 创建三个轴：z0, z1, z2
  // 测试目标：删除 z2，验证 z0 和 z1 被重新编号为 0 和 1
  auto z0 = graph.CreateAxis("z0", s0);
  auto z1 = graph.CreateAxis("z1", s1);
  auto z2 = graph.CreateAxis("z2", s2);
  ...
  *load_op1.y.strides = {s1 * s2, s2, ge::ops::One};
  ...
  *load_op2.y.strides = {s1 * s2, s2, ge::ops::One};
  ...
  *concat_op.y.repeats = {s0, s1, s2};
  *concat_op.y.strides = {s1 * s2, s2, ge::ops::One};
  ...
  *store_op.y.strides = {s1 * s2, s2, ge::ops::One};
  ...
  // 标记需要删除的轴：z2
  std::unordered_set<int64_t> axis_ids_to_remove = {z2.id};

  // 调用 RemoveSpecificAxes 删除 z2 轴
  Status res = optimize::ScheduleUtils::RemoveSpecificAxes(graph, axis_ids_to_remove);
  EXPECT_EQ(res, ge::SUCCESS);

  // 验证：z2 轴已被删除，z0 和 z1 轴保留并重新编号
  // z0: id=0 (原来是 z0.id)
  // z1: id=1 (原来是 z1.id)
  auto graph_attr = ge::AscGraphUtils::GetComputeGraph(graph)->GetOrCreateAttrsGroup<ge::AscGraphAttr>();
  ASSERT_NE(graph_attr, nullptr);
  EXPECT_EQ(graph_attr->axis.size(), 2UL);

  // 验证第一个轴 (原 z0)
  EXPECT_EQ(graph_attr->axis[0]->id, 0);
  EXPECT_EQ(graph_attr->axis[0]->name, "z0");

  // 验证第二个轴 (原 z1)
  EXPECT_EQ(graph_attr->axis[1]->id, 1);
  EXPECT_EQ(graph_attr->axis[1]->name, "z1");

  // 验证 Load, Concat, Store 节点的属性已正确更新
  ...
  // 验证 sched.axis 只包含 {0, 1} (不包含 z2.id)
  for (auto *node : {load1_node.get(), load2_node.get(), concat_node.get(), store_node.get()}) {
    EXPECT_EQ(node->attr.sched.axis.size(), 2UL);
    EXPECT_NE(std::find(node->attr.sched.axis.begin(), node->attr.sched.axis.end(), 0),
              node->attr.sched.axis.end());
    EXPECT_NE(std::find(node->attr.sched.axis.begin(), node->attr.sched.axis.end(), 1),
              node->attr.sched.axis.end());
    EXPECT_EQ(std::find(node->attr.sched.axis.begin(), node->attr.sched.axis.end(), z2.id),
              node->attr.sched.axis.end());

    // 验证输出张量的 axis 已更新
    for (const auto &out : node->outputs()) {
      EXPECT_EQ(out->attr.axis.size(), 2UL);
      EXPECT_EQ(out->attr.axis[0], 0);
      EXPECT_EQ(out->attr.axis[1], 1);
      EXPECT_EQ(out->attr.strides.size(), 2UL);
      EXPECT_EQ(out->attr.repeats.size(), 2UL);
    }
  }
}