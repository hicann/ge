...
TEST_F(LoopNodeLoweringUT, MatMulLowering1) {
  ...
  EXPECT_EQ(kernel.Readable(),
            ...
            "enable_hf32=0, has_bias=0, has_offset_w=0)\n");
}
...
TEST_F(LoopNodeLoweringUT, LoweringSigmoidGrad) {
  ...
  EXPECT_EQ(kernel.Readable(),
            "tmp0 = ops.Load(\"y:0\")\n"
            "tmp1 = ops.Load(\"y:0\")\n"
            "tmp2 = ops.Mul(tmp1, tmp0)\n"
            "tmp3 = ops.Load(\"y:0\")\n"
            "tmp4 = ops.Sub(tmp3, tmp2)\n"
            "tmp5 = ops.Load(\"dy:0\")\n"
...
TEST_F_LOWER_INST3(ClipByValue, ClipByValue)
