...
static Status AttrInfoPrintLog(ComputeGraphPtr &cg) {
  // 本地执行node打印
  for (const auto &node : cg->GetAllNodes()) {
    ...
    for (const auto &node : fused_graph->GetAllNodes()) {
      ...
      if (node->GetType() != "Output") {
        ...
        cout << "**************" << AutofuseUtils::VectorToStr(attr->axis).c_str()
             << AutofuseUtils::VectorToStr(attr->repeats).c_str() << AutofuseUtils::VectorToStr(attr->strides).c_str()
             << TypeUtils::DataTypeToSerialString(attr->dtype).c_str() << endl;
      }
    }
  }
  return SUCCESS;
...
TEST_F(LoweringAndCanfuseUT, EleAndEleLoweringCanfuseWithSameNodeCount) {
  [this]() {
    auto data0 = es_graph_->CreateInput(0, "data0", nullptr);
    data0.SetSymbolShape({"32", "64", "1", "128"});
    auto data1 = es_graph_->CreateInput(1, "data1", nullptr);
    data1.SetSymbolShape({"32", "1", "1", "128"});
    auto data2 = es_graph_->CreateInput(2, "data2", nullptr);
    data2.SetSymbolShape({"32", "64", "64", "128"});
    auto abs2 = es::Abs(data1);
    abs2.SetSymbolShape({"32", "1", "1", "128"});
    auto broadcast = es::BroadcastTo(abs2, data0);
    broadcast.SetSymbolShape({"32", "64", "1", "128"});
    auto abs = es::Abs(data0);
    abs.SetSymbolShape({"32", "64", "1", "128"});
    auto exp = es::Exp(abs);
    exp.SetSymbolShape({"32", "64", "1", "128"});
    auto relu = es::Relu(abs);
    relu.SetSymbolShape({"32", "64", "1", "128"});
    auto add1 = es::Add(relu, broadcast);
    add1.SetSymbolShape({"32", "64", "1", "128"});
    auto broadcast1 = es::BroadcastTo(add1, data2);
    broadcast1.SetSymbolShape({"32", "64", "1", "128"});
    auto add2 = es::Add(broadcast1, data2);
    add2.SetSymbolShape({"32", "64", "64", "128"});
  ...
  EXPECT_EQ(asc_adapt::RemoveInvalidAxisOnAscGraph(cg), SUCCESS);
  FusionStrategySolver fusion_strategy_solver;
  FusionDeciderRegistry::Instance().Register(std::unique_ptr<FusionDecider>(new AscBackendFusionDecider()));
  EXPECT_EQ(fusion_strategy_solver.Fuse(cg), SUCCESS);
  ASSERT_EQ(lowerer.Lifting(cg), GRAPH_SUCCESS);
...
#ifdef LOCAL_LOG
  EXPECT_EQ(AttrInfoPrintLog(cg), SUCCESS);
#endif
}
