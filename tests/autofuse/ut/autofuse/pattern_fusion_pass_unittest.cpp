...
#include "pattern_fusion/slices_to_split_pass.h"
...
TEST_F(PatternFusionPassTest, SlicesToSplitBasic) {
  // 测试基本场景：3个连续的slice可以替换成1个split
  // 图结构：
  //   data (100, 20, 30)
  //     |
  //     +-- slice1([0:30, :, :]) --> relu1
  ...
  //     +-- slice3([60:100, :, :]) --> relu3
  //
  // 预期：替换成 split(axis=0, size_splits=[30, 30, 40])

  auto data = OP_CFG("Data").TensorDesc(FORMAT_ND, DT_FLOAT, {100, 20, 30}).InCnt(0).OutCnt(1).Build("data");

  auto slice1 = OP_CFG("SliceD").TensorDesc(FORMAT_ND, DT_FLOAT, {30, 20, 30}).InCnt(1).OutCnt(1).Build("slice1");
  auto slice2 = OP_CFG("SliceD").TensorDesc(FORMAT_ND, DT_FLOAT, {30, 20, 30}).InCnt(1).OutCnt(1).Build("slice2");
  auto slice3 = OP_CFG("SliceD").TensorDesc(FORMAT_ND, DT_FLOAT, {40, 20, 30}).InCnt(1).OutCnt(1).Build("slice3");

  auto relu1 = OP_CFG("ReLU").TensorDesc(FORMAT_ND, DT_FLOAT, {30, 20, 30}).InCnt(1).OutCnt(1).Build("relu1");
  auto relu2 = OP_CFG("ReLU").TensorDesc(FORMAT_ND, DT_FLOAT, {30, 20, 30}).InCnt(1).OutCnt(1).Build("relu2");
  auto relu3 = OP_CFG("ReLU").TensorDesc(FORMAT_ND, DT_FLOAT, {40, 20, 30}).InCnt(1).OutCnt(1).Build("relu3");

  DEF_GRAPH(test_graph) {
    CHAIN(NODE(data)->DATA_EDGE(0, 0)->NODE(slice1)->NODE(relu1)->NODE("output_0", NETOUTPUT));
    CHAIN(NODE(data)->DATA_EDGE(0, 0)->NODE(slice2)->NODE(relu2)->NODE("output_1", NETOUTPUT));
    CHAIN(NODE(data)->DATA_EDGE(0, 0)->NODE(slice3)->NODE(relu3)->NODE("output_2", NETOUTPUT));
  };

  auto graph = ToComputeGraph(test_graph);

  // 设置 slice 属性
  // slice1: offsets=[0,0,0], sizes=[30,20,30]
  std::vector<int64_t> offsets1 = {0, 0, 0};
  std::vector<int64_t> sizes1 = {30, 20, 30};
  AttrUtils::SetListInt(graph->FindNode("slice1")->GetOpDesc(), "offsets", offsets1);
  AttrUtils::SetListInt(graph->FindNode("slice1")->GetOpDesc(), "size", sizes1);

  // slice2: offsets=[30,0,0], sizes=[30,20,30]
  std::vector<int64_t> offsets2 = {30, 0, 0};
  std::vector<int64_t> sizes2 = {30, 20, 30};
  AttrUtils::SetListInt(graph->FindNode("slice2")->GetOpDesc(), "offsets", offsets2);
  AttrUtils::SetListInt(graph->FindNode("slice2")->GetOpDesc(), "size", sizes2);

  // slice3: offsets=[60,0,0], sizes=[-1,20,30] (-1表示取到维度末尾，即100-60=40)
  std::vector<int64_t> offsets3 = {60, 0, 0};
  std::vector<int64_t> sizes3 = {-1, 20, 30};
  AttrUtils::SetListInt(graph->FindNode("slice3")->GetOpDesc(), "offsets", offsets3);
  AttrUtils::SetListInt(graph->FindNode("slice3")->GetOpDesc(), "size", sizes3);

  // 运行 Pass
  EXPECT_EQ(SlicesToSplitPass().Run(graph), GRAPH_SUCCESS);

  // 验证结果：应该有1个split节点，3个slice节点被删除
  int split_count = 0;
  int slice_count = 0;
  for (const auto &node: graph->GetAllNodes()) {
    if (node->GetType() == AF_SPLITV) {
      split_count++;
    }
    if (node->GetType() == "SliceD" && node->GetName().find("slice") != std::string::npos) {
      slice_count++;
    }
  }

  EXPECT_EQ(split_count, 1);
  EXPECT_EQ(slice_count, 0);

  // 验证 relu 节点的输入现在是 split 的输出
  auto relu1_node = graph->FindNode("relu1");
  auto relu2_node = graph->FindNode("relu2");
  auto relu3_node = graph->FindNode("relu3");

  ASSERT_NE(relu1_node, nullptr);
  ASSERT_NE(relu2_node, nullptr);
  ASSERT_NE(relu3_node, nullptr);

  auto relu1_input = NodeUtils::GetInDataNodeByIndex(*relu1_node, 0);
  auto relu2_input = NodeUtils::GetInDataNodeByIndex(*relu2_node, 0);
  auto relu3_input = NodeUtils::GetInDataNodeByIndex(*relu3_node, 0);

  EXPECT_EQ(relu1_input->GetType(), AF_SPLITV);
  EXPECT_EQ(relu2_input->GetType(), AF_SPLITV);
  EXPECT_EQ(relu3_input->GetType(), AF_SPLITV);

  // 验证三个 relu 都来自同一个 split 节点
  EXPECT_EQ(relu1_input->GetName(), relu2_input->GetName());
  EXPECT_EQ(relu2_input->GetName(), relu3_input->GetName());
}

TEST_F(PatternFusionPassTest, SlicesToSplitDifferentDim) {
  // 测试不同维度的slice不能替换
  // slice1在维度0上切片，slice2在维度1上切片，不能合并
  auto data = OP_CFG("Data").TensorDesc(FORMAT_ND, DT_FLOAT, {100, 20, 30}).InCnt(0).OutCnt(1).Build("data");

  auto slice1 = OP_CFG("SliceD").TensorDesc(FORMAT_ND, DT_FLOAT, {50, 20, 30}).InCnt(1).OutCnt(1).Build("slice1");
  auto slice2 = OP_CFG("SliceD").TensorDesc(FORMAT_ND, DT_FLOAT, {100, 10, 30}).InCnt(1).OutCnt(1).Build("slice2");

  auto relu1 = OP_CFG("ReLU").TensorDesc(FORMAT_ND, DT_FLOAT, {50, 20, 30}).InCnt(1).OutCnt(1).Build("relu1");
  auto relu2 = OP_CFG("ReLU").TensorDesc(FORMAT_ND, DT_FLOAT, {100, 10, 30}).InCnt(1).OutCnt(1).Build("relu2");
  ...
  // slice1: offsets=[0,0,0], sizes=[50,20,30] (在维度0上切片，取前一半)
  std::vector<int64_t> offsets1 = {0, 0, 0};
  std::vector<int64_t> sizes1 = {50, 20, 30};
  AttrUtils::SetListInt(graph->FindNode("slice1")->GetOpDesc(), "offsets", offsets1);
  AttrUtils::SetListInt(graph->FindNode("slice1")->GetOpDesc(), "size", sizes1);

  // slice2: offsets=[0,0,0], sizes=[100,10,30] (在维度1上切片，取前一半)
  std::vector<int64_t> offsets2 = {0, 0, 0};
  std::vector<int64_t> sizes2 = {100, 10, 30};
  AttrUtils::SetListInt(graph->FindNode("slice2")->GetOpDesc(), "offsets", offsets2);
  AttrUtils::SetListInt(graph->FindNode("slice2")->GetOpDesc(), "size", sizes2);
...
TEST_F(PatternFusionPassTest, SlicesToSplitWithGap) {
  // 测试有间隙的slice不能替换
  auto data = OP_CFG("Data").TensorDesc(FORMAT_ND, DT_FLOAT, {100, 20}).InCnt(0).OutCnt(1).Build("data");

  auto slice1 = OP_CFG("SliceD").TensorDesc(FORMAT_ND, DT_FLOAT, {30, 20}).InCnt(1).OutCnt(1).Build("slice1");
  auto slice2 = OP_CFG("SliceD").TensorDesc(FORMAT_ND, DT_FLOAT, {30, 20}).InCnt(1).OutCnt(1).Build("slice2");

  auto relu1 = OP_CFG("ReLU").TensorDesc(FORMAT_ND, DT_FLOAT, {30, 20}).InCnt(1).OutCnt(1).Build("relu1");
  auto relu2 = OP_CFG("ReLU").TensorDesc(FORMAT_ND, DT_FLOAT, {30, 20}).InCnt(1).OutCnt(1).Build("relu2");
  ...
  // slice1: offsets=[0,0], sizes=[30,20]
  std::vector<int64_t> offsets1 = {0, 0};
  std::vector<int64_t> sizes1 = {30, 20};
  AttrUtils::SetListInt(graph->FindNode("slice1")->GetOpDesc(), "offsets", offsets1);
  AttrUtils::SetListInt(graph->FindNode("slice1")->GetOpDesc(), "size", sizes1);

  // slice2: offsets=[50,0], sizes=[30,20] (有间隙，跳过了30:50)
  std::vector<int64_t> offsets2 = {50, 0};
  std::vector<int64_t> sizes2 = {30, 20};
  AttrUtils::SetListInt(graph->FindNode("slice2")->GetOpDesc(), "offsets", offsets2);
  AttrUtils::SetListInt(graph->FindNode("slice2")->GetOpDesc(), "size", sizes2);
