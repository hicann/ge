...
std::shared_ptr<AscGraph> CreatPartBrcBackwardAscGraphWithMulInputs7(ge::AscGraph &graph) {
  ...
  *x_out.y.axis = {a.id, b.id, c.id, d.id, e.id};
  *x_out.y.repeats = {A, B, C, D, E};
  *x_out.y.strides = {B * C * D * E, C * D * E, D * E, E,
                      ONE};  // 刻意内轴设置为1，构造无效轴不删除场景叠加broadcast后移内轴刷新正确
...
std::shared_ptr<AscGraph> CreatAscGraphWithScalarMulRefToAdd3(ge::AscGraph &graph) {
  auto ONE = Symbol(1);
  auto ZERO = Symbol(0);
  const Expression A = graph.CreateSizeVar("A");
  const Expression B = graph.CreateSizeVar("B");
...
std::shared_ptr<AscGraph> CreatAscGraphWithScalarMulRefToAdd4(ge::AscGraph &graph) {
  auto ONE = Symbol(1);
  auto ZERO = Symbol(0);
  const Expression A = graph.CreateSizeVar("A");
  const Expression B = graph.CreateSizeVar("B");
...
TEST_F(AscBackendPostProcessorTest, AdapterAndPass_OK) {
  ...
  for (auto node : af::AscGraphUtils::GetComputeGraph(*(attr1->GetAscGraph()))->GetDirectNode()) {
    ...
    printf(
        "AscAdapterTest_Ok To find node to change precision, current node(%s), type:%s, speats %s, dtype:%s in graph "
        "%s.\n",
        node->GetName().c_str(), node->GetType().c_str(), AutofuseUtils::VectorToStr(attr->repeats).c_str(),
        TypeUtils::DataTypeToSerialString(attr->dtype).c_str(),
        af::AscGraphUtils::GetComputeGraph(*(attr1->GetAscGraph()))->GetName().c_str());
  }
  AscBackendPostProcessor post_processor;
...
TEST_F(AscBackendPostProcessorTest, Adaption_EmptyRepeatsAfterLoad_OK) {
  ...
  for (auto node : af::AscGraphUtils::GetComputeGraph(*(attr1->GetAscGraph()))->GetDirectNode()) {
    ...
    // attr->dtype = DT_FLOAT16;
    printf(
        "Adaption_EmptyRepeats_OK To find node to fallback load, current node(%s), type:%s, repeats %s, dtype:%s in "
        "graph %s.\n",
        node->GetName().c_str(), node->GetType().c_str(), AutofuseUtils::VectorToStr(attr->repeats).c_str(),
...
TEST_F(AscBackendPostProcessorTest, Split_Pass_OK) {
  ...
  for (auto node : af::AscGraphUtils::GetComputeGraph(*(attr1->GetAscGraph()))->GetDirectNode()) {
    ...
    // attr->dtype = DT_FLOAT16;
    printf(
        "AscAdapterTest_Ok To find node to change precision, current node(%s), type:%s, repeats %s, dtype:%s in graph "
        "%s.\n",
        node->GetName().c_str(), node->GetType().c_str(), AutofuseUtils::VectorToStr(attr->repeats).c_str(),
        TypeUtils::DataTypeToSerialString(attr->dtype).c_str(),
        af::AscGraphUtils::GetComputeGraph(*(attr1->GetAscGraph()))->GetName().c_str());
...
TEST_F(AscBackendPostProcessorTest, Adaption_DiffRepeats_OK) {
  ...
  for (auto node : af::AscGraphUtils::GetComputeGraph(*(attr1->GetAscGraph()))->GetDirectNode()) {
    ...
    // attr->dtype = DT_FLOAT16;
    printf(
        "AscAdapterTest_Ok To find node to fallback load, current node(%s), type:%s, repeats %s, dtype:%s in graph "
        "%s.\n",
        node->GetName().c_str(), node->GetType().c_str(), AutofuseUtils::VectorToStr(attr->repeats).c_str(),
        TypeUtils::DataTypeToSerialString(attr->dtype).c_str(),
        af::AscGraphUtils::GetComputeGraph(*(attr1->GetAscGraph()))->GetName().c_str());
...
TEST_F(AscBackendPostProcessorTest, Adaption_DiffRepeatsWithScalar_OK) {
  ...
  for (auto node : af::AscGraphUtils::GetComputeGraph(*(attr1->GetAscGraph()))->GetDirectNode()) {
    ...
    ASSERT_NE(attr, nullptr);
    if (node->GetType() == "Broadcast") {
      ASSERT_EQ(output_tensor_desc->GetDataType(),
                DT_FLOAT);  // dtype类型变换是因为现在scalar也插了broadcast后不做broadcast后移，
      broadcast_cnt++;
      continue;
    }
    if (node->GetType() == "Scalar") {
...
void CheckKeepOriginalAxisIdsReshapeResult(const ReshapeAxisPaddingCase &test_case) {
  const auto graph_attr =
      AscGraphUtils::GetComputeGraph(*(test_case.attr->GetAscGraph()))->GetAttrsGroup<AscGraphAttr>();
  ASSERT_NE(graph_attr, nullptr);
  ASSERT_EQ(graph_attr->axis.size(), 4U);
  EXPECT_EQ(graph_attr->axis[0]->id, 0);
  EXPECT_EQ(graph_attr->axis[1]->id, 3);
  EXPECT_EQ(graph_attr->axis[2]->id, 1);
  EXPECT_EQ(graph_attr->axis[3]->id, 2);

  auto store_node = test_case.attr->GetAscGraph()->FindNode("store");
  ASSERT_NE(store_node, nullptr);
  AscTensorAttr *store_attr = nullptr;
  ASSERT_EQ(asc_adapt::GetOutputTensorAttr(store_node, store_attr), SUCCESS);
  ASSERT_NE(store_attr, nullptr);
  EXPECT_EQ(store_attr->axis, std::vector<int64_t>({0, 3, 1, 2}));
  ASSERT_EQ(store_attr->repeats.size(), 4U);
  EXPECT_TRUE(BackendUtils::IsEqOne(store_attr->repeats[1]));
  ASSERT_EQ(store_attr->strides.size(), 4U);
  EXPECT_TRUE(BackendUtils::IsEqZero(store_attr->strides[1]));

  const auto &reshape_axis_changes = test_case.attr->GetReshapeAxisChanges();
  ASSERT_EQ(reshape_axis_changes.size(), 2U);
  EXPECT_EQ(reshape_axis_changes[0].before_axis, std::vector<int64_t>({0, 1, 2}));
  EXPECT_EQ(reshape_axis_changes[0].after_axis, std::vector<int64_t>({0, 1}));
  EXPECT_EQ(reshape_axis_changes[1].before_axis, std::vector<int64_t>({0, 1}));
  EXPECT_EQ(reshape_axis_changes[1].after_axis, std::vector<int64_t>({0, 1, 2}));

  const auto &axis_group = GetInterAttrs(test_case.attr).axis_group;
  EXPECT_EQ(axis_group.y_group, std::vector<int64_t>({0, 1, 2, 3}));
  EXPECT_TRUE(axis_group.n_group.empty());
  EXPECT_EQ(axis_group.axes_order, std::vector<size_t>({0U, 3U, 1U, 2U}));
}
...
TEST_F(AscBackendPostProcessorTest, CompleteAttrs_RestoreReshapeAxisKeepOriginalAxisIds) {
  ReshapeAxisPaddingCase test_case;
  ASSERT_NO_FATAL_FAILURE(BuildMultipleReshapeAxisChangesCase(test_case));
  ASSERT_NE(test_case.outer_graph, nullptr);
  GetInterAttrs(test_case.attr).axis_group.y_group = {0, 1, 2};
  GetInterAttrs(test_case.attr).axis_group.axes_order = {0U, 1U, 2U};
  ASSERT_EQ(asc_adapt::PadLeadingUnitAxisByInsertIndexesAndCompleteAttrsKeepOriginalAxisIds(
                *(test_case.attr->GetAscGraph()), test_case.asc_backend, {1U}),
            SUCCESS);
  ASSERT_NO_FATAL_FAILURE(CheckKeepOriginalAxisIdsReshapeResult(test_case));
}

TEST_F(AscBackendPostProcessorTest, CollectInsertIndexesByTargetRepeatsKeepsLaterGapPosition) {
  std::vector<size_t> insert_indexes;
  ASSERT_EQ(asc_adapt::CollectInsertIndexesByTargetRepeats(
                {Symbol(2), Symbol(1024), Symbol(1024)}, {Symbol(2), Symbol(1024), Symbol(1), Symbol(1024), Symbol(1)},
                insert_indexes),
            SUCCESS);
  EXPECT_EQ(insert_indexes, std::vector<size_t>({2U, 4U}));
}
...
TEST_F(AscBackendPostProcessorTest, ImprovePrecision_BlackList2) {
  ...
  EXPECT_EQ(AutoFuseConfig::MutableConfig().GetMutableFusionStrategySolver().improve_precision_blacklist.find("Abs") !=
                AutoFuseConfig::MutableConfig().GetMutableFusionStrategySolver().improve_precision_blacklist.end(),
            true);
...
TEST_F(AscBackendPostProcessorTest, ImprovePrecision_BlackList3) {
  ...
  EXPECT_EQ(AutoFuseConfig::MutableConfig().GetMutableFusionStrategySolver().improve_precision_blacklist.find("Sqrt") !=
            ...
            true);
  EXPECT_EQ(
      AutoFuseConfig::MutableConfig().GetMutableFusionStrategySolver().improve_precision_blacklist.find("Rsqrt") !=
          AutoFuseConfig::MutableConfig().GetMutableFusionStrategySolver().improve_precision_blacklist.end(),
      true);
  auto shape_env_attr = compute_graph->GetOrCreateAttrsGroup<ShapeEnvAttr>();
  ASSERT_NE(shape_env_attr, nullptr);
...
TEST_F(AscBackendPostProcessorTest, Fallback_CreatAscGraphWithLoadToBroadcstAndTransposeToStore) {
  ...
  for (auto node : af::AscGraphUtils::GetComputeGraph(*(attr1->GetAscGraph()))->GetDirectNode()) {
    ...
    ASSERT_NE(attr, nullptr);
    printf(
        "CreatAscGraphWithLoadToBroadcstAndTransposeToStore , current node(%s), type:%s, speats %s, dtype:%s in graph "
        "%s.\n",
        node->GetName().c_str(), node->GetType().c_str(), AutofuseUtils::VectorToStr(attr->repeats).c_str(),
        TypeUtils::DataTypeToSerialString(attr->dtype).c_str(),
        af::AscGraphUtils::GetComputeGraph(*(attr1->GetAscGraph()))->GetName().c_str());
...
TEST_F(AscBackendPostProcessorTest, Fallback_CreatAscGraphWithLoadToBroadcstAndTransposeToStore2) {
  ...
  for (auto node : af::AscGraphUtils::GetComputeGraph(*(attr1->GetAscGraph()))->GetDirectNode()) {
    ...
    ASSERT_NE(attr, nullptr);
    printf(
        "CreatAscGraphWithLoadToBroadcstAndTransposeToStore2, current node(%s), type:%s, speats %s, dtype:%s in graph "
        "%s.\n",
        node->GetName().c_str(), node->GetType().c_str(), AutofuseUtils::VectorToStr(attr->repeats).c_str(),
        TypeUtils::DataTypeToSerialString(attr->dtype).c_str(),
        af::AscGraphUtils::GetComputeGraph(*(attr1->GetAscGraph()))->GetName().c_str());
...
TEST_F(AscBackendPostProcessorTest, Fallback_CreatAscGraphWithLoadToBroadcstAndTransposeToStore3) {
  ...
  for (auto node : af::AscGraphUtils::GetComputeGraph(*(attr1->GetAscGraph()))->GetDirectNode()) {
    ...
    ASSERT_NE(attr, nullptr);
    printf(
        "CreatAscGraphWithLoadToBroadcstAndTransposeToStore3, current node(%s), type:%s, speats %s, dtype:%s in graph "
        "%s.\n",
        node->GetName().c_str(), node->GetType().c_str(), AutofuseUtils::VectorToStr(attr->repeats).c_str(),
        TypeUtils::DataTypeToSerialString(attr->dtype).c_str(),
        af::AscGraphUtils::GetComputeGraph(*(attr1->GetAscGraph()))->GetName().c_str());
...
// Broadcast后移判断Scalar节点后的Brc在其后计算节点不支持Scalar时不进行后移（Broadcast多引用）
TEST_F(AscBackendPostProcessorTest, BroadcastBackward_ScalarMulRefsNOK2) {
  ComputeGraphPtr compute_graph = BuildGraph1("AscBackend");
  EXPECT_EQ(compute_graph->GetAllNodesSize(), 5);
...
TEST_F(AscBackendPostProcessorTest, BroadcastBackward_ScalarMulRefsOK3) {
  ...
  // 注册到AscirRegistry
  registry.RegisterAscIr("Sub", ir_def);

  // Sub多输入场景下，支持Scalar输入可以，多输入公共Brc支持后移
  ComputeGraphPtr compute_graph = BuildGraph1("AscBackend");
  EXPECT_EQ(compute_graph->GetAllNodesSize(), 5);
  ...
  ge::AscGraph add_graph1("add");
  attr1->SetAscGraph(CreatAscGraphWithScalarMulRefToAdd3(add_graph1));
  EXPECT_EQ(asc_adapt::FallbackScalarToBroadcastWithoutCheckType(compute_graph), SUCCESS);
  EXPECT_EQ(asc_adapt::GeFallback(compute_graph), SUCCESS);
  EXPECT_EQ(asc_adapt::CompleteNodeAttrsOnAscGraphForSched(compute_graph), SUCCESS);
  BroadcastBackwardPass broadcast_backward_pass;
  ...
  for (auto node : af::AscGraphUtils::GetComputeGraph(*(attr1->GetAscGraph()))->GetDirectNode()) {
    ...
    if (node->GetName() == "add_4") {
      ...
      ASSERT_EQ(pre_add_node->GetType(), "Broadcast");
      asc_adapt::GetPeerOutNode(node, pre_add_node, 1);
      ASSERT_EQ(pre_add_node->GetType(), "Sub");
    }
    if (node->GetName() == "x_out_5") {
      NodePtr pre_add_node;
      asc_adapt::GetPeerOutNode(node, pre_add_node, 0);
...
TEST_F(AscBackendPostProcessorTest, BroadcastBackward_ScalarMulRefsOK4) {
  ...
  ge::AscGraph add_graph1("add");
  attr1->SetAscGraph(CreatAscGraphWithScalarMulRefToAdd4(add_graph1));
  EXPECT_EQ(asc_adapt::CompleteNodeAttrsOnAscGraphForSched(compute_graph), SUCCESS);
  BroadcastBackwardPass broadcast_backward_pass;
  EXPECT_EQ(broadcast_backward_pass.Run(compute_graph), SUCCESS);
