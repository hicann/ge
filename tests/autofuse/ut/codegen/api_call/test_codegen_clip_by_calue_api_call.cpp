...
#include "elewise/clip_by_value_api_call.h"
...
void ClipByValueFloat_BeforeAutofuse(ge::AscGraph &graph, codegen::Tiler &tiler, ge::DataType data_type) {
  ...
  *load3.y.strides = {One};
  ...
  clipbyvalue.attr.tmp_buffers = {{{ge::Symbol(8192), -1}, MemAttr()}};
...
void ClipByValueFloat_AfterAutofuse(ge::AscGraph &graph, std::vector<ge::AscGraph> &impl_graphs, codegen::Tiler &tiler,
                                    ge::DataType data_type) {
  ClipByValueFloat_BeforeAutofuse(graph, tiler, data_type);
...
TEST(CodegenKernel, ClipByValueApiCall) {
  ge::AscGraph graph("clip_by_value_float");
  std::vector<ge::AscGraph> test_impl_graphs;
  codegen::Tiler tiler;
  codegen::TPipe tpipe("tpipe", tiler);

  ClipByValueFloat_AfterAutofuse(graph, test_impl_graphs, tiler, ge::DT_FLOAT);
  ...
  codegen::ClipByValueApiCall call("ClipByValue");
  ...
  EXPECT_EQ(result, std::string{
      "ClipByValue(global_-1[0], global_-1[0], global_-1[0], global_-1[0], global_-1_actual_size, tmp_buf);\n"
  });
}

TEST(CodegenKernel, ClipByValueWithSecondInputAndThirdInputAreScalar) {
  ...
  *load_op.y.strides = {s1, One};
  ...
  call.Generate(tpipe, vector<ge::AxisId>{}, result);
  EXPECT_EQ(result, std::string{
      "ClipByValue(local_3[0], local_0[0], scalar_1, scalar_2, local_0_actual_size, tmp_buf);\n"
  });
}


TEST(CodegenKernel, ClipByValueWithAllInputAreScalar) {
  ge::AscGraph graph("test_graph");
...
TEST(WhereApiCallTest, ClipByValueWithAllInputAreUbScalar) {
  ge::AscGraph graph("test_graph");
