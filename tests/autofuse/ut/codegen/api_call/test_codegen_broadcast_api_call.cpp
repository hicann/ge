...
TEST(CodegenKernel, BroadcastApiCallUBScalar) {
  ...
  *broadcast_op.y.strides = {s1, One};
  ...
  std::string result;
  call.Generate(tpipe, vector<ge::AxisId>{}, result);
  EXPECT_EQ(result, std::string{"Duplicate(local_1[0], local_0_ub_scalar, local_1_actual_size, tmp_buf);\n"});
}
  ...
  *broadcast_op.y.strides = {s1, One};
  ...
  EXPECT_EQ(
      ...
      std::string{"event_t local_0_event_id = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::MTE2_S));\n"
                  ...
                  "Duplicate(local_1[0], local_0.GetValue(0), local_1_actual_size, tmp_buf);\n"});
}
...
TEST(CodegenKernel, BroadcastApiCall_BAB) {
  ...
  *broadcast_op.y.strides = {s1 * s2, s2, One};
  ...
  std::string result;
  call.Generate(tpipe, vector<ge::AxisId>{}, result);
  EXPECT_EQ(result, "Broadcast(local_1[0], local_0[0], 1, 1, t->s1, 1, 1, t->s0, t->s1, t->s1, tmp_buf, 1);\n");
}
...
TEST(CodegenKernel, BroadcastApiCall_ABAB) {
  ...
  *broadcast_op.y.strides = {(s1 * s2 * s3 * s4), (s2 * s3 * s4), (s3 * s4), s4, One};
  ...
  std::string result;
  call.Generate(tpipe, vector<ge::AxisId>{}, result);
  EXPECT_EQ(result, "Broadcast(local_1[0], local_0[0], t->s1, 1, t->s3, 1, t->s1, t->s2, t->s3, t->s4, tmp_buf, t->s1);\n");
}
