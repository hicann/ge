...
TEST(CodegenKernel, MicroApiCall_Load_Abs_Store) {
  ge::AscGraph graph("test_abs_graph");
  ...
  *abs.y.strides = {s1 * s2 * s3 * Four, s2 * s3 * Three, s3 * Two, One};
  ...
  *store_op.y.strides = {s1 * s2 * s3 * Four, s2 * s3 * Three, s3 * Two, One};
  ...
  abs_node->attr.api.compute_type = ge::ComputeType::kComputeLoad;
  abs_node->attr.api.type = ge::ApiType::kAPITypeCompute;
  abs_node->attr.api.unit = ge::ComputeUnit::kUnitMTE2;
  abs_node->attr.sched.loop_axis = z0.id;
  abs_node->outputs[0].attr.vectorized_axis = {z1.id, z2.id, z3.id};
  abs_node->outputs[0].attr.vectorized_strides = {ge::Symbol(8), ge::Symbol(2), One};
  abs_node->outputs[0].attr.dtype = ge::DT_FLOAT;
  abs_node->outputs[0].attr.mem.position = ge::Position::kPositionVecIn;
  abs_node->outputs[0].attr.mem.tensor_id = 1;
  abs_node->outputs[0].attr.mem.position = ge::Position::kPositionVecIn;
  abs_node->outputs[0].attr.mem.alloc_type = ge::AllocType::kAllocTypeQueue;
  abs_node->outputs[0].attr.que.id = 2;
  abs_node->outputs[0].attr.opt.merge_scope = ge::kIdNone;
  ...
  codegen::MicroApiCall call_0("Abs");
  EXPECT_EQ(call_0.Init(abs_node), 0);
