...
static const ErrorInfoConfig ERROR_MAP[] = {
    ...
    {"EK0201",
     "Failed to allocate host memory for Profiling: %s.",
...
*/
static char *FormatString(char *tmplArgList[MAX_ARG_NUMBER], char *tmplErrorMsg, char *args[], char *argValues[],
                          int32_t argsNum) {  // 当模板中errormsg %s个数大于3个时，提前处理
...
static int32_t ComputeAllocate(ErrorItem *firstItem) {
  g_errorThread->errorMsgLen += (ERRCODE_LENGTH + sizeof(": Internal Error!\r\n") + strlen(firstItem->errorMessage) +
...
static int32_t FormatTraceBefore(ErrorItem *firstItem) {
  ...
  if (IsInnerErrorCode(firstItem->errorId)) {
    n = sprintf_s(g_errorThread->errorMsg, g_errorThread->errorMsgLen, "%s: Internal Error!\r\n%s\r\n",
    ...
    if (firstItem->possibleCause != NULL) {
      nFirstCause = sprintf_s(g_errorThread->errorMsg + n, g_errorThread->errorMsgLen, "        PossibleCause: %s\r\n",
    ...
    if (firstItem->solution != NULL) {
      nFirstSolution = sprintf_s(g_errorThread->errorMsg + n, g_errorThread->errorMsgLen, "        Solution: %s\r\n",
                                 firstItem->solution);
      if (nFirstSolution < 0) {
        return -1;
      }
...
static int32_t FormatTraceAfter(ErrorItem *firstItem, size_t errorItemNum, int32_t offset) {
  ...
  for (size_t i = 0; i < errorItemNum; i++) {
    ...
    nErrMsg =
        sprintf_s(g_errorThread->errorMsg + n, g_errorThread->errorMsgLen, "        %s\r\n", errorItem->errorMessage);
    if (nErrMsg < 0) {
      return -1;
    }
    n += nErrMsg;
