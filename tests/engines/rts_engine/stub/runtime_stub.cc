...
rtError_t rtMemcpyAsyncPtr(void *memcpyAddrInfo, uint64_t dest_max, uint64_t count, rtMemcpyKind_t kind,
  ...
  if (memcpyAddrInfo != nullptr) {
    ...
    memcpy_s(reinterpret_cast<void *>(reinterpret_cast<rtMemcpyAddrInfo *>(memcpyAddrInfo) +
            dstPtrOffset), dest_max, reinterpret_cast<void *>(reinterpret_cast<rtMemcpyAddrInfo *>(memcpyAddrInfo) +
            srcPtrOffset), count);
  }
  return RT_ERROR_NONE;
}
...
rtError_t rtNotifyWait(rtNotify_t notify_, rtStream_t stream_) { return RT_ERROR_NONE; }
rtError_t rtGetNotifyID(rtNotify_t notify_, uint32_t *notify_id) { return RT_ERROR_NONE; }
rtError_t rtNotifyRecord(rtNotify_t notify_, rtStream_t stream_) { return RT_ERROR_NONE; }
...
rtError_t rtMemAllocManaged(void **ptr, uint64_t size, uint32_t flag, const uint16_t moduleid) { return RT_ERROR_NONE; }
...
rtError_t rtKernelLaunchWithFlag(const void *stub_func, uint32_t block_dim, rtArgsEx_t *argsInfo,
...
rtError_t rtStreamSwitchN(void *ptr, uint32_t size, void *valuePtr, rtStream_t *trueStreamPtr,
                          uint32_t elementSize, void * stream, rtSwitchDataType_t dataType)
{
    return RT_ERROR_NONE;
}
