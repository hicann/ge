...
TEST_F(UTEST_fusion_engine_fe_graph_optimizer, optimize_original_graph_topological_sort_failed) {
...
// op_compiler_normal.cc:143 - RunCompileProcess with LxFusion changing graph
TEST_F(UTEST_fusion_engine_fe_graph_optimizer, op_compiler_normal_lx_fusion_changed) {
  auto graph = std::make_shared<ComputeGraph>("test_lx_fusion_changed");
  ...
  FusionPriorityMgrPtr fpm = std::make_shared<FusionPriorityManager>(AI_CORE_NAME, frm);
  fpm->Initialize();
  ...
  OpCompilerNormal compiler("normal", AI_CORE_NAME, lx_optimizer);
  compiler.init_flag_ = true;
  Status ret = compiler.RunCompileProcess(*graph);
  EXPECT_TRUE(ret == fe::SUCCESS || ret == fe::FAILED);
}

// op_setter.cc:55 - SetOppKernelPathForAclnn (free function, tested via OpSetter::SetOpInfo)
TEST_F(UTEST_fusion_engine_fe_graph_optimizer, op_setter_opp_kernel_path) {
  OpDescPtr op_desc = std::make_shared<OpDesc>("test_op", "Relu");
  OpSetter op_setter(AI_CORE_NAME);
...
// aicore_ops_kernel_builder.cc:79 - SetTilingSinkCalcResources logs tiling dependency
// aicore_ops_kernel_builder.cc:104 - CalculateTilingSinkWorkspace logs workspace size
// aicore_ops_kernel_builder.cc:157 - ProcDfxBufferSize logs overflow warning
TEST_F(UTEST_fusion_engine_fe_graph_optimizer, aicore_ops_kernel_builder_calc_ext_op_running_param) {
  auto graph = std::make_shared<ComputeGraph>("test_calc_param");
...
TEST_F(UTEST_fusion_engine_fe_graph_optimizer, is_prefix_ops_path_no_underscore) {
  OpDescPtr op_desc = std::make_shared<OpDesc>("test_op", "Relu");
...
TEST_F(UTEST_fusion_engine_fe_graph_optimizer, graph_comm_isolate_node_failed) {
  auto graph = std::make_shared<ComputeGraph>("test_graph_comm");
  ...
  auto graph_comm = std::make_shared<GraphComm>(AI_CORE_NAME);
  graph_comm->Initialize();
  std::vector<ge::NodePtr> node_vec = {inner_node};
}

// op_info_common.cc:1020 - IsPrefixOpsPath without "_"
TEST_F(UTEST_fusion_engine_fe_graph_optimizer, op_info_common_is_prefix_ops_path_no_underscore) {
  OpDescPtr op_desc = std::make_shared<OpDesc>("test_op", "Relu");
  ge::AttrUtils::SetStr(op_desc, OPS_PATH_NAME_PREFIX, "noprefix");
...
// op_info_common.cc:1079 - IsSuppoertedFormat with empty sub_formats and cur_sub_format > DEFAULT_SUB_FORMAT
TEST_F(UTEST_fusion_engine_fe_graph_optimizer, op_info_common_is_supported_format_empty_subformat) {
