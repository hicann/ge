...
te::OpBuildResCode BuildSuperKernelStub(std::vector<Node *> teGraphNode, OpDescPtr op_desc_ptr, uint64_t taskid,
                                        uint64_t tid) {
  string json_file_path = "./kernel_meta/";
  // OpDescPtr op_desc_ptr = output_node->GetOpDesc();
  AttrUtils::SetStr(op_desc_ptr, "json_file_path", json_file_path);
...
bool WaitAllFinishedStub4(uint64_t tid, vector<te::FinComTask> &fin_task) {
  te::FinComTask fin_com_task;
  fin_com_task.taskId = GetAtomicId() - 1;
...
TEST_F(UTEST_FE_TBE_COMPILER, tbe_op_process_fail_pre_compile_1) {
  TbeOpStoreAdapter compile_tbe_op(AI_CORE_NAME);
  // 1.create graph
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
...
TEST_F(UTEST_FE_TBE_COMPILER, tbe_op_process_succ_pre_comp_task) {
  ...
  compile_tbe_op.Initialize(options);
  // 1.create graph
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
  vector<int64_t> dim_weight = {1, 3, 3, 3};
  GeShape shape_weight(dim_weight);
  ...
  tbe_op_info_ptr->SetOpCoreType("Default");
  TbeOpStoreAdapter::CompileTaskPara task_para;
  task_para.task_num = 1;
  task_para.task_node_map.insert(make_pair(1, Node2.get()));
  ...
  // 3. result expected
  EXPECT_EQ(fe::SUCCESS, ret);
  op_kernel_info->op_str_param_vec_[static_cast<size_t>(OP_KERNEL_STR_PARAM::CoreType)] = "AiCore";
  ret = compile_tbe_op.ProcessSuccPreCompTask(task_para);
  EXPECT_EQ(fe::SUCCESS, ret);
  tbe_op_info_ptr->SetOpCoreType("Default");
  op_kernel_info->op_str_param_vec_[static_cast<size_t>(OP_KERNEL_STR_PARAM::CoreType)] = "VectorCore";
  task_para.task_kernel_info_map[1] = op_kernel_info;
  ret = compile_tbe_op.ProcessSuccPreCompTask(task_para);
  EXPECT_EQ(fe::SUCCESS, ret);
  std::string core_num_str;
  tbe_op_info_ptr->GetOption(ge::AICORE_NUM, core_num_str);
  EXPECT_EQ("", core_num_str);
}
...
TEST_F(UTEST_FE_TBE_COMPILER, case_update_dtype_by_allow_fp32_to_fp16_suc) {
  ...
  tbe_adapter_ptr->UpdateDtypeByAllowFp32ToFp16(matmul_desc, matmul_desc->GetAllInputsSize(), in_out_changed_idx_vec,
                                                true);
...
TEST_F(UTEST_FE_TBE_COMPILER, get_short_soc_version_success) {
  ...
  TbeOpStoreAdapter::CompileTaskPara task_para;
  Status ris = tbe_adapter_ptr->ProcessSuccSgtSliceTask(task_para);
  ...
  Status ret =
      tbe_adapter_ptr->SgtSetTeTask(node_vec, taskId, buff_fus_to_del_nodes, compile_strategy, slice_shape_index);
  EXPECT_EQ(fe::FAILED, ret);
  ret = tbe_adapter_ptr->SetTeTask(node_vec, taskId, buff_fus_to_del_nodes, compile_strategy, false);
  EXPECT_EQ(fe::FAILED, ret);
}

TEST_F(UTEST_FE_TBE_COMPILER, case_fusion_compile_check_success) {
  TbeOpStoreAdapter compile_tbe_op(AI_CORE_NAME);
  compile_tbe_op.support_parallel_compile = true;
  compile_tbe_op.TeFusion = TeFusionStub;
  compile_tbe_op.WaitAllFinished = WaitAllFinishedStub;
...
TEST_F(UTEST_FE_TBE_COMPILER, case_SetSuperKernelTask_success) {
  TbeOpStoreAdapter compile_tbe_op(AI_CORE_NAME);
  compile_tbe_op.support_parallel_compile = true;
  compile_tbe_op.TeFusion = TeFusionStub;
  compile_tbe_op.BuildSuperKernel = BuildSuperKernelStub;
  compile_tbe_op.WaitAllFinished = WaitAllFinishedStub;
  compile_tbe_op.GetOpInfo = get_tbe_opinfo_stub;
  ...
  compile_info.is_fusion_check = true;
  CompileResultMap compile_ret_map;
  Status ret = compile_tbe_op.SuperKernelCompile(fusion_nodes, compile_ret_map);
  EXPECT_EQ(fe::SUCCESS, ret);
}

TEST_F(UTEST_FE_TBE_COMPILER, case_fusion_compile_check_fail) {
  TbeOpStoreAdapter compile_tbe_op(AI_CORE_NAME);
  compile_tbe_op.support_parallel_compile = true;
  compile_tbe_op.TeFusion = TeFusionStub;
  compile_tbe_op.WaitAllFinished = WaitAllFinishedStub4;
  compile_tbe_op.GetOpInfo = get_tbe_opinfo_stub;
...
TEST_F(UTEST_FE_TBE_COMPILER, case_need_to_wait_task_finish) {
  TbeOpStoreAdapter compile_tbe_op(AI_CORE_NAME);
  compile_tbe_op.support_parallel_compile = true;
  compile_tbe_op.TeFusion = TeFusionStub;
  compile_tbe_op.WaitAllFinished = WaitAllFinishedStub;
  ...
  Status ret = compile_tbe_op.ParallelCompileOp(compile_info);
  EXPECT_EQ(fe::SUCCESS, ret);
}

TEST_F(UTEST_FE_TBE_COMPILER, case_wait_task_finish_fail) {
  TbeOpStoreAdapter compile_tbe_op(AI_CORE_NAME);
  compile_tbe_op.support_parallel_compile = true;
  compile_tbe_op.TeFusion = TeFusionStub;
...
TEST_F(UTEST_FE_TBE_COMPILER, set_precompile_pattern_case1) {
  ge::OpDescPtr op_desc = std::make_shared<ge::OpDesc>("gather_op", "GatherV2");

  te::TbeOpInfo op_info("gather_op", "impl.gather_v2", "GatherV2", "VectorCore");
  op_info.SetPattern("Elemwise");
  ...
  EXPECT_EQ(op_info.GetOpCoreType(), "VectorCore");
}

TEST_F(UTEST_FE_TBE_COMPILER, set_precompile_pattern_case2) {
  ge::OpDescPtr op_desc = std::make_shared<ge::OpDesc>("gather_op", "GatherV2");

  te::TbeOpInfo op_info("gather_op", "impl.gather_v2", "GatherV2", "AiCore");
  op_info.SetPattern("Elemwise");
  op_info.SetOpCoreType("Default");
  ...
  Status status = tbe_op_store_adapter.SetPreCompilePattern(op_desc, op_info, opKernelInfoPtr);
  EXPECT_EQ(status, fe::SUCCESS);
  EXPECT_EQ(op_info.GetOpCoreType(), "AiCore");
}

TEST_F(UTEST_FE_TBE_COMPILER, set_precompile_pattern_case3) {
  ge::OpDescPtr op_desc = std::make_shared<ge::OpDesc>("gather_op", "GatherV2");
...
TEST_F(UTEST_FE_TBE_COMPILER, test_set_opdesc_custom_op_fail) {
  TbeOpStoreAdapter compile_tbe_op(AI_CORE_NAME);
  ge::OpDescPtr op_desc = std::make_shared<ge::OpDesc>("gather_op", "GatherV2");
