...
//     reflection_builder_ptr_); heavt_format_propagator->Initialize(); Status ret =
...
TEST_F(UTEST_fusion_engine_heavy_format_distribution, get_peer_out_format_output_desc_null) {
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test_peer_out_null");
  ...
  // Remove output desc from producer to trigger nullptr
  producer_op->MutableOutputDesc(0)->SetFormat(ge::FORMAT_NCHW);
  ...
  // The producer has output desc at index 0, so to get nullptr we need an invalid peer_out_idx.
  // Instead, create a producer with no output desc at the connected index.
  Status ret = heavy_format_propagator->GetPeerOutFormat(input_non_format_agnostic_index, peer_out_info);
  // This should return SUCCESS since the format is compatible
...
TEST_F(UTEST_fusion_engine_heavy_format_distribution, match_dtype_empty_kernel) {
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test_match_dtype_empty");
  ...
  // Don't call Initialize() so input_and_output_kernel_ is empty
...
TEST_F(UTEST_fusion_engine_heavy_format_distribution, select_qualified_format_empty_kernel) {
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test_select_empty");
  OpDescPtr op_desc = std::make_shared<OpDesc>("test_op", "Relu");
  GeTensorDesc desc(GeShape({1, 2, 3, 4}), ge::FORMAT_NCHW, ge::DT_FLOAT16);
  op_desc->AddInputDesc("x", desc);
  op_desc->AddOutputDesc("y", desc);
  ...
  // SelectQualifiedFormat calls getAllInputAndOutputKernelInfo first which may return FAILED or
  // input_and_output_kernel_ stays empty/insufficient, triggering line 297
  EXPECT_EQ(ret, fe::FAILED);
}
