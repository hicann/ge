...
class UTEST_fusion_engine_heavy_format_distribution_fzg : public testing::Test {
 ...
 protected:
  ...
  ComputeGraphPtr CreateFirstLayerOpGraph8(ge::NodePtr &conv2d_node, OpDescPtr &conv2d_op, OpDescPtr &conv2d_op_2) {
    ...
    conv2d_op_2->AddInputDesc("x", tensor_desc);
    conv2d_op_2->AddInputDesc("y", tensor_desc);
    conv2d_op_2->AddOutputDesc("z", tensor_desc);

    aipp_op->AddInputDesc(tensor_desc);
    aipp_op->AddOutputDesc(tensor_desc);
    bn_op->AddInputDesc(tensor_desc);
    bn_op->AddOutputDesc(tensor_desc);
    ...
    GraphUtils::AddEdge(aipp_node->GetOutDataAnchor(0), conv2d_node->GetInDataAnchor(0));
    GraphUtils::AddEdge(conv2d_node->GetOutDataAnchor(0), bn_node->GetInDataAnchor(0));
    GraphUtils::AddEdge(weight_node->GetOutDataAnchor(0), conv2d_node->GetInDataAnchor(1));
    GraphUtils::AddEdge(bn_node->GetOutDataAnchor(0), conv2d_node_2->GetInDataAnchor(0));
    return graph;
  }
};
...
TEST_F(UTEST_fusion_engine_heavy_format_distribution_fzg, JudgeFirstLayerConv08) {
  ...
  EXPECT_EQ(op_desc2->HasAttr(IS_FIRST_LAYER_CONV_FOR_OP), false);
  EXPECT_EQ(op_desc2->HasAttr(IS_FIRST_LAYER_CONV), false);
}
