...
TEST_F(FFTSTaskBuilderAdapterUTest, Init_workspace_failed) {
...
void CreateNodeWithWeight(ge::ComputeGraphPtr &graph, ge::NodePtr &node_ptr) {
  ge::GeShape original_shape = ge::GeShape({3, 12, 5, 6});
  GraphConstructor test(graph, "", ge::FORMAT_NHWC, ge::DT_FLOAT, original_shape);
  test.AddOpDesc("BiasAdd", "BiasAdd", 1, 1);
  test.GetNodeByName("BiasAdd", node_ptr);
...
TEST_F(FFTSTaskBuilderAdapterUTest, handle_anchor_weight_small_size) {
  ge::InDataAnchorPtr anchor;
  size_t input_index = 0;
  size_t anchor_index = 0;
  ...
  CreateNodeWithWeight(graph, node_ptr);
  vector<ge::GeTensorPtr> weights = ge::OpDescUtils::MutableWeights(*node_ptr);
  ge::GeTensorPtr weight = weights[0];
  ge::TensorUtils::SetWeightSize(weight->MutableTensorDesc(), 4320);
  ge::TensorUtils::SetDataOffset(weight->MutableTensorDesc(), 4320);
  ge::TensorUtils::SetDataOffset(*(node_ptr->GetOpDesc()->MutableInputDesc(0)), 4320);
...
TEST_F(FFTSTaskBuilderAdapterUTest, handle_anchor_weight_large_size) {
  ...
  vector<ge::GeTensorPtr> weights = ge::OpDescUtils::MutableWeights(*node_ptr);
  ge::GeTensorPtr weight = weights[0];
  ge::TensorUtils::SetWeightSize(weight->MutableTensorDesc(), kMaxWeightOffset);
  ge::TensorUtils::SetDataOffset(weight->MutableTensorDesc(), kMaxWeightOffset);
...
TEST_F(FFTSTaskBuilderAdapterUTest, gen_mix_l2_taskdef) {
...
TEST_F(FFTSTaskBuilderAdapterUTest, gen_dyn_and_opt_mix_l2_taskdef) {
...
TEST_F(FFTSTaskBuilderAdapterUTest, gen_dyn_and_opt_mix_l2_taskdef_exception) {
  auto node = CreateDynAndOptNode2();
  ge::AttrUtils::SetBool(node->GetOpDesc(), "_original_fusion_graph", true);
  vector<void *> input_addrs(6, nullptr);
  vector<void *> output_addrs(3, nullptr);
  ffts_task_builder_adapter_ptr->FeedArgsInfo(domi::ArgsInfo_ArgsType_INPUT, domi::ArgsInfo_ArgsFormat_DIRECT_ADDR, 0);
  ffts_task_builder_adapter_ptr->FeedArgsInfo(domi::ArgsInfo_ArgsType_INPUT, domi::ArgsInfo_ArgsFormat_DIRECT_ADDR, 1);
...
TEST_F(FFTSTaskBuilderAdapterUTest, GenCtxParamAndCtxType_AutoModeInitFailed) {
  auto node = CreateNode();
...
TEST_F(FFTSTaskBuilderAdapterUTest, GenCtxParamAndCtxType_ManualModeInitFailed) {
  auto node = CreateNode();
  (void)ge::AttrUtils::SetInt(node->GetOpDesc(), fe::NON_TAIL_WORKSPACE_SIZE, 10);
  node->GetOpDesc()->SetWorkspaceBytes({100000});
