...
class TeFusionApiUTest : public testing::Test {
 public:
  TeFusionApiUTest() {}
...
TEST(TeFusionApiUTest, GetCurrProcessesUt) {
...
TEST(TeFusionApiUTest, RecordPidTimeIdInfoUt) {
...
TEST(TeFusionApiUTest, PushPidUniqueIdUt) {
  // construct pidUniqueId vector
  std::vector<std::pair<uint32_t, std::string>> pidUniqueId = {{2, "aaaa"}, {3, "cccc"}};
...
TEST(TeFusionApiUTest, RemoveParentKernelMetaDir) {
  std::string currentFilePath = GetCodeDir() + "/tests/engines/nn_engine/ut/testcase/op_compile_adapter";
...
TEST(TeFusionApiUTest, RemovePidTimeIdInfoUt) {
  ...
  std::string filePath = currentFilePath + "/" + PID_FILE_NAME;
  std::string path = te::fusion::RealPath(filePath);
  EXPECT_EQ(path.empty(), true);
}

TEST(TeFusionApiUTest, TestnotexistfileSt_01) {
...
TEST(TeFusionApiUTest, RemovePidTimeIdInfoUt_01) {
  std::string currentFilePath = GetCodeDir() + "tests/engines/nn_engine/ut/testcase";
  te::fusion::TeConfigInfo::Instance().debugDirs_.push(currentFilePath);
  te::fusion::TeConfigInfo::Instance().kernelMetaParentDir_ = currentFilePath + "/kernel_meta";
...
TEST(TeFusionApiUTest, MgrPidUniqueIdInfoJsonUt) {
  // construct pidUniqueId vector
  std::vector<std::pair<uint32_t, std::string>> delPidUniqueId = {{2, "aaaa"}, {3, PID_DELETED}};

  std::vector<std::pair<uint32_t, std::string>> curPidUniqueId = {{1, "aaaa"}, {2, "aaaa"}, {3, "cccc"}, {4, "cccc"}};
...
TEST(TeFusionApiUTest, RemoveBuildLockFileUt) {
  std::string currentFilePath = GetCodeDir() + "tests/engines/nn_engine/ut/testcase";
...
TEST(TeFusionApiUTest, RemoveKernelTempDir) {
  std::string currentFilePath = GetCodeDir() + "tests/engines/nn_engine/ut/testcase";
  te::fusion::TeConfigInfo::Instance().debugDirs_.push(currentFilePath);
...
TEST(TeFusionApiUTest, RemoveKernelTempDir1) {
  std::string currentFilePath = GetCodeDir() + "tests/engines/nn_engine/ut/testcase";
  te::fusion::TeConfigInfo::Instance().debugDirs_.push(currentFilePath);
...
TEST(TeFusionApiUTest, RemoveKernelMetaDir) {
  std::string currentFilePath = GetCodeDir() + "tests/engines/nn_engine/ut/testcase";
...
TEST(TeFusionApiSTest, TestDelCacheFileByAccessTime) {
  ...
  std::string currentFilePath =
      GetCodeDir() + "tests/engines/nn_engine/ut/testcase/op_compile_adapter/disk_cache/atc_data/kernel_cache";
...
TEST(TeFusionApiSTest, TestUpdateOpCacheSizeCfg) {
  te::fusion::TeCacheSpaceManager &fusionSerial = te::fusion::TeCacheSpaceManager::Instance();
  std::string opMaxCacheSize = "122";
  std::string opRemainCacheRadio = "adb";
  fusionSerial.UpdateOpCacheSizeCfg(opMaxCacheSize, opRemainCacheRadio);
  opRemainCacheRadio = "-1";
  fusionSerial.UpdateOpCacheSizeCfg(opMaxCacheSize, opRemainCacheRadio);
}
