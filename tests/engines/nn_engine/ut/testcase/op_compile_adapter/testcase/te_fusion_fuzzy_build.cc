...
TEST(FuzzyBuildUTest, TaskFirstCompileSuccess) {
  ...
  std::string targetJsonPath = te::fusion::RealPath(
...
TEST(FuzzyBuildUTest, FuzzyBuildCompileLockFailUt) {
  ...
  std::string lockFile =
      te::fusion::TeConfigInfo::Instance().GetDebugDir() + "/kernel_meta/kernel_meta_temp/te_Activation_success.lock";
...
TEST(FuzzyBuildUTest, FuzzyBuildFindNoCacheCompileUt) {
  ...
  // check build lock, lock file exist
  std::string lockFile =
      te::fusion::TeConfigInfo::Instance().GetDebugDir() + "/kernel_meta/kernel_meta_temp_test_id/.lock";
  std::string lockFilePath = te::fusion::RealPath(lockFile);
  EXPECT_EQ(lockFilePath.empty(), true);
...
TEST(FuzzyBuildUTest, fusion_serial_ParseResult) {
  ...
  te::fusion::PythonAdapterManager::ParseResult(nullptr, 0, jsonFilePath, pattern, coreType, compileInfo,
                                                compileInfoKey, fusionCheckRes);
  te::fusion::PythonAdapterManager::ParseResult(opResCompile.c_str(), 0, jsonFilePath, pattern, coreType, compileInfo,
                                                compileInfoKey, fusionCheckRes);
