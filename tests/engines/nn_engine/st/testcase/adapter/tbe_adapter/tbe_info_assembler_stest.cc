...
TEST_F(STEST_TbeInfoAssembler, Test_FeedFlagInt64ToTbeOpInfo){
  ...
  vector<int64_t> dim_weight = {1024, 1, 1024, 1024};
  GeShape shape_weight(dim_weight);
  GeTensorDesc weight_desc(shape_weight);

  vector<int64_t> dim_out = {1024, 1, 1024, 1};
...
TEST_F(STEST_TbeInfoAssembler, get_options_01) {
  ...
  auto options = tbe_info_assembler.GetAllOptionsForTBE(*op_desc, "AiCoreEngine", nullptr);
  EXPECT_EQ(options[ge::OP_DEBUG_LEVEL], "");

  putenv("NPU_COLLECT_PATH=");
  options = tbe_info_assembler.GetAllOptionsForTBE(*op_desc, "AiCoreEngine", nullptr);
  EXPECT_EQ(options[ge::OP_DEBUG_LEVEL], "");
}

TEST_F(STEST_TbeInfoAssembler, set_tensor_constvalue_1) {
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
...
TEST_F(STEST_TbeInfoAssembler, set_tensor_constvalue_2) {
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
  NodePtr node = CreateConstDependNode(graph, ge::DT_FLOAT);
  InputOrOutputInfoPtr tensor_info_ptr = std::make_shared<InputOrOutputInfo>("input");
...
TEST_F(STEST_TbeInfoAssembler, set_tensor_constvalue_3) {
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
  NodePtr node = CreateConstDependNode(graph, ge::DT_FLOAT);
  InputOrOutputInfoPtr tensor_info_ptr = std::make_shared<InputOrOutputInfo>("input");
  tensor_info_ptr->op_const_value_depend_ = CONST_REQUIRED;
...
TEST_F(STEST_TbeInfoAssembler, set_tensor_constvalue_4) {
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
...
TEST_F(STEST_TbeInfoAssembler, set_tensor_constvalue_5) {
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
...
TEST_F(STEST_TbeInfoAssembler, set_tensor_constvalue_8) {
  ...
  ge::GeTensorPtr constTensor = std::make_shared<ge::GeTensor>(tensorDesc);
  vector<float> fp_data_value = {1.319, 0.64, 54.983, -234.86, -0.378, -1.345};
