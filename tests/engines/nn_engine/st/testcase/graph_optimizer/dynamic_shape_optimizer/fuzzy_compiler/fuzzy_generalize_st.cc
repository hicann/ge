...
//    tbe_op_store_adapter_ = std::make_shared<TbeOpStoreAdapter>(fe::AI_CORE_NAME);
//    tbe_op_store_adapter_->Initialize(options1);
...
TEST_F(STEST_fusion_engine_fuzzy_generalize, update_dynamic_shape_to_newInputNode) {
  ge::ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
  CreateBatchNormGraph(graph);
  ge::NodePtr data_node;
  ge::NodePtr bn_node;
...
TEST_F(STEST_fusion_engine_fuzzy_generalize, update_op_attrs) {
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
...
TEST_F(STEST_fusion_engine_fuzzy_generalize, is_shape_generalized) {
...
TEST_F(STEST_fusion_engine_fuzzy_generalize, coverage_increase) {
  ge::ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
  CreateBatchNormGraph(graph);
  ge::NodePtr data_node;
  ge::NodePtr bn_node;
  ...
  bool btmp = false;
  EXPECT_EQ(fuzzy_ptr->CheckAndUpdateLimitedNodes(tbe_op_store_adapter_, limited_nodes, btmp), fe::SUCCESS);
  std::vector<size_t> limited_input_indexs;
  EXPECT_NE(fuzzy_ptr->Downgrades(bn_node, false, limited_input_indexs, btmp), fe::SUCCESS);

  std::map<std::string, ge::NodePtr> new_input_nodes;
  new_input_nodes.insert(std::make_pair("nnnnnnnn", bn_node));
  EXPECT_NE(fuzzy_ptr->UpdateDynamicShapeToNewInputNode(sub_graph_input_nodes, new_input_nodes), fe::SUCCESS);
  EXPECT_EQ(fuzzy_ptr->UpdateDynamicShapeToNewBakGraph(*graph), fe::SUCCESS);
  fuzzy_ptr->external_input_nodes_.clear();
  fuzzy_ptr->InitOriginalGraphInfos(*graph);
...
TEST_F(STEST_fusion_engine_fuzzy_generalize, test_generalization_when_pass_registered_as_always_generalization) {
  ge::ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
...
TEST_F(STEST_fusion_engine_fuzzy_generalize, test_read_json_object) {
