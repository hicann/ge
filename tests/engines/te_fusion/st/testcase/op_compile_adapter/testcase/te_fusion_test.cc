...
TEST(TEFUSION, BuildOpFail_ST) {
  ...
  resCode =
      TeFusion(nodeList, opdesc1, empty, static_cast<uint64_t>(rand()), 0, "");  // RLTuneSingleOp fail GetNodeListStr
  EXPECT_EQ(resCode, OP_BUILD_SUCC);
...
TEST(TEFUSION, GetFinishedCompilationTask_PreBuild_ST) {
  ...
  MOCKER(te::fusion::PythonAdapterManager::ParseOpTaskResult).stubs().will(invoke(ParseOpTaskResult_stub));
  ...
  MOCKER(te::fusion::PythonAdapterManager::ParseOpTaskResult).stubs().will(invoke(ParseOpTaskResult_stub3));
  ...
  te::fusion::TeFusionManager::GetInstance()->fusionOpsKernel_.emplace(
      task->kernel, std::make_pair(nullptr, std::vector<OpBuildTaskPtr>()));
  finalRes = te::fusion::TeFusionManager::GetInstance()->GetFinishedCompilationTask(0);
  EXPECT_EQ(finalRes, true);
...
TEST(TEFUSION, GetFinishedCompilationTask_PreBuild_ErrMsg_ST) {
  ...
  MOCKER(te::fusion::PythonAdapterManager::ParseOpTaskResult).stubs().will(invoke(ParseOpTaskResult_stub_1));
...
std::string GetSHA256FromOFile_stub3(te::fusion::TeCacheSpaceManager *This, const std::string &oFilePath) {
  printf("GetSHA256FromOFile_stub3 \r\n");
  // same as "sha256" in
  // stub\binary_stub\te_matmul_2ac1b9067af400eb161d75ccd6c6ca71bed7b97750d79c4476fb65ac9bedd762.json
  std::string oFileSha = "0ffe1abd1a08215353c233d6e009613e95eec4253832a761af28ff37ac5a150c";
  return oFileSha;
}

bool GenerateStrSha256HashValue_stub(te::fusion::PythonApiCall *This, const std::string &str, std::string &res) {
  res = "testSuccess";
  return true;
}
...
TEST(TEFUSION, TbeInitialize_UT0) {
  ...
  MOCKER(setenv).stubs().will(returnValue(-1));
...
TEST(TEFUSION, TbeInitialize_UT1) {
  std::map<std::string, std::string> options = {{"abc", "dfg"}};
  MOCKER(&TeConfigInfo::Initialize).stubs().will(returnValue(false));
  bool parallelComp = true;
  bool *isSupportParallelCompilation = &parallelComp;
  TbeInitialize(options, isSupportParallelCompilation);
}
...
TEST(TEFUSION, TbeInitialize_UT2) {
  std::map<std::string, std::string> options = {{"abc", "dfg"}};
  MOCKER(&HandleManager::Initialize).stubs().will(returnValue(false));
  bool parallelComp = true;
  bool *isSupportParallelCompilation = &parallelComp;
  TbeInitialize(options, isSupportParallelCompilation);
}
...
bool CheckCurrNode_stub(te::fusion::TeFusionManager *This, std::vector<ge::Node *> &elewiseNode,
                        std::vector<ge::Node *> &dReluNode, const ge::Node *currNode, bool &transShape4hd,
                        bool &isDRelu) {
...
TEST(TEST_TEFUSION_ST, TaskFusion_001) {
  ...
  uint64_t taskId = 200;
  uint64_t sgtThreadIndex = 300;
  auto opTaskPtr = std::make_shared<OpBuildTask>(
      OpBuildTask{graphId, taskId, INVALID_SGT_INDEX, teGraphNode, nullptr, OP_TASK_STATUS::OP_TASK_PENDING});
...
TEST(TEST_TEFUSION_ST, BuildSuperKernel_002) {
  ...
  (void)te::fusion::SuperKernelTaskManager::GetInstance().superKernelMap_.emplace(
      super_kernel_name, std::make_pair(nullptr, std::vector<OpBuildTaskPtr>({opTaskPtr})));
  (void)te::fusion::TeFusionManager::GetInstance()->opKernelMap_.emplace(
      kernel_name, std::make_pair(opRes, std::vector<OpBuildTaskPtr>({opTaskPtr})));
  OpBuildResCode res = BuildSuperKernel(teGraphNode, opDescPtr, taskId, graphId);
  EXPECT_EQ(res, OP_BUILD_SUCC);
}
...
TEST(TEST_TEFUSION_ST, BinaryGeneralizeRegistered_001) {
  ...
  uint64_t sgtThreadIndex = 300;
  auto opTaskPtr = std::make_shared<OpBuildTask>(
      OpBuildTask{graphId, taskId, sgtThreadIndex, teGraphNode, opDescPtr, OP_TASK_STATUS::OP_TASK_SUCC});
  opTaskPtr->pTbeOpInfo = tbeOpInfo;
...
void BuildOrderedGraph(ge::NodePtr &node0, ge::NodePtr &node1, ge::NodePtr &node2, ge::NodePtr &node3) {
...
void BuildOrderedGraph1(ge::NodePtr &node0, ge::NodePtr &node1, ge::NodePtr &node2, ge::NodePtr &node3) {
  ge::ComputeGraphPtr graph = std::make_shared<ge::ComputeGraph>("test");
  ge::OpDescPtr opDescPtr0 = std::make_shared<ge::OpDesc>("mul0", "Mul");
  ge::OpDescPtr opDescPtr1 = std::make_shared<ge::OpDesc>("mul1", "Mul");
...
TEST(TEST_TEFUSION_ST, ReportErrMsgs) {
  ...
  ge::OpDescPtr opDescPtr = std::make_shared<ge::OpDesc>("matmul", "Matmul");
  auto task = std::make_shared<te::fusion::OpBuildTask>(
      te::fusion::OpBuildTask{0, 0, 0, teGraphNode, opDescPtr, te::fusion::OP_TASK_STATUS::OP_TASK_FAIL});
...
TEST(TEFUSION, Check_prebuilt_options_in_tbeopinfo) {
  ...
  MOCKER(te::fusion::PythonAdapterManager::ParseOpTaskResult).stubs().will(invoke(ParseOpTaskResult_stub4));
  const std::map<OpTaskKey, OpBuildTaskPtr>::const_iterator historyGraphItr =
      fusion_manager->dispatchedTask_.find(OpTaskKey(0, 0));
...
TEST(TEFUSION, DeleteCaxisValuesFromJsonTest) {
  ...
  //=====================one=========================
  std::string oneCaxisValueFile = currentFilePath + "/one_caxisc_values.json";
  ...
  //=====================two=========================
  std::string twoCaxisValueFile = currentFilePath + "/two_caxisc_values.json";
  ...
  //=====================no=========================
  std::string noCaxisValueFile = currentFilePath + "/two_has_no_caxisc_values.json";
  ...
  //=====================two-withone=========================
  std::string towWithOneCaxisValueFile = currentFilePath + "/two_has_one_caxisc_values.json";
...
TEST(TEST_TEFUSION_ST, BuildBinarySingleOp) {
  ...
  options["all_ops_impl_path"] =
      "built-in/op_impl/ai_core/tbe/impl|customize/op_impl/ai_core/tbe/customize_impl|py_ut_op_stub/impl";
...
TEST(TEST_TEFUSION_ST, BuildBinaryFusionOp) {
  ...
  std::vector<ge::Node *> teGraphNode;
  teGraphNode.push_back(nodeConv2d.get());
  auto opTask = std::make_shared<te::fusion::OpBuildTask>(
      te::fusion::OpBuildTask{0, 0, 0, teGraphNode, opDescPtr, te::fusion::OP_TASK_STATUS::OP_TASK_SUCC});
...
TEST(TEST_TEFUSION_ST, Test_CanReuseBuildBinaryCache_001) {
  ...
  auto opTask = std::make_shared<te::fusion::OpBuildTask>(
      te::fusion::OpBuildTask{0, 0, 0, teGraphNode, opDescPtr, te::fusion::OP_TASK_STATUS::OP_TASK_SUCC});
  auto opTask1 = std::make_shared<te::fusion::OpBuildTask>(
      te::fusion::OpBuildTask{0, 1, 0, teGraphNode, opDescPtr, te::fusion::OP_TASK_STATUS::OP_TASK_SUCC});
...
TEST(TEST_TEFUSION_ST, CheckCommandValid_fp_null) {
  ...
  MOCKER(popen).expects(once()).will(returnValue(fp));
  std::string cmd = "python3-config --prefix";
  char line[1024] = {0};
  bool res = te::fusion::HandleManager::Instance().CheckCommandValid(cmd, line);
  EXPECT_EQ(res, false);
...
TEST(TEST_TEFUSION_ST, LaunchDynamicLib_load_so_fail) {
  ...
  MOCKER(popen).expects(exactly(2)).will(returnValue(fp1)).then(returnValue(fp2));
  void *libHandle = nullptr;
  MOCKER(mmDlopen).expects(once()).will(returnValue(libHandle));
...
TEST(TEFUSION, Test_Parse_Error_Message) {
  ...
  MOCKER(te::fusion::PythonAdapterManager::ParseOpTaskResult).stubs().will(invoke(ParseOpTaskResult_stub_2));
  const std::map<OpTaskKey, OpBuildTaskPtr>::const_iterator historyGraphItr =
      fusion_manager->dispatchedTask_.find(OpTaskKey(0, 0));
  OpBuildTaskPtr relBuildTaskPtr = historyGraphItr->second;
  std::string extraParams = "{\"key1\":\"value1\"}";
