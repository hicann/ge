...
class FuzzyBuildSTest : public testing::Test {
 public:
  FuzzyBuildSTest() {}
...
bool GenerateOFileSha256HashValue_stub(te::fusion::PythonApiCall *This, const char *binData, const size_t binSize,
                                       std::string &sha256Val) {
  sha256Val = "b97559990204d88759446ca413be0189aa572e941ff87908a658d7044ead8854";
  return true;
}

TEST(FuzzyBuildSTest, GeneralizeFuncNotRegister) {
  bool res = false;
...
TEST(FuzzyBuildSTest, GeneralizeFuncRegister) {
  bool res = false;
...
TEST(FuzzyBuildSTest, GeneralizeFuncRegisterWithErrorPyAPI) {
  bool res = false;
...
void CompareGeneralizeShape(pair<vector<int64_t>, vector<int64_t>> &shapeGeneralize,
  ...
  EXPECT_EQ(res, true);
  res =
      (shapeGeneralize.second.size() == oriShape.size() || (shapeGeneralize.second.size() == 1 && oriShape.size() == 0))
          ? true
          : false;
  EXPECT_EQ(res, true);
...
void CompareGeneralizeRange(pair<vector<pair<int64_t, int64_t>>, vector<pair<int64_t, int64_t>>> &rangeGeneralize,
  ...
  for (size_t i = 0; i < range.size(); i++) {
    if (rangeGeneralize.first[i].first != range[i].first || rangeGeneralize.first[i].second != range[i].second) {
      res = false;
    }
  }
  EXPECT_EQ(res, true);
...
void CompareGeneralizeRes(
  ...
  for (auto &inputDataAnchor : nodePtr->GetAllInDataAnchors()) {
    ...
    if (peerNode->GetType() != "Data") {
      std::cout << "peerNode[" << peerNode->GetOpDesc()->GetName() << "] is not data, optype is" << peerNode->GetType()
                << std::endl;
      continue;
    }
...
TEST(FuzzyBuildSTest, GeneralizeFuncReturnNone) {
  bool res = false;
  ...
  AddOpParamToTbeOpInfo({9, 2, 3, 4}, "int64", "NHWC", "input13", range1, true, op_info);
...
TEST(FuzzyBuildSTest, GeneralizeFuncReturnSuccess) {
  bool res = false;
  std::vector<std::pair<int64_t, int64_t>> range;
...
bool FindExistedInputAttr_null_attrs_stub(te::fusion::TeFusionManager *This, vector<ge::NamedAttrs> &existedInputAttrs,
                                          int64_t &existedIndex, int64_t &originalIndex,
                                          vector<ge::NamedAttrs> &supportedAttrs,
                                          vector<ge::NamedAttrs> &existedTensorAttrs) {
  printf("Get in FindExistedInputAttr_null_attrs_stub!!!! \r\n");
  return true;
}

bool SetFuzzBuildAttrFromJsonForInputs_stub(te::fusion::TeFusionManager *This, const nlohmann::json &singleJsonTensor,
                                            std::string &keyName, ge::NamedAttrs &existedTensorAttr, bool isAttrExisted,
                                            ge::NamedAttrs &tensorAttr) {
...
TEST(FuzzyBuildSTest, GeneralizeTbeOpInfoReturnSuccess) {
  bool res = false;
  std::vector<std::pair<int64_t, int64_t>> range;
...
TEST(FuzzyBuildSTest, GeneralizeTbeOpInfoReturnSuccess_001) {
  bool res = false;
  std::vector<std::pair<int64_t, int64_t>> range;
...
TEST(FuzzyBuildSTest, GeneralizeNodeReturnSuccess) {
  bool res = false;
  std::vector<std::pair<int64_t, int64_t>> range;
...
TEST(FuzzyBuildSTest, GeneralizeWithInvalidGeneralizeType) {
  bool res = false;
  std::vector<std::pair<int64_t, int64_t>> range;
...
TEST(FuzzyBuildSTest, GeneralizeFuncReturnNotList) {
  bool res = false;
  std::vector<std::pair<int64_t, int64_t>> range;
  // create node Add
...
TEST(FuzzyBuildSTest, TaskFirstCompileSuccess) {
  bool res = false;
  ...
  auto task = std::make_shared<te::fusion::OpBuildTask>(te::fusion::OpBuildTask{
      graphId, taskId, sgtThreadIndex, teGraphNode, opDescPtr, te::fusion::OP_TASK_STATUS::OP_TASK_SUCC});
  ...
  opRes->jsonFilePath = te::fusion::RealPath(
      currentFilePath + "/kernel_meta/te_matmul_2ac1b9067af400eb161d75ccd6c6ca71bed7b97750d79c4476fb65ac9bedd762.json");
  ...
  if (!jsonFilePath.empty()) {
    ...
    std::string path2 = te::fusion::RealPath(
        currentFilePath +
...
TEST(FuzzyBuildSTest, CacheFuzzyCompileSuccessSt) {
  bool res = false;
  uint64_t graphId = 100;
  uint64_t taskId = static_cast<uint64_t>(rand());
  uint64_t sgtThreadIndex = 300;
  ...
  if (!jsonFilePath.empty()) {
    ...
    std::string path2 = te::fusion::RealPath(
        currentFilePath +
        "/disk_cache/kernel_meta/te_matmul_2ac1b9067af400eb161d75ccd6c6ca71bed7b97750d79c4476fb65ac9bedd762.json");
...
TEST(FuzzyBuildSTest, FuncTeGeneralizeTest) {
  bool res = false;
  ComputeGraphPtr owner_graph = std::make_shared<ComputeGraph>("te");
  ge::OpDescPtr opDescPtr = std::make_shared<ge::OpDesc>("lambNextMVWithDecay", "LambNextMVWithDecay");
  ge::NodePtr nodePtr = std::make_shared<ge::Node>(opDescPtr, owner_graph);
...
TEST(FuzzyBuildSTest, DynamicShapeAccurateBuildReuseFuzzyCacheMatchSt) {
  bool res = false;
  uint64_t graphId = 100;
  uint64_t taskId = static_cast<uint64_t>(rand());
  uint64_t sgtThreadIndex = 300;
...
TEST(FuzzyBuildSTest, DynamicShapeAccurateBuildReuseFuzzyCacheMatchNotHighPerformaceSt) {
  ...
  task->kernel = "te_Activation_success";
  res = te::fusion::TeFusionManager::GetInstance()->BuildTbeOp(task, strategy);
  printf("End of BuildTbeOp!!!!!opName = %s.\r\n", opName.c_str());
...
TEST(FuzzyBuildSTest, DynamicShapeAccurateBuildReuseFuzzyCacheNoMatchSt) {
  TeCacheSpaceManager fusion_serial = TeCacheSpaceManager::Instance();
  MOCKER_CPP(&te::fusion::PythonApiCall::GetBinFileSha256Value,
             bool (te::fusion::PythonApiCall::*)(const char *binData, const size_t, std::string &res) const)
      .stubs()
...
TEST(FuzzyBuildSTest, DynamicShapeAccurateBuildReuseFuzzyCacheConditionDynamicFailSt) {
  bool res = false;
  uint64_t graphId = 100;
  uint64_t taskId = static_cast<uint64_t>(rand());
  uint64_t sgtThreadIndex = 300;
...
// 1127 temp for fuzzy call operater accurate build, maybe deleted later
TEST(FuzzyBuildSTest, TempFuzzyBuildFindCacheSt) {
  bool res = false;
  uint64_t graphId = 100;
  uint64_t taskId = static_cast<uint64_t>(rand());
  uint64_t sgtThreadIndex = 300;
...
TEST(FuzzyBuildSTest, SetShapeAndValueToTensor_001) {
...
TEST(FuzzyBuildSTest, fusion_util_DirPath) {
  std::string res = DirPath("");
  EXPECT_EQ(res, "");

  res = DirPath(
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxsssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss"
      "sssxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxssssss"
      "ssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxsssssssssssssssssssss"
      "sssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxssssssssssssssssssssssssssssssssssss"
      "ssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxsssssssssssssssssssssssssssssssssssssssssssssssssss"
      "sssssssssssssssssssssssssssssssssssssssssssssssssssssssxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss"
      "ssssssssssssssssssssssssssssssssssssssssxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxsssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss"
      "sssssssssssssssssssssssssxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss"
      "ssssssssssxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxssssssssssssss"
      "ssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxsssssssssssssssssssssssssssss"
      "sssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxssssssssssssssssssssssssssssssssssssssssssss"
      "ssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxsssssssssssssssssssssssssssssssssssssssssssssssssssssssssss"
      "sssssssssssssssssssssssssssssssssssssssssssssssxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss"
      "ssssssssssssssssssssssssssssssssxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      "xxxxxxxxxxxxxx");
  EXPECT_EQ(res, "");
...
TEST(FuzzyBuildSTest, fusion_serial_GetFilesAccessTime) {
  std::string currentFilePath = GetCodeDir() + "/tests/engines/nn_engine/st/testcase/op_compile_adapter";
  if (te::fusion::RealPath(currentFilePath).empty()) {
    currentFilePath = "../../../../../../../../../llt/atc/opcompiler/te_fusion/st";
  }
  ...
  std::string jsonPathMixJson = currentFilePath + "/test_files/kernel_meta/matmul_add_1.json";
  std::string jsonPath = currentFilePath + "/test_files/kernel_meta/te_matmul_1.json";
...
TEST(FuzzyBuildSTest, fusion_serial_ParseOpBuildResult) {
  std::string currentFilePath = GetCodeDir() + "/tests/engines/nn_engine/st/testcase/op_compile_adapter";
  if (te::fusion::RealPath(currentFilePath).empty()) {
    currentFilePath = "../../../../../../../../../llt/atc/opcompiler/te_fusion/st";
  }
  ...
  EXPECT_EQ(compileInfo, "\"1234567890\"");
  EXPECT_EQ(compileInfoKey, "c775e7b757ede630cd0aa1113bd102661ab38829ca52a6422ab782862f268646");
}

TEST(FuzzyBuildSTest, fusion_serial_ParseResult) {
  std::string currentFilePath = GetCodeDir() + "/tests/engines/nn_engine/st/testcase/op_compile_adapter";
  if (te::fusion::RealPath(currentFilePath).empty()) {
    currentFilePath = "../../../../../../../../../llt/atc/opcompiler/te_fusion/st";
  }
  ...
  te::fusion::PythonAdapterManager::ParseResult(opResPreCompile.c_str(), 0, jsonFilePath, pattern, coreType,
                                                compileInfo, compileInfoKey, fusionCheckRes);

  EXPECT_EQ(pattern, "Opaque");
  EXPECT_EQ(coreType, "AiCore");

  std::string opResPreCompile_bad =
      "{\
        \"core_type\":\"AiCore\"\
        }";
  te::fusion::PythonAdapterManager::ParseResult(opResPreCompile_bad.c_str(), 0, jsonFilePath, pattern, coreType,
                                                compileInfo, compileInfoKey, fusionCheckRes);
  ...
  te::fusion::PythonAdapterManager::ParseResult(opResCompile.c_str(), 1, jsonFilePath, pattern, coreType, compileInfo,
                                                compileInfoKey, fusionCheckRes);

  EXPECT_EQ(jsonFilePath, "xxx");

  currentFilePath = GetCodeDir() + "/tests/engines/nn_engine/st/testcase/op_compile_adapter";
  if (te::fusion::RealPath(currentFilePath).empty()) {
    currentFilePath = "../../../../../../../../../llt/atc/opcompiler/te_fusion/st";
  }
  currentFilePath = te::fusion::RealPath(currentFilePath);
  ...
  jsonPath = currentFilePath + "/test_files/kernel_meta/te_matmul_1.json";
  ...
  EXPECT_EQ(jsonFilePath, jsonPath);
  ...
  EXPECT_EQ(fusionCheckRes, 0);
}

TEST(FuzzyBuildSTest, OutputsDescToJsonProcess_001) {
...
TEST(FuzzyBuildSTest, OutputsDescToJsonProcess_002) {
  ComputeGraphPtr owner_graph = std::make_shared<ComputeGraph>("te");
  ge::OpDescPtr opDescPtr = std::make_shared<ge::OpDesc>("lambNextMVWithDecay", "LambNextMVWithDecay");
  std::vector<std::pair<int64_t, int64_t>> range;
  std::vector<std::pair<int64_t, int64_t>> range1 = {{1, 10}, {10, 20}, {20, 30}, {30, 40}};
...
TEST(FuzzyBuildSTest, OutputsDescToJsonProcess_003) {
  bool res = false;
  std::vector<std::pair<int64_t, int64_t>> range;
  ...
  TeJsonAssemble::OutputsDescToJsonProcess(json_str, allNodes, outputJsonPara, idx, nodeData0->GetOutDataAnchor(0), 1,
                                           nodes);
  ...
  TeJsonAssemble::OutputsDescToJsonProcess(json_str, allNodes1, outputJsonPara, idx, nodeData0->GetOutDataAnchor(0), 1,
                                           nodes);
}

TEST(FuzzyBuildSTest, FuzzyBuildSecondaryGeneralization_001) {
  bool res = false;
  uint64_t graphId = 100;
  uint64_t taskId = static_cast<uint64_t>(rand());
  uint64_t sgtThreadIndex = 300;
  ...
  for (size_t i = 0; i < inputs.size(); i++) {
    ...
    for (size_t j = 0; j < tensors.size(); j++) {
      ...
      if (i == 0) {
        ...
        shapeAndRangeRes = (shape == newShape && oriShape == newShape && shapeRange == newShapeRange &&
                            oriShapeRange == newShapeRange);
      }
...
TEST(FuzzyBuildSTest, ParseGeneralizedResultFromTbe_failed) {
  nlohmann::json generalizedRes;
  nlohmann::json generalizedInfo;
  bool res =
      te::fusion::BinaryManager::Instance().ParseGeneralizedResultFromTbe(generalizedRes, "matmul", generalizedInfo);
  EXPECT_EQ(res, false);
  nlohmann::json temp;
  temp["result"] = "test";
  generalizedRes.push_back(temp);
  res = te::fusion::BinaryManager::Instance().ParseGeneralizedResultFromTbe(generalizedRes, "matmul", generalizedInfo);
  EXPECT_EQ(res, false);
}

TEST(FuzzyBuildSTest, SetShapeAndValueToTensor_failed) {
  nlohmann::json singleGeneralizeRes;
...
TEST(FuzzyBuildSTest, GeneralizeWitLimitedRule) {
