...
int PyArg_Parse_Stub(PyObject *args, const char *format, ...) {
  ...
  std::cout << "PyArg_Parse args->func:" << args->func << ",args->res:" << args->res << ",testPath:" << testPath
            << std::endl;
  if (args->func == "is_generalize_func_register_from_c" || args->func == "cann_kb_init" ||
      args->func == "cann_kb_finalize") {
  ...
  if (args->func == "get_file_sha256_hash_from_c" || args->func == "get_str_sha256_hash_from_c") {
    char **generalizeResult = va_arg(ap, char **);
    ...
    } else if (args->res == testPath +
                                "/st/disk_cache/kernel_meta/"
                                "te_matmul_2ac1b9067af400eb161d75ccd6c6ca71bed7b97750d79c4476fb65ac9bedd762.o") {
      *generalizeResult = hashfile;
    } else if (args->res == testPath +
                                "/ut/disk_cache/kernel_meta/"
                                "te_matmul_2ac1b9067af400eb161d75ccd6c6ca71bed7b97750d79c4476fb65ac9bedd762.o") {
      *generalizeResult = hashfile;
    } else {
      *generalizeResult = succStub;
...
PyObject *PyObject_CallMethod_Stub(PyObject *obj, const char *name, const char *format, ...) {
  ...
  if (std::string("get_file_sha256_hash_from_c") == name || std::string("get_str_sha256_hash_from_c") == name ||
...
PyObject *PyObject_CallMethod_Stub_ForBinary(PyObject *obj, const char *name, const char *format, ...) {
  ...
  if (std::string("get_file_sha256_hash_from_c") == name || std::string("get_op_args_json") == name ||
      std::string("get_binfile_sha256_hash_from_c") == name) {
    tmpPyObj.func = name;
    return &tmpPyObj;
  }
