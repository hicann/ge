...
TEST(TeFusionUTest, CheckNodeList) {
  ...
  OpDescPtr relu_op =
      CreateOpDesc("relu", "Relu", 0, "RT_DEV_BINARY_MAGIC_ELF", 123, inputnameList, 1, inputdescList, outputdescList);
  OpDescPtr sqrt_op =
      CreateOpDesc("sqrt", "Sqrt", 1, "RT_DEV_BINARY_MAGIC_ELF", 123, inputnameList, 1, inputdescList, outputdescList);
...
TEST(TeFusionUTest, GenerateSocInfoJsonPcieThrough) {
  OpDescPtr op = std::make_shared<ge::OpDesc>("TestOp", "TestOp");
  ComputeGraphPtr graphPtr = std::make_shared<ComputeGraph>("test");
  NodePtr node = graphPtr->AddNode(op);
  TbeOpInfoPtr opInfo = std::make_shared<TbeOpInfo>("TestOp", "", "TestOp", "");
...
TEST(TeFusionUTest, GenerateOptionsMapPcieThrough) {
  OpDescPtr op = std::make_shared<ge::OpDesc>("TestOp", "TestOp");
  ComputeGraphPtr graphPtr = std::make_shared<ComputeGraph>("test");
  NodePtr node = graphPtr->AddNode(op);
  TbeOpInfoPtr opInfo = std::make_shared<TbeOpInfo>("TestOp", "", "TestOp", "");
...
TEST(TeFusionUtil, FilterJsonValueFromJsonListByKey_will_filter) {
  ...
  EXPECT_TRUE(ret);
  for (auto n : testJson) {
    EXPECT_EQ(n["key"], string("1")) << "Json node(" << n.dump() << ") not match key.";
  }
}
