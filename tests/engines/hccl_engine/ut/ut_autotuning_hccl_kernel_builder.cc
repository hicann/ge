...
static nlohmann::json allreduce_topo_switch_connect = {{"topology type", "switch connection"},
                                                       {"topology desc",
                                                        {{{"node type", "TOR"},
                                                          {"node name", "tor0"},
                                                          {"link info",
...
class AutoTuningHcclKernelBuilderTest : public testing::Test {
 protected:
  static void SetUpTestCase() {
    ...
    char file_name[] = "./ut_AutoTuningHcclKernelBuilderTest.json";
...
TEST_F(AutoTuningHcclKernelBuilderTest, ut_CalcOpRunningParam) {
...
TEST_F(AutoTuningHcclKernelBuilderTest, ut_generateTask) {
  ge::NodePtr nodeptr(new NodeTest);
