...
TEST_F(HcomLoweringTest, ut_hcomLaunchKernel_reducescatter)
{
    ...
    *(allGatherContext->GetContext()) = context;
    ...
    MOCKER(HcomLaunchReduceScatterKernel)
    .stubs()
    .with(mockcpp::any())
    .will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcomLoweringTest, ut_hcomLaunchKernel_allGatherv2_When_Normal_Expect_ReturnlsHCCL_SUCCESS)
{
    ...
    MOCKER(HcomHcclAllGather).stubs().with(mockcpp::any()).will(returnValue(HCCL_SUCCESS));
    ...
    *(allGatherContext->GetContext()) = context;
    ...
    MOCKER(HcomLaunchAllGatherKernelV2)
    .stubs()
    .with(mockcpp::any())
    .will(returnValue(HCCL_SUCCESS));
