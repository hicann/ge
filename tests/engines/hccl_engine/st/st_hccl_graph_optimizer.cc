...
TEST_F(HcomGraphOptimizerTest, st_optimize_graphprepare_SetHcomOpParallelLabel) {
  ge ::Status ret;
  ge ::Status ge_ret;
  bool bRet;
...
TEST_F(HcomGraphOptimizerTest, st_Initialize_to_Finalize) {
  ge ::Status ret;
  std::map<std::string, std::string> options;
...
TEST_F(HcomGraphOptimizerTest, st_Initialize_to_Finalize_51) {
  ge ::Status ret;
  std::map<std::string, std::string> options;
...
TEST_F(HcomGraphOptimizerTest, st_OptimizeFusedGraph_GetDeterministic) {
...
TEST_F(HcomGraphOptimizerTest, st_GetFusionOpInfo) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
...
TEST_F(HcomGraphOptimizerTest, st_FuseOps) {
  HcclResult ret;
  bool bRet;
  ge::ComputeGraph graph("test_graph");
...
TEST_F(HcomGraphOptimizerTest, st_GetFusionStrategy) {
  HcclResult ret;
  bool bRet;
  ge::ComputeGraph graph("test_graph");
  HcomAllReduceFusion fusionHcomAllReduceOp;
...
TEST_F(HcomGraphOptimizerTest, st_GetNodeUnknownShapeInfo_known) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
  HcomAllReduceFusion fusionHcomAllReduceOp;
  std::vector<ge::NodePtr> fusionOps(3);
...
TEST_F(HcomGraphOptimizerTest, st_GetNodeUnknownShapeInfo_unknown) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
  HcomAllReduceFusion fusionHcomAllReduceOp;
  std::vector<ge::NodePtr> fusionOps(3);
...
TEST_F(HcomGraphOptimizerTest, st_RunFusionOps) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
  HcomAllReduceFusion fusionHcomAllReduceOp;
  std::vector<ge::NodePtr> fusionOps(3);
...
TEST_F(HcomGraphOptimizerTest, st_RunFusionOps_have_duplication) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
  HcomAllReduceFusion fusionHcomAllReduceOp;
  std::vector<ge::NodePtr> fusionOps(3);
...
TEST_F(HcomGraphOptimizerTest, st_RunFusionOps_bcast) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
...
TEST_F(HcomGraphOptimizerTest, st_HcomReduceScatter_OptimizeFusedGraph) {
  ge ::Status ret;
  std::map<std::string, std::string> options;
...
TEST_F(HcomGraphOptimizerTest, st_HcomAllGather_OptimizeFusedGraph) {
  ge ::Status ret;
  std::map<std::string, std::string> options;
...
TEST_F(HcomGraphOptimizerTest, st_HcomRemoteRead_OptimizeFusedGraph) {
  ge ::Status ret;
  std::map<std::string, std::string> options;
...
TEST_F(HcomGraphOptimizerTest, st_HcomReveive_OptimizeFusedGraph) {
  ge ::Status ret;
  std::map<std::string, std::string> options;
...
TEST_F(HcomGraphOptimizerTest, st_OriginalGraphShapeTypeCfg) {
  ge::Status ge_ret;
  MOCKER(&ge::NodeUtils::GetNodeUnknownShapeStatus)
      .stubs()
...
TEST_F(HcomGraphOptimizerTest, st_SetUnknownShapAttr) {
  HcclResult ret;
...
TEST_F(HcomGraphOptimizerTest, st_SetUnknownShapAttr_AlltoAllv) {
  HcclResult ret;
  int64_t memSize = 200 * 1024 * 1024 + 1;
  HcomGraphOptimizer graphOptimizer;
  ge::ComputeGraphPtr graph = std::make_shared<ge::ComputeGraph>("test_graph");
...
TEST_F(HcomGraphOptimizerTest, st_GetFusionOpInfo_Bcast) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
  HcomBroadcastFusion fusionHcomBroadcastOp;
  FusionInfos fusionOps;
...
TEST_F(HcomGraphOptimizerTest, st_FuseOps_Bcast) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
  HcomBroadcastFusion fusionHcomBroadcastOp;
  std::map<std::string, std::vector<ge::NodePtr>> fusionOps;
...
TEST_F(HcomGraphOptimizerTest, st_RunFusionOps_reduce) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
...
TEST_F(HcomGraphOptimizerTest, st_GetFusionOpInfo_Reduce) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
  HcomReduceFusion fusionHcomReduceOp;
  FusionInfos fusionOps;
...
TEST_F(HcomGraphOptimizerTest, st_FuseOps_Reduce) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
  HcomReduceFusion fusionHcomReduceOp;
  std::map<std::string, std::vector<ge::NodePtr>> fusionOps;
...
TEST_F(HcomGraphOptimizerTest, st_GetFusionSegments_1) {
  std::vector<ge::NodePtr> ops(7);
...
TEST_F(HcomGraphOptimizerTest, st_GetFusionSegments_2) {
  std::vector<ge::NodePtr> ops(3);
  std::vector<ge::NodePtr> nodes;
  nodes.push_back(ops[0]);
  nodes.push_back(ops[1]);
...
TEST_F(HcomGraphOptimizerTest, st_OptimizeFusedGraph_allreduce) {
  ge::ComputeGraphPtr graph = std::make_shared<ge::ComputeGraph>("test_graph");
  ge::OpDesc op;
  const string type = HCCL_KERNEL_OP_TYPE_ALLREDUCE;
  op.SetType(type);
...
TEST_F(HcomGraphOptimizerTest, st_OptimizeFusedGraph_broadcast) {
  ge::ComputeGraphPtr graph = std::make_shared<ge::ComputeGraph>("test_graph");
  ge::OpDesc op;
...
TEST_F(HcomGraphOptimizerTest, st_OptimizeFusedGraph_broadcast_unknown) {
  ge::ComputeGraphPtr graph = std::make_shared<ge::ComputeGraph>("test_graph");
  ge::OpDesc op;
  const string type = HCCL_KERNEL_OP_TYPE_BROADCAST;
  op.SetType(type);
...
TEST_F(HcomGraphOptimizerTest, st_GetCommFromOpDesc_by_group_pytorch) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
...
TEST_F(HcomGraphOptimizerTest, st_GetFusionOpInfo_Reduce_by_comm_pytorch) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
  HcomReduceFusion fusionHcomReduceOp;
  FusionInfos fusionOps;
...
TEST_F(HcomGraphOptimizerTest, st_GetFusionOpInfo_AllReduce_by_comm_pytorch) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
  HcomAllReduceFusion fusionHcomAllReduceOp;
  FusionInfos fusionOps;
...
TEST_F(HcomGraphOptimizerTest, st_FuseOps_AllReduce_by_comm_pytorch) {
  HcclResult ret;
  bool bRet;
  ge::ComputeGraph graph("test_graph");
...
TEST_F(HcomGraphOptimizerTest, st_RunFusionOpss_allreduce_by_comm_pytorch) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
...
TEST_F(HcomGraphOptimizerTest, st_GetFusionOpInfo_Bcast_by_comm_pytorch1) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
  HcomBroadcastFusion fusionHcomBroadcastOp;
  FusionInfos fusionOps;
...
TEST_F(HcomGraphOptimizerTest, st_GetFusionOpInfo_Bcast_by_comm_pytorch2) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
  HcomBroadcastFusion fusionHcomBroadcastOp;
  FusionInfos fusionOps;
...
TEST_F(HcomGraphOptimizerTest, st_GetCommFromOpDesc_by_comm_pytorch1) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
  HcomGraphOptimizer hcomGraphOptimizer;
  std::vector<ge::NodePtr> ops(1);
...
TEST_F(HcomGraphOptimizerTest, st_GetCommFromOpDesc_by_comm_pytorch2) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
  HcomGraphOptimizer hcomGraphOptimizer;
  std::vector<ge::NodePtr> ops(1);
...
TEST_F(HcomGraphOptimizerTest, st_HcomCalcOpRunningParam_by_comm_pytorch) {
  HcclResult ret;
  ge::ComputeGraph graph("test_graph");
  HcomGraphOptimizer hcomGraphOptimizer;
...
TEST_F(HcomGraphOptimizerTest, st_FusionVersion) {
  HcclResult ret;
...
TEST_F(HcomGraphOptimizerTest, st_FusionPath_Tune) {
  HcclResult ret;
  HcomOpUtils ops;
...
TEST_F(HcomGraphOptimizerTest, st_CalculateSegmentIndex) {
  ...
  char file_name_t[] = "./st_hcom_get_new_rank_info_muti_ip.json";
  ...
  u32 localrankid = 0;
  u32 localranksize = 0;
  char *rank_table_file = "./st_hcom_get_new_rank_info_muti_ip.json";
  char *rank_ID = "0";
...
TEST_F(HcomGraphOptimizerTest, st_CalculateSizeRatio) {
...
TEST_F(HcomGraphOptimizerTest, st_CalculateSegmentIndexFromHomeExport) {
  HcclResult ret;
  ...
  MOCKER_CPP(&HcomAllReduceFusion::GetPathFromDefault).stubs().with(mockcpp::any()).will(returnValue(HCCL_SUCCESS));
...
ge::graphStatus FakeGetOption3(ge::GEThreadLocalContext *that, const std::string &optionExec,
...
TEST_F(HcomGraphOptimizerTest, st_offlinebuild_calcSubStreamNum) {
  ge::Status ret;
  HcomGraphOptimizer graphOptimizer;
...
TEST_F(HcomGraphOptimizerTest, st_FuseHcomAlltoAllVCNode) {
...
TEST_F(HcomGraphOptimizerTest, st_RunFusionOpsAlltoAllVC) {
...
TEST_F(HcomGraphOptimizerTest, st_FuseHcomAllGatherNode) {
...
TEST_F(HcomGraphOptimizerTest, st_RunFusionOpsAllGather) {
...
TEST_F(HcomGraphOptimizerTest, st_FuseHcomReduceScatterNode) {
...
TEST_F(HcomGraphOptimizerTest, st_FuseHcomReduceScatterNode1) {
  MOCKER_CPP(&HcomReduceScatterFusion::RunFusionOpsReduceScatter).stubs().will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcomGraphOptimizerTest, st_FuseHcomReduceNode1) {
...
TEST_F(HcomGraphOptimizerTest, st_FuseHcomReduceNode2) {
  MOCKER_CPP(&HcomReduceScatterFusion::RunFusionOpsReduce).stubs().will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcomGraphOptimizerTest, st_RunFusionReduceScatter) {
...
TEST_F(HcomGraphOptimizerTest, st_SetknownShapAttr) {
  HcclResult ret;
  int64_t memSize = 200 * 1024 * 1024 + 1;
  HcomGraphOptimizer graphOptimizer;
...
TEST_F(HcomGraphOptimizerTest, st_getAlltoAllCountsDispl_sendCountMatrix) {
  ge::NodePtr nodeptr(new NodeTest);
...
ge::graphStatus TaskNumGetOption_1(ge::GEThreadLocalContext *that, const std::string &optionExec,
                                   std::string &dumpDebugValue) {
  nlohmann::json group_list = {{{"group_name", "aa"}, {"group_rank_list", {0, 1}}},
  ...
  if (optionExec == ge::OPTION_EXEC_HCOM_GROUPLIST) {
    ...
    dumpDebugValue = "./llt/ace/comop/hccl/stub/workspace/st_task_num_one_server_hcom_test.json";
...
TEST_F(HcomGraphOptimizerTest, st_CalcOpTaskNum_1server_1) {
  HcclResult ret;
  nlohmann::json rank_table = rank_table_1server_8rank;
  char file_name_t[] = "./llt/ace/comop/hccl/stub/workspace/st_task_num_one_server_hcom_test.json";
  std::ofstream outfile(file_name_t, std::ios::out | std::ios::trunc | std::ios::binary);
  ...
  char *rank_table_file = "./llt/ace/comop/hccl/stub/workspace/st_task_num_one_server_hcom_test.json";
  char *rank_ID = "0";
...
TEST_F(HcomGraphOptimizerTest, st_HcomOptimizeOriginalGraph) {
  MOCKER_CPP(&HcomAllGatherFusion::RunFusionOpsAllGather).stubs().will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcomGraphOptimizerTest, st_OptimizeOriginalGraphDynamicGraphNoSuperkernel) {
  ge::Status ge_ret;
  MOCKER(&ge::NodeUtils::GetNodeUnknownShapeStatus)
      .stubs()
      .with(mockcpp::any(), outBound(true))
...
TEST_F(HcomGraphOptimizerTest, st_offlinebuild_calcSubStreamNum_1) {
  HcomGraphOptimizer graphOptimizer;
  ge::NodePtr nodeptr(new NodeTest);
...
TEST_F(HcomGraphOptimizerTest, st_GetInfoFromContentedLibrary_When_AllParamsValid_Expect_CorrectSegmentIndex) {
...
TEST_F(HcomGraphOptimizerTest, st_GetFusionOption_When_FusionAttrNoFusion_Expect_Success) {
  ge::NodePtr nodeptr(new NodeTest);
