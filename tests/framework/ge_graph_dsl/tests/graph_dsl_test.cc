...
TEST_F(GraphDslTest, test_add_output) {
  ...
  const auto &out_nodes_and_indexes = compute_graph->GetGraphOutNodesInfo();
  EXPECT_EQ(out_nodes_and_indexes.size(), 2);
  EXPECT_EQ(out_nodes_and_indexes[0].first->GetName(), "data1");
  EXPECT_EQ(out_nodes_and_indexes[0].second, 0);
  EXPECT_EQ(out_nodes_and_indexes[1].first->GetName(), "data3");
