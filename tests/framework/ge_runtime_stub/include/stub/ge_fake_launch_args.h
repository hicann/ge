...
struct GeFakeLaunchArgs {
  ...
  GeFakeLaunchArgs(const void *handle, uint32_t blockDim, rtArgsEx_t *args, rtSmDesc_t *smDesc, rtStream_t stream,
                   uint32_t flag, std::unique_ptr<std::string> tag);
  GeFakeLaunchArgs(const void *kernel_name, uint32_t block_dim, const void *args, uint32_t args_size, rtStream_t stream,
  ...
  GeFakeLaunchArgs(const rtFuncHandle funcHandle, const uint32_t blockDim, rtStream_t stm,
                   ...
                   std::unique_ptr<std::string> tag);
  GeFakeLaunchArgs(aclrtFuncHandle funcHandle, uint32_t numBlocks, aclrtStream stream, aclrtLaunchKernelCfg *cfg,
                   void *hostArgs, size_t argsSize, aclrtPlaceHolderInfo *placeHolderArray, size_t placeHolderNum,
                   std::unique_ptr<std::string> tag);
  GeFakeLaunchArgs(aclrtFuncHandle funcHandle, uint32_t numBlocks, const void *argsData, size_t argsSize,
                   aclrtLaunchKernelCfg *cfg, aclrtStream stream, std::unique_ptr<std::string> tag);
