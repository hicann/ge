...
class GlobalDataFaker {
 public:
  GlobalDataFaker() {};
  explicit GlobalDataFaker(const ge::GeRootModelPtr &root_model)
      : root_model_(root_model), graph_(root_model->GetRootGraph()) {}
  ...
  GlobalDataFaker &FakeWithHandleAiCore(const std::string &node_type, bool need_atomic, bool need_aicpu = false) {
    node_types_to_faker_[node_type] =
        std::unique_ptr<AiCoreTaskDefFaker>(new AiCoreTaskDefFaker(true, need_atomic, "", need_aicpu));
    return *this;
  }
  GlobalDataFaker &FakeWithoutHandleAiCore(const std::string &node_type, bool need_atomic, bool need_aicpu = false) {
    node_types_to_faker_[node_type] = std::unique_ptr<AiCoreTaskDefFaker>(
        new AiCoreTaskDefFaker(false, need_atomic, node_type + "StubName", need_aicpu));
    return *this;
  }
  ...
  void BuildWithKnownSubgraph(LoweringGlobalData &global_data) {
    ...
    for (const auto &subgraph : graph_->GetAllSubgraphs()) {
      ...
      if (subgraph->GetParentNode()->GetType() != ge::PARTITIONEDCALL &&
          (!subgraph->HasAttr("_stub_force_known_shape"))) {
        continue;
      }
      bool without_kernel_flag = 0;
      if (ge::AttrUtils::GetBool(subgraph->GetParentNode()->GetOpDesc(), "without_kernel_store", without_kernel_flag)) {
        continue;
      }
      ...
      model->SetAttr(ge::ATTR_NAME_GRAPH_FLATTEN_OFFSET, ge::GeAttrValue::CreateFrom<std::vector<int64_t>>(
                                                             {offset, static_cast<int64_t>(model->GetWeightSize())}));
  ...
  LoweringGlobalData Build(size_t stream_num = 1) {
    ...
    if (bg::ValueHolder::GetCurrentFrame() != nullptr) {
      ...
      global_data.SetExternalAllocator(
          bg::ValueHolder::CreateFeed(static_cast<int64_t>(ExecuteArgIndex::kExternalAllocator)));
    }
