...
TEST_F(UtestTensorflowParser, tensorflow_parser_with_serialized_proto1) {
  ...
  ge::graphStatus ret = model_parser->ParseProtoWithSubgraph(
      std::string(""), [](std::string) -> std::string { return ""; }, compute_graph);
...
TEST_F(UtestTensorflowParser, tensorflow_parser_with_serialized_proto2) {
  ...
  auto model_parser = domi::ModelParserFactory::Instance()->CreateModelParser(domi::TENSORFLOW);
  ge::graphStatus ret = model_parser->ParseProtoWithSubgraph(
      std::string("null"), [](std::string) -> std::string { return ""; }, compute_graph);
  EXPECT_NE(ret, ge::SUCCESS);
}
...
TEST_F(UtestTensorflowParser, tensorflow_parser_with_serialized_proto3) {
  ...
  ge::graphStatus ret = model_parser->ParseProtoWithSubgraph(
      graph_def.SerializeAsString(), [](std::string) -> std::string { return ""; }, compute_graph);
...
TEST_F(UtestTensorflowParser, parser_ParseProtoWithSubgraphWithConstValue) {
  ...
  tensor_proto.set_dtype(domi::tensorflow::DT_FLOAT);
  tensor_proto.add_float_val(2.0);
  std::string tensor_str = tensor_proto.SerializeAsString();
  const_value_map.insert({"const1", ge::AscendString(tensor_str.c_str(), tensor_str.size())});
  ...
  for (const auto &node : root_graph->GetDirectNode()) {
    if (node->GetType() == "Constant") {
      ...
      EXPECT_EQ(ge::AttrUtils::GetTensor(opdesc, "value", tensor), true);
    }
  }
}
...
std::unique_ptr<google::protobuf::Message> getGraphCallback(const google::protobuf::Message *root_proto,
...
TEST_F(UtestTensorflowParser, tensorflow_graph_functiondef_BuildFunctionDef_test) {
  ...
  for (ge::NodePtr node : subGraph->GetAllNodes()) {
    for (auto in : node->GetAllInDataAnchors()) {
      if (in->GetPeerOutAnchor() != nullptr &&
          in->GetPeerOutAnchor()->GetOwnerNode()->GetOpDesc()->GetType() == parser::DATA) {
  ...
  Status ret =
      GraphToFunctionDef::BuildFunctionDef(subGraph, "fusionop", &library, &call_node_def, in_anchor, out_anchor);
...
TEST_F(UtestTensorflowParser, Tensorflow_UpdateFusionOpContext_test) {
  ...
  Status ret =
      tensorflow_parser.UpdateFusionOpContext(scope_graph, info, fusion_op_node_context, normal_op_node_context);
  EXPECT_EQ(ret, domi::SUCCESS);
...
TEST_F(UtestTensorflowParser, tensorflow_UpdateEdgesControlInfo_test) {
  ...
  EXPECT_EQ(model_parser.edges_control_map.size(), 0U);
}
...
TEST_F(UtestTensorflowParser, tensorflow_AddControlEdgeAfterRemoveInputs_test) {
  ...
  Status ret =
      tensorflow_parser.AddControlEdgeAfterRemoveInputs(&graph_def, node_def, all_node_map, removed_inputs_vec);
  EXPECT_EQ(ret, SUCCESS);
...
TEST_F(UtestTensorflowParser, tensorflow_optimizer_snapshot_no_retval_test) {
  ...
  bool protoRet = parser::ReadProtoFromBinaryFile(root_proto.c_str(), &graphDef);
...
TEST_F(UtestTensorflowParser, tensorflow_UpdateInnerInputMap_test) {
  ...
  EXPECT_NE(model_parser.op_node_context_map_[fusion_op_name].output_map.size(), 0U);
}
...
TEST_F(UtestTensorflowParser, tensorflow_UpdateInnerOutputMap_test) {
  ...
  EXPECT_NE(model_parser.op_node_context_map_[fusion_op_name].input_map.size(), 0U);
}
...
TEST_F(UtestTensorflowParser, tensorflow_ParserNodeDef1_test) {
  ...
  Status ret = ge::TensorFlowModelParser::ParseNodeDef(&tensorflow_parser, compute_graph, &graphMutex, scope_graph,
                                                       &node_def, error_context);
  EXPECT_EQ(FAILED, ret);
  delete graph;
}
...
TEST_F(UtestTensorflowParser, success_frameworkop_get) {
  ...
  frameworkOp->Frameworktype(0);
  frameworkOp->NodeDefPkg("tf");
  frameworkOp->FuncDefPkg("func");
  frameworkOp->Index(index);
  frameworkOp->TfOpDef(opdef_string);
  EXPECT_EQ(frameworkOp->GetFrameworkType(), 0);
  EXPECT_EQ(frameworkOp->GetNodeDefPkg(), "");
  delete frameworkOp;
}
...
TEST_F(UtestTensorflowParser, op_set_get_success) {
  ...
  vector<int64_t> v;
  op.VectorAttr("key", v);
  ge::DataType dt = op.GetDType();
  ASSERT_EQ(dt, ge::DataType::DT_FLOAT);
}
...
TEST_F(UtestTensorflowParser, success_operator) {
  ...
  EXPECT_NE(tfOperator.op_attrs_.size(), 0U);
}

TEST_F(UtestTensorflowParser, success_shapen_get) {
  EXPECT_NO_THROW(ShapeNOperator *shapen = new ShapeNOperator(); shapen->GetInType(); shapen->GetInType();
                  shapen->GetOutType(); delete shapen;);
}
...
TEST_F(UtestTensorflowParser, success_VarIsInitializedOpOperator_get) {
  ...
  EXPECT_NE(op.op_attrs_.size(), 0U);
}
...
TEST_F(UtestTensorflowParser, param_success_get) {
  EXPECT_NO_THROW(FillOperator *fillOp = new FillOperator(); fillOp->GetDataType(); fillOp->GetAlpha();
                  fillOp->GetBeta(); delete fillOp;);
}
...
TEST_F(UtestTensorflowParser, tensorflow_Pb2Json_TypeBytes2String_test) {
  ...
  EXPECT_EQ(toJson.TypeBytes2String(field_name, type_bytes), type_bytes);
...
TEST_F(UtestTensorflowParser, ProtoFileParser_CombineProtoFileMultiCustomProto_not_exists) {
  ...
  auto ret = proto_file_parser.CombineProtoFileMultiCustomProto(caffe_proto_path.c_str(), custom_proto_path.c_str(),
                                                                fusion_proto_file);
  EXPECT_EQ(ret, FAILED);
}
...
message LayerParameter {
  ...
  ProtoFileParser proto_file_parser((proto_dir + "fusion_proto_file.proto").c_str());
  std::string fusion_proto_file;
  auto ret = proto_file_parser.CombineProtoFileMultiCustomProto(
      caffe_proto_path.c_str(), (custom_proto_path_02 + ":" + custom_proto_path_01).c_str(), fusion_proto_file);
  EXPECT_EQ(ret, SUCCESS);
  EXPECT_EQ(fusion_proto_file, proto_dir + "fusion_proto_file.proto");
  EXPECT_EQ(read_file(fusion_proto_file), R"(syntax = "proto2";
package domi::caffe;
...
message LayerParameter {
  ...
  ProtoFileParser proto_file_parser((proto_dir + "fusion_proto_file.proto").c_str());
  std::string fusion_proto_file;
  auto ret = proto_file_parser.CombineProtoFileMultiCustomProto(caffe_proto_path.c_str(), "", fusion_proto_file);
  EXPECT_EQ(ret, SUCCESS);
...
TEST_F(UtestTensorflowParser, tensorflow_parser_api_test) {
  REGISTER_CUSTOM_OP("Add11").FrameworkType(domi::TENSORFLOW).OriginOpType("Add11").ParseParamsFn(ParseParamsStub1);
  std::map<ge::AscendString, ge::AscendString> options = {{"ge.runFlag", "1"},
                                                          {ge::OPTION_EXEC_ENABLE_SCOPE_FUSION_PASSES, "some_pass"}};
  options.insert(std::pair<ge::AscendString, ge::AscendString>(ge::AscendString(ge::FRAMEWORK_TYPE.c_str()),
...
TEST_F(UtestTensorflowParser, tensorflow_FP16_parser_test) {
  ...
  int8_t i_val = 1;
  fp16.operator=(i_val);
  EXPECT_EQ(fp16.ToInt8(), i_val);

  uint8_t ui_val = 2;
  fp16.operator=(ui_val);
  EXPECT_EQ(fp16.ToUInt8(), ui_val);

  int16_t i_vals = 1;
  fp16.operator=(i_vals);
  EXPECT_EQ(fp16.ToInt16(), i_vals);
  i_vals = 5000;
  fp16.operator=(i_vals);
  EXPECT_EQ(fp16.ToInt16(), i_vals);
  i_vals = 0;
  ...
  EXPECT_EQ(fp16.ToUInt16(), ui16_val);
  ui16_val = 0;
  fp16.operator=(ui16_val);
  EXPECT_EQ(fp16.ToUInt16(), ui16_val);
  ui16_val = 1;
  ...
  EXPECT_EQ(fp16.ToInt32(), i32_val);
  i32_val = 1;
  fp16.operator=(i32_val);
  EXPECT_EQ(fp16.ToInt32(), i32_val);
  i32_val = 5000;
  ...
  EXPECT_EQ(fp16.ToUInt32(), ui32_val);
  ui32_val = 1;
  fp16.operator=(ui32_val);
  EXPECT_EQ(fp16.ToUInt32(), ui32_val);
  ui32_val = 5000;
...
TEST_F(UtestTensorflowParser, parser_LinkInnerAnchor_test) {
  ...
  ParserGraphOptimizer graphOptimizer(subGraph, domi::TENSORFLOW);
  Status ret = graphOptimizer.LinkInnerAnchor(node_map);
  EXPECT_NE(ret, SUCCESS);
}
...
TEST_F(UtestTensorflowParser, parser_RebuildFusionNode_test) {
  ...
  Status ret = graphOptimizer.RebuildFusionNode(input_anchors, output_anchors, output_in_map, input_control_anchors,
...
TEST_F(UtestTensorflowParser, parser_InsertNode_test) {
  ...
  Status ret = graphOptimizer.InsertNode(subGraph, nodes, in_anchor, out_anchor, output_in_map, input_control_anchors,
...
TEST_F(UtestTensorflowParser, tensorflow_OptimizeTranspose) {
  ...
  EXPECT_EQ(transposeInfo.begin()->second.inputIdx, 0);
...
TEST_F(UtestTensorflowParser, tensorflow_SoftmaxAddAttr) {
  ...
  domi::tensorflow::GraphDef graph_def;
  graph_def.add_node();
  EXPECT_NO_THROW(modelParser.SoftmaxAddAttr(&graph_def));
}
...
TEST_F(UtestTensorflowParser, test_plugin_manager_GetOpsProtoPath_09) {
  ...
  std::string custom_opp_path_02 = opp_path + "custom_opp_path_02";
  setenv("ASCEND_OPP_PATH", opp_path.c_str(), 1);
  PluginManager::SetCustomOpLibPath(custom_opp_path_01 + ":" + custom_opp_path_02 + ":" + custom_opp_path_01 + ":" +
                                    custom_opp_path_02);
...
TEST_F(UtestTensorflowParser, test_plugin_manager_GetCustomOpPath_02) {
  ...
  std::string customop_path;
  TBEPluginLoader::GetCustomOpPath(customop_path);
  EXPECT_EQ(customop_path.find(path_vendors + "/customize/framework/:" + path_vendors + "/mdc/framework/:" +
                               path_vendors + "/lhisi/framework/:" + opp_path + "built-in/framework/"),
            0);
  system(("rm -rf " + opp_path).c_str());
}
...
TEST_F(UtestTensorflowParser, test_plugin_manager_GetCustomOpPath_03) {
  ...
  std::string customop_path;
  TBEPluginLoader::GetCustomOpPath(customop_path);
  EXPECT_EQ(customop_path.find(opp_path + "framework/custom/:" + opp_path + "built-in/framework/"), 0);
  system(("rm -rf " + opp_path).c_str());
}
...
TEST_F(UtestTensorflowParser, test_plugin_manager_GetCustomOpPath_04) {
  ...
  TBEPluginLoader::GetCustomOpPath(customop_path);
  EXPECT_EQ(
      customop_path.find(custom_opp_path + "/framework/:" + path_vendors + "/customize/framework/:" + path_vendors +
                         "/mdc/framework/:" + path_vendors + "/lhisi/framework/:" + opp_path + "built-in/framework/"),
      0);
  system(("rm -rf " + opp_path).c_str());
}
...
TEST_F(UtestTensorflowParser, test_plugin_manager_GetCustomOpPath_06) {
  ...
  TBEPluginLoader::GetCustomOpPath(customop_path);
  EXPECT_EQ(
      customop_path.find(custom_opp_path_01 + "/framework/:" + custom_opp_path_02 + "/framework/:" + path_vendors +
                         "/customize/framework/:" + path_vendors + "/mdc/framework/:" + path_vendors +
                         "/lhisi/framework/:" + opp_path + "built-in/framework/"),
      0);
  system(("rm -rf " + opp_path).c_str());
}
...
TEST_F(UtestTensorflowParser, test_plugin_manager_GetCustomOpPath_07) {
  ...
  std::string customop_path;
  TBEPluginLoader::GetCustomOpPath(customop_path);
  EXPECT_EQ(customop_path.find(custom_opp_path + "/framework/:" + opp_path + "framework/custom/:" + opp_path +
                               "built-in/framework/"),
            0);
  system(("rm -rf " + opp_path).c_str());
}
...
TEST_F(UtestTensorflowParser, test_plugin_manager_GetCustomOpPath_09) {
  ...
  TBEPluginLoader::GetCustomOpPath(customop_path);
  EXPECT_EQ(
      customop_path.find(custom_opp_path_01 + "/framework/:" + custom_opp_path_02 +
                         "/framework/:" + custom_opp_path_01 + "/framework/:" + custom_opp_path_02 +
                         "/framework/:" + path_vendors + "/customize/framework/:" + path_vendors +
                         "/mdc/framework/:" + path_vendors + "/lhisi/framework/:" + opp_path + "built-in/framework/"),
      0);
  system(("rm -rf " + opp_path).c_str());
}
...
TEST_F(UtestTensorflowParser, test_plugin_manager_GetCustomCaffeProtoPath_01) {
  ...
  EXPECT_EQ(ret, SUCCESS);
  EXPECT_EQ(customcaffe_path, opp_path + "framework/custom/caffe/");
  system(("rm -rf " + opp_path).c_str());
}
...
TEST_F(UtestTensorflowParser, test_plugin_manager_GetCustomCaffeProtoPath_02) {
  ...
  Status ret = TBEPluginLoader::GetCustomCaffeProtoPath(customcaffe_path);
  EXPECT_EQ(ret, SUCCESS);
  EXPECT_EQ(customcaffe_path, path_vendors + "/customize/framework/caffe/:" + path_vendors +
                                  "/mdc/framework/caffe/:" + path_vendors + "/lhisi/framework/caffe/");
  system(("rm -rf " + opp_path).c_str());
}
...
TEST_F(UtestTensorflowParser, test_plugin_manager_GetCustomCaffeProtoPath_04) {
  ...
  Status ret = TBEPluginLoader::GetCustomCaffeProtoPath(customcaffe_path);
  EXPECT_EQ(ret, SUCCESS);
  EXPECT_EQ(customcaffe_path, custom_opp_path + "/framework/caffe/:" + path_vendors + "/customize/framework/caffe/:" +
                                  path_vendors + "/mdc/framework/caffe/:" + path_vendors + "/lhisi/framework/caffe/");
  system(("rm -rf " + opp_path).c_str());
}
...
TEST_F(UtestTensorflowParser, test_plugin_manager_GetCustomCaffeProtoPath_06) {
  ...
  Status ret = TBEPluginLoader::GetCustomCaffeProtoPath(customcaffe_path);
  EXPECT_EQ(ret, SUCCESS);
  EXPECT_EQ(customcaffe_path, custom_opp_path_01 + "/framework/caffe/:" + custom_opp_path_02 +
                                  "/framework/caffe/:" + path_vendors + "/customize/framework/caffe/:" + path_vendors +
                                  "/mdc/framework/caffe/:" + path_vendors + "/lhisi/framework/caffe/");
  system(("rm -rf " + opp_path).c_str());
}
...
TEST_F(UtestTensorflowParser, test_plugin_manager_GetCustomCaffeProtoPath_07) {
  ...
  Status ret = TBEPluginLoader::GetCustomCaffeProtoPath(customcaffe_path);
  EXPECT_EQ(ret, SUCCESS);
  EXPECT_EQ(customcaffe_path, custom_opp_path + "/framework/caffe/:" + opp_path + "framework/custom/caffe/");
  system(("rm -rf " + opp_path).c_str());
}
...
TEST_F(UtestTensorflowParser, test_plugin_manager_GetCustomCaffeProtoPath_09) {
  ...
  Status ret = TBEPluginLoader::GetCustomCaffeProtoPath(customcaffe_path);
  EXPECT_EQ(ret, SUCCESS);
  EXPECT_EQ(customcaffe_path, custom_opp_path_01 + "/framework/caffe/:" + custom_opp_path_02 + "/framework/caffe/:" +
                                  custom_opp_path_01 + "/framework/caffe/:" + custom_opp_path_02 +
                                  "/framework/caffe/:" + path_vendors + "/customize/framework/caffe/:" + path_vendors +
                                  "/mdc/framework/caffe/:" + path_vendors + "/lhisi/framework/caffe/");
  system(("rm -rf " + opp_path).c_str());
}
...
TEST_F(UtestTensorflowParser, test_ParseOpAttributes) {
  auto node_def = unique_ptr<NodeDef>(new NodeDef());
  tensorflow::AttrValue value1;
  value1.set_i(1);
  node_def->mutable_attr()->insert({ATTR_NAME_QOS_SERVICE_LABEL, value1});

  tensorflow::AttrValue value2;
  value2.set_b(true);
  node_def->mutable_attr()->insert({"_grappler_do_not_remove", value2});

  auto op_desc = make_shared<OpDesc>("test", "test");
  TensorFlowModelParser::ParseOpAttributes(node_def.get(), op_desc);

  int64_t ret1 = 0L;
  EXPECT_EQ(AttrUtils::GetInt(op_desc, ATTR_NAME_QOS_SERVICE_LABEL, ret1), true);
  EXPECT_EQ(ret1, 1L);
  bool ret2 = false;
...
TEST_F(UtestTensorflowCustomOpParser, ParseCustomOp_OutDirContainsPid) {
...
TEST_F(UtestTensorflowParser, ModelSaver_SaveJsonToFile_success) {
  const Json model = {{"key", "value"}};
  const char *file_path = "test_model_saver.json";
  Status ret = ge::parser::ModelSaver::SaveJsonToFile(file_path, model);
  EXPECT_EQ(ret, SUCCESS);
  remove(file_path);
}

TEST_F(UtestTensorflowParser, ModelSaver_CheckPath_simple_filename) {
  std::string path = "simple_file.txt";
  Status ret = ge::parser::ModelSaver::CheckPath(path);
  EXPECT_EQ(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, ModelSaver_CheckPath_with_directory) {
  std::string path = "test_dir/subdir/file.txt";
  Status ret = ge::parser::ModelSaver::CheckPath(path);
  EXPECT_EQ(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, ModelSaver_CheckPath_too_long) {
  std::string path(4097, 'a');
  Status ret = ge::parser::ModelSaver::CheckPath(path);
  EXPECT_EQ(ret, FAILED);
}

TEST_F(UtestTensorflowParser, ModelSaver_CreateDirectory_empty) {
  std::string path = "";
  int ret = ge::parser::ModelSaver::CreateDirectory(path);
  EXPECT_EQ(ret, -1);
}

TEST_F(UtestTensorflowParser, ModelSaver_CreateDirectory_too_long) {
  std::string path(4097, 'a');
  int ret = ge::parser::ModelSaver::CreateDirectory(path);
  EXPECT_EQ(ret, -1);
}

TEST_F(UtestTensorflowParser, ModelSaver_CreateDirectory_valid) {
  std::string path = "test_create_dir_sub";
  int ret = ge::parser::ModelSaver::CreateDirectory(path);
  EXPECT_EQ(ret, 0);
  remove(path.c_str());
}

TEST_F(UtestTensorflowParser, DataOpParser_ParseShape_float_type) {
  std::vector<int64_t> shape = {1, 3, 224, 224};
  ge::OpDescPtr op = std::make_shared<ge::OpDesc>("data1", "Data");
  ge::AttrUtils::SetInt(op, ge::DATA_ATTR_NAME_DATA_TYPE, static_cast<int32_t>(ge::DT_FLOAT));
  DataOpParser opParser;
  Status ret = opParser.ParseShape(shape, op);
  EXPECT_EQ(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, DataOpParser_ParseShape_int32_type) {
  std::vector<int64_t> shape = {1, 3, 224, 224};
  ge::OpDescPtr op = std::make_shared<ge::OpDesc>("data2", "Data");
  ge::AttrUtils::SetInt(op, ge::DATA_ATTR_NAME_DATA_TYPE, static_cast<int32_t>(ge::DT_INT32));
  DataOpParser opParser;
  Status ret = opParser.ParseShape(shape, op);
  EXPECT_EQ(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, DataOpParser_ParseShape_default_type) {
  std::vector<int64_t> shape = {1, 3, 224, 224};
  ge::OpDescPtr op = std::make_shared<ge::OpDesc>("data3", "Data");
  DataOpParser opParser;
  Status ret = opParser.ParseShape(shape, op);
  EXPECT_EQ(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, DataOpParser_ParseShape_5d_format) {
  std::vector<int64_t> shape = {1, 3, 224, 224};
  ge::OpDescPtr op = std::make_shared<ge::OpDesc>("data4", "Data");
  ge::GetParserContext().input_nodes_format_map["data4"] = domi::DOMI_TENSOR_NC1HWC0;
  DataOpParser opParser;
  Status ret = opParser.ParseShape(shape, op);
  EXPECT_EQ(ret, SUCCESS);
...
TEST_F(UtestTensorflowParser, DataOpParser_ParseShape_null_op) {
  std::vector<int64_t> shape = {1, 3, 224, 224};
...
TEST_F(UtestTensorflowParser, DataOpParser_InitNDTensor) {
  std::vector<int64_t> shape = {1, 3, 224, 224};
  ge::GeTensorDesc tensor_desc;
  DataOpParser opParser;
  Status ret = opParser.InitNDTensor(shape, ge::DT_INT32, tensor_desc);
  EXPECT_EQ(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, DataOpParser_InitInputTensor) {
  std::vector<int64_t> shape = {1, 3, 224, 224};
  ge::GeTensorDesc tensor_desc;
  DataOpParser opParser;
  Status ret = opParser.InitInputTensor(shape, tensor_desc);
  EXPECT_EQ(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, DataOpParser_InitInputTensor_dynamic_batch) {
  std::vector<int64_t> shape = {-1, 3, 224, 224};
  ge::GeTensorDesc tensor_desc;
  DataOpParser opParser;
  Status ret = opParser.InitInputTensor(shape, tensor_desc);
...
TEST_F(UtestTensorflowParser, DataOpParser_InitOutputTensor) {
  std::vector<int64_t> shape = {1, 3, 224, 224};
  ge::GeTensorDesc tensor_desc;
  DataOpParser opParser;
  Status ret = opParser.InitOutputTensor(shape, tensor_desc);
  EXPECT_EQ(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, DataOpParser_InitOutputTensor_empty_shape) {
...
TEST_F(UtestTensorflowParser, TBEPluginLoader_GetOppPath) {
  TBEPluginLoader tbe_plugin;
  std::string opp_path;
  Status ret = tbe_plugin.GetOppPath(opp_path);
  EXPECT_EQ(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, TBEPluginLoader_GetOpsProtoPath) {
  TBEPluginLoader tbe_plugin;
  std::string opsproto_path;
  Status ret = tbe_plugin.GetOpsProtoPath(opsproto_path);
  EXPECT_EQ(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, TBEPluginLoader_GetCustomCaffeProtoPath) {
  TBEPluginLoader tbe_plugin;
  std::string customcaffe_path;
  Status ret = tbe_plugin.GetCustomCaffeProtoPath(customcaffe_path);
  EXPECT_EQ(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, TBEPluginLoader_Finalize) {
  TBEPluginLoader &tbe_plugin = TBEPluginLoader::Instance();
  Status ret = tbe_plugin.Finalize();
  EXPECT_EQ(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, TBEPluginLoader_LoadPluginSo_empty) {
  TBEPluginLoader &tbe_plugin = TBEPluginLoader::Instance();
  std::map<string, string> options;
  tbe_plugin.LoadPluginSo(options);
}

TEST_F(UtestTensorflowParser, TBEPluginLoader_GetOppPluginVendors_failed) {
  TBEPluginLoader tbe_plugin;
  std::vector<std::string> vendors;
  Status ret = tbe_plugin.GetOppPluginVendors("nonexistent_config.ini", vendors);
  EXPECT_EQ(ret, FAILED);
}

TEST_F(UtestTensorflowParser, TensorFlowCustomOpParser_ConstructRegOpString) {
  domi::tensorflow::OpDef opdef;
  opdef.set_name("TestCustomOp");
  auto *input_arg = opdef.add_input_arg();
  input_arg->set_name("x");
  input_arg->set_type(domi::tensorflow::DataType::DT_FLOAT);
  auto *output_arg = opdef.add_output_arg();
  output_arg->set_name("y");
  output_arg->set_type(domi::tensorflow::DataType::DT_FLOAT);
  auto *attr = opdef.add_attr();
  attr->set_name("epsilon");
  attr->set_type("float");
  attr->mutable_default_value()->set_f(0.001f);

  std::string reg_op;
  TensorFlowCustomOpParser parser;
  Status ret = parser.ConstructRegOpString(opdef, reg_op);
  EXPECT_EQ(ret, SUCCESS);
  EXPECT_FALSE(reg_op.empty());
}

TEST_F(UtestTensorflowParser, TensorFlowCustomOpParser_ConstructRegCustomOpString) {
  domi::tensorflow::OpDef opdef;
  opdef.set_name("TestCustomOp2");
  auto *input_arg = opdef.add_input_arg();
  input_arg->set_name("x");
  input_arg->set_type(domi::tensorflow::DataType::DT_FLOAT);
  auto *output_arg = opdef.add_output_arg();
  ...
  node_def.set_op("TestCustomOp2");

  std::string reg_op_custom;
  TensorFlowCustomOpParser parser;
  Status ret = parser.ConstructRegCustomOpString(opdef, node_def, reg_op_custom);
  EXPECT_EQ(ret, SUCCESS);
  EXPECT_FALSE(reg_op_custom.empty());
}

TEST_F(UtestTensorflowParser, TensorFlowCustomOpParser_ConstructRegOpString_with_dynamic) {
  domi::tensorflow::OpDef opdef;
  opdef.set_name("DynamicOp");
  auto *input_arg = opdef.add_input_arg();
  input_arg->set_name("inputs");
  input_arg->set_number_attr("N");
  auto *output_arg = opdef.add_output_arg();
  output_arg->set_name("outputs");
  output_arg->set_number_attr("N");
  auto *attr = opdef.add_attr();
  attr->set_name("N");
  attr->set_type("int");
  attr->mutable_default_value()->set_i(1);
...
TEST_F(UtestTensorflowParser, TensorFlowCustomOpParser_WriteTextFile) {
  TensorFlowCustomOpParser parser;
  std::string content = "test content";
  Status ret = parser.WriteTextFile("test_write_text_file.txt", content);
  EXPECT_EQ(ret, SUCCESS);
  remove("test_write_text_file.txt");
}

TEST_F(UtestTensorflowParser, TensorFlowCustomOpParser_WriteWrapperCc) {
  TensorFlowCustomOpParser parser;
  Status ret = parser.WriteWrapperCc("test_wrapper.cc");
  EXPECT_EQ(ret, SUCCESS);
  remove("test_wrapper.cc");
}

TEST_F(UtestTensorflowParser, TensorFlowCustomOpParser_DeleteTmpDirectoryContents) {
  TensorFlowCustomOpParser parser;
  Status ret = parser.DeleteTmpDirectoryContents("nonexistent_dir_12345");
  EXPECT_EQ(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, TensorFlowCustomOpParser_ParseCustomOp_empty) {
  TensorFlowCustomOpParser parser;
  std::unordered_map<std::string, const domi::tensorflow::NodeDef *> empty_map;
  Status ret = parser.ParseCustomOp(empty_map);
  EXPECT_EQ(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, TensorFlowCustomOpParser_RegisteredTfaOps) {
  TensorFlowCustomOpParser parser;
  Status ret = parser.RegisteredTfaOps();
  EXPECT_EQ(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, aclgrphParseTensorFlow_null_file) {
  ge::Graph graph;
  auto ret = aclgrphParseTensorFlow(nullptr, graph);
  EXPECT_NE(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, aclgrphParseTensorFlow_invalid_file) {
  ge::Graph graph;
  auto ret = aclgrphParseTensorFlow("nonexistent_tf_model.pb", graph);
  EXPECT_NE(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, aclgrphParseTensorFlow_with_params_null) {
  std::map<AscendString, AscendString> parser_params;
  ge::Graph graph;
  auto ret = aclgrphParseTensorFlow(nullptr, parser_params, graph);
  EXPECT_NE(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, aclgrphParseTensorFlow_with_params_invalid) {
  std::map<AscendString, AscendString> parser_params;
  ge::Graph graph;
  auto ret = aclgrphParseTensorFlow("nonexistent_tf_model2.pb", parser_params, graph);
  EXPECT_NE(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, TensorFlowModelParser_ToJson_null) {
  TensorFlowModelParser parser;
  auto ret = parser.ToJson(nullptr, "output.json");
  EXPECT_NE(ret, SUCCESS);
  ret = parser.ToJson("model.pb", nullptr);
  EXPECT_NE(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, TensorFlowModelParser_ToJson_invalid) {
  TensorFlowModelParser parser;
  auto ret = parser.ToJson("nonexistent_tf_model.json", "output.json");
  EXPECT_NE(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, TensorFlowModelParser_ParseFromMemory_null) {
  TensorFlowModelParser parser;
...
TEST_F(UtestTensorflowParser, TensorFlowModelParser_ParseFromMemory_invalid_data) {
  TensorFlowModelParser parser;
  ge::ComputeGraphPtr graph = std::make_shared<ge::ComputeGraph>("test");
  const char *data = "invalid data";
  auto ret = parser.ParseFromMemory(data, 11, graph);
  EXPECT_NE(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, AclGraphParserUtil_SetOutputNodeInfo_with_out_nodes) {
  ParerUTestsUtils::ClearParserInnerCtx();
  AclGraphParserUtil acl_graph_parse_util;
  ge::ComputeGraphPtr compute_graph = std::make_shared<ge::ComputeGraph>("test_tf_out");
  ge::OpDescPtr op = std::make_shared<ge::OpDesc>("tf_output", "Identity");
  ...
  ge::GetParserContext().user_out_nodes.push_back({"tf_output", 0});
  ge::GetParserContext().type = domi::TENSORFLOW;
  std::map<AscendString, AscendString> parser_params;
  auto ret = acl_graph_parse_util.SetOutputNodeInfo(graph, parser_params);
  EXPECT_EQ(ret, SUCCESS);
}

TEST_F(UtestTensorflowParser, AclGraphParserUtil_SetOutputNodeInfo_default) {
  ParerUTestsUtils::ClearParserInnerCtx();
  AclGraphParserUtil acl_graph_parse_util;
  ge::ComputeGraphPtr compute_graph = std::make_shared<ge::ComputeGraph>("test_tf_default_out");
  ge::OpDescPtr op = std::make_shared<ge::OpDesc>("default_out", "Identity");
  op->AddInputDesc(ge::GeTensorDesc());
  op->AddOutputDesc(ge::GeTensorDesc());
  ge::NodePtr node = compute_graph->AddNode(op);
  compute_graph->AddOutputNodeByIndex(node, 0);
  ge::Graph graph = ge::GraphUtilsEx::CreateGraphFromComputeGraph(compute_graph);
...
TEST_F(UtestTensorflowParser, AclGraphParserUtil_SetOutputNodeInfo_with_output_format) {
  ParerUTestsUtils::ClearParserInnerCtx();
  AclGraphParserUtil acl_graph_parse_util;
  ge::ComputeGraphPtr compute_graph = std::make_shared<ge::ComputeGraph>("test_tf_format");
  ge::OpDescPtr op = std::make_shared<ge::OpDesc>("format_out", "Identity");
  op->AddInputDesc(ge::GeTensorDesc());
  op->AddOutputDesc(ge::GeTensorDesc());
  ge::NodePtr node = compute_graph->AddNode(op);
  ge::Graph graph = ge::GraphUtilsEx::CreateGraphFromComputeGraph(compute_graph);

  ge::GetParserContext().user_out_nodes.push_back({"format_out", 0});
  ge::GetParserContext().output_formats.push_back(domi::DOMI_TENSOR_NC1HWC0);
  ge::GetParserContext().type = domi::TENSORFLOW;
  std::map<AscendString, AscendString> parser_params;
  auto ret = acl_graph_parse_util.SetOutputNodeInfo(graph, parser_params);
  EXPECT_EQ(ret, SUCCESS);
...
TEST_F(UtestTensorflowParser, TensorFlowModelParser_TrimGraph_empty) {
  ParerUTestsUtils::ClearParserInnerCtx();
  TensorFlowModelParser parser;
  domi::tensorflow::GraphDef input_graph;
  domi::tensorflow::GraphDef output_graph;
...
TEST_F(UtestTensorflowParser, TensorFlowModelParser_ParseFromMemory_valid_graph) {
  ParerUTestsUtils::ClearParserInnerCtx();
  TensorFlowModelParser parser;
  domi::tensorflow::GraphDef graph_def;
  auto *node = graph_def.add_node();
  node->set_name("test_input");
  node->set_op("Placeholder");
  auto *attr = node->mutable_attr();
  domi::tensorflow::AttrValue dtype_val;
  dtype_val.set_type(domi::tensorflow::DT_FLOAT);
  (*attr)["dtype"] = dtype_val;
  domi::tensorflow::AttrValue shape_val;
  auto *shape = shape_val.mutable_shape();
  shape->add_dim()->set_size(-1);
  shape->add_dim()->set_size(3);
  shape->add_dim()->set_size(224);
  shape->add_dim()->set_size(224);
  (*attr)["shape"] = shape_val;

  std::string buffer;
  graph_def.SerializeToString(&buffer);
  ge::ComputeGraphPtr graph = std::make_shared<ge::ComputeGraph>("test_mem");
  ge::GetParserContext().type = domi::TENSORFLOW;
  ge::GetParserContext().input_dims["test_input"] = {1, 3, 224, 224};
  auto ret = parser.ParseFromMemory(buffer.data(), static_cast<int>(buffer.size()), graph);
