...
class LlmEngineV2St(unittest.TestCase):
    ...
    def test_cluster_ops(self):
        cluster_id = 0
        llm_engine = LLMDataDist(LLMRole.DECODER, cluster_id)
        cluster_info = {"cluster_id": 1, "logic_device_id": ["0:0:0:0"]}
        engine_options = {"llm.ClusterInfo": json.dumps(cluster_info)}
    ...
    def test_init_witch_device_id_and_rank_id(self):
        ...
        llm_config.ge_options = {"ge.exec.rankId": "1"}
    ...
    def test_transfer_cache(self):
        ...
        cache_task = kv_cache_manager.transfer_cache_async(
            kv_cache,
            LayerSynchronizerImpl(True),
            transfer_configs,
            block_indices,
            block_indices,
            dst_block_memory_size=32,
        )
        ret = cache_task.synchronize()
        ...
        with self.assertRaises(LLMException):
            _ = kv_cache_manager.transfer_cache_async(
                kv_cache, LayerSynchronizerImpl(False), transfer_configs, [0, 1], [0]
            )
        with self.assertRaises(LLMException):
            _ = kv_cache_manager.transfer_cache_async(
                kv_cache, LayerSynchronizerImpl(False), transfer_configs, [], [], 1
            )
        with self.assertRaises(ValueError):
            _ = kv_cache_manager.transfer_cache_async(
                kv_cache,
