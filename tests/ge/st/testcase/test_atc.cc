...
TEST_F(AtcCommonSTest, distribute_model_1980_failed) {
  std::vector<OpRegistrationData> registrationDatas = domi::OpRegistry::Instance()->registrationDatas;
  for (OpRegistrationData reg_data : registrationDatas) {
    if (reg_data.GetFrameworkType() == domi::TENSORFLOW) {
      (void)ge::OpRegistrationTbe::Instance()->Finalize(reg_data);
  ...
  om_path = PathJoin(om_path.c_str(), "pb_common_1");
  std::string model_arg = "--model=" + pb_path_;
  std::string cluster_config = "--cluster_config=" + st_base_path_ + "st_run_data/json/cluster_config/numa_config_1980_invalid.json";
  std::string output_arg = "--output="+om_path;
  char *argv[] = {"atc",
                  const_cast<char *>(model_arg.c_str()),
                  const_cast<char *>(output_arg.c_str()),
                  ...
                  "--output_type=FP32",
                  "--input_shape=Placeholder_1:1,256,256,3",
                  const_cast<char *>(cluster_config.c_str())
                  };
  auto ret = main_impl(sizeof(argv)/sizeof(argv[0]), argv);
  EXPECT_NE(ret, 0);
  ReInitGe(); // the main_impl will call GEFinalize, so re-init after call it
...
TEST_F(AtcCommonSTest, distribute_model_1980) {
  std::vector<OpRegistrationData> registrationDatas = domi::OpRegistry::Instance()->registrationDatas;
  for (OpRegistrationData reg_data : registrationDatas) {
    if (reg_data.GetFrameworkType() == domi::TENSORFLOW) {
      (void)ge::OpRegistrationTbe::Instance()->Finalize(reg_data);
  ...
  om_path = PathJoin(om_path.c_str(), "pb_common_1");
  std::string model_arg = "--model=" + pb_path_;
  std::string cluster_config = "--cluster_config=" + st_base_path_ + "st_run_data/json/cluster_config/numa_config_1980.json";
  std::string output_arg = "--output="+om_path;
  char *argv[] = {"atc",
                  const_cast<char *>(model_arg.c_str()),
                  const_cast<char *>(output_arg.c_str()),
...
TEST_F(AtcCommonSTest, distribute_model_helper) {
  std::vector<OpRegistrationData> registrationDatas = domi::OpRegistry::Instance()->registrationDatas;
  for (OpRegistrationData reg_data : registrationDatas) {
    if (reg_data.GetFrameworkType() == domi::TENSORFLOW) {
      (void)ge::OpRegistrationTbe::Instance()->Finalize(reg_data);
  ...
  om_path = PathJoin(om_path.c_str(), "pb_common_1");
  std::string model_arg = "--model=" + pb_path_;
  std::string cluster_config = "--cluster_config=" + st_base_path_ + "st_run_data/json/cluster_config/numa_config_1.json";
  std::string output_arg = "--output="+om_path;
  char *argv[] = {"atc",
                  const_cast<char *>(model_arg.c_str()),
                  const_cast<char *>(output_arg.c_str()),
...
TEST_F(AtcCommonSTest, distribute_model_build) {
  std::vector<OpRegistrationData> registrationDatas = domi::OpRegistry::Instance()->registrationDatas;
  for (OpRegistrationData reg_data : registrationDatas) {
    if (reg_data.GetFrameworkType() == domi::TENSORFLOW) {
      (void)ge::OpRegistrationTbe::Instance()->Finalize(reg_data);
  ...
  GE_MAKE_GUARD(recover_context_cfg, [&](){
    ...
    global_options.erase("ge.distributed_cluster_build");
...
TEST_F(AtcCommonSTest, distribute_model_build_params_invalid) {
  std::vector<OpRegistrationData> registrationDatas = domi::OpRegistry::Instance()->registrationDatas;
  for (OpRegistrationData reg_data : registrationDatas) {
    if (reg_data.GetFrameworkType() == domi::TENSORFLOW) {
      (void)ge::OpRegistrationTbe::Instance()->Finalize(reg_data);
