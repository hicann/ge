...
class SessionWrapper {
 public:
  SessionWrapper(GraphAndIoNum graph_io, std::map<AscendString, AscendString> options = {}) : session_(options) {
...
inline GraphAndIoNum BuildGraph_OneAdd(bool dynamic) {
  ...
  auto data_0 =
      OP_CFG(DATA).InCnt(1).OutCnt(1).Attr(ATTR_NAME_INDEX, 0).TensorDesc(FORMAT_ND, DT_FLOAT, {dim}).Build("_data_0");
  auto data_1 =
      OP_CFG(DATA).InCnt(1).OutCnt(1).Attr(ATTR_NAME_INDEX, 1).TensorDesc(FORMAT_ND, DT_FLOAT, {dim}).Build("_data_1");
  auto add_0 = OP_CFG(ADD)
                   .InCnt(2)
                   .OutCnt(1)
                   .Attr(ATTR_NAME_KERNEL_BIN_ID, "_add_0_fake_id")
                   .TensorDesc(FORMAT_ND, DT_FLOAT, {dim})
                   .Build("_add_0");
...
inline GraphAndIoNum BuildGraph_TwoAdd(bool dynamic) {
  ...
  auto data_1 =
      OP_CFG(DATA).InCnt(1).OutCnt(1).Attr(ATTR_NAME_INDEX, 1).TensorDesc(FORMAT_ND, DT_FLOAT, {dim}).Build("_data_1");
  auto add_0 = OP_CFG(ADD).InCnt(2).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {dim}).Build("_add_0");
  auto add_1 = OP_CFG(ADD).InCnt(2).OutCnt(1).TensorDesc(FORMAT_ND, DT_FLOAT, {dim}).Build("_add_1");
