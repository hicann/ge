...
cd -
  ...
  void SetUp() {
    ...
    auto path = PathUtils::Join({EnvPath().GetAirBasePath(), "tests/ge/st/config_file/right_json/host"});
...
TEST_F(DataFlowGraphTest, BuildModelRelationNodeNotUse) {
  auto real_path = PathUtils::Join({EnvPath().GetAirBasePath(), "tests/ge/st/config_file/right_json/host"});
