...
Graph BuildValueRangeInferGraph() {
  DEF_GRAPH(g1) {
    ...
    auto data2 = OP_CFG(DATA)
                     ...
                     .OutCnt(1)
                     .Build("data2");
    auto add = OP_CFG(ADD).TensorDesc(FORMAT_NCHW, DT_FLOAT, {4}).InCnt(1).OutCnt(1).Build("add");
    auto data3 = OP_CFG(DATA)
                     ...
                     .Build("data3");
    auto switchn =
        OP_CFG("SwitchN").TensorDesc(FORMAT_NCHW, DT_FLOAT, {8, 4, 16, 16}).InCnt(1).OutCnt(1).Build("switchn");
    ...
    auto const1 =
        OP_CFG(CONSTANT).TensorDesc(FORMAT_ND, DT_INT32, {1}).Weight(data_tensor).InCnt(1).OutCnt(1).Build("const1");
    auto expanddim = OP_CFG(EXPANDDIMS)
                         .InCnt(2)
                         .OutCnt(1)
                         .Attr("_ge_attr_op_kernel_lib_name", "DNN_VM_GE_LOCAL_OP_STORE")
                         .Build("expanddim");
    CHAIN(NODE(data1)->EDGE(0, 0)->NODE(reshape)->EDGE(0, 0)->NODE("netoutput", NETOUTPUT));
    CHAIN(NODE(data2)->EDGE(0, 0)->NODE(shape)->EDGE(0, 0)->NODE(add)->EDGE(0, 1)->NODE(reshape));
    CHAIN(NODE(shape)->EDGE(0, 1)->NODE(add));
    CHAIN(NODE(data3)->EDGE(0, 0)->NODE(switchn)->EDGE(0, 0)->NODE(expanddim)->EDGE(0, 1)->NODE("netoutput"));
    CHAIN(NODE(const1)->EDGE(0, 1)->NODE(expanddim));
  };
...
Graph BuildValueRangeInferGraph2() {
  DEF_GRAPH(g1) {
    ...
    auto data2 = OP_CFG(DATA)
                     ...
                     .OutCnt(1)
                     .Build("data2");
    auto add = OP_CFG(ADD).TensorDesc(FORMAT_NCHW, DT_FLOAT, {256}).InCnt(1).OutCnt(1).Build("add");
    auto data3 = OP_CFG(DATA)
                     .Attr(ATTR_NAME_INDEX, 2)
                     .TensorDesc(FORMAT_NCHW, DT_FLOAT, {8, 4, 16, 16})
                     .InCnt(1)
...
TEST_F(InferAndFoldingTest, test_infer_value_range) {
  ...
  CHECK_GRAPH(OptimizeStage1_2) {
    ...
    std::vector<std::pair<int64_t, int64_t>> expect_value_range = {make_pair(1, 100), make_pair(1, 240),
 ...
 * 这是一个编译+执行的端到端用例
 * 测试背景： 动态静态的hybrid
 model，若静态子图输出边界的squeeze被优化删除掉（这是一类gelocal算子，只变shape不该内存，在静态模型中通常采取删除掉的方式替代下沉执行），
