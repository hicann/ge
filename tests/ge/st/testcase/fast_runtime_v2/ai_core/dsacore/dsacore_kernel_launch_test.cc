...
TEST_F(DsacoreKernelLaunchST, test_run_launch_dsacore_kernel_update_sqe_seed_count_result_failed) {
  auto run_context = BuildKernelRunContext(18, 0);
  rtStarsDsaSqe_t dsa_sqe;
  size_t workspace_size = 2;
  auto workspace_addr_ptr = ContinuousVector::Create<GertTensorData *>(workspace_size);
  ...
  ASSERT_EQ(registry.FindKernelFuncs("UpdateSqeArg")->run_func(run_context), ge::GRAPH_PARAM_INVALID);
}
}  // namespace
}  // namespace gert
