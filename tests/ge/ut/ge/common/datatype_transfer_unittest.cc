...
TEST_F(UtestDataTypeTransfer, fp16_fp32) {
  ...
  EXPECT_EQ(TransTensorDataType(args2, result2), SUCCESS);
}
...
TEST_F(UtestDataTypeTransfer, fp32_fp16_to_hif8) {
  float f32[256];
  fp16_t f16[256];
  for (size_t n = 0; n < 256; n++) {
    f32[n] = static_cast<float>(HiF8::FromRawBits(n));
    f16[n] = static_cast<fp16_t>(HiF8::FromRawBits(n));
  }
  TransResult result;

  CastArgs args0{reinterpret_cast<uint8_t *>(f32), 256, DT_FLOAT, DT_HIFLOAT8};
  EXPECT_EQ(DataTypeTransfer::TransDataType(args0, result), SUCCESS);
  EXPECT_EQ(result.length, 256);
  for (size_t n = 0; n < 256; n++) {
    EXPECT_EQ(reinterpret_cast<uint8_t *>(result.data.get())[n], n);
  }
  CastArgs args1{reinterpret_cast<uint8_t *>(f16), 256, DT_FLOAT16, DT_HIFLOAT8};
...
TEST_F(UtestDataTypeTransfer, hif8_to_fp32_fp16) {
  ...
  CastArgs args0{u8, 128, DT_HIFLOAT8, DT_FLOAT};
  EXPECT_EQ(DataTypeTransfer::TransDataType(args0, result), SUCCESS);
