...
TEST_F(ExecutorDumperUT, PassChangedInfoResolvesLaunchSpecificPrepareOutput) {
  ...
  dumper.compute_node_name_to_launch_kernel_name_["compute"] = "LaunchKernelV2_target";
  ...
  pass_changed_info.pass_changed_kernels = {
      {{"copy", 0, "LaunchKernelV2_other"}, {"OtherPrepare", 0}},
      {{"copy", 0, "LaunchKernelV2_target"}, {"CopyFlowPrepare", 1}},
  };
...
TEST_F(ExecutorDumperUT, ExceptionDump_PrepareExceptionDumpSuccess_WithEmptyAddrs) {
  ...
  GeModelBuilder builder(graph);
  auto ge_root_model =
      builder.AddTaskDef("Add", AiCoreTaskDefFaker("AddStubBin").WithHandle()).FakeTbeBin({"Add"}).BuildGeRootModel();
