...
struct RuntimeStubGuard {
  RuntimeStubGuard() : runtime_stub(std::make_shared<RuntimeStubImpl>()) {
...
TEST_F(TaskSchedulerUnitTest, should_schedule_without_execute_args_and_skip_stream_binding) {
  ...
  EXPECT_TRUE(runtime_stub_guard.runtime_stub->GetUseStreamResRecords().empty());
  EXPECT_TRUE(runtime_stub_guard.runtime_stub->GetNotUseStreamResRecords().empty());
}
...
TEST_F(TaskSchedulerUnitTest, should_rebind_execute_stream_between_schedules_and_unbind_on_stop) {
  ...
  first_execution_data.Chain({3, 7, 6}).StartNodes({3}).ExecuteStream(reinterpret_cast<rtStream_t>(0x11));

  FakeExecutionData second_execution_data(10);
  second_execution_data.Chain({5, 8, 6}).StartNodes({5}).ExecuteStream(reinterpret_cast<rtStream_t>(0x22));
  ...
  ASSERT_EQ(kStatusSuccess, scheduler->Schedule());
  EXPECT_EQ(runtime_stub_guard.runtime_stub->GetUseStreamResRecords(),
            std::vector<rtStream_t>({reinterpret_cast<rtStream_t>(0x11)}));
  EXPECT_TRUE(runtime_stub_guard.runtime_stub->GetNotUseStreamResRecords().empty());

  ASSERT_EQ(ge::GRAPH_SUCCESS, scheduler->Prepare(TaskScheduler::ScheduleData{second_execution_data.Data()}));
  ASSERT_EQ(kStatusSuccess, scheduler->Schedule());
  EXPECT_EQ(runtime_stub_guard.runtime_stub->GetUseStreamResRecords(),
            std::vector<rtStream_t>({reinterpret_cast<rtStream_t>(0x11), reinterpret_cast<rtStream_t>(0x22)}));
  EXPECT_EQ(runtime_stub_guard.runtime_stub->GetNotUseStreamResRecords(),
            std::vector<rtStream_t>({reinterpret_cast<rtStream_t>(0x11)}));
