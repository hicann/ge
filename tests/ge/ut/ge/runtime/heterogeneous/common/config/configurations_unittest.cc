...
static const char* g_config_string = "{\"host\":{\
  \"resourceType\":\"X86\",\
  \"ctrlPanel\":{\
    \"ipaddr\":\"192.168.1.38\",\
    \"availPorts\":[\"1~60000\"]\
...
// host
TEST_F(UtConfigurations, parse_config_info_from_env_success) {
  setenv("HELPER_RES_CONFIG", g_config_string, 1);
...
TEST_F(UtConfigurations, GetConfigDir_path_invalid) {
  class MockMmpaRealPath : public ge::MmpaStubApiGe {
   public:
    int32_t RealPath(const CHAR *path, CHAR *realPath, INT32 realPathLen) override {
      memcpy_s(realPath, realPathLen, path, strlen(path));
  ...
  EXPECT_NE(config.GetConfigDir(config_dir), SUCCESS);
  unsetenv("HELPER_RES_FILE_PATH");
}

TEST_F(UtConfigurations, GetConfigDir_realpath_failed) {
  class MockMmpaRealPath : public ge::MmpaStubApiGe {
   public:
    int32_t RealPath(const CHAR *path, CHAR *realPath, INT32 realPathLen) override {
      realPath[0] = '\0';
...
TEST_F(UtConfigurations, GetConfigDir_install_path_invalid) {
  class MockMmpaRealPath : public ge::MmpaStubApiGe {
   public:
    int32_t RealPath(const CHAR *path, CHAR *realPath, INT32 realPathLen) override {
      memcpy_s(realPath, realPathLen, path, strlen(path));
...
TEST_F(UtConfigurations, GetConfigDir_install_path_realpath_failed) {
  class MockMmpaRealPath : public ge::MmpaStubApiGe {
   public:
    int32_t RealPath(const CHAR *path, CHAR *realPath, INT32 realPathLen) override {
      realPath[0] = '\0';
