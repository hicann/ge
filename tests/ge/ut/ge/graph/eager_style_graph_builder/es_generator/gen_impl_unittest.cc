...
// 测试含 C++/Python 关键字的算子
TEST_F(GenImplLLT, phony_keyword_OpGeneration) {
  // 根据stub_geir_ops.cc中的定义，phony_keyword应该有以下特征：
...
#ifndef AIR_CXX_COMPILER_GRAPH_EAGER_STYLE_GRAPH_BUILDER_OP_phony_keyword_H_
#define AIR_CXX_COMPILER_GRAPH_EAGER_STYLE_GRAPH_BUILDER_OP_phony_keyword_H_
#include "esb_funcs.h"
#include <stdint.h>
#include "graph/types.h"
#ifdef __cplusplus
...
typedef struct {
  EsCTensorHolder *out_break;
  EsCTensorHolder **out_if;
  int64_t out_if_num;
} Esphony_keywordOutput;
/**
 * @note lifecycles of following subgraph inputs will be transferred to the EsCGraphBuilder:
 *   subgraph_or
 * @note user needs to provide following inputs for dynamic output numbers:
 *   out_if_num: dynamic output number of if
 */
Esphony_keywordOutput Esphony_keyword(EsCTensorHolder *in_for, EsCTensorHolder **in_case, int64_t in_case_num, int64_t out_if_num, EsCGraph *subgraph_or, int64_t attr_not);
#ifdef __cplusplus
}
#endif
#endif
  ...
  std::ifstream read_header(test_output_dir_ + "es_phony_keyword_c.h");
  std::stringstream header_content;
  header_content << read_header.rdbuf();
  read_header.close();
...
TEST_F(GenImplLLT, phony_keyword_OpCppGeneration) {
  std::string expected_cc_content = R"(
/* Copyright (c) 2025 Huawei Technologies Co., Ltd.
 * This file is a part of the CANN Open Software.
 * Licensed under CANN Open Software License Agreement Version 1.0 (the "License").
...
#endif
Esphony_keywordOutput Esphony_keyword(EsCTensorHolder *in_for, EsCTensorHolder **in_case, int64_t in_case_num, int64_t out_if_num, EsCGraph *subgraph_or, int64_t attr_not) {
  ES_ASSERT_NOTNULL(in_for);
  ES_ASSERT_TRUE(ge::IntegerChecker<int32_t>::Compat(in_case_num));
  auto *owner_builder = ge::es::ResolveBuilder(in_for, std::vector<EsCTensorHolder *>(in_case, in_case + in_case_num));
  ES_ASSERT_NOTNULL(owner_builder, "Failed to resolve owner builder: please ensure at least one input tensor or an explicit owner_graph_builder is provided when supported.");
  auto &builder = *owner_builder;
  auto ge_graph = builder.GetGraph();

  ge::Graph* input_subgraph;
  input_subgraph = static_cast<ge::Graph *>(static_cast<void *>(subgraph_or));
  auto subgraph_or_node_type = ge::AscendString();
  int net_output_num_of_subgraph_or = 0;
  std::unordered_set<int64_t> subgraph_or_data_indexes;
  subgraph_or_data_indexes.reserve(in_case_num);
  for (auto &sub_graph_node : input_subgraph->GetDirectNode()) {
    ES_ASSERT_GRAPH_SUCCESS(sub_graph_node.GetType(subgraph_or_node_type));
    if (subgraph_or_node_type != "Data") {
      if (subgraph_or_node_type == "NetOutput") {
        ES_ASSERT_TRUE(net_output_num_of_subgraph_or <= 1);
        net_output_num_of_subgraph_or++;
        ES_ASSERT_TRUE(ge::IntegerChecker<int32_t>::Compat(out_if_num));
        auto subgraph_output_cnt = sub_graph_node.GetInputsSize();
          ES_ASSERT_TRUE(static_cast<int64_t>(subgraph_output_cnt)  == out_if_num);
      }
      continue;
    }
    int64_t index_value;
    ES_ASSERT_GRAPH_SUCCESS(sub_graph_node.GetAttr("index", index_value));
    ES_ASSERT_TRUE(subgraph_or_data_indexes.insert(index_value).second);
    ES_ASSERT_TRUE(index_value < in_case_num);
  }
  ES_ASSERT_TRUE(net_output_num_of_subgraph_or == 1);
  auto subgraph_or_ptr = builder.AddResource(std::unique_ptr<ge::Graph>(input_subgraph));

  // NOTE: IrDefInputs/IrDefOutputs/IrDefAttrs are not ABI-safe across different compilers.
  // The V2 ABI-safe interfaces (IrDefInputsV2/IrDefOutputsV2/IrDefAttrsV2) exist but are
  // not used here due to backward compatibility requirements with older GE versions.
  // This will be replaced with V2 interfaces after the compatibility period ends.
  auto node = ge::es::CompliantNodeBuilder(ge_graph).OpType("phony_keyword")
      .Name( builder.GenerateNodeName("phony_keyword").GetString())
      .IrDefInputs({
          {"for", ge::es::CompliantNodeBuilder::kEsIrInputRequired, ""},
          {"case", ge::es::CompliantNodeBuilder::kEsIrInputDynamic, ""},
      })
      .IrDefOutputs({
          {"break", ge::es::CompliantNodeBuilder::kEsIrOutputRequired, ""},
          {"if", ge::es::CompliantNodeBuilder::kEsIrOutputDynamic, ""},
      })
      .IrDefAttrs({
          {
              "not",
              ge::es::CompliantNodeBuilder::kEsAttrOptional,
              "Int",
              ge::es::CreateFromIfNotEqual(attr_not, static_cast<int64_t>(0))
          },
      })
      .InstanceDynamicInputNum("case", static_cast<int32_t>(in_case_num))
      .InstanceDynamicOutputNum("if", static_cast<int32_t>(out_if_num))
      .Build();

  ES_ASSERT_GRAPH_SUCCESS(ge::es::AddEdgeAndUpdatePeerDesc(*ge_graph, in_for->GetProducer(), in_for->GetOutIndex(), node, 0));
  if ((in_case != nullptr) && (in_case_num > 0)) {
    for (int64_t i = 0; i < in_case_num; ++i) {
      auto one_case = in_case[i];
      ES_ASSERT_NOTNULL(one_case);
      ES_ASSERT_GRAPH_SUCCESS(ge::es::AddEdgeAndUpdatePeerDesc(*ge_graph, one_case->GetProducer(), one_case->GetOutIndex(), node, 1 + i));
    }
  }

  node.SetSubgraph("subgraph_or", *subgraph_or_ptr);

  auto out_if_holders = builder.CreateDynamicTensorHolderFromNode(node, 1, out_if_num);
  return Esphony_keywordOutput{
      builder.GetTensorHolderFromNode(node, 0),
      out_if_holders->data(),
      static_cast<int64_t>(out_if_holders->size()),
  };
}
#ifdef __cplusplus
}
  ...
  std::ifstream read_cc(test_output_dir_ + "es_phony_keyword.cpp");
  std::stringstream cc_content;
  cc_content << read_cc.rdbuf();
  read_cc.close();

  EXPECT_EQ(Normalize(cc_content.str()), Normalize(expected_cc_content))
      << "Generated source content does not match expected content";
}

TEST_F(GenImplLLT, phony_keyword_OpCppHeaderGeneration) {
  std::string expected_cpp_content = R"(
/* Copyright (c) 2025 Huawei Technologies Co., Ltd.
 * This file is a part of the CANN Open Software.
 * Licensed under CANN Open Software License Agreement Version 1.0 (the "License").
...
#ifndef AIR_CXX_COMPILER_GRAPH_EAGER_STYLE_GRAPH_BUILDER_OP_phony_keyword_CPP_H_
#define AIR_CXX_COMPILER_GRAPH_EAGER_STYLE_GRAPH_BUILDER_OP_phony_keyword_CPP_H_
#include <utility>
#include "es_tensor_holder.h"
#include "es_graph_builder.h"
#include "es_tensor_like.h"
...
struct phony_keywordOutput {
  EsTensorHolder out_break;
  std::vector<EsTensorHolder> out_if;
};
/**
 * @note lifecycles of following subgraph inputs will be transferred to the EsCGraphBuilder:
 *   subgraph_or
 ...
 *   out_if_num: dynamic output number of if
 */
inline phony_keywordOutput phony_keyword(const EsTensorLike & in_for, const std::vector<EsTensorHolder> & in_case, int64_t out_if_num, std::unique_ptr<ge::Graph> subgraph_or, int64_t attr_not=0) {
  auto *owner_graph_builder = ResolveBuilder(in_for, in_case);
  ES_ASSERT_NOTNULL(owner_graph_builder, "Failed to resolve owner builder: please ensure at least one input tensor or an explicit owner_builder is provided when supported.");
  auto esb_in_case = TensorsToEsCTensorHolders(in_case);
  auto out = Esphony_keyword(in_for.ToTensorHolder(owner_graph_builder).GetCTensorHolder(), esb_in_case.data(), static_cast<int64_t>(esb_in_case.size()), out_if_num, static_cast<EsCGraph *>(static_cast<void *>(subgraph_or.release())), attr_not);
  std::vector<EsTensorHolder> out_if_dynamic_outs;
  out_if_dynamic_outs.reserve(out.out_if_num);
  for (int64_t dyn_idx = 0; dyn_idx < out.out_if_num; ++dyn_idx) {
    out_if_dynamic_outs.emplace_back(out.out_if[dyn_idx]);
  }
  return {out.out_break, out_if_dynamic_outs};
}
}  // namespace es
}  // namespace ge
#endif
  ...
  std::ifstream read_cpp(test_output_dir_ + "es_phony_keyword.h");
  std::stringstream cpp_content;
  cpp_content << read_cpp.rdbuf();
  read_cpp.close();

  EXPECT_EQ(Normalize(cpp_content.str()), Normalize(expected_cpp_content))
      << "Generated C++ header content does not match expected content";
}
