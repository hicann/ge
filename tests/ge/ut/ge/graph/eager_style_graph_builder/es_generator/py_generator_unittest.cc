...
TEST_F(GenPyImplLLT, PhonyKeywordOpPythonGeneration) {
  std::string expected_py_content = R"(#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# -------------------------------------------------------------------
...
# 为 phony_keyword 定义ctypes结构体
class Cphony_keywordOutput(ctypes.Structure):
    """ctypes structure corresponding to Esphony_keywordOutput"""
    _fields_ = [
        ("out_break", EsCTensorHolderPtr),  # EsCTensorHolder指针
        ("out_if", ctypes.POINTER(EsCTensorHolderPtr)),  # EsCTensorHolder指针数组
        ("out_if_num", ctypes.c_int64),  # 动态输出数量
    ]

# 为 phony_keyword 定义输出结构体
class phony_keywordOutput:
    """Output structure for phony_keyword operator"""
    def __init__(self, out_break: TensorHolder, out_if: List[TensorHolder]):
        self.out_break = out_break
        self.out_if = out_if


# 定义 phony_keyword 函数原型
esb_generated_lib.Esphony_keyword.argtypes = [EsCTensorHolderPtr, ctypes.POINTER(EsCTensorHolderPtr), ctypes.c_int64, ctypes.c_int64, ctypes.POINTER(EsCGraph), ctypes.c_int64]
esb_generated_lib.Esphony_keyword.restype = Cphony_keywordOutput  # Esphony_keywordOutput

# 创建Python包装函数
def phony_keyword(in_for: Union[TensorHolder, TensorLike], case: List[Union[TensorHolder, TensorLike]], out_if_num: int, subgraph_or: Graph, *, attr_not: int = 0) -> phony_keywordOutput:
    """
    Python wrapper for Esphony_keyword function
    Args:
        in_for: TensorHolder or TensorLike object
        case: TensorHolder or TensorLike object list
        out_if_num: int - number of dynamic outputs
        subgraph_or: Graph object
        attr_not: int value (optional)
    Returns:
        phony_keywordOutput object containing dynamic and/or multiple outputs
    Note: at least one of the following input arguments should be TensorHolder object:
          in_for
          case
    """
    # 获取owner_graph_builder
    owner_graph_builder = resolve_builder(in_for, *(case if case else [None]))

    # 将TensorLike输入转换为TensorHolder
    in_for = convert_to_tensor_holder(in_for, owner_graph_builder)
    case = [convert_to_tensor_holder(t, owner_graph_builder) for t in case]

    # 转换动态输入
    case_ptrs = [t._handle for t in case]
    case_array = (EsCTensorHolderPtr * len(case))(*case_ptrs)

    # 转移静态子图所有权到C++侧，并保持对owner的引用
    subgraph_or._transfer_ownership_when_pass_as_subgraph(owner_graph_builder)

    # 调用C函数
    _result = esb_generated_lib.Esphony_keyword(in_for._handle, case_array, len(case), out_if_num, ctypes.cast(subgraph_or._handle, ctypes.POINTER(EsCGraph)), attr_not)

    # 处理动态输出结果
    out_break = TensorHolder._create_from(_result.out_break, owner_graph_builder)
    # 创建动态输出列表: if
    out_if_list = [TensorHolder._create_from(_result.out_if[i], owner_graph_builder) for i in range(_result.out_if_num)]

    # 尝试获取scope内信息设置到节点上
    owner_graph_builder._apply_scope_infos_to_node(out_break)
    return phony_keywordOutput(
        out_break=out_break,
        out_if=out_if_list,
    )
)";

  const char *op_type = "phony_keyword";
  py_gen_->GenOp(ge::OpDescUtils::GetOpDescFromOperator(ge::OperatorFactory::CreateOperator("op_test", op_type)));
  const std::stringstream &py_content = py_gen_->GetPerOpContents().at(op_type);
  EXPECT_EQ(Normalize(py_content.str()), Normalize(expected_py_content))
      << "Generated python wrapper content does not match expected content";
