...
TEST_F(UtestGraphPassesFoldingKernelSsdPriorboxKernel, ParamInvalid) {
  ...
  int dim_size = 0;
  SsdPriorboxKernel kernel;
  ge::Status ret = kernel.GetNumPriorAndDimSize(2U * 1024 * 1024 * 1024, 2U, 1U, 1, 1, num_priors, dim_size);
  EXPECT_EQ(PARAM_INVALID, ret);

  ret = kernel.GetNumPriorAndDimSize(4U * 1024 * 1024 * 1024 - 1, 1U, 1U, 1, 1, num_priors, dim_size);
  EXPECT_EQ(PARAM_INVALID, ret);

  ret = kernel.GetNumPriorAndDimSize(2U * 1024 * 1024 * 1024 - 1, 1U, 1U, 1, 1, num_priors, dim_size);
  EXPECT_EQ(PARAM_INVALID, ret);

  ret = kernel.GetNumPriorAndDimSize(1U, 1U, 1U, 1 * 1024 * 1024 * 1024, 2, num_priors, dim_size);
  EXPECT_EQ(PARAM_INVALID, ret);

  ret = kernel.GetNumPriorAndDimSize(1U, 1U, 1U, 1024 * 1024 * 1024, 1, num_priors, dim_size);
  EXPECT_EQ(PARAM_INVALID, ret);
