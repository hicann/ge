...
#include "host_kernels/array_ops/squeeze_kernel.h"
...
class UtestGraphPassesFoldingKernelSqueenzeKernel : public testing::Test {
 protected:
  ...
  void EXPECT_TestSqueeze(vector<int64_t> &data_vec, vector<int32_t> &dim_value_vec, vector<int64_t> &result) {
  ...
  void EXPECT_TestInvalidSqueeze(vector<int64_t> &data_vec, vector<int32_t> &dim_value_vec, vector<int64_t> &result) {
    ge::ComputeGraphPtr graph = std::make_shared<ge::ComputeGraph>("default");
...
TEST_F(UtestGraphPassesFoldingKernelSqueenzeKernel, Int8Case0) {
  ...
  EXPECT_TestSqueeze<int8_t, DT_INT8>(data_vec, dim_value_vec, result);
}
...
TEST_F(UtestGraphPassesFoldingKernelSqueenzeKernel, Int16) {
  ...
  EXPECT_TestSqueeze<int16_t, DT_INT16>(data_vec, dim_value_vec, result);
}
...
TEST_F(UtestGraphPassesFoldingKernelSqueenzeKernel, Int32) {
  ...
  EXPECT_TestSqueeze<int32_t, DT_INT32>(data_vec, dim_value_vec, result);
}
...
TEST_F(UtestGraphPassesFoldingKernelSqueenzeKernel, Int64) {
  ...
  EXPECT_TestSqueeze<int64_t, DT_INT64>(data_vec, dim_value_vec, result);
}
...
TEST_F(UtestGraphPassesFoldingKernelSqueenzeKernel, Uint8) {
  ...
  EXPECT_TestSqueeze<uint8_t, DT_UINT8>(data_vec, dim_value_vec, result);
}
...
TEST_F(UtestGraphPassesFoldingKernelSqueenzeKernel, Uint16) {
  ...
  EXPECT_TestSqueeze<uint16_t, DT_UINT16>(data_vec, dim_value_vec, result);
}
...
TEST_F(UtestGraphPassesFoldingKernelSqueenzeKernel, Uint32) {
  ...
  EXPECT_TestSqueeze<uint32_t, DT_UINT32>(data_vec, dim_value_vec, result);
}
...
TEST_F(UtestGraphPassesFoldingKernelSqueenzeKernel, Uint64) {
  ...
  vector<int32_t> dim_value_vec = {1};
  vector<int64_t> result = {6, 12, 3, 4, 56, 7};
  EXPECT_TestSqueeze<uint64_t, DT_UINT64>(data_vec, dim_value_vec, result);
}
...
TEST_F(UtestGraphPassesFoldingKernelSqueenzeKernel, Fp16) {
  ...
  vector<int32_t> dim_value_vec = {1};
  vector<int64_t> result = {6, 12, 3, 4, 56, 7};
  EXPECT_TestSqueeze<fp16_t, DT_FLOAT16>(data_vec, dim_value_vec, result);
}
...
TEST_F(UtestGraphPassesFoldingKernelSqueenzeKernel, Float) {
  ...
  EXPECT_TestSqueeze<float, DT_FLOAT>(data_vec, dim_value_vec, result);
}
...
TEST_F(UtestGraphPassesFoldingKernelSqueenzeKernel, Double) {
  ...
  vector<int32_t> dim_value_vec = {0, 1, 2, 4, 5, 6};
  vector<int64_t> result = {11};
  EXPECT_TestSqueeze<double, DT_DOUBLE>(data_vec, dim_value_vec, result);
}
...
TEST_F(UtestGraphPassesFoldingKernelSqueenzeKernel, DoubleContainSameDimsInvalidFormat) {
  ...
  vector<int32_t> dim_value_vec = {0, 1, 0};
  vector<int64_t> result = {1, 11, 1, 1, 1};
  EXPECT_TestInvalidSqueeze<double, DT_DOUBLE, FORMAT_NC1HWC0>(data_vec, dim_value_vec, result);
}
