...
ComputeGraphPtr BuildGraph2() {
  ...
  var1->GetOpDesc()->MutableOutputDesc(0)->SetOriginShape(GeShape(std::vector<int64_t>({8 * 8 * 3, 2})));
  auto reshape1 =
  ...
  reshape1->GetOpDesc()->MutableInputDesc(0)->SetOriginShape(GeShape(std::vector<int64_t>({8 * 8 * 3, 2})));
  reshape1->GetOpDesc()->MutableOutputDesc(0)->SetOriginShape(GeShape(std::vector<int64_t>({8, 8, 3, 2})));
  auto conv1 = builder.AddNode("conv1", "Conv2D", 2, 1, FORMAT_HWCN, DT_FLOAT, std::vector<int64_t>({8, 8, 3, 2}));
  conv1->GetOpDesc()->MutableInputDesc(0)->SetOriginShape(GeShape(std::vector<int64_t>({8, 8, 3, 2})));
...
ComputeGraphPtr BuildGraph4() {
  auto builder = ut::GraphBuilder("g1");
  auto var1 = builder.AddNode("var1", "Variable", 0, 1, FORMAT_ND, DT_FLOAT, std::vector<int64_t>({8, 8, 3, 2}));
  auto cast1 = builder.AddNode("cast1", "Cast", 1, 1, FORMAT_NCHW, DT_FLOAT16, std::vector<int64_t>({8, 8, 3, 2}));
  cast1->GetOpDesc()->MutableInputDesc(0)->SetFormat(FORMAT_NCHW);
  cast1->GetOpDesc()->MutableInputDesc(0)->SetDataType(DT_FLOAT);
  cast1->GetOpDesc()->MutableInputDesc(0)->SetShape(GeShape(std::vector<int64_t>({8, 8, 3, 2})));
  auto conv1 = builder.AddNode("conv1", "Conv2D", 2, 1, FORMAT_NCHW, DT_FLOAT16, std::vector<int64_t>({8, 8, 3, 2}));

  builder.AddDataEdge(var1, 0, cast1, 0);
  builder.AddDataEdge(cast1, 0, conv1, 1);
...
class VariableOpPassSimulator {
 public:
  bool DoTest0() {
    ...
    for (auto name : var_names) {
      ...
      ge::VarManager::Instance(session_id)->AssignVarMem(name, nullptr, var_tensor_desc, RT_MEMORY_HBM);
      ge::VarManager::Instance(session_id)->SetVarAddr(name, var_tensor_desc, dev_ptr, RT_MEMORY_HBM, nullptr);
    }

    ge::GraphNodePtr graph_node = std::make_shared<GraphNode>(0);
    GraphUtilsEx::InferShapeInNeed(compute_graph);
    graph_node->SetComputeGraph(compute_graph);
    auto tmp_graph = GraphUtilsEx::CreateGraphFromComputeGraph(compute_graph);
  ...
  bool DoTest3() {
    ...
    for (auto var_name : var_names) {
      ...
      const uint8_t *dev_ptr = nullptr;
      ge::VarManager::Instance(session_id)->AssignVarMem(var_name, nullptr, var_tensor_desc, RT_MEMORY_HBM);
      ge::VarManager::Instance(session_id)->SetVarAddr(var_name, var_tensor_desc, dev_ptr, RT_MEMORY_HBM, nullptr);
    }
 ...
 private:
  ...
  bool CheckTest8(const ge::ComputeGraphPtr compute_graph) {
    const auto &variable_node = compute_graph->FindNode("Node4D");
    (void)variable_node->GetOpDesc()->GetOutputDesc(0).GetFormat();
    (void)variable_node->GetOpDesc()->GetOutputDesc(0).GetDataType();
    (void)variable_node->GetOpDesc()->GetOutputDesc(0).GetShape().GetDims();
    return true;
  };
};

TEST_F(UtestVariableOpPassUnit, test_trans_data_remove) {
  VariableOpPassSimulator variable_op_pass_simulator;

  bool result = variable_op_pass_simulator.DoTest0();
...
TEST_F(UtestVariableOpPassUnit, test_variable_ref) {
  ...
  bool result = variable_op_pass_simulator.DoTest1();
...
TEST_F(UtestVariableOpPassUnit, test_null_graph) {
  ...
  bool result = variable_op_pass_simulator.DoTest2();
...
TEST_F(UtestVariableOpPassUnit, test_covarage_trans_var_data) {
  ...
  bool result = variable_op_pass_simulator.DoTest3();
...
TEST_F(UtestVariableOpPassUnit, test_illegally_ref) {
  ...
  bool result = variable_op_pass_simulator.DoTest4();
  EXPECT_EQ(result, true);
}
...
TEST_F(UtestVariableOpPassUnit, test_single_node) {
  ...
  bool result = variable_op_pass_simulator.DoTest5();
  EXPECT_EQ(result, true);
}
...
TEST_F(UtestVariableOpPassUnit, test_un_mathed) {
  ...
  bool result = variable_op_pass_simulator.DoTest6();
  EXPECT_EQ(result, true);
}
...
TEST_F(UtestVariableOpPassUnit, test_same_op) {
  ...
  bool result = variable_op_pass_simulator.DoTest7();
  EXPECT_EQ(result, false);
}
...
TEST_F(UtestVariableOpPassUnit, test_error_return) {
  ...
  bool result = variable_op_pass_simulator.DoTest8();
  EXPECT_EQ(result, true);
}

TEST_F(UtestVariableOpPassUnit, DeleteReshapeAndCheckShapeSuccess) {
  // init
  MemManager::Instance().Initialize(std::vector<rtMemType_t>({RT_MEMORY_HBM}));
  ...
  auto tmp_graph = GraphUtilsEx::CreateGraphFromComputeGraph(graph);
  auto tmp_graph_ptr = std::make_shared<Graph>(tmp_graph);
  graph_node->SetGraph(tmp_graph_ptr);
  ...
  EXPECT_EQ(var1->GetOpDesc()->GetOutputDesc(0).GetOriginShape().GetDims(), std::vector<int64_t>({8, 8, 3, 2}));
}
...
TEST_F(UtestVariableOpPassUnit, trans_road_insert_reformat) {
  // init
  MemManager::Instance().Initialize(std::vector<rtMemType_t>({RT_MEMORY_HBM}));
  VarManager::Instance(1)->Init(0, 0, 0, 0);
  auto graph = BuildGraph4();
  graph->SetSessionID(1);
  auto var1 = graph->FindNode("var1");
  ...
  VariableOpPass pass(&ctrl);
  EXPECT_EQ(pass.Run(graph), ge::SUCCESS);
  VarTransRoad *const trans_road = VarManager::Instance(graph->GetSessionID())->GetTransRoad("var1");
  EXPECT_EQ(trans_road->begin()->node_type, "ReFormat");
  MemManager::Instance().Finalize();
}
...
static ComputeGraphPtr BuildGraphVariableOpPass() {
  DEF_GRAPH(variableOpGraph) {
    CHAIN(NODE("translate0", TRANSLATE)->EDGE(0U, 0U)->NODE("cast0", CAST));
    CHAIN(NODE("translate1", TRANSLATE)->EDGE(0U, 1U)->NODE("cast0", CAST));
  };

  const auto graph = ToGeGraph(variableOpGraph);
  const auto compute_graph = GraphUtilsEx::GetComputeGraph(graph);
  compute_graph->TopologicalSorting();

  return compute_graph;
}
TEST_F(UtestVariableOpPassUnit, RunFailed) {
  auto graph = BuildGraphVariableOpPass();
  GraphRebuildStateCtrl ctrl;
  VariableOpPass pass(&ctrl);

  pass.var_accelerate_ctrl_ = nullptr;
  auto status = pass.Run(graph);
  EXPECT_EQ(INTERNAL_ERROR, status);

  graph->impl_->parent_graph_ = std::weak_ptr<ComputeGraph>(graph);
  status = pass.Run(graph);
  EXPECT_EQ(SUCCESS, status);
}

TEST_F(UtestVariableOpPassUnit, UpdateVarAndRefOutputFormatInfoFailed) {
  GraphRebuildStateCtrl ctrl;
  VariableOpPass pass(&ctrl);
  auto graph = BuildGraphVariableOpPass();
  auto node = graph->FindNode("translate0");
  auto status = pass.UpdateVarAndRefOutputFormatInfo(node->GetOpDesc()->GetOutputDesc(0U), nullptr, nullptr);
  EXPECT_EQ(FAILED, status);

  status = pass.UpdateVarAndRefOutputFormatInfo(node->GetOpDesc()->GetOutputDesc(0U), node, nullptr);
  EXPECT_EQ(SUCCESS, status);
}

TEST_F(UtestVariableOpPassUnit, CheckVarAndVarRefAreAlikeFailed) {
  GraphRebuildStateCtrl ctrl;
  VariableOpPass pass(&ctrl);
  auto graph = BuildGraphVariableOpPass();
