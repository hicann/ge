...
TEST_F(UtestGraphPassesMergePass, empty_input_cut_branch) {
  ...
  EXPECT_EQ(graph_->GetDirectNodesSize(), 4UL);
  EXPECT_EQ(graph_->FindNode("Merge"), nullptr);
  EXPECT_EQ(graph_->FindNode("Op2"), nullptr);
  ...
  EXPECT_EQ(graph_->GetDirectNodesSize(), 2UL);
  EXPECT_EQ(graph_->FindNode("Merge2"), nullptr);
  EXPECT_EQ(graph_->FindNode("Op3"), nullptr);
}
...
TEST_F(UtestGraphPassesMergePass, IsMergeInputNeedOptimized1) {
  ...
  auto node2 = NewNode("Op2", MEMCPYADDRASYNC, 1, 1);
  auto node3 = NewNode("Op3", MEMCPYADDRASYNC, 1, 1);
  auto merge_node3 = NewNode("Merge3", MEMCPYASYNC, 3, 3);
  ...
  ret = pass_.IsMergeInputNeedOptimized(merge_node3);
  EXPECT_EQ(ret, false);
}
...
TEST_F(UtestGraphPassesMergePass, IsMergeInputNeedOptimized2) {
  ...
  bool ret = pass_.IsMergeInputNeedOptimized(merge_node3);
  EXPECT_EQ(ret, true);
}
