...
TEST_F(UtestGraphInferValueRangePass, UpdateTensorDesc_failed) {
  ...
  bool changed = true;
  graphStatus ret = infer_pass.UpdateTensorDesc(src, dst, changed);
...
TEST_F(UtestGraphInferValueRangePass, ConstructDataByType) {
  ...
  int32_t weight[1] = {1};
  GeTensorPtr tensor = std::make_shared<GeTensor>(ge_tensor_desc, (uint8_t *)weight, sizeof(weight));

  InferValueRangePass infer_pass;
  graphStatus ret = infer_pass.ConstructDataByType(ge_tensor_desc, use_floor_value, tensor);
  EXPECT_EQ(ret, GRAPH_PARAM_INVALID);

  GeTensorDesc ge_tensor_desc2(GeShape({1, 1, 4, 192}), ge::FORMAT_NCHW, DT_DOUBLE);
  ret = infer_pass.ConstructDataByType(ge_tensor_desc2, use_floor_value, tensor);
  EXPECT_EQ(ret, GRAPH_PARAM_INVALID);

  GeTensorDesc ge_tensor_desc3(GeShape({1, 1, 4, 192}), ge::FORMAT_NCHW, DT_UINT8);
  ret = infer_pass.ConstructDataByType(ge_tensor_desc3, use_floor_value, tensor);
  EXPECT_EQ(ret, GRAPH_PARAM_INVALID);

  GeTensorDesc ge_tensor_desc4(GeShape({1, 1, 4, 192}), ge::FORMAT_NCHW, DT_INT8);
  ret = infer_pass.ConstructDataByType(ge_tensor_desc4, use_floor_value, tensor);
  EXPECT_EQ(ret, GRAPH_PARAM_INVALID);

  GeTensorDesc ge_tensor_desc5(GeShape({1, 1, 4, 192}), ge::FORMAT_NCHW, DT_UINT16);
  ret = infer_pass.ConstructDataByType(ge_tensor_desc, use_floor_value, tensor);
  EXPECT_EQ(ret, GRAPH_PARAM_INVALID);

  GeTensorDesc ge_tensor_desc6(GeShape({1, 1, 4, 192}), ge::FORMAT_NCHW, DT_INT16);
  ret = infer_pass.ConstructDataByType(ge_tensor_desc, use_floor_value, tensor);
  EXPECT_EQ(ret, GRAPH_PARAM_INVALID);

  GeTensorDesc ge_tensor_desc7(GeShape({1, 1, 4, 192}), ge::FORMAT_NCHW, (DataType)(30));
  ret = infer_pass.ConstructDataByType(ge_tensor_desc7, use_floor_value, tensor);
  EXPECT_EQ(ret, GRAPH_PARAM_INVALID);
}
...
TEST_F(UtestGraphInferValueRangePass, UpdateOutputFromSubgraphsForSubgraphMultiDims) {
  ...
  GeTensorDescPtr dst = make_shared<ge::GeTensorDesc>();

  graphStatus ret = infer_pass.UpdateOutputFromSubgraphsForSubgraphMultiDims(src, dst);
  EXPECT_EQ(ret, GRAPH_FAILED);
}
...
TEST_F(UtestGraphInferValueRangePass, SkipInfer_WhenOutputShapeSizeTooLarge_InMemoryPriority) {
  ...
  GeTensorDesc shape_tensor_desc(GeShape({4}), ge::FORMAT_NCHW, ge::DT_INT64);
  std::vector<std::pair<int64_t, int64_t>> value_range = {make_pair(1, 4), make_pair(1, 240), make_pair(4, 4),
                                                          make_pair(192, 192)};
