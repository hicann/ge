...
TEST_F(HeterogeneousModelExecutorTest, TestMbufEnqueueSuccess) {
  constexpr int32_t kQueueDefaultTimeout = 10 * 60 * 1000;
  ASSERT_EQ(default_executor_->Initialize(), SUCCESS);
  const int tensor_des_size = sizeof(RuntimeTensorDesc);
  std::vector<GeTensor> input_tensors;
  for (size_t i = 0; i < default_executor_->input_queue_attrs_.size(); ++i) {
    int64_t tensor_size = 0L;
    auto ge_shape = default_executor_->input_tensor_desc_[i]->GetShape();
    GeTensorDesc tensor_desc = GeTensorDesc(ge_shape);
    TensorUtils::CalcTensorMemSize(ge_shape, FORMAT_ND, DT_FLOAT, tensor_size);
    default_executor_->input_is_no_tiling_[i] = true;

    GeTensor tensor;
    void *pd = malloc(tensor_des_size + tensor_size);
    memset_s(pd, tensor_des_size + tensor_size, 0, tensor_des_size + tensor_size);

    uint8_t *pdata = PtrAdd<uint8_t>(PtrToPtr<void, uint8_t>(pd), tensor_des_size + 1, tensor_des_size);
    tensor.SetTensorDesc(tensor_desc);
    tensor.SetData(pdata, tensor_size, [](uint8_t *){});
    input_tensors.emplace_back(tensor);
  }
  ...
  GetThreadLocalContext().SetSessionOption(options);
  GE_MAKE_GUARD(recover, [options_bk]() { GetThreadLocalContext().SetSessionOption(options_bk); });
  ASSERT_TRUE(ExecutionRuntimeUtils::IsHeterogeneous());
  ASSERT_TRUE(ExecutionRuntime::GetInstance() != nullptr);

  setenv("ENABLE_MBUF_ALLOCATOR", "1", true);
  for (size_t i = 0; i < input_tensors.size(); ++i) {
    ExchangeService::ControlInfo control_info{};
    control_info.timeout = kQueueDefaultTimeout;
    ASSERT_EQ(default_executor_->EnqueueInputTensor(input_tensors[i], i, control_info), SUCCESS);
  }
  for (size_t i = 0; i < input_tensors.size(); ++i) {
    free(const_cast<void *>(PtrToPtr<uint8_t, void>(input_tensors[i].GetData().GetData() - tensor_des_size)));
  }
  ExecutionRuntime::handle_ = original_handle;
...
TEST_F(HeterogeneousModelExecutorTest, TestNotSharedMbufEnqueueSuccess) {
  constexpr int32_t kQueueDefaultTimeout = 10 * 60 * 1000;
  ASSERT_EQ(default_executor_->Initialize(), SUCCESS);
  const int tensor_des_size = sizeof(RuntimeTensorDesc);
  std::vector<GeTensor> input_tensors;
  std::vector<std::shared_ptr<void>> vec_ptr;
  for (size_t i = 0; i < default_executor_->input_queue_attrs_.size(); ++i) {
    int64_t tensor_size = 0L;
    auto ge_shape = default_executor_->input_tensor_desc_[i]->GetShape();
    GeTensorDesc tensor_desc = GeTensorDesc(ge_shape);
    ...
    GeTensor tensor;
    void *pd = malloc(tensor_des_size + tensor_size);
    vec_ptr.push_back(std::shared_ptr<void>(pd, free));
    memset_s(pd, tensor_des_size + tensor_size, 0, tensor_des_size + tensor_size);
  ...
  EXPECT_CALL(*runtime_stub, rtBuffConfirm).WillRepeatedly(Return(RT_FAILED));
  for (size_t i = 0; i < input_tensors.size(); ++i) {
    ExchangeService::ControlInfo control_info{};
    control_info.timeout = kQueueDefaultTimeout;
    ASSERT_EQ(default_executor_->EnqueueInputTensor(input_tensors[i], i, control_info), SUCCESS);
