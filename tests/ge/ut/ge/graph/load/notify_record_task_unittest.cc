...
namespace ge {
class UtestNotifyRecordTask : public testing::Test {
 protected:
  void SetUp() {}
...
// test Init_NotifyRecordTaskInfo
TEST_F(UtestNotifyRecordTask, init_and_distribute_notify_record_task_info) {
...
TEST_F(UtestNotifyRecordTask, init_and_distribute_engine_notify_record_task_info) {
  domi::TaskDef task_def;
  task_def.set_stream_id(0);
  ...
  EXPECT_TRUE(AttrUtils::SetStr(op_desc, "ep_group", "test_group"));
  ...
  task_def.set_private_def("ep_group");
  EXPECT_EQ(task_info.Init(task_def, &model), SUCCESS);
  EXPECT_EQ(task_info.davinci_model_->GetOpByIndex(task_info.op_index_)->GetName(), "op_name");
  EXPECT_EQ(task_info.Distribute(), SUCCESS);
}
