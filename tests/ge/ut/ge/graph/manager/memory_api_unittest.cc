...
TEST_F(UtestMemoryApiTest, RdmaRemoteRegister) {
  HostVarInfo info = {1, 1};
  std::vector<HostVarInfo> var_info = {info};
  rtMemType_t mem_type = RT_MEMORY_RDMA_HBM;
  EXPECT_NE(RdmaRemoteRegister(var_info, mem_type), SUCCESS);
}

TEST_F(UtestMemoryApiTest, MallocSharedMemory) {
  std::vector<int64_t> dims = {1};
  TensorInfo tensor_info = {"test", dims, DT_INT32};
  uint64_t dev_addr;
  uint64_t memory_size;
  EXPECT_NE(MallocSharedMemory(tensor_info, dev_addr, memory_size), SUCCESS);
}
