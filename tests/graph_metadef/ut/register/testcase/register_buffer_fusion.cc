...
vector<BufferFusionPattern *> TbeCommonRules0FusionPass::DefinePatterns() {
  ...
  pattern
      ...
      .AddOpDesc(PATTERN_DEQUANT, {OP_PATTERN_DEQUANT}, TBE_PATTERN_NUM_NONE, TBE_PATTERN_NUM_DEFAULT,
                 TBE_PATTERN_GROUPID_INVALID, IGNORE_SHAPE_TYPE)
      .AddOpDescTypeRules(PATTERN_ELEMWISE, {OP_PATTERN_ELEMWISE, OP_PATTERN_BROAD_CAST}, TBE_PATTERN_NUM_NONE,
                          TBE_PATTERN_NUM_MAX, TBE_PATTERN_GROUPID_INVALID, {IGNORE_SHAPE_TYPE, ONLY_SUPPORT_STATIC})
...
REGISTER_BUFFER_FUSION_PASS("", BUILT_IN_AI_CORE_BUFFER_FUSION_PASS, TbeCommonRules0FusionPass);
...
Status Run(ge::ComputeGraph &graph, std::shared_ptr<BufferFusionPassBase> &pass) {
  ...
  for (BufferFusionPattern *pattern : patterns) {
    ...
    std::vector<BufferFusionOpDesc *> buffer_fusion_op_vec = {conv, dequant, elmw, quant};
    int i = 0;
...
class UB_FUSION_UT_CONV_ELT_RELU : public testing::Test {
 protected:
  ...
  void BuildGraphForL2Fusion(ComputeGraphPtr graph, int32_t reluflag) {
    ...
    rtSmData_t tmp_data = {L2_mirror_addr,           L2_data_section_size, L2_preload,          modified,      priority,
                           prev_L2_page_offset_base, L2_page_offset_base,  L2_page_offset_base, L2_load_to_ddr};
    ...
    elemwise_node->GetOpDesc()->SetExtAttr("task_l2_fusion_info_extend_content", elementwise_l2_info_ptr);
    ...
    relu_node->GetOpDesc()->SetExtAttr("task_l2_fusion_info_extend_content", relu_l2_info_ptr);
    ...
    relu1_node->GetOpDesc()->SetExtAttr("task_l2_fusion_info_extend_content", relu2_l2_info_ptr);
  }
  void BuildGraphForL2Fusion1(ComputeGraphPtr graph, int32_t reluflag) {
    ...
    GraphUtils::AddEdge(conv_node->GetOutDataAnchor(0), elemwise_node->GetInDataAnchor(0));
    GraphUtils::AddEdge(data2_node->GetOutDataAnchor(0), elemwise_node->GetInDataAnchor(1));
    GraphUtils::AddEdge(elemwise_node->GetOutDataAnchor(0), elemwise1_node->GetInDataAnchor(0));
    GraphUtils::AddEdge(elemwise1_node->GetOutDataAnchor(0), relu_node->GetInDataAnchor(0));
    ...
    elemwise1_node->GetOpDesc()->SetExtAttr("task_l2_fusion_info_extend_content", elementwise_l2_info_ptr);
  }
  ...
  void BuildGraphConvEltReluQuant1(ComputeGraphPtr graph, int32_t reluflag) {
    ...
    GraphUtils::AddEdge(data2_node->GetOutDataAnchor(0), eltwise_node->GetInDataAnchor(1));
  ...
  void BuildGraphdoubleConvEltElt(ComputeGraphPtr graph, int32_t reluflag) {
    ...
    ge::OpKernelBinPtr tbe_kernel_ptr = std::make_shared<ge::OpKernelBin>(conv_node->GetName(), std::move(buffer));
    conv_node->GetOpDesc()->SetExtAttr(OP_EXTATTR_NAME_TBE_KERNEL, tbe_kernel_ptr);
    ge::OpKernelBinPtr tbe_kernel_ptr1 = std::make_shared<ge::OpKernelBin>(conv_node1->GetName(), std::move(buffer));
    conv_node1->GetOpDesc()->SetExtAttr(OP_EXTATTR_NAME_TBE_KERNEL, tbe_kernel_ptr1);

    GraphUtils::AddEdge(data_node->GetOutDataAnchor(0), conv_node->GetInDataAnchor(0));
    GraphUtils::AddEdge(data1_node->GetOutDataAnchor(0), conv_node->GetInDataAnchor(1));
    GraphUtils::AddEdge(data_node->GetOutDataAnchor(0), conv_node1->GetInDataAnchor(0));
    GraphUtils::AddEdge(data1_node->GetOutDataAnchor(0), conv_node1->GetInDataAnchor(1));
    GraphUtils::AddEdge(conv_node->GetOutDataAnchor(0), elemwise_node->GetInDataAnchor(0));
    GraphUtils::AddEdge(conv_node1->GetOutDataAnchor(0), elemwise_node->GetInDataAnchor(1));
    GraphUtils::AddEdge(elemwise_node->GetOutDataAnchor(0), relu_node->GetInDataAnchor(0));
    GraphUtils::AddEdge(relu_node->GetOutDataAnchor(0), relu1_node->GetInDataAnchor(0));
  }
