...
RETURN_STATEMENTS = {
    ...
    "CustomOpRegistry& CustomOpFactory::": "    static CustomOpRegistry registry;\n    return registry;",
    ...
    "AscendString::AscendString(const char_t *const name)": r"""
          if (name != nullptr) {
            name_ = MakeShared<std::string>(name);
          }
        """,
    "AscendString::AscendString(const char_t *const name, size_t length)": r"""
          if (name != nullptr) {
            name_ = MakeShared<std::string>(name, length);
          }
        """,
    "const char_t* AscendString::GetString() const": r"""
          if (name_ == nullptr) {
...
def need_generate_func(func_line):
    ...
    """
    if (
        func_line.strip().endswith("default")
        or func_line.strip().endswith("delete")
        or func_line.strip().startswith("typedef")
        or func_line.strip().startswith("using")
    ):
        return False
    return True
...
pattern_virtual_func = re.compile(
    r"""^\s*virtual\s+(?:const\s+)?[:\w]+[ &*]+[:\w]+\([^()]*\)\s+(?:const\s+)?=\s+0;$""",
    re.VERBOSE,
)
# pattern comment
...
pattern_visibility = re.compile(
    r"(FMK_FUNC_HOST_VISIBILITY|FMK_FUNC_DEV_VISIBILITY|VISIBILITY_EXPORT|"
    r"PARSER_FUNC_VISIBILITY|ACL_FUNC_VISIBILITY|GE_FUNC_VISIBILITY|"
    r"ASCEND_FUNC_VISIBILITY|GE_OBJECT_VISIBILITY|GE_FUNC_DEV_VISIBILITY|"
    r"GE_FUNC_HOST_VISIBILITY|FUNC_VISIBILITY) *"
)
# pattern override
...
class H2CC(object):
    ...
    def handle_extern_variable(self):
        ...
        if re.search(r"^\s*extern\s+const.+(?!=).+;$", line):
            ...
            if re.search(r"(std::)?(map|set|vector)", line):
                ...
                default_value = '""'
            line = re.sub(r"^([^;]+)(;)$", rf"\1 = {default_value}\2\n", line)
            # line += ' = 0;\n'
    ...
    def h2cc(self):
        logging.info(
            "start generate cc_file[%s] from h_file[%s]",
            self.output_file,
            self.input_file,
        )
    ...
    @staticmethod
    def _remove_template_content(line):
        result = []
        depth = 0
        for char in line:
            if char == '<':
    ...
    def handle_func(self, line):
        ...
        # or 'VISIBILITY_MACRO returnType \n funcName(...)'
        pre_line = self.input_content[start_i - 1]
        stripped_pre_line = pattern_visibility.sub("", pre_line)
    ...
    def write_func_content(self, content, func_name, need_generate):
        if func_name not in self.func_list_exist and need_generate:
    ...
    def get_return_statements(func):
        ...
        if not m:
            return None
        logging.info(
            "ret_type: %s, class_name: %s, func_name: %s",
...
def collect_header_files(inc_file):
    ...
    need_any_value_headers = [
        "op_impl_registry_base.h",
