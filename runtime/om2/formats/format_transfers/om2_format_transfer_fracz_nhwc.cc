...
#include "formats/format_transfers/om2_format_transfer_fracz_nhwc.h"
...
namespace formats {
namespace {
bool CheckDataTypeSupportedForFracZToNhwc(const DataType data_type) {
...
Status CheckArgsForFracZToNhwc(const TransArgs &args) {
  ...
  if (!CheckDataTypeSupportedForFracZToNhwc(args.src_data_type)) {
    ...
    REPORT_INNER_ERR_MSG("E19999", "Failed to trans shape from FORMAT_FRACTAL_Z to NCHW, invalid data type %s",
                         TypeUtilsInner::DataTypeToSerialString(args.src_data_type).c_str());
...
Status GetDstDataAfterTransForFracZToNhwc(const TransArgs &args, TransResult &result, const int32_t size,
