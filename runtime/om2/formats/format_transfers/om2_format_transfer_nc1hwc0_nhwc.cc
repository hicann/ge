...
#include "formats/format_transfers/om2_format_transfer_nc1hwc0_nhwc.h"
...
namespace formats {
namespace {
bool CheckDataTypeForNc1hwc0ToNhwc(const DataType &data_type) {
...
Status CheckArgsForNc1hwc0ToNhwc(const TransArgs &args) {
  ...
  if ((args.src_primary_format != FORMAT_NC1HWC0) || (args.dst_primary_format != FORMAT_NHWC)) {
    const std::string error = "Dose not support trans format from " +
                              ...
                              FmtToStr(TypeUtilsInner::FormatToSerialString(args.dst_primary_format));
    GE_ERRORLOG_AND_ERRORMSG(ACL_ERROR_GE_FORMAT_INVALID, error.c_str());
    return ACL_ERROR_GE_FORMAT_INVALID;
  }
  if (!CheckDataTypeForNc1hwc0ToNhwc(args.src_data_type)) {
  ...
  if ((src_shape.at(kNc1hwc0H) != dst_shape.at(kNhwcH)) || (src_shape.at(kNc1hwc0W) != dst_shape.at(kNhwcW)) ||
    ...
    GELOGE(ACL_ERROR_GE_SHAPE_INVALID,
           "[Check][Shape]Failed to check relationship between "
           "src shape %s and dst shape %s",
           ShapeToString(src_shape).c_str(), ShapeToString(dst_shape).c_str());
    REPORT_INNER_ERR_MSG("E19999",
                         "Failed to check relationship between src shape %s "
                         "and dst shape %s",
                         ShapeToString(src_shape).c_str(), ShapeToString(dst_shape).c_str());
    return ACL_ERROR_GE_SHAPE_INVALID;
  }
...
Status GetDstDataForNc1hwc0ToNhwc(const TransArgs &args, TransResult &result, const int32_t size,
        ...
        for (int64_t c_idx = 0; c_idx < c; c_idx++) {
          ...
          if (ret != EOK) {
            GELOGE(ACL_ERROR_GE_MEMORY_OPERATE_FAILED,
                   "[Copy][Data]Failed, data from NC1HWC0[%" PRId64 ", %" PRId64 ", %" PRId64 ", %" PRId64
                   ", "
                   "%" PRId64 "] offset %" PRId64 " to NHWC[%" PRId64 ", %" PRId64 ", %" PRId64 ", %" PRId64
                   "] "
                   "offset %" PRId64 ", err-code %d",
                   n_idx, c1_idx, h_idx, w_idx, c0_idx, src_offset, n_idx, c_idx, h_idx, w_idx, dst_offset, ret);
            REPORT_INNER_ERR_MSG("E19999",
                                 "Failed to copy data from NC1HWC0[%" PRId64 ", %" PRId64 ", %" PRId64
                                 ", "
                                 "%" PRId64 ", %" PRId64 "] offset %" PRId64 " to NHWC[%" PRId64 ", %" PRId64
                                 ", "
                                 "%" PRId64 ", %" PRId64 "] offset %" PRId64 ", err-code %d",
                                 n_idx, c1_idx, h_idx, w_idx, c0_idx, src_offset, n_idx, c_idx, h_idx, w_idx,
                                 dst_offset, ret);
...
Status FormatTransferNc1hwc0Nhwc::TransFormat(const TransArgs &args, TransResult &result) {
  ...
  GELOGD(
      "[Trans][Format]Begin to trans format from NC1HWC0 to NCHW, "
      "src shape %s, data type %s, dst shape %s, memory size %" PRId64 "",
      ShapeToString(args.src_shape).c_str(), TypeUtilsInner::DataTypeToSerialString(args.src_data_type).c_str(),
      ShapeToString(args.dst_shape).c_str(), total_size);

  ret = GetDstDataForNc1hwc0ToNhwc(args, result, size, total_size);
  if (ret != SUCCESS) {
    GELOGE(ret,
...
Status FormatTransferNc1hwc0Nhwc::TransShape(const Format src_format, const std::vector<int64_t> &src_shape,
