...
#include "formats/format_transfers/om2_format_transfer_fracz_nchw.h"
...
namespace formats {
namespace {
bool CheckDataTypeSupportedForFracZToNchw(const DataType data_type) {
...
Status CheckArgsForFracZToNchw(const TransArgs &args) {
  ...
  if (!CheckDataTypeSupportedForFracZToNchw(args.src_data_type)) {
    ...
    REPORT_INNER_ERR_MSG("E19999",
                         "Failed to trans shape from FORMAT_FRACTAL_Z to NCHW, "
                         "invalid data type %s",
                         TypeUtilsInner::DataTypeToSerialString(args.src_data_type).c_str());
  ...
  const int64_t c1 = Ceil(dst_shape.at(kNchwC), c0);
  const int64_t n0 = Ceil(dst_shape.at(kNchwN), static_cast<int64_t>(kNiSize));
  if ((src_shape.at(kFracZHWC1) != (dst_shape.at(kNchwH) * dst_shape.at(kNchwW) * c1)) ||
    ...
    REPORT_INNER_ERR_MSG("E19999",
                         "Failed to check relationship between src and dst shape, "
...
Status GetDstDataAfterTransForFracZToNchw(const TransArgs &args, TransResult &result, const int32_t size,
  ...
  const auto h = args.dst_shape.at(kNchwH);
  const auto w = args.dst_shape.at(kNchwW);
  const auto c = args.dst_shape.at(kNchwC);
  const auto n = args.dst_shape.at(kNchwN);
      ...
      for (int64_t h_idx = 0; h_idx < h; h_idx++) {
        ...
        for (int64_t w_idx = 0; w_idx < w; w_idx++) {
          ...
          if (ret != EOK) {
            ...
            REPORT_INNER_ERR_MSG("E19999",
                                 "Failed to copy data from FracZ offset %" PRId64
                                 " to "
...
Status FormatTransferFracZNchw::TransFormat(const TransArgs &args, TransResult &result) {
  ...
  GELOGD("Begin to trans format from FracZ to NCHW, src shape %s, data type %s, dst shape %s, memory size %" PRId64 "",
         ShapeToString(args.src_shape).c_str(), TypeUtilsInner::DataTypeToSerialString(args.src_data_type).c_str(),
         ShapeToString(args.dst_shape).c_str(), total_size);

  ret = GetDstDataAfterTransForFracZToNchw(args, result, size, total_size);
  if (ret != SUCCESS) {
    GELOGE(ret,
    ...
    REPORT_INNER_ERR_MSG("E19999",
                         ...
                         "data type %s, dst shape %s, memory size %" PRId64 "",
...
Status FormatTransferFracZNchw::TransShape(const Format src_format, const std::vector<int64_t> &src_shape,
