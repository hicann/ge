...
#include "formats/format_transfers/om2_format_transfer_nchw_nc1hwc0.h"
...
Status CheckArgsForNchwToNc1hwc0(const TransArgs &args) {
  ...
  if (expect_5d_shape != args.dst_shape) {
    GELOGE(ACL_ERROR_GE_SHAPE_INVALID,
           ...
           TypeUtilsInner::DataTypeToSerialString(args.src_data_type).c_str(), ShapeToString(args.src_shape).c_str(),
           ShapeToString(args.dst_shape).c_str(), ShapeToString(expect_5d_shape).c_str());
    REPORT_INNER_ERR_MSG("E19999",
                         "Failed to trans formats, the src and dst shape are not "
...
Status GetDstDataAfterTransOfNchw2Nc1hwc0(const TransArgs &args, TransResult &result, const int32_t size,
  ...
  if (dst == nullptr) {
    GELOGE(ACL_ERROR_GE_MEMORY_ALLOCATION,
           "[Allocate][Memory]Failed to alloc the memory for dst buf %" PRId64
           ", "
  ...
  const int64_t c0 = GetC0Value(static_cast<int32_t>(args.dst_format));
  if (c0 <= 0) {
    GELOGE(ACL_ERROR_GE_DATATYPE_INVALID, "[Check][Shape]The c0 is invalid %" PRId64 ", data_type %s", c0,
           TypeUtilsInner::DataTypeToSerialString(args.src_data_type).c_str());
    REPORT_INNER_ERR_MSG("E19999", "Check shape failed, the c0 is invalid %" PRId64 ", data_type %s", c0,
                         TypeUtilsInner::DataTypeToSerialString(args.src_data_type).c_str());
    return ACL_ERROR_GE_DATATYPE_INVALID;
  }
          ...
          for (int64_t c0_idx = 0; c0_idx < c0; c0_idx++) {
            ...
            if (cIdx < c) {
              ...
              if (ret != EOK) {
                ...
                REPORT_INNER_ERR_MSG("E19999",
                                     "Failed to copy data from NCHW[%" PRId64 "] offset %" PRId64
                                     " "
                                     "to NC1HWC0[%" PRId64 ", %" PRId64 ", %" PRId64 ", %" PRId64 ", %" PRId64
                                     "] "
