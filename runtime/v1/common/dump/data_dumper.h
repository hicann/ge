...
class DataDumper {
 public:
  ...
  void SetRefInfo(const std::map<OpDescPtr, void *> &ref_info) {
    ref_info_ = ref_info;
  };

  void SetL1FusionAddr(const uintptr_t addr) {
    l1_fusion_addr_ = addr;
  };
  ...
  void SaveDumpTask(const OpDescInfoId &id, const std::shared_ptr<OpDesc> &op_desc, const uintptr_t args,
                    ...
                    rtStream_t const stream = nullptr);
  ...
  void SetSingleOpDebug() {
    is_single_op_debug_ = true;
  }
  Status SaveOpDebugId(const uint32_t task_id, const uint32_t stream_id, const void *const op_debug_addr,
 ...
 private:
  ...
  Status BuildTaskInfoForDumpInput(toolkit::aicpu::dump::OpMappingInfo &op_mapping_info, const InnerDumpInfo &dump_info,
