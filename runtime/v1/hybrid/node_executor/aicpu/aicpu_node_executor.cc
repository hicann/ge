...
Status GetExtendedIoLenOfTfNodeForHostMemInput(const size_t io_len, size_t &host_mem_offset, size_t &extend_len) {
  ...
  GE_CHK_STATUS_RET(CheckUint32AddOverflow(io_len, (kAlignBytes64 - 1U)), "Padding size is beyond the UINT32_MAX.");
...
Status AicpuNodeTaskBase::InitExtInfo(const std::string &kernel_ext_info, const uint64_t session_id) {
  if (kernel_ext_info.empty()) {
    if (node_item_->is_dynamic) {
      ...
      REPORT_INNER_ERR_MSG("E19999", "Node[%s(%s)] parse ext info failed as ext info is empty.", node_name_.c_str(),
                           node_type_.c_str());
      ...
      GELOGD("Node[%s] kernel_ext_info is empty, no need copy to device, is_dynamic=%s.", node_name_.c_str(),
  ...
  if (deploy_type_flag_ == RT_KERNEL_HOST_ONLY) {
    callback_memcpy_kind_ = ACL_MEMCPY_HOST_TO_HOST;
  }
  ...
  GE_CHK_STATUS_RET(aicpu_ext_handle_.UpdateSessionInfoId(session_id),
                    "[Update][SessionInfoSessionId] failed, node:%s(%s), session_id:%lu.", node_name_.c_str(),
                    node_type_.c_str(), session_id);

  if (is_blocking_aicpu_op_) {
    if (UpdateEventIdForBlockingAicpuOp() != SUCCESS) {
      GELOGE(FAILED, "[Call][UpdateEventIdForBlockingAicpuOp] failed, node:%s(%s)", node_name_.c_str(),
             node_type_.c_str());
      return FAILED;
    }
  }
  ...
  GE_CHK_ACL_RET(aclrtMemcpy(ext_info_addr_dev_->GetData(), ext_info_addr_dev_->GetSize(),
                             aicpu_ext_handle_.GetExtInfo(), aicpu_ext_handle_.GetExtInfoLen(), memcpy_kind_));
...
Status AicpuNodeTaskBase::UpdateShapeToOutputDesc(const TaskContext &context, const GeShape &shape_new,
  ...
  GELOGD("Update node[%s] out[%d] shape from %s to %s.", node_name_.c_str(), output_index, shape_old.ToString().c_str(),
  ...
  const auto trans_ret = formats::TransTensorShape(format, shape_new.GetDims(), output_desc->GetDataType(),
  ...
  GE_CHK_STATUS_RET(trans_ret,
                    ...
                    "shape=%s.",
                    node_name_.c_str(), node_type_.c_str(), output_index, origin_format, format,
                    shape_new.ToString().c_str());
...
Status AicpuNodeTaskBase::UpdateArgs(TaskContext &context) {
  GELOGD("Node[%s] update args begin. is_dynamic=%s, unknown_type=%d", node_name_.c_str(),
  ...
  if (node_item_->is_dynamic && is_support_block_) {
    ...
    GE_CHK_STATUS_RET(UpdateBlockDimInfo(axis_index_), "[Update][BlockDiminfo] failed for Node[%s(%s)] blocknum[%u].",
...
Status AicpuNodeTaskBase::ExecuteAsync(TaskContext &context, const std::function<void()> &done_callback) {
  ...
  HYBRID_CHK_STATUS_RET(LaunchTask(context), "[Launch][Task] failed for [%s(%s)].", node_name_.c_str(),
                        node_type_.c_str());
...
Status AicpuNodeTaskBase::UpdateEventIdForBlockingAicpuOp() {
  ...
  if (CheckDeviceSupportBlockingAicpuOpProcess(is_support) != SUCCESS) {
    GELOGE(FAILED, "[Call][CheckDeviceSupportBlockingAicpuOpProcess] for node:%s(%s) failed", node_name_.c_str(),
           node_type_.c_str());
    return FAILED;
  }
  if (!is_support) {
  ...
  auto rt_ret =
      aclrtCreateEventWithFlag(&rt_event_, ACL_EVENT_SYNC | ACL_EVENT_CAPTURE_STREAM_PROGRESS | ACL_EVENT_TIME_LINE);
  if (rt_ret != ACL_SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "Call aclrtCreateEventWithFlag failed for node:%s(%s), ret:%d", node_name_.c_str(),
                         node_type_.c_str(), rt_ret);
    GELOGE(RT_FAILED, "[Call][aclrtCreateEventWithFlag] failed for node:%s(%s), ret:%d", node_name_.c_str(),
           node_type_.c_str(), rt_ret);
    return RT_ERROR_TO_GE_STATUS(rt_ret);
  }
  rt_ret = aclrtGetEventId(rt_event_, &event_id);
  if (rt_ret != ACL_SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "Call aclrtGetEventId failed for node:%s(%s), ret:%d", node_name_.c_str(),
                         node_type_.c_str(), rt_ret);
    GELOGE(RT_FAILED, "[Call][aclrtGetEventId] failed for node:%s(%s), ret:%d", node_name_.c_str(), node_type_.c_str(),
           rt_ret);
    return RT_ERROR_TO_GE_STATUS(rt_ret);
  }
  if (aicpu_ext_handle_.UpdateEventId(event_id) != SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "Update event id failed for node:%s(%s).", node_name_.c_str(), node_type_.c_str());
    GELOGE(FAILED, "[Update][EventId] Update event id failed for node:%s(%s)", node_name_.c_str(), node_type_.c_str());
    return FAILED;
  }
...
Status AicpuNodeTaskBase::DistributeWaitTaskForAicpuBlockingOp(rtStream_t stream) const {
  bool is_support = false;
  if (CheckDeviceSupportBlockingAicpuOpProcess(is_support) != SUCCESS) {
    GELOGE(FAILED, "[Call][CheckDeviceSupportBlockingAicpuOpProcess] failed");
    return FAILED;
  }
  if (!is_support) {
    GELOGD("Device not support blocking aicpu op process.");
...
Status AicpuTfNodeTask::InitForDependComputeTask() {
  if ((unknown_type_ != DEPEND_COMPUTE) || (!node_item_->is_dynamic) || (node_item_->num_outputs == 0)) {
    GELOGD("Node[%s] type[%s] unknown_type is %d, output num is %d.", node_name_.c_str(), node_item_->node_type.c_str(),
  ...
  for (int32_t i = 0; i < node_item_->num_outputs; ++i) {
    GE_CHK_STATUS_RET(
        AllocTensorBuffer(sizeof(aicpu::FWKAdapter::ResultSummary), output_summary_[static_cast<size_t>(i)]),
        "[Alloc][TensorBuffer] failed for Node[%s(%s)] to copy result summary info, size = %zu.", node_name_.c_str(),
        node_type_.c_str(), sizeof(aicpu::FWKAdapter::ResultSummary));
  }
  ...
  GE_CHK_STATUS_RET(AllocTensorBuffer(sizeof(STR_FWK_OP_KERNEL), copy_task_args_buf_),
                    "[Alloc][TensorBuffer] failed for Node[%s(%s)] to copy task args, size = %zu", node_name_.c_str(),
  ...
  GE_CHK_STATUS_RET(AllocTensorBuffer(copy_io_addr_size, copy_ioaddr_dev_),
                    "[Alloc][TensorBuffer] failed for Node[%s(%s)] to copy task ioaddr, size = %zu", node_name_.c_str(),
                    node_type_.c_str(), copy_io_addr_size);
  GE_CHK_ACL_RET(
      aclrtMemcpy(copy_ioaddr_dev_->GetData(), copy_io_addr_size, &copy_io_addr[0U], copy_io_addr_size, memcpy_kind_));
  return SUCCESS;
}
...
static bool CanSyncStream(const OpDescPtr op_desc, const std::string node_type) {
  ...
  if ((node_type == kFunctionOp) && (AttrUtils::HasAttr(op_desc, kExceptionAbort))) {
...
Status AicpuTfNodeTask::AssembleWorkSpaceAddr(const domi::KernelExDef &kernel_ex_def) {
  ...
  GE_CHK_ACL_RET(aclrtMemcpy(kernel_workspace_->GetData(), kernel_workspace_size, kernel_ex_def.task_info().data(),
...
Status AicpuTfNodeTask::AssembleKernelBuffer(STR_FWK_OP_KERNEL *fwk_op_kernel) {
  GE_CHK_STATUS_RET(AllocTensorBuffer(sizeof(STR_FWK_OP_KERNEL), kernel_buf_),
                    ...
                    node_type_.c_str(), sizeof(STR_FWK_OP_KERNEL));
  GE_CHK_ACL_RET(aclrtMemcpy(kernel_buf_->GetData(), sizeof(STR_FWK_OP_KERNEL), fwk_op_kernel,
...
Status AicpuTfNodeTask::Init(const HybridModel &model) {
  ...
  if (ExecutorUtils::HasHostMemInput(node_item_->GetOpDesc())) {
    ...
    // input address of tf kernel cpu must be aligned to 64B
    GE_CHK_STATUS_RET(
        GetExtendedIoLenOfTfNodeForHostMemInput(input_output_size, host_mem_input_data_offset_, extend_len),
        "extend io length failed");
    input_output_size += extend_len;
    GELOGD(
        "Node[%s(%s)] has host memory input, io addr host is extended %zu, length = %zu,"
        "host_mem_input_data_offset = %zu.",
        node_name_.c_str(), node_type_.c_str(), extend_len, input_output_size, host_mem_input_data_offset_);
  }
  // alloc input output addr buf, allow alloc size 0
  GE_CHK_STATUS_RET(AllocTensorBuffer(input_output_size, input_output_addr_),
                    "[Alloc][TensorBuffer] for Node[%s(%s)] to copy io addr, size = %zu.", node_name_.c_str(),
                    node_type_.c_str(), input_output_size);
  ...
  const errno_t sec_ret = memcpy_s(&fwk_op_kernel, sizeof(STR_FWK_OP_KERNEL), kernel_ex_def.args().data(),
                                   static_cast<size_t>(kernel_ex_def.args_size()));
  GE_CHK_BOOL_RET_STATUS(sec_ret == EOK, INTERNAL_ERROR, "[Update][FwkOpKernel] failed for Node[%s(%s)], ret:%d.",
...
Status AicpuTfNodeTask::SetMemCopyTask(const domi::TaskDef &task_def) {
  if (node_item_->num_outputs == 0) {
    GELOGD("Node[%s] type[%s] has no output, no need set mem_copy task.", node_name_.c_str(),
           node_item_->node_type.c_str());
    return SUCCESS;
  }
  ...
  const auto sec_ret = memcpy_s(&aicpu_task, sizeof(STR_FWK_OP_KERNEL), kernel_def.args().data(),
                                static_cast<size_t>(kernel_def.args_size()));
  if (sec_ret != EOK) {
  ...
  GE_CHK_ACL_RET(aclrtMemcpy(copy_workspace_buf_->GetData(), static_cast<uint64_t>(kernel_def.task_info_size()),
  ...
  GE_CHK_ACL_RET(aclrtMemcpy(copy_task_args_buf_->GetData(), sizeof(STR_FWK_OP_KERNEL), &aicpu_task,
                             sizeof(STR_FWK_OP_KERNEL), memcpy_kind_));
...
Status AicpuNodeTaskBase::AllocOutputBuffer(const TaskContext &context, const int32_t idx,
  ...
  if (tensor->GetData() != nullptr) {
    ...
    if (data_size > allocated_size) {
      ...
      std::string reason = "The memory " + std::to_string(data_size) + " required by the output " +
                           std::to_string(idx) + " of the node " + node_name_ + "(" + node_type_ +
                           ") is greater than the allocated memory " + std::to_string(allocated_size);
  ...
  GE_CHK_STATUS_RET(context.SetOutput(idx, TensorValue(std::shared_ptr<TensorBuffer>(tensor_buffer.release()))),
                    "[Set][Output] failed for Node[%s(%s)], output:%d.", node_name_.c_str(), node_type_.c_str(), idx);
  return SUCCESS;
}
...
Status AicpuNodeTask::CopyDataToHbm(TaskContext &context,
  ...
  const auto rt_ret = rtCpuKernelLaunchWithFlag(PtrToPtr<const char, const void>(memcpy_so_name_.c_str()),
                                                ...
                                                &args_ex, nullptr, context.GetStream(), RT_KERNEL_DEFAULT);
...
Status AicpuNodeTaskBase::PrepareCopyInputs(const TaskContext &context,
  ...
  const size_t copy_input_buf_len = static_cast<size_t>(node_item_->num_outputs) * 2U * sizeof(uint64_t);
  GE_CHK_ACL_RET(aclrtMemcpy(copy_input_release_flag_dev_->GetData(), copy_input_release_flag_dev_->GetSize(),
                             &copy_input_release_flag[0U], copy_input_buf_len, memcpy_kind_));
  GE_CHK_ACL_RET(aclrtMemcpy(copy_input_data_size_dev_->GetData(), copy_input_data_size_dev_->GetSize(),
                             &copy_input_data_size[0U], copy_input_buf_len, memcpy_kind_));
  GE_CHK_ACL_RET(aclrtMemcpy(copy_input_src_dev_->GetData(), copy_input_src_dev_->GetSize(), &copy_input_src[0U],
                             copy_input_buf_len, memcpy_kind_));
  GE_CHK_ACL_RET(aclrtMemcpy(copy_input_dst_dev_->GetData(), copy_input_dst_dev_->GetSize(), &copy_input_dst[0U],
                             copy_input_buf_len, memcpy_kind_));
...
Status AicpuNodeTaskBase::UpdateShapeByHbmBuffer(const TaskContext &context,
                                                 const std::vector<std::unique_ptr<TensorBuffer>> &out_shape_hbm) {
  GE_CHK_BOOL_RET_STATUS(out_shape_hbm.size() == static_cast<std::size_t>(node_item_->num_outputs), INTERNAL_ERROR,
                         "[Check][Param] Node[%s(%s)] has %d outputs but out shape is %zu", node_name_.c_str(),
                         node_type_.c_str(), node_item_->num_outputs, out_shape_hbm.size());
  for (int32_t i = 0; i < node_item_->num_outputs; ++i) {
    ...
    if (result_summary.shape_data_size > 0U) {
      ...
      GE_CHK_ACL_RET(aclrtMemcpy(shape_addr.get(), result_summary.shape_data_size, shape_hbm->GetData(),
...
Status AicpuNodeTaskBase::UpdateShapeAndDataByResultSummary(TaskContext &context) {
  ...
  GE_CHK_STATUS_RET(ReadResultSummaryAndPrepareMemory(context, out_shape_hbm),
                    "[Invoke][ReadResultSummaryAndPrepareMemory] failed for Node[%s(%s)].", node_name_.c_str(),
                    node_type_.c_str());
  ...
  GE_CHK_STATUS_RET(CopyDataToHbm(context, out_shape_hbm),
                    "[Invoke][CopyDataToHbm] failed for Node[%s(%s)] copy data to output.", node_name_.c_str(),
                    node_type_.c_str());
  ...
  GE_CHK_STATUS_RET(UpdateShapeByHbmBuffer(context, out_shape_hbm),
                    "[Update][ShapeByHbmBuffer] failed for Node[%s(%s)].", node_name_.c_str(), node_type_.c_str());

  GELOGD("Node[%s] update shape and data by result summary end.", node_name_.c_str());
  return SUCCESS;
}

Status AicpuTfNodeTask::UpdateHostMemInputArgs(const TaskContext &context, void *const args, const size_t args_size) {
  if (!need_host_mem_opt_) {
    return SUCCESS;
  }
  std::vector<rtHostInputInfo_t> host_inputs;
  GE_CHK_RT_RET(
      ExecutorUtils::UpdateHostMemInputArgs(context, args, args_size, host_mem_input_data_offset_, host_inputs, true));
...
Status AicpuTfNodeTask::UpdateIoAddr(TaskContext &context) {
  ...
  for (int32_t i = 0; i < node_item_->num_inputs; ++i) {
    ...
    GELOGD("Node[%s] input[%d] addr = %p, size = %zu, mem_type = %d", node_name_.c_str(), i, inputData->GetData(),
           inputData->GetSize(), inputData->GetMemType());
  ...
  if (need_host_mem_opt_) {
    GE_CHK_STATUS_RET(UpdateHostMemInputArgs(context, args.get(), input_output_addr_->GetSize()),
                      "[Update][HostMemInputArgs] failed for Node[%s(%s)].", node_name_.c_str(), node_type_.c_str());
  }

  // if has input and output, need copy to ioaddr
  if (io_num > 0U) {
    // copy input and output
    GE_CHK_ACL_RET(aclrtMemcpy(input_output_addr_->GetData(), input_output_addr_->GetSize(),
...
Status AicpuTfNodeTask::LaunchTask(TaskContext &context) {
  ...
  if (is_blocking_aicpu_op_) {
    if (DistributeWaitTaskForAicpuBlockingOp(context.GetStream()) != SUCCESS) {
      GELOGE(FAILED, "[Call][DistributeWaitTaskForAicpuBlockingOp] for node:%s(%s) failed", node_name_.c_str(),
             node_type_.c_str());
      return FAILED;
    }
  }
...
Status AicpuNodeTaskBase::TaskCallback(TaskContext &context) {
  GELOGD("Node[%s] task callback start. is_dynamic=%s, unknown_type=%d.", node_name_.c_str(),
         node_item_->is_dynamic ? "true" : "false", unknown_type_);
...
Status AicpuNodeTask::SetMemCopyTask(const domi::TaskDef &task_def) {
  ...
  if (memcpy_args.size() != memcpy_args_size_) {
    ...
    GELOGE(FAILED, "[Check][Size] MemCopy task def args.size = %zu, but args_size = %u not equal.", memcpy_args.size(),
           memcpy_args_size_);
    return FAILED;
  }
  ...
  if (memcpy_args_size_ < sizeof(aicpu::AicpuParamHead)) {
    ...
    GELOGE(FAILED, "[Check][Size] Task def args_size = %u is less than aicpu param head len = %zu.", memcpy_args_size_,
  ...
  if (memcpy_args_ == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "new memory failed for Node[%s(%s)], task_size[%u].", node_name_.c_str(),
                         node_type_.c_str(), memcpy_args_size_);
    GELOGE(FAILED, "[Malloc][Memory] failed for Node[%s(%s)], task_size[%u].", node_name_.c_str(), node_type_.c_str(),
           memcpy_args_size_);
    return FAILED;
  }

  const errno_t sec_ret =
      memcpy_s(memcpy_args_.get(), static_cast<size_t>(memcpy_args_size_), memcpy_args.c_str(), memcpy_args.size());
  if (sec_ret != EOK) {
    REPORT_INNER_ERR_MSG("E19999", "memcpy_s argc_ failed for Node[%s(%s)], ret: %d", node_name_.c_str(),
                         node_type_.c_str(), static_cast<int32_t>(sec_ret));
    GELOGE(INTERNAL_ERROR, "[Update][Args] failed for Node[%s(%s)], ret: %d", node_name_.c_str(), node_type_.c_str(),
           static_cast<int32_t>(sec_ret));
    return FAILED;
  }
...
Status AicpuNodeTask::Init(const HybridModel &model) {
  ...
  if (kernel_type == ccKernelType::CUST_AI_CPU) {
    ...
    GE_CHK_STATUS_RET(ModelManager::GetInstance().LoadCustAicpuSo(op_desc, so_name, loaded),
                      "[Load][CustAicpuSo] failed, op:%s(%s), so:%s.", op_desc->GetName().c_str(),
                      op_desc->GetType().c_str(), so_name.c_str());
    if (!loaded) {
      GE_CHK_STATUS_RET(ModelManager::GetInstance().LaunchCustAicpuSo(), "[Launch][CustAicpuSo] failed, node:%s(%s).",
                        node_name_.c_str(), node_type_.c_str());
    }
  }
  ...
  GE_IF_BOOL_EXEC(
      ...
      REPORT_INNER_ERR_MSG("E19999", "Node[%s(%s)] task def args_size = %u is less than aicpu param head len = %zu.",
                           node_name_.c_str(), node_type_.c_str(), args_size_, sizeof(aicpu::AicpuParamHead));
      GELOGE(FAILED, "[Check][Size] Node[%s(%s)] task def args_size = %u is less than aicpu param head len = %zu.",
             node_name_.c_str(), node_type_.c_str(), args_size_, sizeof(aicpu::AicpuParamHead));
      return FAILED);
  ...
  if (ExecutorUtils::HasHostMemInput(op_desc)) {
    ...
    GELOGD(
        "Node[%s(%s)] has host memory input, args size is extended %zu, args_size_ = %u,"
        " host_mem_input_data_offset = %zu.",
        node_name_.c_str(), node_type_.c_str(), kMaxHostMemInputLen, args_size_, host_mem_input_data_offset_);
  }
  ...
  GE_IF_BOOL_EXEC(args_ == nullptr,
                  ...
                  GELOGE(FAILED, "[Malloc][Memory] failed for Node[%s(%s)], args_size_ = %u.", node_name_.c_str(),
                         node_type_.c_str(), args_size_);
                  return FAILED);
  ...
  GE_IF_BOOL_EXEC(sec_ret != EOK, REPORT_INNER_ERR_MSG("E19999", "memcpy_s argc_ failed for Node[%s(%s)], ret: %d",
                                                       node_name_.c_str(), node_type_.c_str(), sec_ret);
                  GELOGE(INTERNAL_ERROR, "[Update][Args] failed for Node[%s(%s)], ret: %d", node_name_.c_str(),
                         node_type_.c_str(), sec_ret);
                  return FAILED);
  ...
  GE_IF_BOOL_EXEC(
      (aicpu_param_head->ioAddrNum != static_cast<uint32_t>(io_num)),
      REPORT_INNER_ERR_MSG("E19999", "Node[%s(%s)] param head ioAddrNum = %u, but node has %d inputs and %d outputs.",
                           node_name_.c_str(), node_type_.c_str(), aicpu_param_head->ioAddrNum, node_item_->num_inputs,
                           node_item_->num_outputs);
      GELOGE(PARAM_INVALID,
             "[Check][IoAddrNum] Node[%s(%s)] param head ioAddrNum = %u, "
...
Status AicpuNodeTask::UpdateHostMemInputArgs(const TaskContext &context) {
  ...
  GE_CHK_RT_RET(ExecutorUtils::UpdateHostMemInputArgs(
      context, &args_[sizeof(aicpu::AicpuParamHead)], args_size_ - sizeof(aicpu::AicpuParamHead),
      host_mem_input_data_offset_ - sizeof(aicpu::AicpuParamHead), host_inputs));
...
Status AicpuNodeTask::UpdateIoAddr(TaskContext &context) {
  ...
  if (need_host_mem_opt_) {
    GE_CHK_STATUS_RET(UpdateHostMemInputArgs(context), "[Update][HostMemInputArgs] failed for Node[%s(%s)].",
                      node_name_.c_str(), node_type_.c_str());
  }
  return SUCCESS;
}
...
Status AicpuNodeTask::LaunchTask(TaskContext &context) {
  ...
  const auto rt_ret =
      rtAicpuKernelLaunchWithFlag(&launch_name, block_num_, &args_ex_, nullptr, context.GetStream(), flag);
  GE_CHK_RT_RET(rt_ret);
  if (is_blocking_aicpu_op_) {
    if (DistributeWaitTaskForAicpuBlockingOp(context.GetStream()) != SUCCESS) {
      GELOGE(FAILED, "[Call][DistributeWaitTaskForAicpuBlockingOp] failed for node:%s(%s)", node_name_.c_str(),
             node_type_.c_str());
      return FAILED;
    }
  }
...
Status AiCpuNodeExecutor::LoadTask(const HybridModel &model, const NodePtr &node,
  ...
  if ((node_item->shape_inference_type == DEPEND_COMPUTE) && node_item->is_dynamic) {
    ...
    GE_CHK_BOOL_RET_STATUS((*task_defs).size() == 2UL, PARAM_INVALID,
                           "[Check][Size]Node[%s(%s)] task_def num[%zu] != 2", node->GetName().c_str(),
                           node->GetType().c_str(), (*task_defs).size());
  } else {
    GE_CHK_BOOL_RET_STATUS((*task_defs).size() == 1UL, PARAM_INVALID,
                           "[Check][Size]Node[%s(%s)] task_def num[%zu] != 1", node->GetName().c_str(),
                           node->GetType().c_str(), (*task_defs).size());
  }
  ...
  if (static_cast<ModelTaskType>(task_def.type()) == ModelTaskType::MODEL_TASK_KERNEL_EX) {
    ...
    GELOGE(UNSUPPORTED,
           "[Check][Type] Node[%s(%s)] task type = %u is not supported by aicpu node executor,"
    ...
    REPORT_INNER_ERR_MSG("E19999",
                         "Node[%s(%s)] task type = %u is not supported by aicpu node executor,"
                         "ModelTaskType::MODEL_TASK_KERNEL_EX or ModelTaskType::MODEL_TASK_KERNEL is supported.",
                         node->GetName().c_str(), node->GetType().c_str(), task_def.type());
    return UNSUPPORTED;
  }

  GE_CHK_BOOL_RET_STATUS(aicpu_task != nullptr, MEMALLOC_FAILED, "[Check][State]Load task for node %s(%s) failed.",
                         node->GetName().c_str(), node->GetType().c_str());

  GE_CHK_STATUS_RET(aicpu_task->Init(model), "[Init][AicpuNodeTaskBase] failed for Node[%s(%s)].",
                    node->GetName().c_str(), node->GetType().c_str());
