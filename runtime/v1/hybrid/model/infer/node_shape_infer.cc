...
Status CalcTensorSize(const DataType data_type, const std::vector<int64_t> &shape, int64_t &tensor_size) {
  ...
  for (const auto &dim : shape) {
    GE_CHECK_GE(dim, 0);
    GE_CHK_STATUS_RET(CheckInt64MulOverflow(tensor_size, dim), "[Check][Overflow] Shape size overflow, shape = [%s]",
  ...
  GE_CHK_STATUS_RET(CheckInt64AddOverflow(tensor_size, append_size - 1),
                    "[Check][Overflow]Tensor size is too large:%" PRId64
                    ", shape = [%s]"
...
Status CanonicalizeShape(GeTensorDesc &tensor_desc, std::vector<int64_t> &shape, const bool fallback_with_range) {
  ...
  if (tensor_shape.IsUnknownShape()) {
    if (!fallback_with_range) {
      ...
      REPORT_INNER_ERR_MSG("E19999", "Output shape is still unknown after shape inference. shape = [%s].",
                           tensor_shape.ToString().c_str());
      return INTERNAL_ERROR;
    }
    ...
    if (shape_range.size() != shape.size()) {
      ...
      REPORT_INNER_ERR_MSG("E19999", "Number of shape ranges (%zu) mismatches that of dims (%zu)", shape_range.size(),
                           shape.size());
      return INTERNAL_ERROR;
    }
    ...
    GELOGD("After canonicalization, shape = [%s], before = [%s]", GeShape(shape).ToString().c_str(),
           tensor_shape.ToString().c_str());
  }
...
Status NodeShapeInfer::CalcOutputTensorSizes(const bool fallback_with_range) const {
  for (size_t output_index = 0U; output_index < op_desc->GetOutputsSize(); ++output_index) {
    ...
    if (status_result != SUCCESS) {
      GELOGE(ge::FAILED, "[Canonicalize][Shape] failed for [%s(%s)], output %zu.", NodeName().c_str(),
             NodeType().c_str(), output_index);
      return status_result;
    }
    int64_t tensor_size;
    status_result = CalcTensorSize(tensor_desc->GetDataType(), dims, tensor_size);
    if (status_result != SUCCESS) {
      REPORT_INNER_ERR_MSG("E19999", "Invoke CalcTensorSize failed, node:%s(%s), output:%zu.", NodeName().c_str(),
                           NodeType().c_str(), output_index);
      GELOGE(ge::FAILED, "[Calc][TensorSize] failed for [%s(%s)], output %zu.", NodeName().c_str(), NodeType().c_str(),
             output_index);
      return status_result;
    }
...
bool NodeShapeInfer::IsInputShapeStatic(const int32_t index) const {
  ...
  if (static_cast<size_t>(index) >= is_input_shape_static_.size()) {
    GELOGE(PARAM_INVALID, "[Check][Param:index]Input index(%d) out of range: [0, %zu)", index,
           is_input_shape_static_.size());
