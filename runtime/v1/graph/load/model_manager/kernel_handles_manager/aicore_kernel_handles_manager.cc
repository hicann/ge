...
namespace ge {
graphStatus AicoreKernelHandlesManager::RegisterKernel(const KernelRegisterInfo &register_info,
    const std::string &bin_name) {
  GE_ASSERT_TRUE(!bin_name.empty(), "Bin handle name is empty.");
  if (IsKernelHandleRegistered(bin_name)) {
    GELOGI("Aicore kernel has been registered, kernel bin_name: %s", bin_name.c_str());
    return SUCCESS;
  }
