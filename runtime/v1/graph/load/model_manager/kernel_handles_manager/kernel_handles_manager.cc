...
rtBinHandle KernelHandlesManager::FindKernel(const std::string &bin_name) const {
...
rtBinHandle KernelHandlesManager::GetOrRegisterKernel(const KernelRegisterInfo &register_info,
  ...
  GE_ASSERT_SUCCESS(RegisterKernel(register_info, bin_name));
  return FindKernel(bin_name);
}

bool KernelHandlesManager::IsKernelHandleRegistered(const std::string &bin_name) {
  std::lock_guard<std::mutex> lock(mtx_);
  auto iter = global_bin_store_.find(bin_name);
  if (iter != global_bin_store_.end()) {
