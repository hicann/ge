...
graphStatus KernelHandleUtils::LaunchKernel(const rtFuncHandle func_handle, const LaunchKernelParam &launch_param) {
  ...
  attrs[actual_cfg_num].value.isBlockTaskPrefetch =
      static_cast<uint8_t>(launch_param.launch_config.is_block_taks_prefetch);
  actual_cfg_num++;
  ...
  attrs[actual_cfg_num].value.localMemorySize = static_cast<uint8_t>(launch_param.launch_config.local_memory_size);
  actual_cfg_num++;
