    ...
    if (unknown_type != DEPEND_COMPUTE) {
      for (uint32_t i = 0U; i < num_outputs; ++i) {
        ...
        GE_CHK_STATUS_RET(ext_handle.UpdateOutputShapeAndType(i, *output_desc),
                          "[Call][UpdateOutputShapeAndType] Output[%u] update output shape failed, op:%s.", i,
                          op_desc->GetName().c_str());
      }
    }
  }
...
Status KernelExTaskInfo::InitInputOutputAddr(const PisToArgs &args, const IowAddrs &iow_addrs) {
  ...
  GELOGD("Op %s deploy_type_flag %d, input_output_addr %p, addrs_size %zu", op_desc_->GetName().c_str(),
         deploy_type_flag_, input_output_addr_, addrs_size_);
...
aclrtFuncHandle KernelExTaskInfo::GetFuncHandle() {
  ...
  GE_ASSERT_SUCCESS(KernelRegisterInfoBuilder::ConstructAicpuRegisterInfo(op_desc_->GetType(), "libtf_kernels.so",
...
Status KernelExTaskInfo::Init(const domi::TaskDef &task_def, DavinciModel *const davinci_model, const PisToArgs &args,
  ...
  if (args_size > sizeof(STR_FWK_OP_KERNEL)) {
    REPORT_INNER_ERR_MSG("E19999",
                         "Param kernel_ex_def.args().size():%zu > sizeof(STR_FWK_OP_KERNEL):%zu, check invalid",
                         args_size, sizeof(STR_FWK_OP_KERNEL));
    GELOGE(FAILED, "[Check][Param] kernel_ex_def.args().size():%zu > sizeof(STR_FWK_OP_KERNEL):%zu", args_size,
  ...
  GE_CHK_STATUS_RET(InitTaskExtInfo(ext_info, op_desc), "[Init][TaskExtInfo] failed, ext_info size=%zu, op:%s",
  ...
  ModelManager::GetInstance().CreateAicpuKernel(session_id, davinci_model->Id(), davinci_model->SubModelId(),
                                                kernel_id);
...
Status KernelExTaskInfo::ParseTaskRunParam(const domi::TaskDef &task_def, DavinciModel *const davinci_model,
  ...
  pls_ = (deploy_type_flag == static_cast<int32_t>(RT_KERNEL_HOST_ONLY)) ? ArgsPlacement::kArgsPlacementHostSvm
...
Status KernelExTaskInfo::Distribute() {
  ...
  GELOGI("KernelExTaskInfo %s Distribute Success. task id: %u, stream id: %u, stream: %p.", op_desc_->GetNamePtr(),
