...
Status ModelExecutor::GetDeviceMemorySize(size_t &free_mem, size_t &total_mem_size) {
  ...
  GE_CHK_ACL_RET(aclrtGetMemInfo(ACL_HBM_MEM, &free_mem, &total_mem_size));
  if (total_mem_size == 0U) {
    GE_CHK_ACL_RET(aclrtGetMemInfo(ACL_DDR_MEM, &free_mem, &total_mem_size));
  }
...
Status ModelExecutor::UnloadGraph(const GeRootModelPtr &ge_root_model, const uint32_t graph_id) {
  ...
  Status unload_ret;
  if (IsOm2OnlineMode()) {
    unload_ret = UnloadOm2Graph(ge_root_model, graph_id);
  } else {
...
Status ModelExecutor::CheckAndReleaseMemory(const GeRootModelPtr &ge_root_model, const GraphNodePtr &graph_node) {
  ...
  for (const auto &it : graph_nodes_) {
    ...
    if (!ReleaseMemory(tmp_ge_root_model, it.second)) {
      continue;
    }
...
Status ModelExecutor::UnloadOm2Graph(const GeRootModelPtr &ge_root_model, uint32_t graph_id) {
  ...
  GE_CHK_STATUS_RET(Om2ModelManager::GetInstance().UnloadModel(model_id),
                    "[OM2] Unload model failed, modelId=%u, graphId=%u.", model_id, graph_id);
  return SUCCESS;
}
