...
std::vector<ValueHolderPtr> BuildLaunchCommonHead(const CommonLaunchArg &launch_arg, const size_t &io_num) {
  ...
  inputs.emplace_back(launch_arg.kernel_bin_id_holder);
  inputs.emplace_back(launch_arg.magic_holder);
  inputs.emplace_back(launch_arg.kernel_bin_holder);
...
bool BuildLaunchCommonTail(const CommonLaunchArg &launch_arg, std::vector<ValueHolderPtr> &inputs) {
  ...
  if (core_num_v.size() != static_cast<size_t>(CoreNumType::TYPE_NUM)) {
    GELOGD("Node[%s]: core_num_v.size[%lu] is not equal to [%lu].", op_desc->GetNamePtr(), core_num_v.size(),
           static_cast<size_t>(CoreNumType::TYPE_NUM));
    return false;
  }
...
ValueHolderPtr LaunchKernelV2(const CommonLaunchArg &launch_arg, const std::vector<DevMemValueHolderPtr> &input_addrs,
...
ValueHolderPtr AtomicLaunchKernelV2(const CommonLaunchArg &launch_arg, const ValueHolderPtr &clean_workspace_indexes,
