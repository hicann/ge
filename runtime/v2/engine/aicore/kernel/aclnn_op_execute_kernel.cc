...
static void *GetFwkDataByIndex(KernelContext *context, size_t index) {
...
ge::graphStatus ExecuteOpPrepare(KernelContext *context) {
  ...
  auto aclnn_op_fwk_data = reinterpret_cast<AclnnOpFwkData *>(
      GetFwkDataByIndex(context, static_cast<size_t>(OpExecutePrepareInputExtendIndex::kFwkData)));
  GE_ASSERT_NOTNULL(aclnn_op_fwk_data);
  const auto op_execute_prepare_func =
      reinterpret_cast<OpImplRegisterV2::OpExecPrepareFunc>(aclnn_op_fwk_data->op_execute_prepare_func);
...
ge::graphStatus ExecuteOpLaunch(KernelContext *context) {
  ...
  auto aclnn_op_fwk_data = reinterpret_cast<AclnnOpFwkData *>(
      GetFwkDataByIndex(context, static_cast<size_t>(OpExecuteLaunchInputIndex::kFwkData)));
  GE_ASSERT_NOTNULL(aclnn_op_fwk_data);
...
ge::graphStatus BuildAclnnOpFwkData(KernelContext *context) {
  const auto fwk_data = context->GetOutputPointer<AclnnOpFwkData>(0UL);
  GE_ASSERT_NOTNULL(fwk_data);
  const auto execute_op_prepare_func =
      context->GetInputValue<void *>(static_cast<size_t>(AclnnOpFwkDataInput::kExecutePrepareFunc));
  GE_ASSERT_NOTNULL(execute_op_prepare_func);
  const auto execute_op_launch_func =
      context->GetInputValue<void *>(static_cast<size_t>(AclnnOpFwkDataInput::kExecuteLaunchFunc));
  GE_ASSERT_NOTNULL(execute_op_launch_func);
  const auto platform_info =
      context->GetInputValue<fe::PlatFormInfos *>(static_cast<size_t>(AclnnOpFwkDataInput::kPlatformInfo));
...
static ge::graphStatus CreateAclnnOpFwkDataOutput(const ge::FastNode *node, KernelContext *context) {
  (void)node;
  auto fwk_data_av = context->GetOutput(0UL);
  GE_ASSERT_NOTNULL(fwk_data_av);
  auto fwk_data_ptr = new (std::nothrow) AclnnOpFwkData();
...
REGISTER_KERNEL(BuildAclnnOpFwkData).RunFunc(BuildAclnnOpFwkData).OutputsCreator(CreateAclnnOpFwkDataOutput);
}  // namespace kernel
}  // namespace gert
