...
ge::graphStatus SetStreamCoreNumLimit(const rtStream stream, const int64_t op_aicore_num, const int64_t op_vec_core_num,
  ...
  if (op_aicore_num > 0) {
    GE_CHK_ACL_RET(aclrtSetStreamResLimit(stream, ACL_RT_DEV_RES_CUBE_CORE, static_cast<uint32_t>(op_aicore_num)));
    need_set_stream_aicore_num = true;
  }
  if (op_vec_core_num > 0) {
    GE_CHK_ACL_RET(aclrtSetStreamResLimit(stream, ACL_RT_DEV_RES_VECTOR_CORE, static_cast<uint32_t>(op_vec_core_num)));
