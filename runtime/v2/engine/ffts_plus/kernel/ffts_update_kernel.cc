...
ge::graphStatus FftsTaskInfoPreProc(KernelContext *context) {
  ...
  GELOGD("Mem base:%lx, align_base:%lx, offset:%zu, new offset:%zu, buf_len[%zu], left len[%zu].", dev_buf_base,
         align_base, buf_offset, new_offset, buf_len, left_len);
...
ge::graphStatus NodeMemParaAssign(KernelContext *context) {
  ...
  *reinterpret_cast<int64_t *>(guard_vec[cur_idx].guard_ptr) = guard_vec[cur_idx].guard_val;
  GELOGD("Base addr:%lx, node addr base:%lx, size:%zu, offset:%zu, guard[%ld].", host_addr_base, node_para->host_addr,
         node_para->size, pre_para->offset, guard_vec[cur_idx].guard_val);
...
ge::graphStatus FFTSTaskAndArgsCopy(KernelContext *context) {
  ...
  GE_CHK_ACL_RET(aclrtMemcpyAsync(dev_addr_base, dev_addr->GetSize(), host_addr_base, host_addr->GetSize(),
