...
class TaskWorkerImpl : public TaskWorker {
 ...
 private:
  ...
  void SetExecuteStream(rtStream_t stream) override {
 ...
 private:
  ...
  void RebindStreamResLimitIfNeeded(rtStream_t &bound_stream) {
    ...
    if (bound_stream != nullptr) {
      const auto rt_error = rtsNotUseStreamResInCurrentThread(bound_stream);
      if (rt_error != RT_ERROR_NONE) {
        GELOGW("Failed to unbind stream resource limit in thread, stream %p, ret %d", bound_stream, rt_error);
      } else {
    ...
    const auto rt_error = rtsUseStreamResInCurrentThread(execute_stream);
    if (rt_error != RT_ERROR_NONE) {
      GELOGW("Failed to bind stream resource limit in thread, stream %p, ret %d", execute_stream, rt_error);
      return;
    }
  ...
  void ExecuteTasks(TaskThread &thread) {
    rtStream_t bound_stream = nullptr;
    ...
    if (bound_stream != nullptr) {
      const auto rt_error = rtsNotUseStreamResInCurrentThread(bound_stream);
      if (rt_error != RT_ERROR_NONE) {
        GELOGW("Failed to unbind stream resource limit on thread exit, stream %p, ret %d", bound_stream, rt_error);
      }
    }
  }
 ...
 private:
  ...
  std::atomic<rtStream_t> execute_stream_{nullptr};
