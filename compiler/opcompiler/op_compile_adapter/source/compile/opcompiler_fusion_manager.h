...
class TeFusionManager {
 public:
   ...
   * @return [out] NA
   */
  bool ValidateAndGetRelBuildTask(const OpBuildTaskResultPtr &taskRes, OpBuildTaskPtr &relBuildTaskPtr,
                                  std::pair<uint64_t, uint64_t> &graphMapKey);
  void PrepareOpInfo(const OpBuildTaskPtr &relBuildTaskPtr, const OpBuildTaskResultPtr &taskRes, std::string &opNames,
                     std::string &opTypes, std::string &opModuleNames, int loglevel);
  void PreparePreCompileInfo(const OpBuildTaskResultPtr &taskRes, std::string &opPattern, std::string &coreType,
                             std::string &prebuiltOptions);
  void LogTaskDetails(const OpBuildTaskResultPtr &taskRes, const std::string &opNames, const std::string &opTypes,
                      const std::string &opModuleNames, const std::string &opPattern, const std::string &coreType,
                      const std::string &prebuiltOptions, int loglevel);
  void ProcessErrorInfo(const OpBuildTaskPtr &relBuildTaskPtr, const OpBuildTaskResultPtr &taskRes,
                        const std::string &opNames, const std::string &opTypes, const std::string &opModuleNames,
                        int loglevel, std::string &errorMsg);
  bool SetBuildResults(OpBuildTaskPtr &relBuildTaskPtr, OpBuildTaskResultPtr &taskRes);
  void AddToFinishedTaskList(const OpBuildTaskPtr &relBuildTaskPtr, const OpBuildTaskResultPtr &taskRes,
                             FinComTask &finshedTaskItem);
  void LogTaskCompletion(const FinComTask &finshedTaskItem, const OpBuildTaskPtr &relBuildTaskPtr,
 ...
 private:
  ...
  bool CompilePendingTask(std::vector<OpBuildTaskPtr> &vecOpBuildTask, const OpBuildTaskPtr &finBuildTaskPtr,
                          const OpBuildTaskResultPtr &opBuildResult);

  bool FinishPendingTask(const OpBuildTaskPtr &relBuildTaskPtr, const OpBuildTaskResultPtr &opBuildResult);
  ...
  static void GetFusionOpOuterInputCnt(std::unordered_set<ge::Node *> &allNodes, const ge::Node *currentNode,
  ...
  static bool DfsFindOuterInputSingleAnchor(const ge::InDataAnchorPtr &anchor,
                                            const std::unordered_set<ge::Node *> &allNodes, const ge::Node *curNode,
                                            size_t outputIdx, uint32_t &dfsMaxCnt);
  ...
  std::unordered_map<std::string,  // kernelName
                     std::pair<OpBuildTaskResultPtr, std::vector<OpBuildTaskPtr>>>
      opKernelMap_;
