...
namespace {
void PrintOp(const OpBuildTaskPtr &relBuildTaskPtr, const OpBuildTaskResultPtr &taskRes, string &opNames,
             string &opTypes, int loglevel) {
  size_t index = 0;
  ...
  if (taskRes->type == 0) {
    TE_FUSION_LOG_FULL(loglevel, "Failed to pre-compile node [opName:%s, opType: %s].", opNames.c_str(),
                       opTypes.c_str());
    if (relBuildTaskPtr->pPrebuildOp != nullptr) {
      ge::NodePtr nodePtr = nullptr;
      relBuildTaskPtr->pPrebuildOp->GetNode(nodePtr);
      if (nodePtr != nullptr) {
        TE_FUSION_LOG_FULL(loglevel, "The input nodes of node [opName:%s, opType: %s] is: [%s].", opNames.c_str(),
                           opTypes.c_str(), GetInputNodesDesc(*nodePtr).c_str());
      }
    }
  } else if (taskRes->type == 1) {
    TE_FUSION_LOG_FULL(loglevel, "Failed to compile node [opName %s, opType: %s].", opNames.c_str(), opTypes.c_str());
    if (!relBuildTaskPtr->opNodes.empty() && relBuildTaskPtr->opNodes[0] != nullptr) {
      TE_FUSION_LOG_FULL(loglevel, "The input nodes of node[opName:%s, opType: %s] is: [%s].", opNames.c_str(),
                         opTypes.c_str(), GetInputNodesDesc(*relBuildTaskPtr->opNodes[0]).c_str());
    }
...
void ParseAndPrintArgsString(const string &opNames, const string &opTypes, const OpBuildTaskResultPtr &taskRes,
                             int loglevel) {
  if (taskRes->errArgs.empty()) {
  ...
  if (taskRes->statusCode == ERROR_DIED_PROCESS_STATUS_CODE) {
    TE_ERRLOG("===========COMPILER PROCESS DIED OF NODE %s, TYPE %s=============", opNames.c_str(), opTypes.c_str());
  } else {
    TE_FUSION_LOG(loglevel, "===========FOLLOWING IS ARGUMENTS OF NODE %s, TYPE %s=============", opNames.c_str(),
                  opTypes.c_str());
  }
  key = "input";
...
void ParseAndPrintArgsExceptionStack(const string &opNames, const OpBuildTaskResultPtr &taskRes, int loglevel,
                                     string &errorMsg) {
  ...
  size_t pos = 0;
  TE_FUSION_LOG(loglevel, "===========FOLLOWING IS STACK INFO OF NODE %s=============", opNames.c_str());
  ...
  TE_FUSION_LOG(loglevel, "===========END OF DETAILED OP INFO OF NODE %s=============", opNames.c_str());
}
...
int SetLogLevel(OpBuildTaskResultPtr taskRes) {
  ...
  if (taskRes->statusCode != 0) {
    ...
    TE_FUSION_CHECK(taskRes->type == 0 || taskRes->type == 1, { loglevel = TE_FUSION_LOG_ERROR; });
  }
  return loglevel;
}
}  // namespace
...
void TeFusionManager::ReportBuildErrMessage(const OpBuildTaskPtr &relBuildTaskPtr, const string &opModuleNames,
  ...
  std::map<std::string, std::string> mapArgs = {{"op_name", opName}, {"opp_path", opPathInfo}, {"op_type", opType}};
  ...
  if (opTask->opNodes.size() == 1) {
    ...
    if ((opStorePattern.find("rangeAgnostic") != std::string::npos)) {
      ...
      TE_FUSION_CHECK(!bres, {
        TE_FUSION_LOG_EXEC(TE_FUSION_LOG_WARNING, "Failed to get node key name.");
        return;
      });
...
bool TeFusionManager::GetCompileFileFromCache(const std::string &kernelName, std::string &jsonPath,
                                              std::string &binPath) {
  ...
  TE_INFOLOG("Get jsonpath[%s] and binpath[%s] by kernelName[%s].", jsonPath.c_str(), binPath.c_str(),
             kernelName.c_str());
  return true;
}
...
bool TeFusionManager::BuildSingleOp(OpBuildTaskPtr &opTask) {
  ...
  if (teGraphNode.size() != 1) {
    REPORT_TE_INNER_ERROR("The nodes of task[%lu] have the wrong size[%zu], which should be 1.", opTask->taskId,
                          teGraphNode.size());
    return false;
  }
  ...
  TE_FUSION_CHECK(opTask->attrKernelName.empty(), {
    if (CanReuseTaskCache(opTask->kernel, opKernelMap_, opTask, opRes)) {
      ...
      TE_INFOLOG("Cache op compile found, node name=[%s], kernel name=[%s], taskID[%lu:%lu].", pNode->GetName().c_str(),
  ...
  TE_INFOLOGF("Single op building: kernelName[%s], op inputs: %s, outputs: %s, attrs: %s.", kernelName.c_str(),
  ...
  TE_INFOLOGF("Op[%s] Single op building, Pattrs: %s.", pNode->GetName().c_str(),
              PyObjectToStr(pyPrivateAttrs).c_str());
  TE_FUSION_CHECK(!res, {
  ...
  TE_DBGLOG("SuperKernel sub op[%s, %s], spk_options[%s], spk_sub_loc[%d].", node->GetNamePtr(), node->GetTypePtr(),
            spOptions.c_str(), subOpLoc.c_str());
  ...
  res = PythonAdapterManager::Instance().BuildOpAsync(
      opTask, "dispatch_single_op_compile_task", "sssssO(OOOOO)OssOssOsssssssss", opModule.c_str(),
      pNode->GetName().c_str(), pNode->GetTypePtr(), opFuncName.c_str(), kernelName.c_str(), pyUnknownShape, pyInputs,
      pyOutputs, pyAttrs, optionValues, pyPrivateAttrs, pyInt64Mode,
      opTask->pre_task.empty() ? nullptr : opTask->pre_task.c_str(),
      opTask->post_task.empty() ? nullptr : opTask->post_task.c_str(), pyIsDynamicImpl, opPattern.c_str(), nullptr,
      contextDict, passOptList.c_str(), tuneParam.empty() ? nullptr : tuneParam.c_str(), extraParams.c_str(),
      relationStr.c_str(), opImplSwitch.c_str(), opImplMode.c_str(), optionalInputMode.c_str(),
      enableSuperkernelPlus.c_str(), spKernelSubInfo.dump().c_str());

  if (res) {
    (void)opKernelMap_.emplace(opTask->kernel, std::make_pair(nullptr, std::vector<OpBuildTaskPtr>({opTask})));
    TE_INFOLOG("Single op build task dispatched, taskID[%lu:%lu], node=[%s], kernel=[%s].", opTask->graphId,
               opTask->taskId, pNode->GetName().c_str(), opTask->kernel.c_str());
    TraceUtils::SubmitCompileDetailTrace(opTask->graphId, opTask->opNodes.at(0)->GetOpDesc()->GetId(),
...
bool TeFusionManager::DfsFindOuterInput(const ge::Node *curNode, size_t outputIdx,
  ...
  if (dfsMaxCnt > MAX_DFS_CNT) {
    TE_DBGLOG("DfsFindOuterInput from node [%s]'s Output [%u] has reached maxcnt", curNode->GetName().c_str(),
              outputIdx);
    return false;
  }
  ...
  if (paramReused) {
    ...
    if (inputIdx < 0 || inputIdx >= static_cast<int32_t>(curNode->GetAllInDataAnchorsSize())) {
      TE_WARNLOG("The reused output-input relation [%zu-%zu] of node: %s is invalid", outputIdx, inputIdx,
...
void TeFusionManager::GenerateSingleOpRelationParam(const ge::Node *curNode, std::string &relationJson) {
...
bool TeFusionManager::ParallelCompilationProcess(const OpBuildTaskPtr &opTask) {
  ...
  bool res = PythonAdapterManager::Instance().BuildOpAsync(
      opTask, "dispatch_fusion_op_compile_task", "ssssOOssssss", opTask->jsonStr.c_str(), opTask->kernel.c_str(),
      opTask->pre_task.empty() ? nullptr : opTask->pre_task.c_str(),
  ...
  if (res && !opTask->kernel.empty()) {
    (void)fusionOpsKernel_.emplace(opTask->kernel, std::make_pair(nullptr, std::vector<OpBuildTaskPtr>({opTask})));
    TE_INFOLOG("Fusion op build task dispatched, taskID[%lu:%lu]. kernel name=[%s].", opTask->graphId, opTask->taskId,
               opTask->kernel.c_str());
    TE_INFOLOGF("Fusion op building first op_name is [%s], jsonstr is [%s].", firstNode->GetName().c_str(),
                relationStr.c_str());
    TraceUtils::SubmitCompileDetailTrace(opTask->graphId, opTask->opNodes.at(0)->GetOpDesc()->GetId(),
                                         opTask->opNodes.at(0)->GetOpDesc()->GetType(), "dispatch online compile");
    DfxInfoManager::Instance().RecordStatistics(StatisticsType::ONLINE_COMPILE, RecordEventType::TASK_SUBMIT);
...
bool TeFusionManager::TaskFusion(OpBuildTaskPtr &opTask) {
  ...
  if (!PythonApiCall::Instance().GenerateStrSha256HashValue(tmpTaskInfo, fusionkernelNameStr)) {
    TE_ERRLOGF("taskID[%lu:%lu] do not generate static key hash by json(%s).", opTask->graphId, opTask->taskId,
               tmpTaskInfo.c_str());
    return false;
  }
  ...
  TE_DBGLOGF("taskID[%lu:%lu]: FusionTask jsonStr is [%s]", opTask->graphId, opTask->taskId, opTask->jsonStr.c_str());
...
bool TeFusionManager::TaskFusionProcess(const OpBuildTaskPtr &opTask) {
  ...
  if (res && !taskFusionUniqueKey.empty()) {
    (void)taskFusionMap_.emplace(taskFusionUniqueKey, std::make_pair(nullptr, std::vector<OpBuildTaskPtr>({opTask})));
    TE_INFOLOG("Task Fusion task dispatched, taskID[%lu:%lu]. kernel name=[%s].", opTask->graphId, opTask->taskId,
               opTask->kernel.c_str());
  }
  return res;
}
...
void TeFusionManager::DumpFusionOpInfoToJsonFile(const OpBuildTaskPtr &opTask) {
  ...
  try {
    ...
    TE_WARNLOG("Failed to parse jsonStr, the jsonStr is %s and the reason is %s", opTask->jsonStr.c_str(), e.what());
    return;
  }
...
bool TeFusionManager::BuildFusionOp(OpBuildTaskPtr &opTask, const std::string &opCompileStrategyStr) {
  ...
  bool isFESpecStrate =
      opCompileStrategyStr == COMPILE_STRATEGY_KEEP_COMPILING || opCompileStrategyStr == COMPILE_STRATEGY_NO_TUNE;
  if (isFESpecStrate) {
  ...
  bool isOpTuneAfterBuildStage =
      opTuneMode == ge::BUILD_MODE_TUNING &&
  ...
  bool isOpTuneWithStrategy =
      (opTuneMode == ge::BUILD_MODE_TUNING) && (opTuneStep == ge::BUILD_STEP_AFTER_UB_MATCH) && hasOpTuneStrategy;
  ...
  TE_INFOLOG("Set kernel name[%s] for task[%lu:%lu] while build mode[%s], build step[%s], compile strategy[%s].",
             opTask->kernel.c_str(), opTask->graphId, opTask->taskId, opTuneMode.c_str(), opTuneStep.c_str(),
             opCompileStrategyStr.c_str());

  // only fusion check
  if (IsFusionCheckTask(opTask)) {
    opTask->pre_task = opCompileStrategyStr;
    opTask->post_task = opCompileStrategyStr;
    TE_DBGLOGF("Fusion check taskID[%lu:%lu], kernel: %s, args: %s.", opTask->graphId, opTask->taskId,
               opTask->kernel.c_str(), opTask->jsonStr.c_str());
  ...
  bool canReuseCache = (!(hasOpTuneStrategy || onlyDumpJson || opTuneMode == ge::BUILD_MODE_TUNING ||
                          opTuneMode == ge::BUILD_MODE_OPAT_RESULT) &&
                        opTask->attrKernelName.empty());
  TE_DBGLOG("opTuneMode[%s], opTuneStep[%s], onlyDumpJson[%d], canReuseCache[%d], attrKernelName[%s].",
            opTuneMode.c_str(), opTuneStep.c_str(), onlyDumpJson, canReuseCache, opTask->attrKernelName.c_str());
  ...
  if (canReuseCache) {
    ...
    TE_FUSION_CHECK(opTask->opRes != nullptr, {
      TE_FUSION_CHECK(
          opTask->opRes->backToSingleOpBinReuse,
  ...
  TE_FUSION_CHECK(onlyDumpJson, {
    TE_INFOLOG("Get option opTuneMode is tuning, dump json only, taskID[%lu:%lu]", opTask->graphId, opTask->taskId);
    TE_FUSION_CHECK((opTask->opNodes.size() == 1), {
      bool setOPArgsStatus = SetOpArgsToNode(opTask);
      TE_FUSION_CHECK(!setOPArgsStatus, { TE_DBGLOGF("Unable to func exec SetOpArgsToNode."); });
    });
...
bool TeFusionManager::ValidateAndGetRelBuildTask(const OpBuildTaskResultPtr &taskRes, OpBuildTaskPtr &relBuildTaskPtr,
  ...
  if (historyGraphItr == dispatchedTask_.end()) {
    TE_WARNLOG("Couldn't find related compilation task[%lu:%lu] in dispatchedTask_, maybe timed-out", graphMapKey.first,
               graphMapKey.second);
    return false;
  }
...
void TeFusionManager::PrepareOpInfo(const OpBuildTaskPtr &relBuildTaskPtr, const OpBuildTaskResultPtr &taskRes,
                                    std::string &opNames, std::string &opTypes, std::string &opModuleNames,
                                    int loglevel) {
  PrintOp(relBuildTaskPtr, taskRes, opNames, opTypes, loglevel);
  GetOpModuleName(relBuildTaskPtr, opModuleNames);
}

void TeFusionManager::PreparePreCompileInfo(const OpBuildTaskResultPtr &taskRes, std::string &opPattern,
                                            std::string &coreType, std::string &prebuiltOptions) {
  opPattern.clear();
  coreType.clear();
...
void TeFusionManager::LogTaskDetails(const OpBuildTaskResultPtr &taskRes, const std::string &opNames,
                                     const std::string &opTypes, const std::string &opModuleNames,
                                     const std::string &opPattern, const std::string &coreType,
                                     const std::string &prebuiltOptions, int loglevel) {
  TE_FUSION_LOG_FULL(loglevel,
                     ...
                     taskRes->result.c_str(), opModuleNames.c_str(), taskRes->infoMsg.c_str(),
                     taskRes->compile_info_key.c_str(), opPattern.c_str(), coreType.c_str(), prebuiltOptions.c_str(),
                     taskRes->jsonFilePath.c_str());
}

void TeFusionManager::ProcessErrorInfo(const OpBuildTaskPtr &relBuildTaskPtr, const OpBuildTaskResultPtr &taskRes,
                                       const std::string &opNames, const std::string &opTypes,
                                       const std::string &opModuleNames, int loglevel, std::string &errorMsg) {
...
bool TeFusionManager::SetBuildResults(OpBuildTaskPtr &relBuildTaskPtr, OpBuildTaskResultPtr &taskRes) {
  if (relBuildTaskPtr->pPrebuildOp == nullptr) {
...
void TeFusionManager::AddToFinishedTaskList(const OpBuildTaskPtr &relBuildTaskPtr, const OpBuildTaskResultPtr &taskRes,
...
void TeFusionManager::LogTaskCompletion(const FinComTask &finshedTaskItem, const OpBuildTaskPtr &relBuildTaskPtr,
                                        const std::string &opNames, const std::string &opTypes) {
  TE_INFOLOG("Get finished task[%lu:%lu] status[%s] from compile list.", finshedTaskItem.graphId,
             finshedTaskItem.taskId, (finshedTaskItem.status == 0) ? "success" : "unsuccessful");

  std::time_t now = std::time(nullptr);
  TE_DBGLOG("Task[%lu:%lu] for [node:%s, type:%s] cost %lfs after dispatch.", finshedTaskItem.graphId,
            finshedTaskItem.taskId, opNames.c_str(), opTypes.c_str(), std::difftime(now, relBuildTaskPtr->start_time));
}
...
bool TeFusionManager::SetPreBuildResult(const OpBuildTaskPtr &relBuildTaskPtr,
  ...
  if (!preBuildPattern.empty() && preBuildPattern != STR_UNDEFINDED) {
    TE_DBGLOG("Modify preBuildPattern from [%s] to [%s]", opBuildResult->preCompileRetPtr->opPattern.c_str(),
  ...
  if (opBuildResult->preCompileRetType == PreCompileResultType::Online) {
    TE_DBGLOG("Cache prebuild result: build type [%d], kernel name [%s].", relBuildTaskPtr->buildType,
              relBuildTaskPtr->kernel.c_str());
    bool ret = TeCacheManager::Instance().SetPreCompileResult(relBuildTaskPtr->kernel, opBuildResult->preCompileRetPtr);
    if (!ret) {
  ...
  if (!opBuildResult->preCompileRetPtr->prebuiltOptions.empty()) {
    if (extraParams.empty()) {
      ...
      try {
        ...
        TE_WARNLOG("Failed to parse jsonStr, the jsonStr is %s and the reason is %s", extraParams.c_str(), e.what());
        return false;
      }
    }
  }
  ...
  TE_DBGLOG("Set node's %s core type to %s, with kernel name %s and pattern %s.",
            ...
            relBuildTaskPtr->kernel.c_str(), opBuildResult->preCompileRetPtr->opPattern.c_str());
  return true;
}
...
void TeFusionManager::SetCompileResultAttr(const ge::OpDescPtr &opDesc, const CompileResultPtr &compileRetPtr) {
  ...
  TE_DBGLOG("Setting json file path attribute [%s] for node [%s].", compileRetPtr->jsonPath.c_str(),
            opDesc->GetNamePtr());
...
bool TeFusionManager::SetBuildResult(OpBuildTaskPtr &relBuildTaskPtr, OpBuildTaskResultPtr &opBuildResult) {
  ...
  if (opBuildResult->compileRetPtr == nullptr && !opBuildResult->jsonFilePath.empty()) {
    TE_INFOLOG("Reuse binary json file for node[%s] is [%s].", opName.c_str(), opBuildResult->jsonFilePath.c_str());
  ...
  if (opBuildResult->compileRetType == CompileResultType::Online && relBuildTaskPtr->superKernelUniqueKey.empty()) {
    TE_DBGLOG("Begin to cache compile result, build type[%d], cacheKernelName[%s].", relBuildTaskPtr->buildType,
              relBuildTaskPtr->kernel.c_str());
    ...
    if (!ret) {
      TE_WARNLOG("Failed to cache compile result for node[%s] and kernel[%s].", opName.c_str(),
                 relBuildTaskPtr->kernel.c_str());
...
bool TeFusionManager::CompilePendingTask(std::vector<OpBuildTaskPtr> &vecOpBuildTask,
                                         ...
                                         const OpBuildTaskResultPtr &opBuildResult) {
  // the finished task may not be first task in the que when fuzzy cache building
  for (size_t i = 0; i < vecOpBuildTask.size(); ++i) {
    auto &cashedBuildTaskPtr = vecOpBuildTask[i];
    TE_FUSION_CHECK(cashedBuildTaskPtr == nullptr, continue);
    ...
    if (cashedTaskItem.status == 0) {
      TE_INFOLOG("Get finished task[%lu:%lu] from finished pending task: status[success], kernel[%s], taskIndex[%zu]",
                 cashedTaskItem.graphId, cashedTaskItem.taskId, res->result.c_str(), i);
    } else {
      TE_FUSION_LOG_EXEC(
          TE_FUSION_LOG_WARNING,
...
std::pair<OpBuildTaskResultPtr, std::vector<OpBuildTaskPtr>> *TeFusionManager::FindBuildTaskList(
  ...
  if (relBuildTaskPtr->pPrebuildOp != nullptr) {
    ...
    if (pMatch != nullptr) {
      TE_INFOLOG("Get prebuild op finished task from cached task list, kernel [%s]", relBuildTaskPtr->kernel.c_str());
    }
    ...
    if (opIter != fusionOpsKernel_.end()) {
      pMatch = &opIter->second;
      TE_INFOLOG("Retrieve completed fusion operation task from cached task list: [%s]",
                 relBuildTaskPtr->kernel.c_str());
    }
...
bool TeFusionManager::FinishPendingTask(const OpBuildTaskPtr &relBuildTaskPtr,
  ...
  TE_DBGLOG("Find pending task list, kernel [%s], result size: [%lu].", relBuildTaskPtr->kernel.c_str(),
            vecOpBuildTask.size());
...
bool SetNodeCompileInfoAttr(const OpDescPtr &opDesc, const OpBuildTaskResultPtr &opRes) {
  ...
  TE_INFOLOGF("Op [name=%s]: save compile_info_key: %s, compile_info_json: %s.", opName.c_str(), compileInfoKey.c_str(),
              compileInfoStr.c_str());

  bool bres = AttrUtils::SetStr(opDesc, "compile_info_json", compileInfoStr);
  if (!bres) {
    REPORT_TE_INNER_ERROR("Node[%s] save compile_info_json failed. info: %s", opName.c_str(), compileInfoStr.c_str());
    return false;
  }

  bres = AttrUtils::SetStr(opDesc, "compile_info_key", compileInfoKey);
  if (!bres) {
    REPORT_TE_INNER_ERROR("Node [%s] failed to save compile_info_key, key: %s", opName.c_str(), compileInfoKey.c_str());
    return false;
  }
...
OpBuildResCode TeFusionManager::BuildTbeOp(OpBuildTaskPtr &opTask, const std::string &opCompileStrategyStr) {
