...
bool GetPyObjOfShape(const TbeOpTensor &paraTensor, PyObject *&pyTensor, bool &, const TbeOpInfo &opinfo) {
  ...
  TE_FUSION_CHECK_WITH_DUMP_PYERR((!bres), {
    TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Failed to convert current shape to PyObject, shape size=[%zu].",
                       curShape.size());
    return false;
  });
...
bool GetPyObjOfOriShape(const TbeOpTensor &paraTensor, PyObject *&pyTensor, bool &, const TbeOpInfo &opinfo) {
  ...
  TE_FUSION_CHECK_WITH_DUMP_PYERR((!bres), {
    TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Failed to convert original shape to PyObject, shape size=[%zu].",
                       oriShape.size());
    return false;
  });
...
bool SetStringInfoToPyObj(const std::string &info, const std::string &key, PyObject *&pyTensor) {
  ...
  TE_FUSION_CHECK_WITH_DUMP_PYERR((pyString == nullptr), {
    TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Failed to convert parameter from current %s[%s] to PyObject.", key.c_str(),
                       info.c_str());
    return false;
  });
...
bool SetIntInfoToPyObj(const T &info, const std::string &typeStr, const std::string &key, PyObject *&pyTensor) {
  ...
  TE_FUSION_CHECK_WITH_DUMP_PYERR((pyInt == nullptr), {
    TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Failed to convert parameter from %s to PyObject.", key.c_str());
    return false;
  });
...
bool GetPyObjOfValidShape(const TbeOpTensor &paraTensor, PyObject *&pyTensor, const bool &isInput,
  ...
  TE_FUSION_CHECK_WITH_DUMP_PYERR((!bres), {
    TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Failed to convert valid shape to PyObject, shape size=[%zu].",
                       validShape.size());
    return false;
  });
...
bool GetPyObjOfSliceOffset(const TbeOpTensor &paraTensor, PyObject *&pyTensor, bool &, const TbeOpInfo &opinfo) {
  ...
  TE_FUSION_CHECK_WITH_DUMP_PYERR((!bres), {
    TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Failed to convert slice offset to PyObject, shape size=[%zu].",
                       sliceOffset.size());
    return false;
  });
...
bool GetPyObjOfIsFirstLayer(const TbeOpTensor &paraTensor, PyObject *&pyTensor, bool &, const TbeOpInfo &opinfo) {
  ...
  if (hasSet) {
    int ires =
        HandleManager::Instance().TE_PyDict_SetItemString(pyTensor, "is_first_layer", isFirstLayer ? pyTrue : pyFalse);
    TE_FUSION_CHECK_WITH_DUMP_PYERR((ires != 0), {
...
bool GetPyObjectOfInputConst(const TbeOpTensor &paraTensor, PyObject *&pyTensor, bool &, const TbeOpInfo &opinfo) {
  ...
  int ires =
      HandleManager::Instance().TE_PyDict_SetItemString(pyTensor, "is_input_const", isInputConst ? pyTrue : pyFalse);
  TE_FUSION_CHECK_WITH_DUMP_PYERR((ires != 0), {
...
bool GetPyObjectOfNullableOutputExist(const TbeOpTensor &paraTensor, PyObject *&pyTensor, bool &,
                                      const TbeOpInfo &opinfo) {
  (void)opinfo;
  PyObject *pyTrue = HandleManager::Instance().get_py_true();
  PyObject *pyFalse = HandleManager::Instance().get_py_false();
...
const std::vector<GetPyTensorByTbeOpTensor> PY_TENSOR_GET_FUNCS = {
    GetPyObjOfShape,        GetPyObjOfOriShape,      GetPyObjOfFormat,
    GetPyObjOfCurSubFormat, GetPyObjOfOriFormat,     GetPyObjOfDtype,
    GetPyObjOfAddrType,     GetPyObjOfTotalShape,    GetPyObjOfSliceOffset,
    GetPyObjOfL1Info,       GetPyObjOfValidShape,    GetPyObjSplitIndex,
    GetPyObjOfIsFirstLayer, GetPyObjOfShapeRange,    GetPyObjOfOriginalShapeRange,
    GetPyObjOfValueRange,   GetPyObjOfConstValue,    GetPyObjAtomicAtrr,
    GetPyObjOfCAxisValue,   GetPyObjectOfInputConst, GetPyObjectOfNullableOutputExist};
...
bool AddOptTensorArgs(const std::vector<TbeOpTensor> &tensors, PyObject *&pArgs, int32_t &argsIdx, bool &isInput,
  ...
  TE_FUSION_CHECK((tensors.size() > 1), {
    TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Tensor of option type has wrong size[%zu].", tensors.size());
    return false;
  });
  TE_FUSION_CHECK((tensors.size() == 0), {
    ...
    TE_FUSION_CHECK_WITH_DUMP_PYERR((pyNull == nullptr), {
      TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Failed to convert parameter from null to PyObject.");
      return false;
    });
...
bool AddReqTensorArgs(const std::vector<TbeOpTensor> &tensors, PyObject *&pArgs, int32_t &argsIdx, bool &isInput,
  ...
  TE_FUSION_CHECK((tensors.size() != 1), {
    TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Tensor of request type has wrong size[%lu].", tensors.size());
    return false;
  });
  ...
  TE_FUSION_CHECK_WITH_DUMP_PYERR((ires != 0), {
    TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Failed to add request tensor PyObject to total args.");
    return false;
  });
...
template <typename T>
void PyAttrIntGetter(const TbeAttrValue &attr, PyObject *&pyAttr) {
  T value = {};
  attr.GetValue(value);
...
std::map<ATTR_DTYPE, std::function<void(const TbeAttrValue &, PyObject *&)>> g_pyAttrIntGetFuncs = {
    {ATTR_INT8, PyAttrIntGetter<int8_t>},   {ATTR_UINT8, PyAttrIntGetter<uint8_t>},
    {ATTR_INT16, PyAttrIntGetter<int16_t>}, {ATTR_UINT16, PyAttrIntGetter<uint16_t>},
    {ATTR_INT32, PyAttrIntGetter<int32_t>}, {ATTR_UINT32, PyAttrIntGetter<uint32_t>},
    {ATTR_INT64, PyAttrIntGetter<int64_t>}, {ATTR_UINT64, PyAttrIntGetter<uint64_t>}};
 ...
 */
template <typename T>
bool GetPyAttrListInt(const TbeAttrValue &attr, T &listInt, PyObject *&pyAttr) {
  ...
  for (const auto &dim : listInt) {
    ...
    TE_FUSION_CHECK_WITH_DUMP_PYERR((ires != 0), {
      TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR,
                         "Failed to add int attribute to tuple PyObject, index=[%d], list size=[%zu].", idx,
                         listInt.size());
      return false;
    });
  }
...
const std::map<ATTR_DTYPE, GetSimpleAttrPy> PY_SIMPLE_ATTR_GET_FUNCS = {
    {ATTR_FLOAT32, SimpleAttrForFp32},     {ATTR_DOUBLE, SimpleAttrForDouble},
    {ATTR_BOOL, SimpleAttrForBool},        {ATTR_STR, SimpleAttrForString},
    {ATTR_LIST_INT8, SimpleAttrForInt8},   {ATTR_LIST_UINT8, SimpleAttrForUint8},
    {ATTR_LIST_INT16, SimpleAttrForInt16}, {ATTR_LIST_UINT16, SimpleAttrForUint16},
    {ATTR_LIST_INT32, SimpleAttrForInt32}, {ATTR_LIST_UINT32, SimpleAttrForUint32},
    {ATTR_LIST_INT64, SimpleAttrForInt64}, {ATTR_LIST_UINT64, SimpleAttrForUint64},
};
...
bool ComplexAttrForListFp32(const TbeAttrValue &attr, PyObject *&pyAttr) {
  ...
  for (const auto &dim : listFloat) {
    ...
    TE_FUSION_CHECK_WITH_DUMP_PYERR((ires != 0), {
      TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR,
                         "Failed to add float attribute tuple PyObject value, index=[%d], tuple size=[%zu].", idx,
                         listFloat.size());
      return false;
    });
  }
...
bool ComplexAttrForListDouble(const TbeAttrValue &attr, PyObject *&pyAttr) {
  ...
  for (const auto &dim : listDouble) {
    ...
    TE_FUSION_CHECK_WITH_DUMP_PYERR((ires != 0), {
      TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR,
                         "Failed to add double attribute tuple PyObject value, index=[%d], tuple size=[%zu].", idx,
                         listDouble.size());
      return false;
    });
  }
...
bool ComplexAttrForListBool(const TbeAttrValue &attr, PyObject *&pyAttr) {
  ...
  for (const auto &dim : listBool) {
    ...
    TE_FUSION_CHECK_WITH_DUMP_PYERR((ires != 0), {
      TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR,
                         "Failed to add bool attribute tuple PyObject value, index=[%d], tuple size=[%zu].", idx,
                         listBool.size());
      return false;
    });
  }
...
bool ComplexAttrForListString(const TbeAttrValue &attr, PyObject *&pyAttr) {
  ...
  for (const auto &dim : listStr) {
    ...
    TE_FUSION_CHECK_WITH_DUMP_PYERR((ires != 0), {
      TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR,
                         "Failed to add string attribute to python tuple, index=[%d], tuple size=[%zu].", idx,
                         listStr.size());
      return false;
    });
  }
...
bool ComplexAttrForListInt64(const TbeAttrValue &attr, PyObject *&pyAttr) {
  ...
  for (const auto &vec : listListInt64) {
    ...
    for (const auto &dim : vec) {
      ...
      TE_FUSION_CHECK_WITH_DUMP_PYERR((ires != 0), {
        TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR,
                           "Failed to add int attribute to python tuple, index=[%d], list size=[%zu].", idx2,
                           vec.size());
        return false;
      });
    }
...
const std::map<ATTR_DTYPE, GetComplexAttrPy> PY_COMPLEX_ATTR_GET_FUNCS = {
    {ATTR_LIST_FLOAT32, ComplexAttrForListFp32},     {ATTR_LIST_DOUBLE, ComplexAttrForListDouble},
    {ATTR_LIST_BOOL, ComplexAttrForListBool},        {ATTR_LIST_STR, ComplexAttrForListString},
...
bool GetPyAttr(const TbeAttrValue &attr, PyObject *&pyAttr) {
  ...
  bool isNum = (attrDtype == ATTR_FLOAT32 || attrDtype == ATTR_DOUBLE || attrDtype == ATTR_BOOL ||
                attrDtype == ATTR_STR || attrDtype == ATTR_LIST_INT8 || attrDtype == ATTR_LIST_UINT8 ||
                attrDtype == ATTR_LIST_INT16 || attrDtype == ATTR_LIST_UINT16 || attrDtype == ATTR_LIST_INT32 ||
                attrDtype == ATTR_LIST_UINT32 || attrDtype == ATTR_LIST_INT64 || attrDtype == ATTR_LIST_UINT64);

  bool isList = (attrDtype == ATTR_LIST_FLOAT32 || attrDtype == ATTR_LIST_DOUBLE || attrDtype == ATTR_LIST_BOOL ||
                 attrDtype == ATTR_LIST_STR || attrDtype == ATTR_LIST_LIST_INT64);
...
bool AssembleInputsAndOutputs(const TbeOpInfo &opinfo, PyObject *&pyInputs, PyObject *&pyOutputs) {
  ...
  if (!AddTensorArgs(inputs, pyInputs, input_idx, isInput, opinfo)) {
    TE_ERRLOG("Failed to add input tensor for node[name:%s, type:%s], idx[%d].", opinfo.GetName().c_str(),
              opinfo.GetOpType().c_str(), input_idx);
    return false;
  }
  ...
  if (!AddTensorArgs(outputs, pyOutputs, output_idx, isInput, opinfo)) {
    TE_ERRLOG("Failed to add output tensor for node[name:%s, type:%s], idx[%d].", opinfo.GetName().c_str(),
              opinfo.GetOpType().c_str(), output_idx);
    return false;
  }
  return true;
}
...
bool AssembleOpArgs(const TbeOpInfo &opinfo, const std::string &kernelName, PyObject *&pyInputs, PyObject *&pyOutputs,
                    PyObject *&pyAttrs, bool isSingleOpBuild) {
  bool res = false;
  PyLockGIL pyLockGIL;
...
bool AssemleStringDict(const std::map<std::string, std::string> &infoMap, PyObject *&pyInfoDict) {
  for (auto iter = infoMap.begin(); iter != infoMap.end(); iter++) {
    ...
    TE_FUSION_CHECK((pyValue == nullptr), {
      TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "build pyValue PyObject failed with args: %s", iter->second.c_str());
      return false;
    });
    ...
    TE_FUSION_CHECK((res != 0), {
      TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "build TE_PyDict_SetItemString failed: [%s, %s]", iter->first.c_str(),
                         iter->second.c_str());
      return false;
    });
  }
