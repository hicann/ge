...
void BinaryManager::GenerateOpBinaryJsonKey(const OpBuildTaskPtr &opTask, bool hasOmKeyId, bool isOm,
...
bool BinaryManager::CheckBinaryVersionMatch(bool isOmVersion, const OpBuildTaskPtr &opTask) const {
...
bool BinaryManager::CheckReuseBinaryCondition(const OpBuildTaskPtr &opTask) const {
  ...
  if (!performanceMode.empty() && performanceMode != "normal") {
    TE_DBGLOGF("Node(%s) performance mode(%s) is not normal. Need to compile", GetTaskNodeName(opTask).c_str(),
...
bool BinaryManager::CheckAndReadBinaryFile(const OpBuildTaskPtr &opTask, json &binListJson, bool hasOmKeyId,
                                           bool isOm) {
  ...
  if (!TeFileUtils::GetJsonValueFromJsonFile(binaryFileRealPath, jsonValue)) {
    TE_DBGLOG("Node(%s) not read binary file(%s).", GetTaskNodeName(opTask).c_str(), binaryFileRealPath.c_str());
    return false;
  }
  ...
  if (binListJsonIter == jsonValue.end()) {
    TE_WARNLOG("Node(%s) binary json file(%s) has no bin list.", GetTaskNodeName(opTask).c_str(),
               binaryFileRealPath.c_str());
    return false;
  }
...
bool BinaryManager::CheckMatchInBinListByKey(const OpBuildTaskPtr &opTask, const std::string &key,
  ...
  if (!TeJsonUtils::FilterJsonValueFromJsonListByKey(key, keyValue, binListJson)) {
    TE_DBGLOGF("The node %s %s(%s) does not match in binary file", GetTaskNodeName(opTask).c_str(), key.c_str(),
               keyValue.c_str());
    return false;
  }
...
bool BinaryManager::IsBinaryFileValid(bool isOm, const OpBuildTaskPtr &opTask, const json &binListJson,
...
bool BinaryManager::IsAllDimsDyn(const ge::GeTensorDescPtr &tenosrDescPtr) {
  ...
  bool isUnknownRange =
      std::all_of(shapeRange.begin(), shapeRange.end(), [](const std::pair<int64_t, int64_t> &dimRange) {
...
bool BinaryManager::IsDynamicNodes(const OpBuildTaskPtr &opTask) {
  for (const auto &node : opTask->opNodes) {
    ...
    if (!tbeOpInfo->GetIsUnknownShape()) {
      TE_DBGLOG("Node(%s) is not unknown shape, cannot reuse binary result. Need to compile.", node->GetName().c_str());
      return false;
    }
  }
  return true;
...
bool BinaryManager::GetJsonValueByJsonFilePath(const std::string &jsonFilePath, nlohmann::json &jsonValue) {
