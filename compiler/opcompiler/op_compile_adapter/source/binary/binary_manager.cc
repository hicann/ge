...
void BinaryManager::SetBinaryConfigPaths(const int64_t &imply_type, std::string &file_path) {
  ...
  if (pos != string::npos) {
    ...
    TE_DBGLOG("tbe current imply_type is [%lld], binary_config_path is [%s].", imply_type, binary_config_path.c_str());
  }
}
...
void BinaryManager::SetRelocatableBinaryConfigPaths(const int64_t &imply_type, std::string &file_path) {
  ...
  if (pos != string::npos) {
    ...
    TE_DBGLOG("tbe current imply_type is [%lld], relocatable_kernel_info_config_path is [%s]", imply_type,
              binary_config_path.c_str());
  }
}
...
void BinaryManager::GetBuiltinBinaryPath(const std::map<std::string, std::string> &options,
  ...
  TE_DBGLOGF("Get binary opp key(%ld) of path(%s) and cfg(%s)", implType, kernelPath.c_str(), cfgPath.str().c_str());
  return;
}
...
void BinaryManager::GetAllBinaryVersionInfo(bool isOm) {
  ...
  for (; it != pathMap.cend(); ++it) {
    ...
    TE_DBGLOGF("Add impl: %ld. (adkversion = %s, oppversion = %s)", it->first, binAdkVersion.c_str(),
               binOppVersion.c_str());
  }
  return;
}
...
bool BinaryManager::GetBinaryFileName(const OpBuildTaskPtr &opTask, std::string &binaryFileRealPath, bool hasOmKeyId,
                                      bool isOm) const {
  std::string filePath;
  ...
  TE_DBGLOG("Node[%s] get binary file(%s) success", binaryFileRealPath.c_str(), GetTaskNodeName(opTask).c_str());
  return true;
}
...
bool BinaryManager::MatchStaticKey(const OpBuildTaskPtr &opTask, json &staticKeyJson, json &binListJson) const {
  TE_DBGLOG("Get generalized ops JSON: %s", staticKeyJson.dump().c_str());
  ...
  if (!PythonApiCall::Instance().GenerateStrSha256HashValue(staticKeyJsonStr, staticKey)) {
    TE_DBGLOG("Node(%s) do not generate static key hash by json(%s).", GetTaskNodeName(opTask).c_str(),
              staticKeyJsonStr.c_str());
    return false;
  }
...
namespace {
enum DeterministicValue { DETERMINISTIC_IGNORE, DETERMINISTIC_TRUE, DETERMINISTIC_FALSE };
...
bool BinaryManager::MatchDeterministic(const json &opParamsJson, const json &binJson) const {
  ...
  static const string deterministicString[] = {[DETERMINISTIC_IGNORE] = "ignore",
                                               [DETERMINISTIC_TRUE] = "deterministic",
                                               [DETERMINISTIC_FALSE] = "nondeterministic"};
  TE_DBGLOGF("Deterministic match params:%s, bin:%s, result is %d", deterministicString[paramDeterministic].c_str(),
             deterministicString[binDeterministic].c_str(), matchResult);
  return matchResult;
}
...
bool BinaryManager::MatchConstValue(const json &binValueJson) {
  ...
  if (binValue.size() == 0 && binValueRange.size() == 0) {
    TE_DBGLOG(
        "Binary JSON does not have a constant value. Supports all constants. Successfully matched the constant value "
        "parameter.");
    return true;
  }
...
template <typename T>
bool MatchParamFromJson(const std::string &key, T &dftValue, const json &binValueJson, const json &opValueJson) {
...
template <typename T>
bool MatchAttrParamFromJson(const std::string &key, T &dftValue, const json &binValueJson, const json &opValueJson) {
  T binValue = dftValue;
  T opValue = dftValue;
  try {
    auto iter = binValueJson.find(key);
...
template <typename T>
bool MatchParamFromJsonWithTwoDft(const std::string &key, T &dftValue, T &dftValue1, const json &binValueJson,
                                  const json &opValueJson) {
  auto iter = binValueJson.find(key);
  if (iter == binValueJson.end()) {
    // not include in binary, means insensitive to the param, can reuse
    return true;
...
bool BinaryManager::MatchInputsOutputs(const std::string &key, const json &dynamicJson, const json &binJson,
...
template <typename T>
bool CompareAttrValueFloat(const json &opJson, const json &binJson) {
  try {
...
template <typename T>
bool CompareAttrValue(const json &opJson, const json &binJson) {
  try {
    T opValue = opJson.get<T>();
    T binValue = binJson.get<T>();
...
template <typename T>
bool CompareAttrValueInt(const json &opJson, const json &binJson) {
  try {
    T binValue = binJson.get<T>();
...
*/
template <typename T>
bool CompareAttrValueListInt(const json &opJson, const json &binJson) {
  T binValue;
  T opValue;
  try {
...
template <typename T>
bool CompareAttrValueListFloat(const json &opJson, const json &binJson) {
  T binValue;
  T opValue;
  try {
    binValue = binJson.get<T>();
...
template <typename T>
bool CompareAttrValueListBoolOrStr(const json &opJson, const json &binJson) {
  T binValue;
  T opValue;
  try {
    binValue = binJson.get<T>();
...
*/
template <typename T>
bool CompareAttrValueListList(const json &opJson, const json &binJson) {
  T binValue;
  T opValue;
  try {
    binValue = binJson.get<T>();
...
bool BinaryManager::CompareSingleAttr(const std::string &dtype, const json &opAttr, const json &binAttr) {
...
bool BinaryManager::MatchSingleAttr(const json &opAttr, const json &binAttr) {
  ...
  TE_DBGLOGF("Get op attr dtype %s, bin attr dtype %s.", opDtype.c_str(), binDtype.c_str());
  if (opDtype != binDtype) {
...
bool BinaryManager::MatchFusionOpsAttrs(const json &opParamsJson, const json &binJson) {
  ...
  if (opGraphOpParams.size() != binGraphOpParams.size()) {
    TE_DBGLOGF("Op GraphOpParams size(%zu) is not same as binary size(%zu). Match did not succeed.",
               opGraphOpParams.size(), binGraphOpParams.size());
    return false;
  }
...
bool BinaryManager::MatchAttrs(const OpBuildTaskPtr &opTask, const json &opParamsJson, const json &binJson) {
...
bool BinaryManager::MatchOpParams(const OpBuildTaskPtr &opTask, json &binListJson,
                                  GeneralizedResult &generalizedResult) const {
  TE_DBGLOGF("Start to match op params. (dynamicJson = %s, binJson = %s)", generalizedResult.dynamicJson.dump().c_str(),
  ...
  for (auto iter = binListJson.begin(); iter != binListJson.end(); ++iter) {
    ...
    if (MatchAttrs(opTask, generalizedResult.dynamicJson, binJson)) {
      binListJson.clear();
      binListJson = binJson;
      TE_DBGLOGF("Node(%s) match op params successfully with json(%s).", GetTaskNodeName(opTask).c_str(),
                 binListJson.dump().c_str());
      return true;
    } else {
...
bool BinaryManager::MatchStaticKeyWithOptionalInputNull(const OpBuildTaskPtr &opTask, json &binListJson,
                                                        GeneralizedResult &generalizedResult) const {
  ...
  TE_DBGLOG("Generate static key is %s, json is %s", staticKey.c_str(), staticKeyJsonStr.c_str());
...
bool BinaryManager::MatchSimplifiedKey(const OpBuildTaskPtr &opTask, string &jsonFilePath) const {
...
bool BinaryManager::GenerateAndMatchSimpleKey(ge::Node *opNode, const TbeOpInfoPtr &opInfo,
  ...
  TE_DBGLOG("opType = [%s], simpleKeyMode = [%d], opImplMode = [%s], deterministic = [%s]", opType.c_str(),
            simpleKeyMode, opImplMode.c_str(), deterministic.c_str());
  ...
  if (!binaryInfoPtr->MatchSimpleKey(opType, simpleKeystr, jsonFilePath)) {
    TE_WARNLOG("Node[%s], opType[%s] MatchSimpleKey failed. current simpleKey = [%s].", opNode->GetName().c_str(),
               opType.c_str(), simpleKeystr.c_str());
    return false;
  }
  if (isSuperKernel) {
...
bool BinaryManager::ReuseBinKernelBySimpleKey(const OpBuildTaskPtr &opTask) {
  ...
  TE_DBGLOG("Node(%s) jsonFilePath = [%s], binaryPath = [%s]", opName.c_str(), jsonFilePath.c_str(),
            binaryPath.c_str());
  std::string opsPathNamePrefix;
  (void)ge::AttrUtils::GetStr(opTask->opNodes[0]->GetOpDesc(), OPS_PATH_NAME_PREFIX, opsPathNamePrefix);
...
bool BinaryManager::ReuseKernelBinaryCompileRes(const OpBuildTaskPtr &opTask) {
  ...
  if (!CheckAndReadBinaryFile(opTask, binListJson, false, false)) {
    TE_INFOLOG("Node(%s) do not check and read binary file. Need to compile", GetTaskNodeName(opTask).c_str());
    return false;
  }
...
bool BinaryManager::SetOmBinaryReuseResult(const OpBuildTaskPtr &opTask, const std::string &jsonFilePath) const {
  ...
  if (realPath.empty()) {
    TE_INFOLOG("Node(%s)'s om json file path(%s) does not exist.", GetTaskNodeName(opTask).c_str(),
               jsonFilePath.c_str());
    return false;
  }
...
bool BinaryManager::GetNodeStrAttr(const OpBuildTaskPtr &opTask, const std::string &attrKey, std::string &attrValue) {
...
bool BinaryManager::FusionOpReuseOmBinary(const OpBuildTaskPtr &opTask, json &binListJson, std::string &omKeyId) const {
  ...
  if (!SetOmBinaryReuseResult(opTask, jsonFilePath)) {
    TE_DBGLOG("Node(%s) not set om binary reuse result. Not reuse om binary files.", GetTaskNodeName(opTask).c_str());
    return false;
  }
...
bool BinaryManager::ReuseOmBinaryCompileRes(const OpBuildTaskPtr &opTask, bool &hasOmKeyId) {
  ...
  if (!CheckAndReadBinaryFile(opTask, binListJson, hasOmKeyId, true)) {
    TE_DBGLOG("Node(%s) do not check and read binary file. Not reuse om binary files", GetTaskNodeName(opTask).c_str());
    return false;
  }
...
bool BinaryManager::BackToSingleCheck(const OpBuildTaskPtr &opTask) {
  ...
  TE_INFOLOG("Node [%s] is ready to check if all nodes can reuse the op binarykernel.",
             GetTaskNodeName(opTask).c_str());
  for (const auto &node : opNodes) {
    std::vector<ge::Node *> teGraphNode;
    teGraphNode.emplace_back(node);
    OpBuildTaskPtr opTempTask = nullptr;
    TE_FUSION_MAKE_SHARED(
        opTempTask = std::make_shared<OpBuildTask>(0, 0, 0, teGraphNode, nullptr, OP_TASK_STATUS::OP_TASK_PENDING),
        return false);
    if (!MatchAndReuseBinRes(opTempTask)) {
      TE_INFOLOG("Node [%s] cannot find the binarykernel; needs to compile the fusion op online.",
                 node->GetName().c_str());
      return false;
    }
  }
...
void BinaryManager::SetBinaryReuseAttr(const OpBuildTaskPtr &opTask) const {
  ...
  TE_DBGLOG("Ready to set opp binary path for node[%s, %s]", firstNode->GetNamePtr(), firstNode->GetTypePtr());
  int64_t implType = -1;
  ...
  if (implType == AICORE_IMPL_TYPE_NUM && GetBuiltInOppLatestFlag()) {
    ...
    TE_INFOLOG("Set opp binary path enum[%ld] for node[%s, %s].", OPP_LATEST_ENUM, firstNode->GetNamePtr(),
               firstNode->GetTypePtr());
  }
}

void BinaryManager::GetBinaryPath(const OpBuildTaskPtr &opTask, bool isOm, bool isKernelOrModel,
  ...
  for (const auto &currentNode : opTask->opNodes) {
    ...
    TE_DBGLOG("Node[%s] impltype[%ld] get binary path [%s].", currentNode->GetName().c_str(), implType,
              binaryPath.c_str());
    return;
  }
  return;
...
void BinaryManager::GetBinaryVersion(const OpBuildTaskPtr &opTask, bool isOm, std::string &adkVrsion,
