...
bool TeFusionManager::CheckResultInBinaryBuildDir(const OpBuildTaskPtr &opTask, bool &isBuilding, bool &isFound) const {
  ...
  std::string jsonFilePath = TeConfigInfo::Instance().GetKernelMetaParentDir() + "/kernel_meta/" + kernelName + ".json";
  ...
  for (int i = 0; i < size; ++i) {
    ...
    if (kernelMetaParentDir[i] == '\\' || kernelMetaParentDir[i] == '/' || i == size - 1) {
...
bool TeFusionManager::CanReuseBuildBinaryCache(OpBuildTaskPtr &opTask, bool &hasError) {
  // check if build result exists already
  bool isFound = false;
  ...
  if (CheckResultInBinaryBuildDir(opTask, isBuilding, isFound)) {
    ...
    if (isBuilding) {
      ...
      TE_INFOLOG("The same kernel[%s] is building, insert task[%lu:%lu] to waiting list", opTask->kernel.c_str(),
      ...
      TE_INFOLOG("try to reuse same kernel[%s] for task [%lu:%lu] result is [%u].", opTask->kernel.c_str(),
                 opTask->graphId, opTask->taskId, bres);
      return bres;
    }
    return false;
  }
...
bool TeFusionManager::CheckBuildBinaryOpTaskResult(OpBuildTaskPtr &opTask) {
  TE_INFOLOG("Build Binary task dispatched, taskID[%lu:%lu], kernel=[%s]", opTask->graphId, opTask->taskId,
             opTask->kernel.c_str());
  ...
  if (opIter == kernelMap->end()) {
    ...
    TE_INFOLOG("Insert task[%lu:%lu] to op kernel map, kernel name is %s", opTask->graphId, opTask->taskId,
               opTask->kernel.c_str());
    return true;
  }
  TE_ERRLOG("Insert task[%lu:%lu] into op kernel map, duplicated kernel name is: %s", opTask->graphId, opTask->taskId,
            opTask->kernel.c_str());
  opTask->status = OP_TASK_STATUS::OP_TASK_FAIL;
  return false;
}
...
bool TeFusionManager::BuildBinarySingleOp(OpBuildTaskPtr &opTask) {
  ...
  TE_DBGLOG("Start to build single op [%s], taskID[%lu:%lu], json: %s.", pNode->GetName().c_str(), opTask->graphId,
            opTask->taskId, opTask->generalizedJson["supportInfo"].dump().c_str());
  std::string keyName;
