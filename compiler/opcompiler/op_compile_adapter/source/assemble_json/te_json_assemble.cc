...
const std::map<DataType, std::string> DTYPE_STR_MAP{{DT_FLOAT, "float32"}, {DT_DOUBLE, "float64"}};
/**
...
bool TeJsonAssemble::GenerateJsonAndKernelName(const std::vector<Node *> &nodes, const bool isAllowRepeated,
  ...
  if (uniqueJsonStr.empty()) {
    REPORT_TE_INNER_ERROR("Identity json[uniqueJsonStr] of the first node [%s] is empty.", nodes[0]->GetName().c_str());
    return false;
  }
...
bool TeJsonAssemble::GenerateJsonByNodes(const std::vector<ge::Node *> &teGraphNode, nlohmann::json &jsonData) {
  ...
  for (auto &currentNode : teGraphNode) {
    ...
    TE_FUSION_CHECK(tbeOpInfo == nullptr, {
      TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Failed to check op is prebuild or not, op name is [%s].",
                         keyName.c_str());
      return false;
    });
    ...
    if (spCnt != -1 && spSubId != -1 && GetSubOpLoc(spCnt, spSubId, subOpLoc)) {
      ...
      TE_DBGLOG("OpNode: %s, spk_options: %s, sp_cnt: %d, sp_subid: %d.", keyName.c_str(), spOptions.c_str(), spCnt,
                spSubId);
    }
    // get input desc json
    json inputDescJson;
    ...
    TE_FUSION_CHECK(!bres, {
      TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Failed to get builtin op input desc json, op name is [%s].",
                         keyName.c_str());
      return false;
    });
  ...
  // check if output of node has multiple reference, only remain one.
...
bool TeJsonAssemble::GenBuildinIndescJson(const InOutToJsonParam &inputPara,
                                          const std::unordered_set<ge::Node *> &allNodes, nlohmann::json &opJson,
                                          std::map<string, int> &allInputNames,
...
void TeJsonAssemble::GenBuildinOutdescJson(const InOutToJsonParam &outputPara,
                                           ...
                                           const std::vector<ge::Node *> &teGraphNode) {
  TE_FUSION_CHECK(outputPara.node == nullptr, {
    TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Node is null.");
    return;
  });
...
void TeJsonAssemble::GenerateOutputLinkKey(const std::unordered_set<ge::Node *> &allNodes,
                                           const ge::OutDataAnchorPtr &anchor, size_t i,
                                           const InOutToJsonParam &outputParam,
                                           const std::vector<ge::Node *> &teGraphNode, nlohmann::json &outputDescJson) {
  TE_FUSION_CHECK(anchor == nullptr, {
    TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Anchor is null.");
    return;
  });
  auto anchorIdx = anchor->GetIdx();
  auto curNode = anchor->GetOwnerNode();
  TE_FUSION_CHECK(curNode == nullptr, {
    TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Node is null.");
    return;
  });
  ...
  std::string linkKey =
      "o:" + outputParam.verifyOpTypeList[outputParam.nodeListIdx] + "_" + srcTensorName + "__" + postFix;
  outputDescJson["link_key"] = linkKey;
  TE_DBGLOG("Op [%s] output[%d] link key is [%s].", anchor->GetOwnerNode()->GetName().c_str(), anchorIdx,
            linkKey.c_str());
  return;
}
...
void TeJsonAssemble::OutputsDescToJsonProcess(nlohmann::json &json_str, const std::unordered_set<ge::Node *> &allNodes,
  ...
  if (peers.size() > 0) {
    ...
    if (peerDataNodesSize == 0 && peersCrlAnchor.size() > 0 && outputParam.node->GetOutDataAnchor(0) != nullptr) {
      hasDataAnchor = true;
    } else {
      hasDataAnchor = false;
    }
  ...
  for (size_t i = 0; i < peerSize; ++i) {
    ...
    TE_FUSION_CHECK(l1AddrFlag != -1, outputDescJson["L1_addr_flag"] = l1AddrFlag);
    outputDescJson["L1_addr_offset"] = l1AddrOffset;
    TE_FUSION_CHECK(l1AddrFlag == 1, outputDescJson["L1_valid_size"] = l1ValidSize);
...
bool TeJsonAssemble::CheckInputsSize(InputDescJsonParam &inputDescJsonPara) {
  ...
  TE_FUSION_CHECK(inputDescSize > inputDescJsonPara.inputSize, {
    TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Graph input size[%d] should be <= input size=[%d].", inputDescSize,
                       inputDescJsonPara.inputSize);
    return false;
  });
  return true;
...
bool TeJsonAssemble::GenInputsLinkKey(const ge::Node *node, size_t nodeListIdx,
                                      ...
                                      const std::vector<ge::Node *> &teGraphNode) {
  TE_FUSION_CHECK(node == nullptr, {
    TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Node is null.");
    return false;
  });
  TE_FUSION_CHECK(allNodes.size() == 1, {
    TE_FUSION_LOG_EXEC(TE_FUSION_LOG_DEBUG, "SingleOpTask don't need link_key.");
    return true;
  });
  ...
  for (size_t inputIdx = 0; inputIdx < const_para.inputSize; ++inputIdx) {
    ...
    if (isNull) {
      ...
      if (allInputNames.find(const_para.inputNameList[realIdx]) == allInputNames.end()) {
        ...
        inputDescJson["name"] =
            const_para.inputNameList[realIdx] + ":" + std::to_string(allInputNames[const_para.inputNameList[realIdx]]);
...
void TeJsonAssemble::GetPeerInputsOrder(const ge::OutDataAnchorPtr &anchor, uint32_t idx,
  ...
  TE_DBGLOG("Size of peerInNodes is %zu for the %dth out of node %s", peerInNodes.size(), anchor->GetIdx(),
            anchor->GetOwnerNode()->GetName().c_str());
...
void TeJsonAssemble::GetAllInnerSuccessorTypes(const std::unordered_set<Node *> &allNodes,
  ...
  while (!allSuccessors.empty() && index < maxRecursive) {
     ...
     * This operation is to differentiate the following two
     * cases:
     *           A                 A
     *     out1 / \  out2    out2 / \  out1
     *      \  /   \ /        \  /   \ /
...
void TeJsonAssemble::GetPeerOutputsOrder(const ge::Node *node, const std::unordered_set<Node *> &allNodes,
                                         std::map<size_t, int64_t> &peerOutputsOrder) {
  TE_DBGLOG("Get peer output order of node [%s, %s], total node count [%zu].", node->GetNamePtr(), node->GetTypePtr(),
            allNodes.size());
  ...
  for (size_t inputIndex = 0; inputIndex < inDataAnchors.size(); inputIndex++) {
    if (inDataAnchors.at(inputIndex) == nullptr || inDataAnchors.at(inputIndex)->GetPeerOutAnchor() == nullptr ||
  ...
  std::sort(allInputsId.begin(), allInputsId.end(),
            [](std::pair<size_t, int64_t> a, std::pair<size_t, int64_t> b) { return a.second < b.second; });
...
bool TeJsonAssemble::GenDatadescJson(const std::vector<ge::Node *> &teGraphNode, json &jsonStr) {
  ...
  for (auto &currentNode : teGraphNode) {
    ...
    TE_FUSION_CHECK(!bres, {
      TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Failed to get Node[%s] input key name.", currentNode->GetName().c_str());
      return false;
    });
    ...
    for (uint32_t idx = 0; idx < inputTensorSize; ++idx) {
      ...
      TE_FUSION_CHECK(iter == inputOrder.end(), {
        TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Failed to get input index pair, index=[%d].", idx);
        return false;
      });
      bool isNull = false;
      bres = CheckInputsNullTensor(inputs[iter->second], isNull);  // need to consider DYN inputs
      TE_FUSION_CHECK(!bres, {
        TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Failed to check if inputs tensor is null.");
        return false;
      });
      ...
      if (isNull) {
        ...
        TE_FUSION_CHECK(preNode == nullptr, {
          TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Failed to get peer node point, curr keyName=[%s], index=[%d].",
                             keyName.c_str(), idx);
          return false;
        });
        if (std::find(teGraphNode.begin(), teGraphNode.end(), preNode) == teGraphNode.end()) {
          ...
          TE_FUSION_CHECK(!bres, {
            TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Failed to gen data json, index=[%d].", iter->second);
            return false;
          });
...
bool TeJsonAssemble::GenNodeDataJson(const std::vector<ge::Node *> &teGraphNode, ge::Node *currNode,
  ...
  TE_FUSION_CHECK((type == TT_DYN), {
    TE_FUSION_CHECK((inputTensorIdx < tensorCount || inputTensorIdx >= (tensorCount + tensors.size())), {
      TE_FUSION_LOG_EXEC(TE_FUSION_LOG_ERROR, "Failed to get input index error, index=[%d].", inputTensorIdx);
      return false;
    });
  ...
  TE_FUSION_CHECK(l1AddrFlag == 1, { inputDescJson["L1_valid_size"] = tensors.at(tensorIdx).GetL1ValidSize(); });
...
void TeJsonAssemble::GenerateSocInfoJson(const std::vector<ConstTbeOpInfoPtr> &tbeOpInfoVec,
  ...
  if (isValidOpInfo) {
    ...
    if (hardwareInfoMap.find(kAiCoreCnt) != hardwareInfoMap.end()) {
      ...
      TE_DBGLOG("OpNode: %s, set ge ai_core_cnt: %s", firstOpInfo->GetName().c_str(),
                hardwareInfoMap[kAiCoreCnt].c_str());
    }
    if (hardwareInfoMap.find(kVectorCoreCnt) != hardwareInfoMap.end()) {
      socInfoJson[kVectorCoreCnt] = hardwareInfoMap[kVectorCoreCnt];
      TE_DBGLOG("OpNode: %s, set ge vector_core_cnt to %s", firstOpInfo->GetName().c_str(),
                hardwareInfoMap[kVectorCoreCnt].c_str());
    }
...
bool TeJsonAssemble::GenerateOptionsMap(const std::vector<ConstTbeOpInfoPtr> &tbeOpInfoVec,
