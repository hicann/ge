...
bool KernelLaunch(const std::string &stub_func, const uint32_t block_dim, const void *args, uint32_t args_size,
  ...
  kernel_context->set_args_offset(static_cast<void *>(super_args_offset),
                                  static_cast<size_t>(super_args_count * sizeof(uint16_t)));
  return true;
}
...
ge::Status FillTaskDefAfterGenTask(const ge::OpDescPtr &op_desc, domi::TaskDef &task_def, std::string &args_format) {
  ...
  FE_LOGD("FillTaskDefAfterGenTask", "Set schedule mode[%u] on task of op[%s, %s]. ", schedule_mode,
  ...
  for (uint32_t i = 0; i < subTasks.size(); ++i) {
    ...
    for (auto &single_task : subTaskVec) {
      ...
      if (IsAICpuTaskDef(single_task, kernel_context)) {
        ...
        if (GetAICoreArgFormatinLoop(sub_arg_format, sub_nodes, args_size, super_kernel_args_format, shared_node,
                                     sub_node, i) != ge::SUCCESS) {
...
ge::Status GenTaskForSuperKernel(const ge::Node &node, std::vector<std::vector<domi::TaskDef>> &subTasks,
  ...
  if (is_need_ffts) {
    ...
    (void)ge::ArgsFormatDesc::ConvertToSuperKernelArgFormat(shared_node, first_node, super_kernel_first_addr,
                                                            super_kernel_args_format);
    FE_LOGI("GenTaskForSuperKernel get ffts format");
    args_size_total += WORKSPACE_SIZE;  // size of ffts addr
  }

  auto ret = GetArgFormat(sub_nodes, args_size_total, subTasks, super_kernel_op_desc, tasks, shared_node,
                          super_kernel_args_format);
  if (ret != ge::SUCCESS) {
    return ge::FAILED;
  }
  ...
  if (SetArgFormatValue(args_size_workspace, subTasks, sub_nodes, all_args_buff_total, args_size_total) !=
      ge::SUCCESS) {
    free(all_args_buff_total);
  ...
  if (ge::AttrUtils::GetInt(super_kernel_op_desc, ge::TVM_ATTR_NAME_BLOCKDIM, block_dim)) {
    ...
    FE_LOGD("Get block dim[%d] of node[%s, %s]", block_dim, super_kernel_op_desc->GetNamePtr(),
  ...
  FE_LOGI("Node[%s, %s] ready to fill taskdef with argsformat [%s].", super_kernel_op_desc->GetNamePtr(),
          super_kernel_op_desc->GetTypePtr(), super_kernel_args_format.c_str());
  if (FillTaskDefAfterGenTask(super_kernel_op_desc, superkernel_task_def, super_kernel_args_format) != ge::SUCCESS) {
