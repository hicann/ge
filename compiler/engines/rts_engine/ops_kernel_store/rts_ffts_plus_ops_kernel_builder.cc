...
Status RtsFftsPlusOpsKernelBuilder::CalcOpRunningParam(Node &geNode)
{
    ...
    RTS_LOGI("[%s: %s] ffts plus output size=%zu.",
        nodeName.c_str(), nodeType.c_str(), outputSize);
    ...
    for (size_t i = 0; i < outputSize; ++i) {
        ...
        if (grhStatus != GRAPH_SUCCESS) {
            RTS_REPORT_CALL_ERROR("Get ffts plus op[%s: %s] out[%zu] memory size failed, "
        ...
        if (outputMemSize < 0) {
            RTS_REPORT_CALL_ERROR("Got ffts plus op[%s:%s] out[%zu] memory size is negative(not support), format=%s, dataType=%s, outputMemSize=%" PRId64,
                nodeName.c_str(), nodeType.c_str(), i, TypeUtils::FormatToAscendString(format).GetString(),
                TypeUtils::DataTypeToAscendString(dataType).GetString(), outputMemSize);
            return FAILED;
        }
        RTS_LOGI("Calc ffts plus op[%s: %s] out[%zu] mem size is %" PRId64 " format=%s, dataType=%s.",
            nodeName.c_str(), nodeType.c_str(), i, outputMemSize,
            TypeUtils::FormatToAscendString(format).GetString(), TypeUtils::DataTypeToAscendString(dataType).GetString());
        TensorUtils::SetSize(outputTensor, outputMemSize);
        grhStatus = opDesc->UpdateOutputDesc(i, outputTensor);
        if (grhStatus != GRAPH_SUCCESS) {
            RTS_REPORT_CALL_ERROR("Update ffts plus op[%s: %s] out[%zu] description failed, format=%s, dataType=%s, retCode=%#x.",
                nodeName.c_str(), nodeType.c_str(), i, TypeUtils::FormatToAscendString(format).GetString(),
                TypeUtils::DataTypeToAscendString(dataType).GetString(), grhStatus);
            return FAILED;
        }
...
Status RtsFftsPlusOpsKernelBuilder::GenerateTask(const Node &geNode, RunContext &context, vector<TaskDef> &tasks)
{
    ...
    RTS_LOGI("Generate task start, node:%s, node type:%s, tasks.size()=%zu.",
        name.c_str(), type.c_str(), tasks.size());

    auto op = OpFftsPlusFactory::Instance().CreateOp(geNode, context);
    if (op == nullptr) {
        RTS_REPORT_CALL_ERROR("Create op for node:%s, node type:%s failed.",
    ...
    Status ret = op->Init();
    if (ret != SUCCESS) {
        RTS_REPORT_CALL_ERROR("Node:%s, node type:%s op init failed, retCode=%#x",
            name.c_str(), type.c_str(), ret);
        return ret;
    }
    ...
    ret = op->Run(tasks);
    if (ret != SUCCESS) {
        RTS_REPORT_CALL_ERROR("Node:%s, node type:%s op run failed, retCode=%#x",
            name.c_str(), type.c_str(), ret);
        return ret;
    }

    RTS_LOGD("Generate task end, node:%s, node type:%s, tasks.size()=%zu.",
        name.c_str(), type.c_str(), tasks.size());
    return ret;
}
