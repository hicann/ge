...
namespace cce {
namespace runtime {
RecvNotifyOp::RecvNotifyOp(const Node &node, RunContext &runContext)
    : Op(node, runContext), notifyId_(0)
{}
...
Status RecvNotifyOp::Init()
{
    ...
    if (!AttrUtils::GetInt(op_desc_, ATTR_NAME_RECV_ATTR_NOTIFY_ID, notifyId)) {
        RTS_REPORT_CALL_ERROR("Receive op init failed, get notify_id attribute failed, node:%s.",
            name_.c_str());
        return FAILED;
    }
    notifyId_ = notifyId;
...
Status RecvNotifyOp::Run(vector<TaskDef> &tasks)
{
    ...
    RTS_LOGI("Recv op:%s wait notify end, notify_id:%u.",
           name_.c_str(), notifyId_);
    return SUCCESS;
}
