...
Status LabelSwitchByIndexOp::Init()
{
    ...
    if (input_num_ != LABEL_SWITCH_INPUT_NUM || v_input_data_addr_.size() != LABEL_SWITCH_INPUT_NUM) {
        RTS_REPORT_CALL_ERROR(
            "Label switch by index op init failed, input_num should be %u, actually input num is %zu, input addr size is %zu.",
    ...
    if (!AttrUtils::GetListInt(op_desc_, ATTR_NAME_LABEL_SWITCH_LIST, label_idx_list)) {
        RTS_REPORT_CALL_ERROR("LabelSwitchByIndexOp: node: %s get %s failed.",
            name_.c_str(), ATTR_NAME_LABEL_SWITCH_LIST.c_str());
        return FAILED;
    }

    if (label_idx_list.empty()) {
        RTS_REPORT_CALL_ERROR("Label switch by index op init failed, node: %s label list is empty.",
            name_.c_str());
        return FAILED;
    }
