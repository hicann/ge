...
Status RecvOp::Init()
{
    ...
    if (!AttrUtils::GetInt(op_desc_, ATTR_NAME_RECV_ATTR_EVENT_ID, eventId)) {
        RTS_REPORT_CALL_ERROR("Receive op init failed, get event_id attribute failed, node:%s.",
            name_.c_str());
        return FAILED;
    }
...
Status RecvOp::Run(vector<TaskDef> &tasks)
{
    ...
    RTS_LOGI("Recv op:%s wait record end, event_id:%u.",
            name_.c_str(), eventId_);
    return SUCCESS;
}
...
Status RecvOpMem::Init()
{
    ...
    int32_t eventId = 0;
    if (!AttrUtils::GetInt(op_desc_, ATTR_NAME_RECV_ATTR_EVENT_ID, eventId)) {
        RTS_REPORT_CALL_ERROR("RecvOpMem init failed, get event_id attribute failed, node:%s.",
            name_.c_str());
        return FAILED;
    }
