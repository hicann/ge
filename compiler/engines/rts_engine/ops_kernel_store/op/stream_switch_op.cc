...
Status StreamSwitchOp::Init()
{
    ...
    if (active_stream_list.size() != TRUE_BRANCH_STREAM_NUM) {
        RTS_LOGE("Stream num of switch true branch must be %u[node:%s].",
            TRUE_BRANCH_STREAM_NUM, name_.c_str());
        return FAILED;
    }
    ...
    if (op_desc_->HasAttr(ATTR_NAME_SWITCH_DATA_TYPE)) {
        ...
        if (!getAttrSucc) {
            RTS_LOGE("StreamSwitchOp[node:%s] get attr SWITCH_DATA_TYPE fail.",
                name_.c_str());
            return FAILED;
        }
        data_type_ = static_cast<aclrtCompareDataType>(dataType);
    }
    RTS_LOGI("StreamSwitch init, cond: %d.", cond_);
    return SUCCESS;
}
...
Status StreamSwitchOp::Run(vector<TaskDef> &tasks)
{
    if (v_input_data_addr_.size() != STREAM_SWITCH_INPUT_NUM) {
        RTS_LOGE(
            "invalid v_input_data_addr_ size(%zu), v_input_data_addr_ size must be %u.",
            v_input_data_addr_.size(), STREAM_SWITCH_INPUT_NUM);
        return INTERNAL_ERROR;
    }
    RTS_LOGI("StreamSwitchOp run, name: %s, cond: %d, datatype: %d.",
        name_.c_str(), cond_, data_type_);
...
Status StreamSwitchOp::UpdateTaskDef(vector<TaskDef> &tasks)
{
    ...
    RTS_LOGI("update StreamSwitchOp taskDefSize:%zu, stream_id:%u,stream_index:%u.",
        tasks.size(), streamId, streamIndex);
    return SUCCESS;
}
