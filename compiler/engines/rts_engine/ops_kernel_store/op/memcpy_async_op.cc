...
{
    if (v_output_data_addr_.size() != v_input_data_addr_.size() ||
        v_output_data_addr_.size() != v_output_size_.size() ||
...
Status MemcpyAsyncOp::CheckInputSize(size_t index, int64_t &inputSize)
{
    ...
    if (index >= v_output_size_.size()) {
        RTS_LOGE("CheckInputSize failed, index is Invalid, index=%zu, v_output_size_=%zu",
            index, v_output_size_.size());
        return RT_FAILED;
    }
    ...
    if (inputSize == 0) {
        RTS_LOGW("memcpy size of input index:%zu is 0, skip generate task. node_name: %s",
            index, node_.GetName().c_str());
        return RT_FAILED;
    }
    return SUCCESS;
}
...
Status MemcpyAsyncOp::Run(vector<TaskDef> &tasks)
{
    ...
    for (size_t index = 0; index < v_input_data_addr_.size(); index++) {
        ...
        RTS_LOGI("MemcpyAsyncOp run, index:%zu, outSize:%" PRId64 ", inSize:%" PRId64,
                index, v_output_size_[index], inputSize);
