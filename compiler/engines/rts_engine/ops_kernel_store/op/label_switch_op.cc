...
LabelSwitchOp::LabelSwitchOp(const Node &node, RunContext &runContext)
    : Op(node, runContext), branch_max_(0)
{
}
...
Status LabelSwitchOp::Init()
{
    ...
    if (input_num_ != LABEL_SWITCH_INPUT_NUM || v_input_data_addr_.size() != LABEL_SWITCH_INPUT_NUM) {
        RTS_REPORT_CALL_ERROR(
                "LabelSwitchOp input_num should be %u, actually input num is %zu, input addr size is %zu.",
