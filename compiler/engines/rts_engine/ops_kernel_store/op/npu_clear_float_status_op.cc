...
namespace cce {
namespace runtime {
NpuClearFloatStatusOp::NpuClearFloatStatusOp(const Node &node, RunContext &runContext) : Op(node, runContext),
                                                                                         check_mode_(0)
{
}

Status NpuClearFloatStatusOp::Init()
{
    RTS_LOGI("NpuClearFloatStatusOp Init start, node:%s.", name_.c_str());
...
Status NpuClearFloatStatusOp::Run(vector<TaskDef> &tasks)
{
    ...
    npuClearStatusDef->set_op_index(static_cast<uint32_t>(op_desc_->GetId()));
    tasks.push_back(taskDef);
    RTS_LOGI("NPU clear float status op run end stream_id:%u.", op_desc_->GetStreamId());
    return SUCCESS;
}
