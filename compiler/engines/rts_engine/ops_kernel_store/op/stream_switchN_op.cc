...
StreamSwitchNOp::StreamSwitchNOp(const Node &node, RunContext &runContext)
    : Op(node, runContext),
      element_size_(0),
      size_(0),
      data_type_(ACL_RT_SWITCH_INT64) {}
...
Status StreamSwitchNOp::Init()
{
    ...
    bool getAttrSucc =
            AttrUtils::GetInt(op_desc_, ATTR_NAME_BATCH_NUM, elementSize);
    if (!getAttrSucc) {
    ...
    for (size_t i = 0; i < element_size_; i++) {
        if (data_type_ == ACL_RT_SWITCH_INT32) {
            ...
            if (!getAttrSucc) {
                RTS_REPORT_CALL_ERROR("StreamSwitchNOp[node:%s] get attr ATTR_NAME_PRED_VALUE index=%zu failed.", name_.c_str(), i);
                return FAILED;
            }
    ...
    if (!AttrUtils::GetListInt(op_desc_,
                               ATTR_NAME_ACTIVE_STREAM_LIST,
                               activeStreamList_)) {
    ...
    if (activeStreamList_.size() < element_size_) {
        RTS_REPORT_CALL_ERROR("StreamSwitchNOp[node:%s] activeStreamSize:%zu, elementSize:%u failed.", 
                name_.c_str(), activeStreamList_.size(), element_size_);
        return FAILED;
    }
    RTS_LOGI("StreamSwitchNOp get attribute[%s] list size=%zu", 
            ATTR_NAME_ACTIVE_STREAM_LIST.c_str(), activeStreamList_.size());
    return SUCCESS;
}
