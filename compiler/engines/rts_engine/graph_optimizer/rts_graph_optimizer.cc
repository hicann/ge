...
const std::map<std::string, uint32_t> RTS_SUPPORTED_CONDITION_OP_TYPE = {
    {"StreamSwitchN", 1},
    {"StreamSwitch", 2},
    {"LabelSwitchByIndex", 1}
};

const std::vector<std::string> MEM_TYPE_RANGE_NODES = {
    "MemcpyAddrAsync",
    "LabelSwitchByIndex",
    "LabelGoto",
    "LabelGotoEx",
...
ge::Status RtsGraphOptimizer::OptimizeGraphPrepare(ComputeGraph& graph)
{
    ...
    for (auto nodePtr : graph.GetDirectNode()) {
        ...
        if (CheckSupportedOP(opDescPtr->GetType()) == RT_ERROR_NONE) {
            ...
            RTS_LOGD("graph[%s]: node [%s] op type [%s] format agnostic value is set",
                   graph.GetName().c_str(), nodeName.c_str(), opDescPtr->GetType().c_str());
            if (!bRet) {
                RTS_LOGE("graph[%s]: node [%s] op type [%s] set format agnostic failed",
                graph.GetName().c_str(), nodeName.c_str(), opDescPtr->GetType().c_str());
...
ge::Status RtsGraphOptimizer::PorcMemtypeRange(ge::ComputeGraph &graph)
{
    ...
    if (ret != SUCCESS) {
        RTS_LOGE("Set mem type range of root graph:%s failed, retCode=%#x.",
    ...
    for (auto &subGraph : graph.GetAllSubgraphs()) {
        ...
        if (ret != SUCCESS) {
            RTS_LOGE("Set mem type range of graph:%s failed, retCode=%#x",
                subGraph->GetName().c_str(), ret);
            return FAILED;
        }
    }
    ...
    if (ret != SUCCESS) {
        RTS_LOGE("Process condition node of root graph:%s failed, retCode=%#x.",
            graph.GetName().c_str(), ret);
        return FAILED;
    }
    for (auto &subGraph : graph.GetAllSubgraphs()) {
        ...
        if (ret != SUCCESS) {
            RTS_LOGE("Process condition node of graph:%s failed, retCode=%#x.",
                subGraph->GetName().c_str(), ret);
            return FAILED;
        }
    }
...
ge::Status RtsGraphOptimizer::ProcConditionNode(ge::ComputeGraph &graph)
{
    ...
    for (auto nodePtr : graph.GetDirectNode()) {
        ...
        if (ret != SUCCESS) {
            RTS_LOGE("Insert memcpy node failed, inputNodeNum [%u], retCode=%u",
                inputNodeNum, ret);
            return FAILED;
        }
    }
    ...
    ge::OpDescPtr &memcpyAsyncOpDesc, uint32_t index)
{
    ...
    if (nexDesc->HasAttr(ATTR_NAME_STREAM_LABEL)) {
        ...
        if (!AttrUtils::GetStr(nexDesc, ATTR_NAME_STREAM_LABEL, nextIterationName)) {
            RTS_REPORT_CALL_ERROR("Get next op[%s] attribute[ATTR_NAME_STREAM_LABEL] failed.",
                nexDesc->GetName().c_str());
            return FAILED;
        }
    ...
    if (status != SUCCESS) {
        RTS_REPORT_CALL_ERROR("Graph remove edge failed, src index[%d], dst index[%d], dst node [%s],"
                                       "retCode=%u.",
    ...
    if (status != SUCCESS) {
        RTS_REPORT_CALL_ERROR("Graph add edge failed, src index[%d], dst index[%d], dst node [%s],"
                                       "retCode=%u.",
            srcOutAnchor->GetIdx(), inDataAnchor->GetIdx(), inDataAnchor->GetOwnerNode()->GetName().c_str(), status);
        return FAILED;
    }
    ...
    for (const NodePtr &inNode : nexNode->GetInControlNodes()) {
        ...
        if (status != SUCCESS) {
            RTS_REPORT_CALL_ERROR("Graph remove edge failed, src index[%d], dst index[%d], dst node [%s],"
                                           "retCode=%u.",
                srcOutAnchor->GetIdx(), inDataAnchor->GetIdx(), inDataAnchor->GetOwnerNode()->GetName().c_str(),
                status);
            return FAILED;
        }
    ...
    RTS_LOGI("Insert memcpyAsync op success, src node: %s, dst node: %s",
        srcOutAnchor->GetOwnerNode()->GetName().c_str(), nexNode->GetName().c_str());

    return SUCCESS;
}

ge::Status RtsGraphOptimizer::InsertMemcpyAsyncNodeAndSetMemType(const ge::NodePtr &nexNode,
    ge::ComputeGraph &graph, uint32_t inputNodeNum)
{
    if (nexNode == nullptr) {
    ...
    for (index = 0; index < inputNodeNum; index++) {
        ...
        if (AttrUtils::GetBool(nexNode->GetOpDesc(), ATTR_NAME_RTS_LABEL_NODE, isLabelNode)) {
            if (!AttrUtils::SetBool(memcpyAsyncOpDesc, ATTR_NAME_RTS_LABEL_NODE, isLabelNode)) {
                RTS_REPORT_CALL_ERROR("Node [%s] set ATTR_NAME_RTS_LABEL_NODE failed",
        ...
        if (status != SUCCESS) {
            RTS_REPORT_CALL_ERROR("Set mem type input range attr failed, node [%s], retCode=%u.",
                nexNode->GetName().c_str(), status);
            return FAILED;
        }
#endif
    }
...
ge::OpDescPtr RtsGraphOptimizer::CreateMemcpyAsyncOpByIndex(const ge::NodePtr &nexNode, uint32_t index)
{
    ...
    try {
        ...
        RTS_REPORT_INNER_ERROR("Make shared OpDesc failed, other exception raised, node name=%s.",
            nodeName.c_str());
        return nullptr;
    }
    ...
    if (ret != GRAPH_SUCCESS) {
        RTS_REPORT_CALL_ERROR("Create memcpyAsync op [%s], add input desc failed, retCode=%u.",
            nodeName.c_str(), ret);
        return nullptr;
    }
    ...
    if (ret != GRAPH_SUCCESS) {
        RTS_REPORT_CALL_ERROR("Create memcpyAsync op[%s], add output desc failed, retCode=%u.",
            nodeName.c_str(), ret);
        return nullptr;
    }
...
ge::Status RtsGraphOptimizer::SetMemTypeRange(ge::ComputeGraph &graph)
{
    ...
    for (const auto& nodePtr : graph.GetDirectNode()) {
        ...
        if (iter != MEM_TYPE_RANGE_NODES.end()) {
            ...
            if (ret != SUCCESS) {
                RTS_REPORT_CALL_ERROR("Set mem type range attr failed, node[%s], retCode=%u.",
