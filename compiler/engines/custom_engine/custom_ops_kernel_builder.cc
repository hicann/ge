...
Status ValidateKernelBinNames(const OpDescPtr &op_desc, const gert::AnnotatedArgsHandler &args_handler) {
  std::map<std::string, std::vector<uint8_t>> kernel_bins;
  const size_t launch_count = args_handler.GetLaunchCount();
  for (size_t i = 0U; i < launch_count; ++i) {
    ...
    auto iter = kernel_bins.find(name);
    if (iter == kernel_bins.end()) {
      kernel_bins.emplace(name, std::move(current));
      continue;
    }
    GE_ASSERT_TRUE(iter->second == current, "Custom op kernel name %s maps to different bins, op_name:%s, op_type:%s",
                   name, op_desc->GetNamePtr(), op_desc->GetTypePtr());
  }
  return SUCCESS;
}
