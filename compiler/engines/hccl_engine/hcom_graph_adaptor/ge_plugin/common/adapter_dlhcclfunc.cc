...
HcclResult HcceCalcOpResOfflineGraphMode(OpParamGraphModePtr opParam, u64 *opMemSize, u32 *streamNum, u32 *taskNum,
  ...
  return DlHcclFunction::get_instance().dlHcclCalcOpResOfflineGraphMode(opParam, opMemSize, streamNum, taskNum,
                                                                        aivCoreNum);
}
...
HcclResult HcceCalcOpResOnlineGraphMode(OpParamGraphModePtr opParam, u64 *opMemSize, u32 *streamNum, u32 *taskNum,
  ...
  return DlHcclFunction::get_instance().dlHcclCalcOpResOnlineGraphMode(opParam, opMemSize, streamNum, taskNum,
                                                                       aivCoreNum);
}
...
HcclResult HcceAllGatherGraphMode(void *sendBuf, void *recvBuf, uint64_t sendCount, HcclDataType dataType,
  ...
  CHK_PRT_RET(DlHcclFunction::get_instance().init() != HCCL_SUCCESS,
              HCCL_ERROR("DlHcclFunction::get_instance().init() fail \n"), HCCL_E_PARA);
  return DlHcclFunction::get_instance().dlHcclAllGatherGraphMode(
      sendBuf, recvBuf, sendCount, dataType, group, stream, tag, streams, streamCount, scratchMemAddr, scratchMemSize);
}
...
HcclResult HcceBroadcastGraphMode(void *buf, uint64_t count, HcclDataType dataType, uint32_t root, const char *group,
  ...
  CHK_PRT_RET(DlHcclFunction::get_instance().init() != HCCL_SUCCESS,
              HCCL_ERROR("DlHcclFunction::get_instance().init() fail \n"), HCCL_E_PARA);
  return DlHcclFunction::get_instance().dlHcclBroadcastGraphMode(buf, count, dataType, root, group, stream, tag,
...
HcclResult HcceReduceScatterVGraphMode(void *sendBuf, const void *sendCounts, const void *sendDispls, void *recvBuf,
  ...
  CHK_PRT_RET(DlHcclFunction::get_instance().init() != HCCL_SUCCESS,
              HCCL_ERROR("DlHcclFunction::get_instance().init() fail \n"), HCCL_E_PARA);
  return DlHcclFunction::get_instance().dlHcclReduceScatterVGraphMode(
      sendBuf, sendCounts, sendDispls, recvBuf, recvCount, dataType, op, group, stream, tag, streams, streamCount,
...
HcclResult HcceAllGatherVGraphMode(void *sendBuf, void *recvBuf, uint64_t sendCount, const void *recvCounts,
  ...
  CHK_PRT_RET(DlHcclFunction::get_instance().init() != HCCL_SUCCESS,
              HCCL_ERROR("DlHcclFunction::get_instance().init() fail \n"), HCCL_E_PARA);
  return DlHcclFunction::get_instance().dlHcclAllGatherVGraphMode(sendBuf, recvBuf, sendCount, recvCounts, recvDispls,
                                                                  dataType, group, stream, tag, streams, streamCount,
                                                                  scratchMemAddr, scratchMemSize);
}
...
HcclResult HcceAlltoAllGraphMode(const void *sendBuf, uint64_t sendCount, HcclDataType sendType, const void *recvBuf,
  ...
  CHK_PRT_RET(DlHcclFunction::get_instance().init() != HCCL_SUCCESS,
              HCCL_ERROR("DlHcclFunction::get_instance().init() fail \n"), HCCL_E_PARA);
  return DlHcclFunction::get_instance().dlHcclAlltoAllGraphMode(sendBuf, sendCount, sendType, recvBuf, recvCount,
                                                                recvType, group, stream, tag, streams, streamCount,
                                                                scratchMemAddr, scratchMemSize);
}
...
HcclResult HcceAlltoAllVGraphMode(const void *sendBuf, const void *sendCounts, const void *sdispls,
  ...
  CHK_PRT_RET(DlHcclFunction::get_instance().init() != HCCL_SUCCESS,
              HCCL_ERROR("DlHcclFunction::get_instance().init() fail \n"), HCCL_E_PARA);
  return DlHcclFunction::get_instance().dlHcclAlltoAllVGraphMode(sendBuf, sendCounts, sdispls, sendType, recvBuf,
                                                                 recvCounts, rdispls, recvType, group, stream, tag,
                                                                 streams, streamCount, scratchMemAddr, scratchMemSize);
}
...
HcclResult HcceSendGraphMode(void *sendBuf, uint64_t count, HcclDataType dataType, uint32_t destRank, const char *group,
  ...
  CHK_PRT_RET(DlHcclFunction::get_instance().init() != HCCL_SUCCESS,
              HCCL_ERROR("DlHcclFunction::get_instance().init() fail \n"), HCCL_E_PARA);
  return DlHcclFunction::get_instance().dlHcclSendGraphMode(sendBuf, count, dataType, destRank, group, stream, tag,
                                                            streams, streamCount, scratchMemAddr, scratchMemSize);
}
...
HcclResult HcceRecvGraphMode(void *recvBuf, uint64_t count, HcclDataType dataType, uint32_t srcRank, const char *group,
  ...
  CHK_PRT_RET(DlHcclFunction::get_instance().init() != HCCL_SUCCESS,
              HCCL_ERROR("DlHcclFunction::get_instance().init() fail \n"), HCCL_E_PARA);
  return DlHcclFunction::get_instance().dlHcclRecvGraphMode(recvBuf, count, dataType, srcRank, group, stream, tag,
                                                            streams, streamCount, scratchMemAddr, scratchMemSize);
}
...
HcclResult HcceAllReduceGraphMode(void *sendBuf, void *recvBuf, uint64_t sendCount, HcclDataType dataType,
  ...
  CHK_PRT_RET(DlHcclFunction::get_instance().init() != HCCL_SUCCESS,
              HCCL_ERROR("DlHcclFunction::get_instance().init() fail \n"), HCCL_E_PARA);
  return DlHcclFunction::get_instance().dlHcclAllReduceGraphMode(sendBuf, recvBuf, sendCount, dataType, op, group,
...
HcclResult HcceReduceGraphMode(void *sendBuf, void *recvBuf, uint64_t count, HcclDataType dataType, HcclReduceOp op,
  ...
  CHK_PRT_RET(DlHcclFunction::get_instance().init() != HCCL_SUCCESS,
              HCCL_ERROR("DlHcclFunction::get_instance().init() fail \n"), HCCL_E_PARA);
  return DlHcclFunction::get_instance().dlHcclReduceGraphMode(sendBuf, recvBuf, count, dataType, op, root, group,
                                                              stream, tag, streams, streamCount, scratchMemAddr,
                                                              scratchMemSize);
}
...
HcclResult HcceReduceScatterGraphMode(void *sendBuf, void *recvBuf, uint64_t recvCount, HcclDataType dataType,
  ...
  CHK_PRT_RET(DlHcclFunction::get_instance().init() != HCCL_SUCCESS,
              HCCL_ERROR("DlHcclFunction::get_instance().init() fail \n"), HCCL_E_PARA);
  return DlHcclFunction::get_instance().dlHcclReduceScatterGraphMode(sendBuf, recvBuf, recvCount, dataType, op, group,
                                                                     stream, tag, streams, streamCount, scratchMemAddr,
                                                                     scratchMemSize);
}
...
HcclResult HcceSelectAlgGraphMode(const char *group, u64 count, HcclDataType dataType, HcclReduceOp op,
  ...
  CHK_PRT_RET(DlHcclFunction::get_instance().init() != HCCL_SUCCESS,
              HCCL_ERROR("DlHcclFunction::get_instance().init() fail \n"), HCCL_E_PARA);
  return DlHcclFunction::get_instance().dlHcclSelectAlgGraphMode(group, count, dataType, op, opType, aivCoreLimit,
                                                                 ifAiv, algName);
}
...
HcclResult HcceGetAlgExecParamGraphMode(const char *tag, const char *group, u64 count, void *inputPtr, void *outputPtr,
  ...
  CHK_PRT_RET(DlHcclFunction::get_instance().init() != HCCL_SUCCESS,
              HCCL_ERROR("DlHcclFunction::get_instance().init() fail \n"), HCCL_E_PARA);
  return DlHcclFunction::get_instance().dlHcclGetAlgExecParamGraphMode(
