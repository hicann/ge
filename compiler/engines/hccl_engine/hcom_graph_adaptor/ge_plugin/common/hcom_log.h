...
#define HCCL_ERROR_LOG_PRINT(format, ...)                                                                            \
  do {                                                                                                               \
    if (IsErrorToWarn()) {                                                                                           \
      LOG_FUNC(HCCL_LOG_MASK, HCCL_LOG_WARN, "[%s:%d] [%u]ErrToWarn: " format, __FILE__, __LINE__,                   \
               syscall(SYS_gettid), ##__VA_ARGS__);                                                                  \
    } else {                                                                                                         \
      LOG_FUNC(HCCL, HCCL_LOG_ERROR, "[%s:%d] [%u]" format, __FILE__, __LINE__, syscall(SYS_gettid), ##__VA_ARGS__); \
    }                                                                                                                \
  } while (0)

#define HCCL_RUN_LOG_PRINT(format, ...)                                                                    \
  do {                                                                                                     \
    LOG_FUNC(HCCL_LOG_MASK, HCCL_LOG_INFO, "[%s:%d] [%u]" format, __FILE__, __LINE__, syscall(SYS_gettid), \
             ##__VA_ARGS__);                                                                               \
  } while (0)
...
#define HCCL_RUN_WARNING(format, ...)                                      \
  do {                                                                     \
...
#define HCCL_USER_CRITICAL_LOG(format, ...)                        \
  do {                                                             \
    if (LIKELY(HcclCheckLogLevel(HCCL_LOG_INFO, HCCL_LOG_MASK))) { \
      HCCL_RUN_LOG_PRINT(format, ##__VA_ARGS__);                   \
    }                                                              \
...
#define CHK_SMART_PTR_NULL(smart_ptr)                                                                                  \
  do {                                                                                                                 \
    if (UNLIKELY(!(smart_ptr))) {                                                                                      \
      HCCL_ERROR("[%s]errNo[0x%016llx] ptr [%s] is nullptr, return HCCL_E_PTR", __func__, HCCL_ERROR_CODE(HCCL_E_PTR), \
                 #smart_ptr);                                                                                          \
      return HCCL_E_PTR;                                                                                               \
    }                                                                                                                  \
  } while (0)
...
#define CHK_SMART_PTR_RET_NULL(smart_ptr)                                                             \
  do {                                                                                                \
    if (UNLIKELY(!(smart_ptr))) {                                                                     \
      HCCL_ERROR("[%s]errNo[0x%016llx]smart_ptr is nullptr.", __func__, HCCL_ERROR_CODE(HCCL_E_PTR)); \
      return;                                                                                         \
    }                                                                                                 \
  } while (0)
...
#ifdef DUMP_DATA
#define PRINT_ARRAY(arr, cnt, name) ArrayToStringAndPrint(arr, cnt, name)
