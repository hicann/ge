...
#define FFTS_DEF_ERRORNO(sysid, modid, name, value, desc)                                                     \
  static constexpr Status name = ((((static_cast<uint32_t>((0xFF) & (static_cast<uint8_t>(sysid)))) << 24) |  \
                                   ((static_cast<uint32_t>((0xFF) & (static_cast<uint8_t>(modid)))) << 16)) | \
                                  ((0xFFFF) & (static_cast<uint16_t>(value))));

#define FFTS_DEF_ERRORNO_COMMON(name, value, desc) FFTS_DEF_ERRORNO(SYSID_FFTS, FFTS_MODID_COMMON, name, value, desc)
...
FFTS_DEF_ERRORNO_ITF(OPINFO_STORES_FINI_FAILED, 2, "Failed to finalize opinfo_kernel_stores!");  // 0x3330002
FFTS_DEF_ERRORNO_ITF(GRAPHOPTIMIZER_FINI_FAILED, 3, "Failed to finalize graphoptimizer!");       // 0x3330003
...
FFTS_DEF_ERRORNO_OP_KERNEL_STORE(OP_STORE_PARSE_FAILED, 0,
                                 "Failed to parse the op kernel store cfg file!");  // 0x3380000
...
FFTS_DEF_ERRORNO_OP_KERNEL_STORE(OP_STORE_READ_CFG_FILE_FAILED, 3,
                                 "Failed to read cfg file. I/O failed!");  // 0x3380003
