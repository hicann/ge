...
ge::Status CpuKernelBuilder::CalcOpRunningParam(const ge::Node &node) const {
  ...
  AICPU_CHECK_FALSE_EXEC(
      ge::AttrUtils::SetListBool(op_desc_ptr, kWorkspaceReuseFlag, {true}),
      AICPU_REPORT_INNER_ERR_MSG("Set workspace memory reuse flag failed, op[%s].", node_name.c_str());
...
ge::Status CpuKernelBuilder::BuildMemCopyInfo(const ge::OpDescPtr &op_desc_ptr,
                                              [[maybe_unused]] const ge::RunContext &run_context,
  ...
  AicpuParamHead param_head = {
      .length = param_len, .ioAddrNum = io_addrs_num, .extInfoLength = ext_info_length, .extInfoAddr = ext_info_addrs};
  ...
  AICPUE_LOGI(
      ...
      task_args.length(), param_len, kDefaultKernelSo.c_str(), kDefaultFunctionName.c_str());
  return ge::SUCCESS;
}

ge::Status CpuKernelBuilder::GenerateMemCopyTask(uint64_t data_info_size, const ge::RunContext &run_context,
  ...
  ge::NodePtr node = aicpu::GenGeNode(node_name, node_type, in_num, out_num, format, data_type, shape);
  AICPU_CHECK_NOTNULL(node);
  ge::OpDescPtr op_desc_ptr = node->GetOpDesc();
  AICPU_CHECK_NOTNULL(op_desc_ptr)
  AICPU_CHECK_FALSE_EXEC(
      ge::AttrUtils::SetInt(op_desc_ptr, ge::ATTR_NAME_UNKNOWN_SHAPE_TYPE, ge::DEPEND_IN_SHAPE),
  ...
  AICPU_CHECK_FALSE_EXEC(
      ...
      AICPU_REPORT_INNER_ERR_MSG("Call ge::AttrUtils::SetInt failed to set attr[num], op[%s].", node_name.c_str());
      return ErrorCode::ADD_ATTR_FAILED)
  ...
  AICPU_CHECK_RES_WITH_LOG(BuildMemCopyInfo(op_desc_ptr, run_context, kernel_def),
                           "Call BuildMemCopyInfo function failed op[%s].", node->GetName().c_str())
  AICPUE_LOGI("CPUKernel's op [%s][%s] run GenerateMemCopyTask success.", node->GetName().c_str(),
              node->GetType().c_str());
  tasks.emplace_back(task_def);
  return ge::SUCCESS;
}

ge::Status GetAicpuFftsPlusKerneLType(const ge::OpDescPtr op_desc_ptr, uint32_t &kernel_type) {
  ...
  if (*kernel_lib_name == kCustAicpuKernelInfoChoice) {
    ...
    AICPU_REPORT_INNER_ERR_MSG("Get kernel type failed, node[%s], kernel type[%s].", op_desc_ptr->GetName().c_str(),
...
ge::Status GetAicpuKernelType(const ge::OpDescPtr op_desc_ptr, uint32_t &kernel_type) {
  const std::string *kernel_lib_name = ge::AttrUtils::GetStr(op_desc_ptr, "opKernelLib");
  AICPU_CHECK_NOTNULL(kernel_lib_name);
  if (*kernel_lib_name == kCustAicpuKernelInfoChoice) {
...
ge::Status CpuKernelBuilder::GenerateTask(const ge::Node &node, const ge::RunContext &run_context,
                                          std::vector<domi::TaskDef> &tasks) {
  AICPUE_LOGI("CPUKernel's op %s[%s] run GenerateTask.", node.GetName().c_str(), node.GetType().c_str());
  ge::OpDescPtr op_desc_ptr = node.GetOpDesc();
  AICPU_CHECK_NOTNULL_ERRCODE(op_desc_ptr, INPUT_PARAM_NULL)
  if (op_desc_ptr->HasAttr(kAttrNameThreadScopeId)) {
    AICPUE_LOGI("op %s[%s] run GenerateFftsPlusTask.", node.GetName().c_str(), node.GetType().c_str());
    AICPU_CHECK_RES_WITH_LOG(GenerateFftsPlusTask(node), "Call GenerateFftsPlusTask function failed, op[%s].",
                             node.GetName().c_str())
    return ge::SUCCESS;
  }
  ...
  domi::KernelDef *kernel_def = task_def.mutable_kernel();
  AICPU_CHECK_NOTNULL(kernel_def)
  AICPU_CHECK_RES_WITH_LOG(BuildAndLaunchKernel(node, kernel_def), "Call BuildAndLaunchKernel function failed, op[%s].",
                           node.GetName().c_str())
  ...
  if (ge::AttrUtils::HasAttr(op_desc_ptr, kAttrNameUnknownShape)) {
    ...
    // unknow shape
    AICPU_CHECK_FALSE_EXEC(
        ge::AttrUtils::GetInt(op_desc_ptr, ge::ATTR_NAME_UNKNOWN_SHAPE_TYPE, shape_type),
        AICPU_REPORT_INNER_ERR_MSG("Call AttrUtils::GetInt failed to get attr[%s], op[%s].",
                                   ge::ATTR_NAME_UNKNOWN_SHAPE_TYPE.c_str(), op_desc_ptr->GetName().c_str());
        return ErrorCode::ADD_ATTR_FAILED)
    if (shape_type == ge::DEPEND_COMPUTE) {
      uint64_t data_info_size = static_cast<uint64_t>(op_desc_ptr->GetOutputsSize()) * kDataAndShapeNum;
      AICPU_CHECK_RES_WITH_LOG(GenerateMemCopyTask(data_info_size, run_context, tasks),
                               "GenerateMemCopyTask op [%s][%s] failed.", node.GetName().c_str(),
                               node.GetType().c_str());
      AICPUE_LOGI("GenerateMemCopyTask op [%s][%s] task.size [%zu].", node.GetName().c_str(), node.GetType().c_str(),
                  tasks.size());
    }
  }
  return ge::SUCCESS;
...
ge::Status CpuKernelBuilder::UpdateTask(const ge::Node &node, std::vector<domi::TaskDef> &tasks) {
  std::shared_ptr<ge::OpDesc> op_desc_ptr = node.GetOpDesc();
  AICPU_CHECK_NOTNULL_ERRCODE(op_desc_ptr, ErrorCode::INPUT_PARAM_NULL)
...
ge::Status CpuKernelBuilder::MakeAicpuKernelExtInfo(const ge::OpDescPtr &op_desc_ptr, vector<char> &task_ext_info,
...
ge::Status BuildArgs(const ge::Node &node, std::string &task_args, uint32_t &args_len, const uint32_t index,
  ...
  bool has_customized_attr = ge::AttrUtils::GetZeroCopyBytes(op_desc_ptr, k_name, bytes);
  // When it's aicpu customized ops, get customized attr
  ...
  for (uint32_t i = 0; i < input_size; i++) {
    ...
    AICPUE_LOGI("Op[%s], input[%zu] offset[%lu], auto_thread_flag[%d], input_addr_offset_size[%zu]",
                op_desc_ptr->GetName().c_str(), i, offset, static_cast<int32_t>(auto_thread_flag),
                ffts_info.input_addr_offset.size());
...
ge::Status CpuKernelBuilder::BuildAndLaunchKernel(const ge::Node &node, domi::KernelDef *&kernel_def) const {
  std::vector<char> task_ext_info;
  ge::OpDescPtr op_desc_ptr = node.GetOpDesc();
  FftsPlusInfo ffts_info;
  ffts_info.valid = false;
  AICPU_CHECK_RES_WITH_LOG(MakeAicpuKernelExtInfo(op_desc_ptr, task_ext_info, ffts_info),
  ...
  AICPU_CHECK_RES_WITH_LOG(BuildArgs(node, task_args, args_len, 0, ffts_info), "Call BuildArgs function failed, op[%s]",
                           op_desc_ptr->GetName().c_str())
  // set kernel blockdim
  ...
  AICPUE_LOGI(
      ...
      task_args.length(), args_len, kernel_so_name.c_str(), func_name.c_str(), kernel_type);
  return ge::SUCCESS;
}

ge::Status CpuKernelBuilder::BuildFftsInfo(const ge::OpDescPtr &op_desc_ptr, FftsPlusInfo &ffts_info) const {
...
ge::Status CpuKernelBuilder::BuildAiCpuCtx(const ge::OpDescPtr &op_desc_ptr, const FftsPlusInfo &ffts_info,
  ...
  AICPUE_LOGD("mode[%d] [%s][%s] block_dim is[%u], block_tail_dim is[%u].", ffts_info.auto_static_split,
              op_desc_ptr->GetName().c_str(), op_desc_ptr->GetType().c_str(), block_dim, block_tail_dim);
...
ge::Status CpuKernelBuilder::GenerateFftsPlusTask(const ge::Node &node) const {
  ...
  AICPUE_LOGI("op[%s][%s] run GenerateFftsPlusTask state[%u]", node.GetType().c_str(), node.GetName().c_str(), state);
...
uint32_t CpuKernelBuilder::GetOpBlockDimForFftsPlus(const ge::OpDescPtr &op_desc_ptr, const FftsPlusInfo &ffts_info,
  ...
  int64_t total = 0;
  if (block_dim_index == -1) {
    total = std::accumulate(input_0.begin(), input_0.end(), 1, [](int64_t i, int64_t j) -> int64_t { return i * j; });
  } else {
...
uint32_t CpuKernelBuilder::CalcBlockDimByShapeSize(const int64_t total) const {
  ...
  AICPUE_LOGD("GetOpBlockDim total[%ld], ai_cpu_cnt[%ld], blockSize[%ld], blockDim[%u]", total, ai_cpu_cnt, block_size,
              block_dim);
  return block_dim;
}
}  // namespace aicpu
