...
BaseEnginePtr HostCpuEngine::Instance() {
  ...
  std::call_once(flag, []() { instance_.reset(new (std::nothrow) HostCpuEngine); });
  return instance_;
}

ge::Status HostCpuEngine::Initialize([[maybe_unused]] const std::map<std::string, std::string> &options) {
  if (ops_kernel_info_store_ == nullptr) {
    ops_kernel_info_store_ = std::make_shared<AicpuOpsKernelInfoStore>(kHostCpuEngine);
    if (ops_kernel_info_store_ == nullptr) {
...
void HostCpuEngine::GetOpsKernelInfoStores(std::map<std::string, OpsKernelInfoStorePtr> &ops_kernel_info_stores) const {
  if (ops_kernel_info_store_ != nullptr) {
    ops_kernel_info_stores[kHostCpuOpsKernelInfo] = ops_kernel_info_store_;
  }
}

void HostCpuEngine::GetGraphOptimizerObjs(std::map<std::string, GraphOptimizerPtr> &graph_optimizers) const {
  if (graph_optimizer_ != nullptr) {
    graph_optimizers[kHostCpuGraphOptimizer] = graph_optimizer_;
  }
}

void HostCpuEngine::GetOpsKernelBuilderObjs(std::map<std::string, OpsKernelBuilderPtr> &ops_kernel_builders) const {
  if (ops_kernel_builder_ != nullptr) {
...
void GetOpsKernelInfoStores(std::map<std::string, OpsKernelInfoStorePtr> &ops_kernel_info_stores) {
  aicpu::HostCpuEngine::Instance()->GetOpsKernelInfoStores(ops_kernel_info_stores);
}
