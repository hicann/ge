...
Status KernelBuilder::GetWorkspaceInfo(const OpDescPtr &op_desc_ptr, const uint8_t *data_mem_base,
  ...
  AICPUE_LOGI("The workspace_bytes vector's size is [%lu], op[%s].", workspace_bytes.size(),
              op_desc_ptr->GetName().c_str());
  CHECK_EQUAL(workspace_bytes.size(), 1, INPUT_PARAM_VALID, "workspace bytes vector's size[%zu] should be 1, op[%s].",
              workspace_bytes.size(), op_desc_ptr->GetName().c_str())
  CHECK_RES_BOOL((workspace_bytes[0] >= 0), INPUT_PARAM_VALID,
                 AICPU_REPORT_INNER_ERR_MSG("Invalid workspace bytes[%ld], op[%s]", workspace_bytes[0],
                                            op_desc_ptr->GetName().c_str()))
...
ge::Status KernelBuilder::MakeExtInfoForOpName(const ge::OpDescPtr &op_desc_ptr,
  ...
  char *op_name_buf = reinterpret_cast<char *>(reinterpret_cast<uintptr_t>(ext_info_buf) + ext_info_offset);
  errno_t cpy_ret = memcpy_s(op_name_buf, ext_info->infoLen, op_name.c_str(), op_name.length());
  if (cpy_ret != EOK) {
    AICPUE_LOGW("Generate extend info for op[%s] failed, ext info len[%u], ret[%d].", op_name.c_str(),
                ext_info->infoLen, cpy_ret);
  }
  AICPUE_LOGI("Make ext_info for op[%s], ext info len[%u].", op_name.c_str(), ext_info->infoLen);
  return SUCCESS;
}

void KernelBuilder::GetInOutPutsShape(const ge::OpDescPtr &op_desc_ptr, std::vector<std::vector<int64_t>> &inputs_shape,
...
void KernelBuilder::CalcBaseExtInfoLen(const ge::OpDescPtr &op_desc_ptr, const bool op_async_flag,
...
Status KernelBuilder::GetFftsPlusInAddrOffset(const ge::OpDescPtr &op_desc_ptr, FftsPlusInfo &ffts_info) const {
  const uint32_t index = ffts_info.slice_instance_index;
  if (index >= ffts_info.thread_input_shape.size()) {
    AICPU_REPORT_INNER_ERR_MSG("Node[%s] index[%u], thread_input_shape size[%zu]", op_desc_ptr->GetName().c_str(),
                               index, ffts_info.thread_input_shape.size());
  ...
  if (input_size != ffts_info.thread_input_shape[index].size()) {
    AICPU_REPORT_INNER_ERR_MSG(
        ...
        op_desc_ptr->GetName().c_str(), index, ffts_info.thread_input_shape[index].size(), input_size);
    return ge::GE_GRAPH_INIT_FAILED;
  }
  for (size_t i = 0; i < input_size; i++) {
    auto input_desc = op_desc_ptr->GetInputDesc(i);
...
Status KernelBuilder::GetFftsPlusOutAddrOffset(const ge::OpDescPtr &op_desc_ptr, FftsPlusInfo &ffts_info) const {
  ...
  if (output_size != ffts_info.thread_output_shape[index].size()) {
    AICPU_REPORT_INNER_ERR_MSG(
        ...
        op_desc_ptr->GetName().c_str(), index, ffts_info.thread_output_shape[index].size(), output_size);
    return ge::GE_GRAPH_INIT_FAILED;
  }
  for (size_t i = 0; i < output_size; i++) {
    auto output_desc = op_desc_ptr->GetOutputDesc(i);
...
Status KernelBuilder::GetFftsPlusInOutAddrOffset(const ge::OpDescPtr &op_desc_ptr, FftsPlusInfo &ffts_info) const {
...
Status KernelBuilder::MakeBaseExtInfo(const ge::OpDescPtr &op_desc_ptr, std::vector<char> &task_ext_info,
                                      const FftsPlusInfo &ffts_info) const {
  // set attr _all_shape
  CHECK_RES_BOOL(AttrUtils::SetBool(op_desc_ptr, kAllShape, true), INVOKE_GRAPH_ITF_FAILED,
  ...
  if (!AttrUtils::GetInt(op_desc_ptr, ATTR_NAME_UNKNOWN_SHAPE_TYPE, unknow_shape_type)) {
  ...
  *bit_map = 0;
  ...
  if (GetThreadLocalContext().GetOption("ge.exec.exclude_engines", engine_exclude_aicore) == SUCCESS &&
    ...
    uint32_t task_priority_value =
        (kTopicTypeQosPriorityLowest << kTopicTypeQosPriorityPostion) & kTopicTypeQosPriorityMask;
    type_value |= task_priority_value;
  }

  const std::string *kernel_lib_name = ge::AttrUtils::GetStr(op_desc_ptr, "opKernelLib");
  if ((kernel_lib_name != nullptr) && (*kernel_lib_name == kHostCpuKernelInfoChoice)) {
  ...
  *topic_type = type_value;
  ...
  for (size_t index = 0; index < inputs_shape.size(); ++index) {
    ...
    if (dim_length > FWKAdapter::kMaxShapeDims) {
      AICPU_REPORT_INNER_ERR_MSG("op[%s] set Input ExtInfo Shape failed because dimLen [%zu] > [%u]",
                                 op_desc_ptr->GetName().c_str(), dim_length, FWKAdapter::kMaxShapeDims);
      return INVOKE_GRAPH_ITF_FAILED;
    }
  ...
  if (unknow_shape_type != ge::DEPEND_COMPUTE) {
    for (size_t index = 0; index < outputs_shape.size(); ++index) {
      ...
      if (dim_length > FWKAdapter::kMaxShapeDims) {
        AICPU_REPORT_INNER_ERR_MSG("op[%s] set Output ExtInfo Shape failed because dimLen[%zu] > [%u]",
                                   op_desc_ptr->GetName().c_str(), dim_length, FWKAdapter::kMaxShapeDims);
        return INVOKE_GRAPH_ITF_FAILED;
      }
      for (size_t i = 0; i < dim_length; ++i) {
  ...
  AICPUE_LOGI(
      ...
      op_desc_ptr->GetName().c_str(), op_desc_ptr->GetType().c_str(), input_num, output_num, extend_info_len);
...
Status KernelBuilder::SetAttrQueueResource(const std::string &node_name, std::shared_ptr<ge::OpDesc> &op_desc_ptr,
  ...
  AICPU_CHECK_FALSE_EXEC(
      ...
      AICPU_REPORT_INNER_ERR_MSG("Set attr[%s] failed, op[%s]", kResourceType.c_str(), node_name.c_str());
      return ErrorCode::ADD_ATTR_FAILED)

  const std::string *queue_name = ge::AttrUtils::GetStr(op_desc_ptr, kQueueName);
  AICPU_CHECK_FALSE_EXEC((queue_name != nullptr), AICPU_REPORT_INNER_ERR_MSG("Get attr[%s] failed, op[%s]",
                                                                             kQueueName.c_str(), node_name.c_str());
                         return ErrorCode::GET_ATTR_FAILED)
  AICPU_CHECK_FALSE_EXEC(
      ge::AttrUtils::SetStr(resource_queue, kQueueName, *queue_name),
      AICPU_REPORT_INNER_ERR_MSG("Set attr[%s] failed, op[%s]", kQueueName.c_str(), node_name.c_str());
      return ErrorCode::ADD_ATTR_FAILED)
  ...
  AICPU_CHECK_FALSE_EXEC(
      ...
      AICPU_REPORT_INNER_ERR_MSG("Set attr[%s] failed, op[%s]", kQueueDepth.c_str(), node_name.c_str());
      return ErrorCode::ADD_ATTR_FAILED)
  ...
  AICPU_CHECK_FALSE_EXEC(
      ...
      AICPU_REPORT_INNER_ERR_MSG("Set attr[%s] failed, op[%s]", kQueueIdIdx.c_str(), node_name.c_str());
      return ErrorCode::ADD_ATTR_FAILED)
  resource_list.push_back(resource_queue);
  return ge::SUCCESS;
}

Status KernelBuilder::SetAttrResource(const std::string &node_name, std::shared_ptr<ge::OpDesc> &op_desc_ptr) const {
  const std::string *resourceValue = ge::AttrUtils::GetStr(op_desc_ptr, kResource);
  AICPU_CHECK_FALSE_EXEC((resourceValue == nullptr),
                         AICPUE_LOGI("Op[%s] get attr resource is %s", node_name.c_str(), resourceValue->c_str()))
  ...
  for (std::string const &resource : resouceList) {
    if (resource == kResourceQueue) {
      AICPU_CHECK_RES_WITH_LOG(SetAttrQueueResource(node_name, op_desc_ptr, resource_list),
                               "Call SetAttrQueueResource function failed, op[%s].", node_name.c_str());
    } else if (resource == kResourceChannel) {
      ge::GeAttrValue::NAMED_ATTRS resource_channel;
      AICPU_CHECK_FALSE_EXEC(
          ge::AttrUtils::SetStr(resource_channel, kResourceType, kResourceChannel),
          AICPU_REPORT_INNER_ERR_MSG("Set attr[%s] failed, op[%s]", kResourceType.c_str(), node_name.c_str());
          return ErrorCode::ADD_ATTR_FAILED)
      ...
      ge::GeAttrValue::NAMED_ATTRS resource_channel;
      AICPU_CHECK_FALSE_EXEC(
          ge::AttrUtils::SetStr(resource_channel, kResourceType, kResourceVdecChannel),
          AICPU_REPORT_INNER_ERR_MSG("Set attr[%s] failed, op[%s]", kResourceType.c_str(), node_name.c_str());
          return ErrorCode::ADD_ATTR_FAILED)
      resource_list.push_back(resource_channel);
    }
  ...
  if (!resource_list.empty()) {
    AICPUE_LOGI("Op[%s] resource list size is %zu", node_name.c_str(), resource_list.size());
    AICPU_CHECK_FALSE_EXEC(
        ge::AttrUtils::SetListNamedAttrs(*op_desc_ptr, kResourceList, resource_list),
        AICPU_REPORT_INNER_ERR_MSG("Set attr[%s] failed, op[%s]", kResourceList.c_str(), node_name.c_str());
        return ErrorCode::ADD_ATTR_FAILED)
  }
  return ge::SUCCESS;
}
