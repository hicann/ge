...
ge::Status GraphOptimizerUtils::VerifyPldAndEndNode(const ComputeGraph &graph) {
  for (const NodePtr &curr_node : graph.GetDirectNode()) {
    ...
    if (op_type == kPlaceHolderOpType) {
      ...
      bool verify_succ = ((pld_format == pld_original_format) || (pld_format == ge::FORMAT_ND));
      CHECK_RES_BOOL(verify_succ, FAILED,
                     AICPU_REPORT_INNER_ERR_MSG("Invalid format[%s], should be [FORMAT_ND] or same as original"
      ...
      GeTensorDesc end_out_tensor_desc = curr_op_desc_ptr->GetOutputDesc(only_one_output);
      ge::Format end_format = end_out_tensor_desc.GetFormat();
      ge::Format end_original_format = end_out_tensor_desc.GetOriginFormat();
      // verify_succ = 1 if verify succ
      bool verify_succ = ((end_format == end_original_format) || (end_format == ge::FORMAT_ND));
      CHECK_RES_BOOL(verify_succ, FAILED,
                     AICPU_REPORT_INNER_ERR_MSG("Invalid format[%s], should be [FORMAT_ND] or same as original"
                                                " format[%s]. op[%s], op type[%s]",
...
ge::Status CacheGraph::CreateAndInsertCacheUpdate(const OutDataAnchorPtr &src_anchor, const InDataAnchorPtr &dst_anchor,
  ...
  CHECK_RES_BOOL(src_anchor_idx >= 0, FAILED,
                 AICPU_REPORT_INNER_ERR_MSG("Invalid output anchor index[%d] of op[%s]", src_anchor_idx,
...
Status CacheGraph::GenerateNoCacheGraph(ComputeGraph &graph) {
  for (const NodePtr &curr_node : graph.GetDirectNode()) {
    ...
    if (op_type == kPlaceHolderOpType) {
      const string *pld_front_node_engine =
          AttrUtils::GetStr(curr_op_desc_ptr, ge::ATTR_NAME_PLD_FRONT_NODE_ENGINE_NAME);
      CHECK_RES_BOOL((pld_front_node_engine != nullptr), ErrorCode::GET_ATTR_FAILED,
      ...
      // if front name engine is aicore engine or vector engine, insert cache
      // update op
      if ((*pld_front_node_engine == "AIcoreEngine") || (*pld_front_node_engine == "VectorEngine")) {
    ...
    if (op_type == kEndOpType) {
      ...
      CHECK_RES_BOOL((end_rear_node_engine != nullptr), ErrorCode::GET_ATTR_FAILED,
                     AICPU_REPORT_INNER_ERR_MSG("Call ge::AttrUtils::GetStr failed to get attr[%s], op[%s].",
      ...
      CHECK_RES_BOOL((parent_op_type != nullptr), ErrorCode::GET_ATTR_FAILED,
                     AICPU_REPORT_INNER_ERR_MSG("Call ge::AttrUtils::GetStr failed to get attr[parentOpType], op[%s].",
                                                curr_op_desc_ptr->GetName().c_str()));

      // if rear name engine is aicore engine or parent op is End, insert cache
      // update op
      if (*end_rear_node_engine == "AIcoreEngine" || *parent_op_type == "NetOutput") {
        ...
        AICPU_CHECK_NOTNULL(dst_anchor)
        AICPU_CHECK_NOTNULL(nodes_and_anchors.at(0).first)
        AICPU_CHECK_RES_WITH_LOG(CreateAndInsertCacheUpdate(src_anchor, dst_anchor, graph, nullptr, false),
                                 "Call CreateAndInsertCacheUpdate failed to insert CacheUpdate op"
                                 " between op[%s] and op[%s].",
                                 nodes_and_anchors.at(0).first->GetName().c_str(), curr_node->GetName().c_str())
      }
    }
  }
  return SUCCESS;
  ...
  for (ge::NodePtr &node : graph.GetDirectNode()) {
    ...
    for (size_t i = 0; i < input_num; ++i) {
      ...
      if (iter1 != src_to_dst_type.end()) {
        ...
        AICPU_CHECK_NOTNULL(dst_anchor)
        AICPU_CHECK_RES_WITH_LOG(InsertCastForInput(dst_anchor, graph, dst_type, slice_info_ptr, sgt_flag),
    ...
    for (size_t i = 0; i < output_num; ++i) {
      ...
      if (iter2 != dst_to_src_type.end()) {
        ...
        AICPU_CHECK_NOTNULL(src_anchor)
        AICPU_CHECK_RES_WITH_LOG(InsertCastForOutput(src_anchor, graph, src_type, output_type),
...
ge::Status AutoCastGraph::InsertCastForInput(const ge::InDataAnchorPtr &dst_anchor, ge::ComputeGraph &graph,
  ...
  CHECK_RES_BOOL(!(cast_op.IsEmpty()), FAILED,
  ...
  AICPU_CHECK_RES_WITH_LOG(cast_op_desc->UpdateInputDesc(0, src_tensor_desc), "Cast update input 0 desc failed.")
  ...
  AICPU_CHECK_RES_WITH_LOG(GraphUtils::RemoveEdge(src_anchor, dst_anchor),
                           "Remove edge between op[%s] and op[%s] failed.", src_op->GetName().c_str(),
                           dst_op->GetName().c_str())
  InDataAnchorPtr input_anchor = cast_node->GetInDataAnchor(0);
  AICPU_CHECK_NOTNULL(input_anchor)
  OutDataAnchorPtr output_anchor = cast_node->GetOutDataAnchor(0);
  AICPU_CHECK_NOTNULL(output_anchor)
  AICPU_CHECK_RES_WITH_LOG(GraphUtils::AddEdge(src_anchor, input_anchor), "Add edge between op[%s] and Cast failed.",
                           src_op->GetName().c_str())
  AICPU_CHECK_RES_WITH_LOG(GraphUtils::AddEdge(output_anchor, dst_anchor), "Add edge between Cast and op[%s] failed.",
                           dst_op->GetName().c_str())
...
ge::Status AutoCastGraph::InsertCastForOutput(const ge::OutDataAnchorPtr &src_anchor, ge::ComputeGraph &graph,
  ...
  AICPU_CHECK_RES_WITH_LOG(src_op->UpdateOutputDesc(static_cast<uint32_t>(src_anchor_idx), src_tensor_desc),
  ...
  AICPU_CHECK_RES_WITH_LOG(cast_op_desc->UpdateOutputDesc(0, src_tensor_desc), "Cast update output 0 desc failed.")
  // insert Cast op
  NodePtr cast_node = graph.AddNode(cast_op_desc);
  AICPU_CHECK_NOTNULL(cast_node)
  InDataAnchorPtr input_anchor = cast_node->GetInDataAnchor(0);
  ...
  for (auto dst_anchor : src_anchor->GetPeerInDataAnchors()) {
    ...
    AICPU_CHECK_RES_WITH_LOG(GraphUtils::AddEdge(output_anchor, dst_anchor),
                             "Add edge between op[%s] and op[%s] failed.", cast_node->GetName().c_str(),
                             dst_anchor->GetOwnerNode()->GetName().c_str())
  }
  AICPU_CHECK_RES_WITH_LOG(GraphUtils::AddEdge(src_anchor, input_anchor), "Add edge between op[%s] and Cast failed.",
                           src_op->GetName().c_str())
