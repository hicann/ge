...
ge::Status TfFunctionBuilder::BuildFunctionDef(ge::ComputeGraphPtr &graph, const std::string &func_name,
                                               FunctionDefLibrary *func_lib, NodeDef *fused_node_def,
  ...
  *function->mutable_attr() = fused_node_def->attr();
  AICPU_CHECK_FALSE_EXEC(TensorFlowUtil::AddNodeAttr("function", value, fused_node_def),
                         AICPU_REPORT_INNER_ERR_MSG("Add attr[function] failed, op[%s].", name.c_str());
  ...
  if (!arg_data_types.empty()) {
    ...
    AICPU_CHECK_FALSE_EXEC(TensorFlowUtil::AddNodeAttr("Tin", t_in_value, fused_node_def),
                           AICPU_REPORT_INNER_ERR_MSG("Add attr[Tin] failed, op[%s] .", name.c_str());
                           return ErrorCode::ADD_ATTR_FAILED)
  }
  ...
  if (!res_data_types.empty()) {
    ...
    AICPU_CHECK_FALSE_EXEC(TensorFlowUtil::AddNodeAttr("Tout", t_out_value, fused_node_def),
                           AICPU_REPORT_INNER_ERR_MSG("Add attr[Tout] failed, op[%s].", name.c_str());
                           return ErrorCode::ADD_ATTR_FAILED)
  }
...
void TfFunctionBuilder::SetInputForNodeDef(NodeDef *node_def, std::vector<ge::InDataAnchorPtr> &in_anchors) const {
  for (const ge::InDataAnchorPtr &anchor : in_anchors) {
    ...
    if (peer_out_anchor != nullptr) {
      ...
      input_str.append(peer_out_anchor->GetOwnerNode()->GetName())
          .append("_")
...
ge::Status TfFunctionBuilder::CreateInputNodes(ge::ComputeGraphPtr &graph,
  ...
  for (const ge::InDataAnchorPtr &anchor : in_anchors) {
    // generate op name
    std::string op_name;
    op_name.append(anchor->GetPeerOutAnchor()->GetOwnerNode()->GetName())
        .append("_")
        .append(Stringcat(anchor->GetPeerOutAnchor()->GetIdx()))
        .append("_index")
        .append(Stringcat(index))
        .append("_arg");

    // create op desc
    ge::OpDescPtr op_desc = nullptr;
    AICPU_MAKE_SHARED(op_desc = std::make_shared<ge::OpDesc>(op_name, kArgOp),
                      AICPU_REPORT_INNER_ERR_MSG("Create ge::OpDesc object failed, op[%s].", op_name.c_str());
    ...
    AICPU_IF_BOOL_EXEC(tf_data_type == TFDataType::DT_INVALID,
                       AICPU_REPORT_INNER_ERR_MSG("Ge data type[%d] not supported, op[%s]", data_type, op_name.c_str());
                       return ErrorCode::DATA_TYPE_UNDEFILED)

    // add default output desc to op
    AICPU_CHECK_FALSE_EXEC(
        op_desc->AddOutputDesc(ge::GeTensorDesc()) == ge::GRAPH_SUCCESS,
        AICPU_REPORT_INNER_ERR_MSG("Call ge::OpDesc::AddOutputDesc failed, op[%s].", op_name.c_str());
        return ErrorCode::ADD_TF_DATADESC_FAILED)
    // set T and arg_index attribute to op
    AICPU_CHECK_FALSE_EXEC(
        ge::AttrUtils::SetInt(op_desc, "T", static_cast<int32_t>(tf_data_type)),
        AICPU_REPORT_INNER_ERR_MSG("Call ge::AttrUtils::SetInt failed to set attr[T], op[%s].", op_name.c_str());
        return ErrorCode::ADD_ATTR_FAILED)
    AICPU_CHECK_FALSE_EXEC(ge::AttrUtils::SetInt(op_desc, "arg_index", static_cast<int32_t>(index)),
                           AICPU_REPORT_INNER_ERR_MSG(
                               "Call ge::AttrUtils::SetInt failed to set attr[arg_index], op[%s].", op_name.c_str());
                           return ErrorCode::ADD_ATTR_FAILED)
    ...
    for (const ge::NodePtr &node : graph->GetAllNodes()) {
      if (node->GetName() == anchor->GetOwnerNode()->GetName()) {
        ...
        AICPU_CHECK_FALSE_EXEC(
            ge::GraphUtils::AddEdge(new_peer_out_anchor, in_anchor) == ge::GRAPH_SUCCESS,
            AICPU_REPORT_INNER_ERR_MSG("Call ge::GraphUtils::AddEdge failed, op[%s], peer op[%s].",
                                       node->GetName().c_str(), new_peer_out_anchor->GetOwnerNode()->GetName().c_str());
...
ge::Status TfFunctionBuilder::CreateOutputNodes(ge::ComputeGraphPtr &graph,
  ...
  for (const ge::OutDataAnchorPtr &anchor : out_anchors) {
    // generate op desc
    std::string op_name;
    op_name.append(anchor->GetOwnerNode()->GetName()).append("_").append(Stringcat(anchor->GetIdx())).append("_retval");
    ...
    AICPU_CHECK_FALSE_EXEC(op_desc->AddInputDesc(ge::GeTensorDesc()) == ge::GRAPH_SUCCESS,
                           AICPU_REPORT_INNER_ERR_MSG("Call ge::OpDesc::AddInputDesc failed, op[%s].", op_name.c_str());
                           return ErrorCode::ADD_TF_DATADESC_FAILED)
    AICPU_CHECK_FALSE_EXEC(
        op_desc->AddOutputDesc(ge::GeTensorDesc()) == ge::GRAPH_SUCCESS,
        AICPU_REPORT_INNER_ERR_MSG("Call ge::OpDesc::AddOutputDesc failed, op[%s].", op_name.c_str());
    ...
    AICPU_CHECK_FALSE_EXEC(ge::AttrUtils::SetInt(op_desc, "retval_index", static_cast<int32_t>(index)),
                           AICPU_REPORT_INNER_ERR_MSG(
                               "Call ge::AttrUtils::SetInt failed to set attr[retval_index], op[%s].", op_name.c_str());
                           return ErrorCode::ADD_ATTR_FAILED)
...
ge::Status TfFunctionBuilder::TransGraphToFunctionDef(ge::ComputeGraphPtr &graph, const std::string &func_name,
  ...
  for (const ge::NodePtr &node : graph->GetAllNodes()) {
    ...
    if (node->GetOpDesc()->GetType() == kArgOp) {
      ...
      int64_t type = 1;
      AICPU_CHECK_FALSE_EXEC(
          ge::AttrUtils::GetInt(node->GetOpDesc(), "T", type),
      ...
      AICPU_CHECK_FALSE_EXEC(
          ge::AttrUtils::GetInt(node->GetOpDesc(), "arg_index", index),
    ...
    *node_def = original_node_def;
    ...
    for (const ge::InDataAnchorPtr &anchor : in_data_anchors) {
      if (anchor->GetPeerOutAnchor() != nullptr) {
        ...
        tensor_name.append(anchor->GetPeerOutAnchor()->GetOwnerNode()->GetName())
            .append(":")
            .append(Stringcat(anchor->GetPeerOutAnchor()->GetIdx()));
  ...
  AICPU_CHECK_RES_WITH_LOG(RemapFunctionDef(func_def, func_name, tensor_renaming_map, return_value_map),
                           "Call TfFunctionBuilder::RemapFunctionDef function failed, tf function[%s].",
                           func_name.c_str())
  return ge::SUCCESS;
}
...
ge::Status TfFunctionBuilder::RemapFunctionDef(FunctionDef *func_def, const std::string &func_name,
                                               std::unordered_map<std::string, std::string> &tensor_renaming_map,
    ...
    for (int i = 0; i < node_def->input_size(); ++i) {
      if (node_def->input(i).find("^") != std::string::npos) {
        ...
        if (input_ctrl_name.empty()) {
          AICPU_REPORT_INNER_ERR_MSG("Could not remap control input[%s] of node[%s] in function[%s].",
        ...
        *node_def->mutable_input(i) = "^" + input_ctrl_name;
        ...
        if (iter == tensor_renaming_map.end()) {
          AICPU_REPORT_INNER_ERR_MSG("Could not remap input[%s] of node[%s] in function[%s].",
                                     node_def->input(i).c_str(), node_def->name().c_str(), func_name.c_str());
          return ErrorCode::REMAP_FUNCTIONDEF_FAILED;
        }
  ...
  for (int index = 0; index < func_def->signature().output_arg_size(); ++index) {
    ...
    if (ret_name.empty()) {
      AICPU_REPORT_INNER_ERR_MSG("Missing output[%d] to function[%s].", index, func_name.c_str());
      return ErrorCode::REMAP_FUNCTIONDEF_FAILED;
    }
    const std::string &return_value = return_value_map[ret_name];
    if (return_value.empty()) {
      AICPU_REPORT_INNER_ERR_MSG("Could not remap return value[%d], [%s] of [%s] in function[%s].", index,
                                 ret_name.c_str(), return_value.c_str(), func_name.c_str());
      return ErrorCode::REMAP_FUNCTIONDEF_FAILED;
    }

    const auto iter = tensor_renaming_map.find(return_value);
    if (iter == tensor_renaming_map.end()) {
      AICPU_REPORT_INNER_ERR_MSG("Could not find value[%s] in tensorRenaming map, function[%s].", return_value.c_str(),
                                 func_name.c_str());
      return ErrorCode::REMAP_FUNCTIONDEF_FAILED;
    }
...
ge::Status TfFunctionBuilder::GetTfNodeDef(const ge::NodePtr &node, NodeDef &tf_node_def, bool &from_ir_mapping) const {
  ...
  if (ge::AttrUtils::GetBytes(node->GetOpDesc(), kTfNodeDef, node_def_bytes)) {
    AICPU_IF_BOOL_EXEC(node_def_bytes.GetSize() == 0,
                       AICPU_REPORT_INNER_ERR_MSG("Size of [%s] is out of range.", node->GetName().c_str());
                       return ErrorCode::PARSE_NODE_DEF_FAILED)
    AICPU_CHECK_FALSE_EXEC(tf_node_def.ParseFromArray(node_def_bytes.GetData(), node_def_bytes.GetSize()),
                           AICPU_REPORT_INNER_ERR_MSG("Parse node def failed, op[%s], op type[%s].", node_name.c_str(),
                                                      node->GetType().c_str());
                           return ErrorCode::PARSE_NODE_DEF_FAILED)
    return ge::SUCCESS;
  }

  AICPUE_LOGI("Tf node def attr does not exist in op[%s], op type[%s].", node_name.c_str(), node->GetType().c_str());
  // IR->TF
  ...
  if (parser == nullptr) {
    AICPU_REPORT_INNER_ERR_MSG("Create ir parser failed, op[%s], op type[%s].", node_name.c_str(),
...
ge::Status TfFunctionBuilder::GetTfOutputNameRangeMap(const ge::NodePtr &node, NodeDef &tf_node_def,
  ...
  if ((op_def_str != nullptr) && (!node_def_from_ir_mapping)) {
    ...
    if (!op_def.ParseFromString(*op_def_str)) {
      AICPU_REPORT_INNER_ERR_MSG("Parse op def from string failed. op[%s], op type[%s].", node_name.c_str(),
                                 node->GetType().c_str());
      return ErrorCode::PARSE_NODE_DEF_FAILED;
    }
  ...
  AICPUE_LOGI("Tf op def attr does not exist in op[%s], op type[%s].", node_name.c_str(), node->GetType().c_str());
  ...
  if (parser == nullptr) {
    AICPU_REPORT_INNER_ERR_MSG("Create ir parser failed. op[%s].", node_name.c_str());
    return ErrorCode::GET_IR2TF_PARSER_FAILED;
  }
...
ge::Status TfFunctionBuilder::NameRangesForFunc(const ge::NodePtr &node, NodeDef &tf_node_def,
  ...
  AICPU_CHECK_FALSE_EXEC(ge::AttrUtils::GetBytes(node->GetOpDesc(), kTfFuncDef, func_def_bytes),
                         AICPUE_LOGI("function def attr does not exist in ge op[%s], op type[%s].", node_name.c_str(),
                                     node->GetType().c_str());
