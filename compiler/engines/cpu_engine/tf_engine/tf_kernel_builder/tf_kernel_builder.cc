...
ge::Status TfKernelBuilder::BuildFFtsKernelRunParam(const ge::OpDesc &op_desc,
                                                    FWKAdapter::KernelRunParam &kernel_run_param, const uint32_t &index,
  ...
  for (size_t i = 0; i < input_size; i++) {
    ...
    state = SetTensorDataInfo(ge_tensor_desc, input_tensor, ffts_info.thread_input_shape[index][i], is_ref);
  ...
  for (size_t i = 0; i < output_size; i++) {
    ...
    state = SetTensorDataInfo(ge_tensor_desc, output_tensor, ffts_info.thread_output_shape[index][i], false);
    if (state.state != ge::SUCCESS) {
...
aicpu::State TfKernelBuilder::SetTensorDataInfo(const ge::GeTensorDesc &ge_tensor_desc,
                                                ...
                                                const std::vector<int64_t> &tensor_shape, bool is_ref,
  ...
  if (!skip_dim_check) {
    for (uint32_t i = 0; i < dim_shape; i++) {
      ...
      bool is_invalid_dim = ((dim < 0) && (dim != ge::UNKNOWN_DIM) && (dim != ge::UNKNOWN_DIM_NUM));
      if (is_invalid_dim) {
        std::string err_msg = Stringcat("dim[", i, "] is invalid, shape is [", dim, "].");
...
ge::Status TfKernelBuilder::GenerateTask(const ge::Node &node, const ge::RunContext &run_context,
  ...
  AICPU_CHECK_NOTNULL_ERRCODE(op_desc_ptr, ErrorCode::INPUT_PARAM_NULL)
  if (ge::AttrUtils::HasAttr(op_desc_ptr, kAttrNameThreadScopeId)) {
    AICPUE_LOGI("TFKernel's op[%s], op type[%s] run GenerateFftsPlusTask. ", node.GetName().c_str(),
                node.GetType().c_str());
  ...
  for (uint32_t index = 0; index < ffts_info.slice_instance_num; index++) {
    ...
    if (index == 0) {
      uint32_t param_offset =
          static_cast<uint32_t>(task_info.length()) + static_cast<uint32_t>(sizeof(STR_FWK_OP_KERNEL));
    ...
    temp_args.append(reinterpret_cast<const char *>(&str_fwkop_kernel), sizeof(STR_FWK_OP_KERNEL));
...
ge::Status TfKernelBuilder::BuildAicpuFftsPlusKernel(const ge::Node &node, STR_FWK_OP_KERNEL &str_fwkop_kernel,
                                                     const ge::RunContext &run_context, std::string &task_info,
  ...
  if (!ffts_info.is_unknown_shape) {
    ...
    uint64_t min_memory = str_tf_kernel->inputOutputLen + str_tf_kernel->nodeDefLen + str_tf_kernel->funDefLibLen;
  ...
  AICPUE_LOGI("BuildAicpuFftsPlusKernel success, kernel id[%lu], op[%s], op type[%s]", str_tf_kernel->kernelID,
...
ge::Status TfKernelBuilder::ConstructTfKernel(const ge::Node &node, const ge::RunContext &run_context,
  ...
  AICPUE_LOGI("Op type[%s] The kernel id is [%lu], session id is [%lu].", node.GetType().c_str(),
              str_tf_kernel->kernelID, str_tf_kernel->sessionID);
  ...
  g_task_info.append(reinterpret_cast<const char *>(kernel_run_param_buffer.GetData()),
                     kernel_run_param_buffer.GetSize());
...
ge::Status TfKernelBuilder::ConstructExtendInfoAndTaskdef(const ge::Node &node, const ge::RunContext &run_context,
  ...
  bool is_unknown_shape = false;
  if (ge::AttrUtils::HasAttr(op_desc_ptr, kAttrNameUnknownShape)) {
    // kAttrNameUnknownShape attr exist, means unknown shape
    is_unknown_shape = true;
  }
  if (!is_unknown_shape) {
...
ge::Status TfKernelBuilder::BuildAndLaunchKernel(const ge::Node &node, const ge::RunContext &run_context) const {
  ...
  AICPU_MAKE_SHARED(
      g_str_fwkop_kernel_ptr = std::make_shared<STR_FWK_OP_KERNEL>(),
  ...
  AICPUE_LOGI("BuildAndLaunchKernel success, kernel id[%lu], op[%s], op type[%s].", str_tf_kernel->kernelID,
              node.GetName().c_str(), node.GetType().c_str());
  return ge::SUCCESS;
}
...
ge::Status TfKernelBuilder::MakeTaskExtInfo(const ge::Node &node, std::vector<char> &task_ext_info,
                                            const FftsPlusInfo &ffts_info) const {
  ge::OpDescPtr op_desc_ptr = node.GetOpDesc();
  AICPU_CHECK_NOTNULL_ERRCODE(op_desc_ptr, ErrorCode::INPUT_PARAM_NULL)
  // op name extend info
  ...
  if (status != ge::SUCCESS) {
    AICPU_REPORT_INNER_ERR_MSG("Call MakeExtInfoForOpName failed, op[%s].", op_desc_ptr->GetName().c_str());
    return status;
  }
  ...
  if (status != ge::SUCCESS) {
    AICPU_REPORT_INNER_ERR_MSG("Call MakeBaseExtInfo failed, op[%s], op type[%s].", op_desc_ptr->GetName().c_str(),
...
bool TfKernelBuilder::NeedUpdateAddr(const ge::Node &node, int32_t &update_addr_flag) const {
  if (IsKnownNodeDynamic(node)) {
    ...
    AICPUE_LOGI("The known shape node in dynamic shape graph, op[%s], op type[%s].", node.GetName().c_str(),
                node.GetType().c_str());
    return true;
  }
...
bool TfKernelBuilder::IsKnownNodeDynamic(const ge::Node &node) const {
  ...
  if (owner_graph == nullptr) {
    AICPUE_LOGW("Get null owner compute graph, op[%s], op type[%s].", node.GetName().c_str(), node.GetType().c_str());
    return false;
  }
  auto rootGraph = ge::GraphUtils::FindRootGraph(owner_graph);
  if (rootGraph == nullptr) {
    AICPUE_LOGW("Get null root graph, op[%s], op type[%s].", node.GetName().c_str(), node.GetType().c_str());
    return false;
  }
...
ge::Status TfKernelBuilder::GenTaskImply(const ge::NodePtr &node, FWKAdapter::FWKOperateParam *str_tf_kernel,
                                         std::string &task_info, bool skip_dim_check) const {
...
ge::Status TfKernelBuilder::GenMemCopyTask(uint64_t count, STR_FWK_OP_KERNEL &task, std::string &task_info) {
  task.fwkKernelType = FMK_KERNEL_TYPE_TF;
  FWKAdapter::FWKOperateParam *str_tf_kernel = &(task.fwkKernelBase.fwk_kernel);
  // Build Ge node
  ...
  AICPU_CHECK_FALSE_EXEC(
      ge::AttrUtils::SetInt(op_desc, "num", count),
...
void TfKernelBuilder::GetInOutPutsDataType(const ge::OpDescPtr &op_desc_ptr, std::vector<uint32_t> &inputs_type,
