...
Status NetOutputPass::ClearStatus() {
  ...
  targets_.clear();
  is_user_define_ouput_nodes = false;
  return SUCCESS;
}
...
Status NetOutputPass::GetRetvalOutputInfo(const ge::NodePtr &node,
  ...
  if (!AttrUtils::GetInt(node->GetOpDesc(), RETVAL_ATTR_NAME_INDEX, output_index)) {
    REPORT_INNER_ERR_MSG("E19999", "Get Attr:%s from op:%s(%s) failed", RETVAL_ATTR_NAME_INDEX.c_str(),
  ...
  if (retval_node_index_map.count(output_index) > 0) {
    REPORT_INNER_ERR_MSG("E19999", "Attr:%s from op:%s(%s), value:%ld duplicate with other node, check invalid",
...
Status NetOutputPass::CheckOutputNodeInfo(const ComputeGraphPtr &graph, const std::vector<RetvalInfo> &outputs) const {
  for (auto &item : outputs) {
    NodePtr node = item.output_node;
    if (node == nullptr) {
      REPORT_INNER_ERR_MSG("E19999", "Param outputs has item which output_node is nullptr, check invalid");
      GELOGE(PARAM_INVALID, "[Check][Param] Node in outputs is nullptr.");
      return PARAM_INVALID;
    } else {
      if (graph->FindNode(node->GetName()) == nullptr) {
        REPORT_INNER_ERR_MSG("E19999", "Find node:%s from graph:%s failed",
      ...
      if (index < 0 || index >= out_size) {
        REPORT_INNER_ERR_MSG("E19999", "Index:%d in param outputs item, < 0 or > output size:%d of node:%s(%s)",
...
Status NetOutputPass::UpdateNetOutputDesc(const ge::NodePtr &net_output) const {
  OpDescPtr net_output_desc = net_output->GetOpDesc();
  if (net_output_desc == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "OpDesc in Param net_output is nullptr, check invalid");
  ...
  for (const auto &in_anchor : net_output->GetAllInDataAnchors()) {
    ...
    if (index >= net_output_desc->GetAllInputsSize()) {
      REPORT_INNER_ERR_MSG("E19999", "Node:%s(%s) has in_anchor index:%u >= its input desc num:%zu, check invalid",
...
Status NetOutputPass::AddCtrlEdgeForTargets(const ge::NodePtr &net_out_node) {
  if (net_out_node == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Param net_out_node is nullptr, check invalid");
...
Status NetOutputPass::AddEdgesForNetOutput(const ge::ComputeGraphPtr &graph, const ge::NodePtr &net_out_node,
  ...
  for (auto &item : output_nodes_info) {
    ...
    if (status != GRAPH_SUCCESS) {
      REPORT_INNER_ERR_MSG("E19999", "Add edge between op:%s(%s)(index:%u) and op:%s(%s)(index:%d) failed",
    ...
    if (item.parent_node_index >= 0) {
      ...
      if (input_desc == nullptr) {
        REPORT_INNER_ERR_MSG("E19999", "Node:%s(%s) has no input desc index is %d, check invalid",
                          net_out_node->GetName().c_str(), net_out_node->GetType().c_str(), net_input_index);
      ...
      if (!AttrUtils::SetInt(input_desc, ATTR_NAME_PARENT_NODE_INDEX, item.parent_node_index)) {
        REPORT_INNER_ERR_MSG("E19999", "Set Attr:%s to input:%d tensor of op:%s(%s) failed",
...
bool NetOutputPass::CheckNodeIsInOutputNodes(const ge::ComputeGraphPtr &graph, const ge::NodePtr &node) const {
  for (auto &ele : graph->GetGraphOutNodesInfo()) {
...
Status NetOutputPass::UnLinkControlAnchorOfNetoutput(const ge::ComputeGraphPtr &graph,
  ...
  if (in_control_anchor == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "In control anchor of param net_out_node:%s(%s) is nullptr, check invalid",
                       net_out_node->GetName().c_str(), net_out_node->GetType().c_str());
    ...
    if (peer_out_data_anchor == nullptr) {
      ...
      if (iter != targets_.end()) {
        if (!(CheckNodeIsInOutputNodes(graph, node))) {
          ...
          if (ret != SUCCESS) {
            REPORT_INNER_ERR_MSG("E19999", "Op:%s(%s) unlink control edge from op:%s(%s) failed",
                              net_out_node->GetName().c_str(), net_out_node->GetType().c_str(),
                              node->GetName().c_str(), node->GetType().c_str());
...
Status NetOutputPass::CreateNetOutputNode(OpDescPtr &net_output_desc, const ge::ComputeGraphPtr &graph) const {
...
Status NetOutputPass::Run(ge::ComputeGraphPtr graph) {
  ...
  if (output_node != nullptr) {
    ...
    // Add true stream, netoutput is 0
    GE_CHK_BOOL_RET_STATUS(ge::AttrUtils::SetInt(output_node->GetOpDesc(), ATTR_NAME_TRUE_BRANCH_STREAM, 0),
                           FAILED, "Failed to set attr[%s] to op:%s(%s).",
                           ATTR_NAME_TRUE_BRANCH_STREAM.c_str(), output_node->GetName().c_str(),
...
void NetOutputPass::AddInOutForNetOutputOp(const ComputeGraphPtr &graph, const OpDescPtr &net_output_desc,
                                           std::vector<RetvalInfo> &output_nodes_info) {
  (void)graph;
  std::vector<bool> is_input_const;
  for (auto iter = output_nodes_info.begin(); iter != output_nodes_info.end();) {
    ...
    if (src_node == nullptr || src_node->GetOpDesc() == nullptr || net_output_desc == nullptr) {
      REPORT_INNER_ERR_MSG("E19999", "Param output_nodes_info has RetvalInfo item, which src_node is invalid; "
