...
Status SubgraphPass::Run(ComputeGraphPtr graph) {
  ...
  for (const NodePtr &node : graph->GetDirectNode()) {
    ...
    if ((kIfOpTypes.count(node->GetType()) > 0) || (kCaseOpTypes.count(node->GetType()) > 0)) {
      ...
      GE_ASSERT_SUCCESS(SeparateOutputsAddrOfCondSubgraph(node));
    }
  }
...
Status SubgraphPass::SeparateOutputsAddrOfCondSubgraph(const NodePtr &node) const {
  GE_ASSERT_NOTNULL(node->GetOpDesc());
  for (const auto &subgraph_name_2_index : node->GetOpDesc()->GetSubgraphNameIndexes()) {
    const auto subgraph = NodeUtils::GetSubgraph(*node, subgraph_name_2_index.second);
    GE_ASSERT_NOTNULL(subgraph);
    const auto netoutput = subgraph->FindFirstNodeMatchType(NETOUTPUT);
    GE_ASSERT_NOTNULL(netoutput);
    std::unordered_set<NodePtr> output_nodes;
    for (const auto &in_anchor : netoutput->GetAllInDataAnchors()) {
      ...
      if (!output_nodes.emplace(peer_out_node).second) {
        std::string name = peer_out_node->GetName() + "_output_" + std::to_string(in_anchor->GetIdx()) + "_Memcpy";
        GELOGI(
            "Node %s as input of netoutput shares one symbol with other node. Insert identity %s to separate on"
            " addr.",
            peer_out_node->GetName().c_str(), name.c_str());
        GE_ASSERT_SUCCESS(InsertMemcpyNode(peer_out_anchor, {in_anchor}, name));
      }
    }
  }
  return SUCCESS;
 ...
 */
Status SubgraphPass::SubgraphInputNode(const ComputeGraphPtr &graph, const NodePtr &node) const {
  ...
  GE_ASSERT_SUCCESS(NodeUtils::GetInNodeCrossPartionedCallNode(node, 0, in_node));
  if (in_node == nullptr) {
    return SUCCESS;
  }
  GELOGD("Node %s in node is %s.", node->GetName().c_str(), in_node->GetName().c_str());
 ...
 * @return: 0 for SUCCESS / others for FAILED
 */
Status SubgraphPass::SubgraphOutputNode(const ComputeGraphPtr &graph, const NodePtr &node) const {
  for (InDataAnchorPtr &in_data_anchor : node->GetAllInDataAnchors()) {
    ...
    bool insert_flag =
        ...
        IsAtomicRequired(in_node, peer_out_anchor->GetIdx()) ||
 ...
 * @return: 0 for SUCCESS / others for FAILED
 */
Status SubgraphPass::WhileInputNodes(const NodePtr &node) const {
  for (InDataAnchorPtr &in_data_anchor : node->GetAllInDataAnchors()) {
    const OutDataAnchorPtr &peer_out_anchor = in_data_anchor->GetPeerOutAnchor();
    GE_IF_BOOL_EXEC(peer_out_anchor == nullptr, continue);
    NodePtr in_node = peer_out_anchor->GetOwnerNode();
 ...
 * @return: 0 for SUCCESS / others for FAILED
 */
Status SubgraphPass::WhileBodySubgraph(const ComputeGraphPtr &graph, const NodePtr &node) const {
  (void)graph;
 ...
 * @return: 0 for SUCCESS / others for FAILED
 */
Status SubgraphPass::InsertInputMemcpy(const ComputeGraphPtr &graph, const std::vector<NodePtr> &data_nodes) const {
  if (data_nodes.empty()) {
 ...
 */
bool SubgraphPass::CheckInsertInputMemcpy(const NodePtr &node, std::set<uint32_t> &bypass_index) const {
 ...
 */
bool SubgraphPass::IsAtomicRequired(const NodePtr &node, int64_t out_index) const {
...
bool SubgraphPass::IsOutputContinuesRequired(const NodePtr &node, int32_t out_index) const {
  ...
  return MemLayoutConflictUtil::TraverseRefChainReverse(node.get(), out_index, judge_func);
}
...
bool SubgraphPass::IsInputContinuesRequired(const NodePtr &node, int32_t in_index) const {
  ...
  return MemLayoutConflictUtil::TraverseRefChain(node.get(), in_index, judge_func);
}
...
Status SubgraphPass::InsertMemcpyNode(const OutDataAnchorPtr &out_anchor,
  ...
  if ((AttrUtils::GetStr(in_node->GetOpDesc(), ATTR_NAME_BATCH_LABEL, batch_label)) && (!batch_label.empty())) {
...
Status SubgraphPass::InsertNodeBetween(const OutDataAnchorPtr &src, const std::vector<InDataAnchorPtr> &dsts,
  ...
  for (const auto &dst : dsts) {
    ...
    GE_CHK_BOOL_RET_STATUS((GraphUtils::RemoveEdge(src, dst) == GRAPH_SUCCESS) &&
                               (GraphUtils::AddEdge(insert_node->GetOutDataAnchor(output_index), dst) == GRAPH_SUCCESS),
                           FAILED,
                           "Remove edge between op:%s(%s)(index:%d) and op:%s(%s)(index:%d) or "
                           "Add edge between op:%s(%s)(index:%u) and op:%s(%s)(index:%d) failed",
                           src->GetOwnerNode()->GetName().c_str(), src->GetOwnerNode()->GetType().c_str(),
                           src->GetIdx(), dst->GetOwnerNode()->GetName().c_str(),
                           dst->GetOwnerNode()->GetType().c_str(), dst->GetIdx(), insert_node->GetName().c_str(),
                           insert_node->GetType().c_str(), output_index, dst->GetOwnerNode()->GetName().c_str(),
                           dst->GetOwnerNode()->GetType().c_str(), dst->GetIdx());
  }
  return SUCCESS;
}

REG_PASS_OPTION("SubgraphPass").LEVELS(OoLevel::kO0);
}  // namespace ge
