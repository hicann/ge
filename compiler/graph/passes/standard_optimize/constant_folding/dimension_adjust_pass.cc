...
Status DimensionAdjustPass::Run(ge::NodePtr &node) {
  ...
  if (ret != SUCCESS) {
    ...
    REPORT_INNER_ERR_MSG("E19999", "kernel compute for op:%s(%s) failed", node->GetName().c_str(),
                         node->GetType().c_str());
  ...
  if (node->GetInDataNodes().size() > static_cast<size_t>(kRemoveInputIndex)) {
    ...
    GE_CHK_STATUS_RET(DeleteUselessConstAxisNode(axis_node), "Remove const axis input of node:%s failed",
                      node->GetName().c_str());
  }
