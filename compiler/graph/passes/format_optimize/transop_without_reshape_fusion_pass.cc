...
const int32_t kInvalidFusionOpCount = -1;
...
void TransOpWithoutReshapeFusionPass::SetRemainNode(
    const std::vector<std::pair<OutDataAnchorPtr, InDataAnchorPtr>> &nodes_anchor) {
  ...
  while (iter != nodes_anchor.end()) {
    ...
    GE_IF_BOOL_EXEC(!op_desc->SetExtAttr(kRemainNode, true),
                    REPORT_INNER_ERR_MSG("E19999", "Set Attr:%s to op:%s(%s) failed", kRemainNode,
    ...
    if (out_ctrl_anchor != nullptr) {
      remain_out_control_anchors_.emplace(out_ctrl_anchor);
      for (const auto &peer_in_ctrl_anchor : out_ctrl_anchor->GetPeerInControlAnchors()) {
        remain_in_control_anchors_.emplace(peer_in_ctrl_anchor);
      }
    }
  }
}

bool TransOpWithoutReshapeFusionPass::IsFormatContinuous(const OutDataAnchorPtr &out_anchor,
  ...
  GE_IF_BOOL_EXEC(in_node == nullptr,
                  REPORT_INNER_ERR_MSG("E19999", "Param in_anchor's owner node is nullptr, check invalid");
                  GELOGE(INTERNAL_ERROR, "[Check][Param]Param in_anchor's owner node is nullptr"); return false);
  auto in_op = in_node->GetOpDesc();
  auto out_owner_node = out_anchor->GetOwnerNode();
  GE_IF_BOOL_EXEC(out_owner_node == nullptr,
                  REPORT_INNER_ERR_MSG("E19999", "Param out_anchor's owner node is nullptr, check invalid");
                  GELOGE(INTERNAL_ERROR, "[Check][Param] Param out_anchor's owner node is nullptr"); return false);
  auto out_op = out_owner_node->GetOpDesc();
  GE_IF_BOOL_EXEC(in_op == nullptr,
                  REPORT_INNER_ERR_MSG("E19999", "Param in_anchor's owner op_desc is nullptr, check invalid");
                  GELOGE(INTERNAL_ERROR, "[Check][Param] Param in_anchor's owner op_desc is nullptr"); return false);
  GE_IF_BOOL_EXEC(out_op == nullptr,
                  REPORT_INNER_ERR_MSG("E19999", "Param out_anchor's owner op_desc is nullptr, check invalid");
                  GELOGE(INTERNAL_ERROR, "[Check][Param] Param out_anchor's owner op_desc is nullptr"); return false);
  ...
  GE_IF_BOOL_EXEC(in_op_desc == nullptr,
                  REPORT_INNER_ERR_MSG("E19999", "Param in_anchor corresponding tensor is nullptr, check invalid");
                  GELOGE(INTERNAL_ERROR, "[Check][Param] Param in_anchor corresponding tensor is nullptr");
                  return false);
  GE_IF_BOOL_EXEC(out_op_desc == nullptr,
                  REPORT_INNER_ERR_MSG("E19999", "Param out_anchor corresponding tensor is nullptr, check invalid");
  ...
  if (!ShapeEqualCheck(in_op_desc->GetShape(), out_op_desc->GetShape())) {
    GELOGD("Node %s(%s) output shape %s, node %s(%s) input shape %s, not continuous.", out_op->GetNamePtr(),
           out_op->GetTypePtr(), out_op_desc->GetShape().ToString().c_str(), in_op->GetNamePtr(), in_op->GetTypePtr(),
           in_op_desc->GetShape().ToString().c_str());
    return false;
  }

  if (in_op_desc->GetFormat() != out_op_desc->GetFormat()) {
    GELOGD("Node %s(%s) output format %s, node %s(%s) input format %s, not continuous.", out_op->GetNamePtr(),
           out_op->GetTypePtr(), TypeUtils::FormatToSerialString(out_op_desc->GetFormat()).c_str(), in_op->GetNamePtr(),
           in_op->GetTypePtr(), TypeUtils::FormatToSerialString(in_op_desc->GetFormat()).c_str());
    return false;
  }
...
//        ｜
//       CAST （FP16 -> FP32）
// 修改背景：AxpyV2算子的原始输出dtype是BF16，在OpJudge阶段storage
// shape被刷成FP32，被该pass认为两个node的输入输出type一致，将cast节点删除
...
bool TransOpWithoutReshapeFusionPass::IsTransOpDataTypeContinuous(const OutDataAnchorPtr &out_anchor,
  ...
  GELOGD(
      "[Check][DataType] Trans op fusion is skipped, node:%s(%s), input idx:%d, input datatype:%s, "
      "prev node:%s(%s), output idx:%d, output datatype:%s.",
      in_op_desc->GetNamePtr(), in_op_desc->GetTypePtr(), in_anchor->GetIdx(),
      TypeUtils::DataTypeToSerialString(input_desc->GetDataType()).c_str(), out_op_desc->GetNamePtr(),
      out_op_desc->GetTypePtr(), out_anchor->GetIdx(),
...
bool TransOpWithoutReshapeFusionPass::HasPrecisionLoss(const OutDataAnchorPtr &out_anchor,
  ...
  if (in_node->GetType() == CAST && TransOpUtil::IsPrecisionLoss(in_node)) {
    return true;
  }
  if (out_node->GetType() == CAST && TransOpUtil::IsPrecisionLoss(out_node)) {
    return true;
  }
  return false;
}
...
graphStatus TransOpWithoutReshapeFusionPass::NeedRemainNode(const OutDataAnchorPtr &out_anchor,
                                                            const InDataAnchorPtr &in_anchor, bool &need_remain) const {
  need_remain = false;
...
void TransOpWithoutReshapeFusionPass::GetOutDataPeerInControlAnchors(
    const size_t index, std::vector<std::vector<InControlAnchorPtr>> &out_data_peer_in_control_anchors) {
...
void TransOpWithoutReshapeFusionPass::GetInControlPeerOutControlAnchors(
    const size_t index, std::vector<std::vector<OutControlAnchorPtr>> &in_control_peer_out_control_anchors) {
  // The caller guarantees that the index is legal.
...
void TransOpWithoutReshapeFusionPass::GetOutControlPeerAnchors(
    const size_t index, std::vector<std::vector<InControlAnchorPtr>> &out_control_peer_in_control_anchors,
    std::vector<std::vector<InDataAnchorPtr>> &out_control_peer_in_data_anchors) {
      ...
      if (iter == sub_graph_nodes_[index].end()) {
        /*
         ...
         trans1 ---->opD
           |          |
         trans2<-----opE
         临时方案：A-B-C-D若此链路上A所有控制输出节点，都被融合后链路的尾部算子控制，有控制边下移成环的风险。
         转换算子一般为框架成图后插入，且insertbefore的场景的输出控制才重要。
         因此认为链路头部的输出控制不重要，为避免成环，不处理头部转换算子的控制边
         正式方案：控制边链接时需要遍历图，判断有成环风险后放弃融合。
          */
        if (j > 1U) {
...
void TransOpWithoutReshapeFusionPass::GetControlAnchors() {
  std::vector<std::vector<OutControlAnchorPtr>> in_control_peer_out_control_anchors(sub_graph_nodes_.size());
  std::vector<std::vector<InControlAnchorPtr>> out_control_peer_in_control_anchors(sub_graph_nodes_.size());
  std::vector<std::vector<InDataAnchorPtr>> out_control_peer_in_data_anchors(sub_graph_nodes_.size());
  std::vector<std::vector<InControlAnchorPtr>> out_data_peer_in_control_anchors(sub_graph_nodes_.size());
  std::vector<bool> sub_graph_has_control_edge(sub_graph_nodes_.size(), false);
  sub_graph_has_control_edge_.swap(sub_graph_has_control_edge);
  std::vector<bool> sub_graph_has_out_data_peer_in_control_edge(sub_graph_nodes_.size(), false);
...
void TransOpWithoutReshapeFusionPass::UpdateOutputName(const OutDataAnchorPtr &out_anchor,
  ...
  const auto old_peer_in_name_iter = output_names.find(old_peer_in_name);
...
void TransOpWithoutReshapeFusionPass::UpdateInputName(const OutDataAnchorPtr &old_peer_out_anchor,
  ...
  const auto old_peer_out_name_iter = input_names.find(old_peer_out_name);
  ...
  if (sub_graph_has_control_edge_[index]) {
    ...
    if (GraphUtils::AddEdge(out_owner_node->GetOutControlAnchor(), in_owner_node->GetInControlAnchor()) !=
      ...
      GELOGE(GRAPH_FAILED, "[Add][ControlEdge] between op:%s(%s) and op:%s(%s) failed",
             out_owner_node->GetName().c_str(), out_owner_node->GetType().c_str(), in_owner_node->GetName().c_str(),
             in_owner_node->GetType().c_str());
      return GRAPH_FAILED;
    }
  }

  if (sub_graph_has_out_data_peer_in_control_edge_[index]) {
    GELOGI("add out data 2 in control edge.src:%s, dst:%s", out_owner_node->GetName().c_str(),
...
graphStatus TransOpWithoutReshapeFusionPass::RelinkControlEdgesWhenDescNotChanged(
  ...
  for (const auto &peer_out_anchor : in_control_peer_out_control_anchors_[index]) {
    ...
    if (remain_out_control_anchors_.count(peer_out_anchor) > 0) {
      continue;
    }
    ...
    if (GraphUtils::AddEdge(peer_out_anchor, in_owner_node->GetInControlAnchor()) != GRAPH_SUCCESS) {
      REPORT_INNER_ERR_MSG("E19999", "Add control edge between op:%s(%s) and op:%s(%s) failed",
                           ...
                           peer_out_anchor->GetOwnerNode()->GetType().c_str(), in_owner_node->GetName().c_str(),
                           in_owner_node->GetType().c_str());
      GELOGE(GRAPH_FAILED, "[Add]ControlEdge] between op:%s(%s) and op:%s(%s) failed",
...
OpDescPtr TransOpWithoutReshapeFusionPass::GetFormatTransferOp(const GeTensorDesc &format_trans_input_desc,
  ...
  OpDescPtr format_transfer_op = TransOpCreator::CreateTransDataOp(format_transfer_op_name.str(),
                                                                   format_trans_input_desc, format_trans_output_desc);
  if (format_transfer_op == nullptr) {
    GELOGW("[New][OpDesc] transdata failed");
    return nullptr;
  }
  ...
  GE_IF_BOOL_EXEC(!AttrUtils::SetInt(format_transfer_op, ATTR_NAME_INPUT_FORMAT,
                  ...
                  REPORT_INNER_ERR_MSG("E19999", "Set Attr:%s to op:%s(%s) failed", ATTR_NAME_INPUT_FORMAT.c_str(),
  ...
  GE_IF_BOOL_EXEC(!AttrUtils::SetInt(format_transfer_op, ATTR_NAME_OUTPUT_FORMAT,
                  ...
                  REPORT_INNER_ERR_MSG("E19999", "Set Attr:%s to op:%s(%s) failed", ATTR_NAME_OUTPUT_FORMAT.c_str(),
                                       format_transfer_op->GetName().c_str(), format_transfer_op->GetType().c_str());
...
OpDescPtr TransOpWithoutReshapeFusionPass::GetCastOp(const GeTensorDesc &cast_input_desc,
                                                     const GeTensorDesc &cast_output_desc) const {
  ...
  if (cast_op == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Create operator:%s(%s) failed", cast_op_name.str().c_str(), CAST);
  ...
  if (cast_op->GetInputsSize() == 0) {
    GE_IF_BOOL_EXEC(cast_op->AddInputDesc(cast_input_desc) != GRAPH_SUCCESS,
                    REPORT_INNER_ERR_MSG("E19999", "Add input desc to op:%s(%s) failed", cast_op->GetName().c_str(),
                                         cast_op->GetType().c_str());
                    GELOGE(INTERNAL_ERROR, "[Add][InputDesc] to op:%s(%s) failed", cast_op->GetName().c_str(),
                           cast_op->GetType().c_str());
                    return nullptr);
  } else {
    GE_IF_BOOL_EXEC(cast_op->UpdateInputDesc(default_input_index, cast_input_desc) != GRAPH_SUCCESS,
                    REPORT_INNER_ERR_MSG("E19999", "Update input:%d desc of op:%s(%s) failed", default_input_index,
  ...
  if (cast_op->GetOutputsSize() == 0) {
    GE_IF_BOOL_EXEC(cast_op->AddOutputDesc(cast_output_desc) != GRAPH_SUCCESS,
                    REPORT_INNER_ERR_MSG("E19999", "Add output desc to op:%s(%s) failed", cast_op->GetName().c_str(),
                                         cast_op->GetType().c_str());
                    GELOGE(INTERNAL_ERROR, "[Add][OutputDesc] to op:%s(%s) failed", cast_op->GetName().c_str(),
                           cast_op->GetType().c_str());
                    return nullptr);
  } else {
    GE_IF_BOOL_EXEC(cast_op->UpdateOutputDesc(default_output_index, cast_output_desc) != GRAPH_SUCCESS,
                    REPORT_INNER_ERR_MSG("E19999", "Update output:%d desc of op:%s(%s) failed", default_output_index,
                                         cast_op->GetName().c_str(), cast_op->GetType().c_str());
  ...
  if (!AttrUtils::SetInt(cast_op, CAST_ATTR_DST_TYPE, static_cast<int64_t>(cast_output_desc.GetDataType()))) {
    REPORT_INNER_ERR_MSG("E19999", "Set Attr:%s to op:%s(%s) failed", CAST_ATTR_DST_TYPE.c_str(),
                         cast_op->GetName().c_str(), cast_op->GetType().c_str());
    GELOGE(INTERNAL_ERROR, "[Set][Attr] %s to op:%s(%s) failed", CAST_ATTR_DST_TYPE.c_str(), cast_op->GetName().c_str(),
           cast_op->GetType().c_str());
    return nullptr;
  }
  return cast_op;
...
void TransOpWithoutReshapeFusionPass::GetCastOpDesc(const GeTensorDesc &out_desc, const GeTensorDesc &in_desc,
                                                    GeTensorDesc &cast_input, GeTensorDesc &cast_output) const {
  bool insert_cast_first = InsertCastFirstCheck(out_desc, in_desc);
  if (insert_cast_first) {
    cast_input = out_desc;
...
void TransOpWithoutReshapeFusionPass::GetBeginOutDescAndEndInDesc(const int32_t index, GeTensorDesc &out_desc,
  ...
  GE_IF_BOOL_EXEC(
      out_peer_op_desc == nullptr, GELOGE(INTERNAL_ERROR, "[Get][OpDesc] failed, out_peer_op_desc is nullptr"); return);
  ...
  GE_IF_BOOL_EXEC(
      in_peer_op_desc == nullptr, GELOGE(INTERNAL_ERROR, "[Get][OpDesc] failed, in_peer_op_desc is nullptr"); return);
  in_desc = in_peer_op_desc->GetOutputDesc(in_peer_anchor->GetIdx());
}

graphStatus TransOpWithoutReshapeFusionPass::FormatFusion(const int32_t index, OpDescPtr &format_transfer_op,
...
graphStatus TransOpWithoutReshapeFusionPass::DataTypeFusion(const int32_t index, OpDescPtr &cast_op,
  ...
  if ((fusion_op_count != kInvalidFusionOpCount) && (out_desc.GetDataType() != in_desc.GetDataType())) {
    ...
    (void)TransOpCreator::CheckAccuracySupported(cast_op, is_supported);
    if (is_supported) {
      ++fusion_op_count;
...
graphStatus TransOpWithoutReshapeFusionPass::TransOpFuseHandle(const ComputeGraphPtr &graph, const int32_t index) {
  ...
  if ((fusion_op_count > 0) && (fusion_op_count < transop_num_count_[index])) {
    GeTensorDesc out_desc;
    GeTensorDesc in_desc;
    GetBeginOutDescAndEndInDesc(index, out_desc, in_desc);
    bool insert_cast_first = InsertCastFirstCheck(out_desc, in_desc);
...
void TransOpWithoutReshapeFusionPass::RemoveNousedNodes(const ComputeGraphPtr &graph) {
  ...
  for (size_t i = 0; i < sub_graph_nodes_.size(); ++i) {
    ...
    for (const auto &node : sub_graph_nodes_[i]) {
      ...
      GE_IF_BOOL_EXEC(!op_desc->SetExtAttr(kRemainNode, true),
                      GELOGE(INTERNAL_ERROR, "[Set][ExtAttr] for op:%s failed", op_desc->GetName().c_str());
                      return);
...
graphStatus TransOpWithoutReshapeFusionPass::TransOpFuse(const ComputeGraphPtr &graph) {
  for (size_t i = 0; i < sub_graph_anchors_.size(); ++i) {
    ...
    if (FusionFormatSupport(out_desc->GetFormat()) && DescEqualCheck(out_desc, in_desc)) {
      ...
      if (RelinkNodesWhenDescNotChanged(nodes_anchor.front(), nodes_anchor.back(), static_cast<int32_t>(i)) !=
          GRAPH_SUCCESS) {
        return GRAPH_FAILED;
      }
    } else {
      if (TransOpFuseHandle(graph, static_cast<int32_t>(i)) != GRAPH_SUCCESS) {
        return GRAPH_FAILED;
      }
    }
  }
...
graphStatus TransOpWithoutReshapeFusionPass::AddTransNode(const ComputeGraphPtr &graph, const OpDescPtr &transop,
  ...
  if (trans_node == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Add node:%s(%s) to graph:%s failed", transop->GetName().c_str(),
                         transop->GetType().c_str(), graph->GetName().c_str());
    GELOGE(GRAPH_FAILED, "[Add][Node] %s(%s) to graph:%s failed", transop->GetName().c_str(),
           transop->GetType().c_str(), graph->GetName().c_str());
    return GRAPH_FAILED;
  }
  return GRAPH_SUCCESS;
...
graphStatus TransOpWithoutReshapeFusionPass::InsertNewTransOp(const ComputeGraphPtr &graph, const OpDescPtr &cast_op,
  ...
  std::pair<OutDataAnchorPtr, InDataAnchorPtr> begin_out = sub_graph_anchors_[index].front();
  std::pair<OutDataAnchorPtr, InDataAnchorPtr> end_in = sub_graph_anchors_[index].back();
  ...
  GE_CHK_STATUS_RET(GraphUtils::RemoveEdge(end_in.first, in_anchor), "[Remove][Edge] between %s and %s failed",
...
graphStatus TransOpWithoutReshapeFusionPass::AddControlEdgeForNewTransNode(
    const int32_t index, const std::vector<NodePtr> &new_trans_nodes) {
  for (const auto &peer_in_anchor : out_control_peer_in_control_anchors_[index]) {
    GE_CHECK_NOTNULL(peer_in_anchor);
    if (remain_in_control_anchors_.count(peer_in_anchor) > 0) {
      continue;
  ...
  for (const auto &peer_out_anchor : in_control_peer_out_control_anchors_[index]) {
    ...
    if (CheckIfHasSameOutControlEdge(new_trans_nodes.front(), peer_out_anchor->GetOwnerNode())) {
      continue;
    }
  ...
  // deprecated, there is no control anchor link to data anchor
  for (const auto &peer_in_anchor : out_control_peer_in_data_anchors_[index]) {
    GE_CHECK_NOTNULL(peer_in_anchor);
    GELOGI("add control edge.src:%s, dst:%s", new_trans_nodes.back()->GetName().c_str(),
           peer_in_anchor->GetOwnerNode()->GetName().c_str());
...
graphStatus TransOpWithoutReshapeFusionPass::RelinkControlEdge(const int32_t index, const OutDataAnchorPtr &out_anchor,
  ...
  if (AddControlEdgeForNewTransNode(index, new_trans_nodes) == GRAPH_FAILED) {
    return GRAPH_FAILED;
  }
  if (sub_graph_has_out_data_peer_in_control_edge_[index]) {
    ...
    if (GraphUtils::AddEdge(new_trans_nodes.back()->GetOutDataAnchor(0),
      ...
      REPORT_INNER_ERR_MSG("E19999", "Add edge between op:%s(%s) and op:%s(%s) failed",
                           new_trans_nodes.back()->GetName().c_str(), new_trans_nodes.back()->GetType().c_str(),
...
bool TransOpWithoutReshapeFusionPass::FusionFormatSupport(Format format) {
  const auto primary_format = static_cast<Format>(GetPrimaryFormat(static_cast<int32_t>(format)));
  return primary_format == FORMAT_NCHW || primary_format == FORMAT_NHWC || primary_format == FORMAT_FRACTAL_Z ||
         primary_format == FORMAT_NC1HWC0;
}
...
graphStatus TransOpWithoutReshapeFusionPass::GetSubGraphsBetweenNormalNode(
  ...
  if (out_anchor == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Param out_anchor is nullptr, check invalid");
  ...
  for (const auto &peer_in_anchor : out_anchor->GetPeerInDataAnchors()) {
    ...
    if (!IsTransOp(peer_in_node)) {
      ...
      for (const auto &peer_out_anchor : peer_in_node->GetAllOutDataAnchors()) {
        ...
        if (ret != GRAPH_SUCCESS) {
          GELOGE(GRAPH_FAILED, "[Get][SubGraphs] Between Normal Node failed! node:%s", peer_in_node->GetName().c_str());
          return GRAPH_FAILED;
        }
      }
      nodes_list.pop_back();
...
REG_PASS_OPTION("TransOpWithoutReshapeFusionPass").LEVELS(OoLevel::kO3);
}  // namespace ge
