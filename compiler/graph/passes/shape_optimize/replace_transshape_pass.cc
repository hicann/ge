...
Status ReplaceTransShapePass::ReplaceTransShapeNode(ComputeGraphPtr &graph, NodePtr &trans_shape_node) const {
  std::string op_type;
  ...
  NodePtr memcpy_node = graph->InsertNode(trans_shape_node, dst_op_desc);
...
void ReplaceTransShapePass::CopyControlEdges(NodePtr &old_node, NodePtr &new_node) const {
  ...
  if (new_node->GetOutControlAnchor() == nullptr) {
    GELOGW("New node[%s]'s out control anchor is nullptr.", new_node->GetName().c_str());
    return;
  }
...
void ReplaceTransShapePass::RemoveControlEdges(NodePtr &node) const {
  GE_CHECK_NOTNULL_JUST_RETURN(node);
  for (NodePtr &in_node : node->GetInControlNodes()) {
    if (in_node->GetOutControlAnchor() == nullptr) {
      continue;
    }
...
void ReplaceTransShapePass::ReplaceControlEdges(NodePtr &old_node, NodePtr &new_node) const {
  GE_IF_BOOL_EXEC(old_node == new_node, return);
