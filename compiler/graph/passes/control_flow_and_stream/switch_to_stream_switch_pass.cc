...
Status SwitchToStreamSwitchPass::Run(ComputeGraphPtr graph) {
  ...
  for (const auto &node : graph->GetDirectNode()) {
    ...
    if ((node->GetType() == MEMCPYASYNC) && node->GetOutAllNodes().empty()) {
      bypass_nodes_.emplace(node);
    }
  }
...
Status SwitchToStreamSwitchPass::BypassSwitchNode(const NodePtr &switch_node, OutDataAnchorPtr &peer_data_anchor,
                                                  OutDataAnchorPtr &peer_cond_anchor) const {
  for (uint32_t idx = 0; idx < SWITCH_INPUT_NUM; ++idx) {
    ...
    if (idx == SWITCH_DATA_INPUT) {
      ...
      if (FindSwitchCondInput(peer_cond_anchor) != SUCCESS) {
        REPORT_INNER_ERR_MSG("E19999", "Find switch cond input failed.");
        GELOGE(FAILED, "Find switch cond input failed.");
      }
    }
  }
...
/// @return Status
///
Status SwitchToStreamSwitchPass::FindSwitchCondInput(OutDataAnchorPtr &peer_cond_anchor) const {
  NodePtr tmp_node = nullptr;
  std::string type;
  bool pass_flag = true;
  while (pass_flag) {
    ...
    pass_flag = ((type == SWITCH) || (type == REFSWITCH) || (type == MEMCPYASYNC));
  }
...
NodePtr SwitchToStreamSwitchPass::CreateStreamSwitchNode(const ComputeGraphPtr &graph, const NodePtr &switch_node,
                                                         ...
                                                         const OutDataAnchorPtr &peer_cond_anchor) const {
...
/// @param [in] peer_cond_anchor
/// @param [in] stream_switch_node
/// @param [in] true_branch_flag
/// @return Status
///
Status SwitchToStreamSwitchPass::MarkBranches(const OutDataAnchorPtr &peer_cond_anchor,
                                              const NodePtr &stream_switch_node, bool true_branch_flag) {
  ...
  if (it != cond_node_map_.end()) {
    int64_t switch_group_id = GetGroupId(stream_switch_node);
    ...
    if (switch_group_it == it->second.end()) {
      ...
      node_list.emplace_back(stream_switch_node);
      ...
      switch_group_it->second[index].emplace_back(stream_switch_node);
    }
  } else {
    int64_t switch_group_id = GetGroupId(stream_switch_node);
...
/// @return group_id
///
int64_t SwitchToStreamSwitchPass::GetGroupId(const NodePtr &node) const {
...
Status SwitchToStreamSwitchPass::CombineSwitchNode(const ComputeGraphPtr &graph) {
  for (auto iter = cond_node_map_.begin(); iter != cond_node_map_.end(); ++iter) {
    for (auto group_iter = iter->second.cbegin(); group_iter != iter->second.cend(); ++group_iter) {
      ...
      same_cond_switch.insert(false_switch_list.cbegin(), false_switch_list.cend());
      same_cond_switch.insert(true_switch_list.cbegin(), true_switch_list.cend());
      ...
      for (uint32_t i = 0; i < SWITCH_OUTPUT_NUM; ++i) {
        ...
        if (AttrUtils::GetInt(switch_desc, ATTR_NAME_CONTROL_FLOW_GROUP, old_group_index) &&
          ...
          old_2_new_group_index_[old_group_index] = group_index;
        }
        SetControlFlowGroup(stream_switch, group_index);
        for (const NodePtr &node : switch_list) {
          if (node != stream_switch) {
            ...
            old_group_index = -1;
            if (AttrUtils::GetInt(op_desc, ATTR_NAME_CONTROL_FLOW_GROUP, old_group_index) &&
                (old_group_index != group_index)) {
              old_2_new_group_index_[old_group_index] = group_index;
            }
          }
...
NodePtr SwitchToStreamSwitchPass::CreateCastOp(const ComputeGraphPtr &graph, const OutDataAnchorPtr &peer_cond_anchor) {
  ...
  if (!(AttrUtils::SetInt(cast_desc, CAST_ATTR_SRCT, static_cast<int64_t>(DT_BOOL)) &&
...
/// @return Status
///
Status SwitchToStreamSwitchPass::AddConstNode(const ComputeGraphPtr &graph, const NodePtr &stream_switch) const {
...
Status SwitchToStreamSwitchPass::ModifySwitchInCtlEdges(const NodePtr &switch_node, const NodePtr &cast_node,
  ...
  for (const NodePtr &in_ctrl_node : switch_node->GetInControlNodes()) {
    ...
    const auto find_res3 = find_res1->second.find(cast_node->GetName());
...
///
/// @brief Update control flow group
/// @param [in] graph
/// @return Status
///
Status SwitchToStreamSwitchPass::UpdateControlFlowGroup(const ComputeGraphPtr &graph) {
  for (const NodePtr &node : graph->GetDirectNode()) {
    auto op_desc = node->GetOpDesc();
    GE_CHECK_NOTNULL(op_desc);
    ...
    int64_t old_group_index = -1;
    if (AttrUtils::GetInt(op_desc, ATTR_NAME_CONTROL_FLOW_GROUP, old_group_index)) {
      const auto iter = old_2_new_group_index_.find(old_group_index);
...
REG_PASS_OPTION("SwitchToStreamSwitchPass").LEVELS(OoLevel::kO0);
}  // namespace ge
