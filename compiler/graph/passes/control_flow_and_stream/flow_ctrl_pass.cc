...
Status FlowCtrlPass::Run(ComputeGraphPtr compute_graph) {
  ...
  for (auto &node : compute_graph->GetDirectNode()) {
    ...
    if (is_found && (true_stream_id == TRUE_STREAM_ID)) {
      ...
      if (ret != SUCCESS) {
        GELOGE(ret, "[Add][FpBpIteratorCtrl] failed, node:%s, graph:%s.", node->GetName().c_str(),
  ...
  for (auto &node : compute_graph->GetDirectNode()) {
    ...
    if (need_cycle_flag) {
      ...
      if (ret != SUCCESS) {
        GELOGE(ret, "[Add][SpecialNodeIteratorCtrl] failed, node:%s, graph:%s.", node->GetName().c_str(),
               compute_graph->GetName().c_str());
        return ret;
      }
      graph_change = true;
  ...
  for (auto &active_node : active_nodes_in_iter_loop_) {
    auto ret =
        GraphUtils::AddEdge(active_node->GetOutControlAnchor(), assign_add_node_in_fpbp_loop_->GetInControlAnchor());
...
NodePtr FlowCtrlPass::InsertOp(ComputeGraphPtr &compute_graph, const std::string &node_type,
  ...
  for (auto &output_desc : output_list) {
    ...
    if (graph_status != GRAPH_SUCCESS) {
      REPORT_INNER_ERR_MSG("E19999", "Add output desc to op:%s(%s) failed", op_desc->GetName().c_str(),
                           op_desc->GetType().c_str());
      GELOGE(FAILED, "[Add][OutputDesc] to op:%s(%s) failed", op_desc->GetName().c_str(), op_desc->GetType().c_str());
      return nullptr;
    }
  }
  ...
  GE_IF_BOOL_EXEC(compute_graph == nullptr,
                  ...
                  DOMI_LOGE("[Check][Param] compute_graph is nullptr"); return nullptr);
...
NodePtr FlowCtrlPass::InsertStreamSwitchOp(ComputeGraphPtr &compute_graph, const std::string &switch_name,
  ...
  GE_IF_BOOL_EXEC(loop_cond == nullptr || loop_cond->GetOpDesc() == nullptr,
                  ...
                  GELOGE(FAILED, "[Check][Param] Param loop_cond or its op_desc is nullptr"); return nullptr);
  GE_IF_BOOL_EXEC(iter_per_loop == nullptr || iter_per_loop->GetOpDesc() == nullptr,
                  ...
                  GELOGE(FAILED, "[Check][Param] Param iter_per_loop or its op_desc is nullptr"); return nullptr);
  ...
  if (add_ret != GRAPH_SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "Add edge between op:%s(%s)(index:0) and op:%s(%s)(index:0) failed",
                         loop_cond->GetName().c_str(), loop_cond->GetType().c_str(), stream_switch->GetName().c_str(),
                         stream_switch->GetType().c_str());
    GELOGE(FAILED, "[Add][Edge] between op:%s(%s)(index:0) and op:%s(%s)(index:0) failed", loop_cond->GetName().c_str(),
           loop_cond->GetType().c_str(), stream_switch->GetName().c_str(), stream_switch->GetType().c_str());
    return nullptr;
  }
  ...
  if (add_ret != GRAPH_SUCCESS) {
    ...
    GELOGE(FAILED, "[Add][Edge] between op:%s(%s)(index:0) and op:%s(%s)(index:1) failed",
           iter_per_loop->GetName().c_str(), iter_per_loop->GetType().c_str(), stream_switch->GetName().c_str(),
           stream_switch->GetType().c_str());
    return nullptr;
  }
...
NodePtr FlowCtrlPass::AddVariableNode(ComputeGraphPtr &compute_graph, const std::string &name) {
  ...
  if (ret != SUCCESS) {
    ...
    GELOGE(FAILED, "[Get][CurVarDesc] failed, name:%s, session_id:%" PRIu64 "", name.c_str(),
...
NodePtr FlowCtrlPass::InsertAssignOp(ge::ComputeGraphPtr &compute_graph, const std::string &node_type,
  ...
  GE_IF_BOOL_EXEC(ref_node == nullptr || value_node == nullptr || ref_node->GetOpDesc() == nullptr ||
                  ...
                  REPORT_INNER_ERR_MSG("E19999",
                                       "Param ref_node or value_node or their op_desc is nullptr, "
                                       "check invalid");
  ...
  if (add_ret != GRAPH_SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "Add edge between op:%s(%s)(index:0) and op:%s(%s)(index:0) failed",
                         ref_node->GetName().c_str(), ref_node->GetType().c_str(), assign_node->GetName().c_str(),
                         assign_node->GetType().c_str());
    GELOGE(FAILED, "[Add][Edge] between op:%s(%s)(index:0) and op:%s(%s)(index:0) failed", ref_node->GetName().c_str(),
           ref_node->GetType().c_str(), assign_node->GetName().c_str(), assign_node->GetType().c_str());
    return nullptr;
  }
  ...
  if (add_ret != GRAPH_SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "Add edge between op:%s(%s)(index:0) and op:%s(%s)(index:1) failed",
                         value_node->GetName().c_str(), value_node->GetType().c_str(), assign_node->GetName().c_str(),
                         assign_node->GetType().c_str());
    GELOGE(FAILED, "[Add][Edge] between op:%s(%s)(index:0) and op:%s(%s)(index:1) failed",
           value_node->GetName().c_str(), value_node->GetType().c_str(), assign_node->GetName().c_str(),
           assign_node->GetType().c_str());
...
Status FlowCtrlPass::CreateIterCtrlTrueBranch(ComputeGraphPtr &compute_graph, const NodePtr &loop_cond_node,
  ...
  if (status != ge::SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "Set stream_label:%s to op:%s(%s) failed", active_name.c_str(),
                         assign_add_node_in_fpbp_loop_->GetName().c_str(),
                         assign_add_node_in_fpbp_loop_->GetType().c_str());
    GELOGE(status, "[Set][StreamLabel] %s to op:%s(%s) failed", active_name.c_str(),
  ...
  if (status != ge::SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "Set active label list:%s to op:%s(%s) failed", active_name.c_str(),
                         switch_node->GetName().c_str(), switch_node->GetType().c_str());
    GELOGE(status, "[Set][ActiveLabelList] %s to op:%s(%s) failed", active_name.c_str(), switch_node->GetName().c_str(),
  ...
  if (status != ge::SUCCESS) {
    ...
    GELOGE(status, "[Set][StreamLabel] %s to op:%s(%s) failed", active_name.c_str(), active_node->GetName().c_str(),
           active_node->GetType().c_str());
    return status;
  }
  ...
  // add ctrl edges
  graphStatus add_ret =
      GraphUtils::AddEdge(switch_node->GetOutControlAnchor(), assign_add_node_in_fpbp_loop_->GetInControlAnchor());
  if (add_ret != GRAPH_SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "Add control edge between op:%s(%s) and op:%s(%s) failed",
                         switch_node->GetName().c_str(), switch_node->GetType().c_str(),
                         assign_add_node_in_fpbp_loop_->GetName().c_str(),
                         assign_add_node_in_fpbp_loop_->GetType().c_str());
    GELOGE(FAILED, "[Add][ControlEdge] between op:%s(%s) and op:%s(%s) failed", switch_node->GetName().c_str(),
           switch_node->GetType().c_str(), assign_add_node_in_fpbp_loop_->GetName().c_str(),
           assign_add_node_in_fpbp_loop_->GetType().c_str());
    return FAILED;
  }

  add_ret =
      GraphUtils::AddEdge(assign_add_node_in_fpbp_loop_->GetOutControlAnchor(), active_node->GetInControlAnchor());
  if (add_ret != GRAPH_SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "Add control edge between op:%s(%s) and op:%s(%s) failed",
                         assign_add_node_in_fpbp_loop_->GetName().c_str(),
                         assign_add_node_in_fpbp_loop_->GetType().c_str(), active_node->GetName().c_str(),
                         active_node->GetType().c_str());
...
Status FlowCtrlPass::CreateIterCtrlFalseBranch(ComputeGraphPtr &compute_graph, const NodePtr &loop_cond_node,
  ...
  GE_CHK_STATUS_RET(
      GraphUtils::AddEdge(assign_node_in_fpbp_loop_->GetOutControlAnchor(), active_node->GetInControlAnchor()),
      "[Add][ControlEdge] failed.");
...
Status FlowCtrlPass::AddFpBpIteratorCtrl(ComputeGraphPtr &compute_graph, NodePtr &pre_node) {
  ...
  if (status != ge::SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "Set stream label:%s to op:%s(%s) failed", switch_name.c_str(),
                         switch_node->GetName().c_str(), switch_node->GetType().c_str());
    GELOGE(status, "[Set][StreamLabel] %s to op:%s(%s) failed", switch_name.c_str(), switch_node->GetName().c_str(),
           switch_node->GetType().c_str());
    return status;
  }
...
Status FlowCtrlPass::AddSpecialNodeIteratorCtrl(ComputeGraphPtr &compute_graph, NodePtr &loop_after_node) {
  ...
  GE_IF_BOOL_EXEC(loop_after_node == nullptr || compute_graph == nullptr,
                  ...
                  DOMI_LOGE("[Check][Param] loop after node or compute graph is null."); return FAILED);
  ...
  if (in_anchor == nullptr || in_anchor->GetPeerOutAnchor() == nullptr) {
    ...
    GELOGE(FAILED, "[Check][Param] Param loop_after_node:%s(%s) no in data node.", loop_after_node->GetName().c_str(),
  ...
  if (loop_cond_node == nullptr) {
    REPORT_INNER_ERR_MSG("E19999", "Node:%s not found in graph:%s, check invalid", NODE_NAME_FLOWCTRL_LOOP_COND.c_str(),
                         compute_graph->GetName().c_str());
    GELOGE(FAILED, "[Check][Param] Node:%s not found in graph:%s.", NODE_NAME_FLOWCTRL_LOOP_COND.c_str(),
           compute_graph->GetName().c_str());
    return FAILED;
  }
  ...
  if (iter_per_loop_node == nullptr) {
    ...
    GELOGE(FAILED, "[Check][Param] Node:%s not found in graph:%s.", NODE_NAME_FLOWCTRL_LOOP_PER_ITER.c_str(),
           compute_graph->GetName().c_str());
    return FAILED;
  }
  ...
  if (add_ret != GRAPH_SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "Add control edge between op:%s(%s) and op:%s(%s) failed",
                         ...
                         switch_node->GetName().c_str(), switch_node->GetType().c_str());
    GELOGE(FAILED, "[Add][ControlEdge] between op:%s(%s) and op:%s(%s) failed", loop_pre_node->GetName().c_str(),
           loop_pre_node->GetType().c_str(), switch_node->GetName().c_str(), switch_node->GetType().c_str());
    return FAILED;
  }
  ...
  if (add_ret != GRAPH_SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "Add control edge between op:%s(%s) and op:%s(%s) failed",
                         loop_after_node->GetName().c_str(), loop_after_node->GetType().c_str(),
                         switch_node->GetName().c_str(), switch_node->GetType().c_str());
    GELOGE(FAILED, "[Add][ControlEdge] between op:%s(%s) and op:%s(%s) failed", loop_after_node->GetName().c_str(),
           loop_after_node->GetType().c_str(), switch_node->GetName().c_str(), switch_node->GetType().c_str());
    return FAILED;
  }
  ...
  status = SetStreamLabel(active_node, active_name);
  if (status != ge::SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "Set stream label:%s to op:%s(%s) failed", active_name.c_str(),
                         active_node->GetName().c_str(), active_node->GetType().c_str());
    GELOGE(status, "[Set][StreamLabel] %s to op:%s(%s) failed", active_name.c_str(), active_node->GetName().c_str(),
           active_node->GetType().c_str());
    return status;
  ...
  if (add_ret != GRAPH_SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "Add control edge between op:%s(%s) and op:%s(%s) failed",
                         switch_node->GetName().c_str(), switch_node->GetType().c_str(), active_node->GetName().c_str(),
                         active_node->GetType().c_str());
    GELOGE(FAILED, "[Add][ControlEdge] between op:%s(%s) and op:%s(%s) failed", switch_node->GetName().c_str(),
           switch_node->GetType().c_str(), active_node->GetName().c_str(), active_node->GetType().c_str());
    return FAILED;
  }
  ...
  if (status != ge::SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "Set active label list:%s to op:%s(%s) failed", loop_pre_node->GetName().c_str(),
                         active_node->GetName().c_str(), active_node->GetType().c_str());
    GELOGE(status, "[Set][ActiveLabelList] %s to op:%s(%s) failed", loop_pre_node->GetName().c_str(),
           active_node->GetName().c_str(), active_node->GetType().c_str());
    return status;
  }
