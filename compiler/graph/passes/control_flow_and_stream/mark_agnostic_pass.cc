...
Status MarkAgnosticPass::Run(ComputeGraphPtr graph) {
  for (const auto &node : graph->GetDirectNode()) {
    ...
    if (node_type == IDENTITY) {
      GELOGD("Mark format agnostic for node %s", node->GetName().c_str());
      AttrUtils::SetInt(node->GetOpDesc(), ATTR_NAME_FORMAT_AGNOSTIC, 1);
      continue;
    }
    if (node_type == SHAPE) {
      GELOGD("Mark format agnostic for node %s, except output.", node->GetName().c_str());
      AttrUtils::SetInt(node->GetOpDesc(), ATTR_NAME_FORMAT_AGNOSTIC, 1);
      AttrUtils::SetListInt(node->GetOpDesc(), ATTR_NAME_FORMAT_AGNOSTIC_EXCEPT_OUTPUT, std::vector<int64_t>({0}));
      continue;
    }
...
bool MarkAgnosticPass::IsWhileLoop(const NodePtr &merge_node, NodePtr &enter, NodePtr &next) const {
...
Status MarkAgnosticPass::HandWhileLoop(const NodePtr &node) const {
...
Status MarkAgnosticPass::SetContinuousAttr(const NodePtr &node, const std::vector<uint32_t> &indexes) const {
...
REG_PASS_OPTION("MarkAgnosticPass").LEVELS(OoLevel::kO1);
}  // namespace ge
