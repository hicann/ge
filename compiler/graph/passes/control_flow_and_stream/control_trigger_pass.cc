...
NodePtr ControlTriggerPass::InsertMergeNode(ComputeGraphPtr &graph, NodePtr &node, NodePtr &in_ctrl_node,
  ...
  // If the cond inputs of the switch nodes associated with multiple control inputs of control_trigger node are
  // different, the InsertMergeNode will be executed multiple times. If the control_trigger node is used as the name of
  // the merge, it will cause duplication, so the name of the merge node needs to be associated with the cond input of
  // the switch node
