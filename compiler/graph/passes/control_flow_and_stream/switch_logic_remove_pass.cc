...
namespace ge {
namespace {
using PredNodeAndOut = std::pair<NodePtr, int32_t>;
constexpr int32_t kSwitchOutputNum = 2;
constexpr int32_t kSwitchPredIndex = 1;

char const *GetOutputNameFromIndex(int32_t index) {
...
Status SwitchLogicRemovePass::Run(NodePtr &node) {
  ...
  for (int32_t i = 0; i < kSwitchOutputNum; ++i) {
...
Status SwitchLogicRemovePass::RemoveSwitchNodeLogically(int32_t parent_index, NodePtr &switch_node) {
  std::vector<int32_t> isolate_map({-1, -1});
  for (int32_t i = 0; i < kSwitchOutputNum; ++i) {
    if (i == parent_index) {
      isolate_map[i] = 0;
      continue;
