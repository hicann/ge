...
CmpStatus RearrangeConv2d(const char *const input, char *const weightRe, size_t compressTilingK, size_t compressTilingN,
                          CompressParametersConv2d &compressParameters) {
  if (compressTilingK <= 0 || compressTilingN <= 0) {
    LogFatal("compressTiling is invalid: compressTilingK(" << compressTilingK << ") compressTilingN(" << compressTilingN
                                                           << ")");
    return RET_ERROR;
  }
...
CmpStatus CompressConv2d(CompressOpConfig *const param, char *const weightRe,
                         CompressParametersConv2d &compressParameters, char *const zipBuffer, char *const infoBuffer) {
  ...
  auto ret =
      CompressWeights(weightData, param->compressConfig, compIdxBuffer.get(), compDataBuffer.get(), compressedLength);
  if (ret != 0) {
  ...
  if (compressParameters.totalCompressedLength + compressedLength > compressParameters.weightSizeTotal) {
    LogFatal("weightSizeTotal is:" << compressParameters.weightSizeTotal
                                   << ", totalCompressedLength is:" << compressParameters.totalCompressedLength
                                   << ", compressedLength is:" << compressedLength << ".");
    return RET_ERROR;
  }
  res = memcpy_s(zipBuffer + compressParameters.totalCompressedLength, compressedLength, compDataBuffer.get(),
                 compressedLength);
  if (res != 0) {
...
CmpStatus CompressWeightConv2d(char *const weightRe, char *const zipBuffer, char *const infoBuffer,
                               CompressOpConfig *const param, CompressParametersConv2d &compressParameters) {
...
bool SetCompressParametersConv2dConv2d(CompressParametersConv2d &compressParameters, CompressOpConfig *const param) {
  ...
  size_t compressWeightSize = compressParameters.weightK * compressParameters.weightN * compressParameters.weightCo0 *
                              compressParameters.weightCo1;
  ...
  if (compressParameters.nRemainders != 0) {
    compressParameters.firstWeightSize =
        param->compressTilingN * param->compressTilingK * compressParameters.weightCo0 * compressParameters.weightCo1 *
        compressParameters.kbFrac * compressParameters.nbFrac * compressParameters.wDtypeSize;
...
CmpStatus CompressWeightsConv2D(const char *const input, char *const zipBuffer, char *const infoBuffer,
                                CompressOpConfig *const param) {
  if (input == nullptr || zipBuffer == nullptr || infoBuffer == nullptr || param == nullptr) {
    LogFatal("The input parameters contain null pointers.");
    return RET_ERROR;
  }
  if (param->compressConfig.inputSize <= 0) {
    LogFatal("inputSize in CompressConfig is invalid: " << param->compressConfig.inputSize);
    return RET_ERROR;
  }

  LogCompressOpConfig(param);

  std::unique_ptr<char[]> weightRe(new (std::nothrow) char[param->compressConfig.inputSize]());
  ...
  auto res = RearrangeConv2d(input, weightRe.get(), param->compressTilingK, param->compressTilingN, compressParameters);
  if (res == RET_ERROR) {
