...
// 当前仅根据输出类型来判断
const std::map<std::string, std::vector<VfInstructPerf>> &PerfParamTableV2::GetVfInstructPerfTable() {
...
const std::string kParamV2Info = R"(
{
    ...
    "LoadStride": {
        ...
        "model_params": {
            "int64toint64": {"k": 0.043},
            "float16tofloat16": {"k": 0.043},
            "float32tofloat32": {"k": 0.043},
            "uint8touint8": {"k": 0.043},
            "bfloat16tobfloat16": {"k": 0.043}
        }
    },
    "Store": {
        "model_type": "LoadStoreFunc",
    ...
    "StoreStride": {
        "model_type": "LoadStoreStrideV2Func",
        "model_params": {
            "int64toint64": {"k": 0.0385},
            "float16tofloat16": {"k": 0.0385},
            "float32tofloat32": {"k": 0.0385},
            "uint8touint8": {"k": 0.0385},
            "bfloat16tobfloat16": {"k": 0.0385}
        }
    },
    "Nddma": {
        "model_type": "LoadStoreFunc",
...
PipeHeadPerfFunc PerfParamTableV2::GetPipeHeadPerfFunc(PipeType pipe_type) const {
  ...
  if (it == pipes_head_perf_.end()) {
    static auto default_func = [](const std::vector<NodeInfo> &node_infos,
                                  std::map<Expr, TenaryOp, ExprCmp> &tenary_ops) -> Expr {
      (void)node_infos;
      (void)tenary_ops;
      return CreateExpr(0);
