...
ge::Status LoadApi([[maybe_unused]] const std::vector<TensorShapeInfo> &input_shapes,
  ...
  GE_ASSERT_SUCCESS(MergeTensorContinuousDims(kLoad, GetNodeOutTensorName(node, 0), merged_output_shapes));
  NodeDetail dma_info =
      NodeDetail{node_name, kLoad, {merged_output_shapes.data_type}, {merged_output_shapes.data_type}};
...
ge::Status NddmaApi([[maybe_unused]] const std::vector<TensorShapeInfo> &input_shapes,
  ...
  GE_ASSERT_TRUE(!input_shapes.empty());
  GE_ASSERT_TRUE(!output_shapes.empty());
  std::string node_name = node != nullptr ? node->GetName() : "NddmaNode";
  auto merged_output_shapes = output_shapes[0];
  GE_ASSERT_SUCCESS(MergeTensorContinuousDims(kLoad, GetNodeOutTensorName(node, 0), merged_output_shapes));
  NodeDetail dma_info =
      NodeDetail{node_name, kNddma, {merged_output_shapes.data_type}, {merged_output_shapes.data_type}};
  GE_ASSERT_SUCCESS(SetDims(merged_output_shapes.dims, dma_info));
...
/*
StoreApi(DataCopy from UB to GM)的性能公式：（其中a-b-c-d为待拟合参数）
...
ge::Status StoreApi([[maybe_unused]] const std::vector<TensorShapeInfo> &input_shapes,
  ...
  auto merged_output_shapes = output_shapes[0];
  GE_ASSERT_SUCCESS(MergeTensorContinuousDims(kStore, GetNodeOutTensorName(node, 0), merged_output_shapes));
  NodeDetail dma_info =
      NodeDetail{node_name, kStore, {merged_output_shapes.data_type}, {merged_output_shapes.data_type}};
  GE_ASSERT_SUCCESS(SetDims(merged_output_shapes.dims, dma_info));
  GE_ASSERT_SUCCESS(GetDmaPerf(merged_output_shapes, dma_info, perf_res, kMaxDmaLen));
  return ge::SUCCESS;
}
...
REGISTER_EVAL_FUNC_TAG(kStore, V2, ascir_v2::StoreApi);
