        ...
        if (block_.params_.index < block_.params_.totalCnt) {
            ...
            if (block_.params_.singleCoreM > 0 && block_.params_.singleCoreN > 0) {
                ...
                for (uint64_t kIndex = 0; kIndex < block_.params_.splitKRound; kIndex++) {
                    ...
                    UpdateBias(kIndex);
                    mm_.Iterate(kIndex != 0); // 原本代码会引入精度问题：mm_.Iterate();
                }
                mm_.GetTensorC(cGlobal_[block_.offset_.offsetC], enAtomic); // 原本代码会引入精度问题：mm_.GetTensorC(cGlobal_[block_.offset_.offsetC], enAtomic || kIndex != 0);
            }
        }
    }
