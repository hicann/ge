 ...
 */
#ifndef __ASCENDC_API_SCALAR_DIV_H__
#define __ASCENDC_API_SCALAR_DIV_H__
...
inline __aicore__ void Divs(const LocalTensor<T> &dst, const LocalTensor<T> &src, const T constant_x,
  ...
  const uint32_t one_rpt_num = KernelUtils::RptSize<T>();
  const uint32_t repeat_times = calc_cnt / one_rpt_num;
  uint32_t max_rpt_loop_cnt = repeat_times / MAX_REPEAT_TIME;
  SetMaskNorm();
  AscendC::SetVectorMask<T, MaskMode::NORMAL>(one_rpt_num);
  for (uint32_t idx = 0; idx < max_rpt_loop_cnt; idx++) {
    if constexpr (IS_SCALAR_LATTER) {
      AscendC::Div<T, false>(dst[calc_size], src[calc_size], tmp, MASK_PLACEHOLDER, MAX_REPEAT_TIME, binary_param);
    } else {
      AscendC::Div<T, false>(dst[calc_size], tmp, src[calc_size], MASK_PLACEHOLDER, MAX_REPEAT_TIME, binary_param);
    }
  ...
  if (remain_rpt_times != 0) {
    if constexpr (IS_SCALAR_LATTER) {
      AscendC::Div<T, false>(dst[calc_size], src[calc_size], tmp, MASK_PLACEHOLDER, remain_rpt_times, binary_param);
    } else {
      AscendC::Div<T, false>(dst[calc_size], tmp, src[calc_size], MASK_PLACEHOLDER, remain_rpt_times, binary_param);
    }
  ...
  if (calc_tail != 0) {
    ...
    if constexpr (IS_SCALAR_LATTER) {
      AscendC::Div<T, false>(dst[calc_size], src[calc_size], tmp, MASK_PLACEHOLDER, 1, binary_param);
    } else {
      AscendC::Div<T, false>(dst[calc_size], tmp, src[calc_size], MASK_PLACEHOLDER, 1, binary_param);
    }
  }
  AscendC::SetMaskNorm();
  AscendC::ResetMask();
...
template <typename T>
inline __aicore__ void Divs(const LocalTensor<T>& dst, const T x, const T y) {
  T res = x / y;
  AscendC::Duplicate(dst, res, dst.GetSize());
}

#endif  // __ASCENDC_API_SCALAR_DIV_H__
