...
graphStatus CastReshapeCastPass::Run(const ComputeGraphPtr &graph) {
  for (const auto &node : graph->GetAllNodes()) {
    if ((node == nullptr) || (node->GetType() != "Reshape") || (node->GetInDataNodesSize() != 2U) ||
        (node->GetOutDataNodesSize() != 1U)) {
      continue;
    }
    const auto in_node = node->GetInDataNodes().at(0U);
    const auto out_node = node->GetOutDataNodes().at(0U);
    if ((in_node == nullptr) || (out_node == nullptr) || (in_node->GetType() != "Cast") ||
        (out_node->GetType() != "Cast")) {
      continue;
    }
    const auto in_op_desc = in_node->GetOpDesc();
    const auto out_op_desc = out_node->GetOpDesc();
    if (in_op_desc->GetInputDesc(0).GetDataType() != out_op_desc->GetOutputDesc(0).GetDataType()) {
      continue;
    }
