...
void CheckAndInitReduceAllLoadState(const NodePtr &node, AutoFuseAttrs *attr, const std::string &node_desc) {
  ...
  if (attr->HasFuseType(loop::FuseType::kReduction) &&
    ...
    if (BackendUtils::IfNormLikeReduce(node)) {
      ...
      GELOGI("%s is norm-like reduce, all loads satisfy conditions (R axis <= 65536, A axis >= 16)", node_desc.c_str());
    } else {
      // 不是所有load都满足norm-like条件
      attr->SetReduceAllLoadState(REDUCE_ALL_LOAD_NOT_ALL);
      GELOGI("%s is not norm-like reduce, not all loads satisfy conditions", node_desc.c_str());
    }
  }
}
