...
bool AnyOfAttrInfosContainSpecifiedLoadTypes(const NodePtr &node, const std::vector<ViewOpAttrInfo> &attr_infos,
  ...
  for (const auto &attr_info : attr_infos) {
    ...
    if (has_non_view_load) {
      bool is_view_op = (!attr_info.broadcast_info.empty()) || (attr_info.slice_info.two_slice_node_flag) ||
...
bool CheckIfSubGraphLinksHaveSpecifiedLoadTypePairs(const NodePtr &node1, const NodePtr &node2,
  ...
  for (const auto &subgraph_link : node_fuse_info.GetNode1ToNode2LinkMap()) {
    ...
    GELOGD("node %s(%s) input index(%d) is simplest load.", node2->GetName().c_str(), node2->GetType().c_str(),
           subgraph_link.second);
  }
  return false;
}
...
bool CheckIfSliceAscBackendNodeContainsBroadcast(const NodePtr &node) {
  ...
  for (auto &data_node : subgraph_input_nodes) {
    ...
    GELOGD(
        "in the ascgraph of slice node %s(%s), data op %s does not contain broadcast info, and does not followed by "
        "any broadcast op.",
        node->GetName().c_str(), node->GetType().c_str(), data_node->GetName().c_str());
  }
  ...
  if (node->GetType() == kFusedAscBackendType) {
    ...
    for (auto &sub_node : graph->GetAllNodes()) {
      ...
      if (CheckIfSliceAscBackendNodeContainsBroadcast(sub_node)) {
        GELOGD(
            "sub-node %s(%s) in the fused subgraph of node %s(%s) has fuse type slice, and ascgraph of sub-node %s "
            "contains broadcast.",
            sub_node->GetName().c_str(), sub_node->GetType().c_str(), node->GetName().c_str(), node->GetType().c_str(),
            sub_node->GetName().c_str());
        return true;
      }
    }
    GELOGD("fused subgraph of node %s(%s) does not contain broadcast.", node->GetName().c_str(),
           node->GetType().c_str());
    return false;
  }
...
bool SliceSplitFusionStrategy::CanFuse(const NodePtr &node1, const NodePtr &node2) {
  ...
  if ((attr1->HasFuseType(loop::FuseType::kSliceSplit) && CheckIfSliceNodeContainsBroadcast(node1)) ||
    ...
    GELOGI(
        "node1 %s(%s) and node2 %s(%s) cannot fuse, the reason is [%s][This fusion introduces slice node with view op]",
  ...
  if (!is_vertical && attr1->HasFuseType(loop::FuseType::kSliceSplit) &&
      attr2->HasFuseType(loop::FuseType::kSliceSplit)) {
    GELOGI(
        "node1 %s(%s) and node2 %s(%s) cannot fuse, the reason is [%s][horizontal slice/split without vertical link]",
  ...
  if (is_vertical) {
    ...
    if (attr2->HasFuseType(loop::FuseType::kSliceSplit)) {
      std::set prev_load_types = {LoadViewType::NonViewLoad, LoadViewType::ViewLoadBroadcast,
                                  LoadViewType::ViewLoadTranspose};
    ...
    if (attr1->HasFuseType(loop::FuseType::kSliceSplit) &&
      ...
      GELOGI(
          "node1 %s(%s) and node2 %s(%s) cannot fuse, the reason is [%s][This fusion introduces slice node with view "
          "op]",
          node1->GetName().c_str(), node1->GetType().c_str(), node2->GetName().c_str(), node2->GetType().c_str(),
          ge::NotFuseReasonCode(ge::NotFuseReason::kFusedSliceHasViewOp));
      return false;
