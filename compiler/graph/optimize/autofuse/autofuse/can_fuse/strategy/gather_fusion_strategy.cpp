...
bool GatherFusionStrategy::CanFuse(const NodePtr &node1, const NodePtr &node2) {
  ...
  if (attr1->HasFuseType(loop::FuseType::kGather) && attr2->HasFuseType(loop::FuseType::kConcat)){
    if (BackendUtils::IsVertical(node1, node2) && backend_spec->gather_spec.enable_gather_concat_fusion) {
      return true;
    }
    GELOGI(
        "node1 %s(%s) and node2 %s(%s) can not fuse, the reason is [%s][Gather can not fuse with Concat]",
        node1->GetNamePtr(), node1->GetType().c_str(), node2->GetNamePtr(), node2->GetType().c_str(),
  ...
  if (attr1->HasFuseType(loop::FuseType::kGather) && (attr2->GetAllFuseType() == supported_type)) {
    if (BackendUtils::IsVertical(node1, node2)) {
      GELOGI("only node1 %s(Gather) --> node2 %s(Pointwise) can fuse.", node1->GetNamePtr(), node2->GetNamePtr());
      if (!CheckGatherFuseAxis(node1,node2)) {
        return false;
      } else if (!BackendUtils::IsNodeAllInputsAreSimplestLoad(node2)) {
        GELOGI(
            "node1 %s(%s) and node2 %s(%s) can not fuse, the reason is [%s][In gather fusion occasion, node1 is "
            "Gather, node2 is Pointwise and node2 input has view op]", node1->GetNamePtr(), node1->GetType().c_str(), 
            node2->GetNamePtr(), node2->GetType().c_str(), ge::NotFuseReasonCode(ge::NotFuseReason::kGatherNextNodeIsPointWiseAndInputHasViewOp));
        return false;
      }
      return true;
    }
...
// gather 向下垂直融合，设置优先级
uint32_t GatherFusionStrategy::GetFusionPairPriority(const NodePtr &node1, const NodePtr &node2) {
...
bool GatherFusionStrategy::CheckGatherReduceFuse(const NodePtr &node1, const NodePtr &node2, const bool enableGather) {
  if (!enableGather) {
    GELOGI(
