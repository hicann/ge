...
inline Status RefreshReshapeAxisGroupByInsertedAxisIds(const std::vector<int64_t> &inserted_axis_ids,
                                                       const std::vector<int64_t> &graph_axis_after_insert,
                                                       optimize::autoschedule::AxisGroup &axis_group) {
  if (inserted_axis_ids.empty() || axis_group.IsEmpty()) {
    return SUCCESS;
  }

  for (const auto axis_id : inserted_axis_ids) {
    if (std::find(axis_group.y_group.begin(), axis_group.y_group.end(), axis_id) == axis_group.y_group.end()) {
      axis_group.y_group.push_back(axis_id);
    }
  }
  axis_group.axes_order.clear();
  axis_group.axes_order.reserve(graph_axis_after_insert.size());
  for (const auto axis_id : graph_axis_after_insert) {
    axis_group.axes_order.push_back(static_cast<size_t>(axis_id));
  }
  SortUniqueVector(axis_group.y_group);
  return SUCCESS;
}

inline std::vector<int64_t> CollectInsertedAxisIds(const std::vector<int64_t> &axis_before_insert,
                                                   const std::vector<int64_t> &axis_after_insert) {
  std::vector<int64_t> inserted_axis_ids;
  for (const auto axis_id : axis_after_insert) {
    if (std::find(axis_before_insert.begin(), axis_before_insert.end(), axis_id) == axis_before_insert.end()) {
      inserted_axis_ids.push_back(axis_id);
    }
  }
  return inserted_axis_ids;
}
...
inline void CompleteTensorAttrsByGraphOrderPreserveStrides(const NodePtr &node, const size_t output_idx,
  ...
  for (size_t i = 0U; i < graph_axis_order.size(); ++i) {
    if (AppendCompletedTensorAxisByGraphOrder(target_to_tensor_index[i], kInvalidIndex, graph_axis_order, tensor_attr,
      ...
      GELOGD("node %s(%s) output %zu complete preserve tensor attrs with reshape axis id %ld at graph axis idx %zu.",
             node->GetNamePtr(), node->GetType().c_str(), output_idx, graph_axis_order[i], i);
    }
  }
  ...
  if (!inserted_axis_ids.empty()) {
    GELOGD("node %s(%s) output %zu complete preserve tensor attrs with reshape axes %s, axis from %s to %s.",
           node->GetNamePtr(), node->GetType().c_str(), output_idx,
...
inline Status InsertNoOpAxisByGraphOrder(const std::vector<int64_t> &graph_axis_order, const int64_t axis_id,
                                         std::vector<int64_t> &axis, std::vector<Expression> *repeats = nullptr,
                                         std::vector<Expression> *strides = nullptr) {
  if (std::find(axis.begin(), axis.end(), axis_id) != axis.end()) {
    return SUCCESS;
  }
  const auto graph_axis_it = std::find(graph_axis_order.begin(), graph_axis_order.end(), axis_id);
  GE_ASSERT_TRUE(graph_axis_it != graph_axis_order.end());
  const auto insert_it = std::find_if(axis.begin(), axis.end(), [&graph_axis_order, graph_axis_it](const auto axis_id) {
    const auto current_graph_axis_it = std::find(graph_axis_order.begin(), graph_axis_order.end(), axis_id);
    return (current_graph_axis_it != graph_axis_order.end()) && (current_graph_axis_it > graph_axis_it);
  });
...
inline Status CompleteTensorAttrsByInsertedAxisIdsPreserveOrder(const NodePtr &node, const size_t output_idx,
  ...
  for (const auto axis_id : inserted_axis_ids) {
    GE_ASSERT_SUCCESS(InsertNoOpAxisByGraphOrder(graph_axis_order, axis_id, tensor_attr.axis, &tensor_attr.repeats,
                                                 &tensor_attr.strides));
  }
...
inline Status UpdateTensorAttrsPreserveStrides(const NodePtr &node, const std::vector<int64_t> &axis,
  ...
  for (size_t i = 0U; i < node->GetAllOutDataAnchorsSize(); ++i) {
    ...
    GELOGD("after preserve attrs: node %s(%s), axis:%s, repeats:%s stride:%s.", node->GetNamePtr(),
           node->GetType().c_str(), AutofuseUtils::VectorToStr(tensor_attr->axis).c_str(),
           AutofuseUtils::VectorToStr(tensor_attr->repeats).c_str(),
           AutofuseUtils::VectorToStr(tensor_attr->strides).c_str());
  }
...
inline Status UpdateTensorAttrsByInsertedAxisIdsPreserveOrder(const NodePtr &node, const std::vector<int64_t> &axis,
  ...
  for (size_t i = 0U; i < node->GetAllOutDataAnchorsSize(); ++i) {
    ...
    GE_ASSERT_SUCCESS(
        CompleteTensorAttrsByInsertedAxisIdsPreserveOrder(node, i, axis, inserted_axis_ids, *tensor_attr));
    GELOGD("after preserve attrs: node %s(%s), axis:%s, repeats:%s stride:%s.", node->GetNamePtr(),
           node->GetType().c_str(), AutofuseUtils::VectorToStr(tensor_attr->axis).c_str(),
           AutofuseUtils::VectorToStr(tensor_attr->repeats).c_str(),
           AutofuseUtils::VectorToStr(tensor_attr->strides).c_str());
...
inline Status CompleteGatherInputAttrsPreserveStrides(const AscGraph &asc_graph, const NodePtr &gather_node) {
  ...
  GELOGI("node %s(%s) complete gather params axis from %s to %s in graph %s.", params_data_node->GetNamePtr(),
  ...
  GELOGI("node %s(%s) complete gather indices axis from %s to %s in graph %s.", indices_data_node->GetNamePtr(),
...
inline Status CompleteNodeAttrsOnAscGraphPreserveStrides(AscGraph &asc_graph, const NodePtr &asc_node) {
  ...
  for (const auto &node : AscGraphUtils::GetComputeGraph(asc_graph)->GetAllNodes()) {
    if (IsTorchDataType(node)) {
      GELOGI("torch node %s(%s) not complete node attr.", node->GetNamePtr(), node->GetType().c_str());
      continue;
    }
    if (IsCubeRelatedAscNode(node)) {
      GELOGI("cube related node %s(%s) not complete node attr.", node->GetNamePtr(), node->GetType().c_str());
      continue;
    }
    ...
    GELOGI("node %s(%s) before complete sched axis %s to %s in graph %s, preserve tensor strides.", node->GetNamePtr(),
    ...
    GELOGI("node %s(%s) after complete sched axis %s to %s in graph %s, preserve tensor strides.", node->GetNamePtr(),
           node->GetType().c_str(), AutofuseUtils::VectorToStr(node_attr->sched.axis).c_str(),
           AutofuseUtils::VectorToStr(graph_attr.axis).c_str(), asc_graph.GetName().c_str());
  }
...
inline Status CompleteNodeAttrsByInsertedAxisIdsPreserveOrder(AscGraph &asc_graph, const NodePtr &asc_node,
  ...
  for (const auto &node : AscGraphUtils::GetComputeGraph(asc_graph)->GetAllNodes()) {
    ...
    if (node_attr->sched.axis.empty()) {
      ...
      for (const auto axis_id : inserted_axis_ids) {
        GE_ASSERT_SUCCESS(InsertNoOpAxisByGraphOrder(graph_attr.axis, axis_id, node_attr->sched.axis));
      }
    }
    GELOGI("node %s(%s) complete sched axis from %s to %s in graph %s, preserve node axis order.", node->GetNamePtr(),
           node->GetType().c_str(), AutofuseUtils::VectorToStr(old_sched_axis).c_str(),
...
inline Status CompleteNoOpReshapeAxesByInsertIndexesKeepOriginalAxisIds(AscGraph &asc_graph,
                                                                        const std::vector<size_t> &insert_indexes,
                                                                        AscGraphAttr *graph_attr,
                                                                        std::vector<int64_t> &inserted_axis_ids) {
  GE_ASSERT_NOTNULL(graph_attr);
  ...
  std::sort(sorted_insert_indexes.begin(), sorted_insert_indexes.end());
  for (const auto insert_index : sorted_insert_indexes) {
    const auto insert_axis_id = GetNextNoOpReshapeAxisId(graph_attr->axis);
    const auto graph_insert_it =
        graph_attr->axis.begin() + static_cast<ptrdiff_t>(std::min(insert_index, graph_attr->axis.size()));
    graph_attr->axis.insert(graph_insert_it, MakeNoOpReshapeAxis(insert_axis_id, kSymbolOne));
    inserted_axis_ids.push_back(insert_axis_id);
    GELOGD(
        "graph %s complete no-op reshape axis id %ld repeat %s at insert index %zu by relation without shifting "
        "existing axis ids.",
        asc_graph.GetName().c_str(), insert_axis_id, kSymbolOne.Str().get(), insert_index);
  }
  return SUCCESS;
...
inline Status PadLeadingUnitAxisByInsertIndexesAndCompleteAttrs(AscGraph &asc_graph, const NodePtr &asc_node,
  ...
  std::vector<int64_t> inserted_axis_ids;
  GELOGD("node %s(%s) graph %s start to pad leading unit axes by relation insert indexes %s.", asc_node->GetNamePtr(),
         asc_node->GetType().c_str(), asc_graph.GetName().c_str(), AutofuseUtils::VectorToStr(insert_indexes).c_str());
...
inline Status PadLeadingUnitAxisByInsertIndexesAndCompleteAttrsKeepOriginalAxisIds(
    AscGraph &asc_graph, const NodePtr &asc_node, const std::vector<size_t> &insert_indexes) {
  if (insert_indexes.empty()) {
    return SUCCESS;
  }
  constexpr const char *kPadLeadingUnitAxisProcName = "pad_leading_unit_axis_keep_original_axis_ids";
  const auto fused_graph = AscGraphUtils::GetComputeGraph(asc_graph);
  GE_ASSERT_NOTNULL(fused_graph);
  GE_ASSERT_SUCCESS(BackendUtils::AddInputOutputNodesForAscGraph(fused_graph));
  GE_ASSERT_SUCCESS(CacheGraphBeforePostProcess(asc_node, kPadLeadingUnitAxisProcName, fused_graph));
  ...
  std::vector<int64_t> inserted_axis_ids;
  GELOGD(
      "node %s(%s) graph %s start to pad leading unit axes without shifting existing axis ids by relation insert "
      "indexes %s.",
      asc_node->GetNamePtr(), asc_node->GetType().c_str(), asc_graph.GetName().c_str(),
      AutofuseUtils::VectorToStr(insert_indexes).c_str());
  GE_ASSERT_SUCCESS(CompleteNoOpReshapeAxesByInsertIndexesKeepOriginalAxisIds(asc_graph, insert_indexes, graph_attr,
                                                                              inserted_axis_ids));
  GE_ASSERT_SUCCESS(CompleteNodeAttrsByInsertedAxisIdsPreserveOrder(asc_graph, asc_node, inserted_axis_ids));
  GE_ASSERT_SUCCESS(RefreshReshapeAxisGroupByInsertedAxisIds(inserted_axis_ids, CollectAxisIds(graph_attr->axis),
                                                             GetInterAttrs(autofuse_attr).axis_group));
  GE_ASSERT_SUCCESS(DumpPadLeadingUnitAxisResult(asc_graph, asc_node, kPadLeadingUnitAxisProcName, fused_graph));
  return SUCCESS;
}
...
inline Status CollectInsertIndexesByTargetRepeats(const std::vector<Expression> &current_repeats,
  ...
  for (size_t gap_idx = 0U; gap_idx < current_gap_counts.size(); ++gap_idx) {
    ...
    current_gap_start += current_gap_counts[gap_idx] + extra_unit_count;
...
inline Status PadNodeLeadingUnitAxisByInsertIndexesKeepOriginalAxisIds(const NodePtr &node,
                                                                       const std::vector<size_t> &insert_indexes) {
  if (insert_indexes.empty() || !BackendUtils::IsBackendFuseNode(node)) {
    return SUCCESS;
  }
  ...
  if (node->GetType() == kAscBackendType) {
    GE_ASSERT_NOTNULL(attr->GetAscGraph());
    GE_ASSERT_SUCCESS(PadLeadingUnitAxisByInsertIndexesAndCompleteAttrsKeepOriginalAxisIds(*(attr->GetAscGraph()), node,
                                                                                           insert_indexes));
    return SUCCESS;
  }
  if (node->GetType() != kFusedAscBackendType) {
  ...
  for (const auto &inner_node : attr->GetFuseComputeGraph()->GetAllNodes()) {
    ...
    GE_ASSERT_SUCCESS(PadNodeLeadingUnitAxisByInsertIndexesKeepOriginalAxisIds(inner_node, insert_indexes));
  }
  GE_ASSERT_SUCCESS(RefreshReshapeAxisGroupByInsertedAxisIds(
      CollectInsertedAxisIds(axis_before_insert, CollectAxisIds(fused_graph_attr->axis)),
      CollectAxisIds(fused_graph_attr->axis), GetInterAttrs(attr).axis_group));
  return SUCCESS;
}
...
inline bool FindDirectionalNonUnitRepeatByAxisId(const NodePtr &node, const int64_t axis_id, const bool search_forward,
  ...
  GetConnectedPeerOutNodesForReshapeSearch(node, peer_nodes);
  for (const auto &peer_node : peer_nodes) {
    if (FindDirectionalNonUnitRepeatByAxisId(peer_node, axis_id, search_forward, visited, repeat)) {
      return true;
    }
...
inline Status CollectVerticalRelationInsertIndexes(const NodePtr &node1, const NodePtr &node2,
  ...
  for (const auto &relation : fuse_info.GetNode1ToNode2LinkMap()) {
    ...
    GELOGD(
        ...
        node1->GetNamePtr(), relation.first, AutofuseUtils::VectorToStr(node1_output_repeats).c_str(),
        AutofuseUtils::VectorToStr(node1_insert_indexes).c_str(), node2->GetNamePtr(), relation.second,
...
inline Status CollectCommonInputRelationInsertIndexes(const NodePtr &node1, const NodePtr &node2,
  ...
  for (const auto &relation : fuse_info.GetSameInputMap()) {
    ...
    GELOGD(
        ...
        node1->GetNamePtr(), relation.first, AutofuseUtils::VectorToStr(node1_input_repeats).c_str(),
        AutofuseUtils::VectorToStr(node1_insert_indexes).c_str(), node2->GetNamePtr(), relation.second,
        AutofuseUtils::VectorToStr(node2_input_repeats).c_str(),
        AutofuseUtils::VectorToStr(node2_insert_indexes).c_str(), AutofuseUtils::VectorToStr(target_repeats).c_str());
  }
...
inline Status CompletePairReshapeAxes(const NodePtr &node1, const NodePtr &node2, const NodeFuseInfo &fuse_info) {
  ...
  GE_ASSERT_SUCCESS(PadNodeLeadingUnitAxisByInsertIndexesKeepOriginalAxisIds(node1, node1_insert_indexes));
  GE_ASSERT_SUCCESS(PadNodeLeadingUnitAxisByInsertIndexesKeepOriginalAxisIds(node2, node2_insert_indexes));
...
inline Status FlushReshapeAxisByRepeats(const NodePtr &node, const NodePtr &asc_node,
  ...
  GELOGD("Flush %s for node %s, asc_node %s, before: %s, after: %s", info_name, node->GetNamePtr(),
         asc_node->GetNamePtr(), AutofuseUtils::VectorToStr(reshape_axis).c_str(),
