...
Status WhereRegApiCall::Generate(const TPipe &tpipe, const std::vector<ascir::AxisId> &current_axis,
  ...
  if (param.outer_repeats.size() == 0) {  // 没有loop，调用一维where接口
    ...
    ss << x1 << "[" << tpipe.tiler.TensorVectorizedOffset(current_axis, x1) << "], ";       // mask
    if (x2_is_scalar_scene) {
      ss << x2_scalar << ", ";  // 输入1
    } else {
      ss << x2 << "[" << tpipe.tiler.TensorVectorizedOffset(current_axis, x2) << "], ";  // 输入1
    }
    if (x3_is_scalar_scene) {
      ss << x3_scalar << ", ";  // 输入2
    } else {
      ss << x3 << "[" << tpipe.tiler.TensorVectorizedOffset(current_axis, x3) << "], ";  // 输入2
    }
    ...
    if (x2_is_scalar_scene && x3_is_scalar_scene) {
      ...
      scalar_local_blk_tensor_name_x2 = x2.need_alloc_local_blk_tensor_from_tbuf ? scalar_local_blk_tensor_name_x2 +
                                        "_1" : scalar_local_blk_tensor_name_x2;
      scalar_local_blk_tensor_name_x3 = x3.need_alloc_local_blk_tensor_from_tbuf ? scalar_local_blk_tensor_name_x3 +
                                        "_1" : scalar_local_blk_tensor_name_x3;
     // 计算 output stride
      ...
      ss1 << this->api_name_ << "<true, true, 2>(" << y << "[" << output_inner_offset << "], " << x1 << "["
          << input0_inner_offset << "], "
