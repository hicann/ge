...
bool CalculateDmaParams(const TPipe &tpipe, const Tensor &gm_tensor, const Tensor &ub_tensor, DataCopyParams &param,
  ...
  for (vec_axis_pos = ub_tensor.vectorized_axis.size(); vec_axis_pos-- > 0UL;) {
    ...
    bool ignore_zero_axis = has_non_zero_axis || vec_axis_pos == 0UL ||
                            ge::SymbolicUtils::StaticCheckEq(ub_tensor.vectorized_strides[vec_axis_pos - 1],
                                                             ge::ops::One) == ge::TriBool::kTrue ||
                            ge::SymbolicUtils::StaticCheckEq(ub_tensor.vectorized_strides[vec_axis_pos - 1],
                                                             ge::ops::Zero) == ge::TriBool::kTrue;
    if (ge::SymbolicUtils::StaticCheckEq(gm_tensor.axis_strides[axis_pos], ge::ops::Zero) == ge::TriBool::kTrue &&
        ge::SymbolicUtils::StaticCheckEq(ub_tensor.vectorized_strides[vec_axis_pos], ge::ops::Zero) ==
            ge::TriBool::kTrue &&
        ignore_zero_axis) {
      continue;
    }
    ...
    if (ge::SymbolicUtils::StaticCheckEq(cur_axis_stride, gm_tensor.axis_strides[axis_pos]) != ge::TriBool::kTrue ||
        ge::SymbolicUtils::StaticCheckEq(cur_vectorized_axis_stride, ub_tensor.vectorized_strides[vec_axis_pos]) !=
            ge::TriBool::kTrue || merge_info.merge_repeats.empty() ||
