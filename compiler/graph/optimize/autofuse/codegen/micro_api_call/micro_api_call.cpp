...
namespace codegen {
  Status TensorManager::AddTensor(const MicroApiTensor &tensor) {
    auto [ret, is_insert] = this->tensors_.emplace(tensor.id_, tensor);
...
// 生成micro api的调用
  Status MicroApiCall::Generate(const TensorManager &tensor_mng, const TPipe &tpipe, CallParam &param,
                                std::string &result) {
    stringstream ss;
