  ...
  for (size_t i = 0; i < current_axis_ids.size(); i++) {
    ...
    if (axis.type == ascir::Axis::Type::kAxisTypeMerged) {
      ...
      for (size_t j = 0; j < origin_last_axis.size(); j++) {
        match_count = current_ids.count(origin_last_axis[j]) ? match_count + 1 : match_count;
      }
...
Status VfCall::Generate(const TPipe &tpipe, const std::vector<ascir::AxisId> &current_axis,
