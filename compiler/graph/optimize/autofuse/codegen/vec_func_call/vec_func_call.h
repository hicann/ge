...
class VfCall final : public ApiCall {
 public:
  ...
  // 生成vf func的定义
  Status GenerateFuncDefinition(const TPipe &tpipe, const Tiler &tiler, std::stringstream &ss) const;
  ...
  Status ParseSubGraph(const ascir::NodeView &node, const ascir::ImplGraph &graph);
