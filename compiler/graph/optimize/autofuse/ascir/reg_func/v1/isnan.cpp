...
//isnan tmp buf has two part
//1.sign_mask = ONE_BLK_SIZE
//2.half_isnan
//if input_size > MAX_REPEAT_NUM * ONE_REPEAT_BYTE_SIZE
//  half_isnan = MAX_REPEAT_NUM * ONE_REPEAT_BYTE_SIZE
//else
//  half_isnan = input_size
std::vector<std::unique_ptr<ge::TmpBufDesc>> CalcIsnanTmpSize(const ge::AscNode &node)
{
    ...
    Expression min_temp_size = sym::Max(type_size * input_size + ge::Symbol(ONE_BLK_SIZE), ge::Symbol(BASIC_TMP_SIZE));
    min_temp_size = sym::Min(min_temp_size, ge::Symbol(MAX_REPEAT_NUM * ONE_REPEAT_BYTE_SIZE + ONE_BLK_SIZE));
