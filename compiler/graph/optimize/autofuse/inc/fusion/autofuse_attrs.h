...
constexpr float32_t kSplitLowFusionRatioThreshold = 0.2000F;

// Reduce all load状态宏定义
#define REDUCE_ALL_LOAD_INIT    (-1)  // 未初始化
#define REDUCE_ALL_LOAD_NOT_ALL (0)   // 不是所有load都是norm-like
#define REDUCE_ALL_LOAD_ALL     (1)   // 所有load都是norm-like
...
class AutoFuseAttrs : public ge::AttrGroupsBase {
 public:
  ...
  void SetOriginNodes(const std::vector<const ge::Node *> &nodes) { inner_attrs_.origin_nodes = nodes; }
  ...
  [[nodiscard]] const std::vector<const ge::Node *> &GetOriginNodes() const { return inner_attrs_.origin_nodes; }
