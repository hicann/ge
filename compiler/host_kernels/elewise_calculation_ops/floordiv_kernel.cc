...
#include "host_kernels/elewise_calculation_ops/floordiv_kernel.h"
...
Status FloorDivKernel::FloorDivCheck(const OpDescPtr &op_desc_ptr,
  ...
  if (x_tensor->GetTensorDesc().GetShape().GetDimNum() != kFloorDivTensorShapeIsEmpty &&
    ...
    std::vector<int64_t> y_dims = y_tensor->GetTensorDesc().GetShape().GetDims();
...
void FloorDivKernel::ShapeCal(const std::vector<ge::ConstGeTensorPtr> &input, GeTensorPtr output_ptr) const {
...
template <typename T>
bool FloorDivKernel::ZeroCheck(const T &element, DataType data_type) const {
  bool result = false;
...
Status FloorDivKernel::Compute(const OpDescPtr op_desc_ptr, const std::vector<ConstGeTensorPtr> &input,
  ...
  GELOGD("FloorDivKernel success.");
  return SUCCESS;
}
REGISTER_COMPUTE_NODE_KERNEL(FLOORDIV, FloorDivKernel);
}  // namespace ge
