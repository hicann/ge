...
Status ReduceProdKernel::AxisCal(const std::vector<ge::ConstGeTensorPtr> &input) {
  ...
  std::vector<int64_t> data_dims = data_tensor->GetTensorDesc().GetShape().GetDims();
...
Status ReduceProdKernel::DataCal(const std::vector<ge::ConstGeTensorPtr> &input, ge::GeTensorPtr output_ptr) const {
  ConstGeTensorPtr data_tensor = input.at(kReduceProdDataIndex);
...
void ReduceProdKernel::ShapeCal(const ge::OpDescPtr &op_desc_ptr, const std::vector<ge::ConstGeTensorPtr> &input,
  ...
  int32_t axis = *(const_cast<int32_t *>(PtrToPtr<const uint8_t, const int32_t>(axis_data)));
  bool keep_dims = false;
  if (!AttrUtils::GetBool(op_desc_ptr, "keep_dims", keep_dims)) {
    GELOGI("Get the attr keep_dims of %s, no value found.", op_desc_ptr->GetName().c_str());
  }
...
Status ReduceProdKernel::Compute(const ge::OpDescPtr op_desc_ptr, const std::vector<ge::ConstGeTensorPtr> &input,
  ...
  std::vector<int64_t> data_dims = output_ptr->GetTensorDesc().GetShape().GetDims();
...
REGISTER_COMPUTE_NODE_KERNEL(REDUCEPROD, ReduceProdKernel);
}  // namespace ge
