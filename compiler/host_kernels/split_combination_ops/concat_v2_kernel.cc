...
const int32_t kSupportEmptyTensorRank = 1;
...
Status ConcatV2Kernel::Compute(const ge::OpDescPtr op_desc_ptr, const std::vector<ge::ConstGeTensorPtr> &input,
                               std::vector<ge::GeTensorPtr> &v_output) {
  GELOGD("ConcatV2Kernel in");
...
Status ConcatV2Kernel::ConcatV2PreCompute(const std::vector<ConstGeTensorPtr> &input,
  ...
  for (size_t i = 1; i < input_size; i++) {
    if (data_type != input.at(i)->GetTensorDesc().GetDataType()) {
      GELOGI("Data type of N inputs for ConcatV2 not the same, the input index %zu.", i);
      return NOT_CHANGED;
    }
  }
...
REGISTER_COMPUTE_NODE_KERNEL(CONCATV2, ConcatV2Kernel);
}  // namespace ge
