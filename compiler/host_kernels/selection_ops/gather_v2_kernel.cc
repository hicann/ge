...
template <typename T>
Status GatherV2Kernel::GenData(const int64_t data_num, ConstGeTensorPtr tensor_x,
                               int64_t axis, GeTensorPtr output) {
  if (data_num <= 0) {
    return PARAM_INVALID;
  }
  if (CheckInt64MulOverflow(data_num, sizeof(T)) != SUCCESS) {
...
Status GatherV2Kernel::CalcStride(std::vector<int64_t> &stride, std::vector<int64_t> dims) const {
  if ((stride.size() != dims.size()) || (dims.size() == 0)) {
    return PARAM_INVALID;
  }
  int32_t i = static_cast<int32_t>(dims.size() - kGatherV2DimOne);
  stride[static_cast<size_t>(i)] = static_cast<int64_t>(kGatherV2DimOne);
  i--;
  while (i >= 0) {
    size_t index = static_cast<size_t>(i) + kGatherV2DimOne;
    if (CheckInt64MulOverflow(stride[index], dims[index]) != SUCCESS) {
...
Status GatherV2Kernel::SaveIndicesByDataType(ConstGeTensorPtr indices_tensor_ptr, const GeShape &x_shape,
...
Status GatherV2Kernel::Compute(const OpDescPtr op_desc_ptr, const std::vector<ConstGeTensorPtr> &input,
...
REGISTER_COMPUTE_NODE_KERNEL(GATHERV2, GatherV2Kernel);
}  // namespace ge
